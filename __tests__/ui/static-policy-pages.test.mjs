...
import { STATIC_POLICY_ROUTES, policyStaticPath } from '../../scripts/lib/static-policy-routes.mjs';
...
for (const route of STATIC_POLICY_ROUTES) test(`${route.canonical} is a complete policy document without hydration`, () => {
  ...
  walk(parse(html), node => {
    ...
    if (node.tagName === 'link' && attrs.rel === 'canonical') assert.equal(new URL(attrs.href).pathname.replace(/\/$/, ''), route.canonical);
  });
