...
/**
 * 호출부의 `{ skipKeys: [...] }` 를 읽는다. 런타임(`lib/teaHouseCopy.ts`)이 그 필드를 사전으로
 * 덮지 않으므로 가드도 같은 필드를 요구하지 않아야 한다 — 안 맞추면 절대 안 쓰이는 키를
 * 12개 로케일에 채우라고 요구하게 된다.
 *
...
test("운명의 찻집 컴포넌트의 한국어가 12개 로케일 사전에 전부 있다", () => {
  ...
  for (const file of files) {
    ...
    const calls = [
      ...source.matchAll(/useTeaHouseCopy\(\s*"([^"]+)"\s*,\s*([A-Za-z_$][\w$]*)\s*(?:,\s*(\{[^})]*\})\s*)?\)/g),
    ];
    ...
    for (const [, scope, koName, optionsText] of calls) {
      ...
      const allPaths = collectKoPaths(source, koName, file) ?? collectImportedKoPaths(source, file, koName);
      assert.notEqual(
        allPaths,
        null,
