...
test('actual astrology-ai start-screen pending-check effect resumes on every foreground event, respects guards, and cleans up', async () => {
  const effect = extractEffect('app/astrology-ai/AstrologyAiClient.tsx', '/api/astrology-ai/pending');
  ...
  const context = {
    ...
    lockRef,
    authFetch: async () => ({ ok: true, json: async () => ({ sessionId: 'abandoned-astrology-session' }) }),
    resultPath: (id) => `/astrology-ai/result?id=${id}`,
    setResultUrl: () => {},
    openResultPage: (url) => opens.push(url),
  };
  ...
  assert.equal(opens.length, 5, 'an in-flight lock must not overlap');
...
test('actual astrology-ai result-screen resume effect bumps resumeEpoch on every foreground event, respects the hidden guard, and cleans up', () => {
  const effect = extractEffect('app/astrology-ai/result/AstrologyAiResultClient.tsx', 'setResumeEpoch(value => value + 1)');
  const handlers = new Map(), surface = name => ({
    addEventListener: (event, run) => handlers.set(name + event, run),
    removeEventListener: event => handlers.delete(name + event),
  });
