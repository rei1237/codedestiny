...
const source = fs.readFileSync('app/ziwei-ai/ZiweiAiClient.tsx', 'utf8');
...
test('자미두수 실제 화면 재개 함수는 정상 묶음을 표시하며 같은 결과를 완성한다', async () => {
  const f = fixture(); const result = await f.ctx.pollZiweiResult('saved', f.isCurrent, f.onPartial);
...
test('숨김 상태에서는 부분 본문을 유지하고 다음 묶음을 시작하지 않는다', async () => {
  ...
  const result = await f.ctx.pollZiweiResult('saved', f.isCurrent, f.onPartial);
