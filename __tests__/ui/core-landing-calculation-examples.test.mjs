...
test('명반 FAQ 는 화면과 JSON-LD 를 한 배열에서 만들고, 본문 수정일은 검수 노트와 WebPage 에 함께 싣는다',()=>{
  ...
  // 도구 폼과 같은 높이의 로딩 자리가 있어야 도구 아래 서버 본문이 붙을 때 밀리지 않는다.
  assert.match(readFileSync('app/ziwei/chart/ZiweiChartClientLoader.tsx','utf8'),/loading: \(\) => <div className="min-h-\[100dvh\]"/);
});
