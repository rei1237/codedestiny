...
test("시트가 열려 있으면 '새로 만들기' 가 폼을 시트 안으로 데려온다", async () => {
  ...
  // 홈 축약 규칙은 자손 선택자라 시트 안에서도 먹는다 — 빌려간 동안에는 떼야 보인다.
  assert.equal(
...
test("시트를 닫으면 폼이 원위치·원상태로 돌아온다", async () => {
  ...
  assert.ok(form.hasAttribute("data-cd-home-secondary"), "홈 축약 대상에서 빠져 상시 노출된다");
});
...
test("시트가 닫혀 있으면 종전대로 홈을 펼치고 스크롤한다", async () => {
  ...
  assert.equal(doc.getElementById("destinyCardForm").parentNode.id, "inputPage");
});
