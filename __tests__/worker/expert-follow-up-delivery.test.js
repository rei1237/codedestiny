...
let deliver, docs, report, revoked, fault, provider;
const owner = '64b7f2a1c3d4e5f601234567';
const at = (obj, key) => key.split('.').reduce((v, k) => v?.[k], obj);
const matches = (row, filter) => Object.entries(filter).every(([key, value]) => value && typeof value === 'object' && '$ne' in value ? at(row, key) !== value.$ne : at(row, key) === value);
...
beforeAll(async () => {
  ...
  ({ deliverExpertFollowUp: deliver } = await import('../../worker/lib/expert-follow-up-delivery.js'));
});
beforeEach(() => {
