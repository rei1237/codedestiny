...
describe("달빛 이용권 상점 — 결제 준비(prepare)", () => {
  ...
  test("클라이언트가 보낸 주문번호가 형식에 맞으면 그대로 채택해야 한다", async () => {
    // 결제창을 prepare 응답보다 먼저 열려면 주문번호를 클라이언트가 알고 있어야 한다.
    const clientMerchantUid = "sub_ms3vjd4c_s1m_78901234_ab12";
    User.findById = jest.fn().mockReturnValue(chain(freeUser()));
    Payment.findOne = jest.fn().mockReturnValue(chain(null));
    Payment.create = jest.fn().mockResolvedValue({ _id: "payment-doc-client-uid" });

    const request = new Request("https://example.com/api/payments/subscription/prepare", {
      method: "POST",
      headers: { "content-type": "application/json" },
      body: JSON.stringify({
        tier: "standard",
        ...
        durationMonths: 1,
        durationDays: 30,
        amount: WEB_PASS_PRICES.standard,
        currency: "KRW",
        productType: "membership_pass",
        paymentMethod: "card_general",
        merchantUid: clientMerchantUid,
      }),
    });

    const { status, payload } = await readResponse(
      await testUtils.handleSubscriptionPrepare(request, ENV, AUTH),
    );

    expect(status).toBe(201);
    expect(payload.order.merchantUid).toBe(clientMerchantUid);
    expect(Payment.create.mock.calls[0][0].merchantUid).toBe(clientMerchantUid);
  });

  test("형식에 어긋난 주문번호는 무시하고 서버가 생성해야 한다", async () => {
    // 접두사·문자셋·길이(이니시스 oid 40자) 중 하나라도 어긋나면 신뢰하지 않는다.
    for (const badUid of ["hack_1", "sub_한글", `sub_${"x".repeat(60)}`, "sub_bad uid"]) {
      User.findById = jest.fn().mockReturnValue(chain(freeUser()));
      Payment.findOne = jest.fn().mockReturnValue(chain(null));
      Payment.create = jest.fn().mockResolvedValue({ _id: "payment-doc-bad-uid" });
      ...
      expect(status).toBe(201);
      expect(payload.order.merchantUid).not.toBe(badUid);
      expect(payload.order.merchantUid).toMatch(/^sub_[A-Za-z0-9_-]+$/);
      expect(payload.order.merchantUid.length).toBeLessThanOrEqual(40);
    }
  });
