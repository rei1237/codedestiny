 ...
 * 실패 사유는 PG 조회 실패만, 대기 주문에만 남긴다 — 대조 불일치까지 써서 관리자 검토 표시를 덮던 구현(ca7f4eab0·96533b654)은 되돌렸다.
...
describe.each([["ziwei-ai"], ["ziwei-island-ai"]])("%s verifyPaymentForStart", (route) => {
  ...
  test("🔴 같은 상점의 다른 채널(channel.key) 결제로는 확정하지 않는다", async () => {
    ...
    expect(paymentFindOneAndUpdate).not.toHaveBeenCalled(); // 대조 불일치는 로그만 — 주문에 쓰지 않는다
    expect(current.status).toBe("pending");
    expect(current.failureCode).toBeUndefined();
  });

  test("PG 조회 실패는 대기 주문에 사유를 남긴다", async () => {
    fetchPortOnePayment.mockRejectedValue(new Error("PortOne payment lookup failed"));
    const current = order();

    await expect(verify(route, current)).resolves.toEqual({ ok: false });

    expect(paymentFindByIdAndUpdate).not.toHaveBeenCalled();
    expect(current).toMatchObject({ status: "pending", failureCode: "portone_fetch_failed", failureStage: `${route.replace(/-/g, "_")}_portone_fetch` });
  });

  test.each([["cancelled"], ["refunded"], ["failed"]])("🔴 %s 주문에 start 가 다시 와 PG 조회가 실패해도 관리자 검토 표시를 덮지 않는다", async (status) => {
    fetchPortOnePayment.mockRejectedValue(new Error("PortOne payment lookup failed"));
  ...
  test("🔴 PG 에서 부분취소된 대기 주문의 검토 표시(partial_cancel_admin_review)를 덮지 않는다", async () => {
    // PortOne PARTIAL_CANCELLED 는 정규화에서 cancelled 가 된다(worker/lib/portone.js). 부분취소 웹훅은 상태를 안 바꾸고 표시만 남긴다.
    fetchPortOnePayment.mockResolvedValue({ ...paidPg(), status: "cancelled" });
    const current = order({ failureCode: "partial_cancel_admin_review", failureStage: "webhook_cancel_admin_review" });

    await expect(verify(route, current)).resolves.toEqual({ ok: false });

    expect(current).toMatchObject({ status: "pending", failureCode: "partial_cancel_admin_review", failureStage: "webhook_cancel_admin_review" });
  });

  test("🔴 구 경로가 닫은 fulfilled 주문(채널 대조 흔적 없음)은 시작마다 재조회하고, 상태·영수증은 덮지 않는다", async () => {
