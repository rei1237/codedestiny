...
beforeAll(async () => {
  ...
  jest.unstable_mockModule("../../worker/lib/models.js", () => ({
    ...models,
    Payment: { findOne: paymentFindOne, findByIdAndUpdate: paymentFindByIdAndUpdate },
  }));
...
describe.each([["ziwei-ai"], ["ziwei-island-ai"]])("%s verifyPaymentForStart", (route) => {
  ...
  test("🔴 같은 상점의 다른 채널(channel.key) 결제로는 확정하지 않는다", async () => {
    ...
    await expect(verify(route, order())).resolves.toEqual({ ok: false });

    expect(writes().some((set) => set.status === "success")).toBe(false);
    expect(writes()).toEqual([expect.objectContaining({ failureCode: "channel_mismatch" })]);
  });
