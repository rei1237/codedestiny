...
- 메인 UX 구조가 실제로 바뀐 경우에만 전체 로케일 동기화를 수행한다. 단순 문구/미세 스타일 수정은 변경 범위 로케일만 반영한다.
- 전체 동기화가 필요한 경우 수동 대량 편집 대신 검증 가능한 자동 스크립트(또는 단일 패치 전략)로 처리해 누락/오염을 줄인다.
- 반영 후 영향을 받은 로케일에서 핵심 마커(예: 루트 id, data-action, 모달 루트) 검색으로 누락 여부를 검증한다.
- 전체 동기화를 수행한 작업에서만 `npm run verify:locale-main-sync`를 실행해 프리미엄 메인 핵심 마커(액션, 이미지, 클릭 스루 CSS) 누락 여부를 자동 검증한다.
...
실행 원칙:
- 텍스트 파일 수정은 `apply_patch`를 기본으로 사용한다.
- 여러 파일에 동일 치환이 필요할 때도 무차별 일괄 치환보다, 대상 파일 목록을 먼저 확정한 뒤 파일별 검증 가능한 방식으로 적용한다.
...
- Persona & Goal:
	...
	- 커뮤니케이션은 친절하고 간결하게 유지한다.
- Cognitive Workflow:
	- 코드 작성 전 내부적으로 다음을 점검한다: Intent Analysis, Context Check, Architecture First, Anticipatory Debugging.
...
## Rule 15
작업 범위를 잠그고(Scoped Change), 요청과 무관한 탐색/수정/리포맷을 금지한다.

적용 범위:
- 모든 코드/문서 수정, 리팩터링, 버그 수정 작업

실행 원칙:
- 요청된 파일/기능 경계 밖 수정은 원칙적으로 금지한다.
- 관련 파일이 많아질 경우 먼저 "필수 변경 파일"과 "선택 변경 파일"을 분리하고, 필수 변경만 우선 완료한다.
- 전역 포맷팅/대량 치환은 사용자가 명시 요청한 경우에만 수행한다.
- 한 작업에서 3개 이상 파일을 수정하면 각 파일의 필요성을 검증하고, 불필요 변경은 즉시 제외한다.

## Rule 16
응답과 검증은 토큰(코인) 효율을 우선하며, 중복 설명과 과도한 로그 출력을 금지한다.

적용 범위:
- 진행 업데이트, 최종 보고, 디버깅 로그 공유, 검증 커맨드 실행 전 과정

실행 원칙:
- 진행 보고는 "무엇을 했는지/다음에 무엇을 할지"만 짧게 전달하고, 반복 설명을 피한다.
- 동일 맥락의 장문 재설명, 중복 파일 요약, 불필요한 이론 설명을 금지한다.
- 명령 출력은 핵심 오류/핵심 성공 신호만 요약하고 원문 덤프를 최소화한다.
- 검증은 단계적으로 수행한다: 경량 검증 통과 시 중량 빌드/전체 테스트는 필요할 때만 실행한다.

## Rule 17
정확도 우선 개발을 위해 "가설-수정-검증" 1회 루프 완결을 기본으로 한다.

적용 범위:
- 버그 수정, 성능 문제, 회귀 대응, 신규 기능 구현

실행 원칙:
- 수정 전 실패 조건(재현 조건) 1개 이상을 명시하고, 수정 후 동일 조건으로 즉시 확인한다.
- 검증 불가 항목은 완료로 보고하지 않고, 미검증 사유와 대체 검증 방법을 함께 남긴다.
- 회귀 위험이 높은 파일(라우팅, 런타임 로더, 결제/코인 로직)은 최소 1개의 보호 검증(타입체크, 빌드, 단위 점검 중 택1)을 강제한다.
