...
## Rule 14
정적 HTML 이동 액션(`/*.html`)을 추가/수정할 때는 배포 경로에 실제 파일이 존재하도록 동기화하고, 커밋 전에 URL 존재를 검증한다.

적용 범위:
- `index.html`, `public/index.html`의 `data-action`/직접 라우팅에서 `/*.html`로 이동하는 모든 링크
- 루트 정적 HTML 파일과 `public/*.html` 배포 파일

실행 원칙:
- 신규/변경된 정적 페이지(`myungwun_final.html` 등)는 반드시 `public/<same-name>.html`에도 동일 파일로 반영한다.
- 라우팅 URL을 추가한 커밋에는 대상 파일 존재 확인(`Test-Path public/<name>.html`)을 포함한다.
- 404가 확인된 정적 라우팅은 URL 수정보다 파일 배포 경로 누락 여부를 먼저 점검한다.
- 루트 파일만 존재하는 상태로는 배포하지 않는다.
