...
const HANDOFF_ITEMS = [
  ...
  "②이미 끝난 것(PR 번호·머지 여부만 — 🔴 3줄 이내. 과정·SHA·기각된 시도는 쓰지 마라, git log 가 정본)",
  "③남은 작업의 정확한 대상과 개수",
...
export function messageFor(tokens) {
  ...
  if (tokens < HANDOFF) {
    return `⚠️ 컨텍스트 ${k}k — 이후 모든 요청이 이 크기를 다시 지불한다. 새 작업이면 지금 /clear 가 싸다(재시작 고정비 64k).`;
  }
  ...
  if (tokens < HARD) {
    return [
      ...
      "3. 템플릿: `docs/handoff/_TEMPLATE.md` (2,000자 목표) · 완성 예시: `docs/handoff/detail-sheet-copy-rewrite.md`",
  ...
  return [
    ...
    "2. 템플릿: `docs/handoff/_TEMPLATE.md` · 완성 예시: `docs/handoff/detail-sheet-copy-rewrite.md`",
