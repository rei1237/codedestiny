...
상품명은 가격을 포함해 명확히 쓴다(예: `운세 콘텐츠 열람 12,900원`). 티어 SKU라 결제 시트에 기능명이 아닌 일반명이 뜨므로, 이름만으로 무엇을 사는지 알 수 있어야 한다.
...
|---|---|---|---|---|---|
| `cd_pass_standard_30d` | 13,000 | 9,900 | +30.3% | 3,900원 이하 | 3,000원 이하 |
| `cd_pass_premium_30d` | 36,000 | 29,900 | +20.1% | 6,000원 이하 | 5,000원 이하 |
| `cd_pass_vvip_30d` | 75,900 | 59,000 | +28.6% | 12,900원 이하 | 10,000원 이하 |
| `cd_pass_family_30d` | 370,000 | 300,000 | +23.0% | 전체 | 전체 |

> `family`는 고액 상품이라 심사에서 환불 정책 고지를 엄격히 본다.

### SKU 없는 것

- **`fortune-fish-gacha`(웹 ₩500)** — 인상해도 ₩625라 Play KRW 최저 판매가를 밑돌 수 있어 **앱에서만 무료**로 전환한다(`APP_FREE_MAX_COIN_PRICE = 5`). 웹은 ₩500 유료 유지.
- **월정석** — 구매 불가 재화.
