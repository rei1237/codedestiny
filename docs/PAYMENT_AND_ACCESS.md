...
- 두 기능은 표준 회당 결제(B유형)로 옮겼다. 가격 정본은 `worker/lib/paid-feature-registry.js`.
  ...
  - `fusion-fortune-consultation` — 500코인(50,000원). 선착순 하루 100자리와 별개다.
- 3,000원 상담은 기존 `PASS_LIMITS` 적용 가격 범위를 따르고, 50,000원 초융합은 `direct_or_family` 범위로 Family 이용권 또는 단건 결제만 허용한다. Standard/Premium/VVIP와 월정석은 제외한다. Family는 건당 상한이 없고 30일 누적 한도(`MONTHLY_PASS_LIMITS.family` = 5,000코인 = 500,000원)만 적용되며 **횟수 제한은 없다**.
