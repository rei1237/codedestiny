...
긴 Claude Code/Codex 작업은 한 세션에 모든 맥락을 쌓지 않는다. 작업 상태는 `docs/handoff/<주제 이름>.md`에 남기고, 새 세션은 그 문서의 `next`와 “다음 세션에서 바로 할 일”부터 시작한다.
...
- 직전 `docs/handoff/*` 문서에서 `status: active|blocked`인 현재 작업을 확인한다.
...
- `docs/handoff/<주제 이름>.md`를 업데이트한다. 완료 회고보다 현재 상태와 다음 행동을 우선한다.
- 다음 세션 시작 프롬프트에 실제 handoff 경로와 첫 TODO를 넣는다.
- production deploy, secret 변경, 실제 결제·환불·정산, 과금 LLM 호출은 사용자 승인 항목으로 분리한다.

## PR과 머지 운영

- PR에는 변경 범위와 검증 결과를 남긴다. 사용자가 별도로 요청하지 않으면 머지는 사용자가 한다.
