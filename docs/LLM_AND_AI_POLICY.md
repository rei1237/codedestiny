...
- validator가 가시 텍스트 30,000~56,000자(`FUSION_FORTUNE_LENGTH.total` 정본, 2026-09-06 2단계 생성으로 상향 → 같은 날 5차 실호출 51,203자 실측을 담도록 상한 재상향), 7개 결과 섹션(여섯 체계 + 통합 리딩), 안전 표현, 개인정보 노출을 모두 확인한 성공 결과만 이용권과 하루 한도를 commit한다.
