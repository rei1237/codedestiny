...
- 무료 기능: 로컬 계산 또는 local fallback만으로 결과가 먼저 완성되어야 한다. LLM 응답을 기다려야 결과가 보이거나, LLM 실패 때문에 무료 결과가 실패해서는 안 된다.
- 무료 LLM은 새 운세를 계산하거나 판정을 바꾸는 생성기가 아니라, 이미 계산된 근거와 결정론 문장을 다듬는 선택적 보강에만 허용한다. 결제·이용권·월정석·단건 차감이나 유료 결과 저장 계약을 호출해서는 안 된다.
- 새 무료 실 LLM 경로를 기존 무료 기능에 자동으로 확장하지 않는다. 별도 비용·쿼터 검토와 서버측 전역 한도, 캐시, 클라이언트보다 짧은 서버 총시간 예산, 결정론 fallback이 모두 확인된 예외만 허용한다.
- 현재 고객용 예외는 `/api/destiny-compass/narrate`다. 화면은 결정론 해설을 먼저 표시하며, 서버는 충실도 교정 최대 2회, 호출당 Gemini/Workers AI 공유 예산 10초, 총 24초 안에서만 문장을 교체한다. 공용 호출 헬퍼의 요청 단위 재시도는 1회로 고정하고 클라이언트는 32초에 중단한다.
