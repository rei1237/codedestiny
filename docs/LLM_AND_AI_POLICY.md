...
- validator가 가시 텍스트 20,000~30,000자(`FUSION_FORTUNE_LENGTH.total` 정본), 7개 결과 섹션(여섯 체계 + 통합 리딩), 안전 표현, 개인정보 노출을 모두 확인한 성공 결과만 이용권과 하루 한도를 commit한다.
- `worker/lib/fusion-fortune-prompt.js`는 여섯 체계의 전문가 계약, 교차 검증 규칙, 생시·출생지 미확인 단정 금지, 섹션별 최소 깊이와 JSON schema를 서버에서 고정한다. 생성은 단일 호출이 아니라 `FUSION_SECTION_GROUP_SPECS`의 4개 섹션 그룹을 병렬로 호출하고(20,000자를 한 호출로 뽑으면 출력 상한과 시간 예산이 먼저 바닥난다), 실패했거나 목표 분량의 80%에 못 미친 그룹만 예산이 남아 있을 때 1회 재생성한다. 그래도 남은 그룹은 실제 여섯 계산값을 사용하는 결정론 fallback으로 메우고 동일 validator로 검증한다.
