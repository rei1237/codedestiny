...
2. **기반 검증 결함 처리.** `test-feature-visual-details-mobile.mjs`의 React 팝업 단계가 `/app/`에 없는 life-book 링크를 찾는다. 실제 생산자에 맞춰 고친다. `/app/`의 유료 Neo 링크는 팝업 진입이고 찻집 링크는 직접 진입이다. 검사를 삭제하거나 무조건 통과시키지 않는다.
3. **전문가 상담 첫 확장: Neo.** `src/features/neo-war-room/data/method-registry.ts`, 실제 입력/결과 컴포넌트와 결과 타입을 읽는다. 사주/자미두수/베다/점성술은 선택 모드이며 무조건 융합이라고 쓰지 않는다. 각 모드 입력·계산 근거·결과 제공 범위를 확인한다. 신규 실 LLM 호출 없이 fixture로 구성한다.
4. **기능군 순회.** 사주 → 자미두수 → 숙요 → 서양 점성술/베다를 구분 → 타로/궁합 → 초융합/나머지 무료 기능. 69개 목록과 155개 마케팅 별칭, 145개 action, React 동적 생성 경로를 대조해 누락된 대상을 추가한다. 정책·계정·결제 처리 화면은 판매 상세 적용 제외 사유를 기록한다.
...
- 69개 등록 목적지 중 콘텐츠 적용은 3개이며, 그 3개도 전체 기능 여정 완료를 의미하지 않는다. 나머지 66개는 기능 근거 확인부터 필요하다.
...
`test-feature-visual-details-mobile.mjs`는 위에서 명시한 팝업 테스트 수정 전 전체 통과하지 않는다. 현재 없는 링크를 사용자에게 만들도록 요구하지 않는다.
