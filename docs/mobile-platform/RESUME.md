...
3. **기능군 순회.** 사주·자미두수·숙요·베다·서양 점성술은 실제 가이드/입력 구조를 근거로 SVG/HTML 소개와 320/390/430px 독립 주소 검증까지 완료했다. 다음은 타로/궁합 → 초융합/나머지 무료 기능이다. 69개 목록과 155개 마케팅 별칭, 145개 action, React 동적 생성 경로를 대조해 누락된 대상을 추가한다. 정책·계정·결제 처리 화면은 판매 상세 적용 제외 사유를 기록한다.
...
- 69개 등록 목적지 중 콘텐츠 적용은 9개이며, 그 9개도 전체 기능 여정 완료를 의미하지 않는다. 나머지 60개는 기능 근거 확인부터 필요하다.
...
- 최신 국소 검증: 상세 단위 9개, 타입 검사 통과. 핵심 5개 체계의 독립 소개주소는 SVG/HTML 미리보기·320/390/430px 가로 넘침 없음 확인. Neo 독립 소개주소는 OG·SVG/HTML 미리보기·320/360/390/430/1280px 가로 넘침 없음 확인. React Neo 팝업은 대상 교정까지 완료했고 동적 상호작용 전체는 실행 제한 때문에 미완료다.
