...
상세페이지 방향은 사용자 승인됐다. 찻집·인생의 책·운명의 동물 도감·네오의 팩폭 작전실·핵심 5개 체계(사주·자미두수·숙요·베다·점성술)·관계 타로 3종(우리는 무슨 사이·재회운·말과 행동 사이)·궁합 기능군 3종(마스터 인연의 서·동서 통합 궁합·LOVE CODE) 소개를 `/features/[slug]/`에 구현하고 팝업과 같은 렌더러를 사용한다. 인생의 책과 동물 도감은 실제 렌더러의 개인정보 없는 fixture 요약을 사용했으며, Neo·핵심 5개 체계·관계 타로 3종·궁합 기능군 3종은 실제 입력·결과·가이드 근거를 바탕으로 SVG/HTML만 구성한다. 69개 등록 대상 중 15종만 검증 콘텐츠이며 나머지는 공개 상세주소로 생성하지 않는다. 찻집 결과 fixture 캡처와 추천 매핑, 전 기능 확장은 미완료다.

추가 피드백으로 반복되는 둥근 카드·테두리를 없애고 대표 이미지와 문장, 결과 예시, 이용 안내의 위계를 정리했다. 본문은 기존 고딕 폰트 소스를 사용하며 추가 폰트·이미지·애니메이션은 없다. 동물 도감에는 기존 결과 캡처 대신 대표 자산을 독립 소개 OG에 연결했고, 마스터 인연의 서에는 원격 자산 없이도 로컬 mock 환경에서 표시되는 전용 대표 WebP를 추가했다. Neo 독립 소개주소는 320/360/390/430/1280px, 핵심 5개 체계와 관계 타로 3종은 320/390/430px에서 가로 넘침 없이 통과했다. 궁합 3종은 OG·대표 이미지·SVG/HTML 미리보기·CTA·320/390/430/1280px 가로 넘침 없음 확인. 상세 콘텐츠 단위 검사 15개와 typecheck 통과. 전체 15개를 순차 캡처하는 기존 스크립트는 로컬 30초 제한으로 중단되어 완료 증거가 아니다. `/app/` Neo 진입은 현재 모달이 아니므로 Escape 회귀 대상이라는 기존 기록은 사실과 달랐고, 직접 링크 계약으로 정정했다. 정적 팝업 CSS/기존 섹션 숨김·locale 전환은 별도 검증이 남았다. 전체 검사와 build는 이 후속 변경에 대해 아직 완료하지 않았다.
