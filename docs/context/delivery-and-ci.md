...
- **인수인계 최종 보고에는 문서 파일명, 클릭 가능한 절대 경로, 다른 세션에 복사할 재개 명령어를 반드시 함께 제시한다.** “문서를 남겼다”는 설명이나 상대 경로만으로 끝내지 않는다. 재개 명령어는 코드 블록에 넣고 실제 작업 디렉터리·문서 절대 경로·브랜치/PR·첫 번째 다음 행동을 채운다. 인수인계 문서에도 같은 재개 정보를 남긴다. 새 세션이 이전 대화를 읽지 않아도 이어갈 수 있어야 한다.
