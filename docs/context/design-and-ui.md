...
## 큰 디자인 변경 절차 (2026-09-09 사용자 요청 · CLAUDE.md 코딩 원칙 16 상세)
...
2. **방향과 성공 기준을 공유한다** — 바꿀 인상, 유지할 동작, 모바일·접근성·성능 검증 기준을 짧게 밝힌다.
3. **자율 구현한다** — 별도 승인 대기 없이 실제 코드에 반영한다. 이미 목업이 있으면 클래스명·토큰명과 시각 방향을 재사용한다.
4. **실제 화면으로 확정한다** — 모바일과 데스크톱을 한 번에 검토하고 발견한 문제를 한 묶음으로 수정한 뒤, 관련 회귀 검증을 완료한다.

사용자가 명시적으로 “목업을 먼저 보여 달라”, “승인 후 구현하라”고 요청한 작업만 구현 전에 멈춘다. 결제·라우팅·운영 배포 등 RED 경계의 별도 승인 규칙은 이 절차 변경과 무관하게 유지한다.

**목업 아티팩트를 만드는 경우의 구성 규격**
...
**왜 이 순서인가** — 디자인 반복 속도를 높이되, 기존 동작과 품질 기준을 잃지 않기 위해서다. 방향 공유는 재작업을 줄이고, 실제 화면 검증은 목업과 서비스 코드 사이의 차이를 잡는다.
