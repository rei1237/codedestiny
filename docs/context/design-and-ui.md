...
이 문서는 UI 작업 시 참고하는 시각·접근성·반응형 기준을 담는다. 자동 디자인 감지 스킬이나 파일 수정 후 훅은 사용하지 않는다.

## UI/UX 작업 기준

- 실제 화면의 기존 토큰과 컴포넌트를 우선 확인한다.
- 브랜드 색상과 컴포넌트 구조를 유지하면서 필요한 범위만 수정한다.
- 본문 대비는 WCAG AA 기준(일반 텍스트 4.5:1 이상)을 지킨다.
- 모바일 터치 영역, 텍스트 오버플로, reduced motion, 이미지 대체 텍스트를 함께 확인한다.
- 큰 시각 변경은 목업과 사용자 승인 절차를 따른다.
...
## 큰 디자인 변경 절차 (2026-09-04 사용자 요청 · CLAUDE.md 코딩 원칙 16 상세)

**적용 대상** — 화면 인상을 바꾸는 변경. 헤더·히어로·전역 내비·결제창·테마 색계열 교체·페이지 레이아웃 개편.
색 하나, 간격 하나, 오타 한 줄 같은 국소 수정은 해당 없다. 애매하면 "사용자가 보고 '달라졌다'고 말할 크기인가"로 가른다.

**순서 — 코드보다 초안이 먼저다.**

1. **읽는다** — 이 문서 + 손댈 표면의 현재 CSS. 어느 블록이 실제로 이기는지(파일 위치·특이도·`<link>` 순서)를 먼저 확정한다. 셸은 오버라이드가 여러 곳에 흩어져 있어 "첫 매치"를 고치면 아무 일도 일어나지 않는다.
2. **목업 아티팩트를 발행한다** — 코드를 고치지 않은 상태로. 구성 요소는 아래 규격.
3. **사용자 승인을 받는다.** 승인 없이 3번을 건너뛰지 않는다.
4. **구현한다** — 목업의 클래스명·토큰명을 그대로 옮긴다. 목업을 레포의 실제 클래스명으로 그려 두면 이 단계가 옮겨 붙이기가 된다.

**목업 아티팩트 구성 규격** (없으면 승인이 근거 없는 승인이 된다)

- **실물 크기 컴포넌트 1컷** — 데스크톱 기준 폭(셸 헤더는 960px)으로, 실제 문구·실제 탭 라벨로.
- **상태 스펙** — 기본 / hover / 활성 / 포커스 / 비활성. 상태를 빠뜨리면 구현에서 즉흥적으로 채우게 된다.
- **색 토큰 표** — 토큰명 · 값 · 쓰이는 자리. 두 모드가 갈리면 모드별 열을 나란히.
- 🔴 **대비 수치는 합성색 기준으로 잰다** — 반투명 유리 표면은 뒤 배경과 합성된 뒤에야 실제 색이 된다. 토큰 hex 끼리 재면 과대평가된다. 실제 사고: 연이 상단바 유리를 알파 .70 으로 잡았을 때 토큰끼리는 멀쩡했지만 크림 배경 위 합성색이 `#905c76` 로 나와 비활성 탭 3.84:1 · 태그라인 3.53:1 로 떨어졌다(2026-09-04). 알파를 .90 으로 올려 6.5:1 이상으로 회복.
- **캐리오버 주의사항** — 구현 시 남는 위험(예: `backdrop-filter` 가 만드는 containing block, 모바일 blur 성능 결정)을 목업 하단에 적어 승인 시점에 같이 노출한다.

**왜 이 순서인가** — 큰 시각 변경은 코드를 다 고친 뒤에 방향이 뒤집히면 되돌릴 것이 CSS 한 블록이 아니라 셸·미러 7개·로케일까지 걸린다. 초안 단계에서는 되돌리는 비용이 아티팩트 한 번 다시 발행하는 것뿐이다.
