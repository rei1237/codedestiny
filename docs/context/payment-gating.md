...
- 🔴 **이용권 콘텐츠 예산은 잔여 0에서만 소진된다**(2026-09-08). 소진되어도 원래 만료일까지 등급·만료일·프로필 상한은 유지하며, 추가 유료 콘텐츠만 단건 결제 또는 월정석으로 인계한다. `worker/lib/profile-limits.js`의 `isPassBudgetExhausted`가 잔여 0 경계를 정본으로 갖고, `buildPassTerminationFields`는 판정에 쓰지 않는 감사 시각만 기록한다. 새 이용권이 활성화되면 감사 마커를 지운다. 한도 사이클 키가 이용권 만료일이므로 기간 안에서 리셋되지 않으며, 문구에 "다음 달에 다시 열림"을 쓰지 않는다(`scripts/verify-pass-tier-policy.mjs`가 12개 로케일 + 셸 + React를 전수 검사한다).
