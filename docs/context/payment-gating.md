...
- 🔒 **[필수·예외없음] 모든 유료 결제 게이팅 순서** (2026-08-01 개정 — 축이 "언제 검사하는가"에서 "이용권 보유자가 어떤 경로로도 돈을 내지 않는가"로 바뀌었다). 신규/수정 불문 모든 유료 기능은 아래를 그대로 따른다. 벗어나는 결제 구현은 금지이며, 발견 시 즉시 사용자에게 보고한다(작업 중 우연히 마주쳐도 그냥 지나치지 말 것):
  ...
  - **direct_or_family 예외(2026-09-23)**: 영냥이(SoulCat, `/yeongnyangi/*`) 유료 상품 `yeongnyangi-*` 28개는 **Family 이용권 또는 단건 결제만** 받는다. Standard·Premium·VVIP와 월정석은 적용되지 않는다. 정본은 등록소 `paymentScope:"direct_or_family"` 와 `isDirectOrFamilyPaidFeatureKey`이며(featureKey 문자열 분기 금지), 서버는 `allowedPaymentMethods:["FAMILY","DIRECT_KRW"]`를 내린다. Family는 공용 `evaluatePassCoverage → consumePassCoverage` 경로로 `(featureKey, requestId)`당 한 번 차감하고, 다른 이용권은 `FAMILY_PASS_REQUIRED`, 월정석은 `FAMILY_OR_DIRECT_PAYMENT_REQUIRED`로 실패 폐쇄한다. `/checkout/`은 `allowedPaymentModes:["pass","direct"]`와 `passStorePlan:"family"`로 Family 확인 카드와 단건 결제만 보여 준다. `verify:billing-pass-policy`가 이 상태를 단언한다.
...
- 모든 유료 흐름은 현재의 접근 옵션 3종(이용권 · 단건 결제 · 월정석)을 그대로 보존한다(유일한 예외: 위 direct_or_family 영냥이 상품).
