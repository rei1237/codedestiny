...
3. ~~**수술적 변경**~~ — 🔴 **폐기 (2026-09-04, 사용자 요청).** 폐기 사유와 대체 규칙은 `CLAUDE.md` 코딩 원칙 3·14 에 있다. 요지: 이 항목이 **범위 안의 결함까지 손대지 않는 근거**로 쓰였다. 이제 범위 안에서 발견한 결함은 같은 변경에서 고치고 가드까지 붙이며, 범위 밖만 언급에 그친다. 남는 것 둘 — 기존 스타일을 그대로 따른다(자신의 취향과 달라도), 자신의 변경으로 생긴 미사용 import/변수/함수는 제거한다. 🔴 아래 번호는 비우지 않는다(상호참조 정본이 번호다).
...
- **세션 길이가 지배한다.** 컨텍스트 300k 초과 요청이 전체의 45%인데 **총소비의 69.7%** 를 먹었다. 반대로 세션 재시작 고정비는 첫 요청 64k = 총소비의 **0.1%** 였다. 그래서 `/clear` 는 비용이 아니라 이 레포에서 가장 큰 절감 수단이고, 원칙 12 의 임계(150k/200k/300k)는 되돌리지 않는다.
...
- **CI 폴링은 같은 답을 반복해서 실었다.** 같은 기간 폴링 364콜(최악 한 세션 55콜). `gh pr checks <PR 번호> --watch --fail-fast` 한 콜로 끝낸다.
