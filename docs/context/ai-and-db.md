...
- 🔴 **Workers AI 입력도 `env.AI.run()` 전에 50,000토큰 보수 상한으로 하드 차단**한다 — 정본 `lib/workers-ai-input-token-limit.mjs`는 모델 공통 `countTokens` API가 없는 제약 때문에 실제 메시지 JSON의 UTF-8 바이트 수에 채팅 템플릿 여유 1,024를 더한 값을 토큰 상한으로 사용한다. 초과하면 폴백 체인의 첫 모델도 호출하지 않으며, 환경값으로 상한을 올리지 않는다. 공급자 응답에 `usage` 토큰이 있으면 실제 값을 기록하고, 없을 때만 입력 보수 상한·출력 문자 추정을 `estimated` 값으로 남긴다.
