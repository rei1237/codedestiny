...
| 89 | 대통령 사주 적중 | C | `/about/#author` | 공개 원문 3건·게시일·한계 병기, 단정형 메타 금지 |
| 90 | 두 대통령 운명 | C | `/about/#author` | 대통령 원문과 유명인 공개 기록을 분리해 제시 |
...
| 30 | 적중 기록 | `/about/#author`에 대통령 원문 3건·휘성 원문·작성자 색인 연결. 별도 아카이브는 독립 검증 체계가 생긴 뒤 | P3 진행 |

## 20. 기각한 것

- 영냥이 무료 허브: CD 무료 랜딩과 잠식.
- 체계별 천원 랜딩 6개: 자동완성 수요 없음, 얇은 페이지 반복.
- 대통령·유명인 별도 SEO 아카이브: 공개 원문은 `/about/#author`에 연결했지만 독립 검증된 적중률 자료는 없으므로 별도 키워드 페이지를 대량 생성하지 않는다.
