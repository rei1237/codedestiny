...
- 🔴 선물은 **선불·양도 가능 상품**이다(결제자와 이용자가 다를 수 있음). PG 위험 요인으로 숨기지 않고 신청서에 그대로 적는다(D3). 선물 신규 구매는 코드상 기본 꺼짐(`GIFTS_ENABLED` 가 문자열 `"1"` 일 때만, 앱 요청은 차단)이고, 운영 활성 여부는 **OWNER INPUT REQUIRED** 다.
...
| 선물 운영 활성 여부 | NOT READY(OWNER INPUT REQUIRED) |

**최종 판정: OWNER INPUT REQUIRED** — 상품·가격·기간 사실은 확정됐다. 선물을 해외카드 대상으로 신청할지(PG 위험 요인)와 운영 선물 활성 여부는 운영자가 정한다.
