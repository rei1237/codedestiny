...
### 남은 질문 — 오너가 답하면 아래 "기록 위치"에 그대로 옮긴다

코드로 답할 수 없는 순수 운영 결정 3개다. 세션 없이 오너가 이 절에 바로 답해도 되고, 답을 받은 세션은 아래 "기록 위치"만 갱신하면 된다(추가 조사 불필요).

1. **결제 사고 1차 담당자** — 알림 채널(이메일 `admin@code-destiny.com`)로 결제됨·미지급 또는 PG 대조 실패 알림(C7, `§2`)이 오면, 그 메일을 실제로 확인하고 `/admin/orders` 에서 조치할 사람이 누구인가? (이름 또는 역할 하나면 충분, 연락처는 문서에 옮기지 않는다)
   - 기록 위치: 이 표 "결제 사고 1차 담당자" 행 + [09 신청 사실](09-inicis-application-facts.md) 문항 9.
2. **온콜·대응 시간** — 그 담당자가 상시 확인 가능한가, 아니면 특정 시간대(예: 평일 업무시간)만 보는가? "얼마나 빨리 조치되는가"에 대한 내부 기대치(엄격한 SLA 약속이 아니어도 됨)는?
   - 기록 위치: 이 표 "온콜·대응 시간" 행. **신청서·고객 화면에는 옮기지 않는다** — §5 "없는 것"에 적은 대로 24시간·SLA 보장은 만들지 않는 것이 현재 방침이라, 이 답은 내부 대응 체계 파악용이다.
3. **개인정보 유출 대응 절차** — 이메일·전화·결제 마스킹 정보 등이 유출됐다고 의심되면 내부적으로 누가 무엇을 먼저 하는지 정해둔 절차가 있는가? 없다면 "없음"이 답이다(있는 척 적지 않는다). 법정 신고 기한 등 법 해석이 필요한 부분은 오너 답변만으로 닫히지 않고 [07 개인정보](07-personal-data-inventory.md)의 **LEGAL REVIEW REQUIRED** 범위로 남는다 — 내부 절차 유무와 법적 신고 의무는 별개로 기록한다.
   - 기록 위치: 이 표 "개인정보 유출 대응 절차" 행(내부 절차분) + [07 개인정보](07-personal-data-inventory.md) §"국외 이전·보호책임자"(법적 신고 의무분, 변호사 검토 후).
...
| 알림 채널 운영 설정·담당자·온콜 | 알림 채널(이메일 `admin@code-destiny.com`, Discord·Slack 은 오너가 안 씀 — 의도된 상태)은 READY, 담당자·온콜은 NOT READY(OWNER INPUT REQUIRED) |
| 웹훅 반복 실패·환불 실패·크론 실패 알림 채널 | NOT READY(범위 밖 결함) |
| 오류 수집(Sentry 등) | NOT READY(없음) |

**최종 판정: OWNER INPUT REQUIRED** — 알림 채널(이메일 `admin@code-destiny.com`, Discord·Slack 미사용)은 2026-09-18 오너 확인으로 해소. 결제 사고 1차 담당자·온콜·개인정보 유출 대응 절차는 아직 정해지지 않았다. 신청서에는 "이메일 문의, 응답기한 보장 없음(한국어 페이지 기준 영업일 1~3일 안내)"처럼 있는 것만 적는다.
