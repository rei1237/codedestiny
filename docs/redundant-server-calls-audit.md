...
**착수 규칙**: AGENTS.md 워크트리/PR 우선 배송을 따른다. 항목마다 `git switch -c codex/<slug> origin/main`
→ `npm run verify:worktree-policy -- --mode=edit` → 수정 → 해당 검증 → PR. 결제/인증 경로는 PR 필수 레인이다.

---

## 🔴 P0 — 보안 취약점 (다른 모든 항목보다 우선)

### SEC-1. `POST /api/fortune/pig-coin/refund` 로 레거시 코인을 임의 발행할 수 있다 → 월정석 1:10 전환
...
### SEC-2 (연관). 카드결제 실패가 코인 환불 루틴으로 흘러간다
...
### RC-3. 정적 셸: 유료 클릭 실패 시 백오프 재시도가 왕복을 최대 6.5배로 증폭
...
### RC-6. `primePaymentEligibility` 11곳이 게이트가 읽지 않는 `unlock-status` 를 미리 친다
...
**회귀 위험**: 스냅샷 저장 side-effect 는 `warmSubscriptionSnapshotOnEntry`([PaymentProcessingContext.tsx:1016-1053](../app/providers/PaymentProcessingContext.tsx#L1016))가 이미 커버한다.
