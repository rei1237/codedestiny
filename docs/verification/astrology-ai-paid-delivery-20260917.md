...
**`sync:public` 재실행 여부:** 워크트리 병합 직후 미러가 다시 낡을 수 있다는 기존 교훈에 따라 확인했다. `scripts/sync-legacy-static-to-public.mjs`(sync:public이 호출하는 실제 스크립트)를 grep한 결과 이 스크립트가 미러링하는 대상은 `root/*.html`(예: `vedic-astrology.html`)·`styles/` 등 레거시 정적 파일뿐이며 `app/astrology-ai/**` App Router 소스는 어디에서도 참조되지 않는다 — 이번 수정 파일 3개(App Router 클라이언트 컴포넌트 2개 + 신규 테스트 1개)는 sync:public의 대상 범위 밖임을 실측으로 확인했다. sitemap 쪽은 이미 pre-merge·post-merge 두 시점 모두 `sitemap:generate` 재실행으로 diff 0건을 직접 확인했다(위). 따라서 이번 행은 sync:public 재실행이 필요하지 않다.
