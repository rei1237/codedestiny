...
## main CI 실측 — `e87cc3248`

**`CI required` 는 failure 다. 원인은 이 행의 변경이 아니라 상속된 선행 드리프트 1건이다.**

- success: `Critical checks` · `Build Pages and Worker` · `Typecheck and lint` · `gitleaks` · `Risk tier` · `AI locale pipeline invariants` · `Check that landed work reached staging` · `Resolve release necessity`/`staging watch scope`
- failure: `Static guards` · `Main drift` — **둘 다 `npm run verify:public-mirror-fresh` 한 스텝에서만** 멈췄다(`미러가 소스보다 낡았다`, 재생성 시 바뀌는 파일 7개: `index.html` · `public/index.html` · `public/static/index.html` · `public/{en,ja,zh,zh-tw}/index.html`). 같은 잡의 앞 단계인 `verify:sitemap-drift`·숙요/명리 달력 가드·결제 가드는 전부 통과했다(잡이 그 뒤 스텝까지 진행한 것이 증거)
...
### 후속 — 원인 규명과 해소(`44eac0f68`)

옆 세션이 `64f55a98a`(ziwei 패널 스타일 + 미러 재생성)로 그 7개 파일을 전부 다시 커밋했지만 **그때는 닫히지 않았고**, 이후 소유 세션이 원인을 규명해 `44eac0f68` 에서 해소했다. 아래 앞 세 줄은 `341b722e4` 시점의 실측 기록이고, 마지막 줄이 확정된 원인과 해소다.
...
- **원인은 미러 누락이 아니라 `sync:public` 이 1회로 수렴하지 않는 것이었다**(소유 세션 `code-destiny-50` 의 실측). `index.html` 이 물고 있는 `/js/core/index-inline-runtime.js?v=` 토큰이 **한 세대 낡은 채로** 커밋된다 — `341b722e4` 기준 박힌 값 `build-f3607e7cb0bd`, 그 커밋의 런타임 파일 실제 내용 해시 `build-2b5a643397bf`. `verify:public-mirror-fresh` 는 sync 를 한 번 돌려 바뀌는 파일을 세므로 그 한 줄 때문에 셸 7개를 전부 낡았다고 보고한다 — 목록이 "미러 누락"처럼 보인 이유다. 유래는 `9812d039f`(옆 세션), 전파가 `d6ad24bb8` → `8deacfaad` → `e87cc3248` → `64f55a98a` → `341b722e4` → `f48c83fec`, 해소는 `44eac0f68`(sync 2회차까지 수렴시켜 산출물을 같은 커밋에)다. 14개 커밋 훑기로 **내 커밋 3개가 원인이 아님이 재확인**됐다
- **해소 확인(실측):** 그 수정이 포함된 `be930c4e1` 의 `Main drift` = **success**. 27행 전달은 빨간 main 을 남기지 않는다
- 🔴 재발 방지: **`js/**` 를 고쳤으면 `sync:public` 을 두 번 돌리고 2회차 변경 0건을 확인한 뒤 커밋한다.** 기전·확인법은 [`basic-fortune-result-ui-2026-09-19.md`](../handoff/basic-fortune-result-ui-2026-09-19.md) §3 에 있다. CI 로그의 벤더 사본 2건(`js/vendor/sweph-wasm/**`)은 명시적 carve-out 이라 실패 원인이 아니다
