...
- 계정의 `등록한 프로그램 및 서비스`에서 **15% 서비스 수수료 프로그램 가입**을 확인했다. 공식 정책상 가입 개발자는 연간 첫 100만 USD 매출에 15%, 초과분에 30%가 적용되며, 한국 대체결제는 해당 서비스 수수료에서 4%p 감면된다. 다만 아직 없는 v3 SKU의 실제 정산 기준·세금·국가별 가격을 입증하는 자료는 아니다.
...
- 최근 Workers Paid 청구서는 2026-08-30 발행 `IN-77252831`, $5.00이며 구독은 활성 상태다. 9월 Billable usage에는 R2·Workers·D1·Queues 계정 사용량이 포함 한도 안의 $0.00로 표시됐다. 이 청구서는 계정 고정비를 확인할 뿐 Code Destiny 상품 SKU별 상담·저장 비용을 분리하지 않는다.
- 운영 Worker `code-destiny-web`의 최근 7일 로그에서 `[llm token_usage]`를 조회했지만 일치 이벤트는 0건이었다. 보존 구간에 상품 귀속 실사용 표본이 없으므로 비용을 0원으로 간주하지 않는다.
...
- Cloudflare Workers Paid 계정 청구서와 Play 15% 프로그램 가입은 확인했다. 그러나 신규 SKU의 KRW 가격·지역·활성 상태·실정산, 상품별 유료 상담 실사용량, 저장·지원·환불 실원가는 아직 없다.
