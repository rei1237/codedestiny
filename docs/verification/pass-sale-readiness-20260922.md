...
2026-09-22에 같은 목록을 다시 읽기 전용으로 확인했다. 앱은 계속 임시·내부 테스트/프로덕션 비활성 상태였고, 18개 일회성 제품에는 구 `cd_pass_*_30d` 4종만 있었다. v3의 지역 KRW 가격·구매 옵션·정산은 확인할 대상 자체가 없으므로, 이 재확인은 SKU 부재 증거이지 판매 개시 근거가 아니다.
...
이번 재확인에서도 포트원은 9월 거래일 화면의 전체 거래 43건·62,600원, PG 수수료 1,159원·부가세 117원을 표시했다. 일자별 정산 완료 18건의 52,700원·51,598원·수수료 1,001원·부가세 101원 집계와 기준이 다르므로 서로 바꿔 쓰지 않는다. 어느 집계도 v3 또는 상품별 LLM·저장·지원 원가를 제공하지 않는다.
