...
- `npm run check:fast -- --plan`, `npm run check:fast`: 실행. Jest 300 suites / 4,250 tests 통과, paid gate 개별 87개 통과. Node 검사에서 새 모드 이벤트 연결과 기존 사이트맵 주간 날짜 2건 실패. 모드 이벤트는 기존 컨트롤러 직접 연결로 수정 후 custom-event-wiring 5개 통과. 사이트맵 검사는 최신 main에서 5개 통과 확인(이 변경에 사이트맵 수정 없음).
