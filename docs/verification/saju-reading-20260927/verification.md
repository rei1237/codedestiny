...
- 로그인/저장 프로필 전환 및 유료 복구는 기존 mock 회귀 검사로 확인했다. 추가 profile-probe.mjs 브라우저 검사는 로그인 fixture에서 저장 프로필 사주를 열고 새로고침 후 재열람해 명식·오행 및 네오 모드 유지까지 통과했다. 모든 API 요청은 로컬 fixture로 처리했다.
- 실제 모바일 키보드와 실기기 안전 영역, 실결제·실 LLM·운영 DB는 검사하지 않았다.
- 전환율 개선은 입증하지 않았으며 추가 이벤트는 모드·화면·상품키만 기록한다.
- 첫 CI의 정적 검사에서 로케일별 사이트맵 생성물 누락이 확인되어 8개 파일을 추가하고 해당 seo-trust-locales 검사를 통과시켰다. 공식 CI·최종 스테이징 결과는 main 통합 후 기록한다. 운영 승격은 이번 작업 범위가 아니다.
