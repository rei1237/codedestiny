...
status: active
updated: 2026-09-17
next: "①운영 실결제에서 1장만 생성되고 멈추는 원인을 규명·수정한다(운영 DB 읽기 전용 조회와 실제 LLM 호출은 사용자에게 횟수를 정확히 승인받은 뒤에만). ②결과 화면 하단 CodexSeal의 '운명의 지도 열기'(/destiny-island.html)를 '내 서재' 이동으로 교체한다. ③이니시스 창 안의 카카오페이 복귀가 부자연스러운 원인이 카카오페이 쪽인지, 이니시스 창 쪽인지, 우리 복귀 처리 쪽인지 구분한다."
---
