...
### [x] 1. `scoreBoundary` 의 죽은 `gender` 인자 (완료 — 삭제함)

> **2026-09-12 종료**: 사용자 지시로 인자를 삭제했다. 아래는 삭제 근거 기록이며 재조사 방지용으로 남긴다.
> 성별 의존 점수를 나중에 정말 넣는다면 이 절 맨 아래 🔴 문단의 조건이 그대로 유효하다.
...
→ 실제 결과도 그대로였다. 소스 1줄만 바뀌고 테스트·검증기 수정 0곳.
