...
status: active
updated: 2026-09-23
next: "기존 셸·나크샤트라·손금·초융합·자미두수 등 미연결 상담의 저장 결과 공유와 구매 동선·SEO 12개 랜딩 측정을 이어간다. 전체 목표는 미완료다."
---
...
- 구현 커밋 `c5fef4086dfe4318f1510946d890a700c3db67f1`, `c329c49abcb33496167fe3b363df9bfbfdb31b3b`, `145b673ed503eb6e974ee9bf53dface9e76d89bf`. `main`의 동시 `marketing/**` 미커밋 변경을 보존하기 위해 `D:\Development\codedestiny-worktrees\conversion-sharing-followup-20260923-141314`에서 검증했다. 최종 main push·CI는 이 문서의 후속 전달 결과로 판정한다.
...
- `/saju/`·`/astrology/`·`/vedic/` 허브에서 기존 설명 가이드로 가는 목적지 설명 링크를 추가하고, 사주 가이드에서 두 상품의 기존 `/features/` 구성·풀이 예시로 연결했다. 실제 개발 서버에서 허브 3개·가이드·두 예시 200과 링크를 확인했다. 예시는 편집 예시로 표기된 기존 결과 문구이며 고객 후기나 실제 결과라고 표시하지 않는다. `sitemap:generate`와 `verify:sitemap-drift`는 1,284 URL 정합을 확인했다. 두 예시 페이지의 기존 `noindex, follow`는 유지한다.
...
- mock 검사: `node scripts/verify-master-love-codex-sharing.mjs`, `node scripts/verify-astrology-vedic-sharing.mjs` (`CONSULTATION_TEST_BASE=http://127.0.0.1:24322`). 기본 `npm run dev`가 만든 mock 서버를 사용하며 외부 API는 차단했다. 원본 캡처 9개는 `D:\Development\code-destiny\build-cache\consultation-followup-20260923\`에 보존했다.
...
마스터 인연의 서 후속: `CodexReader`의 전 장 도착 판정과 저장된 각 장의 `keySentence`·`actions`·`insight`를 이용해 공개 소개 링크의 편집형 요약 공유를 연결했다. 질문·이름·생년월일·세션 ID 자동 전송은 제외하고 PDF 캡처 대상 밖에 둔다. `docs/consultation-sharing-coverage-20260923.md`에 범위를 기록했다. 다음은 mock 실화면 확인과 카르마·서양점성·베다 결과의 저장 스키마별 어댑터다. 전체 목표는 계속 진행 중이다.

카르마 후속: 구형·신형 저장 결과의 `summaryCards`와 장별 `summary`에서만 선택형 요약 공유를 연결했다. 완료/저장 식별자 조건과 PDF 바깥 배치를 유지한다. 개발 프리뷰의 390/1280px 네 화면에서 편집기·이미지·모바일 버튼 접근을 확인했고 생성 중/실패는 숨겼다. 다음은 서양점성·베다 결과의 저장 스키마와 실기기 카카오 수신 확인이다. 전체 전환·SEO·모든 상담 공유 목표는 여전히 진행 중이다.
