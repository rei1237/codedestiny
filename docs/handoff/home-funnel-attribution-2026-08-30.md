...
- **PR #1307 머지 완료**(2026-08-30, `ea8b63cae`) — 코드는 스테이징에 나갔다. 아래 "남은 작업"만 유효하다.
...
- [x] 요청 18 의 남은 절반 중 **정의**는 2026-08-30 에 썼다 — [analytics-kpi.md](../analytics-kpi.md).
      이벤트 인벤토리(실측)·북극성·단계 KPI 9종·함정 5종·GA4 탐색 3개 구성까지. 베이스라인 표는 비워 뒀다.
- [ ] 남은 것은 **화면 두 가지**다 — (1) GA4 탐색 3개를 실제로 만드는 UI 작업(레포에 커밋할 것 없음),
      (2) `hasPassHint`·`renderer`·`runtime` 별 집계 조회기. 지금은 읽는 경로가 아예 없다(같은 문서 §6 끝).
