...
status: active
updated: 2026-09-16
next: 마스터 수정본의 운영 반영 여부부터 확인하고, 서비스별 재검증표의 초융합 행부터 한 상품씩 A~F를 재현한다.
---
...
**운영은 아직 확정 불가:** 읽기 전용 1회 버전 검사에서 [Pages](https://code-destiny.com/version.json)와 [Worker](https://code-destiny.com/api/version)는 모두 `a3d1b471f319036deb416251a2116ef278097567`. 수정 기준과 다르며 생성/복구/모바일 소스도 차이가 있다. 따라서 현재 실고객 결제 후 전달 해결·운영 반영을 완료로 보고하지 않는다. 운영 승격은 이번에 하지 않았다.

## 다음 작업

1. **마스터 2상품의 운영 확인은 별도 단계.** 최신 main·CI와 실제 배포 버전을 대조한다. 운영 승격 승인 후 Pages/Worker 동일 SHA와 릴리스 성공을 확인한다. 실제 PG/과금 LLM/운영 DB/실기기·고객 주문 복구도 각각 승인 범위 안에서만 확인한다. 승인 없는 운영 단계는 미검증으로 남기고 다른 상품의 mock 점검은 계속한다. 입력 없는 과거 구매는 구매 권리 보존·재입력 후 결제 없는 복구를 유지한다.
...
`npm run verify:deployed-sha -- --sha=77007dc4c1c931ecc148ab73069bf437b78473e9 --attempts=1` → 이번 조회는 버전 불일치로 exit1. 배포 성공 검사로 보고하지 않는다.
