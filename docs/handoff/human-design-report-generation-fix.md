...
status: done
updated: 2026-08-31
next: 없음 — 차트 병목 갈래는 닫혔다. `MONGO_PING_TIMEOUT_MS` 1000→300 이 프로덕션에서 검증됐고(선행 구간 1223–1264ms → **533–545ms**, 아래 *승격 후 검증*), `clampTimeoutMs` 하한 300→150 인하는 근거 부족으로 **기각**했다(아래 *남은 작업* 마지막 항목). 남은 지연 축은 ping 이 아니라 Atlas 핸드셰이크 410–450ms 와 라우트 진입 오버헤드 ≈235ms 이고 둘 다 이 노브 밖이다
---
...
- **살아 있는 소켓의 ping 왕복 단독 ms.** 위 하한 인하 기각을 되돌릴 유일한 근거이고, 지금은
  요청 전체 353–380ms 안에 묻혀 있어 분리되지 않는다.
