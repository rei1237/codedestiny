...
status: active
updated: 2026-08-31
next: 프로덕션 측정까지 끝났다(2026-08-31, 아래 *프로덕션 실측*). 스테이징 퇴행이 아니라 **양쪽 공통 병리**다 — 두 환경 모두 매 요청 재수립하고 매 요청 1000ms 짜리 실패가 예정된 ping 을 태운다. 격차는 정책이 아니라 Atlas 핸드셰이크 비용(스테이징 1256ms vs 프로덕션 493ms)이다. 남은 것은 `worker/lib/db.js` 수정 방향 결정 하나이며, 후보 2안과 회귀 위험은 *남은 작업* 에 있다
---
...
- [ ] 🔴 **남은 것은 `worker/lib/db.js` 수정 방향 결정 하나다 — 차트 라우트가 아니다.**
      두 후보가 있고, 실측이 **1안**을 가리킨다.
      - **1안(추천) `MONGO_PING_TIMEOUT_MS` 를 1000 → 150~200 으로 내린다.** 살아 있는 소켓의
        ping 은 왕복 한 번이고(위 353–380ms 요청 3건이 근거), 죽은 쪽은 어차피 안 돌아온다.
        재사용이 되는 소수 요청은 그대로 살리면서 나머지 전부에서 **약 850ms** 를 걷어낸다.
        🔴 `[vars]` 양쪽 + 코드 기본값을 **함께** 바꿔야 한다 —
        `__tests__/worker/db.vars-code-default-parity.test.js` 가 둘을 묶는다.
      - **2안 요청 경계에서 검증 없이 바로 재수립.** ping 1초를 통째로 없애지만, 위 3건처럼
        살아 있는 소켓을 만나 353ms 로 끝나던 요청까지 ~500ms 재수립으로 끌어올린다. 또
        Atlas M10 의 **노드당 초당 15개 신규 커넥션 생성률 제한**(`maxIdleTimeMS` 주석)을 향해
        모든 요청을 밀어 넣는다. 1안이 실패한 뒤에 갈 곳이다.
      - 🔴 **회귀 선보고(원칙 7).** `connectDb` 는 인증하는 모든 워커 라우트의 공용 경로다.
        1안의 위험은 **ping 이 150ms 안에 못 돌아오는 정상 소켓을 죽은 것으로 오판**하는 것이고,
        그 결과는 실패가 아니라 불필요한 재수립(+500ms)이다. 되돌리기는 값 하나다. 판정 근거를
        만들려면 배포 후 같은 tail 로 `[db-connect]` **건수 비율**(요청 대비)이 올라갔는지 본다 —
        오늘 기준선은 프로덕션 45건 중 18건이다.
...
- 차트 초기 로딩에서 **AUTH · ARCHIVE_LOOKUP · ARCHIVE 가 프로덕션에서 몇 ms 인지**. 공용
  `connectDb` 는 위에서 프로덕션 실측했지만, 차트 라우트 자체의 `authDetail` 은 계측 PR #1344 가
  프로덕션에 승격돼야 읽을 수 있다(2026-08-31 기준 프로덕션 = `66bdb09ac`, 7커밋 뒤).
