...
- [x] **`clampTimeoutMs` 하한 300 → 150 — 기각 확정(2026-08-31).** 선행 조건이던 ping 왕복
      단독 수치를 프로덕션에서 읽었고(아래 *프로덕션 계측 판독*), 결과는 인하를 **되살리지 않고
      못박는다**: `rttMs=266` 대 예산 300 — 여유가 **34ms(11%)** 뿐이다. 150 으로 내리면 그
      살아 있던 소켓이 죽은 것으로 오판돼 유일한 재사용 경로(1/10, 요청 329ms)가 사라지고 전
      요청이 재수립 비용을 낸다. 🔴 표본은 `rttMs` **n=1** 이므로 "266 이 정상값"이라고 단정하지
      말 것 — 다만 인하 판단에는 이 한 건으로 충분하다(여유가 없다는 쪽이라 표본이 늘어도
      방향이 안전한 쪽으로만 바뀐다). 반대로 **올릴 이유도 없다**: 9/10 은 죽은 소켓이라
      예산을 올린 만큼 전부 손해다.
- [x] **잔여 두 축의 분해 계측 — 완료** (PR #1366 / `70433d36b`, 판독 2026-08-31, 아래
      *단계 계측 판독*). `dnsMs` 와 핸드셰이크 갈래는 닫혔고, 잔량 ≈235ms 는 teardown 으로
      확정됐다.
...
- [x] **머지 후 스테이징 재측정 — 완료** (2026-08-31, 스테이징 `9d05ee11b`, 아래
      *머지 후 재측정*). `warmResetMs` 1316 → **0**, `startToConnect` 1623 → **≈310**.
      `[db-detach] elapsedMs` 는 0 · 0 · 698 로 프로브 간격보다 훨씬 작다.
- [ ] **#1372 를 프로덕션에 승격 — 사용자 판단.** 프로덕션은 아직 `1e747a9e5`(승격 run
      `33326529095`, #1372 이전)라 요청당 `warmResetMs` **226ms** 를 그대로 낸다. 승격하면
      `startToConnect` 532 → ≈300 이 되고, 그때 위 *프로덕션 계측 판독* 표를 같은 절차로
      다시 채운다. 🔴 승격은 규칙 2 급이라 사용자가 명시적으로 요청할 때만 실행한다. 명령은
...
## 단계 계측 판독 — 잔량의 정체는 teardown 이다 (2026-08-31)

재현은 위 *승격 후 검증* 과 **같은 절차**다(없는 슬러그 404 를 4초 간격 10회). 로그 두 줄이 늘었다.
...
🔴 **`wrangler tail --format json` 은 NDJSON 이 아니다** — pretty-print 된 객체를 연달아 뱉으므로
줄 단위로 `JSON.parse` 하면 68KB 를 읽고도 0건이 나온다. 컬럼 0 의 `{` ~ `}` 로 끊어야 한다.

**스테이징** `904abdf7a` · 이벤트 10건(웜 9 + 콜드 1) · 값은 중앙값(괄호는 범위)

| startToConnect | warmPingMs | warmResetMs | elapsedMs | dnsMs | helloRttMs | socketReadyMs | wallTime |
|---|---|---|---|---|---|---|---|
| 1623 (1622–1695) | 300 (9/9) | **1316** (1313–1390) | 2440 (2384–2508) | 5 (2–10) | 510 (487–514) | 1324 | 4366 |

**프로덕션** `256c63b99` · 이벤트 10건 — 계측 PR 미승격이라 새 필드는 안 나온다

| startToConnect | elapsedMs | wallTime | 재사용 |
|---|---|---|---|
| 532 (528–541) | 423 (405–440) | 1028 | 1/10 (`[db-connect]` 없이 341ms) |

- 🔴 **범인은 teardown 이다 — 판정표가 예상한 200–250 이 아니라 스테이징 1316ms.** 산술이 정확히
  닫힌다: `startToConnect 1623 ≈ ping 300 + reset 1316`. 프로덕션도 같은 모양이라
  `532 − 300 = 232` 가 곧 teardown 이고, "라우트 진입 오버헤드 ≈235ms" 라는 이름은 폐기한다.
  다음 행동은 위 *남은 작업* 의 별도 PR 이다.
- ✅ **`dnsMs` 갈래 기각** — 2–10ms 로 `elapsedMs` 의 0.3% 미만이다. `mongodb+srv://` → 시드리스트
  전환은 하지 않는다(시크릿 변경 불필요).
- ✅ **핸드셰이크는 전부 왕복 비용이라 코드로 줄일 것이 없다** — 스테이징 `2440 / 510 ≈ 4.8왕복`,
  그중 `socketReadyMs 1324 ≈ 2.6왕복`(TCP+TLS+SCRAM). 종전 "≈7왕복"은 총합끼리 나눈 추정이라
  **틀렸다**. 남는 레버는 워커 콜로와 Atlas 의 거리뿐이다.
- ❌ **`rttMs` 는 못 읽었다 — 스테이징은 ping 성공이 구조적으로 0건이다.** 9/9 가
  `warmPingMs=300` 으로 예산을 전소했는데, 그럴 수밖에 없다: 스테이징의 1왕복이
  `helloRttMs ≈ 510ms` 라 **300ms 예산이 RTT 보다 작다.** 살아 있는 소켓도 통과할 수 없어
  재수립이 10/10 이다. 🔴 이것이 *승격 후 검증* 절 마지막이 경고한 회귀가 스테이징에서 실현된
  모습이다(실패는 아니고 순수 지연 — `db-connect-error`·`db-op-timeout` 0건). 프로덕션은
  반대로 1/10 이 재사용에 성공했으므로 RTT 가 예산 안에 있다. 🔴 **"예산 < RTT" 라는 이 사유는
  2026-08-31 재측정에서 반증됐다** — 아래 *머지 후 재측정* 에서 `helloRttMs` 가 257–268 로 예산
  300 **안**인데도 ping 은 3/3 이 300 을 전소했다. 남는 설명은 소켓이 느린 게 아니라 죽었다는
  쪽이다.

🔴 표본은 한두 건으로 끊지 말 것 — 웜/콜드가 섞인다(위 표본도 첫 건이 콜드였다).

## 머지 후 재측정 — teardown 이 임계 경로에서 사라졌다 (2026-08-31, 스테이징 `9d05ee11b`)

PR #1372 머지 후 스테이징에서, 위 *단계 계측 판독* 과 **같은 절차**(없는 슬러그 404 를 4초 간격
10회). 이벤트 10건 = **웜 3 + 콜드 7** · `outcome=ok` 10/10 ·
`db-connect-error`·`db-op-timeout` **0건**. 아래는 웜 3건(콜드는 웜 판정을 안 타므로 제외).

| | startToConnect | warmPingMs | warmResetMs | `[db-detach]` | elapsedMs | dnsMs | helloRttMs | socketReadyMs | wallTime |
|---|---|---|---|---|---|---|---|---|---|
| **#1372 이후** ×3 | **308–313** | 300 (3/3) | **0** (3/3) | 0 · 0 · 698 | 1270–1335 | 3 | 258–268 | 705–751 | 1755–1817 |
| 기준선(#1372 이전) ×9 | 1623 | 300 (9/9) | **1316** | (없던 줄) | 2440 | 5 | 510 | 1324 | 4366 |

- ✅ **예측이 그대로 맞았다** — `warmResetMs` 1316 → **0**, `startToConnect` 1623 → **≈310**
  (= ping 예산 300 만 남는다). 웜 요청당 **≈1310ms** 가 임계 경로에서 빠졌다.
- ✅ **`[db-detach] elapsedMs` 는 0 · 0 · 698** — 배경 close 의 실비용이고 프로브 간격 4000ms 보다
  훨씬 작아 정리가 겹쳐 쌓이는 징후는 없다. 🔴 표본 3건 · 무부하다. 동시성이 붙으면 다시 본다.
- 🔴 **wallTime 4366 → 1780 을 통째로 이 PR 공으로 돌리지 말 것.** 같은 창에서 핸드셰이크가
  `elapsedMs` 2440 → 1328, `helloRttMs` 510 → 266 으로 함께 빨라졌는데 이건 코드와 무관한
  **스테이징 Atlas 의 그날 편차**다(아래 *모르는 것*). 이 PR 에 귀속되는 것은 `warmResetMs`
  1316 → 0 과 그만큼의 `startToConnect` 감소뿐이다.
- 🔴 **웜/콜드 비율이 뒤집혔다 — 기준선 웜 9 : 콜드 1, 이번 웜 3 : 콜드 7.** 원인 미확인이고 이
  측정으로는 못 가린다. 콜드 아이솔레이트는 `starting` 줄에 `warmPingMs` 필드 자체가 없다(= 웜
  판정을 아예 안 탄다), 즉 **회수는 요청 전체가 아니라 웜 요청에만 적용된다.** 재현할 때 이 비율을
  먼저 세지 않으면 회수분이 과소·과대로 보인다.
- 🔴 **ping 은 여전히 3/3 전소다(`warmPingMs=300`).** 그런데 같은 요청의 `helloRttMs` 가 예산
  **안**(257–268)이므로 위 *단계 계측 판독* 이 적어 둔 "예산 < RTT" 사유는 이번 표본에서 성립하지
  않는다. 남는 설명은 그 웜 소켓이 느린 게 아니라 **죽어 있다**는 쪽이다(2026-08-16 "요청 컨텍스트가
  끝나면 그 소켓은 못 쓴다"). 🔴 그래서 `clampTimeoutMs` 하한을 내려도 재사용은 안 늘어난다 —
  하한 인하 기각(위 *남은 작업*)은 이 표본으로 한 번 더 굳는다.

## 프로덕션 계측 판독 — 미측정 값 넷을 전부 읽었다 (2026-08-31, 승격 후)

승격 run `33326529095`(`1e747a9e5`) 직후, 위와 **같은 절차**(없는 슬러그 404 를 4초 간격 10회).
이 표가 PR #1372 재측정의 **기준선**이다.

이벤트 10건 = 재사용 1 + 재수립 9 · 값은 중앙값(괄호는 범위) · `outcome=ok` 10/10 ·
`db-connect-error`·`db-op-timeout` **0건**

| | startToConnect | warmPingMs | warmResetMs | elapsedMs | dnsMs | helloRttMs | socketReadyMs | wallTime |
|---|---|---|---|---|---|---|---|---|
| 재수립 ×9 | 532 (527–638) | 300 (9/9) | **226** (221–329) | 416 (407–434) | 3 (2–5) | 76 (74–77) | 226 (219–229) | 1014 |
| 재사용 ×1 | – | `rttMs=266` | – | – | – | – | – | **329** |

- ✅ **teardown 산술이 프로덕션에서도 닫혔다** — `warmResetMs` 실측 **226ms** 로, 스테이징에서
  유도한 ≈232ms 와 6ms 차이다. PR #1372 가 임계 경로에서 뺄 값이 바로 이것이고, 남는 선행 구간은
  ping 예산 300 뿐이 된다(532 − 226 ≈ 306).
- 🔴 **`rttMs=266` — 예산 300 대비 여유 34ms.** 하한 인하 기각의 확정 근거다(위 *남은 작업*).
  참고로 이 값은 `helloRttMs 76` 의 3.5배다 — 순수 왕복이 아니라 드라이버 서버선택까지 포함한
  값이라는 뜻이므로, "RTT 가 76 이니 150 이면 넉넉하다"는 식의 환산을 하지 말 것.
- 🔴 **재사용 1건의 값이 이 작업 전체의 상한을 보여 준다** — 같은 라우트가 재수립 1014ms 대
  재사용 **329ms**다. teardown 226 을 빼도 ping 300 + 핸드셰이크 416 은 남으므로, 재수립을 계속
  하는 한 329ms 에는 못 간다. 그 위의 레버는 코드가 아니라 아이솔레이트 간 커넥션 재사용
  자체이고, 2026-08-16 실측("요청 컨텍스트가 끝나면 그 소켓은 못 쓴다")이 그 문을 닫아 두었다.
- ✅ **프로덕션 `dnsMs` 2–5ms · `helloRttMs` 74–77ms** — 스테이징 판독의 두 결론(SRV 전환 무의미,
  핸드셰이크는 전부 왕복 비용)이 프로덕션에서도 그대로다. `socketReadyMs 226 / helloRttMs 76 ≈
  3.0왕복`(TCP+TLS+SCRAM).
- 스테이징 대비: 핸드셰이크 `elapsedMs` 416 vs 2440(**5.9배**), teardown 226 vs 1316(5.8배).
  두 배율이 같다 — 스테이징의 느림은 특정 단계가 아니라 Atlas 왕복 전반이다.
...
- teardown 을 임계 경로 밖으로 내보내는 지점: `worker/lib/db.js` 의 `detachDeadWarmConnection`
- 그 순서·중첩을 값으로 고정하는 테스트: `__tests__/worker/db.warm-teardown-off-critical-path.test.js`
  (4건) · mongoose 업그레이드가 `skipCloseClient` 를 떨구면 실패하는 실계약 테스트:
  `__tests__/worker/db.mongoose-detach-contract.test.js`
- 남의 살아 있는 커넥션을 안 끊는다는 2026-08-08 가드는 여전히
  `__tests__/worker/db.warm-connection-revalidation.test.js` 담당이다(위 두 파일이 대체하지 않는다)
...
- ~~살아 있는 소켓의 ping 왕복~~ · ~~프로덕션의 `warmResetMs`·`dnsMs`·`helloRttMs`~~ — **둘 다
  측정 완료**(2026-08-31, 위 *프로덕션 계측 판독*). 스테이징에서는 여전히 못 읽는다(예산 300 <
  RTT ≈510 이라 ping 성공이 구조적으로 0건) — 이 값이 다시 필요하면 프로덕션에서 잰다.
- **PR #1372 의 프로덕션 회수분.** 스테이징은 쟀다(위 *머지 후 재측정*: `warmResetMs` 0,
  `[db-detach]` 0 · 0 · 698). 프로덕션은 아직 `1e747a9e5` 라 표본이 **0건**이고, 특히 배경 close 가
  아이솔레이트 수명 안에서 어떻게 끝나는지는 승격해야 읽힌다.
- **웜/콜드 비율이 무엇으로 정해지는지.** 같은 절차·같은 간격인데 기준선은 웜 9 : 콜드 1,
  2026-08-31 재측정은 웜 3 : 콜드 7 이었다. 회수분이 웜 요청에만 걸리므로 이 비율이 곧 체감
  개선폭인데, 무엇이 그것을 흔드는지 모른다.
- 스테이징 핸드셰이크가 프로덕션의 5.8배인 **이유**(Atlas 티어·리전·클러스터 차이 중 무엇인지).
  2026-08-31 측정에서 격차가 2.7배(1250 vs 460)에서 더 벌어졌다 — 스테이징 Atlas 가 그날그날
  다르다. 수정 방향에는 영향이 없다 — 병리는 양쪽이 같다.
