...
status: active
updated: 2026-08-31
next: **계측 판독 완료(2026-08-31, 아래 *단계 계측 판독*) — 다음은 teardown 을 임계 경로에서 빼는 별도 PR 이다.** 잔량 ≈235ms 의 정체는 `resetMongooseConnection()` 으로 확정됐다(프로덕션 ≈232ms · 스테이징 1316ms). 🔴 2026-08-08 재연결 폭주가 난 경로라 남의 커넥션을 끊지 않는 경계를 함께 좁히고 같은 PR 에서 재측정한다. `dnsMs`(2–10ms)와 핸드셰이크(전부 왕복 비용) 갈래는 닫혔다. `rttMs` 는 프로덕션에 계측을 승격해야 읽힌다 — `clampTimeoutMs` 하한 인하 재개의 유일한 선행 조건
---
...
- [x] **잔여 두 축의 분해 계측 — 완료** (PR #1366 / `70433d36b`, 판독 2026-08-31, 아래
      *단계 계측 판독*). `dnsMs` 와 핸드셰이크 갈래는 닫혔고, 잔량 ≈235ms 는 teardown 으로
      확정됐다.
- [ ] 🔴 **teardown 을 임계 경로에서 빼는 별도 PR — 착수 안 함.** `resetMongooseConnection()`
      이 요청당 프로덕션 ≈232ms · 스테이징 ≈1316ms 를 먹고, 이게 끝나야 새 커넥션 수립이
      시작된다. 🔴 2026-08-08 재연결 폭주가 난 바로 그 경로다 — "안 기다리게" 만들면서 남의
      살아 있는 커넥션을 끊지 않는 경계(`countActiveMongoOps` 가드)를 함께 좁히고, 같은 PR 에서
      아래 *단계 계측 판독* 의 절차로 재측정한다.
- [ ] **프로덕션에 계측 승격 — 사용자 판단.** `[db-ping] rttMs`(살아 있는 소켓의 ping 왕복)는
      ping 이 실제로 통과하는 프로덕션에서만 읽힌다(스테이징은 구조적으로 0건 — 아래 참조).
      그 값이 `clampTimeoutMs` 하한 300→150 재개의 유일한 선행 조건이다. 승격 명령은
...
- **살아 있는 소켓의 ping 왕복 단독 ms.** 위 하한 인하 기각을 되돌릴 유일한 근거다.
  `[db-ping] rttMs` 로 분리돼 나오지만 **스테이징에서는 ping 이 한 번도 성공하지 않아 못 읽는다**
  (예산 300 < RTT ≈510). 프로덕션에 계측을 승격해야 읽힌다 — 위 *남은 작업* 참조.
- **프로덕션의 `warmResetMs`·`dnsMs`·`helloRttMs` 실측.** 지금 프로덕션 값은 `startToConnect`
  에서 ping 300 을 뺀 **산술 유도**(≈232ms)이고, 스테이징에서 그 산술이 닫히는 것이 근거다.
  승격하면 직접 읽힌다.
