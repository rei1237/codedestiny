...
- ❌ **`rttMs` 는 못 읽었다 — 스테이징은 ping 성공이 구조적으로 0건이다.** 9/9 가
  ...
  반대로 1/10 이 재사용에 성공했으므로 RTT 가 예산 안에 있다.
...
- **PR #1372 머지 후의 실제 회수분.** 위 기대치(`warmResetMs` 한 자릿수)는 코드 경로에서 온
  **예측**이고 실측이 아니다. 특히 배경 close 가 아이솔레이트 수명 안에서 어떻게 끝나는지
  (`[db-detach] elapsedMs`)는 프로덕션 표본이 0건이다.
