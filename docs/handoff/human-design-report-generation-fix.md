...
status: active
updated: 2026-08-31
next: 차트 병목 갈래는 닫혔다. `MONGO_PING_TIMEOUT_MS` 1000→300 이 프로덕션에 올라갔고(PR #1354, 승격 `256c63b99`) 승격 후 실측에서 요청 선행 구간이 1223–1264ms → **533–545ms** 로 줄었다(아래 *승격 후 검증*). 이어서 할 일은 둘 중 하나다 — (1) `clampTimeoutMs` 하한 300 을 내려 150 까지 갈지 판단하려면 **살아 있는 소켓의 ping 왕복 단독 수치**를 먼저 재야 한다, (2) 남은 지연 축은 ping 이 아니라 Atlas 핸드셰이크 410–450ms 와 라우트 진입 오버헤드 ≈235ms 다
---
...
선행 구간 잔량 ≈235ms 는 ping 예산이 아니라 라우트 진입 오버헤드이므로 이 노브로는 못 줄인다.
