...
status: active
updated: 2026-09-16
next: 2단계 1~8번과 후속 과제 전부 완료(거리 라벨 비교 일원화·결제 P0 인벤토리 상품 목록 CI 대조 포함). 이 축에 남은 작업 없음.
---

# 숙요점 자리(役) 방향 정본 교정 — 1·2단계 + 후속 과제 전부 완료

작성: 2026-09-16 · 대상 브랜치: main (직접 커밋, PR 없음)

## 다음 세션 첫 문장

"숙요 자리 방향 축은 후속 과제까지 모두 끝났다. 상품을 추가·삭제하면
`npm run audit:payment-p0-inventory` 로 인벤토리를 재생성해 커밋해야 CI 가 통과한다."
...
### 완료 — 거리 라벨 비교 일원화 (커밋 3d055ac23)

- 실측 전제 정정: "58곳" 중 **비교는 38곳**이고 나머지는 라벨 생산처·기본값이다. 또 셸 i18n 은
  JS 값을 바꾸지 않고 DOM 텍스트 노드만 한국어 역인덱스로 복구하므로(`js/cd-lang-native.js`,
  추출기는 소스를 고치지 않는다) **현재 깨지는 경로는 없었다** — 구조적 위험만 있었다.
- 38곳을 `syDistanceTierFromLabel(label)` 경유 코드 비교(`near/middle/far`)로 바꿨다. 모든 입력에서
  같은 분기로 간다. 생산처·기본값·표시 문구는 그대로다. 워커·TSX 에는 이런 비교가 없다.
- 새 거리 분기를 쓸 때 한글 라벨을 직접 비교하지 말고 이 헬퍼를 거칠 것(가드는 없다).

### 완료 — 결제 P0 인벤토리 배선 (사용자 승인: 상품 목록만 CI 대조)

- 실측: 9/10 스냅샷이 catalog 와 크게 어긋나 있었다(상품 132 vs 158 — 영냥이 28종 누락, 폐기 2종 잔존,
  원천 파일 +65/−34, sha 변경 414).
- `npm run audit:payment-p0-inventory` 로 재생성했고, `verify:payment-p0-inventory` 가 catalog
  featureKey 집합 == 인벤토리 집합만 본다. `base`·sha256·파일 목록은 **일부러 보지 않는다**(커밋마다
  어긋나는 값이라 엄격 대조는 매 커밋 CI 실패). `pr-ci.yml` "Verify payment and auth boundaries" 스텝.
- 상품을 추가·삭제하면 CI 가 빨개진다 → `npm run audit:payment-p0-inventory` 후 `docs/payments/payment-p0-inventory.{json,md}` 커밋.
- 롤백: 해당 커밋 `git revert`.

## 아직 남은 후속 과제

- 없음(이 축). 범위 밖으로 보고만: `docs/payments/payment-inventory.json`(p0 아님)은 생성기가
  레포에 없어 여전히 수동 스냅샷이다.
