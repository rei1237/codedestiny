...
# zh-TW 번역 — 4개 gap 허브 전부 완료 (우선순위 근거 없이 사용자 지시로 진행)

## 왜

`2026-09-17-seo-p1-followup.md` P1 항목("zh-TW 4개 허브: saju, astrology, vedic,
tarot, 순서는 트래픽 근거로 정할 것")의 후속. GSC API/자격증명이 레포에 없어
실측 트래픽으로 순서를 가릴 수 없음을 사용자에게 보고했고, 사용자가 "그럼 안된
번역 작업들을 해줘야지"로 **순서를 가리지 말고 4개 전부 진행**하라고 명시적으로
지시함 — 이 문서 하단 "결정 절차"는 더 이상 유효하지 않고, 아래 상태로 대체됨.

## 지금 상태 — 전부 완료

- saju zh-tw: `ea3417fb6`(main, push 완료).
- vedic/astrology/tarot zh-tw: `70302613b`(main, push 완료).
- 4개 허브 모두 라우트 `/zh-tw/<hub>/`, hreflang 상호참조 확인,
  `verify:sitemap` OK(1269 URLs), `check:fast` 전체 통과(277 suites / 3881 tests).
...
## (참고용, 더 이상 실행 불필요) 원래 검토하려 했던 결정 절차

4개 허브가 이미 전부 끝났으므로 아래는 실행할 필요 없음. 다만 향후 다른 로케일
gap(예: en/ja 쪽 품질 개선, 새 허브 추가 시 우선순위)에 같은 상황이 생기면
재사용 가능:

1. Google Search Console에서 최근 28~90일, 페이지별 클릭수를 새로 뽑는다.
   레포에 스크립트 없음 — GSC UI 또는 API 콘솔에서 수동 조회 필요.
2. 클릭수가 유의미하게 높은 순서대로 착수한다. 표본이 여전히 작으면 순서를
   확정하지 말고 사용자에게 다시 보고한다.
3. 순서가 정해지면 아래 "재사용 패턴"을 그대로 따른다.

## 재사용 패턴 (saju/vedic/astrology/tarot 4개 허브 모두에서 검증됨)
...
4개 허브 zh-TW 클릭이 실제로 늘어나는지는 배포 후 실측해야 함 — 이 세션에서는
검증하지 못함(사후 GSC 확인은 별도 요청 시).
