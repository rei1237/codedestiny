...
status: active
updated: 2026-09-06
next: 🔴 **5차 실측의 잔여 2건을 사용자 선택대로 구현했고(ⓧ·ⓓ), 남은 것은 PR 머지 → 6차 실호출 승인이다.** ⓧ 총 분량 상한 46,000→56,000(5차 51,203자가 **넘쳐서** `degraded` + 유료 품질 고지가 나갔다) · ⓓ `validateFusionFortuneGroup` 에 한 필드 **안** 동어반복 반려(`repeated_sentence`, 30자·3회) + 반복 전용 보완 지시. 🔴 **ⓓ 는 5차의 그 사고를 직접 막지는 못한다** — 그건 잘려서 `parse_failed` 였고 검증까지 못 갔다. ⓓ 가 막는 것은 **잘리지 않고 JSON 이 닫힌 반복이 그대로 유료 배달되는 경로**이고, 5차 덤프에서 그 구멍이 실재함을 확인했다. 6차 판정 항목: ① `degraded` 0 이 되는가 ② `repeated_sentence` 가 실제로 물리는가(아니면 `parse_failed` 가 또 나는가). 🔴 **덤프 `_tmp_fusion-live/2026-09-06T08-04-31-076Z/` 는 이 워크트리와 함께 사라진다** — 재승인 없이 다시 못 만든다.
---
...
- ~~총 분량 상한 초과~~ **ⓧ 로 처리(사용자 선택)** — `total.max` 46,000→56,000. 기각: ⓨ 목표 분량 서술자 하향(#1660 을 되돌리는 방향이라 `section_depth` 미달이 다시 온다) · ⓩ 상한 초과를 `degraded` 사유에서 분리(의미는 더 정확하지만 `length` 한 사유에 하한·상한이 묶인 계약을 검증·프롬프트·mock 게이트가 함께 보고 있어 범위가 넓다). 🔴 **효과 판정은 6차 실호출이다** — mock 은 36,742자라 상한 근처를 밟지 않아 이 변경을 증명하지 못한다.
- 🔴 **astrology 반복 루프(`parse_failed`) — ⓐ 는 5차 실측에서 실패했다(머지됨, PR #1685).** 4차와 같은 형태의 동어반복이 그대로 재발했으므로 **프롬프트 문구를 더 다듬는 방향은 접는다.** 남은 후보: **ⓑ** `maxOutputTokens` 상향 — 🔴 잘림만 막고 반복은 안 막으므로 **반복 원문이 그대로 유료 배달될 위험이 오히려 커진다**(현재는 잘려서 반려되고 보완 물결이 다시 쓴다) · **ⓒ** 잘린 JSON 복구 — ⓑ 와 같은 이유로 나쁘다 · **ⓓ** `validateFusionFortuneGroup` 에 반복 사유 추가 — **사용자가 골랐고 이 세션에서 구현했다**(아래 항목). 반복을 조기에 반려해 보완 물결로 보내는 쪽이라 현재 동작과 결이 같다. 🔴 다만 **5차의 그 호출은 ⓓ 로도 안 걸린다** — 잘려서 `parse_failed` 라 검증까지 가지 못한다. ⓓ 가 닫는 것은 잘리지 않은 반복이 유료 배달되는 경로다. 아래는 ⓐ 를 넣을 때의 기록이다(브랜치 `fusion-anti-repeat-prompt`, `worker/lib/fusion-fortune-prompt.js` 두 곳): ① `lengthDirective()` 에 "같은 말을 되풀이해 채운 응답도 반려된다 · 댈 근거가 없으면 거기서 끝내고 JSON 을 닫는다"를 붙였고(분량 압박과 같은 문자열이라 프롬프트 본문·Gemini `description` 양쪽으로 흐른다), ② `SECTION_WRITING_RULES` 에 **한 섹션 안** 반복 금지를 새 규칙으로 넣었다(기존 규칙은 섹션 *사이* 반복만 다뤘다). 우려했던 트레이드오프(분량이 모자란 응답을 반복보다 낫다고 명시한 대가로 `section_depth` 탈락이 늘 것)는 **5차에서 일어나지 않았다** — 탈락 0이다. 🔴 즉 ⓐ 는 **해롭지도 않았고 듣지도 않았다.** 되돌릴 이유는 없으니 그대로 둔다.
- 🔴 **반복은 그룹 검증에서 안 걸린다** — `hasRepeatedLongSentence`(`worker/lib/fusion-fortune.js:458`)는 **조립 후 전체 품질 판정**에만 쓰여 `repeated_sentence` 로 `degraded` 강등만 시킨다. `validateFusionFortuneGroup` 의 반려 사유 목록에는 없다. 즉 반복해도 JSON 만 닫히면 그 묶음은 통과하고, 4차처럼 잘리면 `parse_failed` 로 폴백된다. 🔴 **ⓓ 로 메웠다(사용자 선택, 이 세션).** `validateFusionFortuneGroup` 에 `repeated_sentence` 반려를 넣었다 — 🔴 **`hasRepeatedLongSentence` 재사용은 불가능했다**: 그 함수는 섹션마다 `Set` 으로 중복을 지운 뒤 **세 섹션 이상**에 나온 문장만 세므로 한 섹션 안 50회 반복도 1로 읽는다. 그래서 필드 단위로 세는 `fusionGroupRepeatedKey` 를 새로 뒀다(30자·3회). 🔴 **30자·3회는 실측이다** — 5차 덤프 11건 + 결정론 폴백에서 정상은 전부 한 필드 최다 **1회**, 사고 응답만 **50회**였고, 교차 섹션 중복의 60자 잣대로는 **하나도 안 걸린다**(되풀이 문장이 30~37자). 반려된 묶음에는 분량 지시 대신 `buildFusionRepeatInstruction` 을 준다 — 반려 그룹은 `merged` 가 비어 "0자로 썼습니다"가 나가는데 그 분량 압박이 애초의 원인이라서다.
