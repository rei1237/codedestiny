...
status: active
updated: 2026-09-06
next: 🔴 **5차의 미결정 2건은 닫혔다(2026-09-06, 사용자 결정 반영).** ⓧ 총 분량 상한 46,000 → **60,000** · ⓓ 그룹 검증에 **한 필드 안 반복 사유 신설**(`findFusionRepeatedSentenceField`) + 반복으로 반려된 묶음에 분량 지시 대신 반복 금지 지시. mock 게이트 전부 통과, **6차 실호출은 사용자 지시로 안 돌렸다.** 다음 세션 첫 문장: **"초융합 6차 실호출을 1회 승인해 주면 대표 1건(`생시O 장소O`)으로 판정 ② `degraded` 0 을 실측으로 닫는다."**
---
...
- [ ] 🔴 **Phase 4 실검증 — 5회 돌렸다. 5차에서 ①③④ 충족·② 만 실패였고, 그 원인 2건은 §모르는 것에서 닫혔다(6차 미실행)**(2026-09-06). 하네스 `scripts/verify-fusion-fortune-live.mjs`(npm `verify:fusion-fortune-live`, 플래그 없으면 호출 0으로 계획만 출력). 재현: `node --env-file=<리포 루트>/.env.local scripts/verify-fusion-fortune-live.mjs --live --dump`. 사용자 지시로 조합 전수(45회) 대신 **대표 1건**(`생시O 장소O`)만 돌렸다 — 조합 커버리지는 mock `verify:fusion-fortune-delivery-floor` 가 맡는다.
...
- ~~총 분량 상한 초과~~ **해결(ⓧ)** — `total.max` 46,000 → **60,000**(5차 실측 51,203 의 1.17배). `verify:fusion-fortune-quality` 에 "상한이 5차 실측 위"를 고정했다. i18n 에 fusion 의 46,000 노출은 없었다(전수 확인: `public/i18n/*.json` 의 46,000 은 전부 마스터 러브 코덱스).
