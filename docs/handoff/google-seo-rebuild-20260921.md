...
- [x] P1: 숙요 연애·결혼 글의 근거 없는 빈도/안정성 주장을 수정하고 나크샤트라 공개 설명을 실제 계산·출처와 대조했다. 본문·메타·FAQ·생성 읽기 자료와 회귀 검사를 일치시켰다.
- [x] P1: `constants/nakshatra-fusion.js`의 27개 유료/런타임 융합 해설을 현행 크로스워크와 전수 대조하고, 다른 핵심 체계도 실제 계산·기능과 대조한다. → 2026-09-24 `18ca99c9c`(27/27 대조표 `docs/nakshatra-fusion-crosswalk-audit-20260924.md`)·`7bf374a77`(베다·점성술·나크샤트라 계산 예시를 엔진에 대조하는 테스트).
...
```text
D:\Development\code-destiny에서 D:\Development\code-destiny\docs\handoff\google-seo-rebuild-20260921.md와 CLAUDE.md를 읽고, 구현 커밋 50e6b41f8 이후 main 변경과 미커밋 작업을 보존하라. constants/nakshatra-fusion.js의 27개 융합 제목·해설을 현행 constants/nakshatra-crosswalk.js의 CROSSWALK_OFFSET=11 및 실제 계산 결과와 전수 대조하라. 과거 대표 별 대응과 서비스 계산 정렬을 섞지 말고, 불일치하면 계산 정본이 아니라 해설만 최소 수정하라. 동시 쓰기가 있으면 기존 D:\wt\seo-20260921-121321의 상태를 확인해 재사용하고, 검증·커밋·main push·CI 확인까지 완료하라. 운영 배포·유료 LLM·광고 활성화·AdSense 재신청은 실행하지 마라. 기존 워크트리 삭제 제한을 우회하지 마라.
```
