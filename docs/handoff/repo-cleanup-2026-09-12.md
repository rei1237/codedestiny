...
- [x] 4) 미러·sitemap·cachebust 커밋 요동 재설계, CSS·자산·로케일 중복 제거, 배포·SEO 체크리스트 통합, 루트 생성 보고서를 `reports/` 로 이동. **전체 완료.**
  ...
  - ✅ **배포·SEO 체크리스트 통합 완료**. 루트 6개(`DEPLOY_CHECKLIST`·`DEPLOYMENT_MODE`·`CLOUDFLARE_PAGES_SETUP` / `ADSENSE_APPROVAL_CHECKLIST`·`GOOGLE_INDEXING_CHECKLIST`·`SEO_SUBMISSION_GUIDE`)를 축별 단일 문서로 병합했다 — 배포 3개 → `docs/deploy-checklist.md`, SEO 3개 → `docs/seo/checklists.md`(구 `SEO_SUBMISSION_GUIDE.md`의 §1~8 하위 번호는 그대로 유지해 다른 문서의 `§N` 참조가 깨지지 않게 했다). `SEO_ADSENSE_AUDIT.md`는 생성물이라 통합 대상이 아니었다(이미 `reports/`, PR-10). `scripts/lib/doc-refs.mjs:24-33`의 `ROOT_REPO_PATHS`에서 `CLOUDFLARE_PAGES_SETUP.md`·`DEPLOY_CHECKLIST.md`를 제거했고, `docs/CONTEXT_AUDIT.md`의 Historical-Only References 목록에서도 뺐다(파일 자체가 없어졌으므로). `docs/seo-strategy/**` 인바운드 참조는 실측 9곳(6개 파일)·`docs/handoff/seo-indexing-2026-08-15.md` 1곳을 새 경로로 갱신했다. **sitemap 요동은 손댈 것이 없다**(확인함) — `config/sitemap-lastmod.json` 내용 서명 원장이 이미 같은 원리로 처리하고 있다.
