...
status: active
updated: 2026-09-12
next: PR-6 머지 후 5) 룰셋에 `CI required` 등록, 이어서 3) 남은 CI 최적화(Playwright 캐시·워치독·크론)
---
...
- PR-6(#1941, 브랜치 `chore/paid-gate-triggers`): paid-flow-gates 트리거 paths 구멍 15개 + 결제 PR jest 2회 실행 제거. 커밋 2개로 분리(되돌리기). 머지는 사용자.
