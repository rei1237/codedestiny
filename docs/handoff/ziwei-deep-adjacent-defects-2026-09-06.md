...
**출처**: 삭제된 `ziwei-upgrade-2026-08-27.md` §2-(B)-3 이 원 항목이다 — PR #1156 이 공용 하네스로 리팩터하면서
(출력 바이트 동일 확인) **낡은 기대값 45개는 손대지 않아** 생긴 실패다. 고칠지·은퇴시킬지·다시 뜰지는 사용자 판단이라고
그때 적혔고, 그 판단은 아직 없다.

🔴 배선하기로 정했다면 [unwired-verify-guards-2026-09-01.md](unwired-verify-guards-2026-09-01.md) 를 먼저 읽는다 —
"미배선 스크립트는 조용히 썩는다(판정 전에 그냥 실행할 것)"와 "도는 가드 ≠ 무는 가드(변이로 물게 만든 뒤 배선)"가
같은 축에서 실측된 함정이고, `verify:ziwei-deep-counseling-quality` 를 배선한 것도 그 세션이다.
