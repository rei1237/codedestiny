...
| #646 | vedic·찻집 캐시 배선 + `cache.minChars` + `attempts: 2` 4곳 | **머지됨** |
| #648 | 사주 프롬프트 불변 접두사 선두 배치 (공통 접두사 240 → 61,548자) | **머지됨** (이 문서가 그 PR 로 들어갔다) |
...
> 🔴🔴 **위 문단은 실측으로 철회됐다 (2026-08-15). 아래 §3-2 를 읽어라.**
> "할인 75%"는 **가정이었고 틀렸다.** 실제 암묵 캐싱 할인은 웨이브1에서 **0%** 다.
> 즉 슬라이싱이 무너뜨릴 캐시 자체가 없다. 다만 슬라이싱보다 나은 답이 따로 있다.

---

## 🔴 3-2. 실호출 실측 (2026-08-15, 사용자 허가 하에 Gemini 직접 호출)

프로덕션 tail 로는 답이 안 나와(창 동안 LLM 기능 호출 0건) **통제 실험**으로 관측했다. 유료 라우트를 타지 않고 Gemini REST 를 직접 불렀으므로 결제·DB 부작용은 없다. 총 14회, 입력 약 32만 토큰, 비용 **약 $0.09**.

### 실측 1 — 암묵 캐싱은 웨이브1에서 **0%** 다

| 조건 | prompt 토큰 | cached | 비율 |
|---|---:|---:|---:|
| 첫 호출 | 26,054 | 0 | 0% |
| 직후 재호출(같은 접두사) | 26,060 | 0 | **0%** |
| **병렬 5회 (프로덕션과 동일한 `Promise.all`)** | 28,003~28,129 | **0 / 5개 전부** | **0%** |
| +30초 | 26,064 | 0 | 0% |
| +90초 | 26,180 | 7,165 | 27.4% |

**암묵 캐싱은 90초쯤 지나야, 그것도 27%만 걸린다.** 사주 웨이브1은 5개 병렬로 ~40초에 끝나므로 **할인이 0이다.**

👉 **PR #648 의 접두사 재배치는 한 상담 안에서 아무 절감도 만들지 못한다.** 되돌릴 필요는 없다(공짜이고, 웨이브2 재생성처럼 90초 뒤에 오는 호출에는 부분적으로 걸릴 수 있다). 다만 **"79.5%가 캐시된다"는 기대는 사실이 아니다.**

### 실측 2 — **명시적 컨텍스트 캐싱은 99% 걸린다.** 이게 답이다

`v1beta/cachedContents` 로 공통 접두사를 한 번 캐시하고 5개 호출이 참조하게 했다:
...
합계 129,982 토큰 중 128,665 캐시 (99.0%)
```

**병렬 팬아웃에서도 100% 확정적으로 걸린다** — 암묵 캐싱과 결정적으로 다른 점이다.

### 그래서 무엇을 해야 하나

입력 비용(사주 상담 1건, `gemini-2.5-flash` 입력 $0.30/M · 캐시 입력 $0.075/M · 캐시 저장 $1.00/M·시간 기준):

| 방식 | 입력 비용 | 품질 위험 |
|---|---:|---|
| **현재** (캐시 없음, 130,014 토큰) | **$0.0390** | — |
...
측정 스크립트는 이 문서와 함께 커밋하지 않았다(일회성 실호출). 재현하려면 `buildSajuAISectionPrompt` 로 5개 프롬프트를 만들고 공통 접두사를 잘라 `cachedContents` 에 넣은 뒤 `generateContent` 에 `cachedContent` 를 실으면 된다. 🔴 **실호출이므로 사용자 허가를 먼저 받을 것.**
