...
- **사이트링크(브랜드 검색 시 하위 링크 6~8개)는 마크업으로 만들 수 없다** — 구글 자동 생성이다. 관련해 실측한 결함은 **브랜드명 표기 분열**이다(2026-08-30, `dist/` 전수): `og:site_name`·WebSite JSON-LD `name` 은 710쪽 전부 "꿀꿀 운세"인데 `<title>` 접미는 "Code Destiny" 95쪽 · "꿀꿀 만세력" 3쪽이고 **나머지 600여 쪽은 브랜드 접미가 아예 없다**. 통일은 **미착수**이며 이번 범위 밖이다.
