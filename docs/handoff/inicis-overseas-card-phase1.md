...
| 결제 후 미이행 | 지급 실패 = `GRANT_PENDING`+크론 재지급(알림 없음). vedic·ziwei 생성 실패는 2026-09-18부로 카드 자동환불 배선 완료(astrology 와 동일 패턴) | 🔴 | C7 감지·알림(자동 환불 신설 없음)은 여전히 과제. C7 알림 채널 자체의 운영 설정 여부는 오너도 미확인(추정: 미설정 가능성 높음) | `reconcile.js`·`fulfillment-alert.js`·`feedback-notify.js`, 문서 05·06 |
