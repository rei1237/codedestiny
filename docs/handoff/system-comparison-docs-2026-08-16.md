...
| 2 | `/compare/sukuyo-vs-vedic` | 숙요점 vs 베다 점성술 | 미착수 |
| 3 | `/compare/astrology-vs-myeongri` | 서양 점성술 vs 동양 명리 | 미착수 |
| 4 | `/compare/tarot-vs-saju` | 타로 vs 사주 | 미착수 |

**2번이 다음 우선순위다.** 27수를 공유하면서 해석 축이 다른 두 체계인데 한국어로 제대로 다룬
문서가 거의 없어 인용 기회가 가장 크다. `/nakshatra` 가 이미 통합 페이지로 존재해 근거도 있다.

각 주제의 실제 차이축(초안):

- **숙요점 vs 베다**: 27수를 공유하지만 숙요점은 관계·궁합 축으로, 베다는 다샤(시간 주기)와
  라시 차트 축으로 읽는다. 같은 별자리 이름이 나와도 묻는 것이 다르다.
