...
status: done
updated: 2026-09-06
next: "코드 과제 A·B 는 종결됐다. 남은 것은 \"사람이 확인해야 할 것\" 3건뿐 — 코드가 아니라 사업 판단이다"
---
...
## 코드 과제 A·B — 종결 (2026-09-06)

A·B 는 **PR #1644 로 구현돼 머지·스테이징 반영**됐고, 뒤이은 결함 수정 PR 로 마무리됐다.
아래 두 절은 **당시 스펙 원문**이며 착수 지시가 아니다 — 지금 정본은 코드다.

| 산출물 | 자리 |
|---|---|
| 판정 유틸 | `lib/payment/pass-eligibility.ts` (`describePassEligibility` · `getPassTierLabel`, 12로케일) |
| 배지 부착 | `app/components/FeatureLandingPage.tsx` |
| 공용 한도 카드 | `app/components/PassCycleCard.tsx` + `.module.css` — `/points` 와 `/points/history` 가 함께 쓴다 |
| 재발 방지 가드 | `npm run verify:pass-eligibility-copy` (`Paid Flow Gates` 스위트) |

**종결 시 함께 고친 것** — PR #1644 가 남긴 사용자 가시 결함 3건:
일본어 문구의 한글 혼입(`利用권` → `利用券`), `/points` 한도 카드의 한국어 고정,
한도 소진 문구에 대안이 빠져 막다른 길이 되던 것.

**아직 남은 것**: 아래 "사람이 확인해야 할 것" 3건(Play Console SKU 가격 · AI 원가 경제성 ·
기존 VVIP 보유자 공지) — 전부 코드가 아니라 사람의 결정이다.
...
20,000원 초과    → Code Destiny Family 이상 이용권 포함
월 한도 소진     → 이용권 적용 대상이지만 이번 달 이용 한도를 모두 사용했어요
                   + 대안(다음 이용기간에 다시 이용 / 단건 구매)
```

🔴 **정정(2026-09-06)**: 위 4번째 줄은 원래 "20,000원 초과 → 이용권 미포함 · 단건 구매" 였는데
서버 정본과 어긋난다. `PASS_LIMITS.family = FAMILY_PASS_MAX_COVERED_COIN`(999999999)이라
Family 는 **건당 금액 상한이 없다**(위 정책 표의 "상한 없음" 과 같은 말이다).
`이용권 미포함 · 단건 구매` 는 등급 자체가 없어 최소 등급을 못 정하는 경우의 문구다.
구현이 맞고 문서가 틀렸었다.
