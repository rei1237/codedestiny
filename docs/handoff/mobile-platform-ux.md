...
status: active
updated: 2026-09-08
next: 궁합 기능군부터 나머지 57개 상세페이지의 기능 근거 확인·확장을 이어가고, 정적 팝업 CSS/기존 섹션 숨김·locale 전환 상태를 별도로 확인한다. `/app/` Neo 진입은 현재 모달이 아닌 직접 링크다.
---
...
- 사용자가 상세페이지 방향을 승인했다. 기존 정본에 검증 근거를 추가하고 공통 렌더러·지연 로더·React/정적 팝업 연결 및 `/features/[slug]/`를 구현 중이다. 69개 대상 중 찻집·인생의 책·동물 도감·네오 전략실·핵심 5개 체계(사주·자미두수·숙요·베다·점성술)·관계 타로 3종(우리는 무슨 사이·재회운·말과 행동 사이)까지 12개만 검증 콘텐츠로 등록했다. 배포하지 않았다.
...
- [ ] 승인된 공통 패널을 나머지 기능으로 확장. 핵심 5개 체계와 관계 타로 3종은 구현 근거·체계별 SVG/HTML·독립 주소 320/390/430px 검증까지 완료했다. 찻집 실제 결과 예시와 궁합·초융합 등 나머지 57개 확장이 남았다. 전체 모바일 상세 스크립트는 12개 직접 진입·새로고침·OG·CTA·8개 너비·404와 `/app/` Neo 직접 링크를 통과했다. 정적 팝업 CSS/기존 섹션 숨김·locale 전환 상태는 별도 증거가 필요하다.
