...
status: active
updated: 2026-09-08
next: Neo 전략실 팝업의 브라우저 전체 회귀를 실행 제한이 없는 환경에서 완료하고, 나머지 65개 상세페이지의 기능 근거 확인·확장을 이어간다.
---
...
- 사용자가 상세페이지 방향을 승인했다. 기존 정본에 검증 근거를 추가하고 공통 렌더러·지연 로더·React/정적 팝업 연결 및 `/features/[slug]/`를 구현 중이다. 69개 대상 중 찻집·인생의 책·동물 도감·네오 전략실 4개만 검증 콘텐츠로 등록했다. 배포하지 않았다.
...
- [ ] 승인된 공통 패널을 나머지 기능으로 확장. 찻집 실제 결과 예시 추가. React 팝업 검사는 실제 유료 Neo 카드로 수정했다. Neo 독립 소개주소의 direct/OG/SVG·HTML/320~1280px 검사는 통과했으나, `/app/`의 동적 모달 상호작용 전체는 현 실행 환경의 30초 브라우저 제한으로 완료 증거가 없다.
