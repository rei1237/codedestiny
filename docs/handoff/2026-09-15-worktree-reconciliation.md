...
| `code-destiny-i18n-20260911` | 이미 반영된 운세 현지화, 옛 public 플레이어/manifest, 빌더 패치와 package 명령 중복, 손상 번역 값 | 현행 원문 해시와 맞는 라이트 노벨 초안만 content/novel/translations로 회수하고 재개/완성도 도구를 유지했다. 미완성 초안을 플레이어에 연결하지 않는다. |
...
- 라이트 노벨 en 39개, zh-CN 39개, zh-TW 2,544개 미완성. 영어 재개가 첫 요청에서 HTTP 429여서 중단했다. 유효 원본은 정식 위치에 보관하므로 오래된 워크트리를 유지할 필요는 없다.
- 언어 혼입/마커 검사와 원문 해시 일치는 편집 품질 또는 4언어×4기간의 실제 화면 검수를 대신하지 않는다.
- 실 PG, 실 LLM, 운영 DB 쓰기, 운영 배포, 외부 게시를 실행하지 않는다.
- 위 5개 과거 워크트리와 정리용 임시 워크트리는 main 통합·CI 성공 후 삭제 완료했다. 공유 node_modules는 유지했다. 후속 작업은 [남은 작업 인수인계](2026-09-15-remaining-ui-i18n-marketing.md)를 따른다.

## 검증 결과

- `npm run check:fast -- --base=8396271d9ca65428afc86969f7fdfa6952afa7df`: exit 0. 결제 게이트 88/88, 린트·타입·사이트맵·Worker 빌드·인코딩 및 Node 검사 통과. Jest 262개 스위트 / 3,559개 테스트 통과.
- 후속 main `522f7d069`·`5a852626c` 통합 후 관계 경계·결제 증빙 왕복 2개 스위트 / 93개 테스트 통과.
- 상세창 회귀 13개, 운세 24프로필×4기간×4언어 해설/근거 검사 3개 통과. 운세 일일 산출물 생성 후 canonical locale 완성도 검사 통과.
- 마케팅 이미지 352개 메타데이터 정상, 회수 원본 392개는 기존 워크트리와 바이트 동일. 같은 JPG 7쌍은 생성 도구 입력과 날짜별 전달본이므로 유지.
- 소설 완성도 검사는 의도대로 미완성으로 실패하며, 영어 재개 첫 요청 HTTP 429 후 중단했다. 미완성 번역은 발행하지 않았다.
