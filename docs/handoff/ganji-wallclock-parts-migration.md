...
- ✅ **별건-2 — 로컬 패치 스토어가 코어를 덮지 못하게 했다(2026-08-28).** `localStorage` 에
  한 번 잘못 들어간 음양력 행이 **영원히** 코어를 이기던 통로를 닫았다(재현 실측 있음).
  **검사 ⑱ 7건** 신설. 아래 "별건-2" 절.
- ✅ **별건-1 — 모달 프로필 상태가 두 번 평가돼도 구독을 잃지 않는다(2026-08-28).** 로더 셋의
  dedupe 규칙이 갈려 그 파일이 재평가될 수 있고, 그때 열려 있던 모달의 구독이 조용히
  사라졌다(재현 실측 있음). **검사 ⑲ 6건** 신설. 아래 "별건-1" 절.
  **남은 것은 별건 3번(MongoDB 박제 값) 하나뿐이고, 그것은 결제 문서라 사용자 판단이 먼저다 —
  아래 "별건-3" 절.**
...
1. ~~`modalProfileState.js` **이중 로드 가능성**~~ ✅ **재평가 쪽은 닫혔다 (2026-08-28, 별건-1).**
   아래 "별건-1" 절. 🔴 **로더 셋의 dedupe 규칙이 갈린 것 자체는 그대로다** — 공유 모듈
   변경이라 별건으로 남긴다(그 절의 "남는 것" 참조).
2. ~~`kasi:local-calendar-patch:v1` — TTL 없음 · 버전 검사 없음.~~ ✅ **닫혔다 (2026-08-28,
   별건-2).** 아래 "별건-2" 절. 야자시 축과 무관하다는 **추정도 실측으로 확인**했다.
3. 🔴 **MongoDB 박제 값(R-h) — 남아 있다.** 조사는 끝났고 **결정만 남았다**(결제 문서라
   에이전트가 임의로 정하지 않는다). 아래 "별건-3" 절.
...
## 🔴 별건-3 — MongoDB 박제 값: 조사 끝, **결정만 남았다** (2026-08-28)

**코드는 한 줄도 안 고쳤다.** 아래는 전부 소스 실측이고, 무엇을 할지는 사용자 판단이다 —
세 문서 모두 **결제 문서**이고(`accessType`·`paymentId`·`usageAppliedAt`) 잘못 건드리면
유료 산출물이 바뀌거나 LLM 재생성 비용이 난다.

### 위험이 **좁혀졌다** — 새 요청에는 안 샌다

세 갈래 전부 재사용 키가 **클라이언트가 준 `idempotencyKey`** 다. `inputHash` 는 "같은 키로
다른 입력을 쓰지 못하게" 막는 대조용이고 재사용 키가 아니다(`worker/routes/ziwei-ai.js:2307` ·
`life-book-ai.js:2362` · `sukuyo-compatibility-ai.js:1803`).

⇒ **새 요청은 항상 오늘의 엔진으로 다시 계산한다.** 박제된 값이 새 결과로 새어 들어가지 않는다.
옛 문서를 다시 여는 것은 "산 것을 다시 보는 것"이라 그 값이 그대로인 것이 오히려 계약에 맞다.

🔴 `ziwei-ai` 의 `inputHash` 는 생년월일·이름·주제만 해싱한다(`ziwei-ai.js:455-477`) —
**달력 파생값도 엔진 버전도 안 들어간다.** 그래서 해시로는 엔진 변경을 절대 알 수 없다.

### 그래서 실제로 남는 결함은 **감사 불가**다

문서만 보고 "이건 어느 엔진으로 만든 값인가"를 답할 수 없다. 후속-2 처럼 프레임이 바뀌었을 때
**어느 문서가 옛 프레임인지 골라낼 수단이 없다**(고객 문의·환불 판정에서 필요하다).

레포에 이미 답이 있다 — `humanDesignCalculationSchema.calculationVersion`
(`worker/lib/models.js:1522`, 인덱스 `{userId, inputHash, calculationVersion}`)과
`PET_ENGINE_VERSION`·vedic 의 `ENGINE_VERSION`. 🔴 그런데 `ziwei-ai`·`life-book-ai`·
`sukuyo-compatibility-ai` 세 갈래에는 그런 상수가 **하나도 없다**(`git grep` 전수, 2026-08-28).

### 선택지 (사용자 결정 필요)

1. **추천 — 감사 필드만 추가한다.** 세 스키마에 `calculationVersion`(기본값 `""`)을 넣고
   **생성 시점에만** 기록한다. 기존 문서는 안 건드리고, 재사용·과금·재생성 규칙은 **하나도**
   안 바꾼다. 위험이 가장 낮고 "어느 문서가 옛 프레임인가"를 바로 답하게 해 준다.
   🔴 다만 그 버전을 **누가 올리는가**를 같이 정해야 한다 — 손으로 올리면 반드시 잊는다.
   `verify:solar-term-frame-kasi` 의 `CONTEXT_LOGIC_LOCK` 처럼 **엔진 소스 지문 lock** 을 붙여
   지문이 바뀌면 CI 가 버전을 올리라고 실패시키는 것이 이 레포의 기존 방식이다.
2. 재사용 키에 버전을 넣는다(휴먼 디자인과 같은 모양). 🔴 **과금 정책 변경이다** — 엔진이
   바뀌면 옛 문서로 재열람이 안 되므로 새 결제가 될 수 있다. 결제 문서 필독 대상.
3. 아무것도 안 한다. 위 "새 요청에는 안 샌다"가 근거이며, 감사 불가만 감수한다.

### 🔴 미측정 (부정 단언 금지)

- 실제 프로덕션 문서 중 옛 프레임으로 만들어진 것이 **몇 건인지 안 셌다** — 프로덕션 DB 읽기는
  하지 않았다(규칙 2).
- 세 갈래 말고 다른 컬렉션에도 달력 파생값이 박혀 있는지는 **이 조사 범위 밖**이다.
  본 절은 원 계획이 지목한 세 갈래만 봤다.

## ✅ 별건-1 — 두 번 평가돼도 모달 프로필 상태가 살아남는다 (2026-08-28)

**오늘 화면 값 변화 0.** 바뀌는 것은 "그 파일이 두 번 평가되는 순간"의 동작뿐이다.

### 이중 로드가 왜 가능한가 — 로더 셋의 dedupe 규칙이 갈렸다 (실측 2026-08-28, 소스 전수)

| 로더 | 쓰는 URL | dedupe 근거 |
|---|---|---|
| 체인 `__cdLoadScriptOnce`(`index-inline-runtime.js:2196`·`:8080`) | `…modalProfileState.js?v=build-<hash>` | DOM 을 훑고 **`?v=` 를 무시**한다(파일명까지 본다) |
| `__loadScriptOnce`(`uiBindings.js:118`) | 무버전 | DOM 을 훑지만 **정확한 문자열** 비교라 `?v=` 붙은 체인 태그를 **못 본다** |
| `cd-lazy-feature-loader`(`index.html:19894` 의 `data-cd-lazy-src`) | 무버전 | **DOM 을 아예 안 보고** 자기 `loadedSrcMap` 만 본다 |

⇒ 체인이 **먼저** 돌면 나머지 둘은 태그를 하나 더 심고 그 파일이 다시 평가된다.
반대 순서(무버전이 먼저)면 체인이 파일명으로 알아채므로 안 는다 — 즉 **순서 의존**이다.
🔴 브라우저에서 실제로 어느 순서가 얼마나 자주 나는지는 **여전히 미측정**이다. 이 PR 은
그 순서 질문 자체를 무의미하게 만든다.

### 무엇이 터졌었나 (재현 — 셸 하네스, 같은 소스를 두 번 평가)

```
정정 전  같은 인스턴스인가: false   ·  첫 평가에 건 구독으로 전달된 dispatch: 0회 (예외 없음)
정정 후  같은 인스턴스인가: true    ·  전달: 1회
```

`_ModalProfileState` 는 `_subs` 를 클로저에 담은 IIFE 라, 다시 평가되면 **빈 `_subs` 를 가진 새
인스턴스**가 전역을 갈아치운다. 그러면 열려 있던 모달의 구독이 조용히 사라진다 — 예외도 로그도
안 남는다. 소비자는 `openSukuyoModal`·`openZiweiModal`·`openAstroModal`
(`index-inline-runtime.js:8113`·`:8412`·`:8480`)이고, 프로필을 바꿔도 다시 안 그려진다.

### 바꾼 것

| 파일 | 무엇 |
|---|---|
| `js/core/saju/modalProfileState.js` | `var _ModalProfileState = window._ModalProfileState \|\| (function () {…})()` — 이미 있으면 그것을 쓴다. 왜 두 번 실행될 수 있는지 머리주석에 로더 3벌을 적었다 |
| `scripts/verify-shell-korean-calendar.mjs` | **검사 ⑲ 6건** 신설 |
| `public/**` 외 | `npm run sync:public` 산출물 |

### 검사 ⑲ — 격리된 컨텍스트에서 **두 번 평가한다**

- 이 가드 안에 둔 이유: 바로 위 ⑭ 가 그 파일의 **무버전 참조 2곳**을 이미 못박고 있다.
  이 검사는 그 사실의 **결과**를 지킨다.
- 프로세스 전역을 안 건드리려고 `vm.createContext` 로 격리해 평가한다.
- 🔴 판별력 자기검사가 검사 안에 있다 — 가드를 걷어낸 소스를 그 자리에서 만들어 평가해
  **인스턴스가 갈리고 전달이 0회가 되는지**를 확인한다. 그 문자열을 못 찾으면(모양이 바뀌면)
  그것도 실패다.

### 음성 테스트 1종 (fail-closed · 바이트 백업으로 복구, 복원 후 95건 초록)

| 심은 것 | 빨강이 된 검사 |
|---|---|
| 소스에서 재평가 가드 제거 | `⑲ 두 번 평가해도 인스턴스가 갈리지 않는다` + `⑲ … 구독이 산다` + `⑲ 판별력 검사의 전제가 성립한다` |

### 🔴 남는 것 (부정 단언 금지)

- **로더 셋을 통일하지 않았다.** `__loadScriptOnce` 가 `?v=` 를 떼고 비교하게 하거나
  `cd-lazy-feature-loader` 가 DOM 을 보게 하는 것이 근본 수정인데, 둘 다 **모든 지연 스크립트**에
  영향이 가는 공유 모듈 변경이다(원칙 7 — 회귀 위험 선보고 대상). 별건으로 남긴다.
- 같은 이유로 **다른 무버전 지연 스크립트들도 두 번 평가될 수 있다.** 이 PR 은
  `modalProfileState.js` 하나만 안전하게 만들었다 — 다른 파일의 재평가 안전성은 **미검사**다.
  ⑭ 이 무버전 참조 목록을 fail-closed 로 들고 있으므로 그 목록이 그 조사의 출발점이다.
- 브라우저에서의 실제 로드 순서·빈도는 여전히 **미측정**이다.

## ✅ 별건-2 — 로컬 패치 스토어가 **코어를 못 덮게** 했다 (2026-08-28)

**오늘 화면 값 변화 0**(스토어가 비어 있거나 코어와 같은 방문자 기준). 값이 바뀌는 것은
**어긋난 행을 들고 있는 방문자뿐**이고, 그 사람의 값이 코어로 돌아오는 것이 이 PR 의 목적이다.

### 재현 (실측 2026-08-28, 셸 하네스)

```
eng.registerCalendarReference({ solar: 2000-06-15, lunar: 2000-05-15 })   // 코어는 05-14
  정정 전  solarToLunarFromParts → 2000-05-15   (코어를 덮는다)
           lunarToSolar(2000-05-15) → 2000-06-15 (역방향도 덮는다)
  정정 후  solarToLunarFromParts → 2000-05-14   (코어 값)
           저장소 → {"solarToLunar":{},"lunarToSolar":{}}  (그 행이 지워졌다)
```
...
| 시점 | 무슨 일 |
|---|---|
| 2026-03-14 (`3b800d3bb`) | 키 `kasi:local-calendar-patch:v1` 도입. 만료도 버전 검사도 없음 |
| 2026-03-14 ~ 08-27 | `resolvePrimaryCalendarContext` 가 컨텍스트의 음력을 **그대로** 등록. 그 값의 출처는 KASI 응답이거나 lunar-javascript(중국 표준시) 프레임일 수 있었다 |
| 2026-08-27 (`54583b444`) | 서비스에 `_applyCoreCalendarCorrection` 이 생겨 컨텍스트 음력이 코어로 정정되기 시작 |

⇒ 그 사이에 쓰인 행은 지금도 방문자 `localStorage` 에 남아 있고, `_getPatchedSolarToLunar` 가
코어보다 **먼저** 읽히므로 계속 이긴다. `localStorage` 에는 만료가 없다.

### 바꾼 것

| 파일 | 무엇 |
|---|---|
| `js/saju-engine.js` | `_getPatchedSolarToLunar`·`_getPatchedLunarToSolar` 가 읽는 자리에서 코어와 대조해, 어긋나면 그 행을 **버리고 저장소에서도 지운다**(`_dropKasiPatchRow`). 코어가 못 답하는 구간(1900~2100 밖)에서는 그대로 답한다 |
| `scripts/verify-shell-korean-calendar.mjs` | **검사 ⑱ 7건** 신설 |
| `public/**` 외 | `npm run sync:public` 산출물 |

🔴 **TTL 을 두지 않았다.** 이 검사는 시간이 아니라 **값**을 본다 — 오래돼도 맞는 행은 살리고,
방금 쓰였어도 틀린 행은 죽인다. 버전 키(`:v2`)로 통째 폐기하는 방법도 있었지만 그것은 한 번만
듣고 다음 세대에 같은 구멍이 다시 열린다.

🔴 **쓰기 쪽에는 검사를 넣지 않았다**(중첩 사전검사 금지 — CLAUDE.md 원칙 6). 읽는 자리 하나가
관문이라 못 쓴 행이 들어와도 **쓰이지 않고 지워진다.**

### 검사 ⑱ — 소스 문자열이 아니라 실제 조회를 본다

- ⑱ 코어와 **같은** 행은 그대로 서빙된다(`source: kasi_primary` 로 확인) — 스토어를 죽이는 것이
  목적이 아니다. 이 줄이 없으면 "전부 막기"로도 초록이 된다.
- ⑱ 🔴 어긋나는 행은 **양방향** 모두 코어를 못 덮는다.
- ⑱ 🔴 어긋나는 행은 저장소에서 지워진다(다음 방문에도 안 남는다).
- ⑱ 코어 범위 밖(1850년) 표본에서는 스토어가 **계속 답한다** — 기능을 지운 게 아니라는 증거.
  그 표본이 실제로 범위 밖인지도 코어에 물어 단언한다.

### 음성 테스트 3종 (전부 fail-closed · 바이트 백업으로 복구, 복원 후 89건 초록)

| 심은 것 | 빨강이 된 검사 |
|---|---|
| 양력→음력 검사를 정정 전으로 되돌림 | `⑱ … 양력→음력을 못 덮는다` + `⑱ … 저장소에서 지워진다` |
| 음력→양력 검사를 정정 전으로 되돌림 | `⑱ … 음력→양력도 못 덮는다` |
| 맞는 행까지 버리게 함(스토어 사망) | `⑱ 코어와 같은 행은 그대로 나온다` + `⑱ 코어 범위 밖에서는 계속 답한다` |

### 🔴 남는 것 (부정 단언 금지)

- **야자시 축과 무관**하다는 것은 이제 추정이 아니라 실측이다 — 스토어는 야자시 시프트가
  끝난 **뒤의** 날짜로 조회되고(키에 시각이 없다), 그 키마다 코어와 대조된다. 2000-06-15 23:30
  표본에서 `yaja:false → 5/14`(코어 D) · `yaja:true → 5/15`(코어 D+1) 로 확인했다.
- **스토어 크기 상한은 여전히 없다.** 방문한 날짜마다 2행씩 는다(행당 100바이트 안팎).
  실사용에서 문제가 된 적은 없고 **측정도 안 했다** — 별건으로 남긴다.
- `resolvePrimaryCalendarContext` 는 폴백으로 만든 음력도 `source: 'kasi_primary'` 로 등록한다.
  이 PR 이후 **값은 안전하지만 라벨은 여전히 출처를 정확히 말하지 않는다**(미수정).
...
### 🔴 R4 의 **TZ 축**은 PR-2 가 아니라 검사 ⑪-h 가 잡는다 (2026-08-29)

PR-2 의 검사 ⑪ 은 러너를 `TZ=UTC` 로 못박은 채 돈다. 그래서 "`atLocal` 이 **브라우저 타임존의
서머타임 구멍에서** 접히는가" 라는 R4 의 원래 문장은 ⑪ 만으로는 여전히 안 재고 있었다.
KASI 커버리지(2000~2028)의 절입 시각이 우연히 어느 존의 구멍에 떨어지는 일도 사실상 없다.

실측(2026-08-29). 픽스처의 2005 실제 KASI 행 24개(우측 공백 포함 `kst`)에 **그 존의 구멍
벽시계를 절기 시각으로 밀어 넣은 행**을 더해 존마다 먹였다:
...
🔴 **1회 실측으로 끝내지 않았다.** 검사 **⑪-h**(`verify:solar-term-frame-kasi`)가 자식 프로세스
하나를 `TZ=America/New_York` 로 띄워 같은 입력을 먹이고 UTC 부모와 대조한다 — 검사 4건(하네스
생존 · 자식 TZ 가 실제로 UTC 가 아님 · 구멍이 표본에 실제로 들어감 · 본체 대조). 음성 테스트
실측: `_partsOf` 를 로컬 `Date` 왕복으로 되돌리면 **다른 검사 61건이 초록인 채 ⑪-h 만
빨개진다** — 이 축을 보는 가드는 레포에 ⑪-h 하나뿐이라는 뜻이다.
...
| R4 | ✅ **닫혔다(PR-C)** — `atLocal` 은 `_partsOf`+`_partsToIsoLocal` 로만 만들어져 Date 왕복이 없다. 🔴 **그런데 같은 자리에 더 큰 것이 있었다** — 셸이 KASI 의 시각 필드 `kst` 를 아예 안 읽어 절입 시각을 전건 자정으로 뭉갰다 | 403 이 풀리기 전에는 실응답 모양을 만나 볼 수 없었다 | ✅ **PR-2 가 수정 + 검사 ⑪**(`verify:solar-term-frame-kasi`). 위 "PR-2" 절. 🔴 **TZ 축은 2026-08-29 의 검사 ⑪-h 가 따로 잡는다** — 아래 |
