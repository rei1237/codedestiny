...
1. 🔴 **`zwFlowGanji`(js/saju-engine.js:24300)의 프로브 시각이 틀렸다.** `(year, 2, 4, 12:00)` 는
   **입춘보다 이른 해가 71해 중 40해**다. 지금은 `getGanjiFromParts` 가 null 이라 1984 기준
   산술식 폴백이 답해서 안 드러나고, null 이 값이 되는 순간 그 40해의 자미 유년 세차가
   **한 해 뒤로 밀린다**. 고치는 자리는 절기표가 아니라 그 프로브 시각이다.
