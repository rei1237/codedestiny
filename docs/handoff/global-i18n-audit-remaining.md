...
> 🔴 **2026-08-24 추가**: 정적 셸 마커 표면(마크업 ↔ `ko.json`)은 이 문서와 별개로 끝났고 fail-closed 가드가 섰다. 그 스윕의 실측·함정·남은 일은 [locale-sweep-2026-08-24.md](locale-sweep-2026-08-24.md) 에 있다.
...
- ~~**알려진 잔여 이슈**: `CodexReportOutro.tsx`의 `codexAccessOutroLine`이 로케일 불문 한국어 문장을 반환해, 비-한국어 사용자가 한 문단 안에서 "한국어 + 로케일화된 문장" 혼합을 본다.~~ **해소(2026-08-25)** — 문장을 `_lib/copy.ts`의 `reportOutroAccessLine`(ko·en·ja·zh-CN·zh-TW)으로 옮기고 `codexAccessOutroLine`은 삭제했다. 판정(`codexAccessLabel().showPrice`)은 `data/premium.ts`에 그대로 남아 있으므로 그 파일의 **콘텐츠 번역이 선행될 필요는 없었다** — 필요한 것은 판정과 문장을 가르는 일이었다.
...
### 3. Wave 7 — alt/aria-label/title 하드코딩 한국어(+ 확대 스코프) — ✅ 2026-08-23 사실상 종료(아래 최신 절 참고). 아래는 종료 전까지의 이력.
...
### 🔴 2026-08-23 재실측 — "3건 이하" 63개 중 상당수가 진짜 미착수가 아니다

neo-operation-room 클러스터 완료 후 이어서 "3건 이하" 그룹에 착수하려고 `main`(4개 PR 머지 반영 후) 기준으로 Grep 패턴을 다시 돌려 표본을 열어본 결과, 두 가지가 이 카운트를 실측보다 부풀리고 있었다:

1. **열려 있는 PR이 같은 파일을 이미 건드리고 있다** — `main` 은 아직 그 PR 의 수정을 안 받았으니 grep 에 그대로 잡힌다. 이번에 직접 대조 확인한 것만: `LifeBookAiResultClient.tsx`/`KarmaDestinyAiResultClient.tsx`/`SectionTabs.tsx`(karma)/`BookOpenCover.tsx`/`LoveSecretChecklist.tsx`(PR #943), `PromptHubClient.tsx`(#947), `SunHealingTarot.tsx`(#976), `AppChrome.tsx`(#954), `NamingAiClient.tsx`(#945), `YeonStarHugClient.tsx`(#946), `SeoLandingBirthForm.tsx`/`MobileBottomNav.tsx`(#951), `tarot/mindscan/page.tsx`(#972), `app/saju/animal-destiny/page.tsx`(#989) — **13개 파일이 이미 다른 열린 PR 소관**이다. 착수 전에 반드시 `gh pr list --state open --search "<파일명 or 키워드> in:body"` 로 먼저 대조할 것 — 그러지 않으면 이미 고쳐지고 있는 파일을 중복 작업하게 된다.
2. **더 근본적인 것 — grep 패턴 자체가 "앱 UI 크롬"과 "한국어 전용 콘텐츠 페이지"를 구분 못 한다.** 직접 열어 확인한 사례: `app/compare/{fortune-apps,sukuyo-vs-vedic,saju-vs-ziwei}/page.tsx`(비교 문서 3편, 로케일 훅 전무, 본문 전체가 국내 앱 이름·검색어를 겨냥한 한국어 전용 SEO 카피), `app/destiny-compass/page.tsx`(가이드 링크 nav, 동일 패턴), `app/stories/[episode]/page.tsx`(연이의 운명 노벨 — 웹소설 리더, 회차 자체가 한국어 산문이라 로케일 개념이 없음). 이 파일들은 전부 `aria-label` **하나만** 걸렸는데(예: "비교 문서 관련 링크", "위치", "화 이동"), 그 라벨 하나만 번역하고 주변 본문(가이드 링크 텍스트, 앱 이름, 소설 회차 제목)은 전부 한국어로 남기면 **"이 페이지는 다국어를 지원한다"는 거짓 신호**만 만든다 — 이 세션 내내 지켜온 "형제 필드 뒤섞임 금지" 원칙과 결이 같다. 이 부류는 Wave 7(앱 UI 크롬) 대상이 아니라, 애초에 다국어로 설계되지 않은 콘텐츠다 — **Wave 9(hreflang/장문 콘텐츠)에도 안 맞고, `app/page.js`처럼 영구 제외 후보로 보는 게 맞다.** 다음 세션이 "3건 이하" 목록에서 파일을 고를 때는 **먼저 그 파일에 `getCurrentLoadingLocale`/`"use client"`/실제 인터랙티브 기능이 있는지 확인**하고, 없으면(=정적 서버 컴포넌트 + 검색어 타깃 카피) 이 항목으로 분류해 스킵할 것.
3. **진짜 미착수·인스코프이지만 훨씬 더 크다는 게 실측으로 확인됨**: `app/components/FeatureMarketingDetailModal.tsx`(749줄, 한국어 매칭 208건, `"use client"` 있음, 로케일 훅 없음)는 사이트 전역 기능 카드에서 여는 마케팅 상세 모달로 실제 인터랙티브 UI다. 파일 자체 주석(34~35행)이 "정본은 정적 셸 index.html 의 `FEATURE_MARKETING_COPY` 이며 여기는 `/app` 허브용 사본"이라고 명시해서 셸 쪽부터 실측했다. **결과: `public/en/index.html`의 `FEATURE_MARKETING_COPY`(35227행~)를 직접 열어 대조한 결과 `public/index.html`(한국어 정본)과 완전히 동일한 한국어 텍스트다 — "영어" 셸 파일인데 이 데이터셋만 번역이 전혀 안 돼 있다.** 즉 이 항목은 리액트 컴포넌트 하나의 로케일 배선 문제가 아니라, **정적 셸 5개 로케일 파일(`index.html` 기준 `public/{en,ja,zh,zh-tw}/index.html`) 전체에 걸쳐 있는, 지금까지 이 프로젝트가 본 것 중 가장 큰 미번역 데이터셋**이다(자미두수/작명/숙요점/점성술/카르마 등 기능별 headline·subheadline·painPoints·unlockBenefits·faq 등 수십 개 항목 × 5개 파일). CLAUDE.md의 "새 셸 카피 = 12개 로케일 수작업" 경고와 "자동 번역기(Gemini) 사용 금지" 원칙이 정면으로 걸리는 규모라, **이 항목은 세션이 임의로 착수하지 말고 사용자에게 먼저 우선순위·범위(전체 5개 셸 파일 + React 사본을 다 맞출지, 아니면 이번엔 React 컴포넌트만 EN 한 언어로 시작할지)를 확인받을 것.** 착수하게 되면 셸 쪽과 React 사본이 갈라지지 않도록 두 곳을 같은 PR 또는 명시적으로 순서를 밝힌 스택으로 묶어야 한다.

### 🔴 2026-08-23 (같은 날 후속 재실측) — Wave 7 "3건 이하" 목록을 실제로 전수 확인한 결과, 사실상 종료로 재분류한다

위 1·2번 발견 이후 `main`에 남은 비-admin 25개 파일을 전수(코드 직접 열람 기준으로) 재확인했다. 결과:

- **13개는 이미 다른 열린 PR 소관**(위 1번 목록).
- **나머지 대부분(`app/compare/*` 3개, `app/destiny-compass/page.tsx`, `app/stories/[episode]/page.tsx`, `app/oracle/rune/page.tsx`, `app/saju/animal-destiny/page.tsx`, `app/saju/destiny-meeting-place/page.tsx`, `app/saju-fpti/page.tsx`, `app/tarot/mindscan/page.tsx`, `app/insights/famous-saju/[slug]/page.tsx`) 은 전부 같은 모양이다** — 서버 컴포넌트 `page.tsx`가 SEO 메타데이터만 내보내고, 본문은 `<section className="sr-only" aria-label="...">`(스크린리더 전용 안내) 또는 눈에 보이는 `<nav aria-label="...">`(관련 링크/차트 aria-label) **하나**만 걸렸는데 그 라벨 주변 전체가 애초에 한국어 전용으로 설계된 콘텐츠다(검색어 타깃 SEO 문서, 웹소설 리더, 유명인 사주 매거진 등). `famous-saju`의 경우 `famousSajuInsightText()`라는 번역-키 조회 함수가 있어 언뜻 로케일 시스템처럼 보이지만, 실제로는 `FAMOUS_SAJU_INSIGHT_TEXT_TRANSLATIONS`에 **`ko` 키 하나만 있고 로케일 분기가 전혀 없다** — 콘텐츠 관리용 키 인다이렉션일 뿐, i18n 시스템이 아니다(실측 확인, 추정 아님). **이 부류는 Wave 7 대상이 아니라 위 2번 항목과 동일하게 영구 제외로 분류한다.**
- `app/saju/destiny-meeting-place/page.tsx`는 추가로 `DESTINY_MEETING_PLACE_METADATA_COPY`에 `ko`/`en`/`ja`/`zh` 4개 로케일 메타데이터(title/description)를 이미 갖고 있으면서 `const metadataCopy = ....ko;`로 **항상 ko만 읽는다** — 겉보기엔 "번역해놓고 안 쓴 버그"처럼 보이지만, 이 패턴이 `app/psychotest`·`app/tarot/self-esteem`·`app/tarot/healing`·`app/saju/sibyl`·`app/palm-reading` 등 **최소 18개 파일에 동일하게 반복**된다(전부 `.ko`만 읽음). 이 사이트는 미들웨어 없이 클라이언트 전용 로케일 감지(`getCurrentLoadingLocale()`)로 돌아가고, `generatePageMetadata`/`buildWebsiteJsonLd` 등 공용 SEO 헬퍼도 브랜드 엔티티를 로케일 무관 한국어로 고정하는 게 기존 결정(과거 `@id` 드리프트 사고 이후)이라, 이 반복 패턴이 "고쳐야 할 버그"인지 "메타데이터는 서버 렌더 시점에 방문자 로케일을 알 방법이 없어 의도적으로 ko 고정"인지 **미검증**이다 — 고치려면 `cookies()`로 서버 컴포넌트에서 로케일 쿠키를 읽어 정적 렌더를 동적으로 바꿔야 하는데, 이는 18개 파일 전체의 캐싱·성능 트레이드오프에 영향을 미치는 아키텍처 결정이라 **세션이 임의 착수하지 말고 사용자 확인이 먼저** — FeatureMarketingDetailModal 항목과 같은 급으로 취급한다.
- **결론: Wave 7(알려진 in-scope 앱 UI 크롬 alt/aria/title 하드코딩)은 사실상 종료다.** 남은 항목은 전부 (a) 이미 다른 PR이 처리 중이거나, (b) 애초에 다국어로 설계되지 않은 콘텐츠(영구 제외), (c) 서버 메타데이터 아키텍처 결정이 필요한 별도 과제(사용자 확인 필요) 셋 중 하나다. 다음 세션이 이 문서에서 "Wave 7 계속"을 찾아 시작하지 않도록 이 절이 최신 결론이다.
