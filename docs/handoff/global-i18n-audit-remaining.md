...
- 🔴 **판정 완료(이번 세션에서 `dialogues.ts` 210줄 전체를 읽고 확인)**: 이건 YeonStarHugClient·fortune-tea-house 류 "LLM/엔진 생성 콘텐츠"가 **아니다** — "네오"라는 고정 캐릭터(무뚝뚝한 조교/작전관 톤)의 **유한하고 열거 가능한 대사 세트**(`hero`/`entrance`/`methodSelect`/`topicSelect`/`intensitySelect`/`loading` 등 카테고리별 몇 줄씩)라, 이 세션에서 이미 번역한 "로딩 단계 문구"·"히어로 태그라인" 류와 본질적으로 같은 종류의 **번역 대상 UI 콘텐츠**다(단, 캐릭터 톤을 각 언어에서 살려야 하는 부담은 있음). `topicSelect`의 객체 키 자체가 한국어 리터럴("연애 / 재회" 등)이라 이 부분은 로케일 전환 시 안전한 슬러그 키로 바꾸는 리팩터가 선행돼야 한다(destiny-compass의 `LUCKY` 템플릿 슬러그화 전례와 동일 기법).
- **다음 세션 착수 순서 제안**: (1) `lib/cms/build-text.ts`가 로케일을 어떻게 다루는지 먼저 읽는다(`cmsText(namespace, key, field, fallback)` 시그니처로 보아 fallback 인자가 이 세션 표준 `copy.*` 역할을 할 가능성이 높다 — CMS 오버라이드가 있으면 그 값을, 없으면 fallback 원문을 쓰는 구조로 추정되나 로케일 분기가 그 안에 있는지는 미확인). (2) `topicSelect` 객체 키를 슬러그로 교체. (3) `dialogues.ts`/`method-registry.ts`/`input-flow.ts` 데이터부터 12로케일(또는 이 세션 표준인 en/ja/zh-CN/zh-TW+EN폴백) 전환. (4) 그 다음에야 3,127줄+1,512줄 페이지 2개의 UI 크롬 배선 — 데이터 계층을 먼저 로케일화해야 페이지가 그 위에서 조회만 하면 되므로 순서가 중요하다.
