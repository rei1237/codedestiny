...
- 🔴 **아직 안 건드린 것(다음 클러스터 후보)**: 폼/버튼/방식-선택 카드(`methodCardCopy`)/결제 게이트 상태 메시지(`이용권 확인 중` 등)/브리핑 결과 화면 등 나머지 ~300줄. `data/method-registry.ts`·`data/input-flow.ts`(아직 안 읽음)와 `NeoOperationRoomResultPage.tsx`(1,512줄, 전혀 착수 안 함)도 남아 있다. `topicSelect`의 객체 키가 한국어 리터럴("연애 / 재회" 등)인 채로 유지됨(내부 lookup 키로만 쓰이고 화면에 안 보이는 값이라 이번엔 안 건드림 — nakshatra dasha 등 이 세션의 "내부 상태값은 유지, 표시 라벨만 로케일화" 전례와 동일 판단, PR #981 본문 참고).
