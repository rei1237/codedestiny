...
3. **진짜 미착수·인스코프로 확인된 것 하나**: `app/components/FeatureMarketingDetailModal.tsx`(749줄, 한국어 매칭 208건, `"use client"` 있음, 로케일 훅 없음) — 사이트 전역 기능 카드에서 여는 마케팅 상세 모달로 실제 인터랙티브 UI다. 단, **착수 전 반드시 알아야 할 것**: 파일 자체 주석(34~35행)이 "정본은 정적 셸 index.html 의 `FEATURE_MARKETING_COPY` 이며 여기는 `/app` 허브용 사본"이라고 명시한다 — 즉 이 파일의 `CATEGORY_COPY`/`EXPLICIT_COPY`(약 250줄짜리 한국어 마케팅 카피 테이블 2개)가 정적 셸(`index.html`, 그리고 `public/{en,ja,zh,zh-tw}/index.html` 등 로케일별 사본)의 동일 데이터와 **중복 유지**되고 있다. 이 파일만 로케일화하면 셸의 사본과 어긋날 위험이 있고, 반대로 셸 쪽 로케일 처리 방식(각 로케일 파일이 이미 자체 번역을 갖고 있는지, 아니면 전부 한국어 고정인지)을 먼저 확인하지 않으면 잘못된 패턴을 이 파일에 이식하게 된다. **다음 착수 권장 순서**: (1) `public/{en,ja,zh-CN 또는 zh,zh-TW 또는 zh-tw}/index.html` 에서 `FEATURE_MARKETING_COPY` 가 실제로 로케일별 값을 갖는지 실측 확인 → (2-a) 이미 번역돼 있으면 그 값을 그대로 이 파일의 `getLocalizedFeatureMarketingCopy(locale)` 류 함수로 옮겨오고 → (2-b) 셸 쪽도 한국어 고정이면 이 세션의 표준 패턴(`getX(locale)` + EN 폴백)으로 이 파일부터 새로 번역하되 **셸 쪽 동기화 여부를 사용자에게 먼저 확인**할 것(문서의 "새 셸 카피 = 12개 로케일 수작업" 경고 참고). 규모상 단독 PR(또는 2개로 분할: 모달 UI 크롬 vs 마케팅 카피 데이터) 대상.
