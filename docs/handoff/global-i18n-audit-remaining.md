...
9. **PR #949** `chore/feedback-page-i18n` — `/feedback`(버그 제보실) 전체(상단 내비·히어로·카테고리 그리드·폼·환경정보 고지·첨부 드롭존·성공화면·로그인 게이트 + `categories.ts`/`draft.ts`/`attachmentUpload.ts`/`api.ts`). `categories.ts`를 구조(`FEEDBACK_CATEGORIES_META` — id·이모지·필드 종류만, `worker/lib/feedback-models.js`의 enum과 매칭되는 id는 불변)와 카피(`buildFeedbackCategories(copy)`)로 분리 — 10개 카테고리×약 20개 필드 분량은 프롬프트허브와 동일하게 ko+en 정본, 나머지 10개 로케일은 en 폴백. `draft.ts`의 `formatSavedAt()`이 `"ko-KR"` 하드코딩이던 버그 발견·수정. 🔴 **관리자 화면이 `details[].label`을 그대로 렌더한다는 것을 발견** — 제보자 로케일로 라벨을 번역하면 관리자가 제보마다 다른 언어 라벨을 보게 돼, 제출 payload용 라벨만 항상 한국어인 별도 카테고리 목록(`FEEDBACK_COPY_KO_CATEGORIES`)을 따로 만들어 화면 표시와 분리했다 — `environment.ts`의 `entries[].label`도 같은 이유로 손대지 않고 그대로 둠(이쪽은 원래부터 한국어 고정).
10. **PR #950** `chore/native-app-i18n` — 네이티브 안드로이드 앱 웹뷰 홈(`app/app/page.tsx`, `MobileAppActions.tsx`, `store/AppPassStoreClient.tsx`, `store/page.tsx`). `page.tsx`가 `metadata`를 export해야 해서 인터랙티브 본문을 신규 `AppHomeClient.tsx`(클라이언트 컴포넌트)로 분리하고 `page.tsx`는 얇은 서버 셸로 남김 — `store/page.tsx`도 동일 패턴으로 정적 헤더를 `AppPassStoreClient.tsx` 안으로 이관. `listServiceFeatures("ko")` 하드코딩(레지스트리 자체는 이미 12로케일 지원) 발견·수정, `buildBenefits()`의 `toLocaleString("ko-KR")` 하드코딩도 함께 수정. `PASS_PLANS`/`SPOTLIGHT_FEATURES`는 `passTier`/`productId`/`slug`/`featureKey`(Play Console·`FeatureMarketingDetailModal`의 `explicitCopy()` 조회 키) 등 구조적 식별자는 그대로 두고 표시 문구만 카피로 분리. `FeatureMarketingDetailModal.tsx` 자체(한국어 키워드 분류 정규식이 딸린 대규모 마케팅 카피 레지스트리)와 공유 앱 셸(`AppShell.tsx`/`PurchaseRecoveryBoot.tsx`, `/app/*` 전체가 공유)은 이번 클러스터 범위 밖으로 명시적으로 제외.
...
**아직 안 한 것**: PR #940~950(문서 PR #948 제외 9개 코드 PR) 전부 push·PR 생성까지만 완료, **머지는 사용자 몫**(정책상 자동 머지 안 함). 62개 "3건 이하" 그룹 중 이번 세션이 처리한 파일들을 뺀 나머지는 여전히 미착수다. `docs/handoff/global-i18n-audit-remaining.md`(이 문서) 자체의 "남은 것" 우선순위 표는 2차 세션 기준 그대로라 다음 세션은 이번 세션 완료분(PR #940~947, #949, #950 대상 파일)을 표에서 먼저 빼고 재개할 것.

**다음 후보로 조사만 끝낸 것(미착수)**: 백그라운드 조사 에이전트 3개를 돌려 아래 두 클러스터를 스코프해 뒀다 — 다음 세션이 바로 착수할 수 있다.
- **nakshatra 클러스터**(`app/nakshatra/`의 `VvipClient.tsx`·`NakshatraAiClient.tsx`·`NakshatraFormClient.tsx`·`LordReportClient.tsx`·`NakshatraResultClient.tsx`(436줄, 최대)·`DashaMapClient.tsx`·`NakshatraCompatClient.tsx` 7개 + 공유 `_premium/PremiumParts.tsx`·`_premium/use-premium-report.ts`) — 로케일 인프라 전무, 전부 순수 폼/결과 표시 UI(콘텐츠 생성 엔진 아님). `NakshatraResultClient.tsx`는 `dongyang.nameKo`/`india.lordKo`/`india.ganaKo`/`india.nadiKo` 등 API 응답이 이미 담아 보내는 한국어 전문용어 데이터와, 이 파일이 하드코딩한 라벨/제목을 구분해서 처리할 것(데이터는 유지, 라벨만 번역). `DashaMapClient.tsx`의 `period.lordKo`/`period.title`/`period.focus`/`period.opens`/`period.demands`/`period.caution`은 서버가 생성한 한국어 **문장**이라 UI 라벨과 다른 범주 — 그대로 둘 것. `AiConsultDecks.tsx`(LLM 생성 에세이 렌더러)·`CompatResultView.tsx`(compat 결과 뷰, `NakshatraCompatClient`와 함께 스코프해야 함)는 7개 파일 목록 밖이지만 직접 의존성이라 같이 열어볼 것. `nakshatra/codex/[index]/page.tsx`는 이미 Wave 9로 재분류된 별개 파일이니 혼동하지 말 것.
- **`app/components/` 독립 컴포넌트 3종** — `SeoLandingBirthForm.tsx`(~15건, 순수 폼, 로케일 데이터 없음, 바로 착수 가능), `MobileBottomNav.tsx`(aria-label 1건뿐, 나머지는 `@/app/_lib/mobile-tabs`가 소유), `AppChrome.tsx`(**루트 레이아웃이 모든 라우트에서 렌더하는 전역 셸** — 번역 대상 자체는 2건뿐이지만 블라스트 반경이 크므로 신중하게). 같은 조사에서 `FeatureMarketingDetailModal.tsx`(수백 개 한국어 마케팅 카피 + 한국어 키워드로 카테고리/가격 정책을 분류하는 정규식 로직)와 `SunHealingTarot.tsx`(913줄, `lib/tarot/tarot-cards.mjs`의 `nameKo`/`nameEn`과 반드시 일치해야 하는 카드명 사전 내장)도 확인했으나 **둘 다 "콘텐츠 생성/데이터 결합 엔진" 범주라 단순 UI 문구 치환이 아니다** — 착수 시 훨씬 신중한 스코핑이 필요하고, 이번 세션의 "UI 크롬만" 기준으로는 후순위로 밀어도 된다.
