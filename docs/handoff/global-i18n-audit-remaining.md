...
12. **PR #952** `chore/nakshatra-i18n`(1/2) — `app/nakshatra/`의 입력 폼(`NakshatraFormClient.tsx`)·영구 해금 리포트 공유 부품(`_premium/PremiumParts.tsx`·`use-premium-report.ts`)·지배성 심화 리포트·다샤 인생지도·VVIP 통합서·동서 궁합(`compat/`). 공유 모듈 `app/nakshatra/_lib/copy.ts` 신규 도입(`useNakshatraCopy()`). 각 `PRODUCT.reason`(결제 게이트 사유 문자열, 화면 비노출)은 한국어 유지. `dasha-map`의 `PeriodRow` 라벨과 `compat`의 관계 유형/액션가이드 라벨처럼 서버 생성 한국어 **문장**과 같은 줄에 있는 라벨은 번역하면 언어가 섞이므로 그대로 둠. `vvip`의 PDF 파일명·표지 제목/부제(한국어 데이터 보간)도 동일 이유로 유지.
13. **PR #953** `chore/nakshatra-i18n-part2`(2/2, #952 위에 스택) — `NakshatraResultClient.tsx`(결과 화면)·`NakshatraAiClient.tsx`+`AiConsultDecks.tsx`(AI 이중 대가 상담). 🔴 **Row 라벨은 dasha-map과 다르게 번역했다** — 결과 화면의 `Row`는 두 칸짜리 정의 목록(dt/dd)에 짧은 용어가 오는 구조라 번역된 라벨+한국어 용어가 일반 용어집처럼 읽혀 문제없음(반면 dasha-map은 라벨 뒤에 서버 생성 **문장**이 붙어 다른 판단). `AiConsultDecks.tsx`의 `toLocaleString("ko-KR")` 하드코딩 로케일 버그 발견·수정(`getNakshatraNumberLocaleTag()` 신규). `section.title`/`body`/`topInsights`(실제 LLM 생성 상담문)는 이 기능의 존재 이유인 서버 콘텐츠라 손대지 않음. **#952를 먼저 머지해야 이 PR이 충돌 없이 머지된다.**

이로써 **nakshatra 클러스터 전체 완료**(#952+#953, 7개 메인 컴포넌트 + 공유 부품 3개 + compat 결과 뷰 + AI 덱 렌더러).
