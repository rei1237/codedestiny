...
status: active
updated: 2026-09-02
next: 착수 조건·이득 실측 **둘 다 끝났다**(재방문 −79~92KB, 첫 방문 +8.8KB). 남은 건 실행 합의 하나뿐 — 합의되면 `run-postbuild.mjs` 의 `minify-dist-css` 뒤에 외부화기를 넣는다. 🔴 별건이던 **셸 ETag** 는 원인 확정 후 종결됐고(Cloudflare JSD, 코드로 못 고침), 그 결과 N3 가 재방문 절감에 남은 **유일한** 레버다
---
...
- ✅ **원인 확정(2026-09-02): Cloudflare JavaScript Detections.** 엣지가 HTML 본문에 `challenge-platform` 스크립트를 주입하며 응답을 다시 써서 `Content-Length`·`ETag` 가 함께 사라진다. 코드로는 못 고치고 사용자는 봇 보호를 유지하기로 했다 — 대조표는 [app-optimization-remaining-2026-09-02.md](app-optimization-remaining-2026-09-02.md) §2.
...
## ✅ 별건 — 셸에 검증자가 없다 (원인 확정 · 종결 2026-09-02)

`public/_headers:26-42` 의 2026-08-15 주석은 `no-cache` 로 바꾼 목적을 "ETag/Last-Modified 로 304 를 받아 Googlebot 재다운로드 40.4MB 를 막는다"로 적어 두었다. **그 목적은 달성되지 않았고, 달성될 수 없다.**

원인은 Cloudflare **JavaScript Detections(Bot Fight Mode)** 다 — 엣지가 모든 HTML 본문에 `/cdn-cgi/challenge-platform/scripts/jsd/main.js` 를 주입하며 응답을 다시 쓰고, 그때 `Content-Length` 와 `ETag` 가 함께 사라진다. `_headers` 로도 코드로도 못 고치며, 사용자는 봇 보호를 유지하기로 결정했다. 대조표·기각된 가설 4종은 [app-optimization-remaining-2026-09-02.md](app-optimization-remaining-2026-09-02.md) §2.

🔴 **그래서 이 축은 N3 의 경쟁자가 아니라 N3 의 근거가 됐다** — 셸 본문을 캐시로 아끼는 길이 닫혔으므로, 재방문 절감에 남은 레버는 **전송량을 줄이는 N3 하나뿐이다.** 재현: `npm run measure:shell-css` 의 [1] 절.
