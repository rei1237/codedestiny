...
status: active
updated: 2026-09-24
next: 설계안 C4(결제 레인 요청 범위 연결, f9fad4b99)를 C3 보다 먼저 main 에 올리고 스테이징 재현 4회로 확인했다 — 결제 레인 프로브 8/8 200(wall 1457–1510ms, C2 의 9006ms 정지 없음), 스코프 8개 열림 = 닫힘, miss·late·결제 레인 타임아웃 0. 다음은 C1+C2+C4 프로덕션 승격 승인 요청(별도 1회) → 승격 뒤 프로덕션 tail(C4 효과·공유 레인 재사용률) → C3
---
...
## 남은 것
1. **C1+C2+C4 프로덕션 승격(RED, 별도 1회 승인) → 승격 뒤 프로덕션 tail 읽기(결정 ②).** 프로덕션은 C1·C2 이전 코드라 지금 tail 에는 scope·lane 이 없어 소켓을 연 쪽을 가릴 수 없다. 그래서 C4 스테이징 통과 뒤 한 번에 올리고 그 tail 로 읽기로 했다. 승격 범위는 main 의 C1(`3b338fbdf`)·C2(`f14d0e36d`)·C4(`f9fad4b99`)와 그 사이 커밋이다. 🔴 승인 뒤 main 에 다른 세션 커밋이 오르면 범위를 다시 묻는다. 승격 뒤 tail(실행 전 승인)에서 볼 것:
   - 결제 레인 GET 의 8초 정지·재시도가 사라졌는지(6단계 기록 8건 중 4건 타임아웃과 비교).
   - 결제 레인 `[db-conn-open]` 스코프 수와 `[db-scope-close]` 수가 같은지, `[db-scope-miss]`·`[db-scope-late]` 가 0 인지.
   - scheduled 이벤트(10분 정산 크론)에서 예외가 0 이고 닫기가 대응하는지(스테이징 미검증 항목).
   - 공유 레인 `[db-ping]`·`[db-conn-open]` 으로 재사용률(C3 입력).
   - 🔴 `[db-cmd]`(E)는 스테이징 전용이라 프로덕션에서는 `others`·`c4` 의 명령 판정이 비어 있다. 열기·닫기 대응과 `[pay]` wall 로 판정한다.
2. **C3(공유 레인 스코프 연결 + 모델 프록시 + 세션 헬퍼).** 위 "설계안 C 제안" 절, "C1 재현 결과"의 autoCreate 입력, 프로덕션 재사용률을 반영한다. C4 가 세운 스코프 수명·닫기(`currentDbScope`·`onDbScopeEnd`)를 쓴다. 다만 스트리밍 응답(SSE·LLM) 때문에 "핸들러 + waitUntil settle" 을 live-op 계수로 넓혀야 한다. 보안 레인 (a)/(b)는 C3·C5 세션에서 정한다(결정 ③, 추천 (a) — 가드 예산에서 연결 수립을 뺀다). 재현은 `YN_READ_REPEATS=10 node scripts/verify-yeongnyangi-worker-mongo-staging.mjs --staging-fixtures` + staging tail(main 체크아웃에서 띄움)로 한다. 판정은 분석기 `summary|others|lane|c4` + `evcheck.mjs` 로 한다. 성공 기준은 공유 레인 others 0건 + 재현 4/4 PASS 다. C4 재현의 우연 PASS 를 기준선으로 쓰지 않는다.
