...
status: active
updated: 2026-09-25
next: C3(공유 레인 스코프 연결)을 main 84e365d97 에 올렸다. 스테이징(c800fbf07, 워커 4f05c597) 재현은 4/4 PASS 이고, 다른 요청 소켓 위 명령은 0건이다(others 빈 출력). 공유 연결은 57개 열려 57개 닫혔고 miss·late 는 0이다. 다음은 C3 프로덕션 승격(1회 승인 필요)과 승격 뒤 tail 확인이다.
---
...
- `dc00f95aa`+`84e365d97` **설계안 C3, 공유 레인 동작 변경**(워크트리에서 만들어 main 에 ff 머지, 커밋 메시지의 `4ec7c3920` 은 rebase 전 SHA = `dc00f95aa`).
  - `dc00f95aa`: 신규 `worker/lib/db-scope-connection.js`. `bindScopeConnection`·`scopeConnection()`, 그리고 `scopedModel(base)` 가 있다. 모델 Proxy 는 스코프에 연결이 걸려 있으면 `base.__subclass(conn)` 하위 모델로 가고, 아니면 기반 모델로 간다. `releaseScopeModels` 는 스코프 끝에 하위 모델을 기반 모델의 subclassed 배열에서 빼낸다. 이 커밋 단독으로는 동작이 바뀌지 않는다.
  - `84e365d97` connectDb: 스코프 안이면 요청마다 `createConnection(uri, {...기존 옵션, autoCreate:false})` 를 연다. 한 요청 안의 동시·순차 호출은 연결 하나를 공유한다. 스코프 끝에 `destroy(false)` 로 닫는다(`[db-scope-close] lane=shared why=end`). 연결 레벨 실패는 그 요청의 연결만 은퇴시킨다(`why=reset`, 닫기는 스코프 끝에 한다 — 형제 op 을 자르지 않기 위해서다). 전역 warm·ping·detach·reset 상태는 건드리지 않는다. 스코프 밖은 예전 전역 경로 그대로이고 `[db-scope-miss] {lane:"shared"}` 를 남긴다.
  - 모델 export 63개를 `scopedModel(...)` 로 감쌌다: models 52·yeongnyangi 4·gift 3·app-store·feedback·review·purchase-entitlement 각 1.
  - 세션과 raw db 호출부 16곳은 `(scopeConnection() || mongoose).startSession()` 또는 `(scopeConnection() || mongoose.connection).db` 로 바꿨다. `payments/db.js` 는 `paymentConn || scopeConnection() || mongoose.connection` 을 쓰고, 컬렉션 이름은 `unscopedModel` 로 읽는다(거짓 miss 방지). `routes/payments.js` 는 동결 상한 3710줄이라 import 를 1행에 붙였다.
  - `verify-cron-mongo-op-coverage` 는 `scopedModel(` 래퍼를 인식한다(self-test 5/5). db.js detach 주석은 실측대로 고쳤다("이미 명령을 보낸 이웃은 깨우지 못한다"). db-scope.js 주석에는 SSE 두 곳(`routes/fortune.js`·`routes/fusion-fortune.js`)이 `ctx.waitUntil` 로 넘겨 추적된다는 점을 적었다.
  - 테스트: `db.shared-lane-scope` 7건(모델 3·connectDb 4)을 더했고, 기존 3건(`db.connect-backoff-jitter`·`db.payment-op-timeout-retry`·`db.op-timeout-instrumentation`)의 단언을 갱신했다.
  - 검증: check:fast 는 jest 296 스위트/4207 과 paid-gate-suite 를 통과했다. `verify-payment-freeze` 도 통과했다. main CI 는 `PR CI`·Paid Flow Gates·Gift·AI Locale·Release(스테이징)가 녹색이다. Secret Scan 은 옆 세션의 연속 push 로 cancelled 됐다.
...
## 설계안 C3 재현 결과 (스테이징 4회, 워커 4f05c597 = c800fbf07, C3 포함) — 공유 레인 소켓 공유 해소
- 재현(`YN_READ_REPEATS=10 … --staging-fixtures`) **4/4 PASS**, fixtureCleanup 4/4 PASS. tail 이벤트 64건, 5xx 0.
- ✅ **다른 요청 소켓 위 명령 0건:** `others` 빈 출력이다. 명령 306건의 응답은 전부 보낸 요청에 도착했다(`own-socket → issuer`). 공유 `[db-conn-open]` 105건은 전부 scope=set 이고 null 은 0이다(프로덕션 C1 tail 은 null 55건이었다).
- ✅ **수명:** 공유 연결 수립 57 = `[db-scope-close] shared end` 57, 결제 4 = 4. `reset` 0, `[db-scope-miss]`·`[db-scope-late]` 0, 스코프 밖 전역 connect 0, `[db-connect-error]` 0. queue 소비자(`yeongnyangi-consultation-staging`)도 자기 스코프 연결로 돌았다.
- ⚪ `[db-op-timeout]` 1건: `free/reading`(200) 의 보안 레인 rate-limit `findAndModify` 가 1000ms 예산을 넘겼다. 그 요청이 방금 연 **자기 소켓**(`connAgeMs 1000`) 위였고, 끝난 요청 소켓 위 타임아웃은 0이다. 기존 "보안 레인이 새 소켓 준비에 밀려 fail-open" 과 같은 현상이다(C5·결정 ③ 영역).
- `[db-cmd-fail] MongoServerError` 4건은 동시 attendance·unlock 의 1회 보장 경로다(응답 200, 재현이 단언하는 계약).
- 비용(실측): 공유 연결 수립 p50 1312ms·p90 1432ms·max 1886ms(n=57). 기준선(같은 날 C3 이전 워커 d2e0fc1b 재현 1회)도 요청마다 재수립 ~1.5–2.7s 에 웜 ping 300ms 를 냈으므로 스테이징에서는 늘지 않았다. 프로덕션 재사용률은 미측정이다(C1 tail 기준 30분에 ping 성공 2건).
- 미검증: 스테이징은 `crons = []` 라 scheduled 스코프의 공유 레인(형제 크론 작업이 연결 하나를 공유, 은퇴는 스코프 끝 닫기)은 프로덕션 승격 뒤 tail 로 본다.
- 분석기: C4 절의 `cmds.mjs summary|others|lane`. 스코프 닫기·miss·op-timeout 추출은 `%TEMP%\claude\d--Development-code-destiny\3ff5e0a2-46f3-4d1b-b349-7484f7ea86ef\scratchpad\c3check.mjs <tail>` 로 한다(원본 tail 은 같은 폴더 `c3-tail2.jsonl`, 커밋 금지).
...
## 남은 것
1. **C3 프로덕션 승격(RED, 명시적 1회 승인 필요) + 승격 뒤 tail.** C3 스테이징 판정은 위 "C3 재현 결과" 절에서 끝났다. 승격 범위는 승인 시점 main 전체(옆 세션 커밋 포함)이며, fetch → SHA 비교 → dispatch 를 한 명령으로 묶는다. 승격 뒤 tail 은 재접속 루프로 30분 받는다. 판정 항목: (가) 공유 `[db-scope-close]` 수 = 공유 연결 수립 수, miss·late 0 (나) `[db-op-timeout]` pending 소켓의 opener 가 self 뿐인지(`opener.mjs`) (다) 크론 스코프의 공유 레인 — 형제 작업의 `Topology is closed`·`closed connection pool` 이 사라졌는지 (라) 인증·목록 GET wall 과 503 (마) 결제 GET 이 창에 들어오면 `[pay]` wall. 회귀 시 `84e365d97` 만 revert 한다(`dc00f95aa` 는 그것만으로는 무동작). 보안 레인 (a)/(b)(결정 ③, 추천 (a) — 가드 예산에서 연결 수립을 뺀다)는 C5 세션에서 정한다. 스트리밍 응답의 live-op 계수는 필요 없었다: SSE 두 곳 모두 ctx.waitUntil 로 넘겨 스코프가 추적한다(C3 절).
...
다음 세션 첫 문장: "docs/handoff/yeongnyangi-paid-result-attach-503.md 를 읽고 남은 것 1(C3 프로덕션 승격 여부 확인과 승격 뒤 tail)부터 진행해줘."
