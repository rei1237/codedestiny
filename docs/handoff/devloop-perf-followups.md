...
status: active
updated: 2026-09-06
next: 머지 후 새 세션이 3개 이상 쌓였는지 세고, 쌓였으면 아래 "재측정" 스크립트를 돌려 After 열을 채운다. 안 쌓였으면 후속 과제 4건 중 하나를 고른다.
---
...
- [ ] **후속 과제 4건** (원칙 14 — 지난 작업 범위 밖이라 보고만 했다. 고칠지는 사용자가 정한다)
  - `git diff` 전문이 평균 63.8초(n=9, 최대 288초). 커밋된 생성물 미러(`index.html` 2.94MB × 7)가 원인으로 **추정**, 미검증.
...
- `git diff` 63.8초의 원인은 **추정일 뿐**이다. 고치기로 한다면 미러 파일을 제외한 diff 와 비교해 먼저 확증해야 한다.
