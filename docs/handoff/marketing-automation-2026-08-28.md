...
- 🔴 **해소됨 — 17:56Z 프로덕션 승격 완료**(run 33539912260, `GET /api/version` → `gitSha 7b8609338`,
  #1428 `4f13d8a35` 포함). 승격 전이었다면 구 코드가 `failed` 를 재선점하고 `postThreadsChain` 이
  index 0 부터 다시 발행해 **1번 글이 두 번째로 올라갔을 것**이다. 이제 같은 크론이
  `already_posted_partial` 로 건너뛴다 — 09-02 는 어차피 비는 날이고 **새 원고는 09-03 부터**다.
  (크론은 프로덕션 워커에서만 돈다 — 스테이징은 `crons=[]` 라 머지만으로는 안 걸린다.)
...
- 🔴 **답글 미지(위 "남은 미지")도 여기서 자동으로 풀린다.** 크론이 뜨면 09-03 은 잠금이 없는 새 날이라
