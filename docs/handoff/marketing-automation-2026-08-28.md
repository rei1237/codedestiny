...
status: active
updated: 2026-09-01
next: §C-5 를 읽는다(2026-09-02 관리자 라우트 실행으로 원인 확정). 🔴 **크론 배관이 아니라 발행 자체가 400 으로 거절된다** — `POST graph.threads.net/v1.0/me/threads` 가 `The requested resource does not exist`. 첫 행동은 Threads 토큰으로 `me/threads` 와 숫자 id `27631393193230212/threads` 를 각각 던져 Graph 원본 `code`·`error_subcode` 를 보고 **스코프(`threads_content_publish`) 냐 경로냐**를 가르는 것. 🔴 토큰은 `GET /me` 가 `200` 이라 **회전 대상이 아니다** — 오류 문구의 `[토큰 회전 필요]` 는 분류기 고정 라벨이다. 🔴 **별건으로 남은 결함 하나**: 크론 3회(08-31·09-01·09-02)가 잠금 문서를 하나도 안 만들었다(수동 실행은 즉시 만든다) — 채널 루프 이전에 죽는 문제는 미해결(§C-5 ②). 🔴 관리자 라우트는 **에이전트도 실행할 수 있다** — 진입 비밀번호만 있으면 `x-admin-token` 헤더로 되고 브라우저 콘솔은 필요 없다. 🔴 **손발행은 끝났다**(사용자 결정 2026-09-01) — 실패하는 날은 계정이 비는 날이다.
---
...
1. 🔴 **2026-09-02 갱신 — 문안은 문제가 아니었다.** 실제 실행에서 6글 체인은 만들어졌고
   Graph 가 `me/threads` 를 400 으로 거절했다(§C-5). 남은 것은 스코프냐 경로냐를 가르는 것 하나와,
   별건인 크론 pre-loop 실패다. 🔴 토큰은 `200` 이라 **회전 대상이 아니다**.
...
**발행 거절의 정체 — 미확정, 후보 2개.** 토큰은 `GET /v1.0/me` 가 `200`(§C-3)이라 죽지 않았다.
따라서 문구의 `[토큰 회전 필요]` 는 **분류기가 `permanent` 에 붙이는 고정 라벨**일 뿐 회전 지시가 아니다.
🔴 **회전하지 말 것**(§C-3 과 같은 결론).

- (a) **스코프** — 토큰에 `threads_content_publish` 가 없으면 읽기는 되고 쓰기 edge 만 막힌다. 코드 수정 0.
- (b) **경로** — [공식 문서](https://developers.facebook.com/docs/threads/posts)는 `POST /{threads-user-id}/threads`
  로 쓰는데 이 레포는 `me/threads` 를 쓴다([worker/lib/threads.js](../../worker/lib/threads.js) `publishOneThread`).
  숫자 id 는 이미 안다 — `27631393193230212`. 이쪽이면 한 줄 수정이다.

🔴 **가르는 법(다음 사람의 첫 행동).** Threads 토큰으로 `POST graph.threads.net/v1.0/me/threads` 와
`.../27631393193230212/threads` 를 각각 던져 **Graph 원본 봉투의 `code`·`error_subcode`·`type`** 을 본다.
워커 로그로는 안 된다 — `threads.js` 의 `console.error` 는 `code` 를 안 찍는다(그것도 결함이다).
🔴 **추측으로 코드를 고치지 말 것.** 웹 검색으로는 이 메시지의 원인이 갈리지 않았다(실측 2026-09-02).
