...
status: active
updated: 2026-09-02
next: "작전실 수정 세션 — 열폭 266px·이탈 재스캔부터(#1452 감지 수정 반영, 아래 레시피 절차)"
---
...
| 낙샤트라 | /nakshatra/ | 1 | 09-02 | 0/1→0/6건→0/—/274px/수동→유(공용 나브) | #1452 | 완료 |

수정 우선순위: 작전실(열폭 266px·이탈 재스캔) → 러브코덱스·찻집(탭 타깃 소수).
...
- 이탈=수동 인 몰입형(작전실·초융합)은 수정 세션에서 손으로 확인. 09-02(#1452)부터 스캐너가 공용 크롬리스 나브(.cd-feature-nav)를 이탈로 인식한다 — 작전실의 '수동'은 같은 감지 구멍이었을 공산이 커서 수정 전에 재스캔부터 한다.
