...
status: active
updated: 2026-09-05
next: "신년운세 결과 화면을 09-05 에 닫았다(아래 §신년운세). 다음은 §다른 전문가 상담 감사 의 keep-all 단독을 기능당 1PR 로 — 숙요궁합(34) → 카르마(13) → 낙샤트라(31) 순. 🔴 스캐너 맹점이 하나 더 확인됐다(진입 애니메이션 opacity) — 세 기능 다 스캐너 수치만으로 전/후를 판정하지 말고 프로브를 함께 돌린다. CI 게이트 2종의 OF 맹점은 여전히 별도 PR 이 필요하다"
---
...
## 신년운세 결과 화면 (`/new-year-ai-consultation/`) — 09-05 수정
