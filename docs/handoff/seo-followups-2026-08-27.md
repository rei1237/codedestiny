...
| #1199 | 정적 셸 구성 실측 + dist 저작 주석 제거 (§3-3) | 🔴 **사용자 머지 대기** |
...
#### (5) 🔴 남은 레버 하나 — 인라인 CSS 653KB 를 외부 시트로 (사용자 결정 필요)

이게 셸 무게의 절반이고, 지금까지 잰 것 중 유일하게 큰 이득이다.

- 이득(실측): `dist/index.html` 에서 `<style>` 84블록을 들어내면 brotli q11 188,136 → 107,435
  (**−80,701, 전체의 43%**). 캐시가 도는 재방문에서는 그만큼이 통째로 사라진다.
- 🔴 위험 ①: 첫 페인트 경로 변경이다. 셸의 CSS 는 FOUC 방지용으로 **의도적으로** 인라인이고,
  늦게 오는 확정 규칙은 CLS 가 된다([late-css-blocks-are-cls-in-the-shell]).
- 🔴 위험 ②: 전례가 나쁘다. `externalize-dist-inline-scripts.mjs` 가 셸 스크립트 85개
  1,521,155B 를 빼냈는데 `defer`·`async` 없는 **파서 차단** `<script src>` 라 성능은 오히려
  나빠졌다(§3-1).
- ✅ 반대로 이 레포에는 **이미 되는 패턴이 있다** — `data-cd-noncritical-style-src` 로 시트 12개를
  지연 로드한다(가장 큰 것이 `/styles/fortune-ui.css`, 583KB). 즉 "전부 외부화"가 아니라
  **"첫 화면에 필요 없는 블록만 그 패턴으로 옮기기"** 가 현실적인 모양이다.
- 그래서 이건 결정 없이 손대면 안 된다. 필요한 결정: **어느 블록이 첫 뷰포트에 필요한가**를
  누가 어떤 기준으로 자를지. 그 판단 없이 84블록을 통째로 옮기면 홈 첫 화면이 깜빡인다.

같은 성격의 잔여 후보로 **인라인 HTML 공백 접기 56,475B** 가 있다(태그 사이 2칸 이상 공백 4,835곳).
인라인 요소 사이 공백은 화면에 보이는 공백이라 기계적으로 접으면 레이아웃이 움직인다 — 역시 결정 대상이다.
...
4. ~~§3-3 정적 셸 5개의 크기 측정 + 결정 없이 되는 것~~ — ✅ PR #1199 (구성 실측 + dist 저작 주석 제거)
   🔴 이어지는 것은 **§3-3(5) 인라인 CSS 653KB 외부화 결정**이다. 이득 −80,701B(brotli, 셸의 43%)로
   지금까지 잰 것 중 가장 크지만, 첫 페인트 경로를 바꾸므로 "어느 블록이 첫 뷰포트에 필요한가"를
   먼저 정해야 한다. 나머지 후보는 이미 재서 이득이 없다고 나왔다 — §3-3(4) 를 다시 재지 말 것
