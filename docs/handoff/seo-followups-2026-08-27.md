...
| #1195 | 문장 중간에서 끊긴 설명 42 → 0 (§1-4(a)) | 🔴 **사용자 머지 대기** |
...
| **문장 중간에서 끊긴 설명 42개** | ✅ PR #1195 | 42 → 0. 가드는 없다 — 폭 가드가 이미 상한을 막고 `…` 는 품질 문제라 §1-4(a) 참고 |
...
1. ~~§2 몰입형 18개~~ — ✅ #1193
2. ~~§1-4(a) `…` 42개~~ — ✅ PR #1195
3. **§3 `/insights` 1.83MB — 결정이 필요 없다. 다음 세션의 기본 시작점.**
   설계(검색 인덱스)가 선행돼야 하므로 코딩 전에 §3 을 먼저 읽을 것
...
🔴 **색인 폭·링크 계열(§1·§2)은 전부 닫혔다.** 남은 것은 성능·배포·측정이라 성격이 다르다.
