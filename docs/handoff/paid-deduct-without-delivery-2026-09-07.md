# 결제 차감 후 결과 미전달 — 잔여 결함 4건 (조사 완료, 수정 미착수)

- 작성 2026-09-07 · status: active
...
"결제 차감 후 결과 미전달 잔여 4건 중 **F1(월정석 자동환불 전멸)** 부터 고친다. `/effort high` 를 켜고 워크트리를 판 뒤, `worker/lib/service-execution-task.js` 의 `resolveMonthlyCreditSourceTransactionId` 를 원장 기준으로 바꾸는 사전 보고 7항목부터 낸다."

## F1 (최우선 · 코드 경로 실측) 월정석 자동환불이 V2 컷오버 이후 전부 skip 된다
...
## F2 (중간 · 서브에이전트 보고, 내가 직접 열지 않음 → 미검증) 카드 단건 실패는 실행 등록 자체를 건너뛴다

`worker/routes/astrology-ai.js:1416` 이 `access.source !== "billing-gate"` 면 `startServiceExecution` 을 건너뛴다는 보고. 카드는 `source:"payment"` 라 실패 시 `failServiceExecution` 이 404 로 끝나 환불·기록이 0. `neo-operation-room.js:1217-1256` 동형. 정본 짝은 `worker/routes/fortune.js:1242`(`autoRefundSinglePaymentDeliveryFailure`).
🔴 **착수 전에 이 두 파일을 직접 열어 재확인할 것.**
...
## 검증 (착수하는 쪽이 돌린다 — 이번 조사에서는 하나도 돌리지 않았다)
...
npx --no-install jest __tests__/worker/per-use-proof-roundtrip.test.js
# F1 을 고치면: __tests__/worker/neo-operation-room.payment-flow.test.js
#              __tests__/worker/human-design-report.start-revive.test.js
