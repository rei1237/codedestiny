...
status: active
updated: 2026-08-31
next: "🔴 2026-08-31 재측정: 관측 CWV(LCP 2.0s·CLS 0.001)는 이미 기준 안, 점수 57 은 Lantern `simulate` 아티팩트다. 저위험 레버는 소진 — 남은 실질 레버는 §5 N3(Style&Layout 5.4s 축소=대형·고위험) 하나뿐. 저위험 제약(2026-08-31 사용자 선택)에선 추가 코드 없음. 점수 자체를 끌어올리려면 N3 를 사용자와 위험 합의 후 착수."
---
...
- 🔴 **2026-08-31 재측정(`7d6cbbed7`, 모바일 3회 중앙값)**: 점수 **57** · FCP(sim) 5,059 · LCP(sim) 5,789 · TBT 253 · **CLS 0.001** · SI(sim) 9,047.
  🔴 **관측값(비시뮬레이션)은 이미 CWV 기준 안**: **LCP 2,008ms(≤2.5s ✓) · CLS 0.001(로딩 중 덜컹거림 없음 ✓) · FCP 1,737ms.**
  점수를 끌어내리는 건 전부 Lantern `throttlingMethod=simulate`(관측 대비 ~3배). 사용자 스크린샷의 CLS 0.149·점수 33 은 **재현 안 됨**(게스트 측정). LCP 요소는 여전히 `h1.moon-hero__title`(텍스트) — 히어로 이미지 아님.
  🔴 **저위험 레버 소진**(§3 기각 + 부트 태스크 분할 완료분). 실질 병목은 `Style & Layout 5,439ms`(= §5 N3, 대형·고위험). 저위험 제약에선 추가 코드 없음이 결론.
