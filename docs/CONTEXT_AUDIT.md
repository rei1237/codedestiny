...
## 2026-09-12 main 단독 개발 전환 — 절대 규칙 3 충돌 해소 (코딩 원칙 14)

- **충돌**: 사용자 지시("브랜치·PR·워크트리를 만들지 않는다. 모든 작업은 `main` 에서 직접 한다")가 `CLAUDE.md` 절대 규칙 3("main 직접 수정·배포 금지. PR→필수 검사→안전한 머지→스테이징 확인")과 정면으로 어긋났다. 원칙 14에 따라 조용히 합치지 않고 여기 기록한다.
- **해소**: **사용자의 이번 지시가 정본**이다. 절대 규칙 3 중 **브랜치·PR·머지 절차 부분은 폐기**하고, `main` 직접 커밋 → push → main CI 1회 → 스테이징 자동을 기본 흐름으로 삼는다. 같은 규칙의 **"로컬에서 프로덕션 배포 금지"는 그대로 유효**하다 — `scripts/lib/production-deploy-guard.mjs` 가 계속 막고, 프로덕션 승격은 여전히 명시적 1회 승인 때만 한다.
