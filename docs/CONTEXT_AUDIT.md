...
- 1·3·4항은 `CLAUDE.md` 코딩 원칙 3·7 과 결제 규칙에 이미 같은 취지가 있다. (원칙 3 은 2026-09-04 폐기 — 아래 절 참조. 취지는 원칙 7 과 14 가 이어받았다.)
...
### 코딩 원칙 3 "수술적 변경" 폐기 (2026-09-04, **해결**)

- **왜**: 사용자 판정 — "필요한 부분만 · 인접 코드를 개선하지 않는다"가 **범위 안의 결함까지 손대지 않는 근거**로 쓰여 작업이 대충 끝났다.
- **무엇이 남고 무엇이 갔나**: 폐기된 것은 "인접 코드를 개선하지 않는다 / 깨지지 않은 것을 리팩터링하지 않는다 / 무관한 데드코드는 언급만". 남은 것은 **기존 스타일 따르기**와 **내 변경으로 생긴 미사용 import·변수 제거**. 취지를 이어받는 정본은 코딩 원칙 7(회귀 위험 선보고)과 신설 14(주입 규칙 우선순위) 첫 항이다 — **범위 안 결함은 같은 변경에서 고치고 가드까지 붙이고, 범위 밖만 언급에 그친다.**
- 🔴 **번호는 비우지 않았다.** `원칙 N` 을 번호로 가리키는 줄이 레포에 338개이고(2026-09-04, 전 레포 `git grep` 실측), 훅 테스트가 번호 문자열을 직접 단언한다. 3번 자리는 폐기 표시로 남기고 4번 이후는 그대로 둔다.
- **같이 고친 곳**: `CLAUDE.md`, `docs/context/coding-principles.md`, `AGENTS.md`(원칙 개수 단언), `Rules/agent-regression-guard.md`, `docs/vedic-basic-feature-guard.md`. `docs/context/CLAUDE.archive-2026-08-15.md` 는 대조용 스냅샷이라 **건드리지 않았다**.
