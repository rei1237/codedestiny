...
status: active
updated: 2026-09-23
next: "인생의 책·연애 비책 결과 화면의 검증과 CI를 마치고, 구매 전 상세와 다른 전문가 상담군을 같은 품질로 순차 확장한다."
---
...
이 문서는 두 결과 화면의 지역 디자인 정본이다. 프로젝트 전역 DESIGN.md나 영냥이·연이 캐릭터 정체성을 덮어쓰지 않는다.
...
|---|---|---|
| 인생의 책·연애 비책 구매 전 상세 | 각 상품 Client와 feature-detail 정본 | 결과에서 받은 책/편지와 실제 구매 전 예시·제공 범위·가격의 시각적 일치. 과장된 리뷰·새 혜택 금지 |
