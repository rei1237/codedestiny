...
### 🔴 2026-09-01 추가 — 번들 해금 SKU 2개 (미완료 · 사람 손)

| productId | 앱가 | 전용이던 상품 | 왜 없애나 |
|---|---:|---|---|
| `cd_content_tier_11` | ₩39,000 | `unlock.premium_astrology` / `_sukuyo` / `_veda` | 이 featureKey 로 결제를 시작하는 화면이 하나도 없다 |
| `cd_content_tier_13` | ₩70,000 | `unlock.all_paid_saju` / `unlock.premium_naming` | 위와 같음 |

"프리미엄 전체 해금" 같은 번들 상품은 **서비스가 존재한 적이 없다.** 2026-09-01 전수 `git grep`(소스 +
`__tests__/` + `scripts/verify-*`, `sync:public` 미러 포함) 결과 클라이언트에서 이 5개 키로 결제를 여는
지점은 0건이었고, `allPaidSaju` 는 *이미 보유한 사람*의 권한 별칭 표에만 남아 있었다. 레지스트리에서 5종을
지우면서 두 티어도 함께 지웠으므로 **앱은 이 SKU 를 더 이상 요청하지 않는다.**

작업 절차는 위 6개와 같다 — **비활성화(Deactivate), 삭제 금지, ID 재사용 금지.** 기존 구매자 영향도 같다
(해금 기록은 우리 DB 의 `featureKey` 기준이라 SKU 폐기와 무관하며, `PERSISTENT_UNLOCK_KEY_SET` 과
클라이언트 별칭 표는 **일부러 남겨 뒀다** — 과거에 이 권한을 가진 계정이 있다면 그대로 열린다).
...
### ✅ 2026-08-27 — `cd_content_tier_14` 신규 등록 (**2026-09-01 완료**)

타로 오라클 상담을 카드 수 구간 4단계로 나누면서 **₩7,000(70코인)** 가격 포인트가 새로 생겼다
([PRICING_TIERS.md](./PRICING_TIERS.md) 의 2026-08-27 항목). 코드 쪽 매핑은 이미 들어갔고
(`worker/lib/app-store-pricing.js` 의 `CONTENT_TIER_TABLE`), **Play Console 등록도 2026-09-01 에
사람 손으로 끝났다**(사용자 확인). 등록가·상태는 다음 AAB 업로드 전에 `npm run play:products:dry`
로 '변경 없음'을 한 번 확인한다.
...
**등록 전까지의 영향(해소됨)**: 등록 전에는 안드로이드 앱에서 **8~10카드 스프레드만** 티어 미등록 503
으로 결제가 막혔다. 웹 결제와 나머지 세 구간(1~4·5~7·11~14장)은 영향이 없었다.
...
4. Play Console 에서 비활성화한 8개 SKU(2026-08-12 정비 6개 + 2026-09-01 번들 2개) 가 앱 결제 시트에 더는 나타나지 않는지 확인
