...
**커버 금액은 앱 기준으로 표기한다.** 같은 콘텐츠가 앱에서 더 비싸므로 웹 표기(5,000원)를 쓰면
결제창 금액과 어긋난다. 커버하는 기능 집합 자체는 앱·웹이 동일하다(코인으로 판정하므로).
반면 **이용권 자체의 가격은 앱·웹이 같다**(2026-08-24) — 콘텐츠 티어만 인상 폭을 유지한다.
...
| 상품 ID | 가격(KRW) | 이름 | 설명 |
|---|---|---|---|
| `cd_pass_standard_30d` | `9900` | 스탠다드 꿀 (30일) | 30일간 6,000원 이하 유료 기능을 월 최대 30,000원 상당까지 이용합니다. 프로필 3개. 자동 갱신되지 않습니다. |
| `cd_pass_premium_30d` | `29900` | 프리미엄 꿀 (30일) | 30일간 13,000원 이하 유료 기능을 월 최대 100,000원 상당까지 이용합니다. 프로필 7개. 자동 갱신되지 않습니다. |
| `cd_pass_vvip_30d` | `59000` | VVIP 꿀단지 (30일) | 30일간 25,000원 이하 유료 기능을 월 최대 200,000원 상당까지 이용합니다. 프로필 15개. 자동 갱신되지 않습니다. |
| `cd_pass_family_30d` | `149000` | Code Destiny Family (30일) | 30일간 이용권 대상 유료 기능 전체를 월 최대 500,000원 상당까지 이용합니다. 프로필 무제한. 자동 갱신되지 않습니다. |

**가격 근거 — 이용권가는 앱·웹이 같다**(2026-08-24 개정). 종전에는 커버 금액이 오른 비율만큼 앱 이용권가를
20~30% 올렸으나, 등급별 적용 가격 범위를 상향하면서 이용권은 웹가와 동일하게 맞췄다. 앱이 더 비싼 것은
**콘텐츠 티어 SKU 쪽뿐**이다(2-1절).

| 등급 | 커버(웹→앱 표시) | 커버 상승률 | 월 이용 한도 | 이용권가(웹=앱) |
|---|---|---|---|---|
| standard | 5,000 → 6,000 | +20.0% | 30,000 | 9,900 |
| premium | 10,000 → 13,000 | +30.0% | 100,000 | 29,900 |
| vvip | 20,000 → 25,000 | +25.0% | 200,000 | 59,000 |
