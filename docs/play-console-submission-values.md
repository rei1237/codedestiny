...
> 🔴 **2026-08-29 — 앱가를 웹가와 같은 값으로 내렸다**(사용자 확정). 아래 가격·이름이 새 값이다.
> Play Console 쪽 인하는 사용자가 2026-08-29 에 먼저 완료했다(작업표:
> [pricing/PLAY_CONSOLE_PRICE_UPDATE_2026-08-29.md](pricing/PLAY_CONSOLE_PRICE_UPDATE_2026-08-29.md)).
> 🔴 다시 올릴 때도 **Play 먼저, 코드 나중** — 반대 순서면 그 사이가 "표시가 < 청구가" 위반 구간이다.

| 상품 ID | 가격(KRW) | 이름 (≤55자) | 설명 (≤200자) |
|---|---|---|---|
| `cd_content_tier_01` | `3000` | 운세 콘텐츠 3,000원 | 선택하신 운세·타로·사주 기능 1건의 단건 결제입니다. 이용권으로 열리는 기능은 결제 없이 이용됩니다. |
| `cd_content_tier_02` | `5000` | 운세 콘텐츠 5,000원 | 선택하신 운세·타로·관상 기능 1건의 단건 결제입니다. 이용권으로 열리는 기능은 결제 없이 이용됩니다. |
...
| 상품 ID | 웹가 | 앱가 | 대상 기능 수 | 대표 기능 |
|---|---|---|---|---|
| `cd_content_tier_01` | 3,000 | 3,000 | 18 | tarot-numerology-reading, animal-totem-basic |
| `cd_content_tier_02` | 5,000 | 5,000 | 58 | tarot-love-relationship, royal-tea-oracle, physiognomy-*, animal-totem-deep |
| `cd_content_tier_14` | 7,000 | 7,000 | 1 | tarot-prompt-maker-deep (타로 오라클 상담 8~10카드) |
...
**이용권은 30일 기간이고, 자동 갱신이 없으며, 등급별 월 이용 한도가 있다.** 커버 범위 안의 기능은
그 한도(스탠다드 3만·프리미엄 10만·VVIP 20만·family 50만원)까지 추가 결제 없이 열린다.
🔴 **"횟수 제한 없음"·"무제한"으로 쓰지 말 것**(2026-08-24) — 모든 등급에 월 이용 한도가 있어 그 표현은
실제 정책과 모순이고, 한도를 다 쓴 사용자가 "제한이 없다더니"를 겪는다.
🔴 **한도를 다 쓰면 이용권이 그 자리에서 끝난다**(2026-09-04) — 남은 기간이 며칠이든 종료되고, 새로 구매하면
그날부터 30일과 한도가 다시 시작된다. 서비스가 막히지는 않고 이후 이용은 단건 결제·월정석으로 넘어간다.
"기간 내 리셋"·"다음 달에 다시 열림"으로 쓰지 말 것 — 한도 사이클 키가 이용권 자신의 만료일이라 기간 안에서
리셋되는 일 자체가 없다(`worker/lib/profile-limits.js` `resolvePremiumQuotaCycleKey`).

**커버 금액은 앱 기준으로 표기한다.** 2026-08-29 부터 앱가 = 웹가라 두 표기가 같은 값이지만, 파생
경로는 앱 티어가(`resolveAppPassCoverageKRW`)로 유지한다 — 다시 인상할 때 이 표기만 조용히 웹가로
남으면 결제창 금액과 어긋난다. 커버하는 기능 집합 자체는 앱·웹이 동일하다(코인으로 판정하므로).

🔴 아래 표는 손으로 쓴 값이 아니라 `scripts/create-play-console-products.mjs` 가 **실제로 등록하는 것**의
전사(轉寫)다. 가격은 `worker/lib/app-store-pricing.js` 의 `PASS_TIER_TABLE`, 설명은 `passDescription()` 이
`MONTHLY_PASS_LIMITS_KRW` 에서 파생시킨다. 값을 이 문서에서 고치지 말고 정본을 고친 뒤 다시 전사한다
(`npm run verify:play-console-products` 가 SKU·가격 일치를 대조한다).

| 상품 ID | 가격(KRW) | 이름 | 설명 |
|---|---|---|---|
| `cd_pass_standard_30d` | `9900` | 스탠다드 꿀 (30일) | 30일간 5,000원 이하 유료 기능을 월 최대 30,000원 상당까지 이용합니다. 프로필 3개. 월 최대 금액이 0이 되어도 만료일·등급·프로필 상한은 유지되며, 추가 콘텐츠는 별도 결제로 이용할 수 있습니다. 자동 갱신되지 않습니다. |
| `cd_pass_premium_30d` | `29900` | 프리미엄 꿀 (30일) | 30일간 10,000원 이하 유료 기능을 월 최대 100,000원 상당까지 이용합니다. 프로필 7개. 월 최대 금액이 0이 되어도 만료일·등급·프로필 상한은 유지되며, 추가 콘텐츠는 별도 결제로 이용할 수 있습니다. 자동 갱신되지 않습니다. |
| `cd_pass_vvip_30d` | `59000` | VVIP 꿀단지 (30일) | 30일간 20,000원 이하 유료 기능을 월 최대 200,000원 상당까지 이용합니다. 프로필 15개. 월 최대 금액이 0이 되어도 만료일·등급·프로필 상한은 유지되며, 추가 콘텐츠는 별도 결제로 이용할 수 있습니다. 자동 갱신되지 않습니다. |
| `cd_pass_family_30d` | `149000` | Code Destiny Family (30일) | 30일간 이용권 대상 유료 기능 전체를 월 최대 500,000원 상당까지 이용합니다. 프로필 무제한. 월 최대 금액이 0이 되어도 만료일·등급·프로필 상한은 유지되며, 추가 콘텐츠는 별도 결제로 이용할 수 있습니다. 자동 갱신되지 않습니다. |

**가격 근거 — 이용권가는 앱·웹이 같다**(2026-08-24 개정). 2026-08-29 에 콘텐츠 티어까지 같아지면서
이제 앱의 모든 SKU 가 웹가와 동일하다 — Play 수수료 15%를 그대로 부담한다는 뜻이고, 의도된 선택이다.

| 등급 | 커버(웹=앱 표시) | 월 이용 한도 | 이용권가(웹=앱) |
|---|---|---|---|
| standard | 5,000 | 30,000 | 9,900 |
| premium | 10,000 | 100,000 | 29,900 |
| vvip | 20,000 | 200,000 | 59,000 |
| family | 상한 없음 | 500,000 | 149,000 |
...
> 🔴 **Play Console 의 실제 등록가가 아직 옛 값(13,000/36,000/75,900/185,000)일 수 있다** — 이 표는 코드
> 정본이며, Play Console 쪽 수정은 사람이 해야 한다.
...
| 대상 | 처리 | 이유 |
|---|---|---|
| `fortune-fish-gacha` (웹 ₩500) | **앱에서만 무료** | 앱가 = 웹가 ₩500 이라 Play KRW 최저 판매가를 밑돌 수 있음. 웹은 ₩500 유료 유지 |
...
| 개인 정보 | 전화번호 | 필수 | 계정 관리, 결제 진행 |
| 개인 정보 | 생년월일·출생시각 | 선택 | **앱 기능(사주·운세 계산의 입력값)** |
| 개인 정보 | 성별 | 선택 | 앱 기능 |
| 사진 | 프로필 이미지 | 선택 | 계정 관리 |
| 사진 | 얼굴 사진(관상) | 선택 | 앱 기능 — **기기 내 처리, 서버 미전송** (`AnalysisEngine.js` 로컬 규칙 엔진) |
| 금융 정보 | 구매 내역 | 필수 | 앱 기능(이용권·콘텐츠 권한) |

> 🔴 2026-08-19 갱신 — 전화번호가 **회원가입 필수**로 바뀌었다(카카오 개인정보 동의항목 심사 대응,
> [docs/kakao-phone-consent-review.md](kakao-phone-consent-review.md)). 목적에서 "본인확인"을 뺀 이유는
> 이 서비스에 SMS/OTP·본인확인 절차가 실제로 없기 때문이다 — 하지 않는 목적을 신고하면 그 자체가
> 허위 신고가 된다. 생년월일·성별은 가입이 아니라 운세 기능을 쓸 때 받으므로 "선택"이 실측에 맞다
> (개인정보처리방침 2항의 서술과 같다).
