...
status: active
updated: 2026-09-23
next: "기존 셸·나크샤트라·손금·초융합·자미두수 등 미연결 결과의 저장 상태와 공유 어댑터를 실제 경로별로 검증한다."
---
...
## 2026-09-23 후속 적용: 책·편지, 작명·나침반·휴먼디자인

인생의 책과 연애 비책은 기존 버튼을 `PremiumResultShare` 편집기로 보낸다. 저장 완료 결과의 허용 요약만 고르고 90자까지 고친 뒤 실제 책·편지 카드 형태를 전송 전에 본다. 이름은 기본 숨김이며 체크한 경우에만 카드에 넣는다. 공개 상담 URL만 보내고 파일명에도 개인 결과 ID·이름을 넣지 않는다. 로딩·부분·실패·미저장 결과에서는 공유하지 않는다. 개발 mock `verify-book-card-sharing.mjs` 35개(360/390/430/1280px와 다크 표본)와 카드 PNG·취소·가로 넘침을 확인했다.

작명은 저장된 최종 선택 이유·명식 균형·이름의 뜻, 운명 나침반은 최종 파동 완료 후 종합 해설·실천, 휴먼디자인은 저장 완료 리포트의 핵심 문장을 공용 편집기에 연결했다. 작명 입력의 이름·생년, 나침반 질문 원문, 휴먼디자인 차트 개인정보는 선택 목록과 공개 URL에 자동 첨부하지 않는다. 나침반의 구형 소개 공유는 결과 ID가 없는 무료 도착 화면에만 남는다. 세 상담은 어댑터 단위 검사를 통과했지만 결과별 실화면 mock 전수와 실기기 카카오 수신은 아직 미검증이다.
...
### 서양점성·베다 저장 결과와 마스터 인연의 서 mock 재검증

서양점성 결과는 저장 식별자와 `status=completed`가 있을 때 저장된 assistant 본문의 첫·마지막 해설 단락에서만 공유 선택지를 만든다. 베다는 완료 결과의 구조화된 `karma_origin`·`dasha_upaya` 본문을 사용하며, 구형 일반 문장 결과는 첫·마지막 단락을 사용한다. 잘린 원시 JSON과 생성 중/실패 결과는 숨긴다. 질문·출생정보·결과 ID를 선택지나 공개 URL에 별도 첨부하지 않는다. 문장 자체의 사적 내용은 편집기에서 사용자가 확인하도록 안내한다. 두 화면 모두 공유 편집기를 PDF 캡처 밖에 둔다.

`scripts/verify-astrology-vedic-sharing.mjs`의 개발 프리뷰에서 두 결과 × 390/1280px의 편집·이미지 미리보기·가로 넘침·실패 숨김을 확인했다. 같은 검사에서 공개 `/astrology/cosmic/`·`/vedic/jyotish/` 설명 랜딩의 전문가 상담 링크도 390/1280px 실제 클릭으로 확인했다. 결제·유료 생성·운영 DB는 사용하지 않았다.

마스터 인연의 서 mock `Unsealing` 고착은 개발 StrictMode의 effect 재설치가 첫 owner scope를 무효화한 뒤, 같은 in-flight Promise를 재사용해 응답을 버리는 경합이었다. 계정 범위와 요청 유효성을 함께 확인하도록 수정했다. `scripts/verify-master-love-codex-sharing.mjs`에서 완료된 20장 결과가 390/1280px에서 열리고 공유 편집기가 PDF 밖에 나타나는 것, 19장 부분 결과에서 공유를 숨기는 것을 확인했다. 실기기 카카오 수신은 여전히 미검증이다.

### 마스터 인연의 서

`CodexReader`의 기존 전 장 도착 판정(`sealed`)에서만 `ConsultationShare`를 연다. 저장된 각 장의 `keySentence`·`actions`·`insight` 중 첫 문장을 선택·편집하며, 개인 결과 주소 대신 `/master-love-codex/` 공개 진입을 보낸다. 부분 생성·누락된 장·미저장 상태에서는 공유하지 않는다. 질문·이름·생년월일·세션 ID는 어댑터 선택 목록에 넣지 않는다. PDF 문서 밖에 공유 UI를 배치해 소장본 캡처를 보존한다. 기존 단위 검사와 `check:fast`는 앞선 전달에서 통과했다. 이번 mock 실화면 고착 해결과 남은 실기기 카카오 수신은 위 최신 절을 따른다.
...
사용자의 후속 요청으로 실제 책과 비밀 편지의 UI·공유 카드 디자인도 바꿨다. 새 에셋·방향·다른 전문가 상담 계획은 docs/premium-consultation-design-20260923.md에 있다. 당시 남겨 둔 소유자 이름 숨김, 문구 편집, 전송 전 카드 미리보기는 이 문서 맨 위의 2026-09-23 후속 적용에서 진행했다.
...
| 인생의 책·연애 비책 | 책/편지 카드의 이름 기본 숨김·문구 편집·전송 전 미리보기·공개 링크, mock 35개 | 실기기 수신·운영 저장본 재열람 확인 |
| 마스터 연애 | 전 장 저장 완료 시 공용 편집기 연결, 문장·행동·통찰 선택, mock 실화면 확인 | 실기기 수신 확인 |
| 카르마 | 구형·신형 저장 요약의 완료 결과 편집기 연결 | 실제 저장 결과 재열람과 실기기 수신 확인 |
| 서양점성·베다 AI | 완료 저장 결과의 문장 선택·편집·이미지 공유, mock 390/1280px 확인 | 실기기 수신 및 운영 저장본 재열람 확인 |
| 작명·운명 나침반·휴먼디자인 | 저장 완료 해석 선택·편집 어댑터와 공개 URL, 단위 검사 | 실제 결과 경로 mock·실기기 수신 확인 |
| 나크샤트라·손금·초융합·자미두수·전문가 상담 | 개별 결과 화면 존재, 공유 개선 미검증 | `docs/handoff/conversion-sharing-20260922.md` 파일 목록 순서로 적용 |
