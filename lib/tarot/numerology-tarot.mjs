...
function cleanNumerologyText(text) {
  return fixCardNameParticleText(text)
    .replace(/은\(는\)/g, "은")
    .replace(/을\(를\)/g, "을")
    .replace(/가능성를/g, "가능성을")
    .replace(/감정를/g, "감정을")
...
function buildNumerologyCardReading({ entry, topic, topicDef, numerology, question, questionKeywords, index }) {
  ...
  const cardMeta = toText(orientation === "reversed" ? entry?.card?.reversed : entry?.card?.upright) || toText(entry?.card?.upright || entry?.card?.reversed || "");
...
function normalizeInterpretation(raw, fallback, cards, topic, question = "") {
  ...
  const cardReadings = normalizedCards.map((entry, idx) => {
    ...
    const cardMeaning = sanitizeModelText(
      toText(fromModel?.directMeaning) || toText(fromModel?.cardMeaning),
