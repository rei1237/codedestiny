...
function buildDefaultStructuredCardMeaning(card) {
  ...
  return {
    ...
    love: {
      upright: toText(card?.love) || fallbackUpright.join(", "),
      reversed: fallbackReversed.join(", "),
    },
    career: {
      upright: fallbackUpright.join(", "),
      reversed: fallbackReversed.join(", "),
    },
    money: {
      upright: fallbackUpright.join(", "),
...
function getSelectedCardNames(cards = []) {
  return new Set(
    normalizeCardInput(cards)
      .flatMap((entry) => [entry.card?.nameKr, entry.card?.name, entry.card?.nameEn])
...
function mentionsCardName(text, cardName) {
  const source = toText(text);
  const name = toText(cardName);
  if (!source || !name) return false;
  const escaped = escapeRegExp(name);
  const pattern = new RegExp(`(^|[^가-힣A-Za-z])${escaped}(?=\\s*(?:카드|정방향|역방향)|[^가-힣A-Za-z]|$)`, "u");
  return pattern.test(source);
}

function findUnselectedCardMentions(text, cards = []) {
  const source = toText(text);
  if (!source) return [];
  const allowed = getSelectedCardNames(cards);
  return TAROT_CARDS
    .flatMap((card) => [card.nameKr, card.name])
    .map((name) => toText(name))
    .filter((name) => name && !allowed.has(name) && mentionsCardName(source, name));
}

function hasBrokenGeneratedText(text) {
  const source = toText(text);
  if (!source) return true;
  if (/\b(undefined|null|NaN)\b/i.test(source)) return true;
  if (/\{\{[^}]+\}\}|\$\{[^}]+\}|\(\s*\)/.test(source)) return true;
  const cardNamePattern = getCardNamePattern();
  if (cardNamePattern && new RegExp(`(${cardNamePattern})(정방향|역방향)?(가|는|와)(?=\\s|$|[.,!?])`, "u").test(source)) return true;
  return false;
}

function sanitizeModelText(value, fallback = "", cards = []) {
  const cleaned = removeRepeatedCrystalSoulPhrases(sanitizeCrystalSoulText(value));
  if (!cleaned || hasBrokenGeneratedText(cleaned) || findUnselectedCardMentions(cleaned, cards).length) {
    return removeRepeatedCrystalSoulPhrases(sanitizeCrystalSoulText(fallback));
  }
  return cleaned;
}
...
function buildContinuationPrompt({ name, numerology, cards, topic, question, interpretation }) {
  const topicDef = getTopicReadingDef(topic);
  const userName = toText(name) || "내담자";
  const selectedCards = normalizeCardInput(cards, topic).slice(0, 5).map((entry, idx) => {
    const identity = getCardIdentity(entry, idx);
    const cardNumber = identity.arcanaNumber === null ? "번호 없음" : `${identity.arcanaNumber}번`;
    return `${idx + 1}. ${identity.cardNameKr}(${identity.cardNameEn}, ${cardNumber}, ${identity.orientationLabel}, ${entry.positionLabel})`;
  });
  return [
    "아래 내용을 바탕으로 이어서 한국어 상담을 진행해 주세요.",
    `이름: ${userName}`,
    `상담 주제: ${topicDef.label}`,
    `질문: ${toText(question)}`,
    `생명수: ${numerology?.lifePathNumber ?? "-"}`,
    `개인일수: ${numerology?.personalDayNumber ?? "-"}`,
    `질문수: ${numerology?.questionNumber ?? "-"}`,
    `선택 카드: ${selectedCards.join(" / ")}`,
    `핵심 답변: ${toText(interpretation?.directAnswer || interpretation?.coreMessage)}`,
    `마무리 조언: ${toText(interpretation?.counselorClosing || interpretation?.conclusion?.finalWord)}`,
    "미래를 단정하지 말고, 사용자가 현실에서 확인할 수 있는 선택지와 다음 행동을 중심으로 답해 주세요.",
  ].filter(Boolean).join("\n");
}
...
function buildNumerologyCardReading({ entry, topic, topicDef, numerology, question, questionKeywords, index }) {
  ...
  const roleLead = [
    "첫 번째 카드는 현재 마음과 출발점을 봅니다.",
    "두 번째 카드는 관계나 상황에서 확인되는 반응을 봅니다.",
    "세 번째 카드는 반복되는 장애물이나 놓치기 쉬운 부분을 봅니다.",
    "네 번째 카드는 도움이 되는 태도와 표현 방식을 봅니다.",
    "다섯 번째 카드는 가까운 시기의 방향과 조언을 봅니다.",
  ][index] || `${positionTitle} 자리를 봅니다.`;
  const roleAdvice = [
    "먼저 내 기대와 실제로 받은 반응을 분리해 보면 출발점이 선명해집니다.",
    "상대나 상황의 반응은 단번에 판단하기보다 반복되는 행동을 기준으로 살피는 편이 안정적입니다.",
    "장애물은 감정의 크기보다 같은 방식으로 반응해 온 습관에서 더 분명하게 드러납니다.",
    "도움이 되는 태도는 긴 설명보다 짧고 분명한 표현에서 시작됩니다.",
    "가까운 방향은 결론을 서두르는 쪽보다 기준을 정리한 뒤 움직이는 쪽에 가깝습니다.",
  ][index] || "이 자리는 질문을 현실에서 확인할 수 있는 행동으로 좁혀 줍니다.";
  const roleNumerologyBridge = [
    topicDef.bridgeSeed,
    "숫자의 흐름은 반응을 크게 해석하기보다 실제로 반복되는 표현과 속도를 확인하게 합니다.",
    "질문수는 같은 장애물이 어디에서 되풀이되는지 좁혀 보게 하고, 카드는 그 장면을 구체화합니다.",
    "개인일수는 오늘 선택할 말의 속도를 낮추고, 이 카드는 도움이 되는 태도를 현실 언어로 바꾸게 합니다.",
    "마지막 자리는 숫자와 카드의 흐름을 한 번에 확정하지 않고 가까운 선택의 방향으로 정리합니다.",
  ][index] || topicDef.bridgeSeed;
  const roleHiddenPattern = [
    "첫 번째 자리에서는 마음의 출발점과 실제로 확인된 반응을 구분하지 못하는 흐름이 숨어 있을 수 있습니다.",
    "두 번째 자리에서는 상대나 상황의 반응을 너무 빠르게 의미 부여하는 흐름이 숨어 있을 수 있습니다.",
    "세 번째 자리에서는 같은 방식으로 반응해 온 습관이 실제 장애물보다 더 크게 느껴질 수 있습니다.",
    "네 번째 자리에서는 마음은 분명해도 표현 방식이 길어지거나 무거워지는 패턴을 살펴야 합니다.",
    "다섯 번째 자리에서는 가까운 결론을 서두르며 작은 신호를 놓치는 흐름이 숨어 있을 수 있습니다.",
  ][index] || topicDef.hiddenIssueSeed;
  ...
  const matchedDirectMeaning = sentenceJoin([
    ...
    `${topicDef.label} 질문에서는 ${structuredMeaning.topicMeaning}을 중심으로 읽습니다.`,
    `명리학으로는 ${structuredMeaning.myeongriBridge.join(", ")} 기운과 연결해 보아야 합니다.`,
    orientationNote,
    topicNote,
  ]);
  const numerologyBridge = sentenceJoin([
    `생명수 ${lifePath}는 안정적으로 확인하려는 성향을, 개인일수 ${personalDay}는 오늘의 움직임 속도를 보여 줍니다.`,
    `질문수 ${questionNumber}은 ${positionTitle} 자리에서 ${questionFocus}를 먼저 살피게 합니다.`,
  ]);
  const contextualInterpretation = sentenceJoin([
    roleLead,
    `사용자의 질문에서 '${questionFocus}'이 핵심으로 올라오므로, '${cardNameKr}' 카드는 이 부분을 추측이 아니라 확인 가능한 반응으로 읽으라고 조언합니다.`,
    roleAdvice,
    roleNumerologyBridge,
  ]);
...
function buildFallbackInterpretation({ numerology, cards, topic, name, question }) {
  ...
  const directAnswer = sentenceJoin([
    ...
    cardReadings[0]?.cardNameKr && cardReadings[4]?.cardNameKr
      ? `첫 카드 '${cardReadings[0].cardNameKr}' 카드와 마지막 카드 '${cardReadings[4].cardNameKr}' 카드가 서로 다른 속도를 보여 주므로, 감정만으로 결론내리기보다 단계적으로 확인하는 편이 좋습니다.`
      : "",
  ]);
  ...
  return {
    ...
    nextActions: conclusion.doThis.slice(0, 3),
...
function normalizeInterpretation(raw, fallback, cards, topic, question = "") {
  ...
  const parsedCards = Array.isArray(parsed.cards)
    ? parsed.cards
    : (Array.isArray(parsed.cardReadings) ? parsed.cardReadings : []);

  const topicReading = {
    topic: normalizedTopic,
    topicLabel: toText(parsed?.topicReading?.topicLabel) || toText(fallback?.topicReading?.topicLabel) || topicDef.label,
    topicOverview: sanitizeModelText(toText(parsed?.topicReading?.topicOverview), toText(fallback?.topicReading?.topicOverview) || `${topicDef.topicOverviewSeed}`, normalizedCards),
    whyThisTopicMatters: sanitizeModelText(toText(parsed?.topicReading?.whyThisTopicMatters), toText(fallback?.topicReading?.whyThisTopicMatters) || `${topicDef.whyThisTopicMattersSeed}`, normalizedCards),
    numerologyTopicBridge: sanitizeModelText(toText(parsed?.topicReading?.numerologyTopicBridge), toText(fallback?.topicReading?.numerologyTopicBridge) || `${topicDef.bridgeSeed}`, normalizedCards),
  };

  const categoryDeepDive = {
    currentFlow: sanitizeModelText(toText(parsed?.categoryDeepDive?.currentFlow), toText(fallback?.categoryDeepDive?.currentFlow) || `${topicDef.currentFlowSeed}`, normalizedCards),
    hiddenIssue: sanitizeModelText(toText(parsed?.categoryDeepDive?.hiddenIssue), toText(fallback?.categoryDeepDive?.hiddenIssue) || `${topicDef.hiddenIssueSeed}`, normalizedCards),
    opportunity: sanitizeModelText(toText(parsed?.categoryDeepDive?.opportunity), toText(fallback?.categoryDeepDive?.opportunity) || `${topicDef.opportunitySeed}`, normalizedCards),
    risk: sanitizeModelText(toText(parsed?.categoryDeepDive?.risk), toText(fallback?.categoryDeepDive?.risk) || `${topicDef.riskSeed}`, normalizedCards),
    timing: sanitizeModelText(toText(parsed?.categoryDeepDive?.timing), toText(fallback?.categoryDeepDive?.timing) || `${topicDef.timingSeed}`, normalizedCards),
  };
  ...
  const cardReadings = normalizedCards.map((entry, idx) => {
    ...
    const identity = getCardIdentity(entry, idx);
    const title = toText(fromModel?.positionTitle) || toText(fromModel?.title) || toText(fallbackCard?.title) || entry.positionLabel;
    ...
    const cardNameKr = identity.cardNameKr;
    const cardNameEn = identity.cardNameEn;
    const orientation = identity.orientation;
    const orientationLabel = identity.orientationLabel;
    const cardMeaning = sanitizeModelText(
      toText(fallbackCard?.directMeaning) || toText(fallbackCard?.cardMeaning),
      toText(fallbackCard?.directMeaning) || toText(fallbackCard?.cardMeaning) || `${entry.card?.upright || entry.card?.reversed || ""}`,
      normalizedCards,
    );
    const numerologyBridge = sanitizeModelText(
      toText(fromModel?.numerologyBridge),
      toText(fallbackCard?.numerologyBridge) || `${topicDef.bridgeSeed}`,
      normalizedCards,
    );
    const topicInterpretation = sanitizeModelText(
      toText(fromModel?.contextualInterpretation) || toText(fromModel?.topicInterpretation),
      toText(fallbackCard?.contextualInterpretation) || toText(fallbackCard?.topicInterpretation) || `${topicDef.focus}`,
      normalizedCards,
    );
    const hiddenPattern = sanitizeModelText(
      toText(fromModel?.hiddenPattern),
      toText(fallbackCard?.hiddenPattern) || `${topicDef.hiddenIssueSeed}`,
      normalizedCards,
    );
    const actionTip = sanitizeModelText(
      toText(fromModel?.practicalAdvice) || toText(fromModel?.actionTip),
      toText(fallbackCard?.practicalAdvice) || toText(fallbackCard?.actionTip) || `${topicDef.actionRules[idx % topicDef.actionRules.length]}`,
      normalizedCards,
    );
    const caution = sanitizeModelText(
      toText(fromModel?.caution),
      toText(fallbackCard?.caution) || `${topicDef.cautionRules[idx % topicDef.cautionRules.length]}`,
      normalizedCards,
    );
    ...
    return {
      ...
      cardNameKr,
      cardNameEn,
      arcanaNumber: identity.arcanaNumber,
      orientation,
      orientationLabel,
      cardMeaning,
      numerologyBridge,
      ...
      actionTip,
      caution,
      usedMeaningKeywords: Array.isArray(fallbackCard?.usedMeaningKeywords) ? fallbackCard.usedMeaningKeywords : [],
      meaningTopicKey: toText(fallbackCard?.meaningTopicKey),
      directMeaning: cardMeaning,
      contextualInterpretation: topicInterpretation,
      practicalAdvice: actionTip,
    };
  });

  const conclusion = {
    summary: sanitizeModelText(toText(parsed?.conclusion?.summary), toText(fallback?.conclusion?.summary) || `${topicDef.finalWordSeed}`, normalizedCards),
    ...
    sevenDayPlan: wantsSevenDayPlan(question) && Array.isArray(parsed?.conclusion?.sevenDayPlan) && parsed.conclusion.sevenDayPlan.length
      ? normalizeTextList(parsed.conclusion.sevenDayPlan, 7)
      : (wantsSevenDayPlan(question) ? uniqueTextList(fallback?.conclusion?.sevenDayPlan || topicDef.sevenDayPlan).slice(0, 7) : []),
    finalWord: sanitizeModelText(toText(parsed?.conclusion?.finalWord), toText(fallback?.conclusion?.finalWord) || `${topicDef.finalWordSeed}`, normalizedCards),
  };

  const synthesis = {
    currentSituation: sanitizeModelText(
      toText(parsed?.synthesis?.currentSituation),
      toText(fallback?.synthesis?.currentSituation) || categoryDeepDive.currentFlow,
      normalizedCards,
    ),
    opportunity: sanitizeModelText(
      toText(parsed?.synthesis?.opportunity),
      toText(fallback?.synthesis?.opportunity) || categoryDeepDive.opportunity,
      normalizedCards,
    ),
    challenge: sanitizeModelText(
      toText(parsed?.synthesis?.challenge),
      toText(fallback?.synthesis?.challenge) || categoryDeepDive.hiddenIssue,
      normalizedCards,
    ),
    likelyDirection: sanitizeModelText(
      toText(parsed?.synthesis?.likelyDirection),
      toText(fallback?.synthesis?.likelyDirection) || categoryDeepDive.timing,
      normalizedCards,
    ),
  };

  const nextActions = Array.isArray(parsed?.nextActions) && parsed.nextActions.length
    ? normalizeTextList(parsed.nextActions, 3)
    : uniqueTextList(fallback?.nextActions || conclusion.doThis || topicDef.actionRules).slice(0, 3);

  const interpretation = {
    readingId: toText(parsed.readingId) || toText(fallback.readingId),
    title: toText(parsed.title) || toText(fallback.title) || `${topicDef.label} 수비학 타로 상담`,
    topic: normalizedTopic,
    userQuestion: toText(parsed.userQuestion) || toText(fallback.userQuestion) || toText(question),
    opening: sanitizeModelText(toText(parsed.opening), toText(fallback.opening), normalizedCards),
    directAnswer: sanitizeModelText(toText(parsed.directAnswer), toText(fallback.directAnswer || fallback.coreMessage), normalizedCards),
    numerologyInsight: {
      summary: sanitizeModelText(toText(parsed?.numerologyInsight?.summary), toText(fallback?.numerologyInsight?.summary || fallback.numerologyReading), normalizedCards),
      relevantNumbers: Array.isArray(fallback?.numerologyInsight?.relevantNumbers)
        ? fallback.numerologyInsight.relevantNumbers
        : [],
    },
    cardStory: sanitizeModelText(toText(parsed.cardStory), toText(fallback.cardStory), normalizedCards),
    cards: cardReadings.map((item) => ({
      cardId: item.cardId,
      cardName: item.cardNameKr,
      arcanaNumber: item.arcanaNumber,
    ...
    synthesis,
    nextActions,
    counselorClosing: sanitizeModelText(toText(parsed.counselorClosing), toText(fallback.counselorClosing || conclusion.finalWord), normalizedCards),
    disclaimer: sanitizeModelText(
      toText(parsed.disclaimer),
      toText(fallback.disclaimer) || "이 상담은 상징을 바탕으로 현재의 선택지를 정리하는 안내이며, 건강·법률·투자·안전 문제는 전문적인 판단을 대신하지 않습니다.",
      normalizedCards,
    ),
    continuationPrompt: sanitizeModelText(
      toText(parsed.continuationPrompt),
      toText(fallback.continuationPrompt) || buildContinuationPrompt({
        name: "",
        numerology: fallback?.numerology || {},
        cards: normalizedCards,
        topic: normalizedTopic,
        question,
        interpretation: { directAnswer: toText(parsed.directAnswer) || toText(fallback.directAnswer), conclusion },
      }),
      normalizedCards,
    ),
    numerologyReading: sanitizeModelText(toText(parsed.numerologyReading), fallback.numerologyReading, normalizedCards),
    coreMessage: sanitizeModelText(toText(parsed.coreMessage), fallback.coreMessage, normalizedCards),
    topicReading,
    cardReadings,
    categoryDeepDive,
    conclusion,
  ...
  const validation = validateNumerologyTarotQuality(interpretation, normalizedTopic, normalizedCards);
...
function buildGeminiPrompt({ numerology, cards, topic, question, name }) {
  ...
  const lifeData = NUMEROLOGY_DATA[numerology.lifePathNumber] || NUMEROLOGY_DATA[9];
  const questionKeywords = extractQuestionKeywords(question);
  const firstIdentity = getCardIdentity(safeCards[0], 0);

  return [
    "당신은 수비학과 타로를 상징적인 성찰 도구로 사용하는 한국어 상담형 리더입니다.",
    "미래를 확정적으로 예언하거나 상대방의 속마음을 사실처럼 단정하지 않습니다.",
    "사용자가 자신의 상황과 감정을 정리하고, 현실에서 확인할 수 있는 선택지를 발견하도록 돕습니다.",
    "반드시 한국어 JSON 객체 하나만 반환하고 markdown, HTML, 코드펜스, 설명문은 출력하지 않습니다.",
    "입력으로 제공된 사용자 질문, 상황 설명, 검증된 수비학 데이터, 실제로 선택된 타로 카드만 사용합니다.",
    "첫 두 문단 안에서 사용자의 질문에 직접 답합니다.",
    "사용자가 뽑지 않은 카드는 어떤 경우에도 언급하지 않습니다.",
    "각 카드 이름, 번호, 정·역방향, 포지션은 입력 데이터와 정확히 일치시킵니다.",
    "카드의 사전적 의미를 길게 설명하지 말고 사용자의 질문과 연결된 의미를 중심으로 씁니다.",
    "수비학 숫자는 운명을 결정하는 값이 아니라 성향과 반복되는 선택 방식을 살피는 참고 정보로 설명합니다.",
    "같은 핵심 문장이나 표현을 여러 섹션에 재사용하지 않습니다.",
    "속삭입니다, 운명, 우주, 필연, 결, 기운, 감정의 온도, 관계 가능성, 현실 조건 같은 표현은 반복하지 않습니다.",
    "한국어 조사를 자연스럽게 사용하고 카드 이름 뒤에 조사를 직접 붙이지 않습니다.",
    "현실에서 확인할 수 있는 행동 2~3개만 제안합니다.",
    "사용자가 요청하지 않았다면 7일 계획, 의식, 명상, 확언을 만들지 않습니다.",
    "건강, 법률, 투자, 안전 질문은 전문적인 판단을 대신하지 않는다는 점을 안내합니다.",
    "전체 길이는 한글 기준 약 1,800~2,800자 안으로 유지합니다.",
    "",
    "[입력 컨텍스트]",
    JSON.stringify({
      ...
      generationRules: [
        "같은 주어를 두 번 붙이지 않습니다.",
        "카드 의미는 selectedMeaningKeywords, selectedTopicMeaning, orientation 값만 기준으로 사용합니다.",
        "카드별 해석은 명리학 핵심 기운, 선택 카드 의미, 연결, 현실 조언, 주의점, 오늘의 행동 1개 순서로 씁니다.",
      ],
      numerologyLens: topicDef.numerologyLens,
      cards: safeCards.map((entry, idx) => {
        const meaning = getNumerologyTarotCardMeaning(entry.card, entry.orientation, normalizedTopic);
        return {
        order: idx + 1,
        ...
        arcanaNumber: getOfficialCardNumber(entry.card),
        ...
        selectedMeaningKeywords: meaning.selectedKeywords,
        selectedTopicMeaning: meaning.topicMeaning,
        meaningTopicKey: meaning.topicKey,
        myeongriBridge: meaning.myeongriBridge,
    ...
    "",
    "[선택된 카드명 화이트리스트]",
    safeCards.map((entry) => `${entry.card.nameKr} / ${entry.card.name}`).join(", "),
    "",
    "[카드 역할]",
    JSON.stringify({
      topicLabel: topicDef.label,
      roles: [
        "첫 카드: 현재 사용자의 마음 또는 출발점",
        "두 번째 카드: 관계나 상황에서 확인되는 반응",
        "세 번째 카드: 반복되는 장애물 또는 놓치고 있는 부분",
        "네 번째 카드: 도움이 되는 태도와 표현",
        "다섯 번째 카드: 가까운 시기의 방향과 조언",
      ],
    }, null, 2),
    "",
    "[반드시 반환할 JSON 스키마]",
    JSON.stringify({
      ...
      title: `${topicDef.label} 수비학 타로 상담`,
      topic: normalizedTopic,
      userQuestion: toText(question),
      opening: "상담사의 첫 답변",
      directAnswer: "질문에 대한 핵심 답변",
      numerologyInsight: {
        summary: "질문과 관련된 수비학 요약",
        relevantNumbers: [{
          label: numerologyTarotText("numerologyTarot.lifePathNumber"),
          value: "4",
          meaning: "안정과 현실",
          relevance: "질문과 연결되는 이유",
        }],
      },
      cardStory: "다섯 장의 카드가 앞뒤로 어떻게 연결되는지 설명",
      cards: [{
        cardId: firstIdentity.cardId,
        cardName: firstIdentity.cardNameKr,
        arcanaNumber: firstIdentity.arcanaNumber,
        orientation: firstIdentity.orientation,
        positionTitle: safeCards[0]?.positionLabel || "첫 번째 자리",
        directMeaning: "이 카드가 질문에 주는 답",
        contextualInterpretation: "사용자 질문에 연결된 해석",
        practicalAdvice: "도움이 되는 행동",
        caution: "주의점",
      }],
      synthesis: {
        ...
        opportunity: "기회",
        challenge: "주의할 부분",
        likelyDirection: "가까운 시기의 방향",
      },
      nextActions: ["행동 1", "행동 2", "행동 3"],
      counselorClosing: "선택권을 돌려주는 따뜻한 마무리",
      disclaimer: "필요한 전문 판단 안내",
      continuationPrompt: "AI 상담 이어가기에 사용할 요약문. 본문에는 렌더링하지 않는다.",
    }, null, 2),
  ].join("\n");
}
...
function validateNumerologyTarotQuality(interpretation, topic, cards = []) {
  const normalizedTopic = normalizeTopic(topic);
  const topicDef = getTopicReadingDef(normalizedTopic);
  ...
  const summaryText = sentenceJoin([
    ...
    interpretation?.numerologyInsight?.summary,
    interpretation?.cardStory,
    interpretation?.synthesis?.currentSituation,
    interpretation?.synthesis?.opportunity,
    interpretation?.synthesis?.challenge,
    interpretation?.synthesis?.likelyDirection,
    ...(Array.isArray(interpretation?.nextActions) ? interpretation.nextActions : []),
    interpretation?.counselorClosing,
    ...
    ...cardReadings.map((item) => [item.cardNameKr, item.cardNameEn, item.orientationLabel, item.cardMeaning, item.topicInterpretation, item.hiddenPattern, item.actionTip, item.caution].join(" ")),
    ...(Array.isArray(interpretation?.cards) ? interpretation.cards.map((item) => [item.cardName, item.positionTitle, item.directMeaning, item.contextualInterpretation, item.practicalAdvice, item.caution].join(" ")) : []),
  ]);
  ...
  if (hasBrokenGeneratedText(summaryText)) {
    warnings.push("broken placeholder, nullish token, or Korean particle pattern found");
  }

  const unselectedMentions = findUnselectedCardMentions(summaryText, cards);
  if (cards.length && unselectedMentions.length) {
    warnings.push(`unselected card mentioned: ${uniqueTextList(unselectedMentions).join(", ")}`);
  }

  if (cards.length && cardReadings.some((item, idx) => Number(item?.arcanaNumber) !== getOfficialCardNumber(cards[idx]?.card))) {
    warnings.push("card number mismatch with selected card registry");
  }

  const duplicateCheckText = sentenceJoin([
    interpretation?.opening,
    interpretation?.directAnswer,
    interpretation?.numerologyInsight?.summary,
    interpretation?.cardStory,
    ...
    interpretation?.counselorClosing,
    interpretation?.disclaimer,
    ...cardReadings.map((item) => [
      item.cardNameKr,
      item.cardNameEn,
      item.orientationLabel,
      item.cardMeaning,
      item.topicInterpretation,
      item.hiddenPattern,
      item.caution,
    ].join(" ")),
  ]);
  const sentences = (duplicateCheckText.match(/[^.!?。！？]+[.!?。！？]?/gu) || [duplicateCheckText])
    .map((sentence) => normalizeComparableText(sentence))
    .filter((sentence) => sentence.length >= 18);
  const duplicateSentence = sentences.some((sentence, idx) => sentences.indexOf(sentence) !== idx);
  if (duplicateSentence) {
    warnings.push("duplicate sentence detected");
  }
