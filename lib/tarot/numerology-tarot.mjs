...
function buildFallbackInterpretation({ numerology, cards, topic, name, question }) {
  ...
  const cardReadings = safeCards.map((entry) => {
    ...
    return {
      ...
      keywordFocus,
      interpretation: `${entry.card.nameKr} (${entry.orientation === "reversed" ? "역방향" : "정방향"})은(는) ${lifeData.keyword} 기운과 연결됩니다. 질문 키워드 '${keywordFocus}' 관점에서는 ${tone}`,
      actionTip: `오늘 '${keywordFocus}'와 직접 연결되는 행동 1가지를 일정으로 고정하세요.`,
    };
  });
...
function buildGeminiPrompt({ numerology, cards, topic, question, name }) {
  ...
  return [
    ...
    "문체는 따뜻하지만 권위 있게 유지하고, 질문 키워드를 해석의 중심축으로 삼아라.",
    "각 카드 해석은 3~5문장으로 작성하고 마지막에 실행 팁 1문장을 포함한다.",
    "출력은 JSON 한 개만 반환한다.",
    "",
    "[입력]",
    JSON.stringify({
    ...
    "[출력 JSON 스키마]",
    JSON.stringify({
      numerologyReading: "생명수와 현재 흐름, 질문 키워드 연결을 4~6문장으로 설명",
      coreMessage: "핵심 계시 한 문장",
      cardReadings: [{
        title: "포지션명",
        keywordFocus: "해당 카드가 집중할 질문 키워드 1개",
        interpretation: "카드와 숫자 연결 해석 3~5문장",
        actionTip: "바로 실행 가능한 1문장 행동 팁",
      }],
      conclusion: {
        summary: "전체 흐름 요약",
