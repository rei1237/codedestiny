...
const TOPIC_READING_DEFS = {
  ...
  general: {
    ...
    cardInterpretationRules: [
      ...
      "카드는 전체 방향을, 수비학 숫자는 실행 리듬을 정리합니다.",
...
function sentenceJoin(parts = []) {
  return uniqueTextList(parts).join(" ").replace(/\s{2,}/g, " ").trim();
}
...
function buildNumerologyCardReading({ entry, topic, topicDef, numerology, question, questionKeywords, index }) {
  ...
  const numerologyBridge = sentenceJoin([
    `${topicDef.label}에서 ${cardNameKr}는 카드 번호 ${cardNumber + 1}과 ${suit} 계열의 상징을 함께 품고 있습니다.`,
    `${topicDef.numerologyLens} 생명수 ${lifePath}, 개인수 ${personalDay}, 질문수 ${questionNumber}가 현재 포지션 ${index + 1}의 질문과 맞물리면서 해석의 우선순위를 정합니다.`,
    `${questionNumber}라는 질문수는 ${positionQuestion}의 답을 바로 넓게 보지 말고, ${questionFocus}부터 먼저 확인하라고 말합니다.`,
  ]);
  ...
  const hiddenPattern = sentenceJoin([
    ...
    `${cardNameKr} ${orientationLabel}은(는) 겉으로 보이는 ${questionFocus} 뒤에 ${orientation === "reversed" ? "지연, 회피, 미해결 감정" : "기대, 가능성, 조정 필요"}를 숨길 수 있습니다.`,
...
function buildTopicReading(topic, numerology, questionKeywords, name, question) {
  ...
  return {
    ...
    topicOverview: sentenceJoin([
      `${userName}님의 ${topicDef.label}은(는) ${topicDef.focus}를 중심으로 읽어야 하며, 질문 '${questionText}'는 현재 ${topKeywords}의 결을 직접 묻고 있습니다.`,
      topicDef.topicOverviewSeed,
    ]),
    whyThisTopicMatters: sentenceJoin([
      `${topicDef.whyThisTopicMattersSeed} 생명수 ${lifePath}, 개인수 ${personalDay}, 질문수 ${questionNumber}가 함께 움직이면서 지금의 선택이 미래의 관계·일·금전 흐름을 바꿀 수 있습니다.`,
    ]),
    numerologyTopicBridge: sentenceJoin([
      `${topicDef.bridgeSeed} ${topicDef.numerologyLens}`,
      `특히 질문수 ${questionNumber}는 ${topicDef.label}에서 가장 먼저 다뤄야 할 포지션을 지정하고, 개인수 ${personalDay}는 오늘 바로 실행할 속도를 정합니다.`,
    ]),
  };
}
...
function buildCategoryDeepDive(topic, numerology, cardReadings) {
  ...
  return {
    currentFlow: sentenceJoin([
      ...
      `첫 카드 ${first.cardNameKr || first.title || "첫 포지션"}는 출발점을, 두 번째 카드 ${second.cardNameKr || second.title || "두 번째 포지션"}는 반응과 기회가 어디로 가는지를 보여 줍니다.`,
    ]),
    ...
    opportunity: sentenceJoin([
      ...
      `네 번째 카드 ${fourth.cardNameKr || fourth.title || "네 번째 포지션"}와 다섯 번째 카드 ${fifth.cardNameKr || fifth.title || "다섯 번째 포지션"}는 ${topicDef.label}이 실제 행동으로 연결되는 기회를 보여 줍니다.`,
    ]),
    risk: sentenceJoin([
      `${topicDef.riskSeed}`,
    ...
    timing: sentenceJoin([
      `${topicDef.timingSeed}`,
      `생명수 ${Number(numerology?.lifePathNumber || 9)}와 개인수 ${Number(numerology?.personalDayNumber || 9)}의 조합은 이번 흐름을 하루 단위보다 7일 단위로 보는 편이 더 정확하다고 말합니다.`,
    ]),
  };
}
...
function buildConclusion(topic, numerology, cardReadings, topicReading, categoryDeepDive) {
  ...
  return {
    ...
    finalWord: sentenceJoin([
      ...
      `${topicReading.topicLabel}은(는) 질문에 대한 대답이기도 하지만, 지금 어떤 순서로 움직여야 하는지를 알려 주는 실행 지도입니다.`,
    ]),
  };
}
...
function buildFallbackInterpretation({ numerology, cards, topic, name, question }) {
  ...
  return {
    numerologyReading: sentenceJoin([
      `${userName}님의 생명수 ${numerology.lifePathNumber}는 '${lifeData.keyword}'의 파장을 품고 있습니다.`,
      `${lifeData.meaning}을(를) ${topicDef.label}의 실제 질문에 연결할수록 결과가 더 또렷해집니다.`,
      `${topicReading.topicOverview} ${topicReading.numerologyTopicBridge}`,
    ]),
    coreMessage: `${topicDef.label}은(는) 감정의 느낌만 보는 것이 아니라, ${topicDef.focus}를 어디까지 현실화할 수 있는지 확인하는 흐름입니다.`,
    topicReading,
    cardReadings,
    categoryDeepDive,
    conclusion,
...
function buildGeminiPrompt({ numerology, cards, topic, question, name }) {
  ...
  return [
    ...
    "수비학 숫자와 카드 번호의 연결, 정/역방향, 포지션 제목, 실제 질문 원문을 모두 결합해야 한다.",
    ...
    "[반드시 반환할 JSON 스키마]",
    JSON.stringify({
      numerologyReading: "수비학과 카드의 전체 해석문",
