...
function removeRepeatedCrystalSoulPhrases(text) {
  const parts = sanitizeCrystalSoulText(text).split(/(?<=[.!?。！？]|다\.|요\.)\s+/u);
  const seen = new Set();
  return parts
    .filter((part) => {
...
function findUnselectedCardMentions(text, cards = []) {
  ...
  return TAROT_CARDS
    ...
    .map((name) => toText(name))
    .filter((name) => name && !allowed.has(name) && source.includes(name));
}
...
function hasBrokenGeneratedText(text) {
  ...
  const cardNamePattern = getCardNamePattern();
  if (cardNamePattern && new RegExp(`(${cardNamePattern})\\s*(정방향|역방향)?\\s*(가|는|와)(?=\\s|$|[.,!?])`, "u").test(source)) return true;
  return false;
}
