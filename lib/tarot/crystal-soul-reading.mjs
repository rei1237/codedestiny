...
function buildReadingText({ gem, position, cardInfo, direction }) {
  ...
  if (position.pos_id === 1) {
    return `지금 당신 안에는 ${gem.core}${josa(gem.core, "이/가")} 실제로 움직이고 있습니다. ${cardInfo.scene} ${cardInfo.nameKo} ${direction}은 ${gem.lens}를 묻고, ${reversed ? "조용히 틀어진 감각을 바로잡으라고 가리킵니다." : "이미 알고 있던 감각을 믿어도 된다고 드러납니다."}`;
  }
  ...
  if (position.pos_id === 3) {
    const cautionTarget = reversed ? "확신 없이 미루는 태도" : "익숙한 방식으로만 밀어붙이는 선택";
    return `지금 ${cautionTarget}${josa(cautionTarget, "을/를")} 조심하세요. ${cardInfo.scene} ${cardInfo.nameKo}${josa(cardInfo.nameKo, "은/는")} ${gem.core}의 보호 아래, 같은 패턴을 반복하기 전에 한 걸음 멈추라고 드러납니다.`;
  }
  if (position.pos_id === 4) {
    return `${gem.name}이 권하는 처방은 ${reversed ? "힘을 덜어내고 기준을 다시 세우는 일" : "작게라도 분명한 행동을 선택하는 일"}입니다. ${cardInfo.scene} ${cardInfo.nameKo}${josa(cardInfo.nameKo, "은/는")} 오늘 당신에게 ${gem.lens}를 따라 정리할 한 가지를 남깁니다.`;
  }
...
function buildAiFortunePrompt(readingData) {
  ...
  return [
    ...
    "아래 원석 소울 타로 결과를 바탕으로, 사용자가 결제 후 확인한 결과의 가장 하단에 이어질 추가 운세를 작성해 주세요.",
    "문체는 전문적이고 신비로우며 감정적으로 자연스럽게 유지합니다. 차가운 안내문이나 판정식 표현은 피합니다.",
    "출력은 1) 오늘의 핵심 운세, 2) 사랑과 관계, 3) 일과 재물, 4) 마음의 경고, 5) 원석이 건네는 마지막 문장 순서로 작성합니다.",
    "각 항목은 2~4문장으로 쓰고, 원석의 의미와 에너지가 카드 해석 안에 자연스럽게 흐르게 합니다.",
    "",
