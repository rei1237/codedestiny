...
const POSITION_LENS = [
  ...
  },
  {
    ...
    detailFocus: "상대가 느끼는 매력과 경계",
    insightFocus: "호감과 부담의 동시 신호",
    adviceFocus: "신뢰 회복 행동",
  },
  {
    title: "상대의 연애 의지와 열망",
    detailFocus: "감정과 실행 의지의 일치 여부",
    insightFocus: "연락/만남/약속 이행 의지",
    adviceFocus: "작은 약속 검증",
  },
  {
    title: "관계를 가로막는 핵심 요인",
    detailFocus: "반복되는 갈등 트리거",
    insightFocus: "오해를 키우는 패턴",
    adviceFocus: "갈등 루프 차단",
  },
  {
...
function buildDiversifiedFallback(field, row, idx) {
  ...
  if (field === "relationshipInsight") {
    return `${lens.insightFocus}을 읽을 때는 말의 온도와 행동 패턴을 함께 비교해야 오해를 줄일 수 있습니다.`;
  }
  if (field === "advice") {
  ...
  if (field === "caution") {
    return row.__meta.orientation === "reversed"
      ? "불안을 확인 요구로 밀어붙이면 상대가 방어적으로 닫힐 수 있습니다."
      : "호감 신호가 보여도 결론을 서두르면 리듬이 무너질 수 있습니다.";
  }
  return "";
}
...
function buildPositionReading(cards, basePosition = []) {
  const rows = cards.map((card, idx) => {
    ...
    const insightFallback = `${lens.insightFocus}을 확인할 때 답장 속도보다 약속 이행률, 말의 톤, 연락 간격의 패턴을 함께 보아야 정확도가 높아집니다.`;
