...
const POSITION_COPY = [
  ...
  {
    detail(row, { prev, next, suitLine, orientLine }) {
      return `${row.cardName} ${row.orientationLabel}은 상대가 이 관계에 어떤 이름을 붙이려 하는지 보여 줍니다. ${suitLine}의 결은 상대가 끌림을 느껴도 바로 약속이나 정의로 옮기지 않을 수 있음을 말합니다. ${orientLine} 기운이 섞여 있어 상대의 기준은 내 기대보다 느리거나 조심스러울 수 있습니다. ${prev ? `1번 ${prev.cardName}에서 생긴 내 시선과 비교하면, 여기서는 상대가 관계를 안전하게 다루려는 방식이 더 선명합니다.` : ""} ${next ? `다음 ${next.cardName}은 그 정의 뒤에 숨어 있는 실제 감정의 온도를 열어 줍니다.` : ""}`;
    },
    insight(row, { suitLine }) {
      return `${row.positionTitle}에서 ${row.cardName} 카드는 관계의 이름, 거리, 속도를 정하는 상대의 방식을 보여 줍니다. ${suitLine}은 마음보다 기준이 먼저 움직일 수 있음을 암시합니다.`;
    },
    advice(row) {
  ...
  {
    detail(row, { prev, next, suitLine, orientLine }) {
      return `${row.cardName} ${row.orientationLabel}은 상대가 나에게 실제로 느끼는 감정의 온도를 비춥니다. 여기서 중요한 것은 관계의 이름이 아니라 마음이 먼저 반응하는 방식입니다. ${suitLine}의 기운은 끌림, 호기심, 조심스러움이 한데 섞여 흐를 수 있음을 보여 줍니다. ${orientLine} 흐름이 있으니 상대의 침묵이 곧 무관심은 아닐 수 있고, 다정한 말이 곧 확정도 아닐 수 있습니다. ${prev ? `앞의 ${prev.cardName}이 관계의 틀을 말했다면, 이 카드는 그 틀 안에서 실제 감정이 얼마나 따뜻한지 알려 줍니다.` : ""} ${next ? `다음 ${next.cardName}은 이 감정이 행동 의지로 이어질 수 있는지를 묻습니다.` : ""}`;
    },
    insight(row, { suitLine }) {
      return `${row.positionTitle}에서 ${row.cardName} 카드는 감정의 세기와 질감을 보여 줍니다. ${suitLine}은 상대 마음이 완전히 닫힌 것보다 아직 말로 정리되지 않은 떨림에 가깝다고 속삭입니다.`;
    },
    advice(row) {
  ...
  {
    detail(row, { prev, next, suitLine, orientLine }) {
      return `${row.cardName} ${row.orientationLabel}은 두 사람 사이를 가로막는 매듭을 보여 줍니다. 이 매듭은 마음 부족만이 아니라 타이밍, 자존심, 과거의 기억, 주변 상황처럼 말로 꺼내기 어려운 현실일 수 있습니다. ${suitLine}의 신호는 병목이 어디에서 반복되는지 알려 주고, ${orientLine} 흐름은 그 매듭을 서두르면 더 단단해질 수 있음을 말합니다. ${prev ? `앞의 ${prev.cardName}에서 보인 행동 의지가 이 자리에서 잠시 멈칫하는 이유가 드러납니다.` : ""} ${next ? `다음 ${next.cardName}은 이 매듭을 그대로 둘 때 가까운 미래가 어떤 모양으로 굳어지는지 보여 줍니다.` : ""}`;
    },
    insight(row, { suitLine }) {
      return `${row.positionTitle}에서 ${row.cardName} 카드는 관계를 늦추는 현실의 매듭입니다. ${suitLine}은 감정보다 구조, 타이밍, 부담의 문제가 더 크게 작동할 수 있음을 비춥니다.`;
    },
    advice(row) {
  ...
  {
    detail(row, { prev, next, suitLine, orientLine }) {
      return `${row.cardName} ${row.orientationLabel}은 가까운 미래에 이 관계가 어떤 온도로 굳어질지 보여 줍니다. 이것은 영원한 결론이 아니라 지금의 말투, 거리, 선택이 2~6주 안에 만들 수 있는 장면입니다. ${suitLine}의 결은 앞으로 남을 감정의 형태를 알려 주고, ${orientLine} 흐름은 서두르지 않을 때 오히려 더 분명해지는 신호가 있음을 말합니다. ${prev ? `5번 ${prev.cardName}의 병목을 어떻게 다루느냐에 따라 이 카드의 미래는 더 부드럽게 바뀔 수 있습니다.` : ""} ${next ? `이어지는 ${next.cardName}의 기운도 함께 살펴야 합니다.` : "마지막 자리로서 전체 배열의 숨결을 하나의 관계 장면으로 모읍니다."}`;
    },
    insight(row, { suitLine }) {
      return `${row.positionTitle}에서 ${row.cardName} 카드는 지금의 흐름이 가까운 미래에 남길 인상입니다. ${suitLine}은 관계가 완전히 닫히기보다 어떤 형태로 기억되고 이어질지를 보여 줍니다.`;
    },
    advice(row) {
...
function buildDiversifiedFallback(field, row, idx, cards) {
  ...
  if (field === "summary") {
    return `${row.positionTitle}에서 ${row.cardName} ${row.orientationLabel}은 두 사람 사이에 흐르는 ${suitLine}의 온도를 비춥니다. 특히 ${orientLine} 기운이 함께 움직여, 같은 말과 침묵도 서로 다른 마음의 무늬로 남기 쉽습니다.`;
  }
  if (field === "detail") {
  ...
  if (field === "orderConnection") {
    if (idx === 0 && next) return `1번 카드는 2번 ${next.cardName} ${next.orientationLabel}로 이어지며, 첫 시선이 관계의 이름을 어떻게 흔드는지 보여 줍니다.`;
    ...
    if (idx === 3 && next) return `4번 카드는 5번 ${next.cardName}에서 멈칫하는 이유를 비추며, 감정이 행동으로 옮겨질 수 있는지를 살핍니다.`;
    if (idx === 4 && next) return `5번 카드는 6번 ${next.cardName} ${next.orientationLabel}의 가까운 미래를 만드는 숨은 매듭입니다.`;
    return "6번 카드는 전체 배열의 마지막 문장으로, 처음의 시선이 가까운 미래의 관계 온도로 굳어지는 모습을 보여 줍니다.";
  }
  return "";
}
...
function buildDefaultSections(rows, matrix) {
  ...
  return {
    counselorTone: "두 사람의 감정 온도와 행동 의지를 따로 살피고, 카드가 놓인 순서대로 관계의 숨은 속도를 부드럽게 읽습니다.",
    overallVibe: `${first.cardName} ${first.orientationLabel}에서 ${last.cardName} ${last.orientationLabel}로 이어지는 이번 배열은 첫 시선과 가까운 미래의 온도 차이를 보여 줍니다. 6장의 카드는 마음의 크기보다 두 사람이 서로에게 허락한 속도, 말하지 못한 기대, 관계의 이름을 붙이는 방식이 더 중요하다고 속삭입니다. 시작 구간에는 강한 끌림이나 해석이 먼저 생겼고, 마지막 구간에는 아직 고르지 못한 기준과 망설임이 남아 있습니다. 지금 필요한 것은 확답을 끌어내는 압박이 아니라, 서로가 편안하게 진심을 드러낼 수 있는 대화의 공간입니다. ${matrix.dominantSuit} 신호가 전체 공기를 이끌며 이 관계의 가장 깊은 흔들림을 비춥니다.`,
    deepReading: `${rows[1].cardName} ${rows[1].orientationLabel}은 상대가 이 관계를 어떤 이름으로 바라보는지 보여 주고, ${rows[2].cardName} ${rows[2].orientationLabel}은 상대가 나에게 실제로 느끼는 감정의 온도를 드러냅니다. 두 카드는 비슷해 보여도 결이 다릅니다. 하나는 관계를 정의하려는 머리의 언어이고, 다른 하나는 마음이 먼저 반응하는 감정의 언어입니다. 이어서 ${rows[3].cardName} ${rows[3].orientationLabel}은 그 감정이 실제 행동으로 옮겨질 힘이 있는지를 묻습니다. 상대가 모순적으로 보인다면 마음이 없는 것이 아니라, 감정과 실행의 속도가 아직 맞지 않는 것일 수 있습니다. 그러므로 대화는 추궁보다 확인, 결론보다 리듬 조율에 가까워야 합니다.`,
...
export function normalizeLoveReadingPayload(reading, cards) {
  ...
  let normalized = {
    ...
    summary: cleanText(src.summary || "6장의 카드는 내가 본 상대, 상대가 바라본 관계, 숨은 감정, 연애 의지, 막힌 매듭, 가까운 미래의 온도를 차례로 비춥니다."),
    ...
    finalAdvice: {
      ...
      checklist: Array.isArray(src?.finalAdvice?.checklist) && src.finalAdvice.checklist.length
        ...
        : [
          `${positionBreakdown[4]?.cardName || "5번 카드"}가 비추는 매듭을 자극하는 급한 확인 메시지 멈추기`,
          `${positionBreakdown[2]?.cardName || "3번 카드"}의 감정 신호는 단정하지 말고 실제 말과 행동으로 확인하기`,
          `${positionBreakdown[3]?.cardName || "4번 카드"}가 보여 주는 의지는 말보다 작은 제안과 약속으로 살피기`,
