...
function analyzeTarotCombinations(entries, questionType, spread) {
  ...
  if (reversedCount >= totalCards * 0.5) {
    insights.push({
      ...
      title: "역방향 비율 우세",
      description: "내면 에너지가 외부 행동보다 느리게 따라오는 구간입니다. 속도 경쟁보다 경계 재정렬과 감정 명료화가 우선입니다.",
    });
  }
  ...
  if (spread && Array.isArray(spread.positions) && safeEntries.length >= 2) {
    ...
    const direction = isSelfEsteemSpread
      ...
      : (orientationOf(last) === "reversed"
        ? "마지막 카드는 속도를 늦추고 기준을 재정렬하라는 신호입니다."
        : "마지막 카드는 중간 합의를 거칠수록 흐름이 안정된다는 신호입니다.");
  ...
  if (!insights.length) {
    insights.push({
      type: "storyFlow",
      title: "기본 흐름",
      description: "카드 간 연결은 조용하지만 일관됩니다. 지금은 예측보다 실천 가능한 작은 행동을 반복하는 편이 결과를 바꿉니다.",
    });
  }
