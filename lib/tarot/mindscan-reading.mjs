...
function buildInnerHeartTarotSection(card, position, orientation, extras = {}) {
  ...
  const cardMeaning = sanitizeInnerHeartTarotText(
    `카드 '${cardNameKo}' ${orientationLabel(orient)}의 첫 장면은 ${keywords.slice(0, 2).join(", ")}의 결을 비춥니다. ${orientationNarrative(orient)} ${suitNarrative(suit)} 이 빛은 특히 '${masterGuide.lens}'에 숨은 마음의 방향을 드러냅니다.`,
  );

  const positionMeaning = sanitizeInnerHeartTarotText(
    `${positionMeta.title} 자리는 '${positionMeta.question}'라는 물음을 비춥니다. 여기서는 ${positionMeta.purpose} 같은 흐름을 중심에 두고, '${masterGuide.coreQuestion}'라는 속삭임을 따라가야 합니다.`,
  );

  const emotionalReading = sanitizeInnerHeartTarotText(
    `${cardNameKo}의 상징은 지금 상대가 마음을 완전히 닫았다기보다, 감정과 방어 사이에서 속도를 재고 있음을 보여줍니다. ${psychologySeed} 이 구간은 감정의 유무를 단정하기보다, 마음이 다시 안전하다고 느낄 조건을 살펴야 하는 국면입니다.`,
  );
  ...
  const advice = sanitizeInnerHeartTarotText(
    sanitizeMindscanAdvice(extras.advice, masterGuide.action) || `짧고 안정적인 대화 신호를 먼저 보내며 반응 간격을 존중하는 태도가 유리합니다.`,
  );
...
function buildSummary(sections) {
  ...
  const contactPossibility = safe[4]?.emotionalReading || "연락의 문은 열릴 수 있으나 압박 없는 접점에서만 부드럽게 움직입니다.";
  const relationshipRisk = removeRepeatedInnerHeartPhrases(
    Array.from(new Set([safe[3]?.caution, safe[4]?.caution].filter(Boolean))).join(" "),
  ) || "불안이 커지는 순간 확인을 강요하면 관계가 다시 닫힐 수 있습니다.";
...
function buildMindscanPrompt({ question, questionKeywords, pairs }) {
  return [
    "너는 상대의 겉태도와 숨은 감정의 간극을 섬세하게 읽는 마인드 스캔 타로 상담가다.",
    "사용자가 입력한 질문을 반드시 리딩의 중심축으로 삼아야 한다.",
    "뻔한 키워드 나열은 금지한다. 카드의 절벽, 하얀 강아지, 두 기둥, 저울, 번개, 물결, 칼끝 같은 구체적인 상징과 원소를 직접 언급해라.",
    "각 섹션은 질문과 카드 상징의 인과관계를 설명해야 하며, 단정적인 예언 대신 왜 그런 결론이 나왔는지 논리적으로 보여줘라.",
    ...
    JSON.stringify({
      ...
      sections: [{
        ...
        subCardName: "보조 카드명",
        orientation: "upright",
        cardMeaning: "카드의 구체적인 상징과 원소를 언급한 3~5문장",
        positionMeaning: "포지션 질문과의 연결 2~4문장",
        emotionalReading: "상대/상황 심리와 질문의 연결 3~5문장",
        hiddenMessage: "숨겨진 메시지 2~3문장",
        caution: "주의점 1~2문장",
        advice: "실행 조언 1~2문장",
      ...
      summaryCard: {
        ...
        emotionalTemperatureText: "감정 온도 한 줄",
        corePsychology: "핵심 심리",
        contactChance: "연락/반응 가능성",
        relationFlow: "관계 흐름",
        reApproachChance: "재접근 여지",
        recommendedAction: "추천 행동",
        relationshipStage: "관계 단계",
        silenceDriver: "침묵/지연 동인",
        situationPressure: "압력 요인",
        emotionalNeed: "숨은 욕구",
      },
      innerHeartSummary: {
        emotionalTemperature: "전체 감정 온도",
        hiddenCore: "숨은 핵심 마음",
        contactPossibility: "연락 가능성",
        relationshipRisk: "관계 위험",
        recommendedAttitude: "권장 태도",
        finalFlow: "최종 흐름",
        oracleMessage: "한 줄 메시지",
      },
      suggestedMessages: [{ tone: "부담 완화", text: "실행 가능한 한 줄 메시지" }],
      oneLineConclusion: "질문에 대한 최종 한 줄",
      closing: "따뜻하지만 단단한 마무리",
    }, null, 2),
  ].join("\n");
}

async function callMindscanGemini({ question, pairs, env = {}, fetchImpl = globalThis.fetch }) {
  const apiKey = pickGeminiKey(env);
  if (!apiKey || typeof fetchImpl !== "function") {
    return { ok: false, reason: "missing_config" };
  }

  const normalizedPairs = Array.isArray(pairs) ? pairs.slice(0, 5).map((pair, idx) => ({
    slot: Number(pair?.slot || idx + 1),
    positionLabel: toText(pair?.positionLabel) || POSITION_TITLES[idx] || `포지션 ${idx + 1}`,
  ...
  const prompt = buildMindscanPrompt({ question, questionKeywords, pairs: normalizedPairs });
...
function buildMindscanLocalReading(pairs, question = "") {
  ...
  const summaryCard = {
    ...
    reApproachChance: "상대의 방어가 풀리는 순간부터 재접근의 문이 조금씩 열리며, 긴 메시지보다 알맞은 간격과 부드러운 첫 문장이 흐름을 더 크게 움직입니다.",
  ...
  return {
    ...
    question: asText(question),
    intro: question
      ? `질문 "${asText(question)}"를 중심에 두고 보면, 카드는 감정이 사라져서 멈춘 것이 아니라 감정과 방어가 동시에 작동해 속도를 늦추는 흐름을 보여줍니다. 따라서 이번 리딩의 핵심은 질문의 초점을 따라, 어떤 조건에서 마음이 열리고 어떤 방식에서 다시 닫히는지를 읽는 데 있습니다.`
      : "카드는 감정이 사라져서 멈춘 것이 아니라, 감정과 방어가 동시에 작동해 속도를 늦추는 흐름을 보여줍니다. 따라서 이번 리딩의 핵심은 '마음이 있다/없다'의 이분법이 아니라, 어떤 조건에서 마음이 열리고 어떤 방식에서 다시 닫히는지를 읽는 것입니다.",
    ...
    masterAdvice: `${summary.recommendedAttitude} 또한 지금 단계에서는 감정 증명보다 대화 속도, 경계 문장, 재접촉 간격을 먼저 가다듬어야 관계의 문이 무리 없이 다시 열립니다.`,
    suggestedMessages: [
...
async function buildMindscanReadingPayload(pairs, options = {}) {
  ...
  if (!question) {
    return { ok: false, message: "상담 질문이 필요합니다." };
  }
  ...
  try {
    const aiResult = await callMindscanGemini({
      question,
      pairs: safePairs.map((pair, idx) => {
        const normalized = normalizePair(pair, idx);
        const pairOrientation = pickOrientation(normalized.mainCard, normalized.subCard, idx + 1);
        return {
          ...normalized,
    ...
    if (aiResult?.ok && aiResult.reading) {
      ...
      const reading = {
        ...
        summaryCard,
        innerHeartSummary,
        insightDeck: fallback.insightDeck,
    ...
    // fall back to the local engine when Gemini fails.
  }
