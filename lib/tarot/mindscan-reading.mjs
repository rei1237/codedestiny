...
function buildMindscanPrompt({ question, questionKeywords, pairs }) {
  return [
    "너는 마인드 스캔 타로를 전문적으로 읽는 한국어 상담가다.",
    "사용자가 입력한 질문을 반드시 상담의 중심축으로 삼아야 한다.",
    ...
    "아래 JSON 스키마의 키 이름을 유지하고, 문자열은 한국어로만 작성해라.",
...
function buildMindscanLocalReading(pairs, question = "") {
  ...
  return {
    ...
    question: asText(question),
    intro: question
      ? `질문 "${asText(question)}"를 중심에 두고 보면, 카드는 상대가 마음이 없어서 멈춘 것이 아니라 감정과 방어가 동시에 작동해 속도를 늦추는 흐름을 보여줍니다. 따라서 이번 리딩의 핵심은 질문의 초점을 따라, 어떤 조건에서 마음이 열리고 어떤 방식에서 다시 닫히는지를 읽는 데 있습니다.`
      : "카드는 상대가 마음이 없어서 멈춘 것이 아니라, 감정과 방어가 동시에 작동해 속도를 늦추는 흐름을 보여줍니다. 따라서 이번 리딩의 핵심은 '좋아하느냐/아니냐'의 이분법이 아니라, 어떤 조건에서 마음이 열리고 어떤 방식에서 다시 닫히는지를 읽는 것입니다.",
