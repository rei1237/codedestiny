...
// 공통 Gemini 키는 GEMINIF_API_KEY 하나만 사용한다(lib/llm-client.ts의 GEMINI_KEY_ORDER와 동일).
function getGeminiKeyPool(env = {}) {
  return [toText(env?.GEMINIF_API_KEY), toText(process?.env?.GEMINIF_API_KEY)].filter(Boolean);
}
...
function buildInnerHeartTarotSection(card, position, orientation, extras = {}) {
  ...
  const cardMeaning = sanitizeInnerHeartTarotText(
    cardCoreText
      ? `카드 '${cardNameKo}' ${orientationLabel(orient)}의 첫 장면은 ${keywords.slice(0, 2).join(", ")}의 결을 비춥니다. ${cardCoreText} 이 빛은 특히 '${masterGuide.lens}'에 숨은 마음의 방향을 드러냅니다.`
  ...
  const emotionalReading = sanitizeInnerHeartTarotText(
    `${cardNameKo} ${orientationLabel(orient)}이 비추는 상대의 속마음은 이렇습니다. ${psychologySeed} ${positionMeta.title} 자리에서는 감정의 유무를 성급히 단정하기보다, 상대가 다시 안전하다고 느낄 조건을 먼저 살펴야 합니다.`,
  );
...
async function requestMindscanGeminiOnce({ prompt, env, fetchImpl, apiKey }) {
    ...
    `https://generativelanguage.googleapis.com/v1beta/models/${encodeURIComponent(getGeminiModel(env))}:generateContent?key=${encodeURIComponent(apiKey)}`,
    {
      ...
      body: JSON.stringify({
        ...
        generationConfig: {
          ...
          maxOutputTokens: boundedNumber(env.MINDSCAN_GEMINI_MAX_OUTPUT_TOKENS, 9000, 3000, 16000),
...
async function callMindscanGemini({ question, pairs, insightSeeds = [], env = {}, fetchImpl = globalThis.fetch }) {
  ...
  for (let attempt = 0; attempt < 2; attempt += 1) {
    try {
      const result = await requestMindscanGeminiOnce({ prompt, env, fetchImpl, apiKey });
