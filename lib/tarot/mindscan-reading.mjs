...
function isStagingLlmMockEnabled(env = {}) {
  const appEnv = toText(env?.APP_ENV).toLowerCase();
  const flag = toText(env?.STAGING_LLM_MOCK_ENABLED).toLowerCase();
  const workersAi = toText(env?.WORKERS_AI_ENABLED).toLowerCase();
