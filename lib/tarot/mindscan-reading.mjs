...
function buildSummaryCard(sections) {
  ...
  const relationshipStage = defensive > reconnective
    ? "유보형 거리두기 단계"
    : "탐색형 재연결 단계";

  const silenceDriver = defensive >= reconnective
    ? "자존심과 상처 재발 우려가 먼저 작동해 연락을 미루는 흐름"
    : "반응을 확인한 뒤 움직이려는 타이밍 탐색 흐름";

  const situationPressure = emotionalTemperature <= 2
    ? "감정 피로도가 높아 작은 자극에도 방어적으로 반응할 수 있음"
    : emotionalTemperature >= 4
      ? "감정 에너지는 남아 있으나 속도 조절 실패 시 급격히 식을 수 있음"
      : "관계 변수를 계산하는 중간 구간으로 접근 강도 조절이 중요함";

  const emotionalNeed = defensive >= reconnective
    ? "안전감과 존중을 먼저 확인받고 싶은 욕구"
    : "부담 없는 연결 속에서 관심이 유지되는지 확인받고 싶은 욕구";
...
function asBulletArray(lines, max = 3) {
...
function buildInsightDeck(sections, summaryCard) {
  const safeSections = Array.isArray(sections) ? sections : [];
  const bySlot = (slot) => safeSections.find((item) => Number(item?.slot) === slot) || {};
  ...
  return [
    {
      ...
      category: "감정 핵심",
      title: "상대방 감정의 중심",
      headline: sanitizeText(summaryCard?.corePsychology || "호감과 경계가 동시에 작동하는 상태"),
      summary: sanitizeText(s2?.summary || s2?.content || "겉반응보다 실제 속마음이 더 복합적으로 움직이는 흐름입니다."),
      bullets: asBulletArray(s2?.detail, 3),
      riskLevel: Number(summaryCard?.emotionalTemperature || 3) <= 2 ? "높음" : "중간",
    },
    {
      ...
      category: "상황 변수",
      title: "현재 상대방 상황 해석",
      headline: sanitizeText(summaryCard?.situationPressure || "관계 변수 계산이 많은 구간"),
      summary: sanitizeText(s5?.summary || "관계를 완전 종료보다 유보 상태로 두고 상황을 점검하는 흐름입니다."),
      bullets: asBulletArray([summaryCard?.relationshipStage, summaryCard?.silenceDriver, ...(s5?.detail || [])], 3),
      riskLevel: /거리|방어|차가움/.test(asText(s5?.loveSignal)) ? "중상" : "중간",
    },
    {
      id: "silence-reason",
      icon: "🧱",
      category: "침묵 이유",
      title: "연락을 미루는 진짜 이유",
      headline: sanitizeText(summaryCard?.silenceDriver || "마음보다 방어심리가 먼저 작동"),
      summary: sanitizeText(s3?.summary || s3?.content || "침묵은 무관심보다 자기보호 해석에 더 가깝습니다."),
      bullets: asBulletArray(s3?.detail, 3),
      riskLevel: /방어|거리두기/.test(asText(s3?.loveSignal)) ? "높음" : "중간",
    },
    {
      id: "contact-window",
      icon: "📨",
      category: "연락 타이밍",
      title: "지금 연락 가능 구간",
      headline: sanitizeText(summaryCard?.contactChance || "반응 가능성은 열려 있으나 강도 조절 필요"),
      summary: sanitizeText(s6?.summary || s6?.content || "짧은 신호 교환 중심의 접근이 유리합니다."),
      bullets: asBulletArray(s6?.detail, 3),
      riskLevel: /중하|시간 간격/.test(asText(summaryCard?.reApproachChance) + asText(summaryCard?.contactChance)) ? "중상" : "중간",
    },
    {
      id: "hidden-need",
      icon: "🫧",
      category: "숨은 욕구",
      title: "상대가 받고 싶은 반응",
      headline: sanitizeText(summaryCard?.emotionalNeed || "존중과 안전감을 먼저 확인받고 싶어함"),
      summary: sanitizeText(s4?.summary || s4?.content || "겉으로 말하지 않지만 관계 연결 신호를 확인받고 싶은 상태입니다."),
      bullets: asBulletArray(s4?.detail, 3),
      riskLevel: "중간",
    },
    {
      id: "action-protocol",
      ...
      category: "행동 프로토콜",
      title: "당신의 실전 접근 순서",
      headline: sanitizeText(summaryCard?.recommendedAction || "부담 없는 안부로 접점 만들기"),
      summary: sanitizeText(s7?.summary || s7?.content || "확답 요구보다 연결감 회복이 우선입니다."),
      bullets: asBulletArray(s7?.detail, 3),
      riskLevel: "중간",
    },
  ].map((item) => ({
    ...item,
    ...
    riskLevel: sanitizeText(item.riskLevel || "중간"),
  }));
}
...
export function buildMindscanReadingPayload(rawPairs) {
  ...
  const insightDeck = buildInsightDeck(sections, summaryCard);
