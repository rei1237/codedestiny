import { logTarotTokenUsage } from "./token-usage.mjs";
...
import { assertGeminiInputTokenLimit } from "../gemini-input-token-limit.mjs";
...
function buildMindscanPrompt({ question, questionKeywords, pairs, insightSeeds = [], locale }) {
  const questionLens = inferMindscanQuestionLens(question);
  const promptBody = [
  ...
  const directive = buildOutputLanguageDirective(toAiLocale(locale));
...
async function requestMindscanGeminiOnce({ prompt, env, fetchImpl, apiKey, maxOutputTokens, timeoutMs }) {
  ...
  try {
    await assertGeminiInputTokenLimit({ apiKey, model, generateContentRequest: requestBody, fetchImpl, timeoutMs });
  } catch (error) {
    return { ok: false, reason: toText(error?.code).toLowerCase() || "llm_input_token_count_unavailable", permanent: true };
  }
  const generationTimeoutMs = Number(timeoutMs) - (Date.now() - startedAt);
  if (generationTimeoutMs <= 0) return { ok: false, reason: "deadline_exceeded", permanent: true };
  // 개별 Gemini 호출에 상한을 건다. 생성이 늘어지면 abort시켜 상위 루프가 로컬 폴백으로 degrade하게 한다.
  const controller = typeof AbortController === "function" ? new AbortController() : null;
  const timeoutId = controller ? setTimeout(() => controller.abort(), generationTimeoutMs) : null;
  let response;
  try {
  ...
  logTarotTokenUsage(payload, { env, model: getGeminiModel(env), serviceId: "tarot-mindscan", maxTokens: maxOutputTokens });
...
async function callMindscanGemini({ question, pairs, insightSeeds = [], env = {}, fetchImpl = globalThis.fetch, locale }) {
  ...
  const prompt = buildMindscanPrompt({ question, questionKeywords, pairs: normalizedPairs, insightSeeds, locale });
...
export function buildMindscanDeliveryFacts(pairs, question, locale = 'ko') {
  if (!Array.isArray(pairs) || pairs.length !== 5 || !asText(question)) return { ok: false, reason: 'INVALID_INPUT' };
  const validCard = value => typeof value === 'number'
    ? Number.isInteger(value) && value >= 0 && value < 78
    : Boolean(asText(value) && getTarotCardByAnyId(value));
  if (pairs.some(pair => !validCard(pair?.mainCardId ?? pair?.mainCardCode ?? pair?.mainCardName)
    || !validCard(pair?.subCardId ?? pair?.subCardCode ?? pair?.subCardName))) return { ok: false, reason: 'INVALID_CARD' };
  const sectionInputs = buildSectionInputs(pairs.map(normalizePair)).map((pair, index) => {
    const position = INNER_HEART_TAROT_SPREAD[index];
    const orientation = pickOrientation(pair.mainCard, pair.subCard, index + 1);
    return { slot: index + 1, positionLabel: position.title, positionQuestion: position.question,
      positionPurpose: position.purpose, positionMeaning: position.purpose, orientation,
      mainCard: buildMindscanCardProfile(pair.mainCard, orientation), subCard: buildMindscanCardProfile(pair.subCard, orientation) };
  });
  const base = buildMindscanLocalReading(pairs, question);
  const prompt = buildMindscanPrompt({ question, questionKeywords: parseQuestionKeywords(question), pairs: sectionInputs,
    insightSeeds: base.insightDeck, locale }).split('[리딩 작성 형식]')[0]
    .split('\n').filter(line => !/sections는|insightSeeds가 주어지면|아래 리딩 형식|emotionalTemperature는|suggestedMessages는|문자열은 한국어로/.test(line)).join('\n');
  return { ok: true, base, sectionInputs, prompt: `${buildOutputLanguageDirective(toAiLocale(locale))}\n${prompt}` };
}
...
async function buildMindscanReadingPayload(pairs, options = {}) {
  ...
  if (isStagingLlmMockEnabled(options?.env || {})) {
    return { ...localFallback, source: "staging_mock_local_fallback", llmFailReason: "staging_mock" };
  }
  let llmFailReason = "";
  try {
