...
//
// 🔴 16개 전부가 자기 주제를 갖는다. 예전에는 6개(self·crisis·choice·spiritual·legal·special)가
// general 로 뭉개져 "법률 분쟁"과 "영적 메시지"가 같은 프롬프트를 받았고, third_party·power 는
// 각각 love·career 로 접혀 고유한 구도를 잃었다. verify:oracle-consultation 의 전수 가드가
// 이 표에 카테고리가 빠지거나 general 로 되돌아가는 것을 막는다.
//
// relationship 은 people 이 아니라 relationship 프로파일로 간다 — 그 프로파일은 원래 있었는데
// 아무 카테고리도 쓰지 않아 사장돼 있었다.
...
function outputLanguageLabel(locale) {
  return AI_LOCALE_LABEL[toAiLocale(locale)];
}
...
export function buildOracleConsultationPrompt({ spreadTitle, category, question, tone, cards, locale = "ko", env = {} }) {
  ...
  const systemPrompt = [
    ...
    buildOutputLanguageDirective(toAiLocale(locale)),
  ].join("\n");
...
// 주입된 어댑터(worker/lib/tarot-oracle-llm.js)를 통해 한 번 부른다.
// 🔴 JSON 파싱은 직접 fetch 경로와 **같은 parseJsonFromText 를 쓴다** — 두 경로가 파서를 공유해야
// mock 검증이 실경로를 대표한다. 어댑터 쪽 재시도는 어댑터가 이미 끝낸 상태로 들어온다.
async function requestOracleConsultationViaAdapter({ callJson, prompt, systemPrompt, maxOutputTokens, timeoutMs }) {
  const ai = await callJson({ systemPrompt, userPrompt: prompt, maxOutputTokens, timeoutMs });
  if (!ai?.ok) return { ok: false, reason: toText(ai?.reason) || "adapter_failed", permanent: true };

  const parsed = parseJsonFromText(ai.text);
  if (!parsed) return { ok: false, reason: ai.truncated ? "invalid_json_truncated" : "invalid_json", permanent: true };
  return { ok: true, consultation: parsed, provider: toText(ai.provider) };
}
...
export async function generateOracleConsultation(input, options = {}) {
  ...
  // 🔴 워커는 정본 경로(lib/llm-client.ts → Gemini + Workers AI 폴백 체인)를 어댑터로 주입한다
  //    (worker/lib/tarot-oracle-llm.js). 이 파일은 node·Jest 에서도 그대로 로드되므로 .ts 를
  //    직접 물 수 없어서 주입으로 받는다. 주입이 없으면 아래 직접 fetch 경로가 그대로 돈다 —
  //    verify 스크립트가 그 경로를 mock 으로 검증한다.
  const callJson = typeof options?.callJson === "function" ? options.callJson : null;
  const apiKey = pickGeminiKey(env);
  // 어댑터가 있으면 키·fetch 는 어댑터가 챙긴다.
  if (!callJson && (!apiKey || typeof fetchImpl !== "function")) {
  ...
  for (let attempt = 0; attempt < 3; attempt += 1) {
    ...
    try {
      const result = callJson
        ? await requestOracleConsultationViaAdapter({ callJson, prompt, systemPrompt, maxOutputTokens, timeoutMs: remainingMs })
        : await requestOracleConsultationGeminiOnce({
          prompt,
          systemPrompt,
          env,
          fetchImpl,
      ...
      if (result.ok) {
        ...
        if (chars >= minChars) {
          // Workers AI 폴백으로 나온 결과는 따로 표시해 둔다 — 응답 source 로 올라가므로
          // "Gemini 가 죽었는데 결과는 나갔다"가 로그에서 보인다.
          return { ok: true, consultation: result.consultation, source: result.provider === "workers-ai" ? "llm_fallback" : "llm" };
        }
      ...
      // 🔴 어댑터가 붙어 있으면 전송 재시도는 **어댑터가 이미 했다**. 여기서 또 돌면 중첩 재시도가
      //    되어 데드라인만 태운다(CLAUDE.md 원칙 6). 분량 미달 재요청은 위에서 continue 로 빠져
      //    이 줄에 닿지 않으므로 영향받지 않는다.
      if (callJson) break;
