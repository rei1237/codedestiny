...
function buildSelfEsteemLevelUpGuide({ spreadTitle, positionReadings }) {
  ...
  const flow = selfEsteemCleanSentence(
    ...
    `이번 흐름은 ${spreadTitle || "자기 기준 회복 타로"}의 다섯 장 연결을 보여줍니다.`,
  );
...
function interpretTarotReading(input) {
  ...
  const summary = `${title} 결과입니다. 이번 리딩은 ${interpretedCards.map((card) => card.cardNameKo).join(", ")} 흐름으로 구성되며, 단정하지 않고 카드의 순서와 방향을 함께 볼수록 선택 기준이 선명해집니다.`;
  ...
  const finalReading = `${summary}\n${combinationReading}\n지금 중요한 건 감정을 크게 해석하는 것보다, 카드가 비춘 흐름을 오늘의 선택 기준과 작은 행동으로 옮기는 일입니다.`.trim();
...
function mapRelationshipReading(result) {
  ...
  const projectionGap = `${c1.cardName} ${c1.orientationLabel}는 내가 상대에게 씌운 기대의 렌즈를, ${c3.cardName} ${c3.orientationLabel}는 카드가 비춘 감정 온도를 보여줍니다. 두 신호가 다르면 지금은 상대의 마음을 단정하기보다, 크게 해석한 장면과 실제로 반복된 행동을 분리해 보는 편이 안전합니다.`;
  ...
  const blockToOutcome = `${c5.cardName} ${c5.orientationLabel}의 병목이 그대로 남으면 ${c6.cardName} ${c6.orientationLabel}의 가까운 흐름은 지연이나 유보처럼 느껴질 수 있습니다. 다만 이 배열은 확정 예언이 아니라, 장애물을 다루는 방식에 따라 온도가 달라지는 구간을 보여줍니다.`;
  ...
  const wholeStory = `${sequenceFlow}. 시작 카드 ${c1.cardName}와 마지막 카드 ${c6.cardName}의 대비는 첫 인상과 가까운 흐름 사이의 온도 차를 보여줍니다. ${majorCount >= 2 ? "메이저 카드가 강하게 관여해 감정만이 아니라 관계 구조를 다시 살필 필요가 있습니다." : "메이저 비중이 낮아도 반복되는 소통 방식은 관계의 리듬을 천천히 굳힐 수 있습니다."} 역방향 비율 ${Math.round((reversedCount / Math.max(1, facts.length)) * 100)}%는 표현 지연과 오해 관리가 중요하다는 신호이며, ${dominantSuitLabel} 기운이 이번 리딩의 주된 공기입니다.`;
  ...
  const positionBreakdown = facts.map((f, idx) => {
    ...
    const detail = `${f.cardName} ${f.orientationLabel}은 ${f.positionTitle} 자리에서 두 사람의 상호작용이 어디서 엇갈리는지 보여줍니다. 특히 ${f.relMeaning.communicationPattern} 흐름이 강하면 같은 말도 다르게 받아들여질 수 있습니다. ${f.relMeaning.commitmentSignal} 신호가 함께 보이므로, 감정 확인과 관계 정의를 한꺼번에 요구하기보다 말의 온도와 행동의 반복을 따로 관찰하는 편이 좋습니다.`;
  ...
  const overallVibe = `${c1.cardName} ${c1.orientationLabel}에서 ${c6.cardName} ${c6.orientationLabel}로 이어지는 6장 배열은 관계를 '내 시선-상대의 프레임-감정 온도-행동 의지-막힘-가까운 흐름'의 순서로 비춥니다. 지금의 핵심은 누가 더 좋아하느냐보다 서로의 속도와 해석 방식을 맞출 수 있느냐입니다. ${dominantSuitLabel} 신호와 역방향 ${reversedCount}장은 감정 자체보다 말의 해석, 타이밍, 기준선 조율이 중요하다고 말합니다.`;

  const deepReading = `${c2.cardName} ${c2.orientationLabel}은 관계의 이름과 속도를 조심스럽게 재는 태도를, ${c3.cardName} ${c3.orientationLabel}은 카드가 비춘 감정 온도를 보여줍니다. 다만 감정 온도와 관계 정의는 같은 뜻이 아닙니다. ${c4.cardName} ${c4.orientationLabel}은 마음이 행동으로 옮겨지는 방식을 보여주므로, 감정 확인을 재촉하기보다 실제로 움직일 조건을 짧고 분명하게 맞추는 대화가 더 안전합니다.`;
...
const REUNION_ACTION_SCRIPT = {
  ...
  their_now: {
    upright: "말보다 행동의 반복을 보세요. 작은 일관성이 남은 마음의 결을 보여 줍니다.",
...
function mapYearlyReading(result) {
  ...
  const monthlyReadings = (Array.isArray(result.cards) ? result.cards : []).slice(0, 12).map((card, idx) => {
    ...
    const triadReading = cleanYearlyLanguage(`월초·월중·월말의 세 갈래 월운은 신호, 선택, 결과의 온도를 나누어 보여줍니다. ${zodiac.theme}의 기운과 ${card.cardNameKo} ${orientationLabelText}이 만나는 지점에서 이번 달의 속도와 기준을 읽어야 합니다.`);
...
function mapSelfEsteemReading(result) {
  ...
  const opening = selfEsteemCleanSentence(
    `자기 기준 회복 타로가 ${cardFlow || "다섯 장의 카드"}의 흐름을 통해 마음의 성장 지도를 엽니다. 오늘의 리딩은 눈치 보기와 자기검열을 비난하지 않고, 그 감각이 어디서 시작되어 어떤 경계와 루틴으로 다시 단단해지는지 보여줍니다.`,
