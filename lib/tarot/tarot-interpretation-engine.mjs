...
function buildHealingCardReading(card, idx) {
  ...
  // 카드 고유 core(재회 오버라이드에 영향받지 않는 필드)만 회복 리딩 본문에 녹여 카드 고유성을 살린다.
  // shadow/recoveryAdvice는 회복 컨셉 전용 템플릿을 유지해 다른 기능의 조언이 새지 않도록 한다.
  const cardCore = asText(card?.coreMeaning || card?.meaning?.coreMeaning);
  return {
