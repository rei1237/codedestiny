...
function buildSelfEsteemLevelUpGuide({ spreadTitle, positionReadings }) {
  ...
  const flow = removeRepeatedSelfEsteemPhrases(
    `${spreadTitle}의 흐름은 '${first?.cardName || "첫 카드"} ${first?.orientation === "reversed" ? "역방향" : "정방향"}'에서 시작해 '${last?.cardName || "마지막 카드"} ${last?.orientation === "reversed" ? "역방향" : "정방향"}'으로 이어집니다. ${suitHint}입니다.`
  );
  const summaryPattern = removeRepeatedSelfEsteemPhrases(`현재 자존감 패턴 요약: ${asText(first?.selfEsteemImpact)} ${asText(third?.selfEsteemImpact)}`);
  const rootCause = removeRepeatedSelfEsteemPhrases(`가장 깊은 원인: ${asText(first?.patternAnalysis)} ${asText(second?.patternAnalysis)}`);
  const drainArea = removeRepeatedSelfEsteemPhrases(`가장 크게 소모되는 영역: ${asText(third?.selfEsteemImpact)}`);
  const recoveryPoint = removeRepeatedSelfEsteemPhrases(`회복의 첫 번째 열쇠: ${asText(fourth?.recoveryAdvice)}`);
  const longTermStandard = removeRepeatedSelfEsteemPhrases(`장기적으로 세워야 할 자기 기준: ${asText(last?.recoveryAdvice)}`);
  const caution = removeRepeatedSelfEsteemPhrases(`가장 주의할 자동 사고: ${asText(second?.caution) || "상대 실망을 내 가치 하락으로 연결하는 자동 사고"}`);
  const practice = removeRepeatedSelfEsteemPhrases(`오늘의 연습 문장: 나는 타인의 감정을 존중하지만 내 기준을 먼저 지킨다.`);
  const mission = [
  ...
  const positionReadings = safeDrawn.map((entry, idx) => {
    ...
    if (spreadId === "self_esteem_levelup_five_card") {
      ...
      const section = ensureSelfEsteemCardMeaningIncluded({
        ...
        cardMeaning: removeRepeatedSelfEsteemPhrases(`${entry?.card?.nameKo || "카드"} ${orientationLabel(orientation)}은 '${context.title}' 질문에서 ${asText(meaning.coreMeaning)}`),
        patternAnalysis: removeRepeatedSelfEsteemPhrases(`심리 패턴 분석: ${asText(meaning.woundPattern?.[0] || meaning.psychologicalMeaning)} ${asText(meaning.boundaryPattern?.[0] || meaning.shadow)}.`),
        selfEsteemImpact: removeRepeatedSelfEsteemPhrases(`자존감 영향: ${asText(meaning.selfEsteem?.[0] || meaning.selfEsteemMeaning)} ${context.focus.join(", ")} 영역의 소모가 커질 수 있습니다.`),
        recoveryAdvice: removeRepeatedSelfEsteemPhrases(`회복 관점: ${asText(meaning.recoveryAdvice?.[0] || meaning.advice)}.`),
        caution: removeRepeatedSelfEsteemPhrases(`주의할 점: ${asText(meaning.caution?.[0] || meaning.shadow)}.`),
      });
      ...
      return {
        ...
        interpretation: removeRepeatedSelfEsteemPhrases(`${entry?.card?.nameKo || "카드"} ${orientationLabel(orientation)} · ${section.keywords.join(", ")} · ${section.cardMeaning} ${section.patternAnalysis}`),
        advice: removeRepeatedSelfEsteemPhrases(`${entry?.card?.nameKo || "카드"} 조언: ${section.recoveryAdvice} ${section.selfEsteemImpact}`),
...
function enrichCardPositionMeaning(card, position, orientation, context = {}) {
  ...
  const actionLead = asText(safePosition.actionLens) || "실행 기준 정리";
  const advice = removeRepeatedPhrases(
    `${actionLead}에 초점을 맞추세요. ${asText(meaning?.advice?.[0]) || asText(meaning?.adviceText)} ${orientation === "reversed"
      ? "한 번에 결론을 요구하지 말고, 짧은 확인 질문 1개만 남기는 방식이 안전합니다."
      : "감정 표현과 현실 합의를 한 번에 몰아치지 말고 두 단계로 나눠 접근하세요."}`
  );
...
function mapSelfEsteemReading(result) {
  ...
  const topSummary = {
    ...
    corePattern: asText(flowCard.summaryPattern || byIndex(0)),
    recoveryKeywords,
    cognitiveTrap: asText(flowCard.caution),
    representativeAction: asText(positionReadings[0]?.todayAction || positionReadings[0]?.actionStep),
  };
