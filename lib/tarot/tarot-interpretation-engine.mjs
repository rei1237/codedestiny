...
function mapRelationshipReading(result) {
  ...
  const positionBreakdown = facts.map((f, idx) => {
    ...
    const orientationCue = f.orientation === "reversed"
      ? "역방향이라 이 힘은 곧장 표현되지 않고 안쪽에서 굴절됩니다."
      : "정방향이라 이 힘은 말투와 반복 행동으로 밖에 드러납니다.";
    const headline = "";
    const summary = `${f.positionTitle}에서 ${f.cardName} ${f.orientationLabel}은 ${f.relMeaning.attractionSignal} ${symbolCue} 상징이 이 자리의 결을 더 선명하게 합니다.`;
    const detail = `${f.cardName} ${f.orientationLabel}은 ${f.positionTitle} 자리에서 ${f.readingFocus}를 가리킵니다. ${symbolCue} 상징과 ${relationshipSuitProfile(f.suit).short}의 결이 겹치며, ${f.relMeaning.attractionSignal} ${orientationCue} 그래서 이 포지션에서는 감정의 크기보다 말의 온도, 반복 행동, 감당 가능한 속도를 따로 보세요.`;
...
function buildReunionFlowInterpretation(position, cardName, orientation, symbol) {
  ...
  if (position?.key === "past_bond") {
    return cleanReunionLanguage(`${titleWithTopic} 상대보다 내 마음의 온도를 먼저 읽습니다. ${symbol.scene} ${modeLine} 이 장면처럼 마음은 아직 닿고 싶은 쪽으로 기울 수 있지만, ${withKoreanParticle(direction, "object")} 분리해 보아야 합니다. 그리움이 올라온다면 재회 확신이 아니라 아직 정리되지 않은 마음의 잔여 온도로 읽는 편이 정확합니다.`);
  }
  if (position?.key === "their_now") {
    return cleanReunionLanguage(`${titleWithTopic} 상대의 겉표현 아래 남은 결을 읽습니다. ${symbol.scene} ${modeLine} 이 장면은 상대 안에서 ${withKoreanParticle(direction, "object")} 섞어 보여 줍니다. 다정함이 적어 보여도 마음이 완전히 사라진 뜻은 아닐 수 있고, 반대로 짧은 반응만으로 확신하기에도 아직 조심스러운 흐름입니다.`);
  }
  if (position?.key === "outside_factor") {
    return cleanReunionLanguage(`${titleWithTopic} 연락을 멈추게 한 실제 이유를 봅니다. ${symbol.scene} ${modeLine} 이 장면은 감정 하나보다 ${withKoreanParticle(direction, "subject")} 함께 얽힌 상태를 가리킵니다. 연락 단절은 마음이 없어서라기보다 다시 열었을 때 감당해야 할 부담을 피하는 방식으로 나타날 수 있습니다.`);
  }
  if (position?.key === "their_heart") {
    return cleanReunionLanguage(`${titleWithTopic} 다시 닿는 속도와 거리를 정합니다. ${symbol.scene} ${modeLine} 이 장면은 ${withKoreanParticle(direction, "object")} 현실적으로 고르라고 말합니다. 재접근은 감정의 크기보다 상대가 부담 없이 받아들일 수 있는 간격과 문장 길이에서 안전해집니다.`);
  }
  return cleanReunionLanguage(`${titleWithTopic} 다시 만나도 오래 갈 수 있는 조건을 읽습니다. ${symbol.scene} ${modeLine} 이 장면은 ${withKoreanParticle(direction, "object")} 세우지 않으면 재회가 다시 같은 파도에 흔들릴 수 있음을 말합니다. 회복은 한 번의 연락보다 반복되는 행동 기준이 실제로 지켜질 때 오래 갑니다.`);
}
...
function enrichCardPositionMeaning(card, position, orientation, context = {}) {
  ...
  const reunionInterpretation = buildReunionFlowInterpretation(safePosition, cardName, orientation, symbol);
  ...
  const caution = buildReunionCautionLine(safePosition.key);
...
function buildYearlyDomainReadings({ zodiac, cardName, symbol, meta, orientation }) {
  ...
  const relationship = cleanYearlyLanguage(`${scene} 모습은 관계에서 상대를 상상으로 채우기보다 실제 말과 행동을 놓고 보아야 함을 드러냅니다. ${relationshipTail}`);
  ...
  const moneyWork = cleanYearlyLanguage(`${cardName}의 ${suit.label} 흐름은 금전과 일에서 ${meta.suit === "pentacles" ? "계약, 일정, 고정비처럼 손에 잡히는 조건" : "선택의 우선순위와 실행 순서"}를 먼저 보게 합니다. ${moneyTail}`);
  const healthMind = cleanYearlyLanguage(`${zodiac.animal}의 월운은 몸에도 같은 리듬으로 들어옵니다. ${scene} 모습처럼 시선이 한곳에 묶이면 피로가 쌓이니, ${reversed ? "수면 시간과 약속 수를 줄여 회복 공간을 만드세요." : "움직일 시간과 쉬는 시간을 달력에 같이 고정하세요."}`);
...
function mapYearlyReading(result) {
  ...
  const monthlyReadings = (Array.isArray(result.cards) ? result.cards : []).slice(0, 12).map((card, idx) => {
    ...
    const domainReadings = buildYearlyDomainReadings({ zodiac, cardName, symbol, meta, orientation });
