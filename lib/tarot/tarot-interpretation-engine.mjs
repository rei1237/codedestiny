...
function buildHealingSectionText(card, fallbackLabel, tailGuide) {
  ...
  const joined = `${positionRole} ${anchor} 지금의 마음을 고치려 하지 말고 먼저 이해해 주세요. ${interpretation} ${emotional} ${tailGuide}`;
...
function buildHealingActionSteps(cards) {
  ...
  (cards || []).forEach((card, idx) => {
    ...
    const line = `${cardName}가 건네는 제안: ${direction} ${rawAdvice}`;
...
function mapHealingReading(result) {
  ...
  const hiddenTruth = buildHealingSectionText(
    cards[0],
    "원인 카드",
    "지금 필요한 것은 누가 잘못했는지 가르는 판단이 아니라, 마음이 아팠던 장면에 다정한 이름을 붙여 주는 일입니다."
  );
  const embracePain = buildHealingSectionText(
    cards[1],
    "수용 카드",
    "감정은 없애야 할 문제가 아니라 지나가도록 돌봐야 할 신호입니다. 오늘은 내 마음에게 괜찮다고 말해 주세요."
  );
  const silverLining = buildHealingSectionText(
    cards[2],
    "회복 카드",
    "작은 안도감을 주는 행동 하나만 정해도 회복 리듬이 살아나고, 잃어버린 자기신뢰가 천천히 돌아옵니다."
  );
  const stepForward = buildHealingSectionText(
    cards[3],
    "행동 카드",
    "완벽한 계획보다 오늘 가능한 10분 실천이 더 큰 변화를 만듭니다."
  );
  const actionPlan = buildHealingActionSteps(cards);
  const summary = dedupeSentences(
    `${result.summary} 오늘의 리딩은 스스로를 몰아붙이지 말고, 내 속도를 존중하며 회복을 다시 설계하라고 말합니다.`
  );
  ...
  return {
    title: result.title,
    summary,
    opening: `${summary} 상담실에서 깊게 숨을 고르듯, 지금은 마음을 고치려 하기보다 지친 지점을 알아차리고 내 편이 되는 말부터 시작해 주세요.`,
    hiddenTruth,
    embracePain,
    silverLining,
    stepForward,
    integrationMessage: dedupeSentences(`${result.combinationReading} 치유는 거대한 결심보다 작은 안정 루틴의 반복에서 완성됩니다. 오늘의 카드 메시지를 현실의 작은 행동으로 바꾸는 순간, 마음은 실제로 회복되기 시작합니다.`),
    actionPlan,
    finalAdvice,
    cardSections: result.cardSections,
    combinations: result.combinations,
