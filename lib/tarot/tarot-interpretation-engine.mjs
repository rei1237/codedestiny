...
function buildAnnualTarotSummary(months) {
  ...
  const annualAdvice = removeRepeatedTarotPhrases([
    bestMonth ? `${bestMonth.monthLabel || `${bestMonth.month}월`}에는 기회가 또렷해지는 문을 먼저 열고,` : "기회가 또렷한 달의 문을 먼저 열고,",
    cautionMonth ? `${cautionMonth.monthLabel || `${cautionMonth.month}월`}에는 서두른 판단보다 속도를 낮춘 선택이 한 해를 지켜 줍니다.` : "흐트러지는 달에는 속도를 낮춘 선택이 한 해를 지켜 줍니다.",
  ].join(" "));

  const summary = removeRepeatedTarotPhrases([
    `올해는 ${dominantSuitLabel} 기운이 한 해의 바탕을 만들고, ${reversedCount}개월은 막힘보다 방향을 가다듬으라는 속도 조절 신호에 가깝습니다.`,
    `열두 달의 월운은 ${topKeywords.slice(0, 3).join(", ") || "핵심 선택과 조율"}에 민감하게 반응하며, 달마다 다른 수호 기운으로 현실의 선택 기준을 비춥니다.`,
  ].join(" "));
...
function buildSelfEsteemPsychologicalReading({ card, orientation, context, previousSection, nextSection, meaning }) {
  ...
  if (safeContext.key === "past_debuff") {
    easyAnswer = `${contextLine}은 사람의 표정과 말투, 분위기 변화를 빠르게 읽어 내는 힘을 드러냅니다. 눈치를 보는 이유는 소심해서가 아니라, 관계의 기류가 흔들릴 때 먼저 맞추며 살아남는 방식을 익혔기 때문입니다.`;
    ...
    easyAnswer = `${contextLine}은 거절을 하면 관계가 끊기거나 상대가 실망해 버릴 것 같은 두려움을 드러냅니다. 그래서 거절이 어려운 이유는 말하기가 힘들어서가 아니라, 상실과 비난을 동시에 떠올리는 마음이 너무 빨리 움직이기 때문입니다.`;
    ...
    recoveryReframe = `회복 방향은 착한 척이 아니라 생존 전략이었던 방식을 인정한 뒤, 지금은 더 이상 나를 소모시키지 않는 방향으로 쓰는 데 있습니다. 반응을 늦추고 사실만 확인하면 과잉 해석이 줄어듭니다.`;
...
function orientationTone(orientation) {
  return orientation === "reversed"
    ? "역방향의 결이 닿아 있어 바로 밀어붙이기보다 지연, 회피, 반복되는 패턴을 먼저 살펴야 합니다."
    : "정방향의 결이 살아 있어 상징은 비교적 밖으로 드러나지만, 현실 조건을 함께 맞출 때 힘이 오래 갑니다.";
}
...
function formatQuestionLine(formatter, title, line) {
  const text = asText(line);
  if (!text) return "";
  const stripped = stripTopicLead(text);
  if (stripped !== text) {
    return `${title}에서는 ${stripped}`;
  }
  return `${title}에서는 ${formatter.label}의 관점으로 ${text}`;
}
...
function selectFeatureAdvice(meaning, formatter) {
  ...
  if (formatter.advice.includes(advice) || advice.includes(formatter.advice)) return formatter.advice;
  return `${formatter.advice} ${advice}`;
}
...
function selectFeatureCaution(meaning, formatter) {
  ...
  if (formatter.caution.includes(caution) || caution.includes(formatter.caution)) return formatter.caution;
  return `${formatter.caution} ${caution}`;
}
...
function buildFeaturePositionReading(entry, position, idx, { questionType, spreadId }) {
  ...
  const positionLens = role
    ? `${title} 자리는 '${role}'에 닿아 있어, ${formatter.context}`
    : `${title}에서는 ${formatter.context}`;

  const interpretation = removeRepeatedTarotPhrases(dedupeSentences([
    core,
    formatQuestionLine(formatter, title, questionLine),
    positionLens,
    orientationTone(orientation),
  ].filter(Boolean).join(" ")));
...
function mapRelationshipReading(result) {
  ...
  const projectionGap = `${c1.cardName} ${c1.orientationLabel}는 내가 상대에게 씌운 기대의 렌즈를, ${c3.cardName} ${c3.orientationLabel}는 카드가 비춘 감정 온도를 드러냅니다. 두 신호가 다르면 지금은 상대의 마음을 단정하기보다, 크게 해석한 장면과 실제로 반복된 행동을 분리해 보는 편이 안전합니다.`;
  const relationshipFrame = `${c2.cardName} ${c2.orientationLabel}은 관계의 이름과 속도를 정하는 태도를, ${c4.cardName} ${c4.orientationLabel}은 그 태도가 행동으로 옮겨지는 방식을 비춥니다. 두 카드가 같은 방향이면 움직임은 비교적 자연스럽지만, 어긋나면 마음보다 속도 조율이 먼저 필요합니다.`;
  const blockToOutcome = `${c5.cardName} ${c5.orientationLabel}의 병목이 그대로 남으면 ${c6.cardName} ${c6.orientationLabel}의 가까운 흐름은 지연이나 유보처럼 느껴질 수 있습니다. 다만 이 배열은 확정 예언이 아니라, 장애물을 다루는 방식에 따라 온도가 달라지는 구간을 비춥니다.`;
  ...
  const wholeStory = `${sequenceFlow}. 시작 카드 ${c1.cardName}와 마지막 카드 ${c6.cardName}의 대비는 첫 인상과 가까운 흐름 사이의 온도 차를 비춥니다. ${majorCount >= 2 ? "메이저 카드가 강하게 관여해 감정만이 아니라 관계 구조를 다시 살필 필요가 있습니다." : "메이저 비중이 낮아도 반복되는 소통 방식은 관계의 리듬을 천천히 굳힐 수 있습니다."} 역방향 비율 ${Math.round((reversedCount / Math.max(1, facts.length)) * 100)}%는 표현 지연과 오해 관리가 중요하다는 신호이며, ${dominantSuitLabel} 기운이 이번 리딩의 주된 공기입니다.`;
  ...
  const positionBreakdown = facts.map((f, idx) => {
    ...
    const headline = `${f.cardName} ${f.orientationLabel}이 ${f.positionOrder}번 '${f.positionTitle}'에 놓이면, ${f.readingFocus}를 먼저 살펴볼 지점으로 봅니다.`;
    const summary = `${f.positionTitle}에서 ${f.cardName} ${f.orientationLabel}은 ${f.relMeaning.attractionSignal} 이 카드는 단순한 호감 판정보다 감정과 판단의 비율을 함께 읽으라는 신호입니다.`;
    const detail = `${f.cardName} ${f.orientationLabel}은 ${f.positionTitle} 자리에서 두 사람의 상호작용이 어디서 엇갈리는지 드러냅니다. 특히 ${f.relMeaning.communicationPattern} 흐름이 강하면 같은 말도 다르게 받아들여질 수 있습니다. ${f.relMeaning.commitmentSignal} 신호가 함께 보이므로, 감정 확인과 관계 정의를 한꺼번에 요구하기보다 말의 온도와 행동의 반복을 따로 관찰하는 편이 좋습니다.`;
    const relationshipInsight = `${f.positionTitle} 관점에서는 ${f.relMeaning.attractionSignal} 동시에 ${f.relMeaning.fearSignal} 이 양쪽 신호가 섞이면 한 사람은 가까워졌다고 느끼고, 다른 한 사람은 아직 탐색 중이라고 느낄 수 있습니다.`;

    let advice = `${f.cardName} ${f.orientationLabel} 기준 조언은 '${f.readingFocus}'를 차분히 확인하는 대화입니다. `;
    ...
    const caution = `${f.cardName} ${f.orientationLabel}의 그림자는 ${f.relMeaning.shadow} ${f.positionTitle}에서 이 그림자가 커지면 침묵을 거절로 단정하거나 작은 호감 신호를 크게 해석해 관계 리듬이 흔들릴 수 있습니다.`;
  ...
  const overallVibe = `${c1.cardName} ${c1.orientationLabel}에서 ${c6.cardName} ${c6.orientationLabel}로 이어지는 6장 배열은 관계를 '내 시선-상대의 프레임-감정 온도-행동 의지-막힘-가까운 흐름'의 순서로 비춥니다. 지금의 핵심은 누가 더 좋아하느냐보다 서로의 속도와 해석 방식을 맞출 수 있느냐입니다. ${dominantSuitLabel} 신호와 역방향 ${reversedCount}장은 감정 자체보다 말의 해석, 타이밍, 기준선 조율이 중요하다는 쪽으로 기울어 있습니다.`;

  const deepReading = `${c2.cardName} ${c2.orientationLabel}은 관계의 이름과 속도를 조심스럽게 재는 태도를, ${c3.cardName} ${c3.orientationLabel}은 카드가 비춘 감정 온도를 드러냅니다. 다만 감정 온도와 관계 정의는 같은 뜻이 아닙니다. ${c4.cardName} ${c4.orientationLabel}은 마음이 행동으로 옮겨지는 방식을 비추므로, 감정 확인을 재촉하기보다 실제로 움직일 조건을 짧고 분명하게 맞추는 대화가 더 안전합니다.`;

  const realityAndFuture = `${c5.cardName} ${c5.orientationLabel}이 보여주는 병목은 관계의 부재보다 속도 불균형, 급한 확인, 타이밍 압박에 가깝습니다. 이 병목이 정리되지 않으면 ${c6.cardName} ${c6.orientationLabel}의 가까운 흐름은 유보나 거리 조절로 느껴질 수 있습니다. 내가 조정할 수 있는 영역은 연락 리듬, 질문의 길이, 결론을 요구하는 강도입니다. 당분간은 짧은 대화와 속도 낮추기를 병행하고, 애매함이 길어질 때 지킬 기준선도 함께 세워야 합니다.`;

  const finalAdvice = {
    instantMission: `${c5.cardName} ${c5.orientationLabel}과 ${c6.cardName} ${c6.orientationLabel} 조합 기준으로, 오늘은 관계의 이름을 묻기보다 연락 속도를 한 단계 낮추고 확인된 사실 질문 1개만 남기세요.`,
    conversationTip: `${c3.cardName} ${c3.orientationLabel}의 감정 온도와 ${c4.cardName} ${c4.orientationLabel}의 실행 신호를 함께 보면, 감정 확인을 몰아붙이기보다 "우리가 편해지려면 어떤 기준 하나를 맞추면 좋을까"라는 문장이 더 부드럽습니다.`,
    relationshipBoundary: `${c6.cardName} ${c6.orientationLabel} 흐름에서는 애매함이 길어질수록 소모가 커질 수 있으니, 내가 지킬 연락 주기와 대화 톤의 기준을 먼저 정해 두세요.`,
    nextSevenDays: `앞으로 7일은 결론을 당기기보다 속도 조절과 오해 축소에 집중하는 편이 좋습니다. 작은 약속 하나가 지켜지는지 보는 것이 가장 현실적인 신호입니다.`,
    checklist: [
  ...
  const mapped = {
    ...
    relationshipMatrix: {
      ...
      majorArcanaSignal: majorCount > 0
        ...
        : "메이저 카드 비중이 낮아 일상적 패턴 관리가 가까운 흐름을 좌우합니다.",
...
const REUNION_POSITION_BLUEPRINT = [
  ...
  },
  {
    key: "their_now",
    title: "상대가 보이는 마음의 결",
    question: "겉표현 아래 남은 감정의 결은 무엇일까요?",
  ...
  },
  {
    key: "outside_factor",
    title: "연락이 멈춘 현실 신호",
    question: "연락이 멈춘 배경에는 어떤 감정과 현실 변수가 있을까요?",
  ...
  },
  {
    key: "their_heart",
    title: "다시 닿을 수 있는 거리",
    question: "다시 닿으려면 어느 정도의 거리와 속도가 안전할까요?",
...
const REUNION_ACTION_SCRIPT = {
  ...
  their_now: {
    upright: "말보다 행동의 반복을 보세요. 작은 일관성에 남은 마음의 결이 드러납니다.",
...
function enrichCardPositionMeaning(card, position, orientation, context = {}) {
  ...
  const reunionInterpretation = cleanReunionLanguage(
    `${safePosition.title || "포지션"} 자리에서 ${safeCard.nameKo || "이 카드"} 카드는 '${asText(meaning?.reunion?.[0])}'라는 신호를 비춥니다. ${roleNote} 관점에서는 '${coreVariable}'라는 신호가 핵심 변수입니다.`
  );
...
function mapReunionReading(result) {
  ...
  const shouldMoveFirst = score >= 62
    ? "가벼운 안부 1회는 가능하지만, 답을 재촉하지 않는 간격이 필요합니다"
    : "먼저 움직이기보다 내 마음과 상황을 정리하는 시간이 우선입니다";
  ...
  const avoidNow = score < 58
    ? "답을 요구하는 긴 메시지와 반복 확인 연락"
    : "재회를 결론으로 몰아가는 말과 과거 책임 추궁";

  const oneLineAdvice = removeRepeatedPhrases(
    score >= 58
      ? "지금의 인연은 완전히 닫힌 흐름으로 보이진 않습니다. 짧고 따뜻한 첫 문장으로 거리만 확인하되, 같은 상처가 반복되지 않게 대화의 약속부터 세워 주세요."
  ...
  return {
    ...
    summary: {
      ...
      bestContactTiming: score >= 70 ? "짧은 안부로 거리 확인" : score >= 52 ? "1~2주 감정 조율 후 가벼운 접촉" : "자연스러운 계기 우선",
    ...
    finalGuide: {
      shouldContactNow: score >= 58
        ? "가벼운 첫 문장은 가능, 다만 답을 재촉하지 않는 간격이 필요"
        : "먼저 움직이기보다 마음 정리와 생활 리듬 회복이 우선",
...
const SUN_RECOVERY_CARD_COPY = {
  ...
  "King of Wands": {
    reversed: {
      ...
      meaning: "완드 킹 역방향은 강해져야 한다는 책임감과 잘 해내야 한다는 불안함이 겹쳐, 나도 모르게 스스로와 타인에게 서슬 퍼런 잣대를 들이대며 마음의 열기를 끓어올렸을 수 있음을 속삭여 줍니다. 오늘의 회복은 상대를 굴복시키거나 끝끝내 이겨내는 단단한 힘에 있지 않습니다. 그저 내 연약함을 솔직히 마주하고, 내 마음의 경계를 부드럽고 다정하게 전하는 유연함에 있습니다.",
...
const HEALING_SUIT_COPY = {
  ...
  major: {
    ...
    meaning: "메이저 아르카나의 웅장한 메시지는 현재 당신이 마주한 내면의 진통이 결코 무의미한 방황이 아님을 말해 줍니다. 이는 당신의 삶이 한 단계 더 성숙하고 단단한 차원으로 거듭나기 위한 영혼의 숭고한 여정입니다. 고치거나 서두르려 하지 마세요. 계절이 흐르듯, 당신 안의 회복 리듬도 완벽한 자기만의 속도로 빛을 향해 가고 있습니다.",
...
function mapHealingReading(result) {
  ...
  return {
    title: "태양 회복 타로",
    subtitle: "마음의 흔적을 억지로 지우기보다, 그 자리에 조용히 새벽빛을 들여놓는 다정한 동행",
    oneLineMessage,
    sunLine: oneLineMessage,
    summary: oneLineMessage,
    opening: cleanHealingLanguage("오늘의 카드는 지친 마음을 섣불리 교정하거나 다그치지 않습니다. 오래 머물렀던 어두운 구석에 부드러운 햇살을 비추고, 지금 편안하게 숨 쉴 수 있는 작고 소박한 온기를 건넵니다."),
    cardReadings,
    cardDeepDive,
    ...
    recoveryRoutines,
    actionPlan,
    affirmation: cleanHealingLanguage("오늘만큼은 나를 다그치지 않고, 흔들리는 마음까지 내 편으로 두며 따스한 온기가 차오르기를 기다리겠습니다."),
...
function mapYearlyReading(result) {
  ...
  const monthlyReadings = (Array.isArray(result.cards) ? result.cards : []).slice(0, 12).map((card, idx) => {
    ...
    const zodiacReading = cleanYearlyLanguage(`${zodiac.emoji} ${zodiac.animal}의 달은 ${zodiac.theme}의 수호 리듬을 열고, ${card.cardNameKo} ${orientationLabelText}과 만나 이달의 운세 결을 드러냅니다. ${monthlySignal}`);
    const triadReading = cleanYearlyLanguage(`월초·월중·월말의 세 갈래 월운은 신호, 선택, 결과의 온도를 나누어 비춥니다. ${zodiac.theme}의 기운과 ${card.cardNameKo} ${orientationLabelText}이 만나는 지점에서 이번 달의 속도와 기준을 읽어야 합니다.`);
    ...
    const opportunity = cleanYearlyLanguage(`${card.cardNameKo} ${orientationLabelText}은 ${monthlyKeywords.slice(0, 3).join(", ") || "실행의 기회"}가 실제로 움직일 수 있는 선택지로 드러나는 달임을 알려줍니다.`);
    ...
    return ensureCardMeaningIncluded({
      ...
      caution,
      advice,
      exam: removeRepeatedTarotPhrases(`${card.cardNameKo}는 시험, 자격, 평가 영역에서 반복 루틴과 컨디션 관리가 결과의 결을 바꾸는 카드입니다.`),
    });
  });
