...
function cleanHealingLanguage(text) {
  return dedupeSentences(asText(text)
    .replace(/은\(는\)/g, "은")
    .replace(/힐링 라이징 타로 리딩입니다\./g, "따뜻한 태양 회복 타로가 오늘의 마음을 비춥니다.")
    .replace(/결과입니다\./g, "리딩입니다.")
    .replace(/단정 대신 흐름을 읽을수록 해석 정확도가 높아집니다\./g, "단정보다 마음의 속도를 따라 읽을 때 회복의 방향이 선명해집니다.")
    .replace(/오늘 가능한 실행 단위를 작게 설정하고 끝까지 완료하세요\./g, "오늘 가능한 회복 장면을 아주 작게 정하고 편안히 마무리해 보세요.")
    .replace(/해석을 실제 선택/g, "마음의 신호를 실제 선택")
    ...
    .replace(/실행 가능한 작은 행동/g, "오늘 가능한 작은 회복 행동")
    .replace(/해석 정확도/g, "회복의 방향")
    .replace(/\s+/g, " ")
    .trim());
}
...
function buildHealingSectionText(card, fallbackLabel, tailGuide) {
  ...
  const directionGuide = card.orientation === "reversed"
    ? "역방향은 빛이 사라졌다는 뜻이 아니라, 아직 말하지 못한 감정이 그늘 속에서 쉬고 있다는 신호입니다."
    : "정방향은 이미 마음 안쪽에서 따뜻한 회복의 불씨가 살아나고 있음을 보여줍니다.";
  const shadowGuide = shadow ? `그림자로는 ${shadow} 흐름이 보이지만, 이 부분은 고쳐야 할 결함이 아니라 조심스럽게 살펴야 할 마음의 흔적입니다.` : "";
  const joined = `${positionRole} ${anchor}. ${interpretation} 회복 상담의 언어로 보면 ${emotional} ${directionGuide} ${shadowGuide} ${tailGuide}`;
  return cleanHealingLanguage(joined);
}
...
function buildHealingCardDeepDive(cards) {
  return (cards || []).map((card, idx) => {
    ...
    const directionReading = card?.orientation === "reversed"
      ? "이 카드는 눌러 둔 감정을 억지로 밝히기보다, 먼저 안전한 그늘을 마련하라고 말합니다."
      : "이 카드는 마음의 창문이 조금씩 열리고 있으며, 작은 온기를 현실의 선택으로 옮길 때 안정감이 커진다고 말합니다.";
    const line = `${idx + 1}. ${position} - ${cardName} ${direction}${keywords ? ` (${keywords})` : ""}: ${core} ${directionReading} 마음의 결로는 ${psyche} ${shadow ? `조심히 살필 그림자는 ${shadow}` : ""} 오늘의 회복 제안은 ${advice}입니다.`;
    return cleanHealingLanguage(line);
...
function buildHealingActionSteps(cards) {
  ...
  (cards || []).forEach((card, idx) => {
    ...
    if (!rawAdvice) return;
    const direction = card?.orientation === "reversed" ? "속도를 늦추고" : "작게 빛을 들이고";
    const cardName = asText(card?.cardNameKo) || `카드 ${idx + 1}`;
    const line = `${cardName} 카드가 건네는 회복 루틴: ${direction} ${rawAdvice} 오늘은 이 조언을 10분 안에 마칠 수 있는 작은 장면 하나로 줄여 주세요.`;
    steps.push(cleanHealingLanguage(line));
  });
...
function mapHealingReading(result) {
  ...
  const hiddenTruth = buildHealingSectionText(
    cards[0],
    "원인 카드",
    "지금 필요한 것은 누가 잘못했는지 가르는 판단이 아니라, 마음이 오래 머물렀던 장면에 다정한 이름을 붙여 주는 일입니다. 이름을 얻은 감정은 더 이상 막연한 어둠이 아니라, 돌볼 수 있는 새벽빛이 됩니다."
  );
  const embracePain = buildHealingSectionText(
    cards[1],
    "수용 카드",
    "감정은 없애야 할 문제가 아니라 지나가도록 안아 주어야 할 신호입니다. 오늘은 내 마음에게 괜찮다고 말해 주세요. 버텨온 자신을 인정하는 한 문장이 회복의 첫 문을 엽니다."
  );
  const silverLining = buildHealingSectionText(
    cards[2],
    "회복 카드",
    "작은 안도감을 주는 행동 하나만 정해도 회복 리듬이 살아나고, 잃어버린 자기신뢰가 천천히 돌아옵니다. 태양빛은 거창한 확신보다 반복 가능한 안정감에서 먼저 자랍니다."
  );
  const stepForward = buildHealingSectionText(
    cards[3],
    "행동 카드",
    "완벽한 계획보다 오늘 가능한 10분 회복 루틴이 더 큰 변화를 만듭니다. 마음이 다시 나를 믿도록, 작고 끝낼 수 있는 행동을 하나만 선택해 주세요."
  );
  const actionPlan = buildHealingActionSteps(cards);
  const summary = cleanHealingLanguage(
    `${result.summary} 오늘의 리딩은 스스로를 몰아붙이지 말고, 내 속도를 존중하며 회복의 온도를 다시 찾으라고 말합니다. 태양 회복 타로는 상처를 지우라고 요구하지 않고, 그 상처를 품은 채로도 다시 따뜻해질 수 있음을 보여줍니다.`
  );
  ...
  return {
    title: result.title,
    summary,
    opening: cleanHealingLanguage(`${summary} 새벽 햇살 앞에서 숨을 고르듯, 지금은 마음을 고치려 하기보다 지친 지점을 알아차리고 내 편이 되는 말부터 시작해 주세요. 타로는 미래를 겁주는 도구가 아니라, 마음이 이미 알고 있던 회복의 방향을 조용히 비추는 거울입니다.`),
    cardDeepDive,
    hiddenTruth,
    embracePain,
    silverLining,
    stepForward,
    integrationMessage: cleanHealingLanguage(`${result.combinationReading} 네 장의 카드는 서로 다른 말을 하는 것 같지만, 결국 하나의 태양 궤도로 이어집니다. 원인을 이해하고, 감정을 밀어내지 않고, 작은 빛을 찾고, 오늘의 행동으로 옮기는 흐름입니다. 회복은 거대한 결심보다 작은 안정 루틴의 반복에서 완성됩니다. 오늘의 카드 메시지를 현실의 작은 장면으로 바꾸는 순간, 마음은 실제로 다시 따뜻해지기 시작합니다.`),
    actionPlan,
    finalAdvice,
    cardSections: result.cardSections,
    combinations: result.combinations,
