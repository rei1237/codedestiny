...
const REUNION_CARD_OVERRIDES = {
  C06: {
    upright: {
      ...
      advice: ["과거를 소환할 때는 감성 호소보다 지금 달라진 태도 1가지를 함께 보여 주세요."],
    },
    reversed: {
      ...
      advice: ["추억을 꺼낼 때는 반드시 현재 기준과 재발 방지 약속을 짧게 덧붙이세요."],
    },
  },
  M15: {
    upright: {
      ...
      advice: ["재회 전 '서로 지킬 경계선 2가지'를 합의하지 않으면 감정 소모가 반복될 수 있습니다."],
    },
    reversed: {
      ...
      advice: ["연락 재개 전 '하지 않을 행동'을 먼저 정해 관계 재발 리스크를 낮추세요."],
    },
  },
  M17: {
    upright: {
      ...
      advice: ["큰 고백보다 안부-근황-짧은 감사 순서의 부담 낮은 메시지가 더 효과적입니다."],
    },
    reversed: {
      ...
      advice: ["막연한 기다림 대신 구체적 행동 1개(짧은 안부 혹은 명확한 정리)를 선택하세요."],
    },
  },
};
...
const SELF_ESTEEM_CARD_OVERRIDES = {
  C07: {
    upright: {
      ...
      selfEsteem: ["남이 좋아할 모습을 상상하느라 내 욕구 기준이 흐려진 상태를 보여줍니다."],
  ...
  M11: {
    ...
    reversed: {
      ...
      psychologicalMeaning: "누군가 실망하는 기색이 보이면 사실 판단보다 '내가 틀렸을 것'이라는 자동 사고가 먼저 올라올 수 있습니다.",
      selfEsteemMeaning: "눈치를 보는 습관의 뿌리는 소심함이 아니라, 불공정한 장면에서도 균형을 맞추려 버텨온 경험일 가능성이 큽니다.",
  ...
  W06: {
    ...
    reversed: {
      ...
      coreMeaning: "외부 반응이 약할 때 내 선택 전체를 의심하기 쉬운 상태를 보여줍니다.",
      psychologicalMeaning: "칭찬이 없으면 가치가 없다고 느끼는 자동 사고가 자존감 리듬을 크게 흔들 수 있습니다.",
  ...
  return ensureMeaningShape({
    ...
    monthly: [
      isReversed
        ...
        : `${nameKo} 정방향은 이번 달 ${coreKeyword}이(가) 실제 흐름을 지탱하며 단계적 진전을 돕습니다.`,
    ],
    coreMeaning: isReversed
      ? `${nameKo} 역방향은 반복되던 대응 방식이 한계에 닿아 기준 재정렬이 필요한 국면입니다.`
      : `${nameKo} 정방향은 해석을 실제 선택으로 연결할 수 있는 실행 창이 열린 국면입니다.`,
    psychologicalMeaning: isReversed
      ? "반응을 늦추고 사실을 재확인하면 과잉 해석을 줄일 수 있습니다."
      : "지금의 감정 에너지는 명확한 선택과 실행으로 안정될 가능성이 큽니다.",
    selfEsteemMeaning: isReversed
      ? "자존감 관점에서는 타인 반응을 내 가치와 분리하는 경계 훈련이 핵심입니다."
      : "자존감 관점에서는 작은 자기신뢰 행동을 반복할수록 회복 속도가 빨라집니다.",
    ...
    general: [isReversed ? "지금은 서두른 결론보다 기준 재정렬이 회복에 도움이 됩니다." : "지금은 작은 실행을 반복할수록 흐름이 안정적으로 강화됩니다."],
    ...
    advice: [isReversed ? "지금은 확인 질문과 짧은 실행으로 리듬을 회복하세요." : "오늘 가능한 실행 단위를 작게 설정하고 끝까지 완료하세요."],
  });
}
...
function applyCardOverrides(card) {
  ...
  if (!selfEsteemOverride) return next;
  return {
    ...next,
