...
function buildCelestialMelodySection(cardInput, planet, orientationInput) {
  ...
  const archetypeReading = ensureMinLength(
    `${planet.planetKo}(${planet.symbol}) 자리에서 ${card.nameKo} ${orientationLabel}이 열린다는 것은 ${planet.title} 영역에 ${cardSignal}의 흐름이 켜졌다는 뜻입니다. ${layerTone(planet.layer)} 이 카드는 단순한 사건 예측보다 "내가 어떤 선택을 반복하고 있는가"를 비춥니다. 특히 ${planet.question}라는 질문 앞에서 ${planetMeaning.conscious}와 ${cardMeaning.psych}가 만나는 지점이 이번 리딩의 기준음입니다.`,
    560,
    [
      `${planet.planetKo}의 원형은 ${planet.archetype}이며, ${card.nameKo}의 키워드인 ${keywordLine || "핵심 상징"}를 통해 지금 삶의 리듬을 세밀하게 보여줍니다.`,
      `이 해석의 목적은 불안을 키우는 것이 아니라 상징을 통해 스스로를 더 정확히 이해하도록 돕는 데 있습니다.`,
      `카드가 말하는 변화는 거창한 결심보다 오늘 반복 가능한 작은 기준을 세울 때 현실로 내려옵니다.`,
    ],
  );

  const consciousMessage = ensureMinLength(
    `${planetMeaning.conscious} 관점에서 지금 필요한 의식 메시지는 분명합니다. ${card.nameKo} ${orientationLabel}은 마음이 흔들리기 전에 기준 문장을 먼저 세우라고 말합니다. 지금의 과제는 감정을 부정하는 것이 아니라 감정을 존중하되 결정은 기준 위에서 내리는 것입니다. ${planet.planetKo} 축은 ${planet.question}라는 질문을 통해 오늘의 선택을 더 선명하게 정돈하라고 요청합니다.`,
    520,
    [
  ...
  const unconsciousPattern = ensureMinLength(
    `${planetMeaning.unconscious} 패턴은 조용하지만 강하게 작동합니다. ${cardMeaning.psych}라는 카드 신호는 지금의 갈등이 현재 사건만의 문제가 아니라 누적된 정서 기억과 연결되어 있음을 보여줍니다. 무의식은 종종 익숙한 고통을 안전으로 오해하기 때문에 낯선 평온보다 익숙한 긴장을 선택하게 만들 수 있습니다.`,
    520,
    [
  ...
  const shadowWarning = ensureMinLength(
    `${planetMeaning.shadow}이 그림자의 경고로 드러납니다. ${cardMeaning.shadow} 이 조합의 위험은 문제 자체보다 과열 반응의 자동화에 있습니다. 마음이 불안정한 시간일수록 결론을 서두르거나, 상대와 상황을 단정하거나, 자신을 심판하려는 경향이 강해질 수 있습니다.`,
    520,
    [
      `그림자 작업의 첫 단계는 "지금 내가 지키려는 것은 무엇인가"를 확인하는 것입니다.`,
      `방어의 목적을 알면 파괴적 선택 대신 보호적 선택으로 전환할 수 있습니다.`,
      `이 경고는 예언이 아니라 더 정확한 자기조율을 위한 안전 신호입니다.`,
    ],
  );

  const soulLesson = ensureMinLength(
    `${planetMeaning.soulLesson}이라는 과제가 핵심입니다. ${cardMeaning.lesson} 카드가 보여주는 성장은 한 번의 극적 변화보다 작지만 일관된 자기 신뢰 행동에서 시작됩니다. 영혼의 성장은 상처를 지우는 과정이 아니라 상처를 해석 가능한 언어로 바꾸는 과정입니다.`,
    520,
    [
  ...
  const integrationPractice = ensureMinLength(
    `${planetMeaning.integration}을 오늘의 통합 조율로 제안합니다. ${cardMeaning.integration}처럼 행동 단위를 작게 쪼개면 해석이 실제 변화로 내려오기 시작합니다. 통합은 모든 답을 아는 상태가 아니라, 아는 것과 하는 것 사이의 간격을 줄이는 과정입니다.`,
    520,
    [
  ...
  return {
    ...
    consciousMessage: ensureMinLength(removeRepeatedCelestialPhrases(consciousMessage), 520, [
      `${planet.planetKo} 축의 의식 메시지는 기준을 먼저 세우고 감정을 그 기준 안에서 다루라는 요청입니다.`,
    ...
    unconsciousPattern: ensureMinLength(removeRepeatedCelestialPhrases(unconsciousPattern), 520, [
      `${planet.planetKo} 축의 무의식 패턴은 감정 기록을 통해 더 빨리 드러납니다.`,
...
function buildSummary(sections) {
  ...
  const overallTheme = ensureMinLength(
    `이번 11장 스프레드는 의식과 무의식, 그림자와 통합의 축이 동시에 울리는 전환기를 가리킵니다. 카드 배열은 단순한 사건 예측보다 내면의 자동 반응을 드러내며, 특히 ${strongestSection?.planetKo || "태양"} 축에서 시작된 신호가 전체 리딩의 기준음을 형성합니다. 당신의 영혼은 빠른 결론보다 섬세한 자기 이해를 요구하고 있으며, 그 이해는 현실의 작은 조율로 번역될 때 비로소 하루의 선율로 정착합니다.`,
    1100,
    [
      ...
      `영혼 층위는 사랑과 가치, 신념의 재정렬을 요청하며 통합 층위는 모든 통찰을 오늘의 행동으로 연결하라고 말합니다.`,
       `결국 이 리딩의 핵심은 미래를 단정하는 데 있지 않고, 자기조율 능력을 회복하는 데 있습니다.`,
  ...
  return {
    overallTheme: ensureMinLength(removeRepeatedCelestialPhrases(overallTheme), 1100, [
      "이 리딩은 사건을 단정하지 않고, 현재의 선택 리듬을 더 섬세하게 읽어 현실의 방향을 회복하도록 돕습니다.",
      "각 행성 자리는 서로 다른 목소리처럼 울리지만 마지막에는 하나의 문장으로 합쳐집니다.",
      "오늘 필요한 것은 모든 답을 한 번에 얻는 것이 아니라, 지금 가장 선명한 한 가지 기준을 세우고 반복하는 일입니다.",
      "카드가 보여주는 상징은 운명을 고정하지 않고, 삶을 다시 조율할 수 있는 작은 손잡이를 건넵니다.",
    ]),
    dominantLayer,
    dominantSuit,
    ...
    finalOracle: `별빛의 선율은 ${strongestSection?.planetKo || "당신의 중심"}에서 시작해 ${shadowSection?.planetKo || "그림자"}를 통과하고, ${soulSection?.planetKo || "영혼"}에서 치유의 음색으로 완성됩니다. 오늘의 당신은 이미 변화를 시작했고, 그 변화는 작은 조율의 반복으로 현실에 새겨질 것입니다.`,
    closingFortune,
  };
}
