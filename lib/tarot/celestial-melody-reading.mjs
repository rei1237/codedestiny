...
// 천체의 선율 타로 프롬프트에서 app/api·worker 두 빌더가 공유하는 고정 문구.
// 두 빌더는 단계가 다르지만(최종 오라클 텍스트 vs 구조화 JSON) 아래 라인은
// 동일해야 하므로 단일 소스로 관리해 divergence를 막는다.
const CELESTIAL_MELODY_PROMPT_PREMISE =
  "이 리딩은 별자리 운세가 아니라 11개 행성 질문축과 11장의 타로 카드가 만나는 심층 상담형 결과입니다.";
const CELESTIAL_MELODY_INTERPRETATION_ORDER =
  "행성 질문축 -> 카드 상징 -> 사용자의 마음과 현실 -> 오늘의 조율 행동";
