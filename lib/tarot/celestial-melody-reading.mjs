...
function buildSummary(sections) {
  ...
  const planetaryPriority = sections.slice(0, 5).map((section) => `${section.planetKo}: ${sanitizeCelestialMelodyText(section.integrationPractice).slice(0, 90)}`);
  ...
  return {
    ...
    insightMatrix: {
      love: `${loveSection?.planetKo || "금성"} 축에서는 ${sanitizeCelestialMelodyText(loveSection?.consciousMessage || "감정 표현의 기준을 다시 세우는 과정")}이 핵심입니다. 관계에서 반응보다 기준 문장을 먼저 세우면 소모가 크게 줄어듭니다.`,
    ...
    ritualPlan: [
      `1일차: ${strongestSection?.planetKo || "태양"} 자리의 핵심 문장을 1줄로 작성하고 오늘 결정 1건에 적용합니다.`,
      `2일차: ${loveSection?.planetKo || "금성"} 관련 감정 반응 1개를 사실/해석/요구로 분리해 기록합니다.`,
      `3일차: ${workSection?.planetKo || "토성"} 영역의 미루던 과제 1개를 20분 단위로 착수합니다.`,
      `4일차: ${moneySection?.planetKo || "목성"} 관련 지출/투자 판단에 검증 질문 2개를 추가합니다.`,
      `5일차: ${healthSection?.planetKo || "달"} 축 회복을 위해 수면 전 10분 감정 정리 루틴을 실행합니다.`,
      `6일차: ${shadowSection?.planetKo || "명왕성"} 그림자 패턴이 올라온 상황에서 대체 행동 1개를 실험합니다.`,
