...
function buildLoveReadingPrompt(baseReading, locale) {
...
async function requestLoveReadingGeminiOnce({ prompt, env, fetchImpl, apiKey }) {
    ...
    `https://generativelanguage.googleapis.com/v1beta/models/${encodeURIComponent(getGeminiModel(env))}:generateContent?key=${encodeURIComponent(apiKey)}`,
    {
      ...
      body: JSON.stringify({
        ...
        generationConfig: {
          ...
          maxOutputTokens: boundedNumber(env.LOVE_READING_GEMINI_MAX_OUTPUT_TOKENS, 9000, 3000, 16000),
...
export async function enhanceLoveReadingWithLlm(baseReading, options = {}) {
  ...
  const prompt = buildLoveReadingPrompt(baseReading, locale);
  let lastReason = "unknown";
  for (let attempt = 0; attempt < 2; attempt += 1) {
    try {
      const result = await requestLoveReadingGeminiOnce({ prompt, env, fetchImpl, apiKey });
      if (result.ok) {
