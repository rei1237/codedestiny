...
async function requestLoveReadingGeminiOnce({ prompt, env, fetchImpl, apiKey, maxOutputTokens, timeoutMs }) {
  ...
  logTarotTokenUsage(payload, { env, model: getGeminiModel(env), serviceId: "tarot-love-reading", maxTokens: maxOutputTokens });
...
export async function enhanceLoveReadingWithLlm(baseReading, options = {}) {
  ...
  if (isStagingLlmMockEnabled(env)) {
    return { ...fallback, llmFailReason: "staging_mock" };
  }
