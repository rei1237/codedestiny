...
async function requestLoveReadingGeminiOnce({ prompt, env, fetchImpl, apiKey, maxOutputTokens }) {
...
export async function enhanceLoveReadingWithLlm(baseReading, options = {}) {
  ...
  for (let attempt = 0; attempt < 3; attempt += 1) {
    ...
    try {
      const result = await requestLoveReadingGeminiOnce({ prompt, env, fetchImpl, apiKey, maxOutputTokens });
      if (result.ok) {
