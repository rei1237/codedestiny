...
const PALM_MAP_ENGINE_TEXT_TRANSLATIONS = {
  ko: {
    "palmMap.001": "막쥔 손금(시미안 라인)",
    "palmMap.002": "M자 손금",
    "palmMap.003": "목성구 발달 + 성장축 강조",
    "palmMap.004": "토성구 발달 + 운명선 뚜렷",
    "palmMap.005": "1. 종합 손금 리딩",
    "palmMap.006": "2. 선천적 손 / 후천적 손 해석",
    "palmMap.007": "3. 생명선 분석",
    "palmMap.008": "4. 두뇌선 분석",
    "palmMap.009": "5. 감정선 분석",
    "palmMap.010": "6. 운명선 분석",
    "palmMap.011": "7. 재물/태양선 분석",
    "palmMap.012": "8. 연애/결혼선 분석",
    "palmMap.013": "9. 구丘 종합 분석",
    "palmMap.014": "10. 손금 조합 해석",
    "palmMap.015": "10-1. 특수 손금 패턴",
    "palmMap.016": "11. 영역별 흐름 해석",
    "palmMap.017": "12. 강점 5개",
    "palmMap.018": "13. 주의점 5개",
    "palmMap.019": "14. 맞춤 조언",
  },
};

function palmMapEngineText(key) {
  return PALM_MAP_ENGINE_TEXT_TRANSLATIONS.ko[key] || "Translation pending";
}
...
function makeLineFeature(raw, ctx) {
  ...
  const evidenceScore = clamp(toNumber(raw?.evidenceScore, confidence), 0, 1);
  const edgeConsistency = clamp(toNumber(raw?.edgeConsistency, evidenceScore), 0, 1);
  const continuity = clamp(toNumber(raw?.continuity, evidenceScore), 0, 1);
  const guideDistance = clamp(toNumber(raw?.guideDistance, 0), 0, 1);
  const sourceKind = String(raw?.sourceKind || "unknown");
...
function lineEvidenceAdjustment(candidate, lineType = "generic") {
  const evidence = clamp(toNumber(candidate?.evidenceScore, candidate?.confidence || 0), 0, 1);
  const edge = clamp(toNumber(candidate?.edgeConsistency, evidence), 0, 1);
  const continuity = clamp(toNumber(candidate?.continuity, evidence), 0, 1);
  const guideDistance = clamp(toNumber(candidate?.guideDistance, 0), 0, 1);
  const isMinor = ["sunLine", "moneyLine", "marriageLine", "communicationLine"].includes(lineType);
  let adjustment = 0;

  if (evidence >= 0.72 && edge >= 0.6) adjustment += isMinor ? 6 : 8;
  else if (evidence >= 0.58) adjustment += isMinor ? 3 : 4;

  if (evidence < (isMinor ? 0.58 : 0.48)) adjustment -= isMinor ? 22 : 16;
  if (edge < (isMinor ? 0.5 : 0.4)) adjustment -= isMinor ? 14 : 10;
  if (continuity < (isMinor ? 0.45 : 0.35)) adjustment -= isMinor ? 12 : 8;
  if (guideDistance > 0.75) adjustment -= 8;
...
function scoreLifeLine(candidate, ctx, refs) {
  ...
  score += lineEvidenceAdjustment(candidate, "lifeLine");
...
function scoreHeadLine(candidate, ctx, refs) {
  ...
  score += lineEvidenceAdjustment(candidate, "headLine");
...
function scoreHeartLine(candidate, ctx, refs) {
  ...
  if (candidate.orientation === "vertical") score -= 30;
  score += lineEvidenceAdjustment(candidate, "heartLine");
...
function scoreFateLine(candidate, ctx, refs) {
  ...
  score += lineEvidenceAdjustment(candidate, "fateLine");
...
function scoreSunLine(candidate, ctx) {
  ...
  score += lineEvidenceAdjustment(candidate, "sunLine");
  return score;
}
...
function scoreMoneyLine(candidate, ctx, refs) {
  ...
  score += lineEvidenceAdjustment(candidate, "moneyLine");
...
function scoreMarriageLine(candidate, ctx, refs) {
  ...
  if (candidate.orientation === "vertical") score -= 30;
  score += lineEvidenceAdjustment(candidate, "marriageLine");
...
function scoreCommunicationLine(candidate, ctx) {
  ...
  score += lineEvidenceAdjustment(candidate, "communicationLine");
...
function selectLine(features, scorer, ctx, refs, options = {}) {
  ...
  for (const f of features) {
    ...
    const reusePenalty = allowUsed && used.has(f.id) ? 12 : 0;
    const score = scorer(f, ctx, refs) - reusePenalty;
    const evidence = clamp(toNumber(f.evidenceScore, f.confidence), 0, 1);
    const bestEvidence = best ? clamp(toNumber(best.feature.evidenceScore, best.feature.confidence), 0, 1) : 0;
    if (!best || score > best.score || (Math.abs(score - best.score) <= 4 && evidence > bestEvidence)) {
  ...
  return {
    ...
    id: f.id,
    reason: detected
      ? `${lineTypeLabel}의 위치와 흐름이 판정 기준에 맞습니다.`
      : `${lineTypeLabel}의 위치와 흐름이 아직 선명하지 않습니다.`,
    rawFeature: f,
  };
}
...
function classifyPalmLines(lineCandidates, ctx) {
  ...
  const communicationLine = selectLine(features, scoreCommunicationLine, ctx, { moneyLine, heartLine }, { highThreshold: 58, lowThreshold: 44, used, lineType: "communicationLine" });
...
function summarizeLineForOverall(name, line) {
  if (!line.detected) return `${name}은 이번 사진에서 명확히 잡히지 않아 단정하지 않고 보수적으로 다룹니다.`;
  const lengthText = line.lengthLabel === "long" ? "길게 이어지고" : line.lengthLabel === "short" ? "짧고 압축되어" : "중간 길이로";
  const depthText = line.depthLabel === "deep" ? "선의 힘이 분명합니다" : line.depthLabel === "faint" ? "결이 섬세하게 드러납니다" : "흐름이 안정적으로 보입니다";
  const branchText = line.branches > 0 ? ` 분기 ${line.branches}곳은 선택지가 갈라지는 지점으로 읽습니다.` : "";
  const breakText = line.breaks > 0 ? ` 끊김 ${line.breaks}곳은 리듬이 바뀌는 구간으로 봅니다.` : "";
  return `${name}은 ${lengthText} ${depthText}.${branchText}${breakText}`;
}

function minPathDistance(a, b) {
  ...
  for (const p1 of left) {
    for (const p2 of right) {
      best = Math.min(best, distance(p1, p2));
    }
  }
  return best;
}
...
function detectSimianLine(lines, ctx) {
  ...
  const confidentLines = head.confidence !== "low" && heart.confidence !== "low";
  const detected = confidentLines && overlapX >= 0.42 && yGap <= 0.08 && (centerRatio >= 0.4 || nearJoinedStart);
  if (!detected) return null;

  const confidence = clamp(0.45 + overlapX * 0.4 + (0.1 - yGap) * 0.9 + centerRatio * 0.2, 0, 0.99);
  return {
    code: "simian_line",
    label: palmMapEngineText("palmMap.001"),
    detected: true,
...
function detectMShapePattern(lines, ctx) {
  ...
  const fateHeadGap = minPathDistance(fate.path, head.path);
  const fateHeartGap = minPathDistance(fate.path, heart.path);
  const measurableLines = fate.confidence !== "unknown" && heart.confidence !== "unknown" && head.confidence !== "unknown";

  const detected =
    measurableLines &&
    branchSignal >= 2 &&
    centerPassRatio >= 0.32 &&
    upperSignal >= 0.5 &&
    rightSignal >= 0.2 &&
    fateHeadGap <= 0.16 &&
    fateHeartGap <= 0.18;
  ...
  return {
    code: "m_shape",
    label: palmMapEngineText("palmMap.002"),
    detected: true,
    confidence: round(confidence, 3),
...
function purposeLabelKo(purpose) {
  const key = String(purpose || "general").toLowerCase();
  if (key === "love") return "연애와 관계";
  if (key === "wealth") return "재물과 가치 창출";
  if (key === "career") return "직업과 목표";
  if (key === "personality") return "성향과 기질";
  if (key === "relationship") return "관계의 흐름";
  return "전체 운세";
}

function confidenceToneKo(value) {
  const score = clamp(toNumber(value, 0), 0, 1);
  if (score >= 0.78) return "매우 뚜렷함";
  if (score >= 0.62) return "뚜렷함";
  if (score >= 0.48) return "보통";
  return "옅음";
}

function scoreFlowKo(value) {
  const score = clampScore(value);
  if (score >= 72) return "강하게 살아 있습니다";
  if (score >= 58) return "안정적으로 열려 있습니다";
  if (score >= 44) return "천천히 다듬어지는 흐름입니다";
  return "아직은 조용히 잠긴 흐름입니다";
}

function mountFullnessKo(value) {
  ...
  return {
    ...
    warnings: hasImage
      ? hasLineCandidates && hasHandLandmarks
        ...
        : [
            "손 위치 기준과 선 후보가 없어 이미지 단독 판정을 보류했습니다.",
            "손바닥 전체가 선명한 사진으로 다시 촬영해 주세요.",
          ]
...
function buildCombinationInsights({ lines, mounts, handShape }) {
  ...
  function describeConditionToken(token) {
    if (typeof token !== "string" || !token.includes(".")) return "여러 손금 신호가 함께 맞물림";
    const parts = token.split(".");
    const lineLabels = {
      lifeLine: "생명선",
      headLine: "두뇌선",
    ...
    const strengthLabels = {
      ...
      long_straight: "길고 곧음",
      long_curved: "길고 유연함",
      separated_start: "시작점이 독립적임",
      joined_start: "시작점이 밀착됨",
      deep_curved: "깊고 부드러움",
      many_branches: "갈래가 풍부함",
      strong_center: "중심축이 강함",
      weak_or_absent: "희미함",
    };

    if (parts[0] === "mounts" && parts.length >= 3) {
      return `${mountLabels[parts[1]] || parts[1]}가 ${strengthLabels[parts[2]] || "특징적으로 드러남"}`;
    }
    if (parts[0] === "minorLines" && parts.length >= 3) {
      return `${lineLabels[parts[1]] || parts[1]}이 ${strengthLabels[parts[2]] || "특징적으로 드러남"}`;
    }
    return `${lineLabels[parts[0]] || parts[0]}이 ${strengthLabels[parts[1]] || "특징적으로 드러남"}`;
  }
  ...
  for (const rule of rules) {
    ...
    insights.push({
      ...
      reason: conditions.map((cond) => describeConditionToken(cond)).join(", "),
  ...
  if (insights.length === 0) {
    if (mounts?.jupiter?.fullness === "strong") {
      insights.push({
        key: "jupiter-lead",
        title: palmMapEngineText("palmMap.003"),
        reason: "목성구의 힘이 뚜렷하게 드러남",
    ...
    if (mounts?.saturn?.fullness === "strong" && lines?.fateLine?.detected) {
      insights.push({
        key: "saturn-fate",
        title: palmMapEngineText("palmMap.004"),
        reason: "토성구의 묵직함과 운명선의 중심축이 함께 보임",
...
function mapLineToCanonicalMajor(line, key) {
  ...
  if (!line?.detected) {
    const safeMeaning = lineData?.safeMeaning || "이번 사진에서는 흐름이 옅어 단정하지 않습니다";
    if (key === "lifeLine") {
      return {
        detected: false,
        length: "unknown",
        ...
        breaks: 0,
        branches: 0,
        summary: `${safeMeaning}. 손바닥의 빛과 각도를 보강하면 흐름을 더 섬세하게 읽을 수 있습니다.`,
    ...
    if (key === "headLine") {
      return {
        ...
        breaks: 0,
        branches: 0,
        summary: `${safeMeaning}. 손바닥 중앙의 가로 흐름이 더 선명할 때 판단을 확장할 수 있습니다.`,
    ...
    if (key === "heartLine") {
      return {
        ...
        breaks: 0,
        branches: 0,
        summary: `${safeMeaning}. 손가락 아래의 감정선 영역이 분명해지면 관계 흐름을 더 깊게 읽을 수 있습니다.`,
    ...
    return {
      ...
      endArea: "unknown",
      breaks: 0,
      summary: `${safeMeaning}. 손바닥 중앙의 세로 축이 분명할 때 목표와 전환 흐름을 더 정밀하게 볼 수 있습니다.`,
  ...
  const variantSummary = variantData?.detail || variantData?.summary || summarizeLineForOverall(lineData?.label || "주요선", line);
...
function mapLineToMinor(line, title, key) {
  ...
  if (!line?.detected) {
    return {
      detected: false,
      strength: null,
      summary: variantData?.detail || `${title}: 이번 사진에서는 흐름이 옅어 단정하지 않습니다.`,
    };
  }
  ...
  return {
    ...
    summary: variantData?.detail || summarizeLineForOverall(title, line),
  };
}
...
function buildStrengths(lines, handShape, mounts) {
  ...
  if (handShape.type !== "mixed") {
    out.push(`${handShape.labelKo}의 결이 강하게 보여 ${handShape.keywords[0]} 성향을 안정적으로 사용합니다.`);
  }
  if (lines.headLine.detected) out.push("두뇌선의 흐름이 살아 있어 생각을 정리하고 기준을 세우는 힘이 보입니다.");
  if (lines.heartLine.detected) out.push("감정선의 결이 분명해 관계의 온도와 신호를 섬세하게 읽는 편입니다.");
  if (lines.fateLine.detected) out.push("운명선의 중심축이 보여 맡은 일을 오래 밀고 가는 힘이 있습니다.");
  if (lines.moneyLine.detected) out.push("재물선의 흐름이 보여 돈을 목적 있게 모으고 쓰려는 감각이 살아 있습니다.");
...
function buildCautions(lines, mounts) {
  ...
  if (lines.lifeLine.detected && lines.lifeLine.depthLabel === "faint") {
    out.push("생명선이 섬세하게 보여 회복 루틴이 흔들리면 컨디션 기복이 커질 수 있습니다.");
  }
  if (lines.heartLine.detected && lines.heartLine.branches >= 2) {
    out.push("감정선의 갈래가 보여 상대의 반응을 마음속에서 여러 방향으로 해석하기 쉽습니다.");
  }
  if (!lines.fateLine.detected) {
    out.push("운명선이 또렷하지 않을 때는 직업 방향을 고정 결론보다 실험과 검증으로 다듬는 편이 좋습니다.");
  }
...
function buildComprehensiveSummary({ handShape, lines, mounts, role, analysisPurpose, combinations, specialPatterns }) {
  ...
  const featureFacts = [
    ...
    `손형은 ${handShape.labelKo}으로 판별되며, 손바닥과 손가락의 비율이 ${handShape.keywords[0]} 흐름을 보여줍니다.`,
    `구丘 신호는 금성구 ${mountFullnessKo(mounts.venus.fullness)}, 월구 ${mountFullnessKo(mounts.moon.fullness)}, 목성구 ${mountFullnessKo(mounts.jupiter.fullness)}, 토성구 ${mountFullnessKo(mounts.saturn.fullness)}, 태양구 ${mountFullnessKo(mounts.sun.fullness)}, 수성구 ${mountFullnessKo(mounts.mercury.fullness)}, 화성구 ${mountFullnessKo(mounts.mars.fullness)}으로 읽힙니다.`,
  ];
  ...
  const summary = [
    ...
    ...featureFacts,
    `${purposeLabel} 관점을 중심에 두되, 흐름이 옅은 항목은 억지로 채우지 않고 관찰 가능한 선만 해석에 반영했습니다.`,
    ...
    specialPatterns?.detected?.length
      ? `특수 패턴은 ${specialPatterns.detected.map((item) => `${item.label}(확신도 ${confidenceToneKo(item.confidence)})`).join(", ")} 흐름으로 읽힙니다.`
  ...
  return summary.length >= 800
    ? summary
    : `${summary} 추가 보완: 촬영 환경과 손바닥 점유율이 좋아질수록 미세선의 결이 더 맑게 드러납니다. 옅은 선은 다음 촬영에서 다시 확인해 해석을 보강하는 편이 좋습니다.`;
}
...
function buildSectionBlocks({ role, handShape, lines, mounts, scores, combinations, strengths, cautions, analysisPurpose, specialPatterns }) {
  ...
  const sections = [
    {
      key: "comprehensive",
      title: palmMapEngineText("palmMap.005"),
      content: comprehensive,
    },
    {
      key: "role",
      title: palmMapEngineText("palmMap.006"),
      content: `${roleLabelKo(role)} 기준 해석입니다. 선천적 손은 타고난 기질, 후천적 손은 현재 습관과 사회적 자아를 중심으로 읽습니다. 혼합형이면 두 축을 함께 보수적으로 반영합니다.`,
    },
    {
      key: "life",
      title: palmMapEngineText("palmMap.007"),
      content: lines.lifeLine.detected
        ? `${summarizeLineForOverall("생명선", lines.lifeLine)} 이 선은 수명이 아니라 에너지 운용, 회복 리듬, 생활 지속성의 패턴을 설명합니다.`
        : "생명선은 이번 사진에서 흐름이 옅어 에너지 운용의 결만 조심스럽게 살폈습니다.",
    },
    {
      key: "head",
      title: palmMapEngineText("palmMap.008"),
      content: lines.headLine.detected
        ? `${summarizeLineForOverall("두뇌선", lines.headLine)} 사고방식, 판단력, 집중력, 현실성/상상력 비율을 읽는 지표로 사용했습니다.`
        : "두뇌선은 이번 사진에서 중앙 흐름이 옅어 판단 방식에 대한 단정은 피했습니다.",
    },
    {
      key: "heart",
      title: palmMapEngineText("palmMap.009"),
      content: lines.heartLine.detected
        ? `${summarizeLineForOverall("감정선", lines.heartLine)} 사랑 방식, 감정 표현, 애착 패턴, 상처 민감 구간을 설명합니다.`
        : "감정선은 이번 사진에서 상단 결이 옅어 관계 흐름은 확인된 신호만 반영했습니다.",
    },
    {
      key: "fate",
      title: palmMapEngineText("palmMap.010"),
      content: lines.fateLine.detected
        ? `${summarizeLineForOverall("운명선", lines.fateLine)} 직업 방향, 사회적 목표, 자립성, 전환점 해석에 사용했습니다.`
        : "운명선은 이번 사진에서 중심 세로축이 옅어 직업 방향의 결론을 서두르지 않았습니다.",
    },
    {
      key: "sun-money",
      title: palmMapEngineText("palmMap.011"),
      content: `${summarizeLineForOverall("재물선", lines.moneyLine)} ${summarizeLineForOverall("태양선", lines.sunLine)} 재물선은 확정 재물운이 아닌 돈을 다루는 습관/가치 창출 방식을, 태양선은 표현력/브랜딩 감각을 설명합니다.`,
    },
    {
      key: "love-marriage",
      title: palmMapEngineText("palmMap.012"),
      content: `${summarizeLineForOverall("결혼선", lines.marriageLine)} 깊은 관계에서 원하는 안정감과 약속 방식을 읽으며, 결혼 횟수 단정은 포함하지 않습니다.`,
    },
    {
      key: "mounts",
      title: palmMapEngineText("palmMap.013"),
      content: [
    ...
    {
      key: "combinations",
      title: palmMapEngineText("palmMap.014"),
      content:
        combinations.length >= 1
          ? combinations
              .map((c, i) => `${i + 1}) ${c.title} - 관찰: ${c.reason} - 해석: ${c.result} - 주의: ${c.caution}`)
              .join(" ")
          : "검출 근거 부족으로 조합 해석은 보수적으로 유보했습니다.",
    },
    {
      key: "special-patterns",
      title: palmMapEngineText("palmMap.015"),
      content:
        specialPatterns?.detected?.length
          ? specialPatterns.detected
              .map(
                (item, index) =>
                  `${index + 1}) ${item.label} - 확신도 ${confidenceToneKo(item.confidence)} - 해석: ${item.summary}`,
              )
              .join(" ")
    ...
    {
      key: "scores",
      title: palmMapEngineText("palmMap.016"),
      content: `연애는 ${scoreFlowKo(scores.love)}. 직업은 ${scoreFlowKo(scores.career)}. 재물은 ${scoreFlowKo(scores.wealth)}. 에너지는 ${scoreFlowKo(scores.vitality)}. 창작은 ${scoreFlowKo(scores.creativity)}. 소통은 ${scoreFlowKo(scores.communication)}. 이 흐름은 절대 예언이 아니라 현재 손금 구조에서 읽힌 상대적 기운입니다.`,
    },
    {
      key: "strengths",
      title: palmMapEngineText("palmMap.017"),
      content: strengths.length ? strengths.join(" ") : "강점 근거가 충분하지 않아 보수적으로 유지했습니다.",
    },
    {
      key: "cautions",
      title: palmMapEngineText("palmMap.018"),
      content: cautions.length ? cautions.join(" ") : "주의점 근거가 충분하지 않아 보수적으로 유지했습니다.",
    },
    {
      key: "advice",
      title: palmMapEngineText("palmMap.019"),
      content:
...
function analyzePalmHandInput(input = {}) {
  ...
  const hasInputLineCandidates = lineCandidates.length > 0;
  const allowSyntheticFallback = input.allowSyntheticFallback === true;
  ...
  const fallbackLandmarks = allowSyntheticFallback && extraction.extractionAttempted && !hasInputLandmarks
  ...
  if (allowSyntheticFallback && lineCandidates.length === 0 && coord.ok && extraction.extractionAttempted) {
  ...
  const hasLandmarks = coord.ok && hasInputLandmarks;
  const hasTrustedLines = coord.ok && hasInputLineCandidates;
  const isPalmDetected = hasLandmarks || hasTrustedLines;
  ...
  return applySafetyExpressionFilter({
    ...
    overlayPaths,
    hasMajorDetected:
      isPalmDetected && (lines.lifeLine.detected || lines.headLine.detected || lines.heartLine.detected || lines.fateLine.detected),
  });
}
