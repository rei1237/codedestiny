const { HAND_COMPARISON_RULES } = require("./palmistry-data.js");
...
function comparisonRule(section, key) {
  return HAND_COMPARISON_RULES?.[section]?.[key] || null;
}

function renderRuleOrFallback(rule, fallback) {
  if (!rule) return fallback;
  return `${rule.title}. ${rule.detail} ${rule.advice}`;
}
...
function compareLifeLine(innate, acquired) {
  ...
  if (iDepth > aDepth) {
    return renderRuleOrFallback(
      comparisonRule("lifeLine", "innateStrong_acquiredWeak"),
      "본래 회복력은 좋지만 현재는 에너지 소모가 커진 상태로 읽힙니다.",
    );
  }

  if (iDepth < aDepth) {
    return renderRuleOrFallback(
      comparisonRule("lifeLine", "innateWeak_acquiredStrong"),
      "후천적 자기관리와 생활 루틴이 잘 형성된 상태로 읽힙니다.",
    );
  }
...
function compareHeadLine(innate, acquired) {
  ...
  if (iDir === "curved" && aDir === "straight") {
    return renderRuleOrFallback(
      comparisonRule("headLine", "innateCurved_acquiredStraight"),
      "본래 감수성이 강하지만 현재는 현실 판단이 강화됨으로 읽힙니다.",
    );
  }

  if (iDir === "straight" && aDir === "curved") {
    return renderRuleOrFallback(
      comparisonRule("headLine", "innateStraight_acquiredCurved"),
      "본래 이성적이지만 현재는 직관과 감성 사용이 늘어남으로 읽힙니다.",
    );
  }
...
function compareHeartLine(innate, acquired) {
  ...
  if (iRich > aRich + 0.35) {
    return renderRuleOrFallback(
      comparisonRule("heartLine", "innateDeep_acquiredFaint"),
      "본래 감정은 깊지만 현재는 표현을 조절하거나 방어적으로 바뀐 흐름이 보입니다.",
    );
  }

  if (aRich > iRich + 0.35) {
    return renderRuleOrFallback(
      comparisonRule("heartLine", "innateFaint_acquiredDeep"),
      "관계 경험을 통해 감정 표현이 열린 상태로 읽힙니다.",
    );
  }
...
function compareFateLine(innate, acquired) {
  ...
  if (a > i) {
    return renderRuleOrFallback(
      comparisonRule("fateLine", "innateWeak_acquiredStrong"),
      "현재 삶에서 목표의식과 사회적 방향성이 강화됨으로 읽힙니다.",
    );
  }

  if (i > a) {
    return renderRuleOrFallback(
      comparisonRule("fateLine", "innateStrong_acquiredWeak"),
      "본래 목표의식은 강했지만 현재는 방향을 재정비 중인 흐름으로 읽힙니다.",
    );
  }
