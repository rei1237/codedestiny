...
/* 서비스 플래그
     builtIn          admin.js 의 buildAdminPromptByService 가 직접 조립한다(나머지는 라우트 로더)
     needsCoordinates 출생지 위도·경도가 있어야 차트가 성립한다. 워커는 이 선언을 보고 출생지
                      텍스트를 좌표로 해석해 주고, 끝내 못 찾으면 400 으로 거절한다.
                      🔴 손목록으로 다시 적지 말 것 — 서버 판정과 가드가 이 플래그만 본다 */
...
export const ADMIN_PROMPT_LAB_SERVICES = Object.freeze([
  ...
  },
  {
    key: "fusion-fortune",
    label: "초융합 운세",
    routes: ["fusion-fortune.js"],
    group: "report",
    inputs: ["profile", "variant", "question"],
    variantLabel: "생성 그룹",
    note: "한 번에 뽑지 않고 4개 그룹을 병렬 생성한다 — 그룹마다 프롬프트가 다르다 (여섯 체계 계산이라 느림)",
  },
  ...
  {
    // 🔴 차트는 2026-09 부터 무료이고 옛 해석 생성은 은퇴했다. 이 항목은 그때 저장된
    //    프롬프트 계약을 들여다보는 용도로 남는다.
    key: "human-design",
    label: "휴먼 디자인 (레거시 해석)",
    routes: ["human-design.js"],
    group: "special",
    inputs: ["profile", "question"],
  },
  {
    key: "human-design-report",
    label: "휴먼 디자인 프리미엄 리포트",
    routes: ["human-design-report.js"],
    group: "special",
    inputs: ["profile", "variant", "question"],
    variantLabel: "섹션",
  },
...
/* 랩에서 뽑을 수 없는 기능과 그 이유. __tests__/worker/admin-prompt-lab-coverage.test.js 가 이 목록을 읽어
...
export const ADMIN_PROMPT_LAB_EXCLUSIONS = Object.freeze([
  ...
  {
    route: "relationship-boundary-test",
    reason: "대상자 생년 정보와 결제 완료 증빙을 함께 다루는 회당 유료 리포트다. 관리용 랩에서 대상자 관계 분석을 재현하지 않는다.",
  },
]);
...
/** 출생지 좌표가 있어야 성립하는 서비스인가. 워커의 좌표 자동 해석·400 판정이 이것만 본다. */
export function promptLabServiceNeedsCoordinates(key) {
  return Boolean(getAdminPromptLabService(key)?.needsCoordinates);
}
