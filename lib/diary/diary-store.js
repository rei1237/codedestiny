 ...
 *
 * 원본 위치(2026-09-07 기준): `js/luck-sync-diary.js` — LS_KEY:587 / ensureEntryShape:601 /
 * loadDiary:645 / saveDiary:703 / getTodayEntry:778
 *
...
/**
 * 하루치 빈 엔트리를 만든다. 원본 `getTodayEntry:781-810` 의 리터럴 축자 사본이고,
...
/**
 * 저장소 전체를 읽는다. 원본 `loadDiary:645` 축자 사본 — 깨진 JSON 은 조용히 `{}` 다.
...
/** 저장소 전체를 쓴다. 원본 `saveDiary:703` 축자 사본(토스트 제외). 실패하면 `false`. */
 ...
 * 원본 `getTodayEntry:778` 과 같은 동작이라, 호출자는 받은 객체를 고친 뒤
