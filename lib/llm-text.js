 ...
 * @template T
 * @param {T[]} items
 * @param {(item: T) => unknown} [selectText] 비교할 텍스트를 고르는 셀렉터(객체 항목용). 기본은 항목 자체.
 * @returns {T[]}
 */
export function dedupeTextList(items, selectText) {
  const pick = typeof selectText === "function" ? selectText : (item) => item;
  const seen = new Set();
  const result = [];
  for (const item of Array.isArray(items) ? items : []) {
    const source = String(pick(item) ?? "");
    // 정규화 결과가 비면(구두점뿐인 항목) 원문을 키로 써서 정상 항목을 조용히 버리지 않는다.
    const key = source.toLowerCase().replace(DEDUPE_NOISE_PATTERN, "") || source;
...
// 🔴 예전에는 여기가 `/[A-Za-z]/` 였다. 그래서 **라틴 문자를 쓰지 않는 언어는 CJK 가 아닌 한
//    본문 전체가 폐기됐다** — 힌디(데바나가리)가 정확히 그 경우다. 폐기되면 상위
//    hasRenderableLlmText 가 false 를 돌려주고, 응답이 잘려 온 요청은 결제 권한 보존(환불)
//    경로로 빠진다. 영어에서 실제로 났던 사고(위 주석)와 같은 형태이고, 원인만 문자 집합이다.
//    글자면 무엇이든 통과시키고, "본문인가"는 아래 단어 수·길이 규칙이 계속 판정한다.
...
function isReadableValue(value) {
  ...
  if (!LETTER_PATTERN.test(value)) return false; // 숫자·날짜·기호만 있는 값
...
/**
 * @typedef {{ titleKeywords?: RegExp, fallbackTitles?: string[], minHeadings?: number, numberedHeadings?: boolean }} AssistantSectionOptions
 */
...
export function parseAssistantSections(content, options) {
  const { titleKeywords, fallbackTitles = ["핵심 요약"], minHeadings = 3, numberedHeadings = false } = options || {};
  ...
  if (titleKeywords) {
    ...
    // 🔴 상한이 46자였다. 한국어 장 제목은 그 안에 들어가지만 번역된 제목은 넘는다 — 실측:
    //    영어 작명첩 2장 "Your chart, and the favourable element verified" 는 47자라 헤딩으로
    //    잡히지도 않았다. 아래 numberedHeadings 경로는 이미 80자를 허용하므로 창을 그쪽에 맞춘다.
    //    길이만 넓히는 것이고 titleKeywords 일치 조건은 그대로라, 통과 조건이 느슨해지는 것은
    //    "장 제목 낱말을 품은 47~80자 줄"뿐이다.
  ...
  // 🔴 키워드가 안 맞는 응답(예: 프롬프트가 출력 언어를 바꿔 장 제목이 번역된 경우) — 번호 붙은
  //    헤딩으로 다시 잡는다. 프롬프트는 "## N." 의 번호와 순서를 언어와 무관하게 고정하라고 지시한다.
  //    이 폴백이 없으면 아래 문단 균등 분할로 떨어져 내용이 엉뚱한 장 제목 아래로 들어간다.
  if (numberedHeadings) {
    const numbered = [];
    const numberedPattern = /^#{1,3}\s*(\d{1,2})[.)]\s*([^\n]{1,80})\n+/gm;
    let numberedMatch = numberedPattern.exec(normalized);
    while (numberedMatch) {
      numbered.push(numberedMatch);
      numberedMatch = numberedPattern.exec(normalized);
    }
    if (numbered.length >= minHeadings) {
      return numbered
        .map((item, index) => {
          const start = item.index + item[0].length;
