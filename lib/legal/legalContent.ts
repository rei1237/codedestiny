  ...
  ja: {
    ...
    sections: [
      ...
      { id: "collected-data", heading: "2. 収集項目と利用目的", paragraphs: [
        "会員登録時、氏名、メールアドレス、必須同意記録を、アカウント管理、カスタマーサポート、通知の送信、不正利用防止のために処理します。携帯電話番号は会員登録時の必須項目です。収集方法は三つで、会員登録画面でお客様が直接入力する場合、ソーシャルログイン提供者（ネイバー・カカオ）がお客様の同意を得て提供する場合、番号が未登録の既存会員が初回のカード決済時に別途同意のうえ入力する場合です。利用目的は会員の識別およびアカウント管理と決済の遂行であり、この番号に広告目的のメッセージや電話をお送りすることはありません。携帯電話番号は決済代行会社に購入者情報として提供する必要があるためサービス利用中は削除できませんが、サーバーに保存する際はAES-256方式で暗号化して保管します。メール登録の場合はパスワードの原文ではなく安全にハッシュ化された値が保存され、ソーシャル登録の場合は選択した認証プロバイダーのアカウント識別子が連携されます。生年月日、性別、出生時刻、出生地等、運勢計算に必要な入力値は、登録後に該当する運勢機能またはプロフィールを使用する際に別途入力を受け、結果生成に使用します。",
  ...
  zh: {
    ...
    sections: [
      ...
      { id: "collected-data", heading: "2. 收集项目与使用目的", paragraphs: [
        "会员注册时，公司出于账号管理、客户服务、通知发送及防止不当使用等目的，处理姓名、邮箱及必要同意记录。手机号码为会员注册时的必填项目。收集方式有三种：您在注册页面亲自填写、社交登录提供商（Naver·Kakao）经您同意后提供，或尚未登记号码的现有会员在首次银行卡结算时另行同意并填写。使用目的为会员识别与账号管理以及结算处理，本公司不会向该号码发送广告短信或拨打广告电话。由于手机号码须作为购买者信息提供给支付代理机构，在您使用服务期间无法删除，但存储于服务器时将以AES-256方式加密保管。以邮箱注册时，保存的并非密码原文，而是经过安全哈希处理的数值；以社交账号注册时，将关联所选身份验证提供商的账号识别符。生辰、性别、出生时间、出生地等运势计算所需的输入值，将在注册后使用相应运势功能或个人资料时另行收集，并用于结果生成。",
  ...
  "zh-TW": {
    ...
    sections: [
      ...
      { id: "collected-data", heading: "2. 蒐集項目與使用目的", paragraphs: [
        "會員註冊時，公司基於帳號管理、客戶服務、通知發送及防止不當使用等目的，處理姓名、電子郵件及必要同意紀錄。行動電話號碼為會員註冊時的必填項目。收集方式有三種：您於註冊頁面親自填寫、社群登入提供商（Naver·Kakao）經您同意後提供，或尚未登記號碼的現有會員於首次信用卡結帳時另行同意並填寫。使用目的為會員識別與帳號管理及結帳處理，本公司不會向該號碼發送廣告簡訊或撥打廣告電話。由於行動電話號碼須作為購買者資訊提供予支付代理機構，於您使用服務期間無法刪除，但儲存於伺服器時將以AES-256方式加密保管。以電子郵件註冊時，儲存的並非密碼原文，而是經過安全雜湊處理之數值；以社群帳號註冊時，將連結所選身分驗證提供商之帳號識別碼。出生年月日、性別、出生時間、出生地等運勢計算所需之輸入值，將於註冊後使用相應運勢功能或個人檔案時另行收集，並用於結果生成。",
