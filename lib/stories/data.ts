...
const STORY_DATA_TEXT_TRANSLATIONS = {
  ko: {
    ...
    "story.title.031": "제26화. 금빛 사자의 발톱",
...
[메모 3: 다음 줄을 기대한다는 말은 부담을 준 것일 수도 있다.]
...
const codeDestinyChapter14Revised = `제14화. 인성의 섬, 다음 줄이 태어나는 곳
...
[목적] 현재 사건의 원인과 반응을 한 줄로 묶는다.
[오해] 즉시 단정하지 말고 사실만 먼저 맞춘다.
[지연] 즉시 반응 대신 짧은 검증 후 개입한다.
[파장] 누락된 관계가 생기지 않게 뒤엣줄을 붙잡는다.
[다음 훅] 관계의 지연을 먼저 세우고, 증거로 되돌린다.
[금지 규칙] 네오 정체 전면 공개, 무성 동기 단순화 금지, 치즈색 햄스터 용어 고정.
...
const codeDestinyChapter15Revised = `제15화. 기억 도서관
...
[지연] 즉시 반응 대신 짧은 검증 후 개입한다.
[파장] 누락된 관계가 생기지 않게 뒤엣줄을 붙잡는다.
[다음 훅] 다음 문장으로 넘어가기 전 오해를 한 줄로 정리한다.
[금지 규칙] 네오 정체 전면 공개, 무성 동기 단순화 금지, 치즈색 햄스터 용어 고정.
...
const codeDestinyChapter16Revised = `제16화. 너무 친절한 사서
...
[지연] 즉시 반응 대신 짧은 검증 후 개입한다.
[파장] 누락된 관계가 생기지 않게 뒤엣줄을 붙잡는다.
[다음 훅] 네오의 정체는 조각으로 남기고, 사람의 반응부터 붙인다.
[금지 규칙] 네오 정체 전면 공개, 무성 동기 단순화 금지, 치즈색 햄스터 용어 고정.
...
const codeDestinyChapter17Revised = `제17화. 해석을 바꾸는 자
...
const codeDestinyChapter18Revised = `제18화. 첫 울림과 메모
...
[지연] 즉시 반응 대신 짧은 검증 후 개입한다.
[파장] 누락된 관계가 생기지 않게 뒤엣줄을 붙잡는다.
[다음 훅] 치즈색 햄스터의 기준으로 감정 기준점을 고정한다.
[금지 규칙] 네오 정체 전면 공개, 무성 동기 단순화 금지, 치즈색 햄스터 용어 고정.
...
const codeDestinyChapter19Revised = `제19화. 본뜻을 되찾는 법
...
[지연] 즉시 반응 대신 짧은 검증 후 개입한다.
[파장] 누락된 관계가 생기지 않게 뒤엣줄을 붙잡는다.
[다음 훅] 검은 비 속 남자의 실루엣은 정체 공개 없이 압박만 남긴다.
[금지 규칙] 네오 정체 전면 공개, 무성 동기 단순화 금지, 치즈색 햄스터 용어 고정.
...
const codeDestinyChapter20Revised = `제20화. 다시 읽는 마음
...
const codeDestinyChapter21Revised = `제21화. 아직 쓰는 중인 이름
...
const codeDestinyChapter22Revised = `제22화. 사주 명리학의 힘을 깨우는 법
...
const codeDestinyChapter23Revised = `제23화. 청토끼 금융그룹
...
const codeDestinyChapter24Revised = `제24화. 경매장에 걸린 토끼
...
const codeDestinyChapter25Revised = `제25화. 선택의 가격
...
const codeDestinyChapter26Revised = `제26화. 금빛 사자의 발톱
...
const codeDestinyChapter27Revised = `제27화. 계약의 우리
...
const codeDestinyChapter28Revised = `제28화. 담보가 되지 않는 것
...
const codeDestinyChapter29Revised = `제29화. 글자는 따로 빛나지 않는다
...
const codeDestinyChapter30Revised = `제30화. 원본 계약서
...
const codeDestinyChapter31Revised = `제31화. 마지막 기둥, 신미
...
const codeDestinyChapter32Revised = `제32화. 년주의 기억
...
const codeDestinyChapter33Revised = `제33화. 현실의 아침, 운세 세계의 알림
...
const codeDestinyChapter34Revised = `제34화. 오늘의 운세 제작진
...
const codeDestinyChapter35Revised = `제35화. 흰 고양이가 말을 걸었다
...
const codeDestinyChapter36Revised = `제36화. 운은 쓰는 것이다
...
const codeDestinyChapter37Revised = `제37화. 오후 세 시 십칠분의 예언
...
const codeDestinyChapter38Revised = `제38화. 다른 운명의 상대
...
const codeDestinyChapter39Revised = `제39화. 인생 최초 최대 학점
...
const codeDestinyChapter40Revised = `제40화. 별자리 운세가 이상하다
...
const codeDestinyChapter41Revised = `제41화. 별을 읽는 법
...
const codeDestinyChapter42Revised = `제42화. 별의 이름을 돌려줘
...
const codeDestinyChapter43Revised = `제43화. 회중시계를 준 자
...
const codeDestinyChapter44Revised = `제44화. 별빛이 너무 세거나 너무 약한 날
...
모든 궁이 별빛 궁전처럼 떠 있었지만, 지금은 중심을 잃고 조금씩 기울어져 있었다.
...
다음 순간, 문 안에서 거대한 금빛 사자의 포효가 터져 나왔다.
...
그의 등 뒤에는 거대한 금빛 사자의 형상이 일렁이고 있었다.
...
금빛 사자의 형상이 그 뒤를 따라 솟구쳤다.

연이는 반대편 자미성의 문으로 뛰어들었다.

경계는 얇아지고 있었다.

현실과 운세 세계가 서로를 비추기 시작했다.

그리고 무성은, 그 틈을 통해 두 세계를 동시에 꺼뜨리려 하고 있었다.
`;
...
const CODE_DESTINY_ACT_RULES: Array<{ act: StoryAct; startChapter: number; endChapter: number; focus: string }> = [
  ...
  },
  {
    ...
    startChapter: 37,
    endChapter: 47,
    focus: "‘검은 비 속 남자 실루엣’을 실체 공개보다 관계 압박형 미스터리로 처리.",
  },
];
