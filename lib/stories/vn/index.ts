...
//
// 로그라인은 편집자가 관리하는 요약이다. 폐기한 구 소설 산문 코드는 참조하지 않는다.
// 텍스트 리더와 삽화 플레이어는 모두 라이트 노벨 정본을 사용한다.
...
export const STORY_ARCS: StoryArc[] = [
  {
    ...
    title: "1부 · 비겁의 섬",
    summary:
      "평범한 대학생 연이가 정체 모를 앱을 깔고 꽃돼지의 몸으로 운세 세계에 떨어진다. 자기 자신과 같은 기운을 가진 존재들이 모인 첫 섬에서, 연이는 스스로를 어떻게 부를지부터 다시 배운다.",
    from: 0,
    to: 8,
  },
  {
...
export const STORY_LOGLINES: Record<string, string> = {
  "ep-01a": "갈대에 갇힌 검은 호랑이를 구한다. 작은 몸이 숨긴 시선은 연이의 약점을 먼저 읽는다.",
  "ep-06a": "멈추지 못하는 북소리. 빌린 박수를 돌려주는 동안 누군가는 다른 장부를 쓰고 있다.",
  "ep-12a": "전투 뒤의 붕대와 따뜻한 물. 서로를 돌보는 방식이 누군가에게는 공격할 틈이 된다.",
  "ep-14a": "서로의 호흡을 맞추는 연이와 네오. 지키는 사람도 기대어 쉬는 법을 배운다.",
  "ep-21a": "잊힌 이름을 따라 넘어간 페이지에서, 돌아가야 할 이유가 하나 더 생긴다.",
  "ep-27a": "돌아갈 문 앞에서 네오와 연이는 서로의 자리를 쉽게 말하지 못한다.",
  "ep-27b": "익숙한 방을 담은 구슬의 가격은 네오의 이름. 호랑이의 선물에는 보이지 않는 이가 있다.",
  "ep-28a": "인간 세계의 후원자가 붉은 귀걸이를 만진다. 검은 호랑이와 같은 버릇이다.",
  "ep-28b": "안전한 내일을 약속하는 계약서. 연이가 뒷면을 읽자 서한비는 다음 덫을 연다.",
  "ep-28c": "비 오는 정류장에서 두 세계의 힘이 만난다. 서한비가 기다린 것은 두 사람이 끝까지 버티는 순간이다.",
  "ep-28d": "사라진 악역이 남긴 망토에는 이름들이 숨었다. 연이는 다정했던 기억 대신 증거를 챙긴다.",
  "ep-32a": "삼켰던 이야기들을 돌려주는 무성. 자기 이름을 찾는 길이 다른 이름들과 이어진다.",
  "ep-36a": "사랑이라는 이름으로 빼앗은 것들. 서한비가 원하는 자리에 다른 사람의 선택은 없다.",
  "ep-38a": "각자의 힘을 고른 일곱 개의 손이 끊어진 길을 다시 받친다.",
  "ep-41a": "베틀이 멎어도 모든 상처가 사라지지는 않는다. 빼앗긴 이름들은 주인의 목소리로 돌아간다.",
  "ep-42a": "평범한 봄을 함께 고르는 연이와 병하. 강 건너에서는 아직 반환 목록을 대조하고 있다.",
