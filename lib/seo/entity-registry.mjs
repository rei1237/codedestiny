 ...
 * Long-tail priorities are validated against Search Console / Naver / Bing
 * exports before they are promoted to page metadata.
 */
...
const CORE_ROUTE_PROFILES = [
  ...
  },
  {
    ...
    longTail: ["생년월일과 출생시간으로 만세력 보는 법"],
  ...
  },
  {
    ...
    relatedPaths: ["/daily-fortune", "/saju", "/astrology", "/tarot"],
    schema: ["WebPage", "Service", "FAQPage", "BreadcrumbList"],
  },
  {
    path: "/daily-fortune",
    title: "일일 운세",
    primary: "일일 운세",
    secondary: ["오늘 운세", "하루 운세"],
    longTail: ["하루를 시작하기 전 오늘 운세를 확인하는 방법"],
    topicSummary: "짧은 일일 운세를 오늘의 운세 허브와 관련 해석 서비스로 연결합니다.",
    relatedPaths: ["/today", "/saju", "/tarot", "/astrology"],
    schema: ["WebPage", "Service", "FAQPage", "BreadcrumbList"],
  },
  {
    path: "/dream",
...
export const SEO_ROUTE_PROFILES = Object.freeze(
  Object.fromEntries(CORE_ROUTE_PROFILES.map((profile) => [profile.path, Object.freeze(profile)])),
);

/**
 * The actual feature work owns when this profile becomes public. Do not link
 * to it until the feature route is merged and its public contract is verified.
 */
export const FUSION_FORTUNE_PENDING_PROFILE = Object.freeze({
  path: "/fusion-fortune",
  publicationState: "pending-feature-merge",
  primary: "초융합 운세",
  secondary: ["AI 초융합 운세", "통합 운세", "AI 운세"],
  longTail: ["AI가 여러 운세를 동시에 분석하는 서비스"],
  schema: ["WebPage", "Service", "FAQPage", "BreadcrumbList"],
});
...
export function getSeoProfileKeywords(path) {
  ...
  return [profile.primary, ...profile.secondary, ...profile.longTail];
}
