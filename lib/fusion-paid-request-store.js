...
export function readFusionPaidRequest(options = {}) {
  const { now = Date.now(), ownerId, ...stores } = options;
  if (ownerId !== undefined && !ownerId) return null;
  const key = ownerId === undefined ? FUSION_PAID_REQUEST_KEY : `${FUSION_PAID_REQUEST_KEY}:${encodeURIComponent(ownerId)}`;
  ...
  if (raw) {
    try {
      ...
      if (requestId && Number.isFinite(savedAt) && (ownerId !== undefined ? parsed.ownerId === ownerId : now - savedAt >= 0 && now - savedAt <= FUSION_PAID_REQUEST_TTL_MS)) {
  ...
  // Account-less legacy records cannot be attributed to the current signed-in user.
...
export function writeFusionPaidRequest({ requestId, body } = {}, options = {}) {
  ...
  const record = { requestId: String(requestId), body: normalizeBody(body), savedAt: now, ...(ownerId === undefined ? {} : { ownerId }) };
  const key = ownerId === undefined ? FUSION_PAID_REQUEST_KEY : `${FUSION_PAID_REQUEST_KEY}:${encodeURIComponent(ownerId)}`;
  safeSet(local, key, JSON.stringify(record));
  return record;
}
...
export function clearFusionPaidRequest(options = {}) {
  if (options.ownerId !== undefined) {
    if (options.ownerId) safeRemove(resolveStores(options).local, `${FUSION_PAID_REQUEST_KEY}:${encodeURIComponent(options.ownerId)}`);
    return;
  }
