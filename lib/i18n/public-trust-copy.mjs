  ...
  ja: {
    ...
    contact: {
      ...
      sections: [
        ...
        ["お支払い・返金と海外発行カードについて", "決済は韓国ウォン(KRW)で承認され、為替はカード会社のレートが適用されるため、明細の金額が決済画面の表示と異なることがあります。決済が承認されない場合や、請求はあるのにサービスが開かない場合は、下記の窓口へおおよその日時、決済画面に表示された金額、明細に記載された通貨、カードの発行国をお知らせください。カード番号は下4桁のみとし、全桁・セキュリティコード・認証メッセージは送らないでください。返金の条件はこのページではなく利用規約と返金ポリシーが定めており、海外で発行されたカードでのお支払いは現在準備中です。", "payment-help"],
  ...
  en: {
    ...
    contact: {
      ...
      sections: [
        ...
        ["Payment, refund and cards issued outside Korea", "Charges are authorised in Korean won (KRW), and your card issuer applies its own exchange rate, so the amount on your statement can differ from the figure shown at checkout. If a payment was declined, or a charge appears without the service opening, write to the address below with the approximate time, the amount displayed at checkout, the currency on your statement and the country where the card was issued. Send only the last four digits of the card — never the full number, the security code or a verification message. Refund conditions are set by the terms of service and the refund policy rather than by this page, and payment with cards issued outside Korea is still being prepared.", "payment-help"],
  ...
  zh: {
    ...
    contact: {
      ...
      sections: [
        ...
        ["支付、退款与境外发行的银行卡", "款项以韩元(KRW)授权，汇率由发卡机构自行计算，因此账单金额可能与结算页面显示的数字不同。如果支付未获授权，或已产生扣款但服务没有开启，请通过下方邮箱告知大致时间、结算页面显示的金额、账单上的币种以及发卡国家或地区。卡号只需提供后四位，请勿发送完整卡号、安全码或验证短信。退款条件以服务条款和退款政策为准，本页不另行约定；使用境外发行的银行卡支付目前仍在准备中。", "payment-help"],
...
// Existing canonical trust documents; labels are shared by policy navigation.
export const TRUST_DOCUMENT_COPY = {
  ko: { disclaimer: "면책 고지", editorial: "콘텐츠 편집 원칙", methodology: "콘텐츠 방법론" },
  en: { disclaimer: "Disclaimer", editorial: "Editorial Policy", methodology: "Content Methodology" },
  ja: { disclaimer: "免責事項", editorial: "編集方針", methodology: "コンテンツ方法論" },
  zh: { disclaimer: "免责声明", editorial: "编辑政策", methodology: "内容方法论" },
  "zh-tw": { disclaimer: "免責聲明", editorial: "編輯政策", methodology: "內容方法論" },
};
export function trustDocumentLinks(locale = "ko") {
  const copy = TRUST_DOCUMENT_COPY[locale] || TRUST_DOCUMENT_COPY.ko;
  return [
    { key: "disclaimer", href: "/disclaimer/", label: copy.disclaimer },
    { key: "editorial", href: "/editorial-policy/", label: copy.editorial },
    { key: "methodology", href: "/methodology/", label: copy.methodology },
  ];
}

export const METHODOLOGY_NOTE_COPY = { ko: {
  calculation: "명식·행성 위치·27숙은 계산 엔진이 냅니다. 출생 정보·장소·시간대·계산 설정과 엔진 버전이 같은지 확인해야 같은 조건의 값을 비교할 수 있습니다. 카드 뽑기와 AI 해설 문장은 별도 단계입니다.",
  reviewTitle: "검수 여부와 검색 공개는 다릅니다",
  review: "규칙으로 조립되는 페이지와 개별 원고는 제작 방식이 다릅니다. 검색에 공개되어 있다는 사실만으로 사람의 개별 검수가 끝났다는 뜻은 아닙니다. 검수자·검수일은 실제 확인 기록이 있는 경우에만 표시합니다.",
} };

export const SUPPORT_REPLY_NOTE_COPY = { ko: {
  general: "이 목록은 자주 묻는 것만 담습니다. 개별 상황은 문의 페이지로 보내 주세요. 보통 영업일 기준 1~3일 내 답변하며, 확인 절차에 따라 더 걸릴 수 있습니다.",
  account: "개별 계정이나 결제 건은 이 목록으로 답하기 어렵습니다. 문의 페이지로 보내 주세요. 보통 영업일 기준 1~3일 내 회신하며, 확인 절차에 따라 더 걸릴 수 있습니다.",
} };
