...
- 🔴 [docs/guard-integrity-2026-08-13.md](docs/guard-integrity-2026-08-13.md) — **가드가 초록불인데 아무것도 안 지키던 사례 모음(G-1~G-8).** 가드를 만지거나 검증기를 지우기 전에 읽을 것. **G-1~G-7 조치 완료**(2026-08-14 실측 재확인): 빌드 산출물 오인은 `isBuildArtifactDir()` 제외로, 배선 누락과 `deploy:critical`↔paid-flow-gates 커버리지 차이는 `scripts/verify-guard-wiring.mjs`(fail-closed 3방향)가 `pr-ci.yml` 에 배선되어 각각 막는다.
  - 🔴 **G-8 은 미조치다(2026-08-15 발견).** `verify:mobile-cdp-smoke` 는 `dist/index.html` 이 있으면 신선도를 안 보고 **무조건 그것을 서빙**한다(`scripts/verify-mobile-cdp-smoke.mjs:9`). `dist/` 는 gitignore 된 로컬 산출물이라 낡은 채로 남고, 그러면 소스를 아무리 고쳐도 옛 셸을 검사하면서 조용히 통과한다(실측: 로컬 `dist/` 에 `data-pvw-cta-bypass` 0건 · 루트 21건). 이 검증기를 쓸 때는 `mv dist dist.bak` 로 루트를 서빙시킬 것. 단 루트 서빙 시 Next 라우트가 파일로 없어 **라우트 도달을 축으로 삼는 단언은 환경 탓으로 실패**하므로, 새 단언은 `window.__cdLastMobileAction`(액션 발화 기록)을 축으로 쓴다.
