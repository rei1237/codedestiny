...
3. 🔴 **`main` 직접 작업·직접 배포 금지.** 브랜치(`feature/*`·`fix/*`·`refactor/*`·`chore/*`) → 커밋 → push → PR → CI → **사용자가 머지 → 자동 스테이징 배포**. 🔴 **2026-08-20 컷오버 — "머지 = 프로덕션 라이브"는 더 이상 아니다.** 머지는 스테이징(`staging.code-destiny.com`, DB 분리)까지만 자동으로 가고, 프로덕션(`code-destiny.com`)은 사람이 GitHub Actions 에서 `workflow_dispatch(mode=production)` 을 수동 실행해야 승격된다 — **그 실행이 실제 프로덕션 배포 행위이므로 규칙 2와 같은 급으로 명시적 허락 없이는 하지 않는다.** 프로덕션이 main HEAD 보다 뒤처져 있는 것은 정상 상태다. 상세: [docs/context/delivery-and-ci.md](docs/context/delivery-and-ci.md). 브랜치 룰셋이 직접 push를 막고, `scripts/lib/production-deploy-guard.mjs` 가 로컬 프로덕션 배포를 막는다. 우회하지 않는다.
