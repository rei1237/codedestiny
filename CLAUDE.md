...
**모든 수정 요청은 GREEN 또는 RED 다. 판정이 애매하면 RED 다(fail-closed).** 이 절이 아래 모든 절보다 먼저 적용되고, **등급이 아래 절들의 적용 범위를 정한다.** 등급을 보고 첫 줄에 한 단어로 적고, **그 옆에 권장 모델·권장 effort 를 함께 적는다**(원칙 13).
...
🔴 **디자인 축은 "최소 편집" 편향에서 제외한다**(2026-09-06 사용자 지시) — UI·레이아웃·테마·대비·시각 정합을 고칠 때는 토큰을 아끼려고 한 줄씩 국소 패치하지 않는다. **화면 하나가 일관되게 맞을 때까지 관련 스타일·상태(기본·hover·활성·포커스)·색 토큰을 함께** 고친다. 주입 규칙 `oh-my-fable` 의 "surgically edit rather than rewrite" 와 원칙 2 는 **로직 축에만** 적용된다. 🔴 이것은 **요청 범위 안을 끝까지 고치라는 뜻이지 범위를 넓히라는 뜻이 아니다** — 범위 밖 결함은 원칙 14 그대로 보고만 한다.
...
13. **모델 사용** — 판단이 들어가는 일(구현·디버깅·회귀 분석·**코드 리뷰**·설계·삭제 영향 판정)은 세션 주력 모델에서 그대로 한다. 규칙에 구 모델명을 박아 두지 않는다. 위치만 찾는 조회는 메인 세션에서 훑지 말고 `code-locator` 로 보내되, 돌아온 목록은 **재료지 결론이 아니다.**
    - 🔴 **작업을 시작하기 전에 "권장 모델 · 권장 effort" 를 한 줄로 먼저 제안한다** — 실행 계약이 요구하는 등급 한 단어 옆에 붙여 **첫 응답 첫 줄**에 적는다. 형식: `[RED] 권장: 주력 모델 / effort high — 결제 게이팅 축`. 🔴 **제안이 곧 변경은 아니다** — effort 는 내가 `/effort` 로 직접 올리고(축에 들어가기 전에), 모델 교체는 사용자가 `/model` 로 한다. 사용자가 다르게 지시하면 그 지시가 이긴다. 판정 근거가 애매하면 높은 쪽을 제안한다(fail-closed).
    - 권장 조합(🔴 버전명을 박지 않는다 — 티어로만 적는다):
      - **주력 모델 + high** — 결제·이용권·월정석 · 인증/세션 · 프로덕션 DB·마이그레이션 · 배포/릴리스 · Worker 라우팅·R2 · 삭제·리네임 영향 판정(원칙 9) · 회귀 분석(원칙 7) · 코드 리뷰 · 아키텍처 설계 · 재현 안 되는 버그. 즉 **RED 등급은 기본이 여기다.**
      - **주력 모델 + medium** — 일반 기능 구현·다중 파일 국소 수정·테스트 작성·문서 갱신. 🟢 **GREEN 등급 단일 파일 수정도 여기다** — 올리지 않는다.
      - **경량 모델(서브에이전트)** — 판단이 없는 기계적 조회는 메인 세션에서 훑지 말고 `code-locator` 로 내린다. 🔴 `deletion-auditor`·`regression-scout`·`paid-gate-auditor`·`visual-checker` 는 결과물이 **판정**이므로 내리지 않는다.
