...
동시 편집 때문에 수정은 워크트리에서 한다. 예외는 읽기 전용 또는 사용자의 명시적 현재 위치 지시다.
...
변경 파일만 커밋한다. 사용자 승인된 전달 흐름은 PR 검사 통과 후 에이전트가 안전하게 머지하고 스테이징 정상 반영까지 확인한다. 프로덕션 승격은 명시적 1회 승인 때만 진행한다.
