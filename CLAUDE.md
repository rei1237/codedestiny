...
12. 🔴 **컨텍스트가 모자라면 밀어붙이지 말고 인수인계** — 훅 [.claude/hooks/session-context-budget.mjs](.claude/hooks/session-context-budget.mjs) 가 150k/200k/300k 구간에서 담을 항목 8개·템플릿·`/clear` 절차를 통째로 안내한다. 🔴 **그 임계는 실측이라 되돌리지 않는다** — 2026-08-29 측정에서 컨텍스트 300k 초과 요청이 전체의 45%인데 **총소비의 69.7%** 를 먹었고, 반대로 세션 재시작 고정비는 첫 요청 64k = 총소비의 **0.1%** 였다. `/clear` 는 비용이 아니라 이 레포에서 가장 큰 절감 수단이다. 훅이 말해 주지 않는 것 셋만 여기 둔다:
    - 🔴 **판단은 작업을 시작하기 전에** 한다.
    - 🔴 **인수인계는 "프로젝트 설명서"가 아니라 "작업 상태 저장소"다** — 템플릿 [docs/handoff/_TEMPLATE.md](docs/handoff/_TEMPLATE.md) 를 쓰고 **2,000자를 목표**로 한다. 🔴 **머지된 PR 의 과정·브랜치명/SHA 나열·기각된 시도의 서사·코드 전문 복사는 쓰지 않는다** — `git log`·`gh pr list`·코드가 정본이고, 문서는 *현재 상태와 다음 행동*만 담는다. 실측 2026-08-29: 기존 문서 표본에서 그 "완료 회고"가 평균 55%(최악 90%)를 먹고 "남은 작업"은 8%뿐이었으며, 54개 중 **29개는 302개 세션을 통틀어 한 번도 다시 읽히지 않았다.**
...
- 🔴 **의존 없는 확인은 한 메시지에 묶는다** — 왕복 자체가 비용이다. 실측 2026-08-29(트랜스크립트 83세션·요청 14,383건): Bash 11,019콜이 **신규 컨텍스트의 50.0%**인데 API 응답당 도구 호출은 **1.08개(93%가 단일 호출)**였고, 그 콜의 **33%는 결과가 200자 미만**인 잔검사였다. 순서가 필요 없는 `git status`·`grep`·`cat` 은 한 응답에 함께 보내고, 200자짜리 확인은 다음 명령에 합친다.
- 🔴 **CI 대기는 폴링하지 않는다** — `gh pr checks <PR 번호> --watch --fail-fast` 한 콜로 끝낸다. 실측 같은 기간 폴링 364콜(최악 한 세션 55콜)이었고 전부 같은 답을 다시 컨텍스트에 실었다.
- 파일 일부만 필요하면 `sed -n` 대신 Read 의 `offset`/`limit` 을 쓴다(실측 `sed -n` 664콜 = Bash 의 8.5%). 위치만 찾는 조회는 `code-locator` 로 보낸다(원칙 13)
