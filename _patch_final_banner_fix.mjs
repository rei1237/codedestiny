/**
 * 1) 배너 이미지 크롭 제거 (height:auto, 자연 비율 표시)
 * 2) public/index.html 의 구 famousSajuPanel 섹션 완전 삭제
 * 3) 양쪽 파일에서 fsp-calc 관련 CSS 제거
 * 4) public/index.html에서 initFspCalc JS 함수 제거
 */
...
.cd-tap-badge{position:absolute;top:12px;right:12px;padding:4px 11px;border-radius:999px;background:rgba(0,0,0,0.52);backdrop-filter:blur(8px);-webkit-backdrop-filter:blur(8px);border:1px solid rgba(255,255,255,0.14);color:rgba(255,255,255,0.85);font-size:0.66rem;font-weight:600;letter-spacing:0.06em;pointer-events:none;transition:opacity 0.2s;}
.cd-banner-btn[aria-expanded="true"] .cd-tap-badge{opacity:0;pointer-events:none;}
.cd-insight-tile{display:flex;align-items:center;gap:9px;text-decoration:none;padding:10px 13px;border-radius:13px;transition:opacity 0.15s;}
.cd-insight-tile:active{opacity:0.75;}
.cd-banner-inner{position:relative;overflow:hidden;}
.cd-banner-inner img{display:block;width:100%;height:auto;}
.cd-banner-overlay{position:absolute;inset:0;background:linear-gradient(180deg,rgba(10,13,46,0.05) 0%,rgba(10,13,46,0.45) 50%,rgba(10,13,46,0.93) 100%);}
.cd-banner-text{position:absolute;bottom:0;left:0;right:0;padding:14px 16px 16px;display:flex;align-items:flex-end;justify-content:space-between;gap:10px;}
    </style>
    <div style="margin:30px auto 12px;max-width:860px;display:flex;flex-direction:column;gap:14px;padding:0 12px;">

      <!-- ━━━ 카드①: 운세 인사이트 허브 ━━━ -->
      <div style="border-radius:20px;overflow:hidden;border:1px solid rgba(212,175,55,0.34);background:rgba(10,13,46,0.98);box-shadow:0 14px 44px rgba(0,0,0,0.42),0 0 28px rgba(201,168,76,0.11);"
        ...
        <div class="cd-banner-btn" role="button" tabindex="0"
          ...
          <div class="cd-banner-inner">
            ...
            <div class="cd-banner-overlay"></div>
            <div class="cd-tap-badge">▶ 탭하여 열기</div>
            <div class="cd-banner-text" itemprop="description">
              <div>
                ...
                <p style="margin:0;font-size:0.74rem;color:rgba(220,210,255,0.88);text-shadow:0 1px 6px rgba(0,0,0,0.85);">사주·타로·숙요점·베다점·점성술·자미두수 핵심 원리 아카이브</p>
              </div>
      ...
      <!-- ━━━ 카드②: 유명인 사주 분석 아카이브 ━━━ -->
      <div style="border-radius:20px;overflow:hidden;border:1px solid rgba(183,148,244,0.32);background:rgba(10,13,46,0.98);box-shadow:0 14px 44px rgba(0,0,0,0.44),0 0 28px rgba(124,58,237,0.10);"
        ...
        <div class="cd-banner-btn" role="button" tabindex="0"
          ...
          <div class="cd-banner-inner">
            <img src="/fuctionassets/famous.webp"
              ...
              loading="lazy" decoding="async">
            <div class="cd-banner-overlay" style="background:linear-gradient(180deg,rgba(10,13,46,0.04) 0%,rgba(10,13,46,0.42) 48%,rgba(10,13,46,0.92) 100%);"></div>
            <div class="cd-tap-badge" style="border-color:rgba(183,148,244,0.25);color:rgba(220,200,255,0.88);">▶ 탭하여 열기</div>
            <div class="cd-banner-text">
              <div>
                ...
                <p itemprop="description" style="margin:0;font-size:0.73rem;color:rgba(210,200,240,0.88);text-shadow:0 1px 6px rgba(0,0,0,0.85);">역사 위인·K-스타·세계 유명인 50인+의 사주팔자 명리 심층 분석</p>
              </div>
              <span class="cd-chevron" aria-hidden="true">▾</span>
            </div>
          </div>
...
function replaceBetween(html, startMarker, endMarker, replacement) {
  ...
  const eStart = html.indexOf(endMarker);
  if (s === -1 || eStart === -1) return null;
  const e = eStart + endMarker.length;
...
function removeBlock(html, startMarker, endMarker) {
  const s = html.indexOf(startMarker);
  const eStart = html.indexOf(endMarker);
  if (s === -1 || eStart === -1) return html; // 없으면 무시
  const e = eStart + endMarker.length;
  return html.slice(0, s) + html.slice(e);
}

function removeLines(html, ...patterns) {
  const lines = html.split('\n');
  return lines.filter(line => !patterns.some(p => line.includes(p))).join('\n');
}
...
/* ════════ index.html 패치 ════════ */
{
  ...
  // 1) 인사이트 허브 섹션 교체
  const result = replaceBetween(html,
    '    <!-- ═══ 운세 인사이트 허브 ═══ -->',
    '    <!-- ═══ 운세 인사이트 허브 끝 ═══ -->',
    NEW_SECTION
  );
  if (!result) { console.error('❌ index.html: 마커 미발견'); process.exit(1); }
  html = result;

  // 2) 죽은 CSS 제거
  html = removeLines(html,
    '#fsp-input-year:focus,#fsp-input-month:focus,#fsp-input-day:focus',
    '#fsp-calc-btn:hover'
  );

  writeFileSync('index.html', html, 'utf-8');
  console.log(`✅ index.html 패치 완료 (${html.length}자)`);
}

/* ════════ public/index.html 패치 ════════ */
{
  ...
  if (!result) { console.error('❌ public/index.html: 마커 미발견'); process.exit(1); }
  html = result;

  // 2) 구 famousSajuPanel 섹션 완전 삭제
  ...
  html = removeLines(html,
    '#fsp-input-year:focus,#fsp-input-month:focus,#fsp-input-day:focus',
    '#fsp-input-hour:focus',
    '#fsp-calc-btn:hover'
  );

  // 4) initFspCalc JS 함수 블록 제거 (function 선언부터 닫는 } 까지)
  const calcStart = html.indexOf('\nfunction initFspCalc()');
  if (calcStart !== -1) {
    // 함수 끝 '}\n' 찾기 (함수 본문이 한 단계 들여쓰기)
    let depth = 0, idx = calcStart;
    let funcStart = calcStart;
    ...
    html = html.slice(0, funcStart) + html.slice(idx);
    console.log('  - initFspCalc 함수 제거 완료');
  }

  // 5) initFspCalc() 호출 제거
  html = removeLines(html, '  initFspCalc();');

  writeFileSync('public/index.html', html, 'utf-8');
  console.log(`✅ public/index.html 패치 완료 (${html.length}자)`);
}
