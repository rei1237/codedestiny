...
const ANSWER_MAP: PigAnswerEntry[] = [
  {
    keywords: ["남자친구", "남친", "연애", "사귀"],
    answer:
      "꿀꿀~ 있었어요! 네오라는 백사자 오빠인데... 운세 상담사였다가 저한테 '평생 꿀 빨게 해줄게'라고 하더니 갑자기 회사 때려치우고 뭔가 열심히 만들고 있어요 🦁 응원은 하는데 언제 꿀 빨 수 있는 건지 모르겠어요~ 꿀꿀 💛",
  },
  {
    keywords: ["전남친", "헤어", "이별", "전 남자친구"],
    answer:
      "전남친은 아니고 현재진행형이에요! 네오 오빠가 저를 위해 code-destiny.com 만들고 있다고 하더라고요~ 뭔지는 잘 모르겠지만 열심히 하는 거 보면 귀엽긴 해요 🐾 꿀꿀~",
  },
  {
    keywords: ["좋아하는 음식", "좋아하는 거", "뭐 좋아해", "음식"],
    answer:
      "당연히 꿀이죠! 🍯 진짜 꿀에 막 빠져 있어요. 꿀 케이크, 꿀 라떼, 꿀 떡볶이... 뭐든 꿀 들어가면 최고예요. 꿀꿀 그래서 제 이름도 꿀꿀 운세예요~",
  },
  {
    keywords: ["꿈", "장래희망", "되고 싶"],
    answer:
      "운명을 알려주는 꽃돼지로 계속 살고 싶어요! 🌸 사람들이 오늘 하루 조금 더 씩씩하게 살 수 있도록 달콤한 운세를 전해주는 거요~ 네오 오빠랑 같이 code-destiny.com 크게 키우는 것도 꿈이에요 💪 꿀꿀!",
  },
  {
    keywords: ["취미", "좋아하는 것"],
    answer:
      "저는요~ 달빛 아래에서 꽃 구경하기랑 🌙 연이 언니 노래 듣기를 엄청 좋아해요! 가끔 운세 카드도 뒤적이고요. 꿀꿀 최근엔 네오 오빠가 만든 숙요점 공부도 하고 있어요 📚",
  },
  {
    keywords: ["나이", "몇 살", "생일"],
    answer:
      "꽃돼지는 나이를 안 먹어요~ 🌸 꽃처럼 항상 봄이에요! 꿀꿀. 그냥 꽃이 피는 날이 제 생일이에요. 매년 봄마다 생일이니까 엄청 자주 오는 셈이죠 😆",
  },
  {
    keywords: ["연이", "언니"],
    answer:
      "연이 언니요~? 저의 소울 메이트예요! 💜 언니는 엄청 따뜻하고 감성적이에요. 저는 씩씩하고 언니는 부드럽고~ 둘이서 code-destiny.com 사람들 운세 봐주고 있어요. 꿀꿀 최고의 짝꿍~",
  },
  {
    keywords: ["네오", "백사자", "오빠"],
    answer:
      "네오 오빠는요~ 🦁 원래 엄청 유명한 운세 상담사였는데 저한테 '너 하나만 평생 책임지겠다'고 하더니 갑자기 개발자가 된 거예요 꿀꿀. 처음엔 황당했는데... 지금은 응원해요! code-destiny.com 잘 되길 바라요 💛",
  },
  {
    keywords: ["운세", "오늘 운세", "내 운세"],
    answer:
      "꿀꿀~ 운세는 code-destiny.com에서 제대로 봐드려요! 🔮 저는 여기서 귀여운 얘기만 해드릴 수 있고, 진짜 사주 팔자랑 별자리는 저희 홈에서 연이 언니랑 같이 봐줄게요 💜",
  },
  {
    keywords: ["무서워", "걱정", "힘들어", "슬퍼"],
    answer:
      "괜찮아요? 🌸 꽃돼지가 옆에 있을게요! 꿀꿀. 힘든 날일수록 달콤한 거 하나 먹고~ code-destiny.com에서 오늘의 운세 한 번 봐요. 분명 좋은 일이 기다리고 있을 거예요 💛",
  },
  {
    keywords: [],
    answer:
      "꿀꿀~ 그건 저도 잘 모르겠어요 😅 하지만 궁금한 건 code-destiny.com에서 운세로 알아볼 수 있어요! 연이 언니가 잘 알려줄 거예요 💜",
    isDefault: true,
  },
];
...
function getPigAnswer(question: string) {
  ...
  return ANSWER_MAP.find((entry) => entry.isDefault)?.answer || "";
}
...
function HoneyPigQnaPanel() {
  ...
  const [messages, setMessages] = useState<PigChatMessage[]>([
    {
      id: "pig-greeting",
      role: "pig",
      text: "꿀 모드가 열렸어요. 꽃돼지에게 궁금한 걸 물어봐요. 꿀꿀~",
    },
  ]);
  ...
  function submitQuestion(nextQuestion = input) {
    ...
    window.setTimeout(() => {
      const answer = getPigAnswer(question);
      setMessages((current) => [
        ...current.slice(-8),
  ...
  return (
    <section className={styles.honeyPigQnaSection} aria-label="꽃돼지 꿀 모드 대화">
      <div className={styles.honeyPigQnaBadge}>✦ 꿀방울 10개 달성 · 꽃돼지에게 물어봐</div>
      <div className={styles.honeyPigMeter} aria-label="꿀방울 10개 중 10개 달성">
        <span>
      ...
      <div className={styles.honeyPigChatCard}>
        ...
        <div className={styles.honeyPigChatLog} role="log" aria-live="polite">
          ...
          {isTyping ? (
            <div className={styles.honeyPigBubbleRow} data-role="pig">
              <div className={`${styles.honeyPigBubble} ${styles.honeyPigTypingDots}`} aria-label="꽃돼지가 답변을 쓰는 중">
                <span />
                <span />
                <span />
              </div>
        ...
        <div className={styles.honeyPigQuickQuestions} aria-label="추천 질문">
        ...
        <div className={styles.honeyPigInputRow}>
          ...
          />
          <button type="button" aria-label="꽃돼지에게 질문 보내기" disabled={!canSend} onClick={() => submitQuestion()}>
    ...
    <aside className={styles.honeyRewardLayer} data-mode={honeyModeActive ? "active" : "compact"} aria-live="polite">
      <div className={styles.honeyHeaderDock} data-unlocked={unlocked ? "true" : "false"} data-open={showInfo ? "true" : "false"}>
        ...
        >
          ...
          <span className={styles.honeyAlbumText}>달빛 앨범</span>
      ...
      {unlocked ? (
        ...
        >
          {honeyModeActive ? "꿀 모드 닫기" : "꿀 모드 활성화"}
        </button>
      ) : null}
      ...
      {showBurst ? (
        <div className={styles.honeyRewardToast} key={`toast-${burstKey}`} data-unlocked={unlocked ? "true" : "false"}>
          ...
          <div>
            ...
            <p>
              {unlocked
                ? "꽃돼지의 꿀 모드도 함께 열려 있어요."
                : "운명의 찻집에서 상담이 완성될 때마다 조용히 쌓이는 작은 보상이에요."}
            </p>
          </div>
        </div>
      ) : null}
