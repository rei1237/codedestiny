 ...
 * 화면 문구만 담지만 `data/` 상수는 그렇지 않다 — `id: "lotus-moon"` 은 `getTeaHouseCupById`
 * 의 조회 키이고 `particleTone: "pink"` 는 CSS 토큰이라, 사전이 이걸 덮으면 조회가 어긋나거나
 * 스타일이 죽는다. 이름이 걸리면 그 필드는 하위까지 통째로 원문을 유지한다.
 */
...
export type TeaHouseCopyOptions = {
...
export function useTeaHouseCopy<T>(scope: string, ko: T, options?: TeaHouseCopyOptions): T {
  ...
  return useMemo(() => localize(pick, `fortuneTeaHouse.${scope}`, ko, skip), [pick, scope, ko, skip]);
}
