...
function renderStory(story, heading) {
  ...
  const art = safeImage(story.image);
  return `<section class="fortuneStory">${art ? `<img src="${escape(art)}" alt="${escape(story.alt)}" width="960" height="540" loading="lazy" decoding="async">` : ''}<div><${heading}>${escape(story.title)}</${heading}><p>${escape(story.text)}</p></div></section>`;
}
...
export function renderFeatureDetailPanels(detail, { headingLevel = 3, conversionPrompt = false, startPriceLabel } = {}) {
  ...
  const material = ['book', 'letter'].includes(detail.material) ? detail.material : '';
  const portrait = material && hero ? `<figure class="fortuneObject" aria-label="${escape(detail.imageAlt)}"><img src="${escape(hero)}"${srcset ? ` srcset="${srcset}" sizes="(max-width: 699px) 190px, 280px"` : ''} width="${Number(detail.imageWidth) || 900}" height="${Number(detail.imageHeight) || 1200}" alt="" decoding="async" fetchpriority="high"><figcaption>${escape(detail.title)}</figcaption></figure>` : '';
  ...
  return `<article class="featureVisualDetail" data-fortune-theme="${theme}"${material ? ` data-fortune-material="${material}"` : ''}><header data-purchase-stage="awareness" class="featureDetailPanel featureDetailHero">${portrait || (hero ? `<img class="featureDetailArt" src="${escape(hero)}"${srcset ? ` srcset="${srcset}" sizes="(max-width: 699px) 100vw, 720px"` : ''} width="960" height="540" alt="${escape(detail.imageAlt || `${detail.title}의 이야기를 담은 일러스트`)}" loading="eager" decoding="async" fetchpriority="high">` : '')}<div class="featureDetailBody">${material ? '' : `<p class="fortuneCategory">${escape(detail.category || detail.title)}</p>`}<${heading}>${escape(hook)}</${heading}><p>${escape(detail.description)}</p>${detail.edition ? `<p class="fortuneEdition">${escape(detail.edition)}</p>` : ''}<div data-fortune-hero-action>${heroActionHtml || prompt}</div></div></header>${benefits}${questionList}${renderStory(detail.storySections?.[0], heading)}${renderContents(detail, heading)}${renderSample(detail, heading)}${renderFounder(detail.founder, heading)}${method}${renderStory(detail.storySections?.[1], heading)}${trust}${faq}${related}${share}</article>`;
}
