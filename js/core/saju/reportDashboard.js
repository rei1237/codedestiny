...
var REPORT_CARDS = [
  ...
  { id:'relationshipTemptation', thumb:'saju-relationship-temptation-768.webp', label:'사주로 보는 그 사람의 바람끼는?', shortTitle:'그 사람의 바람끼는?', desc:'대상자의 생년 정보로 관계 경계와 외부 자극에 흔들릴 수 있는 경향을 살펴봅니다.', note:'도화 하나로 단정하지 않고 계산된 사주 근거와 현실적인 관계 기준을 함께 읽어드려요.', cta:'관계 성향 확인하기', accent:'#b31955', glow:'rgba(179,25,85,.38)', target:'relationshipBoundaryTestEntry', action:'openRelationshipBoundaryTestRoute', coinCost:0, badge:'NEW' },
...
function _rptIsDirectAction(actionName) {
  return actionName === 'openFortunePlanner' || actionName === 'openLegacyLuckSyncDiary' || actionName === 'openRelationshipBoundaryTestRoute' || actionName === 'openSecretHouseRoute' || actionName === 'openAnimalDestinyRoute' || actionName === 'openDestinyMeetingPlaceRoute' || actionName === 'openLoveSimulationRoute';
}
...
// 사주 결과 카드의 바람끼 테스트는 결과 화면 안의 구형 인라인 카드가 아니라
// 독립 유료 리포트로 진입해야 한다. 기존 target 기반 복구 경로에 맡기면
// relationshipTemptationAnalysis.js 가 다시 렌더되어 새 결제·LLM 흐름을 우회한다.
window.openRelationshipBoundaryTestRoute = function() {
  var target = '/relationship-boundary-test/';
  try {
    window.location.assign(target);
  } catch (e) {
  ...
  try {
    ...
    switch (targetId) {
      ...
      case 'dopamineCard':
        ...
        break;
      case 'relationshipTemptationCard':
        if (typeof window.renderRelationshipTemptation === 'function' && p && natal) window.renderRelationshipTemptation(p, natal, power, johu, window.GENDER || window._gender || '');
        break;
      default:
        break;
    }
