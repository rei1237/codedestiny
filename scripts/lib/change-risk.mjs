...
const deepVerificationRules = [
  ...
  [/^scripts\/lib\/(worker-deploy-base-guard|change-risk|verification-plan|mock-test-config)\.mjs$/i, "배포·검증 가드"],
  [/^scripts\/check-changed\.mjs$/i, "변경 검사 판정기"],
  [/^scripts\/run-mock-tests\.mjs$/i, "mock 테스트 실행 계약"],
  [/^scripts\/(dev-with-(local|live)-auth|mock-dev-api)\.mjs$/i, "개발 서버 mock·실연동 경계"],
  [/^scripts\/lib\/mock-(network-guard\.cjs|dev-settings\.mjs)$/i, "개발·테스트 네트워크 격리"],
  [/^scripts\/fixtures\/mock-dev-responses\.mjs$/i, "mock 응답 계약"],
...
export function selfTest() {
  ...
  const deepCases = [
    ...
    ["scripts/lib/verification-plan.mjs", true],
    ["scripts/lib/mock-test-config.mjs", true],
    ["scripts/run-mock-tests.mjs", true],
    ["scripts/dev-with-local-auth.mjs", true],
    ["scripts/dev-with-live-auth.mjs", true],
    ["scripts/mock-dev-api.mjs", true],
    ["scripts/lib/mock-network-guard.cjs", true],
    ["scripts/lib/mock-dev-settings.mjs", true],
    ["scripts/fixtures/mock-dev-responses.mjs", true],
