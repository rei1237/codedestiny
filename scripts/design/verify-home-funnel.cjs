...
(async()=>{
 ...
 try {
  for(const [width,height] of [[360,640],[390,844],[430,932],[1280,900]]) {
   ...
   results.push({width,height,geometry,freeInput:true,search:true,errors,externalBlocked:[...new Set(blocked)]});
   await page.close();
  }
  for(const route of ['terms-of-service','privacy-policy','refund-policy','contact-us','about','faq']) {
   ...
   results.push({route,status:response.status(),summaryCards:3,anchors:true});
   await page.close();
  }
