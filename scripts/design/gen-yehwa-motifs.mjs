 ...
 */
function renderNav() {
  const sealVar = `  --cd-yehwa-mask-seal: ${toDataUri(MASKS.find((m) => m.name === 'seal'))};`;
  return `/* generated by scripts/design/gen-yehwa-motifs.mjs — do not hand-edit (yehwa-motifs-nav v1, 2026-09-04)
 * 하단 탭바(App Router .cd-mnav / 정적 셸 .cd-mobile-bottom-nav) 활성 탭 인장.
 * styles/yehwa-motifs.css 는 셸 홈에서만 로드되므로 전 라우트에 태울 수 없다 — 탭바가 쓰는
 * 마스크 한 종(seal)만 여기 따로 낸다. 마크업은 건드리지 않는다(::after 는 두 탭바 모두 비어 있다). */
.cd-mnav__link,
...
body:has(main.cd-yeoni-surface) .cd-mnav__link {
  --cd-mnav-seal-ink: color-mix(in srgb, var(--cd-yehwa-line-deep, #a97b3e) 52%, transparent);
}
