...
// An explicit snapshot compares against the previous uncommitted implementation,
// rather than silently using an older Git HEAD as the performance baseline.
const baselineArg=process.argv.find(arg=>arg.startsWith('--baseline-root='));
const baselineRoot=baselineArg ? path.resolve(baselineArg.slice('--baseline-root='.length)) : null;
const baseline=baselineRoot ? fs.readFileSync(path.join(baselineRoot,'index.html')) : execFileSync('git',['show','HEAD:index.html'],{maxBuffer:8e6});
let mode='current';
const server=http.createServer((req,res)=>{
 ...
 if(mode==='baseline'&&/^(js|styles)\//.test(relative)) {
  if(baselineRoot) {
   const snapshot=path.join(baselineRoot,relative);
   // Public-only vendor files have no source counterpart and stay unchanged.
   if(fs.existsSync(snapshot)) data=fs.readFileSync(snapshot);
  }
  else {try{data=execFileSync('git',['show','HEAD:'+relative],{maxBuffer:20e6,stdio:['ignore','pipe','ignore']})}catch{}}
 }
