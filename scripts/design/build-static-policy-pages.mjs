...
const nav = STATIC_POLICY_ROUTES.map((route) => `<a href="${route.canonical}/">${route.label}</a>`).join('')
  + trustDocumentLinks().map(({ href, label }) => `<a href="${href}">${escape(label)}</a>`).join('');
const summary = [];
