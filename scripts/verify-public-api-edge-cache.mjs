...
/**
 * /api/insights 상세만 엣지 캐시한다. 목록은 검색·필터별 키를 정확히 무효화할 수 없어
 * MongoDB 집계로 페이지 단위 조회한다. 상세는 404 를 캐시에 넣지 않아야 한다.
 */
export function auditInsights(source) {
  ...
  const total = countOccurrences(source, CACHE_CALL);
  need(total === 1, `readCmsThroughCache 호출이 ${total}곳이다. 인사이트 목록은 쿼리별 캐시가 아닌 MongoDB 집계여야 하고, 상세만 캐시한다.`);

  let listBody = "";
  try { listBody = sliceFunction(source, "async function handleInsightsList(request, env) {", "handleInsightsList"); } catch { /* 위에서 보고됨 */ }
  if (listBody) {
    need(!listBody.includes(CACHE_CALL), "handleInsightsList: 목록 전체를 엣지 캐시한다 — 쿼리별 무효화 불가 상태가 되살아난다.");
    need(listBody.includes("await connectDb(env)"), "handleInsightsList: MongoDB 연결 없이 집계를 실행한다.");
    need(listBody.includes("$facet"), "handleInsightsList: 페이지·건수·메타데이터를 DB 집계로 계산하지 않는다.");
    need(!listBody.includes(".limit(6000)"), "handleInsightsList: 최대 6,000건을 Worker로 가져오는 경로가 남아 있다.");
    need(listBody.includes('"X-CD-Query"'), "handleInsightsList: Atlas Search/집계 경로 진단 헤더가 없다.");
  }
  ...
  if (detailBody) {
    need(detailBody.includes(CACHE_CALL), "handleInsightDetail 이 엣지 캐시를 지나지 않는다.");
    need(cacheComesBeforeConnect(detailBody), "handleInsightDetail: connectDb 가 캐시 조회보다 앞에 있다.");
  ...
  for (const key of collectCacheKeys(source)) {
    need(key.length > 0, "readCmsThroughCache 호출에 key: 가 없다.");
    need(/key: insightDetailCacheKey\(slug\),?$/.test(key), `인사이트 캐시 키는 insightDetailCacheKey(slug)만 허용한다 — 다른 키는 admin.js의 무효화가 못 지운다. (${key})`);
    for (const token of FORBIDDEN_KEY_TOKENS) {
      need(!key.includes(token), `캐시 키에 '${token}' 이 들어갔다 — 공개 캐시가 아니라 남의 응답을 나눠주는 장치가 된다. (${key})`);
    }
  }
...
function runSelfTest() {
  ...
  const mutations = [
    ...
    [
      ...
      /상세만 캐시한다|엣지 캐시를 지나지 않는다/,
    ],
    [
      "insights: 목록 DB 집계를 제거",
      () => auditInsights(base.insights.split("$facet:").join("facetRemoved:")),
      /DB 집계로 계산하지 않는다/,
    ],
    [
...
function main() {
  ...
  console.log("  ✓ worker/routes/insights.js — 목록은 DB 집계, 상세만 캐시하며 404 는 캐시하지 않는다");
