...
const LOCALE_DICTIONARIES = { en: "en", ja: "ja", zh: "zh-cn" };
 ...
 */
const SIMPLE_TRANS_ELEMENT = /<([a-z][a-z0-9]*)((?:[^<>]*?)\sdata-cd-trans="([^"]+)"(?:[^<>]*?))>([^<]*)<\/\1>/gi;
...
for (const baseDir of ["dist", "out", ".open-next/assets"]) {
  for (const [locale, dictionaryName] of Object.entries(LOCALE_DICTIONARIES)) {
    ...
    const totalKeys = (html.match(/\sdata-cd-trans="/g) || []).length;
    const simpleKeys = [...html.matchAll(SIMPLE_TRANS_ELEMENT)].length;
