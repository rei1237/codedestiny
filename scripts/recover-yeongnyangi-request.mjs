...
if(!['code_destiny_staging','code_destiny'].includes(database)||!/^[a-f0-9]{64}$/.test(id)||!reason||reason.length>300||!Number.isInteger(allowance)||allowance<1||allowance>5){
...
try{
 ...
 if(!row||row.state!=='FORTUNE_FAILED'||row.errorCode!=='GENERATION_REVIEW_REQUIRED'||!row.paymentId)throw Error('Only paid requests held for generation review can be recovered');
 const payment=await Payment.findOne({_id:row.paymentId,userId:row.userId,'metadata.consumedBy':id,status:{$in:['paid','success','fulfilled']}}).select('_id').lean();
 ...
 const increase=Math.max(0,row.attempts-row.snapshot.manifest.length*3-(row.additionalAttempts||0))+allowance;
 const output={database,requestId:id,completedChapters:row.chapters.length,attempts:row.attempts,additionalCallsAllowed:allowance,applied:false};
 if(args.includes('--apply')){
  const result=await YeongnyangiRequest.updateOne({_id:id,state:row.state,errorCode:row.errorCode,attempts:row.attempts,additionalAttempts:row.additionalAttempts||0},
   {$inc:{additionalAttempts:increase},$set:{errorCode:'',state:'PAID'},$push:{recoveryAudit:{at:new Date(),reason,allowance,previousAttempts:row.attempts}}});
