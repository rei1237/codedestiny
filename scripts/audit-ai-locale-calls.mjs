...
const excluded = /^(?:node_modules|\.claude|\.codex|\.cleanup|reports|docs|__tests__|scripts)\//;
const gateways = /^(?:callLLM|callGeminiText|callGeminiJsonWithRetry)$/;
const provider = /generativelanguage\.googleapis\.com|api\.openai\.com|api\.anthropic\.com/;
const entries = [];
for (const file of files) {
  if (excluded.test(file) || !/\.(?:[cm]?js|tsx?|html)$/.test(file)) continue;
  ...
  function visit(node) {
    if (ts.isCallExpression(node)) {
      ...
      if (injected || gateways.test(callee) || /(?:\.AI\.run|chat\.completions\.create|responses\.create|generateText|streamText)$/.test(callee)) {
        matches.push({ line: ast.getLineAndCharacterOfPosition(node.getStart(ast)).line + 1, kind: 'call', symbol: callee });
      }
    }
    if (ts.isImportDeclaration(node) && /(?:gemini|llm-client|structured-consultation)/.test(node.moduleSpecifier.text || '')) {
