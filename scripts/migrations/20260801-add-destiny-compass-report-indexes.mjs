/**
 * MongoDB 마이그레이션 — 운명의 지도 심층 리포트 인덱스
 *
 * 배경:
 *   db.js 가 autoIndex:false 로 연결하므로 models.js 의 스키마 선언만으로는 실제 인덱스가
 *   생성되지 않는다. 이 컬렉션은 두 경로에서 매 요청 조회된다:
 ...
 *
 * 실행:
 *   MONGO_URI="mongodb+srv://..." node scripts/migrations/20260801-add-destiny-compass-report-indexes.mjs [--dry-run|--check]
 */

import { config } from "dotenv";
import { connectDb, mongoose } from "../../worker/lib/db.js";
import { DestinyCompassReport } from "../../worker/lib/models.js";
...
const INDEXES = [
  { spec: { id: 1 }, options: { unique: true, name: "report_id_unique" }, label: "id 재열람 조회(unique)" },
...
async function migrate() {
  ...
  if (CHECK) {
    const existing = await DestinyCompassReport.collection.indexes().catch(() => []);
  ...
  console.log(`\n🧭 인덱스 생성/확인: [destinyCompassReports] ${DRY_RUN ? "(dry-run 건너뜀)" : ""}`);
  for (const index of INDEXES) {
    if (DRY_RUN) {
      console.log(`  ⏭️  ${index.label}`);
      continue;
    }
    await DestinyCompassReport.collection.createIndex(index.spec, index.options);
