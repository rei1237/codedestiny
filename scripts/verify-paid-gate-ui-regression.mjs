...
assertContains(billingClientSource, 'PAID_SERVICE_RUNTIME_SRC = "/js/destiny-profile.js?v=build-d1697646a65e"', "React paid runtime cache key carries the moonstone 409 same-requestId retry");
...
for (const source of [indexSource, staticIndexSource]) {
  ...
  if (source.includes('id="cdHomeFunnel"')) {
    assert(
      source.includes('href="/styles/home-funnel.css') || source.includes('data-cd-home-funnel-critical="1"'),
      "replacement home critical stylesheet",
    );
  }
