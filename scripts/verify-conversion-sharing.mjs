...
const records=JSON.parse(await readFile(new URL('../lib/brand/prediction-records.json',import.meta.url),'utf8'));
...
try{
 for(const width of [390,1280]){
  ...
  try{
   ...
   await f.page.getByRole('heading',{name:'두 대통령의 2025년을 맞힌 명리학자'}).waitFor();
   ...
   assert.deepEqual(await f.page.locator('#founder-timeline a[target="_blank"]').evaluateAll(links=>links.map(a=>a.href)),records.map(r=>r.url));
   assert.equal(await f.page.locator('#founder-timeline blockquote').count(),records.length);
   assert.equal(await f.page.locator('#founder-records a[href="#founder-timeline"]').count(),1);
   assert.equal(await f.page.locator('#founder-records a[href="/yeongnyangi/fortune/?domain=saju&fish=mackerel"]').count(),1);
   const trustImage=await f.page.locator('#founder-records figure img').evaluate(img=>({width:img.getAttribute('width'),height:img.getAttribute('height'),loading:img.loading}));
   assert.deepEqual(trustImage,{width:'960',height:'640',loading:'lazy'});
   const smallTargets=await f.page.locator('#founder-records a').evaluateAll(links=>links.filter(a=>a.getBoundingClientRect().height<44).map(a=>a.textContent));
