...
// 해시 원문이 클라이언트로 넘어오지 않도록 반복수별 집계는 서버(DB)에서 끝낸다.
async function countPbkdf2ByIterations(prefixPattern, iterationIndex) {
  return User.collection.aggregate([
    { $match: { passwordHash: { $regex: prefixPattern } } },
    {
      $project: {
        _id: 0,
        iterations: {
          $convert: {
            input: { $arrayElemAt: [{ $split: ["$passwordHash", "$"] }, iterationIndex] },
            to: "int",
            onError: null,
            onNull: null,
          },
...
const [currentPbkdf2, legacyPbkdf2, totalUsers, bcryptCount, hmacCount, noPasswordCount] = await Promise.all([
  countPbkdf2ByIterations("^pbkdf2-sha256\\$", 1),
  countPbkdf2ByIterations("^pbkdf2\\$sha256\\$", 2),
...
const allPbkdf2 = [...currentPbkdf2, ...legacyPbkdf2];
const pbkdf2Count = allPbkdf2.reduce((sum, row) => sum + row.count, 0);
const belowCurrent = allPbkdf2
  .filter((row) => Number.isInteger(Number(row._id)) && Number(row._id) > 0 && Number(row._id) < PBKDF2_MAX_ITERATIONS)
  .reduce((sum, row) => sum + row.count, 0);
const atCurrent = allPbkdf2
  .filter((row) => Number(row._id) === PBKDF2_MAX_ITERATIONS)
  .reduce((sum, row) => sum + row.count, 0);
const overLimit = allPbkdf2
  .filter((row) => Number(row._id) > PBKDF2_MAX_ITERATIONS)
  .reduce((sum, row) => sum + row.count, 0);
const unparseable = allPbkdf2
  .filter((row) => row._id === null)
  .reduce((sum, row) => sum + row.count, 0);
const currentPrefixCount = currentPbkdf2.reduce((sum, row) => sum + row.count, 0);
const legacyPrefixCount = legacyPbkdf2.reduce((sum, row) => sum + row.count, 0);
const currentPrefixBelow = currentPbkdf2
  .filter((row) => Number.isInteger(Number(row._id)) && Number(row._id) > 0 && Number(row._id) < PBKDF2_MAX_ITERATIONS)
  .reduce((sum, row) => sum + row.count, 0);
const legacyPrefixVerifiable = legacyPbkdf2
  .filter((row) => Number.isInteger(Number(row._id)) && Number(row._id) > 0 && Number(row._id) <= PBKDF2_MAX_ITERATIONS)
  .reduce((sum, row) => sum + row.count, 0);
...
console.log(JSON.stringify({
  totalUsers,
  pbkdf2: pbkdf2Count,
  pbkdf2CurrentPrefix: currentPrefixCount,
  pbkdf2LegacyPrefix: legacyPrefixCount,
  pbkdf2BelowCurrent: belowCurrent,
  pbkdf2AtCurrent: atCurrent,
  ...
  rehashEligibleKnown: bcryptCount + hmacCount + currentPrefixBelow + legacyPrefixVerifiable,
}, null, 2));

console.log("\n반복수 분포(계정 수):");
for (const [label, rows] of [["pbkdf2-sha256", currentPbkdf2], ["pbkdf2$sha256 (legacy)", legacyPbkdf2]]) {
  for (const row of rows) {
    const iterations = row._id === null ? "(파싱 불가)" : String(row._id);
    const flag = Number(row._id) > PBKDF2_MAX_ITERATIONS ? "  ← 워커에서 검증 불가" : "";
    console.log(`  ${label.padEnd(24)} ${iterations.padStart(8)} : ${row.count}${flag}`);
  }
}
