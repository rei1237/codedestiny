...
if (raw.includes('id="cdHomeFunnel"')) {
  ...
  const sceneCss = readFileSync(resolve(root, 'styles/home-character-hero.css'), 'utf8');
  ...
  const sceneLink = raw.indexOf('href="/styles/home-character-hero.css');
  assert(sceneLink > link && sceneLink < raw.indexOf('id="cdHomeFunnel"'), 'Character scene styles must precede first paint');
  assert.match(raw, /class="cdh-character-scene__art"[^>]*width="960" height="640"[^>]*fetchpriority="high"/);
  assert.match(raw, /id="honeypigLogo"[^>]*width="176" height="176"/);
  ...
  assert.match(sceneCss, /\.cdh-character-scene__art\s*\{[^}]*width:\s*100%[^}]*aspect-ratio:\s*3\s*\/\s*2/);
