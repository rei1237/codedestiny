...
import {
  pad2, TZ_MATRIX, buildSamples, cacheYearsFrom, coreTermRows, sampleYears,
...
// 🔴 TZ 매트릭스·표본 생성기는 scripts/lib/ganji-samples.mjs 가 정본이다.
...
function termsFor(year) {
  if (!termCache.has(year)) termCache.set(year, coreTermRows(win.KoreanCalendar, year));
  return termCache.get(year);
}

// ── 표본 — 손으로 날짜를 적지 않는다 ───────────────────────────────────────
//
// 🔴 생성기는 scripts/lib/ganji-samples.mjs 가 정본이다. `measure:ganji-null-transition` 이
// **글자 그대로 같은 표본**을 봐야 그 측정값이 아래 픽스처의 행과 대응한다. 코어와 서비스는
// 주입한다 — 그 모듈은 셸을 로드하지 않는다.

const CACHE_YEARS_INFO = cacheYearsFrom(win.KasiCalendarService);
const CACHE_YEARS_ALL = CACHE_YEARS_INFO.all;
/** 🔴 캐시가 커져도 표본이 폭발하지 않게 상한을 둔다. 버린 해는 **찍는다**(조용한 절단 금지). */
const CACHE_YEARS = CACHE_YEARS_INFO.used;
// 🔴 자식 모드에서는 stdout 이 JSON 봉투 전용이다 — 여기에 한 줄이라도 찍으면 부모의 파싱이 깨진다.
if (!IS_CHILD && CACHE_YEARS_INFO.dropped.length) {
  console.log(
    `[verify:ganji-surface-parity] 검증캐시 해 ${CACHE_YEARS_ALL.length}개 중 ${CACHE_YEARS.length}개만 표본에 넣는다`
    + ` — 쓴 해 ${CACHE_YEARS.join(",")} · 버린 해 ${CACHE_YEARS_INFO.dropped.join(",")}`,
  );
}
/** 표본을 만드는 해 전부. 🔴 6개 프로세스에서 **같은 순서**여야 한다. */
const YEARS = sampleYears(CACHE_YEARS);

const SAMPLES = buildSamples(win.KoreanCalendar, YEARS);
...
// **비동기**라 아직 안 나간 바이트가 그대로 버려진다. 이 봉투는 파이프 버퍼(64KB)를 훌쩍 넘는다
// — 실측 2026-08-31 로 323,629 bytes(표본 1645건)이고 표본이 늘면 같이 커진다. 재현:
// `CD_GANJI_SURFACE_PROBE=1 node scripts/verify-ganji-surface-parity.mjs | wc -c`.
// 그래서 잘린다. Windows 는 파이프 쓰기가 동기라 **로컬에서는 멀쩡하고 CI 에서만** 터졌다
