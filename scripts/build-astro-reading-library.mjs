...
const articles = INSIGHT_SEED_ARTICLES.filter((article) => article.category === '점성술' && article.slug && article.contentHtml);
const files = new Map();
...
if (!index.length || new Set(index.map((article) => article.slug)).size !== index.length) {
  throw new Error('Invalid astrology article catalogue');
}

files.set('index.json', JSON.stringify(index, null, 2) + '\n');
const directory = resolve('public/data/astro-reading');
...
for (const [name, content] of files) {
  ...
  if (process.argv.includes('--check')) {
    if (await readFile(file, 'utf8').catch(() => '') !== content) {
      throw new Error(`Astrology article mirror is stale: ${name}`);
    }
  } else {
...
console.log(`[astro-reading] ${index.length} authored articles ${process.argv.includes('--check') ? 'verified' : 'generated'}`);
