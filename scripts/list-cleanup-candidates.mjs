 ...
 *
 * 🔴 정정(2026-09-01) — 위 문장은 **rg 기준으로만 옳다.** Glob 툴은 `.gitignore` 도 루트
 *    `.ignore` 도 읽지 않아서 워크트리를 그대로 훑는다. 그날 실측: 전 경로 package.json
 *    Glob 조회가 사본 16개를 반환했고 `.claude/worktrees` 아래 파일이 132,255개(메인 추적
 *    4,441개)였다. Glob 은 100건에서 잘리므로 그만큼 진짜 파일이 밀려난다.
 *
 * 🔴 그래서 **이 정리는 Glob 비용을 줄이는 유일한 수단이다.** 같은 날 `.ignore` 에
 *    `/.claude/worktrees/` 를 넣어 봤지만 효과 0이라 되돌렸다(그 파일의 헤더에 실측을
 *    남겨 뒀다). 검색 비용을 줄이려면 여기서 판정해 워크트리를 실제로 지우는 수밖에 없다.
 *
 * 사용:
