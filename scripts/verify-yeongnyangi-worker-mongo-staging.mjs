...
try{
 ...
 // 결제 소켓 레인(withPaymentDb) 관측, 설계안 C2: 같은 픽스처 주문을 두 번 읽어 두 번째가 첫 요청이 연 레인 소켓을 타는지 tail 로 본다. 판정은 PASS 직전이라 뒤 흐름을 끊지 않는다.
 const paymentLaneProbes=[];for(let i=0;i<2;i++)paymentLaneProbes.push((await api(`/api/payments/orders/qa-edge-${run}-0`)).status);
 ...
 for(const item of attendance)assert.equal(item.status,200,JSON.stringify(item.body));assert.equal(attendance.filter(item=>item.body.awarded).length,1);assert.ok(attendance.every(item=>item.body.balance===1));
 const unlocked=await Promise.all([api('/api/yeongnyangi/free/unlock',{}),api('/api/yeongnyangi/free/unlock',{})]);
 for(const item of unlocked)assert.equal(item.status,200,JSON.stringify(item.body));assert.equal(unlocked.filter(item=>item.body.newlyUnlocked).length,1);assert.ok(unlocked.every(item=>item.body.unlocked&&item.body.balance===0));
 const readings=[];
 ...
 const horary=await api('/api/yeongnyangi/free/reading',{category:'horary',profileId,draft:{question:'오늘의 선택을 어떻게 정리할까?'}});assert.equal(horary.status,400);assert.equal(horary.body.code||horary.body.error?.code,'HORARY_FREE_PROMPT_REQUIRED');
 ...
 assert.equal(await YeongnyangiAnchovyAccount.countDocuments({_id:owner,balance:0}),1);assert.equal(await YeongnyangiAnchovyLedger.countDocuments({userId:owner}),2);assert.equal(await YeongnyangiFreeReading.countDocuments({userId:owner}),freeCategories.length);
 assert.deepEqual(paymentLaneProbes,[200,200]);
 report.checks=['payment lane order read x2','login','created read','concurrent activation idempotency','atomic Payment proof consumption','paid restore','guest401','foreign404','amount mismatch402','concurrent attendance once','concurrent daily unlock once',`${freeCategories.length} fixed free readings`,'horary free-prompt only','free reading restore','zero provider calls'];report.status='PASS';writeFileSync('build-cache/yeongnyangi-staging-worker-mongo-qa.json',JSON.stringify(report,null,2));console.log(JSON.stringify(report));
}finally{
