...
const SUITE = [
  ...
  { run: "npm run verify:session-cache-contract", why: "사용자 접근 상태 fetch 캐시가 두 벌(셸 인라인 · React user-session-cache)인데 엔드포인트 표와 TTL 표가 손으로 맞춰져 있다. 한쪽만 고치면 같은 access-store 호출이 런타임에 따라 다른 신선도를 받아 '해금했는데 잠겨 보인다'가 된다 — 화면 증상만 남고 원인이 안 보이는 종류다. 두 표를 파싱해 대조한다(fail-closed: 표를 못 뽑으면 실패)." },
  ...
  { run: "npm run verify:paid-gate-scope", why: "🔴 이 스위트를 **돌릴지 말지** 정하는 판정기 자신의 사전 판정이 무는지. 2026-09-06 실측으로 문구만 고친 PR #1671 이 사전 12벌 때문에 이 게이트를 세 번 깨웠고, 그 필터를 넣으면서 반대 위험(금액·환불 문구 변경을 문구 편집으로 오독해 통째로 스킵)이 생겼다. 스킵은 출력이 없어 사람이 못 알아채므로, 금액·외화·환불 어휘 표본과 구조 변경 줄을 합성 입력으로 직접 찔러 본다." },
  ...
  { run: "npm run verify:love-code-permanent-unlock", why: "러브 코드 영구 해금이 레거시 별칭과 정본 featureKey 사이에서 유실되지 않고, 재방문 시 다시 결제를 요구하지 않는지 확인한다." },
  ...
  { run: "npm run verify:mindscan-reading", why: "🔴 마인드스캔 타로(회당 유료, /mindscan)는 oracle 과 달리 어댑터를 거치지 않고 lib/tarot/mindscan-reading.mjs 안에서 Gemini 를 직접 친다 — 위 oracle 가드 2개가 이 경로를 한 줄도 덮지 않는다. 스크립트 자체는 2026-08 부터 있었으나 package.json 에 배선돼 있지 않아 **신호가 0** 이었다(2026-09-13 배선). LLM 정상 응답 머지·HTTP 500 재시도 상한(3회)·깨진 JSON 폴백·locale 출력 지시문·staging mock 게이트를 fetchImpl 주입으로 본다. 실호출 0회." },
  ...
  { run: "npm run verify:pass-eligibility-copy", why: "서비스 카드·`/points` 의 이용권 커버 배지 문구 12로케일. PR #1644 가 일본어 문구에 한글을 섞은 채(`利用권対象外`) 통과해 스테이징까지 나갔다 — 그때 lib/payment/pass-eligibility.ts 를 여는 테스트·검증기가 0건이었다. 로케일 키 집합을 RUNTIME_LOCALES 에서 읽어 대조하고, ko 밖의 한글과 '한도 없음' 오인 표현을 잡는다. 테이블을 파싱하지 못하면 통과가 아니라 실패다(fail-closed)." },
  ...
  { run: "npm run verify:fusion-fortune-quality", why: "초융합 상담(fusion-fortune-consultation, 50,000원)의 그룹 생성·재시도·결정론 백필·여섯 체계 판정 계약. 🔴 기본 경로가 mock 이다 — 실호출은 --live 뒤에 있고 키도 'verify-only-not-a-real-key' 를 넣는다. CI 는 플래그 없이 부르므로 과금이 발생하지 않는다." },
