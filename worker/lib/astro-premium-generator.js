...
const ASTRO_RISKY_ASSERTION_RE = /(반드시\s*(결혼|이혼|성공|실패|큰돈|수익|파산|이별)|100\s*%|확정|무조건|결혼하면\s*불행|질병을\s*얻게|암에\s*걸|우울증|공황장애|투자\s*수익|수익\s*보장|대박|파산|죽음|사망|직장을\s*잃|큰\s*사고|병에\s*걸|돈을\s*잃|평생\s*외롭|운명.*나쁘|차트.*나쁘)/i;
