...
export function buildEvidenceRuleLines(basisPayload, options = {}) {
  ...
  lines.push(
    ...
    "- 사주·자미두수·숙요·베다·서양 점성술·타로는 서로 다른 체계다. 제공된 체계의 계산값과 용어만 사용하고 다른 체계의 해석을 근거 없이 섞지 않는다.",
    "- 전문용어는 첫 등장에 쉬운 뜻을 붙인다. 한 근거의 강점과 과해질 때의 주의점을 함께 설명하고, 질문에 해당하는 생활 장면으로 연결한다.",
    "- 출생시각 미상·가정값·미계산 항목은 확정값과 구분한다. 실제 계산된 기간 밖의 월운·사건 날짜·상대의 속마음을 만들어내지 않는다.",
...
export function buildDomainAnalysisRuleLines(domains = DEFAULT_ANALYSIS_DOMAINS) {
  ...
  return [
    ...
    "- 분야별로 관련 있는 확정값을 고른다. 같은 근거를 다시 쓸 때는 그 분야의 질문과 행동을 다르게 설명하고, 근거가 부족하면 없는 근거를 채워 넣지 않는다.",
    ...list.flatMap(domain => {
      const rules = [];
      if (/연애|관계|결혼|love|relationship/i.test(domain)) rules.push("감정 표현·관계 속도·갈등 조건을 구분하고 실제 대화 예시를 제시한다. 상대의 마음이나 재회·결혼 성공을 확정하지 않는다.");
      if (/재물|금전|재정|money|wealth|finance/i.test(domain)) rules.push("수입을 만드는 방식·지출 습관·위험을 감수하는 패턴을 구분하고 점검할 행동을 제시한다. 투자 수익이나 횡재를 보장하지 않는다.");
      if (/직업|직장|진로|사업|career|work/i.test(domain)) rules.push("일하는 방식·맞는 환경·협업에서의 강점과 부담을 구분하고 작은 실행 실험을 제시한다. 직업명이나 승진·이직 시점을 단정하지 않는다.");
      return rules.map(rule => `- ${domain}: ${rule}`);
    }),
