import { expertSolarBirthDate, pickExpertFields } from "../expert-evidence.js";
...
export async function buildVedicAdapter(input, options = {}) {
  ...
  const raw = await calculate(
    ...
    {
      birthInfo: {
        birthDate: options.fusionExpert ? expertSolarBirthDate(input) : input.birthDate,
        birthTime: input.birthTime,
        birthTimeUnknown: !input.hasBirthTime,
        calendarType: options.fusionExpert ? "solar" : input.calendarType,
        gender: input.gender,
  ...
  const lagnaSummary = input.hasBirthTime
    ...
    : undefined;
  const dashaSummary = nonEmptyText(dasha.currentMahadasha?.lord || (typeof dasha.currentMahadasha === "string" ? dasha.currentMahadasha : undefined) || dasha.current?.lord || dasha.firstDashaLord);
  ...
  return {
    ...(options.fusionExpert ? { expertEvidence: {
      ...pickExpertFields(raw, ["lagna", "moon", "moonNakshatra", "houses", "transits"]),
      dasha: pickExpertFields(raw.vimshottariDasha, ["currentMahadasha", "currentAntardasha", "periods"]),
    } } : {}),
