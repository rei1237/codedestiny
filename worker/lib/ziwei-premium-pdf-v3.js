...
function hasSectionHeadingEcho(sectionTitle = "", body = "") {
  ...
  const first = firstReportSentence(body);
  if (!first.startsWith(title)) return false;
  const rest = clean(first.slice(title.length));
...
function forbiddenZiweiReportTerms(value = "") {
  const text = String(value || "");
  return [...FORBIDDEN_ZIWEI_REPORT_TERMS, ...FORBIDDEN_ZIWEI_FILLER_SECTIONS]
    .map((term) => clean(term))
    .filter((term, index, list) => term && list.indexOf(term) === index && text.includes(term));
}

function hasForbiddenZiweiReportTerm(value = "") {
  return forbiddenZiweiReportTerms(value).length > 0;
}
...
function countZiweiEvidenceMatches(value = "", configuredTerms = [], actualTerms = []) {
  return Math.max(
    countEvidenceTerms(value, configuredTerms),
    countEvidenceTerms(value, actualTerms),
    countRegexMatches(value, ZIWEI_CORE_TERMS_RE),
  );
}

function hasZiweiEvidenceMatch(value = "", configuredTerms = [], actualTerms = []) {
  return countZiweiEvidenceMatches(value, configuredTerms, actualTerms) >= 1;
}
...
function buildSystemPrompt() {
  return [
    "당신은 최고 수준의 자미두수 고수이자 깊이 있는 상담 문장을 쓰는 해석가입니다.",
    "제공된 자미두수 명반 계산값만 근거로 삼고, 주성·보좌성·살성·궁·사화·대한·유년 정보를 임의로 바꾸거나 지어내지 마세요.",
    "명궁, 신궁, 12궁, 14주성, 보좌성, 살성, 선천 사화, 대한, 유년 흐름을 중심으로 한국어 상담 원고를 작성하세요.",
    ...
    "자미두수 용어를 정확히 사용하세요. 서양 점성술식 표현을 피하고, 궁·주성·보좌성·살성·사화·대한·유년이라고 쓰세요.",
    "각 section은 서로 다른 관점으로 쓰고, 이전 section 문장을 복사하거나 재사용하지 마세요.",
    "조언·주의·전환을 제목처럼 붙인 고정 라벨을 별도 섹션처럼 추가하지 마세요.",
    "기회, 행동, 도움 같은 말을 상투적인 결론으로 반복하지 말고 명반 근거와 현실 장면으로 풀어 쓰세요.",
    "외국어 조각과 깨진 토큰을 절대 출력하지 마세요.",
...
function buildChapterPrompt({ facts, chapterSpec, previousSummary = "" }) {
  ...
  const evidenceFocusLines = asArray(ZIWEI_CHAPTER_EVIDENCE_FOCUS[chapterSpec.id])
    .map((line) => `- ${line}`)
    .join("\n");
  const requiredTermLines = asArray(ZIWEI_CHAPTER_REQUIRED_TERMS[chapterSpec.id])
    .map((line) => clean(line))
    .filter(Boolean)
    .map((line) => `- ${line}`)
    .join("\n");
  const requiredEvidenceCount = requiredChapterEvidenceCount(chapterSpec);
  ...
  return [
    ...
    requiredTermLines ? `필수 명반 용어:\n${requiredTermLines}` : "",
    requiredTermLines ? "필수 명반 용어는 본문 전체에 정확한 표기로 자연스럽게 포함하세요." : "",
    evidenceFocusLines ? `이 장에서 우선 사용할 명반 근거:\n${evidenceFocusLines}` : "",
    evidenceFocusLines ? `이 장의 본문 전체에는 우선 명반 근거 중 최소 ${requiredEvidenceCount}개 이상을 정확한 표기로 포함하세요.` : "",
    sectionEvidenceLines ? `section별 우선 근거:\n${sectionEvidenceLines}` : "",
    sectionEvidenceLines ? "각 section의 첫 문단에는 해당 section별 우선 근거 중 하나 이상을 정확한 표기로 포함하세요." : "",
    categoryQualityLines ? `카테고리별 완성 기준:\n${categoryQualityLines}` : "",
    "검증이나 결과라는 말로 설명하지 말고, 자미두수 해석가가 바로 상담하듯 문장을 여세요.",
    "",
...
function buildRepairPrompt({ facts, chapterSpec, issues, previousSummary }) {
  const requiredTerms = asArray(ZIWEI_CHAPTER_REQUIRED_TERMS[chapterSpec.id])
    .map((line) => clean(line))
    .filter(Boolean)
    .join(", ");
  const sectionEvidenceLines = buildSectionEvidenceHints(chapterSpec);
  return [
    ...
    requiredTerms ? `이번 재작성에서는 다음 필수 명반 용어를 본문에 그대로 넣으세요: ${requiredTerms}` : "",
    sectionEvidenceLines ? `각 section에는 아래 우선 근거 중 하나 이상을 정확한 표기로 반영하세요.\n${sectionEvidenceLines}` : "",
    ...
    "반복 문장, 외국어 토큰, 서양 점성술 표현, 조언·주의·전환 고정 라벨을 모두 피하세요.",
...
export function validateZiweiPremiumChapterHtml(html, chapterSpec, facts = {}) {
  ...
  const sourceForbiddenReportTerms = forbiddenZiweiReportTerms(source);
  if (sourceForbiddenReportTerms.length) issues.push(`html.forbidden-report-term:${sourceForbiddenReportTerms.join(",")}`);
  ...
  if (chapterEvidenceTerms.length && countZiweiEvidenceMatches(chapterBodyText, chapterEvidenceTerms, actualEvidenceTerms) < requiredChapterEvidenceCount(chapterSpec)) {
  ...
  if (countOccurrences(stripTags(source), "새로운") > 14) issues.push("phrase.too-many-new");
  ...
  chapterSpec.sections.forEach((title, index) => {
    ...
    const sectionForbiddenReportTerms = forbiddenZiweiReportTerms(section.body);
    if (sectionForbiddenReportTerms.length) issues.push(`section.report-term.${index + 1}:${sectionForbiddenReportTerms.join(",")}`);
    ...
    const sectionTerms = sectionEvidenceTerms(chapterSpec, index);
    if (sectionTerms.length && !hasZiweiEvidenceMatch(section.body, sectionTerms, actualEvidenceTerms)) {
...
function resolveZiweiPremiumChapterPlan(input = {}) {
  const requestedPlan = asArray(input?.chapterSpecs || input?.ziweiMasterJson?.chapterSpecs || []);
  if (requestedPlan.length === 0) {
    return {
      ok: true,
      source: "default",
      specs: ZIWEI_PREMIUM_CHAPTERS_V3,
      issues: [],
    };
  }

  const expectedPlan = ZIWEI_PREMIUM_CHAPTERS_V3;
  const issues = [];
  ...
  for (let index = 0; index < expectedPlan.length; index += 1) {
    ...
    const rawSections = asArray(chapterSpec.categories).length
      ? asArray(chapterSpec.categories)
      : asArray(chapterSpec.sections);
    const sourceSections = rawSections
      .map((section) => clean(typeof section === "string" ? section : section?.title || section?.name || ""))
      .filter(Boolean);
    const expectedSections = asArray(expected.sections).map((item) => clean(item));
...
function resolveLlmProviders(env = {}) {
  const providers = String(env?.ZIWEI_PREMIUM_LLM_PROVIDERS || "gemini,workers-ai")
    .split(",")
    .map((item) => clean(item))
  ...
  if (clean(env?.ZIWEI_PREMIUM_DISABLE_WORKERS_AI_FALLBACK).toLowerCase() === "true") {
...
function resolveProviderModelName(env = {}, provider = "") {
  ...
  if (provider === "gemini") {
    return clean(env?.ZIWEI_PREMIUM_GEMINI_MODEL || env?.PREMIUM_GEMINI_MODEL || env?.GEMINI_MODEL || "gemini-2.5-flash");
  }
...
async function generateChapter(env, facts, chapterSpec, previousSummary) {
  ...
  for (const provider of providers) {
    ...
    const modelEndpoint = provider === "gemini" ? geminiEndpoint : "";
    ...
    for (let retry = 0; retry <= repairLimit; retry += 1) {
      ...
      if (validation.ok) {
        ...
        const actualModelName = clean(result.modelName || modelName);
...
function validateZiweiFinalReportHtml(html = "", chapters = []) {
  ...
  const sourceForbiddenReportTerms = forbiddenZiweiReportTerms(source);
  if (sourceForbiddenReportTerms.length) issues.push(`final.forbidden-report-term:${sourceForbiddenReportTerms.join(",")}`);
  ...
  for (const [index, chapter] of chapters.entries()) {
    ...
    const spec = ZIWEI_PREMIUM_CHAPTERS_V3[index];
    ...
    if (articleMatches.length === 1 && spec) {
      ...
      if (chapterEvidenceTerms.length && countZiweiEvidenceMatches(articleBodyText, chapterEvidenceTerms) < requiredChapterEvidenceCount(spec)) {
      ...
      articleSections.forEach((section, sectionIndex) => {
        ...
        const sectionTerms = sectionEvidenceTerms(spec, sectionIndex);
        if (sectionTerms.length && !hasZiweiEvidenceMatch(section.body, sectionTerms)) {
...
function buildChapterQuality(chapters = []) {
  ...
  const chapterReports = chapters.map((chapter, index) => {
    ...
    const chapterEvidenceTerms = asArray(ZIWEI_CHAPTER_EVIDENCE_FOCUS[spec.id]).map((item) => clean(item)).filter(Boolean);
    if (chapterEvidenceTerms.length && countZiweiEvidenceMatches(chapterBodyText, chapterEvidenceTerms) < requiredChapterEvidenceCount(spec)) {
...
export async function generateZiweiPremiumReport(env = {}, input = {}, options = {}) {
  ...
  if (ZIWEI_PREMIUM_CHAPTERS_V3.length !== ZIWEI_PDF_CHAPTER_COUNT) {
  ...
  const modelName = modelSet.size === 1
    ? [...modelSet][0]
    : [...modelSet].filter(Boolean).join(",") || resolveProviderModelName(env, "gemini");
  const llmAssembly = {
    enabled: true,
    source: ZIWEI_PDF_CONFIG.generationMode,
    provider,
