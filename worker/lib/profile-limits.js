...
// 구조적으로 없다(resolvePremiumQuotaCycleKey). 한도를 다 써도 남은 기간의
// 등급·만료일·프로필 상한은 유지하고, 추가 유료 콘텐츠만 다른 결제수단으로 인계한다.
/**
 * 이 등급의 월 한도가 정확히 소진됐는가(잔여 0).
...
export function isPassBudgetExhausted(tier, usedCoin, limitCoin) {
  ...
  // 넘기지 않거나 기본 한도보다 작으면 등급 기본값을 쓴다 — 소진을 **앞당기는** 방향으로는 안 움직인다.
  ...
  if (budgetCoin <= 0) return false; // 한도를 못 세는 상태는 소진으로 판정하지 않는다.
  const used = Math.max(0, Math.floor(Number(usedCoin || 0)));
  return used >= budgetCoin;
}

/**
 * 월 한도 소진을 profileSubscription 에 기록하는 감사 필드 집합($set 값 부분).
 * 콘텐츠 예산만 0이 된 것이므로 등급·만료일·프로필 상한은 원래 만료일까지 유지한다.
 */
export function buildPassTerminationFields({ now = new Date(), previousExpiresAt = null } = {}) {
  ...
  return {
    passExhaustedAt: now,
    // CS 문의 때 어느 이용권 주기에서 한도가 소진됐는지 확인하는 감사 정보다.
