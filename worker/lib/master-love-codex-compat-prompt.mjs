...
export function buildMasterLoveCodexCompatChapterPrompt({
  ...
  if (chapter.structured !== false) {
    return [
      ...
      '  "evidence": [{"evidenceId":"근거 목록의 id 그대로","subject":"self 또는 partner 또는 pair","system":"saju 또는 ziwei","period":"기록의 period 그대로","certainty":"기록의 certainty 그대로","label":"실제 근거 용어","explanation":"쉬운 설명"}],',
      '  "crossChecks": [{"id":"근거 목록 crossChecks의 id 그대로","status":"기록의 status 그대로","explanation":"두 체계를 함께 읽은 이유"}],',
      ...
      `  "body": "## 소제목 4~6개로 나눈 이 장의 완성 원고(Markdown, 공백 포함 ${min + 500}~${min + 900}자)"`,
      "}",
