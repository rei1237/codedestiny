...
export async function hasConsumedPassFeature(user, featureKey, requestId, db = nativeDb) {
  const { buildPassConsumeMarker, findPassUsageEvidence } = await loadPassPolicy();
  const marker = buildPassConsumeMarker(featureKey, requestId);
  if (marker && Array.isArray(user?.recentConsumeRequestIds) && user.recentConsumeRequestIds.includes(marker)) return true;
...
export async function consumePassForFeature({ user, entitlement, userId, featureKey, requestId = "", coinCost = 0, db = nativeDb }) {
  ...
  // Consume and durable evidence commit together. A lost response can be recovered
