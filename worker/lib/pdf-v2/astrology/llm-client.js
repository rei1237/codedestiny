...
async function callWorkersAi(params, env) {
  ...
  try {
    ...
    const model = clean(params.model || env.ASTROLOGY_PREMIUM_WORKERS_AI_MODEL || env.WORKERS_AI_MODEL || "@cf/meta/llama-3.1-8b-instruct");
    const result = await withTimeout(env.AI.run(model, {
      ...
      max_tokens: Number(params.maxTokens || env.ASTROLOGY_PREMIUM_CHAPTER_MAX_TOKENS || 12000),
    }), Number(params.timeoutMs || env.ASTROLOGY_PREMIUM_LLM_TIMEOUT_MS || PROVIDER_TIMEOUT_MS));
...
async function callGemini(params, env) {
  ...
  try {
    const result = await generateWithGemini(env, `${params.systemPrompt}\n\n${params.userPrompt}`, {
      modelEnvKeys: ["ASTROLOGY_PREMIUM_GEMINI_MODEL", "GEMINI_MODEL"],
      timeoutMs: Number(params.timeoutMs || env?.ASTROLOGY_PREMIUM_LLM_TIMEOUT_MS || env?.PREMIUM_GEMINI_TIMEOUT_MS || PROVIDER_TIMEOUT_MS),
      maxOutputTokens: Number(params.maxTokens || env?.ASTROLOGY_PREMIUM_GEMINI_MAX_TOKENS || env?.ASTROLOGY_PREMIUM_CHAPTER_MAX_TOKENS || 12000),
      temperature: Number(params.temperature ?? env?.ASTROLOGY_PREMIUM_LLM_TEMPERATURE ?? 0.68),
    ...
    if (result?.ok === false) {
      return {
        ok: false,
        provider: "gemini",
        model: clean(result.model || env?.ASTROLOGY_PREMIUM_GEMINI_MODEL || env?.GEMINI_MODEL),
    ...
    return {
      ok: true,
      text,
      model: clean(result?.model || env?.ASTROLOGY_PREMIUM_GEMINI_MODEL || env?.GEMINI_MODEL || "gemini"),
      provider: "gemini",
      usage: result?.usage,
      latencyMs: Date.now() - started,
    };
