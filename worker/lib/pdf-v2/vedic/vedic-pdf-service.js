...
import { generateVedicPremiumReport } from "./vedic-llm-engine.js";
import { assembleFinalVedicHtml } from "./vedic-html-renderer.js";
import { VEDIC_PREMIUM_PROMPT_VERSION } from "./vedic-prompts.js";
import { validateVedicPdfCompletionPayload } from "./vedic-validator.js";
...
function buildVedicArchiveMetadata({ input, generated, pdfReady, reportId, sessionId }) {
  ...
  return {
    ...
    displayName: "베다점 프리미엄 PDF",
    title: `${clean(input?.userProfile?.name || "사용자")} 베다점 프리미엄 PDF`,
    mode: "personal",
    birthName: clean(input?.userProfile?.name),
    summary: clean(generated?.chapters?.[0]?.summary || generated?.chapters?.[0]?.sections?.[0]?.body || "", 1000),
...
function emitProgress(callback, patch = {}) {
...
export async function generateVedicPremiumPdfV2(params = {}) {
  ...
  try {
    ...
    emitProgress(params.onProgress, { stage: "validating", status: "validating", progress: 5 });
    ...
    const fullHtml = assembleFinalVedicHtml({
