  ...
  try {
    ...
    return await runPaidNarrativeDelivery(request, env, auth, body, {
      ...
      produce: async (task, state) => {
        const ai = await callGeminiText(env, `${state.prompt}\n\n[고정 계산 근거]\n${JSON.stringify(state.facts)}\n[이번 부분: ${task.id}] ${task.title}\nJSON {"evidenceHash":"${state.evidenceHash}","claims":[{"factId":"fact-1","value":"해당 근거의 원래 값"}],"body":"상담 본문"}만 출력하세요. claims에는 실제 사용하는 계산 근거를 하나 이상 정확하게 복사합니다. 본문은 공백 제외 최소 ${task.minChars}자, 목표 2600자입니다. 이 부분에 해당하는 근거와 구체적 생활 사례·반대 조건·행동을 각각 다른 문단으로 쓰고 마지막 문장을 완결합니다. 다른 부분은 쓰지 않습니다.`, {
