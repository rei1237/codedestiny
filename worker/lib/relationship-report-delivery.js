...
export async function generateRelationshipWave(env, meta, checkpoint) {
  ...
  const settled = await Promise.allSettled(missing.map(async id => {
    ...
    const valid = genuine && value?.evidenceHash === meta.evidenceHash && (frame
