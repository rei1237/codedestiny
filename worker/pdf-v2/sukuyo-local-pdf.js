...
function createSukuyoLocalPdfError(message, code, status = 500, details = null) {
...
function assertSukuyoLocalPdfResult(result = {}, config = {}, expectedChapterCount = 15) {
  ...
  const manuscriptSource = clean(result?.manuscriptSource || pdfReady?.manuscriptSource);
  const storedUrl = clean(result?.downloadUrl || result?.pdfUrl || pdfReady?.downloadUrl || pdfReady?.pdfUrl || pdfReady?.htmlUrl);

  const issues = [];
  if (![generationMode, "local"].includes(manuscriptSource)) issues.push("manuscript_source");
...
export async function generateSukuyoLocalPdf(input = {}, options = {}) {
  const buildLocalPdf = options.buildLocalPdf;
  if (typeof buildLocalPdf !== "function") {
    throw createSukuyoLocalPdfError(
      "숙요점 PDF 로컬 생성기가 연결되지 않았습니다.",
      "SUKUYO_LOCAL_BUILDER_MISSING",
      500,
    );
  }
  ...
  try {
    ...
    throw createSukuyoLocalPdfError(
      "숙요점 PDF 로컬 생성에 실패했습니다. 입력 정보와 숙요 계산 결과를 확인해 주세요.",
      "SUKUYO_LOCAL_PDF_GENERATION_FAILED",
  ...
  const localContract = assertSukuyoLocalPdfResult(
    result,
    options.config || {},
  ...
  if (!localContract.ok) {
    throw createSukuyoLocalPdfError(
      "숙요점 PDF 로컬 생성 결과가 완성 조건을 만족하지 못했습니다.",
      "SUKUYO_LOCAL_PDF_CONTRACT_INVALID",
      500,
      localContract,
    );
  }
