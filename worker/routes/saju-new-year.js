...
const LEGACY_NEW_YEAR_CHAPTER_TEMPLATES = Object.freeze([
  { no: 1, title: (year) => `Chapter 1. ${year}년 총운 - 올해의 큰 기류와 운명의 방향`, categories: ["올해 전체 운의 핵심", "세운이 원국에 주는 자극", "기회가 열리는 영역", "조심해야 할 반복 패턴", "올해의 한 문장"] },
  { no: 2, title: (year) => `Chapter 2. 대운과 세운의 교차 - 인생 흐름 속 ${year}년의 위치`, categories: ["현재 대운의 기본 성격", "{YEAR}년 세운과 대운의 충돌/협력", "삶의 방향이 바뀌는 지점", "오래된 문제가 다시 올라오는 이유", "올해 반드시 정리해야 할 것"] },
  { no: 3, title: () => "Chapter 3. 일과 커리어 운 - 성취, 직업, 프로젝트의 흐름", categories: ["직업운의 핵심 흐름", "성과가 나는 방식", "직장에서 조심해야 할 관계와 압박", "창업/서비스/콘텐츠 운", "올해 커리어 전략"] },
  { no: 4, title: () => "Chapter 4. 재물운 - 돈이 들어오고 나가는 구조", categories: ["올해 재물운의 성격", "수입이 늘어나는 조건", "지출과 손실을 부르는 패턴", "투자/사업/부업 판단 기준", "돈을 지키는 실전 전략"] },
  { no: 5, title: () => "Chapter 5. 연애와 인연운 - 사랑, 재회, 새로운 인연의 가능성", categories: ["올해의 연애 기류", "과거 인연과 재회 가능성", "새로운 인연이 들어오는 방식", "집착과 진심을 구분해야 하는 순간", "사랑을 현실로 만들기 위한 태도"] },
  { no: 6, title: () => "Chapter 6. 인간관계와 귀인운 - 도움, 갈등, 협력의 사람들", categories: ["올해 사람 복의 흐름", "귀인이 나타나는 방식", "피해야 할 사람과 관계 패턴", "오해와 갈등을 줄이는 방법", "관계를 운으로 바꾸는 법"] },
  { no: 7, title: () => "Chapter 7. 건강과 멘탈운 - 몸과 마음의 균형", categories: ["올해 몸의 긴장 포인트", "마음이 흔들리는 구간", "과로와 번아웃 신호", "회복력을 높이는 생활 전략", "운을 지키는 몸 관리법"] },
  { no: 8, title: () => "Chapter 8. 월별 운세 - 1월부터 12월까지의 흐름", categories: ["상반기 흐름", "하반기 흐름", "기회가 강한 달", "조심해야 할 달", "월별 실전 행동표"] },
  { no: 9, title: () => "Chapter 9. 위기와 반전 - 올해 반드시 넘어서야 할 시험", categories: ["올해 가장 큰 위험 신호", "반복되는 실패 패턴", "무너지는 순간의 원인", "반전이 생기는 조건", "위기를 기회로 바꾸는 선택"] },
  { no: 10, title: (year) => `Chapter 10. ${year}년 마스터플랜 - 올해를 성공으로 끝내는 전략`, categories: ["올해 가장 중요한 목표", "3개월 실행 전략", "6개월 실행 전략", "12개월 완성 전략", "최종 조언과 선언문"] },
]);

function buildSajuNewYearChapterSpecs(targetYear) {
  const year = toInt(targetYear, resolveDefaultTargetYear());
  return LEGACY_NEW_YEAR_CHAPTER_TEMPLATES.map((chapter) => ({
    no: chapter.no,
    id: String(chapter.no),
    title: chapter.title(year),
    categories: chapter.categories.map((category) => category.replace("{YEAR}", String(year))),
  }));
}
...
function validateSajuNewYearPdfLLMInterpretationQuality({ chapters = [], expectedChapters = buildSajuNewYearChapterSpecs(resolveDefaultTargetYear()), minChapterLength = 3000, minSectionLength = 600 } = {}) {
  ...
  expectedChapters.forEach((spec, chapterIndex) => {
    ...
    if (spec.no === 10 && !/(3개월|6개월|12개월)/.test(chapter.text || "")) errors.push("chapter_10_plan_missing");
