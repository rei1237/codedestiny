...
async function fetchKasiUpstream(env, method, params) {
  ...
  const timeoutMs = Math.max(1000, Number(env.KASI_PROXY_TIMEOUT_MS || 2500));
      ...
      for (let attempt = 0; attempt <= MAX_RETRIES; attempt += 1) {
        ...
        try {
          ...
          if (error?.name === "AbortError") {
            lastError = createHttpError(503, "KASI API 타임아웃", {
              code: "KASI_TIMEOUT",
              upstreamBase: kasiBaseUrl,
            });
          } else {
          ...
          const upstreamStatus = Number(
            lastError?.payload?.upstreamStatus
...
export async function handleKasiRoutes(request, env = {}) {
  try {
    ...
    const emergencyInput = {
      year: toInt(new Date().getUTCFullYear(), new Date().getUTCFullYear()),
      month: toInt(new Date().getUTCMonth() + 1, 1),
      day: toInt(new Date().getUTCDate(), 1),
      hour: 12,
      minute: 0,
      calendarType: "solar",
    };
