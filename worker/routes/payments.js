...
/* 월 가격 정본은 lib/payment/pass-pricing.js 하나다(숫자를 다시 적으면 prepare/confirm 이 갈려 400).
   🔴 호출부는 없다(구 /subscription/prepare 와 함께 2026-09-06 에 지웠다). 그래도 남기는 이유는
   scripts/verify-billing-pass-policy.mjs:837-843 이 이 파일 소스에서 profileLimit/maxCoveredCoin 이
   HONEY_PASS_POLICY 를 참조하는지, 그리고 회차 카운트 필드가 없는지를 단언하기 때문이다.
   그 가드를 V2(worker/payments/passes.js)로 옮겨 겨눈 다음에 함께 지운다. */
...
async function upsertSinglePaymentUnlockRecord({ payment, paidAt }) {
  ...
  // resolveProfileUnlockContentKey 하나다(코인 지급 경로 billing.js isProfileScopedUnlockKey 도
  // 같은 판정을 쓴다).
  //
...
export async function handlePaymentRoutes(request, env, ctx) {
  ...
  try {
    ...
    /* /prepare · /subscription/prepare · /subscription/confirm · /confirm 은 worker/index.js 가 V2
       (worker/payments/index.js)로 가로채므로 여기까지 오지 않는다 — 네 경로의 구 핸들러는 2026-09-06 에
       전부 지웠다. 🔴 여기에 다시 등록하지 말 것: 훅이 폴스루보다 앞이라는 계약은
       __tests__/worker/router.legacy-payment-alias.static.test.js 가 고정한다. */
