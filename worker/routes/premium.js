...
const ASTRO_PERSONAL_CHAPTER_META = [
  { key: "C1", num: 1, mode: "personal", title: "I. 출생 차트 총론 — 나의 우주적 설계도", subtitle: "차트 핵심 기질과 인생 테마 통합", icon: "star" },
  { key: "C2", num: 2, mode: "personal", title: "II. 빅3 해석 — 태양·달·상승궁", subtitle: "내면·외면 핵심축 정밀 해석", icon: "star" },
  { key: "C3", num: 3, mode: "personal", title: "III. 10행성 완전 해석", subtitle: "행성별 동기와 현실 작동 패턴", icon: "star" },
  { key: "C4", num: 4, mode: "personal", title: "IV. 12하우스 분석", subtitle: "삶의 영역별 운명 지도", icon: "star" },
  { key: "C5", num: 5, mode: "personal", title: "V. 원소와 양식 분석", subtitle: "기질 균형·과부족·대응 전략", icon: "star" },
  { key: "C6", num: 6, mode: "personal", title: "VI. 어스펙트 분석", subtitle: "긴장·재능·반복 패턴 진단", icon: "star" },
  { key: "C7", num: 7, mode: "personal", title: "VII. 사랑과 관계운", subtitle: "연애·결혼·매력 구조 해석", icon: "star" },
  { key: "C8", num: 8, mode: "personal", title: "VIII. 직업·재물·성공운", subtitle: "사회 성취와 수익화 전략", icon: "star" },
  { key: "C9", num: 9, mode: "personal", title: "IX. 심리·무의식·상처 분석", subtitle: "내면 그림자와 회복 루틴", icon: "star" },
...
const VEDIC_TOTAL_CHAPTERS = VEDIC_PERSONAL_TOTAL_CHAPTERS;
...
function getVedicChapterMetaList(reportType = "personal") {
  return VEDIC_PERSONAL_CHAPTER_META;
}

function getVedicChapterGuideList(reportType = "personal") {
  return VEDIC_PERSONAL_CHAPTER_GUIDES;
}
...
function findVedicChapterMetaByKey(chapterKey) {
  ...
  if (!key) return null;
  return VEDIC_PERSONAL_CHAPTER_META.find((meta) => String(meta?.key || "") === key) || null;
}
...
function resolveVedicChapterMeta(chapterId, modeHint = "personal", chapterKey = "") {
  ...
  const source = VEDIC_PERSONAL_CHAPTER_META;
...
function normalizeChapterCategoryLabel(value) {
...
function toChapterSpecificSections(values = []) {
  if (!Array.isArray(values)) return [];
  const seen = new Set();
  const normalized = [];
  for (const value of values) {
    const label = normalizeChapterCategoryLabel(value);
    if (!label) continue;
...
async function generateGuaranteedPremiumChapter(env, {
  ...
  let generated = "";
  try {
    generated = await callGemini(env, prompt, ["PREMIUM_GEMINI_MODEL"], {
      temperature: 0.72,
      topP: 0.92,
    ...
    console.warn("[PremiumPDF] GuaranteedRecoveryGeminiFailed", {
      requestId,
      reportType: context?.reportType || "",
  ...
  let fallbackReason = failureCode
    ? `GUARANTEED_RECOVERY:${String(failureCode).slice(0, 80)}`
    : "GUARANTEED_RECOVERY";
  ...
  if (!text || countKoreanLikeChars(text) < minChars || !hasAllHeadings) {
    ...
    usedFallback = true;
    fallbackReason = "GUARANTEED_LOCAL_SKELETON";
  }
...
function normalizePremiumRequestBodyForPipeline(reportType, sourceInput = {}) {
  ...
  if (reportType === "westernAstrologyPremium") {
    ...
    normalized.reportMode = "personal";
    normalized.reportType = "personal";
    normalized.includeCompatibility = false;
    normalized._compatibilityRequired = false;
...
function validatePremiumChapterResponseEnvelope({ reportType, chapterId, data, chapterContract, previousChapterTexts = [] }) {
  ...
  if (reportType === "ziweiPremium") {
    ...
    const targetPalace = String(contract?.targetPalace || "").replace(/\(.*?\)/g, "").trim();
    const shouldCheckTargetPalace = Boolean(targetPalace) && /궁/.test(targetPalace);
    if (shouldCheckTargetPalace) {
      const chapterText = [
        text,
        String(chapterJson?.summary || ""),
        String(chapterJson?.masterAdvice || chapterJson?.masterConclusion || ""),
      ].join("\n");
      if (!chapterText.includes(targetPalace)) {
        missingJsonFields.push("targetPalaceFocus");
      }
    }
  }
...
function findAstroChapterMetaByKey(chapterKey) {
  const key = String(chapterKey || "").trim();
  if (!key) return null;
  return ASTRO_PERSONAL_CHAPTER_META.find((meta) => String(meta?.key || "") === key) || null;
}
...
function resolveAstroChapterMeta(chapterId, modeHint = "personal", chapterKey = "") {
  ...
  if (fromKey) return fromKey;
  const source = ASTRO_PERSONAL_CHAPTER_META;
  if (!Array.isArray(source) || source.length === 0) return null;
  const index = clampInt(chapterId, 1, 1, source.length) - 1;
  return source[index] || null;
}
...
function buildAstroChapterPrompt(chapterMeta, canonical, previousTexts = [], premiumInput = null) {
  ...
  const premiumChapterJsonPacks = premiumInput && typeof premiumInput === "object"
    ? compactChapterJsonPacksForPrompt(premiumInput.chapterJsonPacks)
    : {};
  ...
  const requiredHeadings = Array.isArray(chapterContract?.requiredHeadings)
    ...
    : buildAstroRequiredHeadings(chapterMeta, "personal");
  ...
  const lengthPolicy = getAstroChapterLengthPolicy(chapterMeta?.num || chapterMeta?.chapter || 1, "personal");
  const summary = {
  ...
  const coverageRule = "각 챕터는 실제 별자리/하우스/애스펙트 근거를 충분히 반영해 독립적인 관점으로 완성한다.";
  ...
  return [
    ...
    `premiumChapterJsonPacks: ${stringifyCompact(premiumChapterJsonPacks, 2200)}`,
...
function astroMissingMarkers(text, chapterMeta) {
  ...
  const chapterKey = String(chapterMeta?.key || "").trim();
  const required = buildAstroRequiredHeadings(chapterMeta, "personal").concat(["| 항목 | 값 |"]);
  if (chapterKey === "C11") {
    required.push("3개월");
    required.push("6개월");
    required.push("12개월");
...
const PREMIUM_REPORT_RECOVERABLE_CODES = new Set([
  "PREMIUM_REPORT_PAYLOAD_MISSING_FIELD",
  "PDF_REPORT_PAYLOAD_MISSING_FIELD",
  "PREMIUM_REPORT_PREFLIGHT_FAILED",
  "SAJU_REPORT_PAYLOAD_INCOMPLETE",
  "SUKYO_REPORT_PAYLOAD_INCOMPLETE",
  "VEDIC_REPORT_PAYLOAD_INCOMPLETE",
  "ASTRO_REPORT_PAYLOAD_INCOMPLETE",
  "ZIWEI_REPORT_PAYLOAD_INCOMPLETE",
  "PREMIUM_REPORT_CHAPTER_FAILED",
]);
...
function isRecoverablePremiumChapterFailure(errorCode, errorMessage, statusCode) {
  const code = String(errorCode || "").trim();
  if (code && PREMIUM_REPORT_RECOVERABLE_CODES.has(code)) return true;
  const status = Number(statusCode || 0);
  if (Number.isFinite(status) && status >= 500) return true;
  return isLlmRecoverableError(`${code} ${String(errorMessage || "")}`);
}
...
function buildVedicChapterPlan(canonicalVedicChart, reportType) {
  ...
  return metaList.map((meta) => {
    const targetChars = Number(VEDIC_SOLO_TARGET_CHARS[meta.num - 1] || 4200);
...
function buildVedicPremiumPrompt(meta, chapter, reportType, context, previousChapterTexts = []) {
  ...
  const minChars = Math.max(3200, Math.floor(targetChars * 0.85));
  const reportTitle = VEDIC_REPORT_TITLE_PERSONAL;
  const reportSubtitle = VEDIC_REPORT_SUBTITLE_PERSONAL;
...
function buildVedicLocalFallbackChapter(chapter, meta, canonicalVedicChart, reportType) {
  ...
  allLabels.forEach((label, idx) => {
    ...
    const dataRef = `${lagnaL}과 ${nakshatraL}`;

    const dashaRef = `${dashaL}${anterDasha ? ` / ${antardashL}` : ""}`;
    ...
    parts.push(
      ...
      `${dataRef}을 기준으로 보면, 이 영역에서 당신이 가진 핵심 에너지 패턴은 삶의 여러 맥락에서 반복적으로 작동하는 구조적 특성을 지니고 있습니다. ` +
...
function buildZiweiDataContext(body, input, canonicalZiweiChart, reportType, partnerOverview, dataQuality, premiumInput = null) {
  ...
  const premiumJsonText = premiumInput && typeof premiumInput === "object"
    ? stringifyCompact(compactChapterJsonPacksForPrompt(premiumInput.chapterJsonPacks || premiumInput), 2200)
    : "";
...
async function handleSukuyoLife(request, env, authInfo = null) {
  ...
  if (reportType === "personal") {
    const chapterMetaList = getSukyoPdfChapters("personal").map((chapter, idx) => ({
      ...
      title: chapter.title,
      subtitle: chapter.goal,
      chapterSpecificSections: toChapterSpecificSections(chapter.sections || []),
    }));
  ...
  const chapterMetaList = getSukyoPdfChapters("compatibility").map((chapterDef, idx) => ({
    ...
    chapterSpecificSections: toChapterSpecificSections(buildSukuyoCompatRequiredHeadings({ key: chapterDef?.key || chapterDef?.id || `C${idx + 1}` })),
  }));
  const totalChapters = chapterMetaList.length;
...
async function handleAstroLife(request, env, authInfo = null) {
  ...
  if (prepareOnly) {
    const normalizedChapterPlan = chapterPlan.map((planRow) => ({
      ...planRow,
      chapterSpecificSections: toChapterSpecificSections(
        buildAstroRequiredHeadings(planRow, "personal"),
      ),
    }));
    return json({
      ok: true,
      ...
      reportType,
      totalChapters: chapterPlan.length,
      chapterPlan: normalizedChapterPlan,
      canonicalAstroChart,
      chart,
      partnerChart,
      synastry,
  ...
  const responsePayload = {
    ...
    chapterMeta: meta,
    chapterSpecificSections: toChapterSpecificSections(
      buildAstroRequiredHeadings(meta, "personal"),
    ),
    chart,
    partnerChart,
    synastry,
  ...
  responsePayload.storage = writeReportSessionChapter(
    ...
    {
      reportType,
      chapterSpecificSections: responsePayload.chapterSpecificSections,
    }
  );
...
async function handleVedicLife(request, env, authInfo = null) {
  ...
  if (prepareOnly) {
    const normalizedChapterPlan = chapterPlan.map((planRow) => ({
      ...planRow,
      chapterSpecificSections: toChapterSpecificSections(
        buildVedicRequiredHeadings(planRow, reportType),
      ),
    }));
    return json({
      ok: true,
...
function compactPromptValue(value, depth = 0, maxDepth = 3, maxItems = 8) {
  ...
  if (Array.isArray(value)) {
    return value
      .slice(0, maxItems)
      .map((item) => compactPromptValue(item, depth + 1, maxDepth, maxItems));
  }

  if (typeof value === "object") {
    const out = {};
    for (const [key, val] of Object.entries(value).slice(0, maxItems)) {
      out[key] = compactPromptValue(val, depth + 1, maxDepth, maxItems);
    }
    return out;
  }
...
function compactChapterJsonPacksForPrompt(raw) {
  const input = toPlainObject(raw);
  if (!input || typeof input !== "object") return null;
  return {
    chapterCore: compactPromptValue(input.chapterCore || null, 0, 3, 10),
    core: compactPromptValue(input.core || null, 0, 3, 10),
    signals: compactPromptValue(input.signals || null, 0, 3, 10),
    timing: compactPromptValue(input.timing || null, 0, 3, 10),
    actions: compactPromptValue(input.actions || null, 0, 3, 10),
  };
}
...
function buildSessionPrompt(kind, title, chapter, totalChapters, body, sectionHeaders, options = {}) {
  ...
  const premiumChapterJsonPacks = options?.premiumChapterJsonPacks && typeof options.premiumChapterJsonPacks === "object"
    ? compactChapterJsonPacksForPrompt(options.premiumChapterJsonPacks)
    : null;
  ...
  return [
    ...
    premiumChapterJsonPacks ? stringifyCompact(premiumChapterJsonPacks, 2200) : null,
    "",
...
async function handleSajuNewYearSession(request, env, authInfo = null) {
  ...
  if (prepareOnly) {
    ...
    const chapterPlan = SAJU_NEW_YEAR_CHAPTERS.map((meta, idx) => ({
      ...meta,
  ...
  const chapterSpecificSections = toChapterSpecificSections(getSajuNewYearChapterPromptSpec(chapter)?.markers || []);
...
async function handleLifebookSession(request, env, authInfo = null) {
  ...
  if (prepareOnly) {
    ...
    const chapterPlan = buildLifeBookChapterPlan().map((plan, idx) => ({
      ...plan,
  ...
  if (fullGenerateRequested && !chapterRequestProvided(strictBody)) {
    ...
    generatedChapters.forEach((chapterResult, idx) => {
      const num = idx + 1;
      const chapterSpecificSections = toChapterSpecificSections(
        chapterResult?.chapterJson?.sections?.map((section) => section?.heading)
          || getLifeBookChapterByNumber(num)?.sections
          || [],
      );
    ...
    return json({
      ...
      totalChapters: LIFE_BOOK_TOTAL_CHAPTERS,
      minTotalChars: LIFE_BOOK_MIN_TOTAL_CHARS_CONFIG,
      chapters: generatedChapters.map((chapterResult, idx) => ({
        ...chapterResult,
        chapterSpecificSections: toChapterSpecificSections(
          chapterResult?.chapterJson?.sections?.map((section) => section?.heading)
            || getLifeBookChapterByNumber(idx + 1)?.sections
            || [],
        ),
      })),
      pdfData: allGenerated.pdfData || null,
  ...
  const chapterSpecificSections = toChapterSpecificSections(
    chapterResult?.chapterJson?.sections?.map((section) => section?.heading)
      || getLifeBookChapterByNumber(chapter)?.sections
      || [],
  );
...
async function handleLoveSecretSession(request, env, authInfo = null) {
  ...
  if (prepareOnly) {
    const chapterPlan = (modeConfig.chapters || []).map((meta, idx) => {
      ...
      return {
        num: chapterNo,
        title: meta?.title || `Chapter ${chapterNo}`,
        subtitle: meta?.subtitle || "",
        chapterSpecificSections: toChapterSpecificSections(chapterPlanning?.dataDrivenSections || requiredSections),
      };
    });
    return json({
      ok: true,
...
async function handleZiweiBookSession(request, env, authInfo = null) {
  ...
  if (prepareOnly) {
    const chapterPlan = ZIWEI_CHAPTER_META.map((metaRow, idx) => ({
      ...metaRow,
      chapterSpecificSections: toChapterSpecificSections((ZIWEI_PDF_CHAPTERS_V2[idx] || {}).sections || []),
    }));
    return json({
      ok: true,
      prepared: true,
  ...
  const storage = writeReportSessionChapter(
    ...
    safeGeneratedText,
    {
      ...
      chapterSpecificSections: toChapterSpecificSections((ZIWEI_PDF_CHAPTERS_V2[chapter - 1] || {}).sections || []),
      reportType,
      requestId,
      generationId,
      canonicalSummary,
...
async function handlePremiumReportChapter(request, env, authInfo) {
  ...
  if (!context.isCompleteForPdf) {
    ...
    if (!context.isCompleteForPdf) {
      ...
      context.warnings = Array.from(new Set([
        ...
        "PREMIUM_REPORT_FAIL_OPEN_INCOMPLETE_CANONICAL",
        ...missingFields.map((field) => `MISSING:${field}`),
      ]));
      console.warn("[PremiumPDF] FailOpenIncompleteCanonical", {
        requestId,
        reportType: context?.reportType,
        reportSessionId,
        chapterId,
  ...
  if (!successResponse || !successData) {
    ...
    try {
      const guaranteed = await generateGuaranteedPremiumChapter(env, {
        ...
        failureMessage: String(lastFailure.message || "챕터 생성 실패"),
      });
      if (guaranteed?.ok) {
        const guaranteedText = ensurePremiumChapterLength(String(guaranteed.text || "").trim(), {
          reportType: context.reportType,
          chapter: chapterId,
          totalChapters: Number(context.totalChapters || context.requiredChapters || 13),
          chapterMeta: guaranteed?.chapterMeta || resolvePremiumChapterMeta(context, chapterId),
          minTotalChars: PREMIUM_GLOBAL_MIN_TOTAL_CHARS,
          engineData: chapterJsonPacks,
        });
        const guaranteedLengthCheck = validateChapterLength({
          reportType: context.reportType,
          featureType: context.featureType,
          mode: context.modeKey,
          chapterId,
        ...
        if (!guaranteedLengthCheck.ok) {
          guaranteedLengthCheck.ok = true;
          guaranteedLengthCheck.warnings = Array.from(new Set([...(guaranteedLengthCheck.warnings || []), "RECOVERABLE_LENGTH_SHORT"]));
        }
        successResponse = { status: 200 };
        successData = {
          ...guaranteed,
        ...
        successLengthCheck = guaranteedLengthCheck;
        successAttempt = maxChapterAttempts + 1;
      }
    } catch (guaranteedError) {
      console.warn("[PremiumPDF] GuaranteedRecoveryFailed", {
        requestId,
        reportType: context?.reportType,
        chapterId,
        errorCode: String(lastFailure?.code || "PREMIUM_REPORT_CHAPTER_FAILED"),
        error: String(guaranteedError?.message || guaranteedError || "unknown"),
      });
    }
  }
  ...
  return json(attachPremiumApiMeta({
    ...
    ...successData,
    usedFallback: Boolean(successData?.usedFallback),
    fallbackReason: String(successData?.fallbackReason || ""),
    text: chapterText,
