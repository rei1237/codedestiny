...
async function verifyPaymentForStart({ env, auth, paymentId, idempotencyKey, inputHash, pricing }) {
  ...
  // 🔴 확정된 주문도 verifyPgPayment 를 통과한 흔적(rawPortOne.channelCheck — 이 검증기와 V2 확정이 남기며 "absent" 도 포함)이 있을 때만 재조회를 건너뛴다 — 구 확정 경로(레거시 정산)가 닫은 주문은 흔적이 없어 매번 재조회하고, L1 이전 주문은 채널 대조도 없었다(2026-09-24).
  ...
  let verified;
  // 🔴 실패를 삼키면 카드 승인 후 원인 추적이 불가능해진다(2026-07 PortOne 401 장애). 동작은 그대로 두고 조회 실패 사유만 남긴다. 대조는 V2 확정 경로와 같은 verifyPgPayment 한 벌.
  try { verified = await verifyPgPayment(env, { orderId: normalizedPaymentId, expectedAmountKRW: alreadyPaid ? (Number(order.paymentAmount) || pricing.amountKRW) : pricing.amountKRW }); } catch (error) {
    ...
    console.error("[ziwei-island-ai] PortOne payment verify failed", normalizedPaymentId, lookupFailed ? "portone_fetch_failed" : String(error.code).toLowerCase(), error?.message || error);
    // 🔴 대조 불일치는 로그만 — 대기 주문의 부분취소 검토 표시를 덮지 않게. 조회 실패만 대기 주문에 CAS 로 쓴다(취소·환불 주문의 표시도 지킨다, 2026-09-24 감사).
    if (lookupFailed) {
