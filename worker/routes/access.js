...
async function handleStatus(request, env) {
  ...
  await connectDb(env);
  // ?ºÏãú???Ä Ï¥àÍ∏∞?îÏóê???¥Í∏à ?ÅÌÉúÎ•??ïÌôï??Î∞òÌôò?òÎèÑÎ°?Ï°∞ÌöåÎ•??¨Ïãú?ÑÎ°ú Í∞êÏãº??handleUnlocks?Ä ?ôÏùº ?®ÌÑ¥).
...
async function handleUnlocks(request, env) {
  ...
  // ?ºÏãú???Ä Ï¥àÍ∏∞?îÏóê???¥Í∏à ?ÅÌÉúÎ•??ïÌôï??Î∞òÌôò?òÎèÑÎ°?Ï°∞ÌöåÎ•??¨Ïãú?ÑÎ°ú Í∞êÏãº??
  // (verifyProfileOwnership??404 ??Îπ??ºÏãú???êÎü¨???¨Ïãú???ÜÏù¥ Ï¶âÏãú ?ÑÌåå?úÎã§.)
  ...
  try {
    ...
    if (request.method === "GET" && (isTransientMongoError(error) || isAuthDbInfraError(error))) {
      return json({
        ...
        message: "?ºÏãú?ÅÏù∏ ?∞Í≤∞ Î¨∏Ï†úÍ∞Ä ?àÏñ¥?? ?†Ïãú ???§Ïãú ?úÎèÑ??Ï£ºÏÑ∏??",
      }, { status: 503 });
    }
