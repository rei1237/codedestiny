...
const SUKUYO_SECTION_SPECS = [
  { key: "overview", title: "☯ 總論 — 종합 궁합 총평", minChars: 1300, guide: "종합 스코어 한 문장 요약 뒤 운명인연도, 기질조화도, 감정공명도, 성장시너지, 장기안정도를 각각 원국 요소와 연결해 풀이. 좋은 점만 쓰지 말고 이 관계가 실제로 취약한 지점도 같은 무게로 밝힌다. 총평 첫머리에 두 사람이 각각 어느 자리(역할)에 서는지를 이름과 함께 한 줄로 못 박고 시작한다" },
  ...
  { key: "attraction", title: "✦ 引力 — 끌림의 구조", minChars: 1300, guide: "두 사람이 서로에게 끌리는 이유를 외모·인상, 감정, 가치관, 대화, 에너지, 카르마 여섯 요소로 나눠 각각 어느 쪽이 더 강하게 작동하는지 밝힌다. 처음 만났을 때의 구체적 시나리오 1개와 지금 관계에서 끌림을 재확인하는 시나리오 1개를 포함한다. 여섯 요소마다 두 사람의 자리가 서로 다르게 작동하는 지점을 이름으로 지목한다" },
  ...
  { key: "communication", title: "🗣 疏通 — 대화법과 관계 유지 전략", minChars: 1400, guide: "대화법, 갈등 해결, 감정 표현, 거리 조절, 신뢰 형성 다섯 축으로 나눠 각 축마다 두 사람 각자에게 효과적인 방식을 대조하고 실제 대사 예시를 붙인다. 마지막에 오늘부터 해볼 수 있는 행동 5가지를 번호로 정리하되, 언제 무엇을 하는지가 드러나게 쓴다. 행동 지침은 두 사람에게 같은 것을 주지 말고 각자의 자리에 맞춰 나눠 준다" },
  ...
  { key: "conflict", title: "〜 波紋 — 갈등의 파문과 위기", minChars: 1400, guide: "가장 자주 반복될 갈등 3가지를 각각 '어떤 상황에서 터지는가 → 근본 원인 → 그때 두 사람 각자의 심리 → 실제 해결법과 대사 예시' 네 단계 고정 순서로 제시하고, 이어서 권태기·장거리·가치관 충돌 중 이 조합이 가장 취약한 위기 국면 1개와 단계별 행동 지침을 붙인다. '그때 두 사람 각자의 심리'는 각자의 자리에서 나오는 반응으로 설명하고, 한쪽 자리에 갈등의 책임을 몰지 않는다" },
  ...
  { key: "karma", title: "☸ 宿業 — 카르마 분석", minChars: 1400, guide: "숙요점 관점에서 이 두 사람이 왜 만났는지, 이번 생에서 서로를 통해 배우는 것이 무엇인지, 이 관계의 목적이 무엇인지, 인연 자체의 의미가 무엇인지를 네 갈래로 나눠 해석한다. 관계 유형은 쌍 이름(명·업태·영친·우쇠·안괴·성위)으로, 두 사람이 각각 선 자리는 그 쌍을 이루는 단일 자리(명·업·태·영·친·우·쇠·안·괴·성·위)로 구분해 부르고, 순행·역행 거리와 함께 반드시 논거로 인용한다. 전생 서사는 사실처럼 단정하지 말고 상징으로 읽는다" },
  ...
  { key: "domains", title: "💞 領域 — 관계 영역별 궁합", minChars: 1500, guide: "결혼 → 동거 → 사업·비즈니스 → 친구 → 직장 동료 다섯 영역을 모두 다루고, 각 영역을 현재 흐름 → 해석 근거 → 추천 행동 세 조각으로 풀이. 영역마다 서로 다른 계산 확정값을 근거로 들고, 적합도가 낮은 영역은 낮다고 분명히 말한다. 추천 행동은 두 사람의 자리가 다르므로 각자에게 다른 몫으로 나눠 배정한다" },
...
function buildCompatibilityRelationMeta(compatibility = {}) {
  ...
  return {
    ...
    distance,
    intensity,
    role_a: directional.aRole,
    role_b: directional.bRole,
    directional_meaning: {
      a_to_b: directional.aRoleMeaning,
      b_to_a: directional.bRoleMeaning,
      a_experience: directional.aRoleExperience || "",
      b_experience: directional.bRoleExperience || "",
      a_advice: directional.aRoleAdvice || "",
      b_advice: directional.bRoleAdvice || "",
    },
  ...
  return buildAnalysisBasisPayload({
    ...
    groups: [
      ...
      basisGroup("relation", "두 별 사이의 방위", [
        ...
        // 자리 의미·거리 안내가 인용 가능 목록에 없어, 모델이 방향 서술을 피하고 대칭 문장으로 뭉개 왔다.
        relation.directional_meaning?.a_to_b ? basisItem(`${personA.name}이(가) 선 자리`, `${relation.type_a_to_b} · ${relation.directional_meaning.a_to_b}`, { term: "자리" }) : null,
        relation.directional_meaning?.b_to_a ? basisItem(`${personB.name}이(가) 선 자리`, `${relation.type_b_to_a} · ${relation.directional_meaning.b_to_a}`, { term: "자리" }) : null,
        relation.directional_distance_guide ? basisItem("방향별 거리", relation.directional_distance_guide) : null,
...
function buildSectionGroupPrompt(input, calculation, group) {
  ...
  return [
    ...
    "🔴 두 사람이 선 자리(역할)는 방향에 따라 서로 다릅니다. 각 장에서 두 사람을 각각 이름으로 지목하고 그 사람의 자리를 함께 부르며, 자리가 다르면 두 사람에 대한 서술도 실질적으로 달라야 합니다. '서로·둘 다·함께'로 자리 서술을 대체하지 마십시오.",
    "🔴 관계 이름과 자리 이름의 한자는 처음 나올 때 한 줄로 풀어 줍니다(예: 안괴(安壞) — 한쪽은 안(安)의 자리에, 다른 쪽은 괴(壞)의 자리에 섭니다).",
    "🔴 괴(壞)·쇠(衰)·위(危)처럼 무거운 자리도 그 사람을 단정하거나 겁주지 말고, 그 자리에 선 사람이 실제로 할 수 있는 일을 함께 제시합니다.",
