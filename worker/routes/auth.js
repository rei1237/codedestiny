...
import { handleMeThroughEdgeCache } from "../lib/auth-me-cache.js";
import {
  ACCESS_COOKIE_NAME,
  REFRESH_COOKIE_NAME,
...
export async function handleAuthRoutes(request, env, ctx) {
  ...
  try {
    ...
    // Set-Cookie 동반)은 래퍼가 그대로 통과시킨다. 근거와 안전조건은 worker/lib/auth-me-cache.js.
    if (method === "GET" && path === "/me") return await handleMeThroughEdgeCache(request, env, handleMe);
