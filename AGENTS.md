...
- Generated mirrors are excluded from default search. Repo-root `.ignore` (regenerated by `sync:public`) hides the 169 `public/` copies from ripgrep-backed Grep/Glob so searches return the canonical file once. git does not read `.ignore`, so tracking, builds, deploys, and CI are unaffected. To search including mirrors use `git grep` or `rg -u`; to read one, pass its path directly. 🔴 Pre-deletion three-way greps must use `git grep` — see [docs/context/search-discipline.md](docs/context/search-discipline.md).
