...
(function () {
  ...
  window.generateVedicBook = function(){
    ...
    function _getVedicPreparePayload() {
      if (_premiumPreparePayload) return _premiumPreparePayload;
      _premiumPreparePayload = _buildVedicChapterPayload(0);
    ...
    function _ensurePremiumReportSession() {
      ...
      return window.__cdPremiumAuthJson('/api/premium-report/prepare', {
        ...
        requestBody: _getVedicPreparePayload()
        ...
        return new Promise(function(resolve){
          ...
          _ensurePremiumReportSession().then(function(prepared) {
            ...
            window.__cdPremiumAuthJson('/api/premium-report/chapter', {
              ...
              featureType: _premiumFeatureType,
              requestBody: _getVedicPreparePayload(),
              requestId: 'vedic:chapter:' + (idx + 1) + ':' + Date.now().toString(36) + ':' + tryNo,
            }, {
              maxAttempts: 2,
            }).then(function(data) {
              clearTimeout(tid);
    ...
    (function generateNext(idx){
      ...
      _fetchChapter(idx).then(function(data){
        ...
        if (isSessionMissing || statusCode === 402 || statusCode === 503) {
          console.error('[베다 프리미엄 PDF][BLOCKED]', {
            chapter: idx + 1,
            code: errorCode,
            status: statusCode,
          ...
          _generating = false;
          if (_mysticTimer) { clearInterval(_mysticTimer); _mysticTimer = null; }
          var blockedErrEl = _qs('vdErrorMsg');
          if (blockedErrEl) {
