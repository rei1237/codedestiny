...
(function () {
  ...
  var MYSTIC_QUOTES = [
    ...
    '나크샤트라는 달이 태어날 때 머문 별자리 — 무의식의 원형 코드.',
    '다샤는 영혼이 각 행성 에너지를 교과서처럼 배우는 시간입니다.',
    ...
    '카르마는 피할 수 없는 감옥이 아니라 성장의 교과서입니다.',
  ...
  function _showScreen(id){
    var screens=['vdStartScreen','vdLoadingScreen','vdResultScreen','vdErrorScreen'];
  ...
  window.generateVedicBook = function(){
    ...
    (function generateNext(idx){
      ...
      _fetchChapter(idx).then(function(data){
        ...
        else{_failCount++;var msg=(data&&data.message)?data.message:'알 수 없는 오류';console.warn('[베다] Chapter '+(idx+1)+' 실패:',msg);_chapters[idx]='⚠️ **이 챕터의 분析을 불러오는 데 실패했습니다.**\n\n오류: '+msg+'\n\n잠시 후 다시 시도해 주세요.';}
        _setProgress(idx+1);
        generateNext(idx+1);
      });
    })(0);
  ...
  window.downloadVedicBookPdf = function(){
    ...
    var fullHtml='<!DOCTYPE html><html lang="ko"><head><meta charset="UTF-8"><title>'+_escHtml(name)+'</title>' +
      ...
      '<p style="font-size:1rem;color:#fdba74;margin-bottom:20px;">인도 조티쉬(Jyotish) 기반 12챕터 카르마 운명 리포트</p>' +
