export type VisualDetail = {
  ...
  panels: { title: string; text?: string; visualPreview?: 'book' | 'animal' | 'master-codex' | 'love-code' | 'nakshatra-compat' | 'neo' | 'saju' | 'ziwei' | 'sukuyo' | 'vedic' | 'astrology' | 'tarot-love' | 'tarot-reunion' | 'tarot-mindscan'; items?: string[]; steps?: {label: string; detail?: string}[]; verifiedCapture?: {src: string; alt: string; label: string; width?: number; height?: number} }[];
};
