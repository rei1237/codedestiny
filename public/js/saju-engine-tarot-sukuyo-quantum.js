...
var TAROT_SPREAD_LABELS = {
  default: ['지금까지의 흐름', '현재의 핵심 기운', '다음 선택 기준'],
...
function validateMyeongriTarotReading(reading) {
  ...
  var fullText = [
    ...
    reading && reading.categoryLens && reading.categoryLens.theme,
    reading && reading.categoryLens && reading.categoryLens.action
  ].join(' ');
  ...
  if (main) {
    var mainCount = (String(fullText).match(new RegExp(main, 'g')) || []).length;
    if (mainCount < 2) {
      issues.push('주 십성이 본문에 충분히 반영되지 않았습니다.');
    }
    MYEONGRI_TEN_GODS.forEach(function(tg) {
      if (tg === main) return;
  ...
  if (reading && reading.categoryLens && reading.categoryLens.theme && String(fullText).indexOf(reading.categoryLens.theme) < 0) {
    issues.push('질문 카테고리 전용 렌즈가 본문에 충분히 반영되지 않았습니다.');
  }
  ...
  return '' +
    '<div style="margin-bottom:18px;padding:14px;border:1px solid rgba(255,255,255,0.14);border-radius:12px;background:rgba(17,24,39,0.34);">' +
      ...
      '<div style="margin-top:12px;line-height:1.9;color:#e5e7eb;">' +
        ...
        '<span>' + escapeTarotHtml(reading.oneLineConclusion || reading.opening || '') + '</span>' +
      '</div>' +
      '<div style="margin-top:12px;line-height:1.9;color:#fef3c7;">' +
        '<b style="color:#fcd34d;">카드별 명리 해석</b><br>' +
        '<span>' + escapeTarotHtml(reading.currentSituation) + '</span>' +
      '</div>' +
      '<div style="margin-top:12px;line-height:1.9;color:#dbeafe;">' +
        '<b style="color:#93c5fd;">십성 작동 방식</b><br>' +
        '<span>' + escapeTarotHtml(reading.tenGodInterpretation) + '</span>' +
      '</div>' +
      '<div style="margin-top:12px;line-height:1.9;color:#d1fae5;">' +
        '<b style="color:#6ee7b7;">질문별 현실 조언' + (lens.theme ? ' · ' + escapeTarotHtml(lens.theme) : '') + '</b><br>' +
...
function getTenGodGroupKey(tenGod) {
  var meta = TEN_GOD_DETAILS[normalizeTenGod(tenGod)] || {};
  return meta.group || '';
}

function getTenGodGroupLabel(group) {
  var labels = {
    self: '비겁: 중심과 자기 기준',
    output: '식상: 표현과 결과물',
    wealth: '재성: 돈과 현실 성과',
    officer: '관성: 책임과 관계의 규칙',
    resource: '인성: 회복과 배움'
  };
  return labels[group] || '혼합 기운';
}

function analyzeMyeongriTenGodFlow(readings) {
  var items = (readings || []).map(function(reading) {
    var tenGod = normalizeTenGod(reading && reading.tenGod && reading.tenGod.main);
    var group = getTenGodGroupKey(tenGod);
    return { tenGod: tenGod || '미상', group: group || 'mixed' };
  });
  var names = items.map(function(item) { return item.tenGod; }).join(' → ');
  var groups = items.map(function(item) { return item.group; });
  var uniqueGroups = {};
  groups.forEach(function(group) { uniqueGroups[group] = true; });
  var supportMap = {
    resource: { self: '배움과 보호가 자기 기준을 세워주는 생조 흐름' },
    self: { output: '내 의지가 표현과 결과물로 빠져나가는 생산 흐름' },
    output: { wealth: '표현과 결과물이 재물·성과로 이어지는 회수 흐름' },
    wealth: { officer: '성과가 책임과 공적 기준으로 묶이는 관리 흐름' },
    officer: { resource: '책임이 배움과 회복으로 돌아오는 정리 흐름' }
  };
  var tensionMap = {
    output: { officer: '표현이 규칙을 건드려 말과 책임의 충돌이 생기기 쉬운 흐름' },
    wealth: { resource: '현실 이익과 내면 회복이 서로 다른 속도를 요구하는 흐름' },
    officer: { self: '책임과 자기 고집이 맞서며 압박이 커질 수 있는 흐름' },
    self: { wealth: '내 방식과 돈의 조건이 충돌해 조율이 필요한 흐름' }
  };
  var steps = [];
  ...
  return {
    ...
    names: names,
    detail: detail,
    oneLine: label + ': ' + names + ' 흐름은 ' + detail
  };
}
...
function startThreeCardFlow() {
  ...
  if (guideEl) guideEl.textContent = (labels[0] || '지금까지의 흐름') + ' 자리의 카드를 먼저 열어보세요';
...
function flipTarotSpreadCard(index) {
  ...
  if (data && msgEl) {
    msgEl.innerHTML = '✨ ' + data.card.name_kr + ' — ' + (data.isReversed ? '역방향의 신호가 드러납니다.' : '정방향의 흐름이 열립니다.');
  }
...
function buildFortuneHTML(res, p){
  ...
  var TSADVICE=isNeoSaju?{
    ...
    '편재':'큰 기회와 큰 변동성이 공존합니다. 작은 것부터 잡으세요.',
...
function calcSukuyoData(lunarObj, opt = { leapRule: 'current' }) {
    ...
    const baseData = {
        "각": {icon:"✨",celebs:"아인슈타인, 마리 퀴리",talent:95,
            ...
            karma:"각숙 태생은 어디서나 모임의 중심이 되는 타고난 자석입니다. 첫인상부터 빛나는 존재감으로 상대의 마음을 사로잡으며, 세대와 배경을 넘나드는 유연한 소통 능력을 지녔습니다. 다만 너무 많은 관계에 에너지를 쏟다가 정작 소중한 인연을 놓치는 실수를 경계해야 합니다. 적의 마음도 무장해제시키는 당신의 매력은 인생 최고의 자산입니다. 특히 창의적이고 감수성 풍부한 이들과 깊은 유대가 형성됩니다.",
        ...
        "위1": {icon:"🔥",celebs:"알렉산더 대왕, 율리우스 카이사르",talent:89,char:"危",
            ...
            work:"변동성이 클수록 역량이 살아나지만, 방향을 정해야 성과가 안정되는 타입입니다.",
            love:"평범한 흐름보다 선명한 감정 자극에 반응하기 쉬운 연애 패턴입니다.",
        ...
        "실": {icon:"💧",celebs:"이순신, 세종대왕",talent:99,
            ...
            hidden:"실숙는 모든 걸 시작하는 기세 뒤에 지속과 마무리에 대한 두려움을 감추고 있습니다. 시작의 에너지는 빠르게 올라오지만 끝을 보지 못하거나 루틴이 생기면 흥미를 잃는 경향이 있습니다. 이 패턴을 스스로 인식하고, 끝맺음을 함께 할 파트너나 실행 구조를 곁에 두는 것이 성공의 핵심입니다.",
        ...
        "자": {icon:"🌋",celebs:"나폴레옹, 잔 다르크",talent:90,
            ...
            karma:"자숙 태생 주변에는 늘 다양한 사람들이 모여듭니다. 당신의 신선한 지식과 유머 감각은 어떤 모임에도 활기를 불어넣습니다. 다만 가끔 말이 지나쳐 상대를 상처입히거나 실수를 유발할 수 있으므로, 말의 힘과 함께 침묵의 힘도 함께 키우는 것이 중요합니다.",
        ...
        "정": {icon:"🌪️",celebs:"마리 앙투아네트, 볼테르",talent:84,
            core:"정숙(井宿)는 하늘의 우물을 의미하는 별로, 차갑고 청명한 이성의 원천을 상징합니다. 정숙 아래 태어난 영혼은 세상의 모든 것을 논리와 합리의 눈으로 분류하고 체계화하기 위해 왔습니다. 냉정한 분석력으로 진실을 가려내는 탐정이자, 실행 구조의 비효율을 가장 먼저 발견하는 눈을 가졌습니다.",
        ...
        "성": {icon:"🔥",celebs:"마이클 조던, 알렉산더",talent:98,
            ...
            work:"사업이나 중책을 맡아 리더십을 발휘할 때 재능이 선명해집니다.",
            love:"헌신적인 서포터형 연인과 장수합니다.",
            wealth:"씀씀이가 크지만 그만큼 크게 버는 스케일을 갖고 있습니다."},
        "장": {icon:"⚡",celebs:"펠레, 마라도나",talent:97,
            core:"장숙(張宿)는 활짝 펼쳐진 새의 날개, 주작의 날개를 상징하는 풍요로움과 화려함의 별입니다. 장숙 아래 태어난 사람은 삶의 무대 위에서 자신의 빛으로 주변을 밝히려는 기질이 강합니다. 타고난 존재감과 강한 표현력으로 어느 자리에서나 중심에 서기 쉽습니다.",
            ...
            karma:"장숙 태생 주변에는 삶이 드라마처럼 풍요롭고 화려해집니다. 당신으로 인해 주변 사람들의 표정이 밝아지고, 환경의 분위기가 좋아질 수 있습니다. 다만 이 빛이 때로는 주변 사람들을 위축시키거나 의존하게 만들 수 있으므로, 함께 빛날 수 있도록 주변의 별들을 일으켜 세우는 지혜가 필요합니다.",
    ...
    const professionalSukuyoTraits = {
        "각": {
            ...
            health: "위로 뻗는 목(木)의 기운이라 움직일 때 생기가 돌지만, 과열되면 두통과 눈의 피로, 수면 리듬이 먼저 흔들립니다. 벌여 둔 일이 마무리되지 않고 쌓이면 긴장성 피로로 이어지기 쉽습니다. 하루 한 가지를 끝내는 완결 루틴과 가벼운 유산소 운동이 기운을 고르게 지켜 줍니다.",
        ...
        "항": {
            ...
            health: "곧게 버티는 금(金)의 기운이라 자세와 관절, 어깨와 목에 긴장이 쌓이기 쉽습니다. 원칙을 지키려 스스로를 몰아붙일 때 두통과 소화 불편이 신호로 나타납니다. 정해진 시간에 쉬는 규칙을 원칙처럼 지키면 기운이 오래 유지됩니다.",
            timing: "질서를 세우고 기준을 다듬는 시기, 조직이 재정비되는 국면에서 운이 강해집니다. 반대로 규칙 없이 흘러가는 혼란기에는 스트레스가 커집니다. 급하게 결론 내기보다 기준을 문서로 정리하는 시기로 삼으면 흐름이 안정됩니다.",
        ...
        "저": {
            ...
            health: "대지에 뿌리내리는 토(土)의 기운이라 기초 체력은 튼튼하지만 소화기와 근육 뭉침에 신호가 잘 옵니다. 감정을 오래 담아 두면 몸이 무겁고 순환이 정체되기 쉽습니다. 규칙적인 식사와 스트레칭, 감정을 말로 푸는 루틴이 기운을 지켜 줍니다.",
        ...
        "방": {
            ...
            health: "양기가 강한 방숙은 활력이 넘치지만 과로와 혈압, 심장 부담에 주의가 필요합니다. 성과를 몰아칠 때 수면과 휴식이 가장 먼저 희생됩니다. 강약을 조절하는 운동과 정기적인 휴식이 큰 에너지를 오래 쓰게 합니다.",
            timing: "판을 키우고 성과를 거두는 확장기, 협상과 결정이 몰리는 시기에 운이 강해집니다. 성장이 멈춘 정체기에는 조급함이 커질 수 있습니다. 멈춘 시기에는 새 판을 벌이기보다 사람과 신뢰를 다지는 데 힘을 쓰는 편이 좋습니다.",
        ...
        "심": {
            ...
            health: "예민한 감지력을 지닌 심숙은 자율신경과 수면, 감정 기복에 영향을 받기 쉽습니다. 속으로 의미를 곱씹을 때 긴장과 불면이 신호로 나타납니다. 명상과 규칙적인 수면, 감정을 글로 꺼내는 습관이 기운을 맑게 합니다.",
            timing: "속을 들여다보고 핵심을 파악하는 시기, 전략과 판단이 필요한 국면에서 운이 강해집니다. 정보가 과하게 몰릴 때는 판단이 흐려질 수 있습니다. 결정을 앞두고는 사실과 느낌을 분리해 적어 보는 시간을 가지면 통찰이 선명해집니다.",
        ...
        "미": {
            ...
            health: "타오르는 화(火)의 기운이라 폭발적인 힘이 있지만 염증과 열, 급성 피로에 취약합니다. 계속 싸우듯 몰아붙이면 번아웃과 근육·관절 부상이 따라옵니다. 강도 높은 활동 뒤 확실한 회복 시간을 넣어야 에너지가 지속됩니다.",
        ...
        "기": {
            ...
            health: "흐르는 수(水)의 기운이라 순환은 좋지만 기복이 크고 신장·방광, 불규칙한 생활에 영향을 받습니다. 자유를 좇다 수면과 식사 리듬이 흐트러지면 면역이 쉽게 떨어집니다. 최소한의 생활 리듬을 정해 두면 자유로움이 건강하게 유지됩니다.",
        ...
        "두": {
            core: "두숙(斗宿)은 방향을 제시하고 지혜를 나누는 숙입니다. 큰 그림을 보고 사람을 이끄는 감각이 있으며, 배움과 가르침을 통해 운이 열립니다. 조언이 훈계처럼 들리지 않도록 상대의 준비도를 먼저 살피는 것이 좋습니다.",
            ...
            health: "위로 자라는 목(木)의 기운을 지닌 두숙은 활동적이나 어깨·목의 긴장과 눈의 피로가 잘 옵니다. 늘 현명해야 한다는 압박이 두통과 소화 불편으로 나타납니다. 배우는 자리에 서서 힘을 빼는 시간이 오히려 기운을 회복시킵니다.",
            timing: "가르치고 방향을 제시하며 체계를 세우는 시기에 운이 강해집니다. 배움 없이 소모만 반복되는 시기에는 정체됩니다. 흐름이 막히면 새로운 지식을 들이거나 배우는 입장에 서 보면 길이 다시 열립니다.",
        ...
        "여": {
            ...
            health: "다지는 토(土)의 기운이라 기초는 안정적이나 소화기와 긴장성 근육 뭉침에 신호가 옵니다. 완벽을 추구하며 감정을 감출 때 몸이 굳고 피로가 쌓입니다. 작은 실수를 허용하고 몸의 긴장을 푸는 루틴이 회복을 돕습니다.",
            timing: "다듬고 완성도를 높이는 시기, 품질과 마무리가 중요한 국면에서 운이 강해집니다. 거칠게 밀어붙여야 하는 속도전에서는 지칠 수 있습니다. 흐름이 급할 때는 완벽보다 적절한 완성을 택하면 리듬이 가벼워집니다.",
        ...
        "허": {
            ...
            health: "비움을 지향하는 허숙은 기운의 진폭이 커서 무기력과 수면 리듬, 감정의 공허에 영향을 받습니다. 의미를 찾지 못할 때 몸도 함께 가라앉기 쉽습니다. 햇빛을 쬐는 산책과 규칙적인 리듬, 몸을 쓰는 활동이 균형을 잡아 줍니다.",
        ...
        "위1": {
            ...
            health: "긴장 속에서 예민해지는 위숙은 자율신경과 수면, 스트레스성 증상에 영향을 받습니다. 위기 대응으로 각성 상태가 길어지면 소진이 빠릅니다. 안전 장치와 충분한 수면을 먼저 확보해야 기운이 유지됩니다.",
            timing: "전환점과 위기 국면에서 판단이 선명해지고 운이 강해집니다. 평온이 길게 이어지는 시기에는 오히려 불안해질 수 있습니다. 큰 변화를 앞두고는 리스크 목록을 먼저 만들어 두면 흐름을 안정적으로 탑니다.",
        ...
        "실": {
            ...
            health: "시작의 열기를 지닌 실숙은 초반 에너지가 강하지만 급전개 뒤 번아웃이 오기 쉽습니다. 열정이 앞설 때 수면과 회복이 뒤로 밀립니다. 착수와 함께 회복 일정을 같이 짜 두면 기운이 오래 유지됩니다.",
        ...
        "벽": {
            ...
            health: "흐르는 수(水)의 기운이라 순환은 좋지만 감정을 미루면 신장·방광과 순환이 정체됩니다. 평화를 지키려 참을 때 몸이 먼저 무거워집니다. 감정을 초기에 표현하고 물을 충분히 마시는 습관이 기운을 지켜 줍니다.",
            timing: "사람 사이를 잇고 조율하는 시기, 협업이 중요한 국면에서 운이 강해집니다. 혼자 밀어붙여야 하는 독자 승부에서는 힘이 덜 실립니다. 갈등이 커질 때는 승패를 가르기보다 서로의 자리를 정리하는 편이 좋습니다.",
        ...
        "규": {
            ...
            health: "곧게 자라는 목(木)의 기운을 지닌 규숙은 자세와 관절, 눈의 피로에 신호가 옵니다. 익숙함을 고집하며 긴장이 쌓이면 어깨와 목이 굳습니다. 새로운 자극과 가벼운 변화를 일상에 넣으면 몸과 기운이 부드러워집니다.",
            timing: "정돈하고 기록하며 가치를 세우는 시기에 운이 강해집니다. 급격한 변화와 파격이 요구되는 시기에는 답답함을 느낍니다. 흐름이 막히면 익숙한 틀에 새 관점을 한 가지 들여 보면 길이 열립니다.",
        ...
        "루": {
            ...
            health: "곧고 예민한 금(金)의 기운이라 호흡기와 피부, 과로에 신호가 옵니다. 두루 어울리다 에너지를 쓰고 나면 급격히 지치기 쉽습니다. 혼자 정리하는 회복 시간을 확보해야 친화력이 맑게 유지됩니다.",
            timing: "사람과 정보를 연결하고 관계망을 넓히는 시기에 운이 강해집니다. 홀로 깊이 파고들어야 하는 시기에는 집중이 흩어집니다. 흐름이 정체되면 새로운 사람과 자리를 만들어 연결을 넓히면 기회가 열립니다.",
        ...
        "위2": {
            ...
            health: "모으고 다지는 토(土)의 기운이라 소화기와 위장, 스트레스성 긴장에 신호가 옵니다. 불확실한 상황을 곱씹으며 예민해질 때 몸이 먼저 굳습니다. 규칙적인 식사와 걱정을 적어 비우는 루틴이 기운을 지켜 줍니다.",
            timing: "모으고 분석하며 자원을 정리하는 시기에 운이 강해집니다. 즉흥적인 속도전에서는 결정을 놓칠 수 있습니다. 흐름이 막히면 기준을 세워 실행으로 옮기는 시점을 정하면 정체가 풀립니다.",
        ...
        "묘": {
            ...
            health: "따뜻이 품는 묘숙은 남을 돌보다 자기 회복을 놓쳐 만성 피로가 오기 쉽습니다. 감정을 대신 짊어질 때 소화기와 면역이 먼저 신호를 보냅니다. 내 몸과 마음을 먼저 챙기는 시간을 일정에 넣어야 오래 돌볼 수 있습니다.",
            timing: "사람을 살피고 공동체를 안정시키는 시기에 운이 강해집니다. 냉정한 경쟁만 이어지는 국면에서는 지칠 수 있습니다. 소진이 느껴지면 돌봄의 범위를 줄이고 자신을 돌보는 편이 좋습니다.",
        ...
        "필": {
            ...
            health: "묵묵히 다지는 필숙은 기초 체력은 좋지만 반복 긴장과 관절·근육 뭉침에 취약합니다. 익숙한 방식을 고집하며 무리하면 몸이 서서히 굳습니다. 작은 변화와 스트레칭을 섞으면 완성도와 유연성이 함께 살아납니다.",
            timing: "오래 다듬어 결과물을 완성하는 시기에 운이 강해집니다. 빠른 전환과 즉흥이 요구되는 시기에는 답답함을 느낍니다. 막힘이 느껴지면 방식보다 목적을 먼저 확인하면 길이 트입니다.",
        ...
        "자": {
            ...
            health: "말과 생각이 빠른 자숙은 신경과 수면, 두통에 영향을 받기 쉽습니다. 머리가 계속 돌아갈 때 긴장과 불면이 신호로 나타납니다. 생각을 멈추는 시간과 규칙적인 수면이 기운을 맑게 합니다.",
            timing: "정보를 모아 핵심을 정리하고 전달하는 시기에 운이 강해집니다. 침묵과 인내가 필요한 국면에서는 실수가 나올 수 있습니다. 흐름이 급할 때는 말을 내보내기 전에 방향을 먼저 고르면 좋습니다.",
        ...
        "삼": {
            ...
            health: "빠르게 흐르는 삼숙은 변화를 만들며 긴장과 수면 불규칙, 급성 피로에 취약합니다. 정체를 견디지 못해 자꾸 일을 벌이면 소진이 옵니다. 변화의 속도를 조절하고 회복 리듬을 지키면 기운이 유지됩니다.",
        ...
        "정": {
            ...
            health: "맑게 정리하는 정숙은 목(木)의 기운으로 활동적이나 신경 긴장과 어깨·눈의 피로가 옵니다. 감정을 통제하며 억누를 때 몸이 굳고 소화가 불편해집니다. 감정을 인정하고 몸을 푸는 루틴이 균형을 잡아 줍니다.",
            timing: "복잡한 것을 분류하고 기준을 세우는 시기에 운이 강해집니다. 감정과 직관이 앞서야 하는 국면에서는 경직될 수 있습니다. 흐름이 막히면 논리에 감정의 온도를 더하는 시점으로 삼으면 좋습니다.",
        ...
        "귀": {
            ...
            health: "예민한 귀숙은 기분 변화와 수면 리듬, 이유 없는 불안에 영향을 받습니다. 감각이 과하게 열리면 신경이 쉽게 지칩니다. 몸의 리듬과 수면을 정돈하면 직감이 더 선명해집니다.",
            timing: "변화의 조짐을 먼저 감지하는 전환기에 운이 강해집니다. 확실하고 반복되는 시기에는 감각이 무뎌질 수 있습니다. 예감이 올 때는 현실 확인을 거치면 직감이 지혜로 바뀝니다.",
        ...
        "류": {
            ...
            health: "깊이 몰입하는 류숙은 토(土)의 기운으로 소화기와 근육 긴장, 감정 정체에 신호가 옵니다. 감정을 오래 붙들면 몸도 함께 무거워집니다. 감정을 몸 밖으로 꺼내는 기록과 주기적인 휴식이 기운을 지켜 줍니다.",
            timing: "한 분야를 깊게 파고드는 시기, 장기 몰입이 필요한 국면에서 운이 강해집니다. 여러 일을 동시에 벌여야 하는 시기에는 소모가 큽니다. 집착이 굳는다 싶으면 주기적으로 숨을 고르는 시간을 넣으면 좋습니다.",
        ...
        "성": {
            ...
            health: "승부의 열기를 지닌 성숙은 과열과 혈압, 수면 부족에 취약합니다. 인정받고 싶은 마음이 강해질 때 몸을 몰아붙이게 됩니다. 승패보다 성장 지표를 세우고 회복을 챙기면 힘이 오래갑니다.",
            timing: "목표를 향해 성과를 거두는 시기, 경쟁과 도전의 국면에서 운이 강해집니다. 성과가 보이지 않는 정체기에는 조급함이 커집니다. 멈춘 시기에는 승패 대신 실력을 다지는 데 힘을 쓰는 편이 좋습니다.",
        ...
        "장": {
            ...
            health: "펼치는 힘을 지닌 장숙은 에너지의 기복과 수면 리듬, 과로에 신호가 옵니다. 여러 곳에 관심을 쏟다 보면 쉽게 방전됩니다. 중심 주제를 정해 에너지를 모으면 컨디션이 안정됩니다.",
            timing: "자신을 드러내고 무대를 키우는 확장기에 운이 강해집니다. 조용히 안으로 다져야 하는 시기에는 흔들리기 쉽습니다. 흐름이 흩어지면 중심 주제 하나로 힘을 모으면 빛이 강해집니다.",
        ...
        "익": {
            ...
            health: "높은 기준을 지닌 익숙은 긴장성 두통과 어깨 결림, 수면 부족에 취약합니다. 부족함을 참지 못해 스스로를 몰아붙일 때 소진이 옵니다. 단계별 완성을 인정하고 긴장을 푸는 루틴이 기운을 지켜 줍니다.",
            timing: "품질과 완성도를 끌어올리는 시기에 운이 강해집니다. 거칠고 빠른 속도전에서는 지칠 수 있습니다. 기준이 너무 높아지면 지금 단계의 완성을 인정하는 편이 흐름을 가볍게 합니다.",
        ...
        "진": {
            ...
            health: "정리하고 마무리하는 진숙은 수(水)의 기운으로 순환과 신장·방광, 신경 피로에 신호가 옵니다. 여러 가능성을 살피며 마음이 피로해질 때 몸도 함께 지칩니다. 우선순위를 줄이고 충분히 쉬면 기운이 회복됩니다.",
            timing: "복잡한 것을 마무리하고 다음을 준비하는 전환기에 운이 강해집니다. 새로 크게 벌여야 하는 시기에는 결정이 늦어질 수 있습니다. 흐름이 막히면 완벽한 답보다 지금 정리할 한 가지를 고르면 좋습니다.",
    ...
    const mapped = Object.assign({}, baseData[key] || {
        ...
        health: "기운의 리듬을 관찰하며 규칙적인 수면과 휴식을 지키면 컨디션이 안정됩니다. 무리한 뒤에는 회복 시간을 반드시 넣어야 합니다. 몸이 보내는 작은 신호를 일찍 살피는 습관이 중요합니다.",
        timing: "흐름을 거스르기보다 때를 기다려 힘을 모으면 운이 열립니다. 정체가 느껴질 때는 새로운 배움이나 작은 변화를 만들어 리듬을 되살리는 편이 좋습니다. 서두르지 않는 판단이 기회를 지켜 줍니다.",
        social: "사람들과의 관계에서 진심과 경계를 함께 지킬 때 신뢰가 쌓입니다. 모두에게 맞추기보다 지킬 선을 분명히 하면 관계가 건강해집니다. 작은 배려가 오래가는 인연을 만듭니다.",
        advice: "오늘 할 수 있는 한 가지에 집중하면 흐름이 정돈됩니다. 감정이 흔들릴 때는 결론을 서두르지 말고 잠시 숨을 고르세요. 나다운 속도를 지키는 것이 가장 큰 성장입니다."
    }, professionalSukuyoTraits[key] || {});
...
function getDailyKarmicGuidance(lunarObj, m) {
    ...
    const insightPool = [
        `오늘의 ${m}별 기운은 고요한 수면 위 달빛처럼 직관을 또렷하게 비춥니다. 마음속에서 오래 묵힌 질문을 차분히 정리하기 좋습니다.`,
        `${m}의 별자리 에너지가 오늘 말의 결을 다듬어줍니다. 중요한 대화나 글쓰기, 제안은 확정 전에 초안을 정리하기 좋습니다.`,
        `오늘 ${m}의 기운은 씨앗처럼 잠재적입니다. 눈에 띄는 결과보다 보이지 않는 토대를 쌓는 데 집중하면 흐름이 단단해집니다.`,
        `${m}별의 진동수가 오늘 인간관계의 감각을 깨웁니다. 오래된 인연은 성급히 붙잡기보다 가볍게 안부를 전하기 좋습니다.`,
        `오늘 ${m}의 우주적 파동은 창의력을 부드럽게 자극합니다. 평소와 다른 루트를 걸으며 작은 영감을 받아들이기 좋습니다.`,
        `${m}별은 오늘 돈을 다루는 습관을 점검하게 합니다. 오래 미룬 지출 기준이나 예산표를 차분히 정리해보세요.`,
        `${m}의 기운이 감정 에너지를 정화하는 날입니다. 과거의 집착이나 오해는 결론보다 거리 두기로 가볍게 내려놓기 좋습니다.`,
        `오늘 ${m}별의 주파수는 작은 용기를 깨웁니다. 두렵다고 미뤄온 일은 큰 결단보다 첫 준비를 시작하기 좋습니다.`,
        `${m}의 별빛이 오늘 집중력의 렌즈가 되어줍니다. 산만하게 흩어지는 에너지를 하나의 작은 목표에 모아보세요.`,
        `${m}별이 따뜻한 인연의 온기를 퍼뜨리는 날입니다. 주변 사람에게 작은 호의를 건네며 관계의 결을 부드럽게 다듬기 좋습니다.`
    ];
...
function syRequirePaidSukuyoFeature(feature, onGranted) {
  ...
  if (isAdminBypass) {
    if (typeof onGranted === 'function') onGranted({ status: 'admin_bypass' });
    return true;
  }
  var token = '';
  ...
  window.alert('결제 모듈을 불러오지 못했습니다. 잠시 후 다시 시도해 주세요.');
  return true;
}
...
function syPaidFeatureUserScope() {
  ...
  try {
    if (typeof readAuthUser === 'function') user = readAuthUser();
    else if (typeof root.readAuthUser === 'function') user = root.readAuthUser();
  } catch (_) {}
  if (user && typeof user === 'object') {
    var id = user.id || user.uid || user.userId || user.email || user.phone || user.username;
...
function syPaidFeatureStorageKey(featureKey) {
  return 'cd_sukuyo_paid_unlock:' + syPaidFeatureUserScope() + ':' + String(featureKey || '').trim();
}

function syFeatureListHasKey(list, featureKey) {
  if (!Array.isArray(list)) return false;
  var key = String(featureKey || '').trim();
  for (var i = 0; i < list.length; i += 1) {
    if (String(list[i] || '').trim() === key) return true;
  }
  return false;
}
...
function syIsPaidSukuyoFeatureUnlocked(featureKey) {
  ...
  try {
    if (root.unlockedFeatureMap && root.unlockedFeatureMap[key] === true) return true;
  } catch (_) {}
  try {
    var user = null;
    if (typeof readAuthUser === 'function') user = readAuthUser();
...
function syMarkPaidSukuyoFeatureUnlocked(featureKey) {
  var key = String(featureKey || '').trim();
  var root = typeof window !== 'undefined' ? window : {};
  if (!key) return;
  try {
...
function syCanonicalPhaseApproxFromDaily(dailyMoon) {
  ...
  if (label.indexOf('보름') >= 0) {
    return { phaseName: label, rhythmLight: '가득 찬 빛', rhythmPosition: '가장 밝은 구간', waxingOrWaning: '밝음' };
  }
  if (label.indexOf('상현') >= 0) {
    return { phaseName: label, rhythmLight: '차오르는 빛', rhythmPosition: '성장권', waxingOrWaning: '차오름' };
  }
  if (label.indexOf('반달') >= 0) {
    return { phaseName: label, rhythmLight: '균형의 빛', rhythmPosition: '중간권', waxingOrWaning: '균형' };
  }
  if (label.indexOf('초승') >= 0) {
    return { phaseName: label, rhythmLight: '막 피어나는 빛', rhythmPosition: '시작권', waxingOrWaning: '차오름' };
  }
  if (label.indexOf('그믐') >= 0) {
    return { phaseName: label, rhythmLight: '내면으로 접힌 빛', rhythmPosition: '정리권', waxingOrWaning: '기울음' };
  }
  return { phaseName: label || '미확인', rhythmLight: '미확인', rhythmPosition: '미확인', waxingOrWaning: '미확인' };
}
...
function syBuildLocalCanonicalData(lunarObj, sData, daily, sourceProfile) {
  ...
  var mansionIndexZeroBased = Number(sData.mansionIdx);
  var displayIndex = Number.isFinite(mansionIndexZeroBased) ? mansionIndexZeroBased + 1 : null;
  var missing = [];
  ...
  return {
    ...
    natalSukuyo: {
      nameKo: koName,
      nameHan: hanName,
      index: Number.isFinite(mansionIndexZeroBased) ? mansionIndexZeroBased : null,
      indexZeroBased: Number.isFinite(mansionIndexZeroBased) ? mansionIndexZeroBased : null,
      displayIndex: displayIndex,
      displayLabel: displayIndex ? String(displayIndex) + ' / 27' : '미확인',
    ...
    sukuyoAttributes: {
      ...
      relationshipStyle: syCanonicalTokenize((traits.love || '') + '. ' + (traits.karma || ''), '관계 성향 정보 없음'),
      careerStyle: syCanonicalTokenize(traits.work, '커리어 성향 정보 없음'),
      wealthStyle: syCanonicalTokenize(traits.wealth, '돈 습관 정보 없음'),
      recoveryPattern: syCanonicalTokenize((daily && daily.insight ? daily.insight : '') + '. ' + (daily && daily.ritual && daily.ritual.action ? daily.ritual.action : ''), '회복 패턴 정보 없음')
    },
    validation: {
...
function syReferenceBand(score) {
  var n = Number(score);
  if (!Number.isFinite(n)) return '참고';
  if (n >= 85) return '매우 활발';
  if (n >= 72) return '활발';
  if (n >= 60) return '안정';
  if (n >= 48) return '조율';
  return '휴식';
...
var SY_MANSION_PROFILE_OVERRIDES = {
  0: {
    symbolLine: '동방 청룡의 뿔, 목(木)의 기운 · 교룡(蛟龍)이 새벽 하늘을 가장 먼저 여는 시작의 별입니다.',
    archetype: '각숙은 남들이 아직 알아보지 못한 것을 가장 먼저 알아보고, 먼저 손을 내미는 사람이에요. 뿔이 앞장서듯 새로운 길을 여는 재능이 참 빛나죠. 시작한 것을 끝까지 지키는 힘은 천천히 길러 가면 충분합니다. 이 ‘먼저 나서는 마음’이 앞으로 볼 연애·돈·일의 뿌리가 되어 줍니다.',
    attractionType: '겉모습보다, 자기 세계가 뚜렷하고 스스로 빛나는 사람에게 마음이 먼저 기울어요. 뻔한 만남은 금세 시들해질 수 있으니, 함께 새로운 걸 만들어 갈 수 있는 사람 곁에 있을 때 가장 편안하고 오래갑니다.',
    loveStyle: '좋아하는 마음이 생기면 먼저 다가가 분위기를 만드는 편이에요. 표현이 솔직해서 초반의 설렘이 빠르게 차오르죠. 익숙해질수록 마음이 식은 것처럼 보일 수 있으니, 처음의 다정함을 말로 자주 전해 주시면 관계가 한결 따뜻해집니다.',
    deepeningMoment: '무언가를 함께 시작할 때 마음의 거리가 부쩍 가까워져요. 여행 계획이든 작은 취미든, 둘이 나란히 새로운 걸 만들어 가는 순간 상대를 진짜 내 편으로 느끼게 됩니다.',
    anxietyPoint: '상대가 나만큼 열정을 보이지 않으면 혼자 마음이 식어 버리기 쉬워요. 반응이 조금 느린 걸 무관심이라 여기지 말고, 편하게 먼저 물어봐 주세요. 대개는 마음이 없는 게 아니라 속도가 다를 뿐이랍니다.',
    longTermAdvice: '새로운 자극에만 끌리는 마음을 가끔 다독여 주세요. 한 사람과 오래 가려면, 익숙함 속에서 새로움을 함께 만들어 가면 됩니다. 그 작은 노력이 각숙의 사랑을 가장 오래 지켜 주는 방법이에요.',
    avoidPattern: '설렘이 식었다는 이유만으로 관계를 서둘러 정리하는 일',
    moneyFlow: '재능과 아이디어로 스스로 돈을 만들어 내는 힘이 있어요. 다만 영감이 차오르면 지갑도 함께 열리기 쉽죠. ‘기분을 위한 지출’과 ‘나를 키우는 지출’을 살짝 나눠 두시면, 버는 만큼 곁에 남게 됩니다.',
    spendingHabit: '새것과 첫 경험 앞에서 지갑이 먼저 열리곤 해요. 사고 싶은 게 생기면 하루만 살짝 미뤄 보세요. 그 하루가 충동의 절반을 부드럽게 걸러 줍니다.',
    savingPoint: '매달 버는 돈에서 일정 비율을 먼저 떼어, 손대지 않는 계좌로 옮겨 두세요. 남는 돈으로 저축하려 하면 좀처럼 모이지 않지만, 먼저 떼어 두고 나머지로 생활하면 어느새 든든하게 쌓여 있을 거예요.',
    workStrength: '아무것도 없던 자리에서 처음을 만들어 내는 일에 가장 강해요. 기획, 창업 초기, 새 판을 여는 자리에서 특히 빛이 납니다. 길이 없는 곳에 길을 내는 순간을 마음껏 즐겨 보세요.',
    workCaution: '여러 일을 벌여 놓고 마무리를 미루는 게 조금 아쉬운 점이에요. 새 일을 시작하기 전에, 지금 벌여 둔 것부터 하나 마무리해 보세요. 그 끝맺음 하나가 열 번의 시작보다 더 큰 신뢰를 만들어 줍니다.',
    collaborationTip: '당신이 방향을 열고, 마무리는 꼼꼼한 분에게 맡겨 보세요. 혼자 다 짊어지면 쉽게 지치거든요. 시작은 나, 완성은 함께 — 이 역할 나눔이 각숙의 성과를 훨씬 크게 키워 줍니다.'
  },
  2: {
    symbolLine: '동방 청룡의 뿌리, 토(土)의 기운 · 담비가 땅속 깊이 굴을 파듯 시간을 견뎌 내는 별입니다.',
    archetype: '저숙은 느긋해 보여도 한번 마음먹은 건 좀처럼 놓지 않는 사람이에요. 겉은 부드러워도 속에는 산을 깎아 내는 뚝심이 있죠. 빠른 성과보다 오래 쌓아 가는 방식이 잘 맞고, 이 ‘뿌리내리는 마음’이 앞으로 볼 연애·돈·일에 그대로 흐릅니다.',
    attractionType: '첫눈에 반하기보다, 함께 시간을 보내며 믿음이 천천히 쌓일 때 마음이 열려요. 말이 앞서는 사람보다 행동이 한결같은 사람 곁에서 가장 편안합니다. 그런 사랑이 저숙에게 오래 남아요.',
    loveStyle: '한번 마음을 주면 오래도록 깊게 가는 편이에요. 표현이 서툴러도 곁을 지키는 것으로 사랑을 보여 주죠. 다만 속마음을 너무 아끼면 상대가 외로워질 수 있으니, 아끼는 만큼 말로도 조금씩 전해 주세요.',
    deepeningMoment: '힘든 시간을 함께 견뎌 낸 순간에 관계가 단단해져요. 어려운 시기를 나란히 지나고 나면, 저숙은 그 사람을 평생의 인연으로 마음에 새깁니다.',
    anxietyPoint: '서운함을 속으로만 쌓다가 어느 날 마음이 훅 식어 버리기 쉬워요. 혼자 담아 두지 말고, 사흘을 넘기기 전에 짧게라도 마음을 꺼내 보세요. 쌓이고 나면 되돌리기가 참 어렵거든요.',
    longTermAdvice: '익숙함을 당연함으로 두지 않으시면 좋겠어요. 오래된 사이일수록 고마움을 소리 내어 표현해 주세요. 저숙의 한결같음은 상대가 알아줄 때 비로소 가장 큰 힘이 됩니다.',
    avoidPattern: '불편함을 말없이 참다가 한꺼번에 터뜨리는 일',
    moneyFlow: '티끌을 모아 태산을 만드는 데 타고난 사람이에요. 크게 버는 것보다 새 나가지 않게 지키는 쪽이 강점이죠. 조급하게 굴리려 하기보다 하던 대로 꾸준히 쌓아 가면 됩니다. 그게 저숙에게는 가장 빠르고 단단한 길이에요.',
    spendingHabit: '충동구매는 적은 편이지만, 아까운 마음에 좋은 기회를 놓치기도 해요. 아끼는 것과 인색한 것은 다르답니다. 나를 키우는 일에는 기꺼이 쓸 줄 아는 용기도 조금 내어 보세요.',
    savingPoint: '저축은 이미 충분히 잘하고 계세요. 이제는 모아 둔 돈을 가만히만 두지 마시고, 안전한 장기 투자로 한 칸만 옮겨 보세요. 저숙의 인내가 시간이 지날수록 복리로 되돌아옵니다.',
    workStrength: '긴 호흡이 필요한 일에 가장 강해요. 남들이 지쳐 떠날 때 끝까지 남아 결과를 만들어 냅니다. 연구, 운영, 자산 관리처럼 시간이 곧 실력이 되는 일에서 특히 빛이 나요.',
    workCaution: '변화가 빠른 환경에서는 적응이 조금 늦어 스트레스를 받기 쉬워요. 판을 한 번에 갈아타기보다, 단계를 나눠 천천히 옮겨 가 보세요. 저숙은 서두를 때가 아니라 버텨 낼 때 가장 강한 사람이니까요.',
    collaborationTip: '묵묵히 일하다 보면 공을 놓치기 쉬워요. 해낸 일을 기록해 두고, 중간중간 가볍게 공유해 주세요. 저숙의 성실함은 보여 줄 때 비로소 신뢰로 이어집니다.'
  },
  4: {
    symbolLine: '동방 청룡의 심장, 붉은 별 안타레스 · 달여우(心月狐)의 직관으로 마음을 읽어 내는 별입니다.',
    archetype: '심숙은 말보다 분위기를, 표정보다 속마음을 먼저 읽어 내는 사람이에요. 여우가 어둠 속 작은 기척을 알아채듯, 사람의 진심을 섬세하게 헤아리죠. 이 깊은 직관이 강점이자 조심할 점이라, 앞으로 볼 성향도 모두 ‘너무 깊이 읽는 마음’에서 비롯됩니다.',
    attractionType: '밋밋한 사람에게는 좀처럼 마음이 가지 않아요. 깊이가 있고 속을 다 보여 주지 않는 사람에게 강하게 끌리죠. 다만 강렬함만 좇다 보면 마음이 힘들어질 수 있으니, 편안함도 충분히 소중한 매력이라는 걸 기억해 주세요.',
    loveStyle: '상대의 감정을 손바닥 보듯 읽으며 깊이 몰입하는 편이에요. 그 몰입이 큰 매력이지만, 지나치면 상대가 숨 막혀 할 수도 있어요. 다 안다고 여기기보다, 직접 물어보는 여백을 조금 남겨 두시면 관계가 한결 편안해집니다.',
    deepeningMoment: '속마음을 꺼내 놓고 서로의 약한 부분을 나눌 때 관계가 깊어져요. 심숙은 비밀을 함께 나눈 사람에게 비로소 마음의 문을 온전히 엽니다.',
    anxietyPoint: '말 없는 신호를 혼자 곱씹다가 의심을 키우기 쉬워요. 느낌과 사실을 잠시 나눠 보세요. 불안할 때는 머릿속으로 결론 내리기 전에, 상대에게 편하게 확인해 보시면 마음이 훨씬 가벼워집니다.',
    longTermAdvice: '다 읽어 냈다는 확신을 가끔은 살며시 내려놓아 보세요. 오래 가려면, 그 통찰로 상대를 판단하기보다 이해하는 데 써 주시면 됩니다. 같은 눈이 벽이 될 수도, 다리가 될 수도 있으니까요.',
    avoidPattern: '혼자 의미를 곱씹으며 상대의 마음을 넘겨짚는 일',
    moneyFlow: '정보와 때를 읽는 감각이 뛰어나, 남보다 먼저 기회를 알아봐요. 다만 직감만 믿고 크게 지르면 위태로울 수 있어요. 촉이 왔을수록 한 번 더 차분히 확인해 보세요. 그러면 그 예리한 직감이 든든한 돈이 되어 줍니다.',
    spendingHabit: '겉으로는 절제하지만, 마음이 흔들릴 때 조용히 큰돈을 쓰곤 해요. 감정이 요동칠 때는 결제를 잠시 멈춰 보세요. 심숙의 지출은 지갑이 아니라 마음에서 시작되는 경우가 많거든요.',
    savingPoint: '따로 굴리는 자금이나 조용히 모아 둔 비상금이 잘 맞아요. 큰돈은 꼭 한 번 검증하는 단계를 거쳐 넣어 주세요. 확인 없이 들어간 돈이 심숙에게는 가장 크게 새 나가기 쉽답니다.',
    workStrength: '숨은 구조와 사람의 속내를 읽는 일에 정말 강해요. 상담, 분석, 기획, 조사처럼 표면 아래를 들여다보는 일에서 실력이 잘 드러납니다. 남이 보지 못하는 걸 보는 자리를 찾아보세요.',
    workCaution: '혼자 다 파악하려다 정보에 짓눌리기 쉬워요. 생각이 무거워질 때는 사실과 느낌을 나눠 적어 보세요. 심숙의 통찰은 판단이 흐려질 때가 아니라, 마음이 정리될 때 가장 맑게 살아납니다.',
    collaborationTip: '속을 잘 드러내지 않아 오해를 사기도 해요. 결론만 전하기보다 이유를 한 줄 덧붙여 주세요. 심숙이 생각을 살짝 열어 보이면, 사람들은 당신을 훨씬 더 깊이 신뢰하게 됩니다.'
  },
  7: {
    symbolLine: '북방의 남두육성(南斗) · 국자로 덜고 담듯, 사람과 일의 균형을 가만히 재는 별입니다.',
    archetype: '두숙은 어디서나 중심을 잡아 주는 사람이에요. 남두성이 무게를 재듯 상황을 차분히 가늠하고 조율하는 데 능하죠. 다만 늘 어른스러워야 한다는 부담을 스스로에게 지우기 쉬워, 앞으로 볼 성향도 모두 ‘기준을 쥔 사람의 무게’에서 비롯됩니다.',
    attractionType: '말과 행동에 책임감이 느껴지는 사람에게 마음이 스르르 기울어요. 화려한 매력보다, 약속을 지키는 태도와 쉽게 흔들리지 않는 단단함에서 깊은 신뢰를 느낍니다.',
    loveStyle: '사랑을 시작하면 상대가 미처 말하지 못한 불편함까지 먼저 헤아리는 편이에요. 다정함을 말보다 행동으로 보여 주죠. 다만 너무 챙기다 상대의 몫까지 떠안지 않도록, 가끔은 기대어 주셔도 괜찮아요.',
    deepeningMoment: '서로를 가르치려 하기보다 각자의 속도를 존중해 줄 때 관계가 깊어져요. 배려를 한쪽만 주기보다, 마음과 기대를 솔직히 주고받으면 신뢰가 한결 단단해집니다.',
    anxietyPoint: '늘 어른스러워야 한다는 마음이 커질수록 연애에서 속내를 숨기게 돼요. 혼자 단단해지려 애쓰기보다, 불안한 이유를 먼저 나눠 주시면 마음의 무게가 한결 가벼워집니다.',
    longTermAdvice: '오래 가고 싶다면 상대를 이끌어야 한다는 부담을 잠시 내려놓아 보세요. 조언보다 들어 주는 비율을 조금만 높여도, 애정과 안정감이 함께 자라납니다.',
    avoidPattern: '상대의 감정까지 혼자 책임지려다 홀로 결론을 내려 버리는 일',
    moneyFlow: '전체를 가늠하고 균형을 맞추는 감각이 좋아, 큰 돈의 흐름을 안정적으로 다뤄요. 다만 남을 챙기다 정작 나를 위한 몫을 놓치기 쉽죠. 내 몫의 저축을 가장 먼저 챙겨 두시면 마음도 지갑도 든든해집니다.',
    spendingHabit: '가치 있다고 느낀 곳에는 과감하지만, 기준이 흐려지면 체면을 위한 지출이 늘기도 해요. 한 달 쓸 금액을 먼저 정해 두시면 소비의 리듬이 편안하게 잡힙니다.',
    savingPoint: '목표 금액을 눈에 보이게 정해 두면 두숙의 성실함이 진가를 발휘해요. 매달 자동으로 모이는 계좌 하나만 만들어 두시면, 신경 쓰지 않아도 차곡차곡 쌓입니다.',
    workStrength: '흩어진 상황을 정리하고 기준을 세워 주는 일에 강해요. 사람과 자원을 조율하는 자리, 중심을 잡아야 하는 역할에서 특히 빛이 납니다.',
    workCaution: '늘 현명해야 한다는 부담이 쌓이면 어느새 번아웃으로 이어질 수 있어요. 완벽한 판단을 유지하려 애쓰기보다, 정기적으로 쉬어 가는 시간을 먼저 넣어 두시면 좋습니다.',
    collaborationTip: '방향을 정리하고 기준을 세우는 역할이 잘 어울려요. 모두를 이끌어야 한다는 짐을 혼자 지기보다, 역할을 나누고 서로의 전문성을 인정할 때 더 큰 성과가 납니다.'
  },
  1: {
    symbolLine: '동방 청룡의 목, 금(金)의 기운 · 하늘의 뜻을 곧게 세우는 용(龍)의 별입니다.',
    archetype: '항숙은 한번 옳다고 믿으면 폭풍 속에서도 흔들리지 않는 사람이에요. 원칙과 정의가 삶의 중심이라 곧은 신념이 큰 힘이 되죠. 다만 그 곧음을 스스로에게 너무 엄격하게 겨누기 쉬워, 앞으로 볼 성향도 모두 ‘굽히지 않는 마음’에서 비롯됩니다.',
    attractionType: '가벼운 말보다, 자기 신념이 분명하고 언행이 일치하는 사람에게 마음이 기울어요. 함께 있으면 존중받는다고 느껴지는 사람 곁에서 가장 편안합니다.',
    loveStyle: '한번 마음을 열면 우직하고 헌신적으로 사랑해요. 다만 옳고 그름을 또렷이 따지다 보면 상대가 다그침으로 느낄 수 있어요. 판단을 잠시 내려놓고 마음부터 안아 주시면 관계가 부드러워집니다.',
    deepeningMoment: '함께 지킬 원칙과 약속을 맞춰 갈 때 관계가 깊어져요. 서로의 신념을 존중하며 한 방향을 바라보는 순간, 항숙은 그 사람을 진짜 동지로 느낍니다.',
    anxietyPoint: '완벽해야 한다는 마음에 스스로를 가장 혹독하게 몰아세우기 쉬워요. 상대에게 건넬 다정한 위로를, 부디 자신에게 먼저 건네 주세요. 그럴 때 관계도 한결 편안해집니다.',
    longTermAdvice: '나와 생각이 다른 사람에게도 자리를 내어 주는 넉넉함을 길러 보세요. 원칙은 지키되 사람은 잃지 않는 지혜가, 항숙의 사랑을 가장 오래 지켜 줍니다.',
    avoidPattern: '옳고 그름을 따지느라 상대의 마음을 뒤로 미루는 일',
    moneyFlow: '스스로 길을 개척해 자수성가하는 힘이 있어요. 편법보다 정직한 방식으로 차근차근 기반을 다지죠. 그 뚝심을 믿고 원칙을 지키면, 느려도 무너지지 않는 재물이 쌓입니다.',
    spendingHabit: '충동보다는 신념에 따라 지갑을 여는 편이에요. 다만 옳다고 믿는 일에는 과감해져 부담이 될 때가 있죠. 큰 지출은 하루 정도 마음을 식힌 뒤 결정해 보시면 좋습니다.',
    savingPoint: '원칙을 세우면 끝까지 지키는 사람이라, 저축 규칙을 정해 두는 게 가장 잘 맞아요. ‘매달 얼마’를 원칙으로 정해 두시면, 항숙의 곧음이 그대로 든든한 자산이 됩니다.',
    workStrength: '기준을 세우고 원칙대로 밀고 나가는 일에 강해요. 독립적으로 방향을 정하거나 조직의 중심을 잡는 자리에서 타고난 지도력이 빛납니다.',
    workCaution: '내 기준이 절대적이라 여기면 함께 일하는 사람이 지칠 수 있어요. 방향은 분명히 하되, 방법은 서로 다를 수 있음을 열어 두시면 영향력이 더 커집니다.',
    collaborationTip: '원칙을 세우는 역할은 잘 맞지만, 다른 의견에도 귀를 열어 보세요. 나와 다른 방식도 존중해 줄 때, 항숙의 리더십이 두 배로 단단해집니다.'
  },
  3: {
    symbolLine: '동방 청룡의 심장 · 해(日)의 기운을 품은 하늘의 마부, 토끼(兔)의 별입니다.',
    archetype: '방숙은 어디서나 사람과 기회를 끌어당기는 사람이에요. 존재감과 협상력이 뚜렷해 현실의 성취를 만들어 내는 힘이 있죠. 다만 주도권이 강해질수록 상대의 자율성을 함께 살펴야 해, 앞으로 볼 성향도 모두 ‘판을 이끄는 카리스마’에서 비롯됩니다.',
    attractionType: '자신감 있고 자기 삶을 주도하는 사람에게 매력을 느껴요. 함께 있으면 더 큰 무대로 나아갈 것 같은 사람 곁에서 설렘이 커집니다.',
    loveStyle: '표현이 화려하고 상대를 이끄는 매력이 있어요. 다만 사랑을 소유로 붙들려 하면 상대가 답답해질 수 있죠. 마음을 쥐기보다 곁에 머물고 싶게 만들 때, 방숙의 매력이 가장 오래갑니다.',
    deepeningMoment: '함께 무언가를 이뤄 낼 때 관계가 깊어져요. 목표를 나란히 세우고 하나씩 성취해 가는 순간, 방숙은 그 사람을 든든한 동반자로 느낍니다.',
    anxietyPoint: '성공과 인정으로 사랑을 증명하려는 마음 뒤에는, 사실 버림받을까 하는 여린 마음이 있어요. 그 약함을 살짝 내보일 때 오히려 관계가 더 깊고 단단해집니다.',
    longTermAdvice: '소유하려는 마음을 조금 내려놓고 상대를 진심으로 믿어 보세요. 통제가 아니라 신뢰로 곁을 지킬 때, 방숙의 관계는 한계 없이 넓어집니다.',
    avoidPattern: '사랑을 소유욕으로 붙들며 상대를 통제하려는 일',
    moneyFlow: '큰 흐름을 읽고 기회를 잡는 감각이 뛰어나, 재물이 마르지 않는 편이에요. 다만 과시를 위한 지출이 새는 구멍이 되기 쉽죠. 남에게 보이기 위한 돈을 조금 줄이면, 그만큼 진짜 부가 쌓입니다.',
    spendingHabit: '좋아 보이는 것, 품격 있어 보이는 것에 지갑이 쉽게 열려요. 사기 전에 ‘나를 위한 건지, 보여 주기 위한 건지’ 한 번만 물어보세요. 그 한 마디가 지출을 크게 줄여 줍니다.',
    savingPoint: '큰 목표를 세우면 강하게 밀어붙이는 사람이라, 저축도 뚜렷한 목적과 연결하면 잘돼요. ‘무엇을 위한 돈’인지 이름을 붙여 모으시면 훨씬 즐겁게 쌓입니다.',
    workStrength: '사람을 움직이고 협상하는 일에 타고났어요. 비즈니스, 조직 운영, 판을 키우는 자리에서 특히 실력이 드러납니다. 권한과 책임을 나눌수록 성과가 커져요.',
    workCaution: '주도권이 강해지면 함께 일하는 사람의 자율성을 누르기 쉬워요. 결정을 내리기 전에 상대의 생각을 먼저 물어보면, 리더십이 한결 부드럽고 단단해집니다.',
    collaborationTip: '방향을 이끄는 힘은 좋지만, 협력자에게 권한을 나눠 주세요. 함께 만든 성과라고 느끼게 할 때, 방숙의 영향력이 오래도록 확장됩니다.'
  },
  5: {
    symbolLine: '동방 청룡의 꼬리, 불(火)의 기운 · 세상을 뒤흔드는 호랑이(虎)의 별입니다.',
    archetype: '미숙은 거대한 변화를 일으키기 위해 태어난 사람이에요. 어떤 시련도 오히려 의지를 다지는 연료로 삼는 강한 생명력이 있죠. 물러서지 않는 이 뜨거움이, 앞으로 볼 연애·돈·일 성향을 모두 관통합니다.',
    attractionType: '뜨겁게 몰입할 수 있는 사람, 함께 세상에 맞설 수 있는 사람에게 강하게 끌려요. 미지근한 관계보다 서로를 뜨겁게 불태우는 사랑에 마음이 움직입니다.',
    loveStyle: '사랑에 온몸으로 뛰어들어 뜨겁게 표현해요. 그 열정이 큰 매력이지만, 감정이 격해지면 부딪힘도 커지죠. 화가 오를 때 한 박자만 쉬어 가면, 그 열정이 상처가 아니라 힘이 됩니다.',
    deepeningMoment: '함께 어려움에 맞서 이겨 낸 순간에 관계가 뜨겁게 단단해져요. 나란히 시련을 넘고 나면, 미숙은 그 사람을 전우처럼 깊이 신뢰합니다.',
    anxietyPoint: '뜨거운 만큼 감정의 파고도 커서, 순간의 격함에 후회할 말을 쏟기 쉬워요. 욱하는 마음이 올라올 때는 잠시 자리를 뜨는 것만으로도 관계를 지킬 수 있습니다.',
    longTermAdvice: '모든 걸 정면 돌파하려는 마음을 가끔 부드럽게 다독여 보세요. 이기는 것보다 함께 가는 것을 택할 때, 미숙의 뜨거운 사랑이 오래 이어집니다.',
    avoidPattern: '감정이 격해진 순간에 날 선 말을 쏟아 내는 일',
    moneyFlow: '한번 마음먹으면 강하게 밀어붙여 크게 벌어들이는 힘이 있어요. 다만 뜨거운 기세에 크게 지르다 크게 잃기도 쉽죠. 큰 결정일수록 하루만 열기를 식힌 뒤 움직이면, 그 추진력이 든든한 부가 됩니다.',
    spendingHabit: '갖고 싶은 게 생기면 기세로 질러 버리기 쉬워요. 큰 지출 앞에서는 딱 하루만 미뤄 보세요. 열이 가라앉고 나면 정말 필요한 것만 또렷하게 남습니다.',
    savingPoint: '평소엔 저축이 뒷전이 되기 쉬우니, 버는 즉시 자동으로 떼어 두는 장치가 필요해요. 손에 쥐기 전에 먼저 빠져나가게 해 두면, 미숙에게도 목돈이 붙기 시작합니다.',
    workStrength: '어려운 일, 판을 뒤집어야 하는 일에 가장 강해요. 위기 상황이나 강한 추진력이 필요한 자리에서 진가를 발휘합니다. 남들이 못 넘는 벽을 넘는 걸 즐겨 보세요.',
    workCaution: '한번 불이 붙으면 쉼 없이 달리다 몸과 마음이 함께 지치기 쉬워요. 전력으로 달린 뒤에는 반드시 회복하는 시간을 넣어, 열정을 오래 쓸 수 있게 해 주세요.',
    collaborationTip: '추진력은 강하지만 속도가 빨라 동료가 따라오기 벅찰 수 있어요. 가끔 뒤를 돌아보며 보폭을 맞춰 주면, 미숙의 열정이 팀 전체의 힘이 됩니다.'
  },
  6: {
    symbolLine: '동방 청룡의 끝자락, 물(水)의 기운 · 바람을 부르는 키(箕)와 표범(豹)의 별입니다.',
    archetype: '기숙은 바람처럼 사람과 정보를 모으고 흩는 사람이에요. 사교적이고 감각이 빨라 재물과 인연을 끌어모으는 힘이 있죠. 다만 바람의 방향이 자주 바뀌듯 마음도 변하기 쉬워, 앞으로 볼 성향도 모두 ‘모으고 흩는 기질’에서 비롯됩니다.',
    attractionType: '대화가 즐겁고 새로운 세계를 열어 주는 사람에게 끌려요. 함께 있으면 지루할 틈 없이 통하는 사람 곁에서 마음이 활짝 열립니다.',
    loveStyle: '표현이 밝고 사교적이라 연애 초반이 즐거워요. 다만 흥미가 식으면 마음도 쉽게 바람을 타죠. 설렘이 잦아든 자리에 편안함이라는 깊이를 채워 가면, 사랑이 오래 머뭅니다.',
    deepeningMoment: '속 깊은 이야기를 나누며 마음이 통한다고 느낄 때 관계가 깊어져요. 가벼운 대화 너머의 진심을 나눌 때, 기숙은 그 사람에게 오래 머물고 싶어집니다.',
    anxietyPoint: '새로운 자극에 마음이 쉽게 흔들려 스스로도 불안해질 때가 있어요. 흔들릴 때는 관계를 바꾸기보다, 지금 곁의 사람과 새로움을 함께 만들어 보세요. 마음이 한결 단단해집니다.',
    longTermAdvice: '넓게 만나는 매력만큼, 한 사람에게 깊이 머무는 연습을 더해 보세요. 익숙함 속에서 재미를 찾아낼 때, 기숙의 사랑은 바람이 아니라 뿌리가 됩니다.',
    avoidPattern: '설렘이 식자마자 새로운 인연으로 눈을 돌리는 일',
    moneyFlow: '기회와 정보를 빠르게 포착해 돈을 끌어모으는 감각이 좋아요. 다만 들어온 만큼 여기저기로 흩어지기도 쉽죠. 들어온 돈의 일부를 흩기 전에 먼저 묶어 두면, 모으는 재능이 진짜 부로 남습니다.',
    spendingHabit: '사람들과 어울리고 새것을 경험하는 데 지갑이 쉽게 열려요. 즐거운 지출은 소중하지만, 한 달 놀이 예산을 미리 정해 두시면 마음 편히 즐기면서도 돈이 새지 않아요.',
    savingPoint: '한자리에 오래 두면 근질거리니, 목적별로 나눈 여러 통장이 잘 맞아요. ‘여행’ ‘비상금’처럼 이름을 붙여 나눠 모으면, 흩어지려는 돈이 재미있게 쌓입니다.',
    workStrength: '사람과 정보를 잇고 분위기를 살리는 일에 강해요. 영업, 기획, 네트워킹, 트렌드를 다루는 자리에서 특유의 감각이 빛납니다. 사람이 모이는 곳에서 힘이 나요.',
    workCaution: '관심이 여러 곳으로 흩어져 일이 끝맺음 없이 벌어지기 쉬워요. 오늘 꼭 마칠 하나를 먼저 정해 두면, 넓은 관심이 성과로 모입니다.',
    collaborationTip: '분위기를 살리는 힘은 좋지만, 약속과 마감은 흐트러지기 쉬워요. 맡은 몫의 기한을 눈에 보이게 적어 두면, 기숙의 사교성이 팀의 신뢰로 이어집니다.'
  },
  8: {
    symbolLine: '북방 현무의 처녀별, 흙(土)의 기운 · 어둠 속에서도 길을 아는 박쥐(蝠)의 별입니다.',
    archetype: '여숙은 작은 것도 꼼꼼히 챙기는 야무진 사람이에요. 손끝이 야물고 실속을 챙기는 감각이 뛰어나죠. 다만 세심함이 지나치면 스스로를 옥죄기 쉬워, 앞으로 볼 성향도 모두 ‘섬세하고 야무진 마음’에서 비롯됩니다.',
    attractionType: '허세 없이 성실하고, 작은 배려를 아는 사람에게 마음이 열려요. 화려한 말보다 꾸준한 정성에서 진짜 사랑을 느낍니다.',
    loveStyle: '상대를 세심하게 챙기며 알뜰히 사랑해요. 그 다정함이 큰 힘이지만, 기대만큼 돌아오지 않으면 서운함이 쌓이기 쉽죠. 바라는 마음을 속에 두지 말고 살며시 말로 전해 주세요.',
    deepeningMoment: '소소한 일상을 함께 챙기며 지낼 때 관계가 깊어져요. 작은 약속을 성실히 지켜 주는 사람에게, 여숙은 마음을 온전히 내어 줍니다.',
    anxietyPoint: '섬세한 만큼 상대의 작은 변화에 마음을 졸이기 쉬워요. 혼자 여러 의미를 재기보다, 궁금한 건 가볍게 물어보세요. 확인하고 나면 마음이 훨씬 편해집니다.',
    longTermAdvice: '상대를 챙기는 만큼 자신도 돌봐 주세요. 다 맞춰 주려 애쓰기보다 가끔 기대어도 괜찮다는 걸 허락할 때, 여숙의 사랑이 한결 여유로워집니다.',
    avoidPattern: '서운함을 속으로 재며 혼자 마음의 점수를 매기는 일',
    moneyFlow: '알뜰하게 관리하고 실속을 챙기는 데 타고났어요. 새는 돈을 잘 막아 안정적인 기반을 만들죠. 그 야무진 감각을 믿되, 너무 아끼다 나를 위한 몫까지 미루지 않으면 마음도 넉넉해집니다.',
    spendingHabit: '충동구매는 드물고 꼼꼼히 따져 사는 편이에요. 다만 아끼는 데 몰두하다 정작 필요한 순간을 놓치기도 하죠. 나를 위한 작은 사치 하나쯤은 예산에 넣어 두셔도 좋습니다.',
    savingPoint: '가계부와 잘 맞는 사람이라, 흐름을 눈으로 확인하면 저축이 즐거워져요. 고정 지출을 한번 정리하고 나면, 여숙의 야무짐이 그대로 목돈으로 쌓입니다.',
    workStrength: '꼼꼼함과 손재주가 필요한 일에 강해요. 정교한 실무, 관리, 디테일이 중요한 자리에서 특히 신뢰를 받습니다. 빈틈을 메우는 눈이 큰 자산이에요.',
    workCaution: '세부에 매몰되면 큰 그림을 놓치거나 스스로를 지치게 하기 쉬워요. 완벽을 조금 내려놓고 ‘충분히 좋음’에서 멈추는 연습이 여숙을 오래 일하게 합니다.',
    collaborationTip: '꼼꼼히 챙기다 혼자 다 떠안기 쉬워요. 맡길 일은 믿고 맡기고, 필요한 도움은 편하게 청해 보세요. 그럴 때 여숙의 세심함이 팀 전체를 든든하게 합니다.'
  },
  9: {
    symbolLine: '북방 현무의 빈 하늘, 해(日)의 기운 · 어둠 속을 민첩하게 살피는 쥐(鼠)의 별입니다.',
    archetype: '허숙은 남이 보지 못하는 깊은 곳을 들여다보는 사람이에요. 사색과 직관이 풍부해 영적인 감수성이 남다르죠. 다만 마음의 기복이 커 스스로도 종잡기 어려울 때가 있어, 앞으로 볼 성향도 모두 ‘깊고 흔들리는 내면’에서 비롯됩니다.',
    attractionType: '깊은 대화가 통하고 정신적으로 이어지는 사람에게 끌려요. 가벼운 만남보다, 함께 있으면 마음속까지 이해받는 듯한 사람 곁에서 편안합니다.',
    loveStyle: '조용하지만 깊게 사랑하며, 상대의 내면을 섬세하게 살펴요. 다만 기분의 파고가 커서 상대가 이유를 몰라 당황할 수 있죠. 마음의 날씨를 짧게라도 나눠 주시면 관계가 한결 편안해집니다.',
    deepeningMoment: '보이지 않던 속마음을 서로 꺼내 놓을 때 관계가 깊어져요. 남에게 안 하던 이야기를 나눌 수 있는 사람에게, 허숙은 온 마음을 엽니다.',
    anxietyPoint: '혼자만의 생각에 오래 잠기면 없던 불안까지 커지기 쉬워요. 마음이 가라앉을 때는 머릿속에 머물기보다, 밖으로 나가 몸을 움직이면 안개가 걷힙니다.',
    longTermAdvice: '기분에 관계가 휘둘리지 않도록, 나만의 리듬을 지키는 루틴을 만들어 보세요. 내면이 안정될 때 허숙의 깊이는 상대에게 큰 위로가 됩니다.',
    avoidPattern: '혼자 생각에 잠겨 감정을 관계 밖으로 흘려보내는 일',
    moneyFlow: '흐름을 읽는 직관이 좋아 뜻밖의 기회를 잡기도 해요. 다만 기분에 따라 돈 관리가 들쭉날쭉해지기 쉽죠. 감정과 무관하게 돌아가는 자동 관리 장치를 두면, 허숙의 재물이 안정됩니다.',
    spendingHabit: '마음이 허할 때 위로 삼아 지갑을 여는 경우가 많아요. 사기 전에 ‘지금 필요한 게 물건인지 마음인지’ 물어보세요. 그 한 번의 질문이 감정 지출을 크게 줄여 줍니다.',
    savingPoint: '눈에 안 보이면 관리가 흐트러지니, 잔액이 한눈에 보이는 방식이 잘 맞아요. 자동이체로 먼저 떼어 두면, 기복과 상관없이 꾸준히 쌓입니다.',
    workStrength: '통찰과 상상력이 필요한 일에 강해요. 연구, 기획, 창작, 상담처럼 깊이 파고드는 자리에서 남다른 감각이 빛납니다. 혼자 몰입할 시간을 확보해 주세요.',
    workCaution: '컨디션과 기분에 따라 능률의 편차가 커요. 잘 되는 시간대를 파악해 중요한 일을 그때 몰아 두면, 기복이 성과를 흔들지 못합니다.',
    collaborationTip: '속을 잘 안 드러내 오해를 사기 쉬워요. 생각을 짧게라도 공유하고, 컨디션이 안 좋은 날은 미리 알려 두면, 허숙의 깊이가 팀의 신뢰로 이어집니다.'
  },
  10: {
    symbolLine: '북방 현무의 지붕별, 달(月)의 기운 · 높이 날아오르는 제비(燕)의 별입니다.',
    archetype: '위숙(危宿)은 높은 이상을 품고 남다른 길을 걷는 사람이에요. 위태로워 보여도 그 위에서 아름다움을 만들어 내는 예술적 기질이 있죠. 다만 이상과 현실의 간극에 마음이 오르내리기 쉬워, 앞으로 볼 성향도 모두 ‘높이 나는 마음’에서 비롯됩니다.',
    attractionType: '평범함에 안주하지 않고 자기만의 세계가 있는 사람에게 끌려요. 함께 더 높은 곳을 꿈꿀 수 있는 사람 곁에서 마음이 설렙니다.',
    loveStyle: '사랑에 낭만과 이상을 담아 표현해요. 그 감성이 큰 매력이지만, 현실의 상대에게 이상을 겹쳐 실망하기도 쉽죠. 눈앞의 사람을 있는 그대로 바라볼 때, 사랑이 더 따뜻해집니다.',
    deepeningMoment: '같은 꿈과 가치를 나눌 때 관계가 깊어져요. 서로의 이상을 응원하며 나란히 날아오르는 순간, 위숙은 그 사람을 진짜 동반자로 느낍니다.',
    anxietyPoint: '기대가 높은 만큼 작은 어긋남에도 마음이 크게 흔들려요. 이상과 다르다고 실망하기 전에, 상대의 마음을 먼저 물어보세요. 대개 오해는 대화 한 번에 풀립니다.',
    longTermAdvice: '이상을 사랑하되 곁의 현실도 함께 안아 주세요. 완벽한 그림보다 지금의 사람을 소중히 여길 때, 위숙의 사랑이 오래 이어집니다.',
    avoidPattern: '이상과 다르다는 이유로 눈앞의 상대에게 실망하는 일',
    moneyFlow: '남다른 안목으로 뜻밖의 기회를 잡는 힘이 있어요. 다만 큰 꿈을 좇다 무리한 모험에 흔들리기도 쉽죠. 도전과 안전을 나눠, 잃어도 괜찮은 만큼만 실험하면 재능이 부로 이어집니다.',
    spendingHabit: '아름답고 특별한 것에 마음이 쉽게 열려요. 감성을 채우는 지출은 소중하지만, ‘가지고 싶은 것’과 ‘꼭 필요한 것’을 나눠 두면 씀씀이가 편안해져요.',
    savingPoint: '뚜렷한 꿈과 연결하면 저축에 힘이 붙어요. ‘이 돈으로 이루고 싶은 것’을 그려 두고 모으면, 위숙의 이상이 현실의 자산이 됩니다.',
    workStrength: '창의와 미감이 필요한 일에 강해요. 예술, 기획, 디자인, 새로운 시도가 필요한 자리에서 특유의 감각이 빛납니다. 남다른 시선을 마음껏 펼쳐 보세요.',
    workCaution: '이상이 높아 현실의 속도와 어긋나면 지치기 쉬워요. 큰 꿈을 작은 단계로 쪼개 하나씩 이루면, 이상이 좌절이 아니라 성취로 남습니다.',
    collaborationTip: '멋진 그림을 그리는 힘은 좋지만, 현실적인 실행은 동료와 나눠 보세요. 이상을 현실로 옮겨 줄 파트너와 함께할 때, 위숙의 비전이 완성됩니다.'
  },
  11: {
    symbolLine: '북방 현무의 집별, 불(火)의 기운 · 우직하게 곳간을 채우는 돼지(猪)의 별입니다.',
    archetype: '실숙은 튼튼한 기반을 쌓아 올리는 사람이에요. 근면하고 성실해 삶의 곳간을 차곡차곡 채우는 힘이 있죠. 다만 안정을 지키려는 마음이 강해 변화를 꺼리기 쉬워, 앞으로 볼 성향도 모두 ‘터를 다지는 기질’에서 비롯됩니다.',
    attractionType: '안정감 있고 책임감이 느껴지는 사람에게 마음이 열려요. 함께 미래를 그릴 수 있는, 믿음직한 사람 곁에서 편안함을 느낍니다.',
    loveStyle: '한번 마음을 주면 성실하게 오래 지켜 주는 편이에요. 그 든든함이 큰 힘이지만, 표현이 무뚝뚝해 마음이 전해지지 않을 때가 있죠. 다정한 말을 조금만 더해 주면 사랑이 훨씬 따뜻해집니다.',
    deepeningMoment: '함께 일상의 터전을 가꿔 갈 때 관계가 깊어져요. 소소한 미래를 나란히 계획하는 순간, 실숙은 그 사람을 평생의 가족으로 느낍니다.',
    anxietyPoint: '변화나 불확실함 앞에서 마음이 크게 불안해져요. 걱정을 혼자 끌어안기보다 상대와 나누면, 든든한 편이 곁에 있음을 확인하고 안심하게 됩니다.',
    longTermAdvice: '익숙함을 지키는 만큼, 가끔은 새로운 경험도 함께 시도해 보세요. 안정 속에 작은 변화를 더할 때, 실숙의 사랑이 지루해지지 않고 오래갑니다.',
    avoidPattern: '변화가 두려워 익숙한 자리에만 머무르려는 일',
    moneyFlow: '꾸준히 모으고 지키는 데 타고나, 시간이 지날수록 든든한 곳간이 채워져요. 큰 욕심보다 착실한 축적이 강점이죠. 그 성실함을 믿고 이어 가면, 실숙의 재물은 무너지지 않습니다.',
    spendingHabit: '충동구매는 드물고 실용을 따지는 편이에요. 다만 아끼는 데 익숙해 나를 위한 소비엔 인색해지기 쉽죠. 가끔은 스스로에게 보상을 허락해도 곳간은 넉넉합니다.',
    savingPoint: '저축은 이미 잘하시니, 이제 모은 돈이 스스로 일하게 해 보세요. 안전한 자산으로 한 칸만 옮기면, 실숙의 근면이 시간과 함께 불어납니다.',
    workStrength: '꾸준함과 안정성이 필요한 일에 강해요. 운영, 관리, 기반을 다지는 자리에서 오래도록 신뢰를 받습니다. 묵묵히 쌓아 올리는 힘이 큰 자산이에요.',
    workCaution: '익숙한 방식만 고집하면 변화의 흐름에 뒤처지기 쉬워요. 큰 변화가 아니어도 새 방식을 조금씩 받아들이면, 실숙의 성실함이 더 오래 빛납니다.',
    collaborationTip: '맡은 일을 든든히 해내지만, 다른 방식에는 마음을 잘 안 열기도 해요. 새로운 제안에 한 번 귀를 기울여 보면, 실숙의 안정감이 팀에 더 큰 힘이 됩니다.'
  },
  12: {
    symbolLine: '북방 현무의 벽별, 물(水)의 기운 · 깊은 물속을 지키는 유(貐)의 별입니다.',
    archetype: '벽숙은 지식과 기록을 차곡차곡 쌓아 두는 사람이에요. 학문과 문서를 다루는 감각이 남다르고, 속을 잘 드러내지 않는 신중함이 있죠. 이 ‘안으로 쌓는 기질’이 앞으로 볼 연애·돈·일 성향을 모두 관통합니다.',
    attractionType: '지적이고 속이 깊은 사람에게 마음이 끌려요. 말이 잘 통하고 배울 점이 있는 사람 곁에서 존경과 애정이 함께 자랍니다.',
    loveStyle: '조용하고 진중하게, 속으로 깊이 사랑하는 편이에요. 다만 마음을 잘 드러내지 않아 상대가 답답해할 수 있죠. 속에 담아 둔 마음을 조금씩 꺼내 보이면 관계가 한결 가까워집니다.',
    deepeningMoment: '생각과 가치를 깊이 나눌 때 관계가 깊어져요. 서로의 세계를 이해받는다고 느끼는 순간, 벽숙은 굳게 닫아 둔 마음의 문을 엽니다.',
    anxietyPoint: '속마음을 안으로만 담아 두다 혼자 오해를 키우기 쉬워요. 담아 두기 전에 짧게라도 말로 꺼내 보세요. 표현하고 나면 관계가 훨씬 투명해집니다.',
    longTermAdvice: '신중함은 지키되, 마음의 문을 조금 더 자주 열어 보세요. 감정을 나눌수록 벽숙의 깊이는 벽이 아니라 든든한 울타리가 됩니다.',
    avoidPattern: '속마음을 감춘 채 혼자 결론을 정리해 버리는 일',
    moneyFlow: '정보를 모으고 분석해 신중하게 굴리는 감각이 좋아요. 충동보다 계획으로 움직여 큰 실수가 적죠. 공부한 만큼 확신이 서면 움직이는 그 방식이, 벽숙에게 안정적인 부를 만들어 줍니다.',
    spendingHabit: '따져 보고 사는 편이라 낭비는 적어요. 다만 책·배움처럼 쌓아 두는 것엔 지갑이 쉽게 열리죠. 정말 활용할 것인지 한 번만 생각하면, 지식 지출도 알차게 남습니다.',
    savingPoint: '계획을 세우면 잘 지키는 사람이라, 저축 로드맵을 문서로 정리하면 잘 맞아요. 목표와 단계를 적어 두면, 벽숙의 치밀함이 그대로 자산이 됩니다.',
    workStrength: '지식과 자료를 다루는 일에 강해요. 연구, 기록, 분석, 전문 지식이 필요한 자리에서 깊이가 빛납니다. 파고들 수 있는 주제를 만나면 진가를 발휘해요.',
    workCaution: '혼자 파고들다 소통이 뜸해지면 오해를 사기 쉬워요. 진행 상황을 중간중간 공유하면, 벽숙의 전문성이 더 크게 인정받습니다.',
    collaborationTip: '아는 것을 속에만 두기보다 팀과 나눠 보세요. 지식을 열어 함께 쓸 때, 벽숙은 없어서는 안 될 든든한 기둥이 됩니다.'
  },
  13: {
    symbolLine: '서방 백호의 첫별, 나무(木)의 기운 · 무리를 이끄는 늑대(狼)의 별입니다.',
    archetype: '규숙은 지성과 개혁의 기운을 품은 사람이에요. 배움을 즐기고 낡은 것을 새롭게 바꾸려는 의지가 강하죠. 독립적이면서도 무리를 이끄는 늑대처럼, 이 ‘앞서 나가는 지성’이 앞으로 볼 성향을 모두 관통합니다.',
    attractionType: '생각이 깊고 함께 성장할 수 있는 사람에게 끌려요. 지적인 자극을 주고받을 수 있는 사람 곁에서 마음이 활짝 열립니다.',
    loveStyle: '솔직하고 주도적으로 사랑을 이끄는 편이에요. 다만 옳은 방향을 제시하려다 상대를 가르치는 느낌을 줄 수 있죠. 이끌기보다 나란히 걸을 때, 규숙의 사랑이 더 편안해집니다.',
    deepeningMoment: '함께 배우고 성장할 때 관계가 깊어져요. 서로를 더 나은 사람으로 이끌어 주는 순간, 규숙은 그 사람을 진짜 짝으로 느낍니다.',
    anxietyPoint: '독립심이 강해 힘들 때 오히려 혼자 감당하려 해요. 무리에서 떨어져 홀로 버티기보다, 곁의 사람에게 기대어 보세요. 나눌 때 관계가 더 단단해집니다.',
    longTermAdvice: '바꾸고 이끌려는 마음을, 있는 그대로 받아 주는 다정함과 섞어 보세요. 상대를 고치려 하지 않을 때, 규숙의 사랑이 오래 이어집니다.',
    avoidPattern: '옳은 방향을 앞세워 상대를 가르치려 드는 일',
    moneyFlow: '새로운 흐름을 빠르게 읽고 앞서 움직이는 감각이 좋아요. 다만 개혁적인 시도에 크게 걸다 흔들리기도 쉽죠. 앞서가되 안전한 축을 함께 두면, 규숙의 안목이 든든한 부가 됩니다.',
    spendingHabit: '배움과 자기계발엔 지갑이 잘 열려요. 성장을 위한 투자는 소중하지만, 다 활용할 수 있는지 살펴보면 더 알차게 쓰입니다.',
    savingPoint: '목표를 세우면 앞장서 밀고 나가니, 저축도 도전 과제처럼 만들면 잘돼요. ‘몇 개월 안에 얼마’ 같은 목표를 세우면, 규숙의 추진력이 자산으로 쌓입니다.',
    workStrength: '새로운 판을 짜고 개선하는 일에 강해요. 기획, 교육, 개혁이 필요한 자리에서 지성과 리더십이 빛납니다. 낡은 것을 바꾸는 자리를 찾아보세요.',
    workCaution: '앞서 나가다 팀의 속도를 앞질러 홀로 지치기 쉬워요. 방향을 나누고 함께 보폭을 맞추면, 규숙의 추진력이 모두의 힘이 됩니다.',
    collaborationTip: '이끄는 힘은 좋지만, 동료의 방식도 존중해 보세요. 다른 생각을 품을 여백을 둘 때, 규숙의 리더십이 더 넓게 인정받습니다.'
  },
  14: {
    symbolLine: '서방 백호의 별, 쇠(金)의 기운 · 무리를 불러 모으는 충직한 개(狗)의 별입니다.',
    archetype: '루숙은 사람을 모으고 이어 주는 성실한 사람이에요. 부지런하고 충직해 주변의 신뢰를 두텁게 쌓죠. 다만 남을 챙기느라 스스로를 뒤로 미루기 쉬워, 앞으로 볼 성향도 모두 ‘모으고 챙기는 마음’에서 비롯됩니다.',
    attractionType: '진실하고 성실한 사람에게 마음이 열려요. 말보다 행동으로 꾸준함을 보여 주는 사람 곁에서 깊은 신뢰를 느낍니다.',
    loveStyle: '헌신적이고 부지런하게 상대를 챙기는 편이에요. 그 정성이 큰 사랑이지만, 다 맞춰 주다 지치기도 쉽죠. 받는 것도 사랑의 일부임을 기억하고, 가끔은 기대어 보세요.',
    deepeningMoment: '함께 사람들과 어울리며 신뢰를 쌓아 갈 때 관계가 깊어져요. 곁을 든든히 지켜 주는 사람에게, 루숙은 온 마음으로 충실합니다.',
    anxietyPoint: '상대에게 소홀해질까 늘 마음을 졸이기 쉬워요. 다 챙기지 못해도 괜찮다고, 자신에게 먼저 말해 주세요. 마음의 짐을 덜면 사랑이 한결 가벼워집니다.',
    longTermAdvice: '주는 사랑에 익숙한 만큼, 바라는 것도 솔직히 표현해 보세요. 나를 돌보는 마음을 더할 때, 루숙의 헌신이 오래 지치지 않습니다.',
    avoidPattern: '혼자 다 챙기려다 자기 마음을 뒤로 미루는 일',
    moneyFlow: '성실하게 벌고 꾸준히 모으는 힘이 있어요. 다만 사람 좋아 남에게 베풀다 정작 내 몫이 새기 쉽죠. 나눔은 예산 안에서 하고, 내 저축을 먼저 챙기면 마음도 지갑도 든든해집니다.',
    spendingHabit: '자신보다 남을 위한 지출이 많은 편이에요. 베푸는 마음은 소중하지만, 관계 지출에도 한도를 정해 두면 부담 없이 오래 나눌 수 있습니다.',
    savingPoint: '매달 자동으로 떼어 두는 방식이 잘 맞아요. 남을 챙기기 전에 나를 위한 몫을 먼저 떼면, 루숙의 성실함이 그대로 자산으로 남습니다.',
    workStrength: '사람을 모으고 조율하며 성실히 완수하는 일에 강해요. 조직 관리, 커뮤니티, 팀을 잇는 자리에서 신뢰를 받습니다. 부지런함이 큰 무기예요.',
    workCaution: '부탁을 거절하지 못해 일이 과하게 쌓이기 쉬워요. 감당할 수 있는 선을 정하고 정중히 거절하는 연습이, 루숙을 지치지 않게 합니다.',
    collaborationTip: '궂은일을 도맡다 공을 놓치기 쉬워요. 해낸 몫을 자연스럽게 드러내고 도움도 청하면, 루숙의 성실함이 정당하게 인정받습니다.'
  },
  15: {
    symbolLine: '서방 백호의 곳간별, 흙(土)의 기운 · 풍요를 부르는 꿩(雉)의 별입니다.',
    archetype: '위숙(胃宿)은 현실 감각이 뛰어나 재물을 담아 두는 곳간 같은 사람이에요. 부지런하고 실속 있어 삶을 넉넉하게 채우는 힘이 있죠. 다만 채우는 데 몰두하다 마음을 놓치기 쉬워, 앞으로 볼 성향도 모두 ‘풍요를 담는 기질’에서 비롯됩니다.',
    attractionType: '든든하고 현실감 있는 사람에게 마음이 열려요. 함께 안정된 삶을 꾸릴 수 있는, 믿음직한 사람 곁에서 편안함을 느낍니다.',
    loveStyle: '넉넉하게 베풀고 상대를 실속 있게 챙기는 편이에요. 그 든든함이 큰 매력이지만, 물질로 마음을 대신하려 할 때도 있죠. 다정한 말과 시간도 함께 건네면 사랑이 더 깊어집니다.',
    deepeningMoment: '함께 풍요로운 일상을 가꿔 갈 때 관계가 깊어져요. 맛있는 것을 나누고 안정을 함께 쌓는 순간, 위숙은 그 사람을 진짜 내 사람으로 느낍니다.',
    anxietyPoint: '가진 것을 잃을까 하는 불안이 관계에도 스며들기 쉬워요. 물질의 안정만큼 마음의 안정도 챙겨 보세요. 곁의 사람을 믿을 때 그 불안이 잦아듭니다.',
    longTermAdvice: '베푸는 마음을 물질에만 담지 말고, 함께하는 시간에도 담아 보세요. 마음을 나누는 만큼, 위숙의 넉넉함이 더 따뜻하게 전해집니다.',
    avoidPattern: '마음의 표현을 물질로만 대신하려는 일',
    moneyFlow: '현실 감각과 축적의 재능이 뛰어나 재물이 잘 모여요. 벌고 담아 두는 힘이 강점이죠. 그 감각을 믿되, 쌓는 즐거움만큼 쓰는 기쁨도 누리면 삶이 한결 풍요로워집니다.',
    spendingHabit: '먹고 누리는 것엔 넉넉히 쓰지만, 실속 없는 지출은 잘 안 해요. 지금의 균형은 좋으니, 가끔 미래를 위한 투자에도 한 칸 나눠 두면 더 든든합니다.',
    savingPoint: '모으는 데 타고났으니, 이제 목적을 나눠 관리하면 좋아요. ‘생활·비상·투자’로 곳간을 나눠 두면, 위숙의 풍요가 더 단단해집니다.',
    workStrength: '현실적인 성과와 자원 관리가 필요한 일에 강해요. 재무, 운영, 실무를 다지는 자리에서 안정적으로 결과를 냅니다. 실속을 챙기는 눈이 큰 자산이에요.',
    workCaution: '눈앞의 안정에 머물면 더 큰 기회를 놓치기 쉬워요. 익숙한 곳간을 지키면서도, 가끔 새로운 시도에 한 발 내밀면 성장의 폭이 넓어집니다.',
    collaborationTip: '실속을 챙기는 힘은 좋지만, 손익만 따지면 사람을 잃을 수 있어요. 함께의 가치를 먼저 헤아리면, 위숙의 현실 감각이 팀의 든든한 버팀목이 됩니다.'
  },
  16: {
    symbolLine: '서방 백호의 별무리, 해(日)의 기운 · 어둠을 깨우는 닭(鷄)의 별입니다.',
    archetype: '묘숙은 총명하고 예리한 눈을 지닌 사람이에요. 작은 어긋남도 놓치지 않는 섬세함과 판단력이 강점이죠. 다만 그 날카로움을 자신에게 겨누면 스스로 힘들어지기 쉬워, 앞으로 볼 성향도 모두 ‘또렷하게 보는 눈’에서 비롯됩니다.',
    attractionType: '똑똑하고 감각이 있는 사람에게 끌려요. 대화가 예리하게 통하고 서로의 안목을 알아봐 주는 사람 곁에서 마음이 열립니다.',
    loveStyle: '상대를 섬세하게 살피며 세련되게 사랑해요. 다만 눈이 밝은 만큼 상대의 흠이 잘 보여 지적이 늘기 쉽죠. 좋은 점을 먼저 말해 주는 습관이 관계를 훨씬 부드럽게 합니다.',
    deepeningMoment: '서로의 안목과 취향을 알아봐 줄 때 관계가 깊어져요. 사소한 센스를 알아차려 주는 사람에게, 묘숙은 마음을 활짝 엽니다.',
    anxietyPoint: '완벽을 바라는 마음에 상대의 작은 실수도 크게 느껴지기 쉬워요. 지적하기 전에 한 박자 쉬며 ‘이게 정말 중요한가’ 물어보세요. 대개는 넘어가도 좋은 일입니다.',
    longTermAdvice: '날카로운 눈을 상대의 좋은 점을 찾는 데 써 보세요. 흠보다 매력을 먼저 볼 때, 묘숙의 섬세함이 사랑을 오래 지켜 줍니다.',
    avoidPattern: '상대의 작은 흠을 짚어 내며 자꾸 지적하는 일',
    moneyFlow: '분석력과 판단이 좋아 돈을 꼼꼼하게 관리해요. 감정보다 계산으로 움직여 큰 실수가 적죠. 그 예리함을 믿되, 너무 재다 좋은 기회를 놓치지 않도록 결단의 순간도 잊지 마세요.',
    spendingHabit: '따져 보고 사는 편이지만, 안목이 높아 좋은 것에는 지갑이 열려요. 품질을 보는 눈은 강점이니, 한 달 예산 안에서라면 마음껏 골라도 지갑이 흔들리지 않아요.',
    savingPoint: '숫자를 관리하는 감각이 좋으니, 지출을 항목별로 나눠 보면 저축이 즐거워져요. 흐름이 한눈에 보이면, 묘숙의 분석력이 그대로 자산으로 쌓입니다.',
    workStrength: '정확함과 분석이 필요한 일에 강해요. 검수, 기획, 편집, 품질 관리처럼 디테일이 중요한 자리에서 신뢰를 받습니다. 예리한 눈이 큰 무기예요.',
    workCaution: '완벽을 좇다 마감이 늦거나 스스로를 몰아세우기 쉬워요. ‘충분히 좋음’에서 멈추는 연습이, 묘숙을 지치지 않게 합니다.',
    collaborationTip: '지적이 잦으면 동료가 위축될 수 있어요. 개선점을 말하기 전에 잘한 점을 먼저 짚어 주면, 묘숙의 예리함이 팀을 성장시킵니다.'
  },
  17: {
    symbolLine: '서방 백호의 그물별, 달(月)의 기운 · 끈질기게 거두는 까마귀(烏)의 별입니다.',
    archetype: '필숙은 목표한 것을 그물로 거두듯 끝까지 손에 넣는 사람이에요. 실용적이고 끈기가 있어 원하는 결실을 반드시 챙기죠. 다만 붙들려는 마음이 강해 놓아야 할 때를 놓치기 쉬워, 앞으로 볼 성향도 모두 ‘거두어들이는 기질’에서 비롯됩니다.',
    attractionType: '든든하고 현실적인 사람에게 마음이 열려요. 함께 안정된 결실을 만들어 갈 수 있는 사람 곁에서 편안함을 느낍니다.',
    loveStyle: '한번 마음먹으면 끈기 있게 곁을 지키며 사랑해요. 그 우직함이 큰 힘이지만, 붙들려는 마음이 집착으로 비칠 수 있죠. 손에 쥐기보다 곁에 두는 여유가 사랑을 편안하게 합니다.',
    deepeningMoment: '함께 목표를 이루고 결실을 나눌 때 관계가 깊어져요. 곁을 오래 지켜 준 사람에게, 필숙은 흔들림 없는 신뢰를 줍니다.',
    anxietyPoint: '잃을까 하는 마음에 상대를 꽉 붙들기 쉬워요. 불안할수록 조이기보다 믿어 주세요. 여백을 줄 때 오히려 관계가 더 가까워집니다.',
    longTermAdvice: '거두는 힘만큼, 놓아 주는 여유도 길러 보세요. 붙들지 않아도 곁에 머물 거라 믿을 때, 필숙의 사랑이 오래 이어집니다.',
    avoidPattern: '잃을까 두려워 상대를 지나치게 붙드는 일',
    moneyFlow: '기회를 놓치지 않고 실속을 거두는 감각이 뛰어나요. 끈질기게 모아 든든한 기반을 만들죠. 그 힘을 믿되, 쥐기만 하지 말고 흐르게도 하면 재물이 더 건강하게 커집니다.',
    spendingHabit: '아끼고 모으는 데 강해 낭비는 적어요. 다만 아까운 마음에 필요한 지출까지 미루기도 하죠. 나를 위한 소비는 인색함이 아니라 투자임을 기억하시면 좋습니다.',
    savingPoint: '모으는 재능은 충분하니, 이제 모은 돈을 굴려 보세요. 안전한 자산으로 한 칸 옮기면, 필숙의 끈기가 시간과 함께 불어납니다.',
    workStrength: '끈기와 실행력이 필요한 일에 강해요. 영업, 수주, 성과를 거두는 자리에서 끝까지 결실을 만들어 냅니다. 포기하지 않는 힘이 큰 자산이에요.',
    workCaution: '한번 문 것을 놓지 못해 안 되는 일까지 붙들기 쉬워요. 물러설 때를 아는 것도 실력이니, 손절의 기준을 미리 정해 두면 좋습니다.',
    collaborationTip: '결과를 챙기는 힘은 좋지만, 성과에만 집중하면 사람을 놓칠 수 있어요. 과정을 함께 나누면, 필숙의 끈기가 팀의 든든한 추진력이 됩니다.'
  },
  18: {
    symbolLine: '서방 백호의 부리별, 불(火)의 기운 · 재치 넘치는 원숭이(猴)의 별입니다.',
    archetype: '자숙은 말과 재치로 세상과 소통하는 사람이에요. 지식을 전하고 사람을 웃기는 감각이 뛰어나죠. 다만 말이 앞서 마음보다 빠를 때가 있어, 앞으로 볼 성향도 모두 ‘말과 재치의 기질’에서 비롯됩니다.',
    attractionType: '말이 잘 통하고 지적인 대화가 즐거운 사람에게 끌려요. 함께 이야기하다 시간 가는 줄 모르는 사람 곁에서 마음이 열립니다.',
    loveStyle: '대화로 사랑을 키우고 재치로 분위기를 밝혀요. 그 유쾌함이 큰 매력이지만, 말이 가벼워 보이면 진심이 흐려질 수 있죠. 가끔은 농담을 접고 진지한 마음을 전해 보세요.',
    deepeningMoment: '깊은 대화로 마음이 통할 때 관계가 깊어져요. 생각을 주고받으며 서로를 이해하는 순간, 자숙은 그 사람에게 진심을 엽니다.',
    anxietyPoint: '말로는 술술 풀어도 속마음은 잘 못 꺼내기 쉬워요. 재치 뒤에 감정을 숨기지 말고, 진짜 마음을 짧게라도 전해 보세요. 그럴 때 관계가 더 단단해집니다.',
    longTermAdvice: '말의 재미만큼, 말을 지키는 무게도 더해 보세요. 가벼운 약속도 꾸준히 지킬 때, 자숙의 매력이 신뢰로 깊어집니다.',
    avoidPattern: '진심을 농담으로 덮어 마음을 흐리게 하는 일',
    moneyFlow: '정보가 빠르고 사람을 잇는 감각이 좋아 기회를 잘 잡아요. 다만 관심이 여기저기 흩어져 돈도 새기 쉽죠. 들어온 돈의 일부를 먼저 묶어 두면, 자숙의 재치가 실속으로 남습니다.',
    spendingHabit: '사람들과 어울리고 새로운 걸 경험하는 데 지갑이 쉽게 열려요. 즐거운 지출은 소중하니, 놀이 예산을 정해 두고 그 안에서 마음껏 누리면 씀씀이가 가벼워져요.',
    savingPoint: '한자리에 두면 지루해하니, 목적별로 나눈 통장이 잘 맞아요. 재미있는 이름을 붙여 나눠 모으면, 흩어지려는 돈이 즐겁게 쌓입니다.',
    workStrength: '말과 소통, 지식 전달이 필요한 일에 강해요. 교육, 방송, 마케팅, 상담처럼 말이 무기가 되는 자리에서 빛납니다. 사람을 설득하는 힘이 큰 자산이에요.',
    workCaution: '관심이 여러 곳으로 튀어 마무리가 흐려지기 쉬워요. 오늘 끝낼 하나를 정해 두면, 넓은 재능이 성과로 모입니다.',
    collaborationTip: '분위기를 살리는 힘은 좋지만, 말이 앞서면 신뢰가 얕아질 수 있어요. 한 약속이라도 끝까지 지키면, 자숙의 재치가 팀의 큰 힘이 됩니다.'
  },
  19: {
    symbolLine: '서방 백호의 세 별, 물(水)의 기운 · 거침없는 유인원(猿)의 별입니다.',
    archetype: '삼숙은 생각보다 몸이 먼저 움직이는 행동파예요. 경쟁을 두려워하지 않고 독립적으로 길을 개척하는 힘이 있죠. 다만 앞만 보고 달리다 주변을 놓치기 쉬워, 앞으로 볼 성향도 모두 ‘거침없는 실행력’에서 비롯됩니다.',
    attractionType: '함께 부딪히며 도전할 수 있는, 활력 있는 사람에게 끌려요. 미지근한 관계보다 서로를 자극하는 역동적인 사랑에 마음이 움직입니다.',
    loveStyle: '표현이 직진이고 행동으로 사랑을 보여 줘요. 그 시원함이 매력이지만, 속도가 빨라 상대가 벅찰 수 있죠. 상대의 보폭에 맞춰 주면 그 열정이 더 오래 통합니다.',
    deepeningMoment: '함께 도전하고 부딪혀 이겨 낼 때 관계가 깊어져요. 나란히 무언가를 해낸 순간, 삼숙은 그 사람을 든든한 파트너로 느낍니다.',
    anxietyPoint: '급한 마음에 감정을 앞세워 말이 거칠어지기 쉬워요. 욱하는 순간 잠시 멈추면, 관계를 지키는 여유가 생깁니다.',
    longTermAdvice: '이기고 앞서가려는 마음을, 함께 가는 마음과 섞어 보세요. 속도를 늦춰 곁을 돌아볼 때, 삼숙의 사랑이 오래 이어집니다.',
    avoidPattern: '급한 마음에 말과 행동을 먼저 내지르는 일',
    moneyFlow: '기회를 보면 과감히 뛰어들어 크게 버는 힘이 있어요. 다만 속도가 빨라 검토 없이 지르다 잃기도 쉽죠. 큰 결정은 하루만 늦춰 확인하면, 그 추진력이 든든한 부가 됩니다.',
    spendingHabit: '갖고 싶으면 바로 사 버리는 편이에요. 큰 지출 앞에서 하루만 미뤄 보세요. 열이 식고 나면 정말 필요한 것만 또렷하게 남습니다.',
    savingPoint: '평소엔 저축이 뒷전이 되기 쉬우니, 버는 즉시 자동으로 빠져나가게 해 두세요. 손에 쥐기 전에 떼면, 삼숙에게도 목돈이 붙습니다.',
    workStrength: '추진력과 실행이 필요한 일에 강해요. 영업, 현장, 경쟁이 치열한 자리에서 거침없이 결과를 냅니다. 먼저 부딪히는 힘이 큰 무기예요.',
    workCaution: '앞만 보고 달리다 몸도 관계도 지치기 쉬워요. 전력으로 달린 뒤엔 회복하는 시간을 넣어, 에너지를 오래 쓰게 해 주세요.',
    collaborationTip: '속도는 빠르지만 동료가 따라오기 벅찰 수 있어요. 가끔 보폭을 맞추고 과정을 나누면, 삼숙의 추진력이 팀의 힘이 됩니다.'
  },
  20: {
    symbolLine: '남방 주작의 우물별, 나무(木)의 기운 · 곧은 들개 안(犴)의 별입니다.',
    archetype: '정숙은 우물처럼 깊이 길어 올려 사람들과 나누는 사람이에요. 배움을 즐기고 정의감이 뚜렷해 지식을 베푸는 힘이 있죠. 다만 옳음을 앞세우다 뻣뻣해지기 쉬워, 앞으로 볼 성향도 모두 ‘길어 올려 나누는 기질’에서 비롯됩니다.',
    attractionType: '생각이 깊고 함께 배울 수 있는 사람에게 끌려요. 서로의 세계를 넓혀 주는 대화가 통하는 사람 곁에서 마음이 열립니다.',
    loveStyle: '진솔하고 헌신적으로, 아는 것과 마음을 아낌없이 나눠 사랑해요. 다만 옳고 그름을 따지다 상대를 가르치게 될 때가 있죠. 판단을 잠시 접고 마음을 먼저 들어 주면 관계가 부드러워집니다.',
    deepeningMoment: '함께 배우고 서로를 성장시킬 때 관계가 깊어져요. 지식과 마음을 나누며 넓어지는 순간, 정숙은 그 사람을 진짜 벗으로 느낍니다.',
    anxietyPoint: '옳다는 확신이 강해 다름을 틀림으로 여기기 쉬워요. 다르다고 밀어내기 전에, 그 사람의 이유를 물어보세요. 이해가 넓어지면 관계도 편해집니다.',
    longTermAdvice: '나누는 마음을 지키되, 상대를 있는 그대로 받아 주는 넉넉함을 더해 보세요. 가르치기보다 함께 배울 때, 정숙의 사랑이 오래갑니다.',
    avoidPattern: '옳음을 앞세워 다른 생각을 틀렸다 여기는 일',
    moneyFlow: '지식과 정보로 길을 찾아 착실히 벌어들이는 힘이 있어요. 충동보다 배움에 근거해 움직여 실수가 적죠. 아는 만큼 신중히 움직이는 그 방식이, 정숙에게 안정적인 부를 만들어 줍니다.',
    spendingHabit: '배움과 사람을 위한 지출엔 지갑이 잘 열려요. 나누는 마음은 소중하니, 예산 안에서라면 부담 없이 베풀며 흐름을 지킬 수 있습니다.',
    savingPoint: '계획을 세우면 잘 지키는 사람이라, 저축 규칙을 정해 두면 잘 맞아요. 목표와 단계를 적어 두면, 정숙의 성실함이 그대로 자산이 됩니다.',
    workStrength: '지식을 다루고 나누는 일에 강해요. 교육, 연구, 상담, 공공의 가치를 다루는 자리에서 신뢰를 받습니다. 배우고 전하는 힘이 큰 자산이에요.',
    workCaution: '원칙이 앞서면 융통성이 부족해 보일 수 있어요. 방향은 지키되 방법은 유연하게 열어 두면, 정숙의 신념이 더 넓게 통합니다.',
    collaborationTip: '아는 것을 나누는 힘은 좋지만, 다른 방식도 존중해 보세요. 함께 배우는 태도를 더할 때, 정숙은 팀의 든든한 스승이자 벗이 됩니다.'
  },
  21: {
    symbolLine: '남방 주작의 혼불별, 쇠(金)의 기운 · 온화한 양(羊)의 별입니다.',
    archetype: '귀숙은 보이지 않는 것을 느끼는 영적인 감수성이 깊은 사람이에요. 직관이 예민하고 사람을 보살피는 온화함이 있죠. 다만 남의 감정까지 떠안아 지치기 쉬워, 앞으로 볼 성향도 모두 ‘깊이 느끼고 보살피는 마음’에서 비롯됩니다.',
    attractionType: '따뜻하고 마음이 통하는, 정서적으로 안전한 사람에게 끌려요. 함께 있으면 마음이 편안해지는 사람 곁에서 사랑이 자랍니다.',
    loveStyle: '섬세하게 상대의 마음을 보살피며 헌신적으로 사랑해요. 그 다정함이 큰 힘이지만, 상대의 기분에 지나치게 물들기 쉽죠. 내 감정과 상대의 감정 사이에 선을 두면 사랑이 오래갑니다.',
    deepeningMoment: '서로의 여린 마음을 보듬어 줄 때 관계가 깊어져요. 안전하다고 느끼게 해 주는 사람에게, 귀숙은 온 마음을 내어 줍니다.',
    anxietyPoint: '상대의 작은 기분 변화에도 마음이 크게 흔들려요. 그 감정이 정말 내 것인지 한 번 짚어 보세요. 남의 감정과 나를 나누면 마음이 한결 가벼워집니다.',
    longTermAdvice: '보살피는 마음만큼 자신도 돌봐 주세요. 다 받아 주기보다 나를 지키는 선을 둘 때, 귀숙의 다정함이 지치지 않고 오래 이어집니다.',
    avoidPattern: '상대의 감정을 다 떠안아 내 마음까지 소진하는 일',
    moneyFlow: '직감으로 흐름을 느끼는 감각이 있어요. 다만 정에 이끌려 남의 부탁에 지갑을 열다 새기 쉽죠. 마음은 마음대로, 돈은 원칙대로 나누면 귀숙의 재물이 안정됩니다.',
    spendingHabit: '남을 위한 지출과 마음을 달래는 소비가 많은 편이에요. 베푸는 마음은 소중하니, 관계·위로 지출에 한도를 정해 두면 부담 없이 나눌 수 있습니다.',
    savingPoint: '감정과 무관하게 돌아가는 자동이체가 잘 맞아요. 먼저 떼어 두고 나머지로 생활하면, 마음의 기복과 상관없이 꾸준히 쌓입니다.',
    workStrength: '사람의 마음을 헤아리는 일에 강해요. 상담, 돌봄, 치유, 예술처럼 정서를 다루는 자리에서 특유의 따뜻함이 빛납니다. 공감의 힘이 큰 자산이에요.',
    workCaution: '감정 노동에 쉽게 소진되니, 나를 비우는 회복 시간이 꼭 필요해요. 마음을 재충전하는 루틴을 두면, 귀숙의 공감이 오래 유지됩니다.',
    collaborationTip: '남을 배려하다 정작 내 의견을 삼키기 쉬워요. 필요한 말은 부드럽게라도 꺼내 보세요. 그럴 때 귀숙의 다정함이 팀의 균형을 지켜 줍니다.'
  },
  22: {
    symbolLine: '남방 주작의 버들별, 흙(土)의 기운 · 여린 노루(獐)의 별입니다.',
    archetype: '류숙은 버들처럼 부드럽고 감성이 풍부한 사람이에요. 분위기를 살리고 사람을 편안하게 하는 매력이 있죠. 다만 마음이 여려 감정의 결에 쉽게 흔들려, 앞으로 볼 성향도 모두 ‘부드럽고 예민한 감성’에서 비롯됩니다.',
    attractionType: '분위기가 다정하고 정서적으로 통하는 사람에게 끌려요. 함께 있으면 마음이 몽글해지는 사람 곁에서 사랑이 피어납니다.',
    loveStyle: '감성적으로 깊이 사랑하며 분위기를 소중히 여겨요. 그 다정함이 매력이지만, 기분에 따라 마음이 오르내리기 쉽죠. 감정의 날씨를 상대에게 살짝 알려 주면 오해가 줄어듭니다.',
    deepeningMoment: '감성을 함께 나누고 분위기를 공유할 때 관계가 깊어져요. 마음의 결을 알아주는 사람에게, 류숙은 부드럽게 마음을 엽니다.',
    anxietyPoint: '작은 말투 하나에도 마음이 크게 출렁이기 쉬워요. 상처받았다 단정하기 전에, 상대의 진짜 뜻을 물어보세요. 확인하고 나면 마음이 훨씬 편해집니다.',
    longTermAdvice: '흔들리는 감정에 관계가 휘둘리지 않도록, 마음을 다독이는 나만의 방법을 만들어 보세요. 내면이 편안할 때 류숙의 다정함이 상대에게 위로가 됩니다.',
    avoidPattern: '기분의 파도에 휩쓸려 마음을 크게 출렁이는 일',
    moneyFlow: '분위기와 기분에 따라 씀씀이가 오르내리기 쉬워요. 기분 좋을 때 크게 쓰고 우울할 때 위로 삼아 쓰기도 하죠. 감정과 분리된 자동 관리를 두면, 류숙의 재물이 안정됩니다.',
    spendingHabit: '예쁘고 분위기 있는 것에 마음이 쉽게 열려요. 감성을 채우는 소비는 소중하니, 한 달 예산을 정해 두면 마음 편히 누리면서도 지갑이 편안해져요.',
    savingPoint: '눈에 보이면 관리가 잘되니, 잔액이 한눈에 보이는 방식이 좋아요. 자동이체로 먼저 떼어 두면, 기분과 상관없이 꾸준히 쌓입니다.',
    workStrength: '감성과 분위기를 다루는 일에 강해요. 디자인, 서비스, 콘텐츠처럼 정서를 어루만지는 자리에서 특유의 감각이 빛납니다. 사람을 편안하게 하는 힘이 자산이에요.',
    workCaution: '컨디션과 기분에 따라 능률의 편차가 커요. 마음이 좋은 시간대에 중요한 일을 몰아 두면, 기복이 성과를 흔들지 못합니다.',
    collaborationTip: '분위기를 살리는 힘은 좋지만, 감정이 드러나면 팀이 눈치를 볼 수 있어요. 마음 상태를 담담히 나누면, 류숙의 다정함이 팀을 부드럽게 합니다.'
  },
  23: {
    symbolLine: '남방 주작의 별, 해(日)의 기운 · 거침없이 달리는 말(馬)의 별입니다.',
    archetype: '성숙은 무대 위의 별처럼 빛나는 사람이에요. 표현력이 화려하고 존재감이 뚜렷해 사람들의 시선을 끄는 힘이 있죠. 다만 인정받고 싶은 마음이 커 시선에 흔들리기 쉬워, 앞으로 볼 성향도 모두 ‘빛나고 싶은 마음’에서 비롯됩니다.',
    attractionType: '나를 알아봐 주고 빛나게 해 주는 사람에게 끌려요. 함께 있으면 더 반짝이는 것 같은 사람 곁에서 마음이 설렙니다.',
    loveStyle: '표현이 화려하고 사랑을 밝게 드러내요. 그 매력이 사람을 끌지만, 시선을 즐기다 상대를 외롭게 할 수 있죠. 무대 밖에서도 상대에게 집중해 주면 사랑이 깊어집니다.',
    deepeningMoment: '무대 위가 아닌 민낯의 나를 보여 줄 때 관계가 깊어져요. 꾸미지 않은 모습을 받아 주는 사람에게, 성숙은 진짜 마음을 엽니다.',
    anxietyPoint: '인정받지 못한다고 느끼면 사랑도 흔들리기 쉬워요. 상대의 표현이 화려하지 않아도 마음이 없는 건 아니에요. 애정의 방식이 다를 뿐임을 기억해 주세요.',
    longTermAdvice: '밖의 박수만큼, 곁의 한 사람의 마음을 소중히 여겨 보세요. 무대를 내려와 함께 쉴 수 있을 때, 성숙의 사랑이 오래 빛납니다.',
    avoidPattern: '바깥의 시선을 좇느라 곁의 사람을 뒤로 미루는 일',
    moneyFlow: '자신을 빛내는 재능으로 돈을 끌어당기는 힘이 있어요. 다만 화려함을 위한 지출이 새는 구멍이 되기 쉽죠. 보여 주기 위한 돈을 조금 줄이면, 그만큼 진짜 부가 쌓입니다.',
    spendingHabit: '멋지고 돋보이는 것에 지갑이 쉽게 열려요. 사기 전에 ‘나를 위한 건지, 보이기 위한 건지’ 물어보세요. 그 한 마디가 지출을 크게 줄여 줍니다.',
    savingPoint: '뚜렷한 목표와 연결하면 저축에 힘이 붙어요. ‘무엇을 위한 돈’인지 그려 두고 모으면, 성숙의 열정이 즐겁게 자산으로 쌓입니다.',
    workStrength: '표현과 매력으로 사람을 끌어당기는 일에 강해요. 무대, 홍보, 브랜딩, 창작처럼 빛나야 하는 자리에서 진가를 발휘합니다. 존재감이 큰 무기예요.',
    workCaution: '주목받는 일엔 열정적이지만, 드러나지 않는 일은 소홀해지기 쉬워요. 보이지 않는 기본기를 챙기면, 성숙의 빛이 더 오래 지속됩니다.',
    collaborationTip: '스포트라이트를 즐기는 만큼, 공을 함께 나눠 보세요. 동료의 몫을 밝혀 줄 때, 성숙의 매력이 팀 전체를 빛나게 합니다.'
  },
  24: {
    symbolLine: '남방 주작의 확장별, 달(月)의 기운 · 너그러운 사슴(鹿)의 별입니다.',
    archetype: '장숙은 품을 넓게 벌려 사람과 풍요를 끌어안는 사람이에요. 너그럽고 사교적이라 곁에 사람이 모이는 힘이 있죠. 다만 베풀다 자기 몫을 놓치기 쉬워, 앞으로 볼 성향도 모두 ‘넓게 베푸는 기질’에서 비롯됩니다.',
    attractionType: '마음이 넓고 함께 있으면 편안한 사람에게 끌려요. 여유 있게 품어 주는 사람 곁에서 사랑이 자랍니다.',
    loveStyle: '넉넉하게 베풀고 상대를 편안하게 품어 사랑해요. 그 여유가 큰 매력이지만, 모두에게 다정해 상대가 특별함을 못 느낄 수 있죠. 그 사람만을 위한 표현을 더해 주면 사랑이 깊어집니다.',
    deepeningMoment: '함께 사람들과 어울리며 따뜻함을 나눌 때 관계가 깊어져요. 넓은 품 안에서 특별한 자리를 내어 줄 때, 상대는 장숙에게 온전히 마음을 엽니다.',
    anxietyPoint: '두루 잘 지내려다 정작 소중한 사람에게 소홀해지기 쉬워요. 모두를 챙기기 전에, 곁의 한 사람을 먼저 살펴보세요. 우선순위를 지킬 때 관계가 단단해집니다.',
    longTermAdvice: '넓게 베푸는 마음에, 깊게 집중하는 마음을 더해 보세요. 한 사람에게 온전히 머무를 때, 장숙의 너그러움이 사랑으로 완성됩니다.',
    avoidPattern: '두루 다정하려다 정작 소중한 이를 소홀히 하는 일',
    moneyFlow: '사람과 기회를 끌어모아 풍요를 만드는 힘이 있어요. 다만 인심 좋아 베풀다 정작 내 몫이 새기 쉽죠. 나눔은 예산 안에서 하고 내 저축을 먼저 챙기면, 장숙의 풍요가 오래 유지됩니다.',
    spendingHabit: '사람을 대접하고 함께 누리는 데 지갑이 넉넉히 열려요. 베푸는 기쁨은 소중하니, 관계 지출에 한도를 정해 두면 부담 없이 오래 나눌 수 있습니다.',
    savingPoint: '베풀기 전에 내 몫을 먼저 떼어 두는 습관이 필요해요. 매달 자동으로 저축이 빠져나가게 하면, 장숙의 너그러움과 재물이 함께 지켜집니다.',
    workStrength: '사람을 모으고 판을 키우는 일에 강해요. 네트워킹, 영업, 조직을 넓히는 자리에서 특유의 사교성이 빛납니다. 사람을 품는 힘이 큰 자산이에요.',
    workCaution: '벌여 놓은 관계와 일이 많아 관리가 흐트러지기 쉬워요. 넓힌 만큼 정리하는 시간을 두면, 장숙의 확장이 성과로 단단히 남습니다.',
    collaborationTip: '두루 챙기는 힘은 좋지만, 핵심에 집중하는 것도 필요해요. 가장 중요한 관계와 일에 힘을 모으면, 장숙의 사교성이 팀의 큰 자산이 됩니다.'
  },
  25: {
    symbolLine: '남방 주작의 날개별, 불(火)의 기운 · 매혹적인 뱀(蛇)의 별입니다.',
    archetype: '익숙은 날개를 펼쳐 자기만의 세계를 표현하는 예술가 같은 사람이에요. 감각이 남다르고 자유로운 상상이 풍부하죠. 다만 이상이 높아 현실과 부딪히면 마음이 지치기 쉬워, 앞으로 볼 성향도 모두 ‘높이 나는 예술혼’에서 비롯됩니다.',
    attractionType: '감각이 있고 자기 세계가 뚜렷한 사람에게 끌려요. 함께 아름다움과 상상을 나눌 수 있는 사람 곁에서 마음이 설렙니다.',
    loveStyle: '사랑을 예술처럼 감각적으로 표현해요. 그 매력이 사람을 사로잡지만, 이상이 높아 현실의 상대에게 실망하기도 쉽죠. 눈앞의 사람을 있는 그대로 볼 때 사랑이 따뜻해집니다.',
    deepeningMoment: '감성과 아름다움을 함께 나눌 때 관계가 깊어져요. 자기만의 세계를 이해받는다고 느끼는 순간, 익숙은 그 사람에게 마음을 엽니다.',
    anxietyPoint: '기대가 높아 작은 어긋남에도 마음이 크게 흔들려요. 이상과 다르다고 실망하기 전에 상대의 마음을 물어보세요. 대개 오해는 대화로 풀립니다.',
    longTermAdvice: '이상을 사랑하되 현실의 상대도 함께 안아 주세요. 완벽한 그림보다 지금의 사람을 소중히 여길 때, 익숙의 사랑이 오래 날아오릅니다.',
    avoidPattern: '이상과 다르다는 이유로 눈앞의 상대에게 실망하는 일',
    moneyFlow: '남다른 감각으로 창의적인 기회를 만들어 내는 힘이 있어요. 다만 이상을 좇다 현실적인 관리가 뒤로 밀리기 쉽죠. 창작과 살림의 몫을 나눠 두면, 익숙의 재능이 안정적인 부로 이어집니다.',
    spendingHabit: '아름답고 감각적인 것에 마음이 쉽게 열려요. 영감을 채우는 지출은 소중하니, ‘가지고 싶은 것’과 ‘꼭 필요한 것’을 나눠 두면 마음도 지갑도 가벼워져요.',
    savingPoint: '뚜렷한 꿈과 연결하면 저축에 힘이 붙어요. ‘이 돈으로 만들고 싶은 것’을 그려 두고 모으면, 익숙의 이상이 현실의 자산이 됩니다.',
    workStrength: '창의와 표현이 필요한 일에 강해요. 예술, 디자인, 기획, 콘텐츠처럼 상상을 펼치는 자리에서 진가를 발휘합니다. 남다른 감각을 마음껏 펼쳐 보세요.',
    workCaution: '이상이 높아 현실의 속도와 어긋나면 지치기 쉬워요. 큰 꿈을 작은 단계로 쪼개 하나씩 이루면, 이상이 좌절이 아니라 성취로 남습니다.',
    collaborationTip: '멋진 상상을 그리는 힘은 좋지만, 현실적인 실행은 동료와 나눠 보세요. 꿈을 현실로 옮겨 줄 파트너와 함께할 때, 익숙의 예술혼이 완성됩니다.'
  },
  26: {
    symbolLine: '남방 주작의 수레별, 물(水)의 기운 · 묵묵히 땅을 일구는 지렁이(蚓)의 별입니다.',
    archetype: '진숙은 묵묵히 실리를 챙기며 삶을 치밀하게 굴려 가는 사람이에요. 근면하고 관리에 빈틈이 없어 착실히 결실을 만들죠. 다만 신중함이 지나쳐 마음을 여는 데 시간이 걸려, 앞으로 볼 성향도 모두 ‘치밀하고 신중한 기질’에서 비롯됩니다.',
    attractionType: '진실하고 한결같아 믿을 수 있는 사람에게 마음이 열려요. 겪어 보며 신뢰가 쌓일 때 비로소 깊은 애정을 내어 줍니다.',
    loveStyle: '천천히, 그러나 한번 열면 깊고 성실하게 사랑해요. 다만 경계가 풀리기까지 시간이 걸려 상대가 서운해할 수 있죠. 마음을 여는 중이라는 걸 살며시 알려 주면 관계가 편안해집니다.',
    deepeningMoment: '오랜 시간 곁을 지키며 신뢰가 쌓일 때 관계가 깊어져요. 의심이 풀리고 나면, 진숙은 그 사람에게 흔들림 없는 사랑을 줍니다.',
    anxietyPoint: '상처받을까 하는 마음에 상대를 자꾸 의심하기 쉬워요. 확신이 설 때까지 재기보다, 작은 신뢰부터 건네 보세요. 믿음을 연습할수록 관계가 편해집니다.',
    longTermAdvice: '신중함은 지키되, 마음의 문을 조금 더 일찍 열어 보세요. 의심을 내려놓고 믿어 줄 때, 진숙의 성실한 사랑이 오래 이어집니다.',
    avoidPattern: '상처가 두려워 상대를 자꾸 의심하고 재는 일',
    moneyFlow: '단돈 한 푼도 허투루 쓰지 않는 치밀한 관리로 재물을 서서히 키워요. 새는 곳을 꼼꼼히 막는 힘이 강점이죠. 그 관리력을 믿고 이어 가면, 진숙의 재물은 조용히 크게 불어납니다.',
    spendingHabit: '계획 없는 지출은 거의 없고 꼼꼼히 따져 써요. 다만 너무 아끼다 나를 위한 순간을 놓치기도 하죠. 가끔의 작은 보상은 낭비가 아니라 재충전임을 기억하세요.',
    savingPoint: '관리에 타고났으니, 이제 모은 돈을 굴려 보세요. 안전한 장기 자산으로 한 칸 옮기면, 진숙의 치밀함이 시간과 함께 든든히 불어납니다.',
    workStrength: '치밀한 관리와 실무가 필요한 일에 강해요. 재무, 운영, 물류처럼 빈틈없이 굴려야 하는 자리에서 신뢰를 받습니다. 꼼꼼한 관리력이 큰 자산이에요.',
    workCaution: '신중함이 지나치면 결정이 늦거나 사람을 못 믿어 혼자 떠안기 쉬워요. 작은 일부터 믿고 맡기면, 진숙의 치밀함이 더 큰 성과로 이어집니다.',
    collaborationTip: '혼자 관리하려다 소통이 뜸해지기 쉬워요. 진행 상황을 나누고 사람을 믿어 보세요. 신뢰를 열 때, 진숙의 성실함이 팀의 든든한 기반이 됩니다.'
  }
};
 ...
 * @property {{moneyFlow:string,spendingHabit:string,savingPoint:string,workStrength:string,workCaution:string,collaborationTip:string,talent:number,talentLabel:string}} workMoney
...
function syBuildBasicReading(canonicalData, sData, daily, guardian) {
  ...
  var wealthTokens = syCanonicalTokenize(traits.wealth, '작은 절약과 큰 집중이 돈의 흐름을 지켜줍니다.');
  ...
  var guardianName = guardian && guardian.name ? guardian.name : '달빛 지표';
  ...
  var reading = {
    ...
    relationMiniMap: [
      ...
      },
      {
        ...
        guide: '역할, 기대치, 결정권을 먼저 나누면 긴장이 성장 동력으로 바뀝니다.',
        tone: 'axis'
      },
      {
    ...
    workMoney: {
      moneyFlow: syReadingOverrideByIndex(mansionIndex, 'moneyFlow') || syComposeFromTokens(wealthTokens, '수입과 지출의 기준을 가볍게 나누면 돈의 흐름이 안정됩니다.'),
      spendingHabit: spendingHabitText,
      savingPoint: syReadingOverrideByIndex(mansionIndex, 'savingPoint') || syComposeFromTokens(syCanonicalTokenize((ritual.action || '') + '. ' + (traits.work || ''), '고정 지출을 먼저 정리하면 저축 속도가 빨라집니다.'), '고정 지출을 먼저 정리하면 저축 속도가 빨라집니다.'),
      workStrength: syReadingOverrideByIndex(mansionIndex, 'workStrength') || syComposeFromTokens(workTokens, '역할을 분명히 하면 성과가 안정됩니다.'),
      workCaution: workCautionText,
      collaborationTip: collaborationTipText,
      talent: Number(sData.talent) || 80,
      talentLabel: syReferenceBand(Number(sData.talent) || 80)
    },
    emotionRhythm: {
      morning: '아침 참고 구간 · ' + syReferenceBand(morningScore) + ' · ' + (morningScore >= 75 ? '차분한 집중이 잘 붙는 시간입니다.' : '시동이 느릴 수 있어 루틴 워밍업이 필요합니다.') + ' 추천: 가장 작은 할 일 1개를 먼저 완료하세요.',
      day: '낮 참고 구간 · ' + syReferenceBand(dayScore) + ' · ' + (dayScore >= 75 ? '실행력과 판단력이 안정되는 구간입니다.' : '결정 피로가 올라오기 쉬운 시간대입니다.') + ' 추천: 우선순위 1개만 고정하고 나머지는 보류하세요.',
      night: '밤 참고 구간 · ' + syReferenceBand(nightScore) + ' · ' + (nightScore >= 75 ? '감정 공감과 대화가 부드럽게 열립니다.' : '감정 파고가 커질 수 있어 정리 대화가 유리합니다.') + ' 추천: 결론보다 의도 확인 질문을 먼저 하세요.',
...
function syRenderRelationMiniMapCard(reading, options) {
  ...
  var wrapperOpen = embedded
    ...
    : (inline
      ? '<section class="sy-relation-guide-inline" id="syRelationGuideMap" data-sy-relation-guide-map="basic" data-active-relation="' + syCanonicalEsc(activeLabel) + '">'
      : '<div class="sy-card sy-relation-guide-card" id="syRelationGuideMap" data-sy-relation-guide-map="basic" data-active-relation="' + syCanonicalEsc(activeLabel) + '">');
  var wrapperClose = (embedded || inline) ? '</section>' : '</div>';
  ...
  return wrapperOpen
    ...
    + nodes.map(function(node) {
      var nodeKey = syNormalizeRelationMiniMapLabel(node.label);
      var isActive = activeLabel && nodeKey === activeLabel;
      return '<article class="sy-mini-node sy-relation-node' + (isActive ? ' is-active' : '') + '" data-tone="' + syCanonicalEsc(node.tone) + '" data-sy-relation-node="' + syCanonicalEsc(nodeKey) + '">'
...
function syRenderCanonicalDashboard(canonicalPayload, reading) {
  ...
  var rawDisplayIndex = Number(natalInfo.displayIndex);
  var rawMansionIndex = Number(natalInfo.index);
  var indexText = Number.isFinite(rawDisplayIndex)
    ? String(rawDisplayIndex)
    : (Number.isFinite(rawMansionIndex) ? String(rawMansionIndex + 1) : '미확인');
  var directionText = natalInfo.direction || '미확인';
  var elementText = natalInfo.element || '미확인';
  var phaseLabel = phase.phaseName || '미확인';
  var illum = phase.rhythmLight || (phase.illumination == null ? '미확인' : String(phase.illumination) + '%');
  var angle = phase.rhythmPosition || (phase.elongationAngle == null ? '미확인' : String(phase.elongationAngle) + '도');
  var waxingRaw = phase.waxingOrWaning || '미확인';
  var waxing = waxingRaw === 'waxing' ? '차오름' : (waxingRaw === 'waning' ? '기울음' : waxingRaw);
  ...
  var relationshipRhythm = syCanonicalList(attrs.relationshipStyle, '관계 성향 정보 없음');
  var careerRhythm = syCanonicalList(attrs.careerStyle, '커리어 성향 정보 없음');
  var wealthRhythm = syCanonicalList(attrs.wealthStyle, '돈 습관 정보 없음');
  var recoveryRhythm = syCanonicalList(attrs.recoveryPattern, '회복 패턴 정보 없음');
  ...
  var integratedHeroSubtitle = reading && reading.hero && reading.hero.subtitle
    ...
    : '태어난 날의 본명숙으로 성향, 인연, 감정 리듬을 읽어드립니다.';
  ...
  var integratedGuardianLabel = reading && reading.guardian
    ? String(reading.guardian.name || '달빛 지표')
    : '달빛 지표';
  ...
  return ''
    ...
    + '<div class="sy-canon-overline">기본 숙요점 · 27숙 기준</div>'
    + '<h4>월하의 기본 숙요점</h4>'
    ...
    + '<span class="sy-canon-pill">27숙 순번 ' + syCanonicalEsc(indexText) + '/27</span>'
    + '<span class="sy-canon-pill">오늘 참고 리듬 ' + syCanonicalEsc(phaseLabel) + '</span>'
    + '<span class="sy-canon-pill">' + syCanonicalEsc(integratedMoonLabel) + '</span>'
    + '<span class="sy-canon-pill">월하의 숙요 동물 ' + syCanonicalEsc(integratedGuardianLabel) + '</span>'
    + '</div>'
    + '</div>'
    ...
    + '</div>'
    + '<div class="sy-canon-panel active" id="syCanonPanelOverview" role="tabpanel" aria-labelledby="syCanonTabOverview" data-sycanon-panel="overview">'
    + '<div class="sy-canon-grid">'
    + '<div class="sy-canon-kv"><span>본명숙</span><strong>' + syCanonicalEsc(mansionLabel) + '</strong></div>'
    + '<div class="sy-canon-kv"><span>27숙 순번</span><strong>' + syCanonicalEsc(indexText) + ' / 27</strong></div>'
    ...
    + '<article><h5>관계 성향</h5>' + renderBulletList(relationshipRhythm) + '</article>'
    + '<article><h5>일 성향</h5>' + renderBulletList(careerRhythm) + '</article>'
    + '<article><h5>돈 습관</h5>' + renderBulletList(wealthRhythm) + '</article>'
    + '<article><h5>회복 리듬</h5>' + renderBulletList(recoveryRhythm) + '</article>'
    + '</div>'
    + '</div>'
    + '<div class="sy-canon-panel" id="syCanonPanelMoon" role="tabpanel" aria-labelledby="syCanonTabMoon" data-sycanon-panel="moon">'
    + '<div class="sy-canon-grid">'
    + '<div class="sy-canon-kv"><span>오늘 참고 리듬</span><strong>' + syCanonicalEsc(phaseLabel) + '</strong></div>'
    + '<div class="sy-canon-kv"><span>상징 밝기</span><strong>' + syCanonicalEsc(illum) + '</strong></div>'
    + '<div class="sy-canon-kv"><span>상징 위치</span><strong>' + syCanonicalEsc(angle) + '</strong></div>'
    + '<div class="sy-canon-kv"><span>리듬 흐름</span><strong>' + syCanonicalEsc(waxing) + '</strong></div>'
    ...
    + '<div class="sy-canon-kv"><span>참고 키</span><strong>' + syCanonicalEsc(interpretationKey) + '</strong></div>'
    + '</div>'
    + '</div>'
    + '</div>';
}
...
function renderSukuyo(p, natal, bazi, lunarObj, canonicalPayload, sourceProfile) {
    ...
    if (!document.getElementById('sy-main-style')) {
        ...
        .sy-container::before { content:''; position:absolute; top:-70px; right:-40px; width:236px; height:236px; border-radius:999px; background:radial-gradient(circle at 36% 30%, rgba(255,255,255,0.99) 0%, rgba(240,244,252,0.92) 22%, rgba(214,226,246,0.7) 44%, rgba(191,219,254,0.26) 63%, transparent 78%); box-shadow:0 0 58px rgba(219,234,254,0.32), 0 0 120px rgba(167,139,250,0.14); pointer-events:none; z-index:0; animation:syMoonDrift 8s ease-in-out infinite; }
        ...
        .sy-lunar-rim::before { content:''; position:absolute; left:-12%; right:-12%; top:9%; height:1px; background:linear-gradient(90deg, transparent, rgba(248,231,183,0.36), rgba(219,234,254,0.22), transparent); transform:rotate(-7deg); }        .sy-lunar-rim i,.sy-lunar-rim b { position:absolute; width:5px; height:5px; border-radius:999px; background:#f8fafc; box-shadow:0 0 12px rgba(219,234,254,0.72); }
        ...
        .sy-header::before { content:''; position:absolute; left:50%; top:-42px; width:96px; height:96px; transform:translateX(-50%); border-radius:999px; background:radial-gradient(circle at 38% 32%, rgba(255,255,255,0.82), rgba(219,234,254,0.36) 42%, transparent 68%); opacity:0.26; pointer-events:none; animation:syHeaderMoonGlow 5.8s ease-in-out infinite; }
        ...
        .sy-intro-card { animation:syIntroFade 0.7s ease both; }
        .sy-intro-head { display:flex; align-items:center; gap:12px; margin-bottom:12px; }
        .sy-intro-icon { flex:0 0 auto; width:46px; height:46px; display:flex; align-items:center; justify-content:center; font-size:1.5rem; border-radius:999px; background:radial-gradient(circle at 34% 30%, rgba(255,255,255,0.22), rgba(191,219,254,0.1) 60%, transparent 100%); border:1px solid rgba(219,234,254,0.3); box-shadow:0 0 20px rgba(219,234,254,0.14); }
        .sy-intro-eyebrow { margin:0; font-size:0.72rem; letter-spacing:0.14em; color:#a5b4fc; }
        .sy-intro-title { margin:3px 0 0; font-size:1.16rem; color:#f1f5ff; font-weight:600; line-height:1.4; }
        .sy-intro-title b { background:linear-gradient(135deg, #f8fafc, #f8e7b7 55%, #c4b5fd); -webkit-background-clip:text; -webkit-text-fill-color:transparent; background-clip:text; }
        .sy-intro-symbol { margin:0 0 9px; font-size:0.9rem; color:#f8e7b7; line-height:1.7; }
        .sy-intro-archetype { margin:0; font-size:0.94rem; line-height:1.85; color:#e2e8f0; }
        @keyframes syIntroFade { from { opacity:0; transform:translateY(8px); } to { opacity:1; transform:none; } }
        ...
        .sy-ntab-glyph--growth { background:radial-gradient(circle at 30% 30%, rgba(254,243,199,0.96) 0%, rgba(251,191,36,0.78) 45%, rgba(180,83,9,0.5) 100%); }
        ...
        .daily-flow-val { width: 62px; text-align: right; color:#fbbf24; font-weight: bold; font-size:0.82rem;}
        ...
        @keyframes syMoonVeil { 0%,100% { transform:translate(-50%, -50%) scale(1); opacity:0.24; } 50% { transform:translate(-52%, -54%) scale(1.08); opacity:0.45; } }
        @keyframes syGuardianGleam { 0%,100% { filter:brightness(1); opacity:0.88; } 50% { filter:brightness(1.25); opacity:1; } }
        @keyframes syGuardianPulse { 0%,100% { box-shadow:0 0 26px rgba(196,181,253,0.26), inset 0 0 30px rgba(196,181,253,0.14); } 50% { box-shadow:0 0 40px rgba(196,181,253,0.42), inset 0 0 40px rgba(196,181,253,0.22); } }
        @keyframes syTinyTwinkle { 0%,100% { opacity:0.35; transform:scale(0.75); } 50% { opacity:0.95; transform:scale(1.15); } }
        .sy-guardian-card { margin-top:14px; border-left-color:#c4b5fd; position:relative; overflow:hidden; border-radius:18px; border:1px solid rgba(196,181,253,0.45); background: radial-gradient(circle at 16% 10%, rgba(199,210,254,0.2) 0%, rgba(30,32,55,0.92) 44%, rgba(18,20,40,0.95) 100%); box-shadow:0 18px 52px rgba(2,6,23,0.55), inset 0 0 24px rgba(196,181,253,0.16); }
        .sy-guardian-card::before { content:''; position:absolute; right:-48px; top:-56px; width:190px; height:190px; background:radial-gradient(circle, rgba(196,181,253,0.26) 0%, transparent 72%); pointer-events:none; animation:syMoonVeil 7.8s ease-in-out infinite; }
        .sy-guardian-card::after { content:''; position:absolute; left:-26px; bottom:-24px; width:150px; height:150px; border-radius:999px; background:radial-gradient(circle at 40% 38%, rgba(147,197,253,0.22) 0%, transparent 68%); pointer-events:none; }
        .sy-guardian-head { font-size:0.88rem; text-transform:uppercase; letter-spacing:0.11em; color:#f5d0fe; font-weight:800; margin-bottom:10px; text-shadow:0 0 12px rgba(196,181,253,0.44); }
        .sy-guardian-stage { border:1px solid rgba(196,181,253,0.4); border-radius:16px; padding:15px 13px; background:linear-gradient(165deg, rgba(15,23,42,0.84), rgba(27,34,57,0.72)); text-align:center; position:relative; overflow:hidden; animation:syGuardianPulse 8s ease-in-out infinite; }
        .sy-guardian-stage::before { content:''; position:absolute; left:50%; top:50%; width:160px; height:160px; transform:translate(-50%, -50%); border-radius:999px; border:1px solid rgba(196,181,253,0.32); opacity:0.5; background:radial-gradient(circle at 50% 50%, rgba(255,255,255,0.14), transparent 62%); pointer-events:none; }
        .sy-guardian-stage::after { content:''; position:absolute; inset:-34% auto auto -8%; width:240px; height:240px; background:radial-gradient(circle, rgba(251,191,36,0.1) 0%, transparent 58%); pointer-events:none; }
        ...
        .sy-guardian-main-emoji { font-size:2.8rem; line-height:1; margin-bottom:9px; filter:drop-shadow(0 0 14px rgba(196,181,253,0.36)); animation:syMoonBob 2.8s ease-in-out infinite, syGuardianGleam 5.4s ease-in-out infinite; }
        .sy-guardian-main-name { font-size:1.26rem; letter-spacing:0.015em; font-weight:900; color:#fef3c7; margin-bottom:6px; text-shadow:0 0 10px rgba(251,191,36,0.35); }
        .sy-guardian-main-desc { margin:0 0 12px; font-size:0.96rem; line-height:1.84; color:#dbeafe; }
        .sy-guardian-detail-list { margin-top:10px; display:grid; grid-template-columns:repeat(2,minmax(0,1fr)); gap:10px; }
        .sy-guardian-detail-item { padding:10px 12px; border-radius:12px; border:1px solid rgba(147,197,253,0.24); background:linear-gradient(170deg, rgba(15,23,42,0.84), rgba(11,18,32,0.66)); }
        .sy-guardian-detail-item h5 { margin:0 0 5px 0; font-size:0.7rem; letter-spacing:0.07em; text-transform:uppercase; color:#93c5fd; }
        .sy-guardian-detail-item p { margin:0; font-size:0.84rem; line-height:1.87; color:#dbeafe; }
        ...
        .sy-guardian-art-board { margin-top:12px; text-align:left; border:1px dashed rgba(196,181,253,0.48); border-radius:12px; padding:11px 11px; background:linear-gradient(155deg, rgba(30,41,59,0.72), rgba(17,24,39,0.34)); }
        .sy-guardian-art-board h5 { margin:0 0 6px 0; font-size:0.74rem; letter-spacing:0.08em; text-transform:uppercase; color:#ddd6fe; }
        .sy-guardian-art-board p { margin:0 0 8px 0; font-size:0.84rem; line-height:1.78; color:#dbeafe; }
        .sy-guardian-art-prompt { width:100%; min-height:120px; border-radius:12px; border:1px solid rgba(196,181,253,0.35); background:rgba(3,7,18,0.64); color:#eef2ff; padding:11px 12px; font-size:0.79rem; line-height:1.68; resize:vertical; box-sizing:border-box; font-family:"Pretendard","Noto Sans KR",sans-serif; }
        .sy-guardian-art-copy-btn { width:100%; cursor:pointer; border:none; border-radius:10px; padding:9px 10px; font-size:0.79rem; font-weight:800; color:#0f172a; background:linear-gradient(140deg, #fde68a, #fbcfe8); border:1px solid rgba(251,191,36,0.42); transition:transform .2s ease, box-shadow .2s ease; }
        .sy-guardian-art-copy-btn:hover { transform:translateY(-1px); box-shadow:0 10px 18px rgba(251,191,36,.28); }
        ...
        .sy-paid-card-head { display:flex; justify-content:space-between; align-items:flex-start; gap:12px; margin-bottom:9px; flex-wrap:wrap; }
        ...
        .sy-relation-guide-card { border-left-color:#60a5fa; background:linear-gradient(150deg, rgba(9,18,38,0.96), rgba(17,24,48,0.94) 62%, rgba(24,27,52,0.92)); }
        .sy-relation-guide-card--embedded { margin:0 0 14px; border:1px solid rgba(147,197,253,0.32); border-radius:16px; padding:15px; box-shadow:inset 0 1px 0 rgba(255,255,255,0.06), 0 16px 34px rgba(2,6,23,0.28); }
        .sy-relation-guide-inline { margin-top:14px; padding-top:14px; border-top:1px solid rgba(244,114,182,0.22); }
        .sy-relation-guide-head { display:flex; align-items:flex-start; justify-content:space-between; gap:12px; margin-bottom:7px; flex-wrap:wrap; }
        .sy-relation-guide-eyebrow { color:#93c5fd; font-size:0.68rem; letter-spacing:0.04em; font-weight:900; margin-bottom:3px; }
        .sy-relation-guide-head h4 { margin:0; color:#dbeafe; font-size:1.02rem; line-height:1.38; }
        ...
        @media (max-width: 768px) {
          ...
          .sy-guardian-head { font-size:0.82rem; }
          .sy-guardian-main-name { font-size:1.11rem; }
          .sy-guardian-main-emoji { font-size:2.42rem; }
          .sy-guardian-main-desc { font-size:0.92rem; line-height:1.78; }
          .sy-guardian-detail-list { grid-template-columns:1fr; }
          .sy-guardian-detail-item { padding:11px 12px; }
          .sy-guardian-detail-item p { font-size:0.83rem; line-height:1.78; }
          .sy-guardian-art-prompt { min-height:112px; }
        ...
        @media (max-width: 430px) {
          ...
          .sy-guardian-main-name { font-size:1rem; }
          .sy-guardian-main-emoji { font-size:2.05rem; }
          .sy-guardian-detail-item h5 { font-size:0.68rem; }
          .sy-guardian-art-copy-btn { font-size:0.75rem; padding:8px 10px; }
    ...
    html += `<div class="sy-lunar-rim" aria-hidden="true"><i></i><b></b></div>`;
    html += `<div class="sy-header"><h3>☽ 월하의 숙요점 · Lunar Nexus ☾</h3><p style="font-size: 0.82rem; opacity: 0.72; margin-top:6px; letter-spacing:0;">본명숙의 결을 27숙 리듬으로 정리해, 달빛 아래에서 성향과 관계 흐름을 한 번 더 조용히 열어봅니다.</p></div>`;
    ...
    window._syLastSukuyoBasicResult = sData ? {
      ...
      displayIndex: Number.isFinite(Number(sData.mansionIdx)) ? Number(sData.mansionIdx) + 1 : null,
      icon: sData.icon,
      talent: sData.talent,
      traits: sData.traits || {},
    ...
    var _selfGuardian = (function() {
      if (!sData || typeof syResolveMansionGuardian !== 'function') return { name: '달빛 지표', emoji: '✨' };
    ...
    if (sData) {
      var _introTr = sData.traits || {};
      var _introSymbol = syReadingOverrideByIndex(sData.mansionIdx, 'symbolLine');
      var _introArchetype = syReadingOverrideByIndex(sData.mansionIdx, 'archetype')
        || syEnsureSentenceEnding(String(_introTr.desc || _introTr.core || '').trim());
      html += `<div class="sy-card sy-intro-card" style="border-left-color:#e5edff;">
        <div class="sy-intro-head">
          <span class="sy-intro-icon" aria-hidden="true">${syCanonicalEsc(sData.icon || '🌙')}</span>
          <div>
            <p class="sy-intro-eyebrow">당신의 본명숙</p>
            <h4 class="sy-intro-title">당신은 <b>${syCanonicalEsc(sData.mansion || '')}</b> 입니다</h4>
          </div>
        </div>
        ${_introSymbol ? `<p class="sy-intro-symbol">${syCanonicalEsc(_introSymbol)}</p>` : ''}
        <p class="sy-intro-archetype">${syCanonicalEsc(_introArchetype)}</p>
      </div>`;
    ...
    if (sData) {
        ...
        if (reading) {
          ...
          _dailySectionHtml = '<div class="sy-card" style="margin-top:15px; border-left-color:#818cf8;">'
            + '<h4 style="margin: 0 0 4px 0; color: #818cf8;">오늘의 참고 리듬</h4>'
            ...
            + '<p style="margin:6px 0 10px; color:#a5b4fc; font-size:0.78rem; line-height:1.6;">날짜와 본명숙을 함께 본 상징 참고 흐름입니다. 실제 결정은 현실 조건을 함께 확인하세요.</p>'
            ...
            + renderGauge('감정 리듬', daily.love, 'linear-gradient(90deg,#f43f5e,#fb923c)')
            + renderGauge('돈 습관 점검', daily.wealth, 'linear-gradient(90deg,#eab308,#fde68a)')
            ...
            + '</div>'
            + '<div style="font-size:0.78rem; color:#9ca3af; text-transform:none; letter-spacing:0; margin-top:14px; margin-bottom:6px;">달빛 실천 루틴</div>'
    ...
    if (_dailySectionHtml) {
      ...
      const _guardianStory = (function() {
        if (typeof syBuildGuardianStory !== 'function') {
          return {
          overview: '월하의 숙요 동물은 본명숙의 기질을 부드럽게 보완하고, 선택의 호흡을 정돈하는 안내 표지로 읽힙니다.',
            reason: '숙요 해석의 핵심은 본명숙이며, 이 상징은 오늘의 정서 흐름을 덧붙인 참고 안내입니다. 지금의 상징은 감정과 판단의 균형을 회복하라는 신호에 가깝습니다.',
            traits: '이 상징은 마음의 소란을 가라앉히고, 중요한 갈림길에서 본능과 이성을 같은 방향으로 정렬시키는 역할을 합니다. 조용한 장면에서 더 선명하게 작동합니다.',
            spirit: '영적인 기운은 꿈의 상징, 갑작스러운 예감, 반복되는 장면처럼 은근한 신호로 도착합니다. 조급함을 내려놓을수록 그 신호는 더 선명하고 따뜻해집니다.',
            ritual: '잠들기 전 조명을 낮추고 오늘의 감정 한 줄을 기록한 뒤 3번 깊게 호흡해 보세요. 짧은 반복이 마음의 결을 정돈하고 다음 선택을 맑게 합니다.'
          };
        }
      ...
      html += `<div class="sy-card sy-guardian-card">
        ...
        <div class="sy-guardian-stage">
          ...
          <div class="sy-guardian-detail-list">
            ...
            <div class="sy-guardian-detail-item">
              <h5>나의 월하의 숙요 동물</h5>
              <p>상징 색: ${_guardianView.color} · 상징 장소: ${_guardianView.place}<br>오늘의 행동: ${_guardianView.action}<br>정렬 문장: ${_guardianView.mantra}</p>
            </div>
          </div>
    ...
    setTimeout(() => {
        ...
        if (canonicalRoot) {
          canonicalRoot.querySelectorAll('.sy-canon-tab').forEach(btn => {
            btn.addEventListener('click', () => {
              ...
              canonicalRoot.querySelectorAll('.sy-canon-tab').forEach(t => {
        ...
        document.querySelectorAll('.sy-ntab').forEach(btn => {
          btn.addEventListener('click', () => {
            ...
            if (!panelRoot) return;
            panelRoot.querySelectorAll('.sy-ntab').forEach(b => {
        ...
        if (guardianArtCopyBtn && guardianArtPromptEl) {
          guardianArtCopyBtn.addEventListener('click', function() {
            syCopyText(guardianArtPromptEl.value || '').then(function() {
              if (guardianArtStatusEl) {
                guardianArtStatusEl.style.color = '#86efac';
                guardianArtStatusEl.textContent = '월하의 숙요 동물 프롬프트가 복사되었습니다. AI에 붙여넣어 사용하세요.';
              }
            }).catch(function() {
              if (guardianArtStatusEl) {
                guardianArtStatusEl.style.color = '#fda4af';
                guardianArtStatusEl.textContent = '복사에 실패했습니다. 텍스트를 직접 선택해 복사해 주세요.';
              }
            });
          });
        }
      ...
      if (D === 0) {
        ...
        base = {
          ...
          prescription: [
            ...
            '상대의 단점을 발견할 때, 그것이 내 안의 반응과도 연결되는지 함께 살펴보세요. 그 지점에서 관계의 조율이 시작됩니다.'
          ],
          shadow: '나의 약한 반응까지 또렷하게 비추는 관계입니다. 서로를 지나치게 동일시하면 경계가 흐려질 수 있습니다.'
        };
        ...
        base = {
          ...
          prescription: [
            ...
            '감정이 과열되기 전, 24시간의 물리적 거리를 규칙으로 삼으세요. 식힌 뒤 대화할 때 이 관계의 배움이 더 분명해집니다.'
          ],
          shadow: isAn
  ...
  var SY_RELATION_STORY_COPY = {
    ...
    ankai: {
      ...
      development: '가까워질수록 서로의 약점과 욕망을 빠르게 건드립니다. 잘 다루면 성장의 계기가 되고, 놓치면 소모전이 될 수 있습니다.',
  ...
  var SY_PURPOSE_COMPAT_COPY = {
    yeongchin: {
      love: '연애에서는 편안함과 성장감이 함께 살아납니다. 순간적인 자극보다 믿음이 쌓이면서 마음이 깊어지는 구조입니다.',
  ...
  function syBuildSukuyoExpertFinal(ctx) {
    ...
    var relationAdvice = {
      ankai: '이 인연은 자극의 강도가 높지만, 경계선이 없으면 상처도 빠르게 깊어집니다. 사랑을 시험하지 말고 감정의 속도를 관리해야 합니다.',
      ...
      taegeuk: '이 인연은 오래된 약속처럼 느껴질 수 있습니다. 인연감은 선명하지만 현실의 작은 행동으로 증명해야 길이 열립니다.',
  ...
  function syBuildSukuyoAnnualMonthlyReading(canonicalData, sData, daily, sourceProfile) {
    ...
    return {
      ...
      profileId: profileId,
      targetYear: year,
      localMansion: sData && sData.mansion ? String(sData.mansion) : '본명숙',
      sourceProfile: sourceProfile || null
    };
  }

  function syRenderSukuyoScoreBar(score, color) {
  ...
  function syResolveSukuyoYearlyTargetYear(reading) {
    var input = document.querySelector('[data-sy-yearly-input]');
    var raw = input ? String(input.value || '').trim() : String((reading && reading.targetYear) || new Date().getFullYear()).trim();
    var year = Number(raw);
    if (!/^\d{4}$/.test(raw) || !Number.isInteger(year) || year < 1900 || year > 2100) {
      window.alert('숙요점 1년운은 1900년부터 2100년까지의 연도만 입력할 수 있습니다.');
      return null;
    }
    return year;
  }
    ...
    return '<div class="sy-lunar-month-grid">'
      + months.map(function(item, idx) {
        ...
        return '<article class="sy-lunar-month-item">'
          + '<div class="sy-lunar-month-head"><strong class="sy-lunar-month-title"><span class="sy-lunar-phase">' + syCanonicalEsc(moonMarks[idx % moonMarks.length]) + '</span><span>' + syCanonicalEsc(item.month) + '월 · ' + syCanonicalEsc(item.title) + '</span></strong><span class="sy-lunar-band" style="color:' + color + ';">' + syCanonicalEsc(item.score) + '점</span></div>'
          + syRenderSukuyoScoreBar(item.score, 'linear-gradient(90deg,#10b981,#86efac)')
          + '<p>' + syCanonicalEsc(item.theme) + '</p>'
          + '<p class="sy-lunar-note sy-lunar-note--relation">기회 · ' + syCanonicalEsc(item.opportunity) + '</p>'
          + '<p class="sy-lunar-note sy-lunar-note--work">실행 · ' + syCanonicalEsc(item.action) + '</p>'
  ...
  function syFetchSukuyoYearlyJson(path, options) {
    if (typeof fetchJsonWithAuth === 'function') return fetchJsonWithAuth(path, options || {});
  ...
  function syRenderSukuyoYearlyLocked(payload) {
    var preview = payload && payload.preview ? payload.preview : {};
    var theme = preview.yearlyTheme || {};
    var summary = preview.profileSummary || {};
    var keywords = Array.isArray(theme.keywords) ? theme.keywords : [];
    var sample = Array.isArray(preview.monthlyPreview) ? preview.monthlyPreview : [];
    return '<div class="sy-month-lock-panel">'
      + '<div class="sy-month-lock-orbit" aria-hidden="true">☾</div>'
      + '<div class="sy-month-lock-body">'
        + '<strong>' + syCanonicalEsc(theme.title || '숙요점 1년운 전체 해석') + '</strong>'
        + '<p>' + syCanonicalEsc(theme.summary || '올해의 월별 흐름과 사랑, 일, 금전, 건강운은 잠금 해제 후 이어집니다.') + '</p>'
  ...
  function syRenderSukuyoYearlyFull(result) {
    if (!result) return '';
    ...
    return ''
      ...
      + '<div style="display:flex;flex-wrap:wrap;gap:6px;margin-top:9px;">' + (Array.isArray(theme.keywords) ? theme.keywords : []).map(function(key) { return '<span class="sy-paid-status is-unlocked">' + syCanonicalEsc(key) + '</span>'; }).join('') + '</div>'
      + '</div>'
      ...
      + '<article><strong>전체 총운</strong><span>' + syCanonicalEsc(result.totalFortune && result.totalFortune.score) + '점</span><p>' + syCanonicalEsc(result.totalFortune && result.totalFortune.text) + '</p></article>'
      ...
      + domains.map(function(pair) {
        var item = pair[1] || {};
        return '<article><strong>' + syCanonicalEsc(pair[0]) + '</strong><span>' + syCanonicalEsc(item.score) + '점</span><p>' + syCanonicalEsc(item.text) + '</p><p>' + syCanonicalEsc(item.advice) + '</p></article>';
      }).join('')
  ...
  function syBuildEnhancedSukuyoResult(ctx) {
    ...
    var tempBand = temp <= 30 ? '차분한 인연' : (temp <= 50 ? '천천히 데워지는 인연' : (temp <= 70 ? '따뜻한 현실형 인연' : (temp <= 85 ? '선명한 끌림의 인연' : '강한 끌림의 인연')));
  ...
  const SY_GUARDIAN_PROFILE_MAP = Object.freeze({
    ...
    '오소리': { core: '끈기와 현실 대응력', resonance: '느리더라도 꾸준히 밀어붙일 때 결과가 크게 돌아옵니다.', traits: '한번 맡은 일은 끝까지 완주하며, 예상치 못한 변수에서도 생존 전략을 빠르게 찾습니다.', spirit: '대지 기운이 강해 생활 패턴, 체력, 재정 관리처럼 기반을 다질수록 판단의 안정감이 커집니다.', ritual: '오늘 해야 할 일 중 가장 작은 단위 하나를 즉시 완료하면 추진력이 정렬됩니다.' },
    ...
    '독수리': { core: '원대한 목표와 돌파력', resonance: '크게 보고 과감히 결단하는 순간에 흐름이 크게 열립니다.', traits: '목표 중심성이 강하고 결정적 순간에 과감한 실행으로 판을 뒤집습니다.', spirit: '강한 태양-풍기 결합으로 리더십과 성취 에너지를 끌어올립니다.', ritual: '오늘의 최우선 목표 1개를 선언하고 가장 어려운 단계부터 시작하면 기운이 정렬됩니다.' },
    ...
    '달빛 지표': { core: '직감 회복과 감정 정돈', resonance: '복잡한 정보를 잠시 멈추고 감각을 믿을 때 길이 열립니다.', traits: '미세한 기류를 읽어 불필요한 소모를 줄이고, 자신만의 리듬을 빠르게 되찾습니다.', spirit: '은은한 달빛 기운이 불안을 낮추고 내면 나침반을 다시 켭니다.', ritual: '눈을 감고 10회 심호흡 후 오늘 가장 중요한 한 가지를 적어보세요.' },
    'default': { core: '내면 나침반 정렬', resonance: '감정과 행동의 간격을 줄일수록 흐름이 안정됩니다.', traits: '현재 상황의 핵심을 식별하고 필요 없는 소음을 정리하는 힘이 활성화됩니다.', spirit: '달의 리듬이 직감과 관계 감수성을 높여 중요한 신호를 놓치지 않게 돕습니다.', ritual: '짧은 호흡 명상과 감사 한 줄 기록으로 마음의 결을 안정화할 수 있습니다.' }
  });
  ...
  function syResolveMansionGuardian(mansionText, mansionIdx) {
    ...
    return { name: '달빛 지표', emoji: '✨' };
  }

  function syBuildGuardianStory(guardian, mansionText) {
    var guardianName = guardian && guardian.name ? String(guardian.name) : '달빛 지표';
    ...
    var spirit = profile.spirit || '달의 상징 리듬이 직감과 관계 감수성을 깨워 중요한 신호를 살피게 돕습니다.';
    var ritual = profile.ritual || '짧은 호흡 명상과 감사 한 줄 기록으로 마음의 결을 안정화할 수 있습니다.';

    return {
      overview: guardianName + ' 상징은 달의 결 속에서 본명숙의 기질을 보완하는 안내 표지입니다. 지금 시기에는 특히 ' + baseCore + '의 힘을 끌어올려 감정의 물결을 가라앉히고 선택의 타이밍을 바로잡아 줍니다.',
      reason: '숙요 해석의 핵심은 본명숙이며, 월하의 숙요 동물은 오늘의 참고 리듬을 덧붙여 읽는 안내입니다. ' + mansionLabel + '의 흐름에서는 ' + guardianName + ' 상징이 감정과 판단의 균형을 회복하라는 신호로 읽힙니다. ' + resonance,
      traits: guardianName + ' 상징의 본성은 화려함보다 정확함에 있습니다. ' + traits + ' 평소에는 잔잔하게 머물지만, 중요한 인연과 결정의 갈림길에서는 감지력이 먼저 반응해 선택의 방향을 좁혀 줍니다.',
      spirit: '이 상징의 기운은 큰 목소리보다 미세한 결로 다가옵니다. ' + spirit + ' 특히 새벽 직전의 꿈, 반복해서 마주치는 상징, 이유 없는 직감의 끌림은 내면이 보내는 안내 신호일 가능성이 큽니다.',
      ritual: '상징의 흐름을 일상에 잇고 싶다면 작은 의식을 꾸준히 반복하는 것이 좋습니다. ' + ritual + ' 같은 시간대에 짧게라도 이어가면 마음의 결이 안정되고 선택 신호가 더 빠르고 선명해집니다.'
    };
  }
  ...
  function buildSukuyoAiPromptTemplate(payload) {
    ...
    return [
      ...
      '4. [주의해야 할 순간]: 갈등이 과열되거나 오해가 생기기 쉬운 특정 상황.',
  ...
  function syPromptHasAnyKeyword(text, keywords) {
  ...
  function syInferSukuyoPromptDomain(question, preferCompatibility) {
    var text = String(question || '').toLowerCase();
    var relationContext = ['상대', '그 사람', '애인', '전남친', '전여친', '연인', '배우자', '부부', '썸', '이별', '헤어진', '연락', '관계'];
    if (syPromptHasAnyKeyword(text, ['재회', '다시 만나', '다시 연락', '돌아올', '돌아오'])) return 'reconciliation';
    if (syPromptHasAnyKeyword(text, ['회복', '다시']) && syPromptHasAnyKeyword(text, relationContext)) return 'reconciliation';
    if (syPromptHasAnyKeyword(text, ['이별', '헤어', '정리', '끝내'])) return 'breakup';
    if (syPromptHasAnyKeyword(text, ['건강', '멘탈', '불안', '스트레스', '수면', '회복력', '몸', '마음'])) return 'health';
    if (syPromptHasAnyKeyword(text, ['돈', '재물', '수입', '투자', '저축', '매출', '재정', '자산'])) return 'money';
    if (syPromptHasAnyKeyword(text, ['직업', '진로', '이직', '커리어', '직장', '승진', '사업', '업무', '일자리'])) return 'career';
    if (syPromptHasAnyKeyword(text, ['성격', '기질', '강점', '약점', '본성', '패턴', '성향'])) return 'personality';
    if (syPromptHasAnyKeyword(text, ['인생', '방향', '미래', '전환', '선택', '다시 일어', '앞으로'])) return 'life_direction';
    if (preferCompatibility || syPromptHasAnyKeyword(text, ['궁합', '상대', '인연', '관계 유형'])) return 'compatibility';
    if (syPromptHasAnyKeyword(text, ['연애', '결혼', '썸', '사랑'])) return 'love';
    if (syPromptHasAnyKeyword(text, ['인간관계', '가족', '친구', '동료', '갈등'])) return 'relationship';
    return 'life_direction';
  }

  function syPromptDomainNeedsCompatibility(domain) {
    var key = String(domain || '').trim();
    return key === 'compatibility' || key === 'reconciliation' || key === 'breakup';
  }
  ...
  function syRequestSukuyoPromptByQuestion(question, options) {
    ...
    var domain = String(opts.domain || syInferSukuyoPromptDomain(question, !!opts.preferCompatibility)).trim();
    if (syPromptDomainNeedsCompatibility(domain) && !compatibilityResult) {
      return Promise.resolve({
        ok: false,
        status: 400,
        payload: {
      ...
      setTimeout(async function() {
        ...
        try {
          ...
          window._syLastCompat = {
            ...
            partnerTraits: (tData && tData.traits) ? {
              core: tData.traits.core || tData.traits.desc || '',
              hidden: tData.traits.hidden || '',
              love: tData.traits.love || '',
              work: tData.traits.work || '',
              wealth: tData.traits.wealth || '',
              karma: tData.traits.karma || ''
            } : {}
          };
          try {
        ...
        try {
          ...
          if (window._syLastCompat) {
            window._syLastCompat.enhanced = {
              ...
              roleA: enhanced.roleA,
              roleB: enhanced.roleB,
          ...
          const precisionExtensionSections = [
          ...
          const precisionUnlockSection = `
            <section id="syCompatPrecisionGate" data-sy-precision-gate="20260615-sukuyo-paid-precision" style="background:linear-gradient(145deg,rgba(69,26,3,0.42),rgba(15,23,42,0.72));border:1px solid rgba(251,191,36,0.34);border-radius:16px;padding:15px;margin-bottom:14px;">
          ...
          _styleEl.textContent = [
            ...
            '.sy-compat-metric-value{font-size:1.18rem;font-weight:900;line-height:1.28;word-break:keep-all}',
          ...
          const compactMetricSection = `
            <section style="background:rgba(15,23,42,0.5);border:1px solid rgba(147,197,253,0.28);border-radius:14px;padding:12px;margin-bottom:14px;">
              <div style="font-size:0.74rem;color:#93c5fd;text-transform:uppercase;font-weight:900;margin-bottom:9px;">핵심 구간</div>
              <div class="sy-compat-metric-grid" data-sy-compat-indicators-ui="20260613-compact-natural-ko">
                <article class="sy-compat-metric-card"><div class="sy-compat-metric-label">관계 구간</div><div class="sy-compat-metric-value" style="color:${compatIndexColor};">${syCanonicalEsc(syReferenceBand(compatibilityIndex))}</div><p class="sy-compat-metric-note">참고값 ${syCanonicalEsc(compatibilityIndex)} · ${syCanonicalEsc(compatibilityIndexNote)}</p></article>
                <article class="sy-compat-metric-card"><div class="sy-compat-metric-label">온도 구간</div><div class="sy-compat-metric-value" style="color:${tempInfo.color};">${syCanonicalEsc(syReferenceBand(rel.temperature))}</div><p class="sy-compat-metric-note">참고값 ${syCanonicalEsc(rel.temperature)} · ${syCanonicalEsc(syMetricNote('temperature', rel.temperature))}</p></article>
                <article class="sy-compat-metric-card"><div class="sy-compat-metric-label">끌림 구간</div><div class="sy-compat-metric-value" style="color:${th.color1};">${syCanonicalEsc(syReferenceBand(mgVal))}</div><p class="sy-compat-metric-note">참고값 ${syCanonicalEsc(mgVal)} · ${syCanonicalEsc(syMetricNote('magnetism', mgVal))}</p></article>
              </div>
            </section>`;
          ...
          var precisionGate = rd.querySelector('#syCompatPrecisionGate');
          if (precisionGate) {
            var precisionBtn = precisionGate.querySelector('[data-sy-precision-unlock]');
            var precisionContent = precisionGate.querySelector('[data-sy-precision-content]');
            var precisionStatus = precisionGate.querySelector('[data-sy-precision-status]');
            var revealPrecision = function() {
              if (!precisionContent) return;
              precisionContent.innerHTML = precisionExtensionSections;
