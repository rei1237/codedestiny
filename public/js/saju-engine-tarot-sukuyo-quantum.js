...
function renderSukuyo(p, natal, bazi, lunarObj, canonicalPayload, sourceProfile) {
  ...
  var SUKYO_RELATION_RICH_LIBRARY = {
    ankai: {
      summary: ['?? ??? ???? ??? ??? ?????. ??? ??? ?? ?? ??????, ??? ???? ??? ??? ?? ? ????.'],
      overview: ['???? ???? ?? ?????. ??? ??? ????? ?? ??? ??? ??? ???, ??? ??? ?? ??? ? ? ????.'],
      firstImpression: ['? ???? ??? ??? ????. ??? ??? ??? ????, ??? ? ???? ?? ??? ? ????.'],
  ...
  function syBuildRichReading(type, distanceKo) {
    var key = SUKYO_RELATION_RICH_LIBRARY[type] ? type : 'default';
    var tpl = SUKYO_RELATION_RICH_LIBRARY[key];
    var distanceKey = distanceKo === '???' ? 'near' : (distanceKo === '???' ? 'middle' : 'far');
    return {
      summary: tpl.summary.join(' '),
      overview: tpl.overview.join(' '),
      firstImpression: tpl.firstImpression.join(' '),
      emotionalBond: tpl.emotionalBond.join(' '),
      powerBalance: tpl.powerBalance.join(' '),
      love: tpl.love.join(' '),
      marriage: tpl.marriage.join(' '),
      work: tpl.work.join(' '),
      conflict: tpl.conflict.join(' '),
      distanceReading: {
        near: tpl.distanceReading.near || '',
        middle: tpl.distanceReading.middle || '',
        far: tpl.distanceReading.far || ''
      },
      longTermAdvice: tpl.longTermAdvice.join(' '),
      actionGuide: tpl.actionGuide.join(' '),
  ...
  function syBuildEnhancedSukuyoResult(ctx) {
    ...
    var richReading = syBuildRichReading(relationKey, distanceKo);
  ...
  window._triggerSynergyCheckCore = function(myIdx, myMansionName) {
      ...
      setTimeout(async function() {
        ...
        try {
          ...
          const richReadingSection = enhanced.richReading ? (
            '<section style="background:rgba(15,23,42,0.5);border:1px solid rgba(251,191,36,0.28);border-radius:16px;padding:14px 14px 10px;margin-bottom:14px;">' +
            '<div style="font-size:0.74rem;color:#fcd34d;letter-spacing:0.12em;text-transform:uppercase;font-weight:900;margin-bottom:8px;">' + enhanced.relationTypeKo + ' ?? ??</div>' +
            '<article style="border:1px solid rgba(251,191,36,0.24);border-radius:12px;padding:10px 11px;margin-bottom:8px;background:rgba(2,6,23,0.52);">' +
              '<strong style="color:#fde68a;">?? ??</strong>' +
              '<p style="margin:6px 0 0;color:#e2e8f0;line-height:1.82;font-size:0.83rem;">' + enhanced.richReading.summary + '</p>' +
              '<p style="margin:8px 0 0;color:#f8fafc;line-height:1.82;font-size:0.83rem;">' + enhanced.richReading.overview + '</p>' +
            '</article>' +
            '<article style="border:1px solid rgba(125,211,252,0.24);border-radius:12px;padding:10px 11px;margin-bottom:8px;background:rgba(2,6,23,0.52);">' +
              '<strong style="color:#bae6fd;">???? ??</strong>' +
              '<p style="margin:6px 0 0;color:#dbeafe;line-height:1.82;font-size:0.83rem;">' + enhanced.richReading.firstImpression + '</p>' +
            '</article>' +
            '<article style="border:1px solid rgba(196,181,253,0.24);border-radius:12px;padding:10px 11px;margin-bottom:8px;background:rgba(2,6,23,0.52);">' +
              '<strong style="color:#ddd6fe;">??? ???? ??</strong>' +
              '<p style="margin:6px 0 0;color:#e9d5ff;line-height:1.82;font-size:0.83rem;">' + enhanced.richReading.emotionalBond + '</p>' +
            '</article>' +
            '<article style="border:1px solid rgba(251,191,36,0.24);border-radius:12px;padding:10px 11px;margin-bottom:8px;background:rgba(2,6,23,0.52);">' +
              '<strong style="color:#fde68a;">??? ??? ??</strong>' +
              '<p style="margin:6px 0 0;color:#fde68a;line-height:1.82;font-size:0.83rem;">' + enhanced.richReading.powerBalance + '</p>' +
            '</article>' +
            '<article style="border:1px solid rgba(96,165,250,0.24);border-radius:12px;padding:10px 11px;margin-bottom:8px;background:rgba(2,6,23,0.52);">' +
              '<strong style="color:#bfdbfe;">?? ??</strong>' +
              '<p style="margin:6px 0 0;color:#bfdbfe;line-height:1.82;font-size:0.83rem;">' + enhanced.richReading.love + '</p>' +
            '</article>' +
            '<article style="border:1px solid rgba(134,239,172,0.24);border-radius:12px;padding:10px 11px;margin-bottom:8px;background:rgba(2,6,23,0.52);">' +
              '<strong style="color:#bbf7d0;">?? ??</strong>' +
              '<p style="margin:6px 0 0;color:#dcfce7;line-height:1.82;font-size:0.83rem;">' + enhanced.richReading.marriage + '</p>' +
            '</article>' +
            '<article style="border:1px solid rgba(167,139,250,0.24);border-radius:12px;padding:10px 11px;margin-bottom:8px;background:rgba(2,6,23,0.52);">' +
              '<strong style="color:#ddd6fe;">????/?? ??</strong>' +
              '<p style="margin:6px 0 0;color:#e9d5ff;line-height:1.82;font-size:0.83rem;">' + enhanced.richReading.work + '</p>' +
            '</article>' +
            '<article style="border:1px solid rgba(248,113,113,0.24);border-radius:12px;padding:10px 11px;margin-bottom:8px;background:rgba(2,6,23,0.52);">' +
              '<strong style="color:#fecaca;">?? ???</strong>' +
              '<p style="margin:6px 0 0;color:#fecaca;line-height:1.82;font-size:0.83rem;">' + enhanced.richReading.conflict + '</p>' +
            '</article>' +
            '<article style="border:1px solid rgba(45,212,191,0.24);border-radius:12px;padding:10px 11px;margin-bottom:8px;background:rgba(2,6,23,0.52);">' +
              '<strong style="color:#99f6e4;">???? ??</strong>' +
              '<p style="margin:6px 0 0;color:#99f6e4;line-height:1.82;font-size:0.83rem;">' + enhanced.richReading.distanceReading[enhanced.distanceKo === '???' ? 'near' : (enhanced.distanceKo === '???' ? 'middle' : 'far')] + '</p>' +
            '</article>' +
            '<article style="border:1px solid rgba(244,114,182,0.24);border-radius:12px;padding:10px 11px;margin-bottom:8px;background:rgba(2,6,23,0.52);">' +
              '<strong style="color:#f9a8d4;">?? ?? ?? ??</strong>' +
              '<p style="margin:6px 0 0;color:#f9a8d4;line-height:1.82;font-size:0.83rem;">' + enhanced.richReading.longTermAdvice + '</p>' +
            '</article>' +
            '<article style="border:1px solid rgba(251,191,36,0.24);border-radius:12px;padding:10px 11px;background:rgba(2,6,23,0.52);">' +
              '<strong style="color:#fde68a;">???? ???? ?? ??</strong>' +
              '<p style="margin:6px 0 0;color:#fef3c7;line-height:1.82;font-size:0.83rem;">' + enhanced.richReading.actionGuide + '</p>' +
            '</article>' +
          '</section>'
        ) : '';
