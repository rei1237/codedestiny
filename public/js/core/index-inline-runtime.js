...
function __cdInitGlobalPaymentLoading() {
  ...
  function __cdObserveApiAuthFailure(response, reqUrl) {
    try {
      if (!response || !reqUrl || !__cdIsOwnApiOrigin(reqUrl)) return;
      /* 🔴 403 은 세션 신호가 **아니다**(2026-08-21). 워커의 403 은 전수로 확인해 보면 전부
         권한·이용권 판정이다 — lib/access-control.js · lib/fortune-access-guard.js ·
         payments/errors.js(PASS_NOT_APPLICABLE · PURCHASE_POLICY_DENIED · MINOR_PAYMENT_BLOCKED ·
         ORDER_FORBIDDEN) · routes/fortune.js("구독자만 이용할 수 있습니다") · fusion-fortune.js(준비중).
         인증이 죽어서 나는 403 은 routes/auth.js 의 CSRF 뿐이고, 그건 아래 /api/auth/ 제외로 이미 빠진다.

         403 을 여기 두면 **유료 게이트 거절 하나하나가 강제(캐시 우회) /api/auth/me 를 부른다.**
         세션은 멀쩡한데 요청마다 Mongo 핸드셰이크 ~2.5초를 낸다(docs/context/ai-and-db.md "지리").
         🔴 되돌리지 말 것 — 403 을 세션 문제로 읽는 것은 진단 자체가 틀린 것이다. */
...
// 🔴 값은 lib/i18n/locales.ts 의 LOCALE_CONFIG[*].pathPrefix 와 같아야 한다.
// __tests__/ui/locale-prefix-map.static.test.js 가 그 일치를 강제한다.
//
// 예전에는 `/ja-jp`·`/zh-cn`·`/en-us` 라는 **구 프리픽스**였다. 그 경로에는 산출물이 없어서
// public/_redirects 의 `/ja-jp/* → /ja/:splat 301` 이 받아 줬고, 결국 로케일 사용자는 링크를
// 누를 때마다 301 을 한 번씩 더 탔다.
//
// 🔴 더 나빴던 것 — hi·es·fr·de·nl·ms 는 그 301 규칙조차 없어서 그냥 **404** 였다.
// 그리고 이 여섯은 셸 언어 전환기(`data-lang`)가 실제로 제공하는 값이다. 즉 프랑스어를 고른
// 사용자가 링크를 누르면 없는 페이지로 갔다(2026-08-16 실측: `/fr-fr/insights/` 산출물 없음).
...
// 🔴 next.config.mjs 가 trailingSlash:true 라 내부 링크는 `/insights/` 처럼 슬래시로 끝난다.
// 아래 경로 비교는 슬래시 없는 값으로 적혀 있으므로, 비교 전에만 슬래시를 뗀다.
// 이동 경로에는 슬래시를 **남긴다** — 떼면 `/ja/insights` 로 보내 308 을 한 번 더 탄다.
function cdTrimTrailingSlash(pathname) {
  var p = String(pathname || '/');
...
function cdResolveLocalizedFeatureHref(rawHref, langCode) {
  ...
  var comparePath = cdTrimTrailingSlash(basePath);
  // 🔴 로케일 빌드가 **실제로 있는** 경로만 넣는다. 여기 없는 경로는 프리픽스 없이 한국어
  // 페이지로 가는데, 그건 읽을 수 있는 페이지이고 없는 주소보다 낫다.
  //
  // 2026-08-16 실측 — `out/{ja,zh,en}/<경로>/index.html` 존재 여부:
  //   /insights                 ko=Y ja=Y zh=Y en=Y   ← 유일하게 로케일 빌드가 있다
  //   /olympus                  ko=Y ja=N zh=N en=N
  //   /oracle/rune              ko=Y ja=N zh=N en=N
  //   /oracle/sikojen-povailu   ko=Y ja=N zh=N en=N
  // 뒤 셋은 여기 있는 동안 로케일 사용자를 없는 주소로 보냈다(`_redirects` 에도 규칙이 없다).
  // 나중에 그 세 경로의 로케일 빌드가 생기면 그때 다시 넣는다.
  var isAppLocalized = (comparePath === '/insights');
  var isStandaloneHtml = (
    comparePath === '/vedic-ai' ||
    comparePath === '/geomancy-oracle-v4.html' ||
    comparePath === '/royal-tea-oracle.html' ||
    comparePath === '/destiny-poker.html'
  );
...
var __cdLazyActionLoaders = {
  openKemetModal: function() { return __cdLoadScriptOnce('/js/oracle-kcg.js?v=build-785f4cf16ecf'); },
  openDreamModal: function() { return __cdLoadScriptOnce('/js/dream-ledger.js?v=build-785f4cf16ecf'); },
  openPsychoDreamModal: function() { return __cdLoadScriptOnce('/js/psycho-dream-analyzer-freuds-study.js?v=build-785f4cf16ecf'); },
  ...
  openJuyukModal: function() { return __cdLoadScriptOnce('/js/iching-engine.js?v=build-785f4cf16ecf').then(function() { return __cdLoadScriptOnce('/js/iching-modal.js?v=build-785f4cf16ecf'); }); },
  openRuneOracle: function() { window.location.assign('/oracle/rune/'); return Promise.resolve(true); },
  openAnimalTotemModal: function() { return __cdLoadScriptOnce('/js/services/animal-totem-content-engine.js?v=build-785f4cf16ecf').then(function() { return __cdLoadScriptOnce('/js/animal-totem-experience.js?v=build-785f4cf16ecf'); }); },
  openDestinyEggPage: function() { return Promise.resolve(window.location.assign('/tadagochi.html')); },
  openTarotLoveModal: function() { return __cdLoadScriptOnce('/js/tarot-love-experience.js?v=build-785f4cf16ecf'); },
  openTarotReunionModal: function() { return __cdLoadScriptOnce('/js/tarot-reunion-experience.js?v=build-785f4cf16ecf'); },
  ...
  openTarotSelfEsteemModal: function() { return __cdLoadScriptOnce('/js/tarot-self-esteem-experience.js?v=build-785f4cf16ecf'); },
  openTarotYearFortuneModal: function() { return __cdLoadScriptOnce('/js/tarot-year-fortune-experience.js?v=build-785f4cf16ecf'); },
  openSibylModal: function() {
    return __cdLoadScriptOnce('/js/sibyl-system.js?v=build-785f4cf16ecf').then(function() {
  ...
  openLegacyLuckSyncDiary: function() { return __cdLoadScriptOnce('/js/luck-sync-diary.js?v=build-785f4cf16ecf'); },
  ...
  // 같은 계열의 버그 — 위 주석 참고).
  shareKakao: function() { return __cdLoadScriptOnce('/js/share.js?v=build-785f4cf16ecf'); },
  shareInstagram: function() { return __cdLoadScriptOnce('/js/share.js?v=build-785f4cf16ecf'); },
  shareSajuResultImage: function() { return __cdLoadScriptOnce('/js/share.js?v=build-785f4cf16ecf'); }
};
...
function __cdOpenLegacyLuckSyncDiaryProxy() {
  return __cdLoadScriptOnce('/js/luck-sync-diary.js?v=build-785f4cf16ecf').then(function() {
    if (window.LuckSyncDiary && typeof window.LuckSyncDiary.open === 'function') {
      return window.LuckSyncDiary.open();
    }
    var open = window.openLegacyLuckSyncDiary;
    if (typeof open === 'function' && open !== __cdOpenLegacyLuckSyncDiaryProxy) {
      return open();
    }
    return false;
  }).catch(function(err) {
    console.warn('[index-inline-runtime] legacy diary open failed:', err);
    return false;
  });
}
window.openLegacyLuckSyncDiary = __cdOpenLegacyLuckSyncDiaryProxy;
 ...
 *
 * 왜 필요한가 — __cdEnsureSajuCoreLoaded 의 체인은 순차 reduce 라 11번의 왕복이 직렬이다.
 * 앞 파일을 다 받고 실행해야 다음 파일이 출발한다. 실측(2026-08-16, dist·390x844·CPU 4x·5회,
 * docs/handoff/inp-round3-2026-08-16.md §4-4): 프로필 카드 모달이 뜨자마자 즉시 탭하면
 * 코어 준비까지 506ms [451-524] 가 걸린다.
 *
 * 🔴 `<link rel="preload" as="script">` 는 받아서 캐시에 넣기만 하고 **평가하지 않는다.**
 *    그래서 실행 순서도, "앞 파일 실패가 뒤쪽 전체를 죽인다"는 체인의 계약도 그대로다.
 *    바뀌는 건 다운로드가 병렬이 된다는 것 하나뿐이다 — reduce 는 한 줄도 건드리지 않았다.
 * 🔴 crossorigin 을 붙이지 말 것. __cdLoadScriptOnce 가 만드는 <script> 에는 crossorigin 이
 *    없어 no-CORS 요청이다. preload 에만 붙이면 요청 모드가 어긋나 캐시가 재사용되지 않고
 *    같은 파일을 두 번 받는다(서드파티 lunar 가 여기 해당한다).
 * 🔴 preconnect 를 따로 걸지 않는다. 여기서 거는 preload 가 그 자리에서 DNS+TLS 를 열므로
 *    같은 시점의 preconnect 는 같은 일을 두 번 시키는 것뿐이다(CLAUDE.md 코딩 원칙 6).
 *
 * 대가: 체인이 1번 파일에서 실패하는 예외 경로에서는 예전에 받지 않던 뒤쪽 바이트까지 받는다.
 * 정상 경로에서는 어차피 전부 받으므로 바이트 총량은 같다.
 */
function __cdPreloadScripts(list) {
  if (!list || !list.length) return;
  if (typeof document === 'undefined') return;
  var head = document.head;
  if (!head) return;
  if (!__cdPreloadedScriptHrefs) __cdPreloadedScriptHrefs = {};
  for (var i = 0; i < list.length; i += 1) {
    var href = __cdNormalizeScriptSrc(list[i]);
    /* 호출자가 실패 후 재진입할 수 있으므로(__cdSajuCoreLoadPromise 를 비운다) 중복을 막는다. */
    if (!href || __cdPreloadedScriptHrefs[href] === 1) continue;
    __cdPreloadedScriptHrefs[href] = 1;
...
function __cdEnsureSajuCoreLoaded() {
  ...
  /* 🔴 이 체인에 서드파티 URL 을 넣지 말 것. 예전에는 2번이 lunar-javascript CDN 이었는데,
   *    아래 reduce 는 **순차**이고 `.catch` 가 체인 전체를 죽인다 — 그 한 원소가 막히면
   *    kasi-calendar-service·saju-engine 이하 전부가 안 뜨고, 스텁의 catch 는 console.error 만
   *    찍으므로 사용자에게는 **run-btn 이 조용히 무반응**으로 보였다. 값 계산에 그 라이브러리를
   *    한 글자도 안 쓰는데도 그랬다(가드가 `global.Solar` 를 지운 채 셸을 평가해 매번 증명한다).
   *    제거 실측(2026-08-28): 직렬 왕복 1개 · 전송 96,753 B 감소.
   *    verify:lunar-conversion-core 가 되살아나는 것을 잡는다. */
  var chain = [
    ...
    '/js/core/korean-calendar.js?v=build-785f4cf16ecf',
    '/js/core/kasi-calendar-service.js?v=build-785f4cf16ecf',
    '/js/compat-llm-prompts.js?v=build-785f4cf16ecf',
    '/js/saju-engine.js?v=build-785f4cf16ecf',
      '/js/core/saju/extremeTResult.js?v=build-785f4cf16ecf',
      /* 숙요 정본(Swiss 항성 달 황경). quantum.js 의 calcSukuyoData 가 이것 없이는 수(宿)를 내지 않는다. */
      '/js/core/sukuyo-astronomy.js?v=build-785f4cf16ecf',
      '/js/saju-engine-tarot-sukuyo-quantum.js?v=build-785f4cf16ecf',
    '/js/core/saju/basicFortunePresentation.js?v=build-785f4cf16ecf',
    '/js/core/saju/modalProfileState.js?v=build-785f4cf16ecf',
    '/js/core/saju/reportDashboard.js?v=build-785f4cf16ecf',
    '/js/saju-engine-continuation.js?v=build-785f4cf16ecf',
    '/js/entertain-engine.js?v=build-785f4cf16ecf',
    /* 체인은 순차 reduce라 앞에 끼우면 이 파일의 실패가 뒤쪽 전체를 죽인다 — 신규 카드 스크립트는 맨 뒤에 둔다. */
    '/js/core/saju/dopamineResult.js?v=build-785f4cf16ecf',
    '/js/core/saju/relationshipTemptationAnalysis.js?v=build-785f4cf16ecf'
  ];

  /* 다운로드만 병렬로 앞당긴다 — 아래 reduce 의 순차 실행은 그대로다(__cdPreloadScripts 주석 참고). */
  __cdPreloadScripts(chain);
...
function __cdEnsureDestinyProfileLoaded() {
  ...
  __cdDestinyProfileLoadPromise = __cdLoadScriptOnce('/js/destiny-profile.js?v=build-785f4cf16ecf')
...
function __cdEnsureSwissEphLoaded() {
  ...
  __cdSwissEphLoadPromise = new Promise(function(resolve, reject) {
    var src = '/js/swisseph-loader.js?v=build-785f4cf16ecf';
    var norm = __cdNormalizeScriptSrc(src);
    if (!norm) {
...
function __cdScheduleCollectionTask(run) {
...
function __cdScheduleCollectionHydration(collection, forceHydrateAll) {
  __cdScheduleCollectionTask(function() {
    __cdHydrateCollectionImagesChunked(collection, forceHydrateAll);
  });
}
...
/* 열기와 닫기가 같은 사다리를 탄다. 닫기가 동기였을 때는 탭 태스크 안에서 querySelectorAll 4벌과
   첫 배치의 이미지 노드 제거까지 끝냈다.
   🔴 지연 실행 시점에 data-collection-open 을 다시 읽어 상태가 뒤집혔으면 건너뛴다 — 안 그러면
   "닫자마자 다시 열기" 에서 해제의 뒤쪽 배치가 새로 그린 타일의 플레이스홀더를 되살린다.
   dispatch 하는 쪽(모바일 오버레이·toggleCollection 액션·VVIP 폴백)이 전부 이 속성을 먼저 쓴다. */
function __cdScheduleCollectionToggleWork(collection, isOpen) {
  __cdScheduleCollectionTask(function() {
    if ((collection.getAttribute('data-collection-open') === 'true') !== isOpen) return;
    if (isOpen) __cdHydrateCollectionImagesChunked(collection, true);
    else __cdReleaseCollectionImagesChunked(collection);
  });
}

/* 🔴 가드 플래그 이름은 ES 모듈 쌍둥이 uiBindings.js 와 **같아야** 한다. 예전에는
   __cdUiCollectionToggleHydrationBound / __cdCollectionToggleHydrationBound 로 갈려 있어
   서로를 못 막았고, 토글 한 번에 하이드레이션/해제가 통째로 두 벌 돌았다. */
function __cdBindCollectionToggleHydration() {
  ...
  document.addEventListener('cd:collection-toggle', function(event) {
    ...
    if (detail.isOpen === true) {
      __cdScheduleCollectionToggleWork(collection, true);
    } else if (detail.isOpen === false) {
      __cdScheduleCollectionToggleWork(collection, false);
    }
  });
}
  ...
  root.addEventListener('click', function(event) {
    ...
    if (action === 'toggleCollection') {
      ...
      /* 🔴 여기서 하이드레이션/해제를 다시 부르지 않는다 — 바로 위 dispatch 가 이미
         __cdBindCollectionToggleHydration 의 리스너를 깨워 같은 일을 시킨다(이 파일이 그 리스너를
         무조건 등록하므로 항상 붙어 있다). 예전에는 이 자리의 비강제 하이드레이션이 리스너의
         강제 하이드레이션과 섞여 한 번의 토글에 컬렉션 전수 스윕이 두 벌 돌았다. */
      return;
    }
...
var __cdRouteActionAllowList = {
  ...
  // 🔴 아래 5개는 홈 허브·랜딩이 ?action= 으로 내보내는데 예전엔 허용목록 밖이라
  // legacy-action-launcher.js 의 폴백(window[action]() 직접 호출)으로 흘렀다. 그 경로는
  // __cdRequireTileLockGate 를 거치지 않아 유료 타일의 잠금 판정을 통째로 건너뛴다.
  // openCelestialHarmony·openDestinyBias 는 전역 함수 자체가 없어 폴백도 못 타고 무반응이었다.
  // 다섯 개 모두 셸에 data-action 타일이 있으므로 여기(타일 경유)가 유일한 정답이다.
  openCelestialHarmony: true,
  openDestinyBias: true,
  openGeomancyOracle: true,
  openKemetModal: true,
  openMbtiModal: true,
  // start* 3종 중 셸 타일이 있는 것은 startIjikTarot 뿐이다. 나머지 둘은 타일이 없어
  // 런처 폴백(^start 정규식)이 유일한 경로다 — 세 함수 모두 본문이 인증 확인 + 페이지 이동뿐이라
  // 결제 게이트는 목적지 라우트가 쥔다.
...
function __cdRunRouteActionOnce() {
  if (window.__cdRouteActionAttempted || window.__cdRouteActionHandled) return;
  window.__cdRouteActionAttempted = true;
  ...
  if (!actionEl) {
    // 🔴 여기서 __cdRouteActionHandled 를 세우면 안 된다. 그 플래그는
    // legacy-action-launcher.js 의 폴백을 세우는 단일비행 신호라, 타일을 못 찾은 채로 세우면
    // 이쪽도 저쪽도 실행하지 않아 액션이 통째로 죽는다. 셸에 data-action 타일이 없는
    // 허용목록 액션이 실제로 있다(navigateToVedic·openSajuAnimalPage — 셸의 클릭 디스패처가
    // 다른 타일 이름으로 처리한다). 그런 액션은 런처 폴백이 유일한 경로다.
    __cdReleaseDeepLinkBootCover();
    return;
  }
...
function __cdBindAnimalTotemTileDirect() {
  ...
  /* 🔴 게이트가 없는 열기 코어다. 결제 후 재개 핸들러도 이쪽을 부른다 — 게이트를 다시 타는
     openTotemModal 을 부르면 이미 낸 결제가 또 일어난다(배선 레시피 4번). */
  function _doOpenTotem() {
    ...
    raf(function() {
      loadScriptOnce('js/services/animal-totem-content-engine.js?v=build-785f4cf16ecf')
        .then(function() { return loadScriptOnce('js/animal-totem-experience.js?v=build-785f4cf16ecf'); })
        .then(function() {
          try {
  ...
  /* ── 결제 후 자동 재개(모바일 리다이렉트 복귀) ────────────────────────────
     모바일 PortOne 은 상위 프레임을 리다이렉트하므로 아래 _cdCoinGatePerUse 의 콜백이 페이지와
     함께 죽는다 — 복귀 문서는 영수증으로 재과금만 막을 뿐 토템 모달을 열지 않았다. 이 파일은 셸에
     항상 로드되므로 딥링크(action)는 비우고 핸들러만 둔다. 🔴 여기서 action 을 채우면 runPaidResume
     이 타일을 다시 눌러 게이트가 한 번 더 돌고, 뒤늦게 등록된 핸들러가 또 열어 이중 오픈이 된다. */
  var TOTEM_RESUME_KIND = 'animal-totem-entry';
  (function registerTotemResumeHandler() {
    var entry = null;
    ...
    try {
      entry.registerPaidResumeHandler(TOTEM_RESUME_KIND, function() { _doOpenTotem(); return true; });
    } catch (_) {}
  })();
    ...
    try {
      ...
      if (_coinCost > 0 && _tile && !_tile.getAttribute('data-pvw-bypass')) {
        ...
        if (typeof window._cdCoinGatePerUse === 'function') {
          window._cdCoinGatePerUse(_coinCost, '애니멀 토템 리딩', function() { _doOpenTotem(); }, null, {
            // 🔴 이 진입점은 optionBag 자체가 없어 재개 서술자를 실을 자리가 없었다 — 결제하고 복귀하면
            //    홈만 보였다. featureKey 는 그대로 reason 에서 유도한다(과금 대상을 바꾸지 않는다).
            resume: { kind: TOTEM_RESUME_KIND, action: '', args: {} },
          });
          return;
        }
...
function _dfExtractSukuyoLiveData(birthCtx) {
  ...
  var lunarObj = null;
  try {
    if (window.KasiEngine && typeof window.KasiEngine.solarToLunarFromParts === 'function') {
      lunarObj = window.KasiEngine.solarToLunarFromParts(window.KasiEngine.partsOf(
        Number(birthCtx.year),
        Number(birthCtx.month),
        Number(birthCtx.day),
        Number(birthCtx.hour),
 ...
 * 로 옮겨 갔고, 결과를 주는 곳은 `POST /api/destiny-flower/match` 하나다. 그 라우트가
 * `flower-fc` 해금을 서버에서 확인한다 — 예전에는 결제만 서버에서 차감되고 결과는 브라우저가
 * 만들어서, 콘솔 한 줄로 1만원짜리 결과를 공짜로 뽑을 수 있었다.
 *
 * 렌더 경로는 전부 동기라, 여기서는 캐시만 읽고 없으면 null 을 돌려준다. 요청이 도착하면
 * `_dfRefreshStudioForSource` 를 다시 불러 화면을 채운다(4-up 의 "판정 대기" 칸이 그 사이를 메운다).
 */
var _dfServerMatch = {
  fingerprint: '',
  sources: null,
...
function _dfServerMatchFingerprint(payload) {
  ...
  var ziwei = (payload.domains && payload.domains.ziwei) || payload.ziwei || {};
  var astro = (payload.domains && payload.domains.astrology) || payload.astrology || {};
  var sukuyo = (payload.domains && payload.domains.sukuyo) || payload.sukuyo || {};
  return [
...
/**
 * 서버에 매칭을 요청한다. 같은 지문이면 재요청하지 않고, 진행 중이면 그 약속을 돌려준다
 * (단일비행 — 4-up 이 네 소스를 동시에 물어도 왕복은 1회다).
 */
function _dfRequestServerMatch(payload, onSettled) {
  var fingerprint = _dfServerMatchFingerprint(payload);
  if (!fingerprint) return null;
  if (_dfServerMatch.fingerprint === fingerprint && _dfServerMatch.sources) return null;
  if (_dfServerMatch.fingerprint === fingerprint && _dfServerMatch.inflight) return _dfServerMatch.inflight;

  var token = __cdGetAuthTokenForLockSync();
  var url = __cdResolveApiBaseForLockSync() + '/api/destiny-flower/match';
  var headers = { 'Content-Type': 'application/json' };
  if (token) headers.Authorization = 'Bearer ' + token;

  _dfServerMatch.fingerprint = fingerprint;
  _dfServerMatch.sources = null;
  _dfServerMatch.theme = null;
  _dfServerMatch.status = 'loading';
  ...
  var promise = fetch(url, {
    ...
    if (_dfServerMatch.fingerprint !== fingerprint) return;
    if (result.status === 200 && result.data && result.data.ok) {
      _dfServerMatch.sources = result.data.sources || {};
      _dfServerMatch.theme = result.data.theme || null;
      _dfServerMatch.status = 'ready';
    } else if (result.status === 402) {
      _dfServerMatch.status = 'locked';
    } else if (result.status === 401) {
      _dfServerMatch.status = 'unauthorized';
    } else {
      _dfServerMatch.status = 'error';
    }
  }).catch(function(error) {
    if (_dfServerMatch.fingerprint !== fingerprint) return;
    console.warn('[DestinyFlower] 서버 매칭 실패:', error);
    _dfServerMatch.status = 'error';
  }).then(function() {
    if (_dfServerMatch.fingerprint === fingerprint) _dfServerMatch.inflight = null;
    if (typeof onSettled === 'function') {
      try { onSettled(_dfServerMatch.status); } catch (_) {}
    }
  });

  _dfServerMatch.inflight = promise;
  return promise;
}
...
function _dfGetServerMatched(source) {
  ...
  return _dfServerMatch.sources[_dfNormalizeSource(source)] || null;
}

/**
 * 서버 응답이 도착했을 때 화면을 다시 그린다.
 * 🔴 소스별 캐시(`_dfStudioState.flowerData.sources`)에 이미 null 이 박혀 있으므로 강제 갱신으로 부른다.
 */
function _dfOnServerMatchSettled(status) {
  if (status !== 'ready') {
  ...
  try {
    // 네 소스 전부에 null 이 박혀 있다 — 비워야 4-up 이 다시 계산한다.
    if (_dfStudioState.flowerData) _dfStudioState.flowerData.sources = {};
    _dfRefreshStudioForSource(_dfStudioState.activeSource || 'saju', true);
  } catch (error) {
    console.warn('[DestinyFlower] 서버 결과 반영 실패:', error);
  }
}
...
function _dfResolveSelection() {
  ...
  // 🔴 매칭은 서버가 한다. 캐시에 없으면 요청만 걸고 null 을 돌려준다 — 도착하면 다시 그린다.
  ...
  // `window.flowerSymbology` 폴백은 사라졌다 — 카탈로그가 워커로 갔다. 서버가 꽃을 못 고르면
  // 그건 서버가 판정할 데이터가 없다는 뜻이라, 억지 연꽃 대신 빈 상태를 보여 준다.
...
function _dfHasReadySourceData(source, payload) {
  ...
  if (normalized === 'saju') {
    // 매처 유무 검사는 사라졌다 — 매칭은 서버가 하므로 브라우저에 매처가 있을 수 없다.
    // 여기서는 "서버에 물어볼 만한 데이터가 있는가"만 본다.
    var saju = data.saju || {};
...
function _dfReloadSourceData(source, options) {
  ...
  if (!window.CDFlowerArt) {
    loader = loader.then(function() {
      return import('/js/core/bootstrapDestinyFlower.js?v=build-785f4cf16ecf').then(function(mod) {
        if (mod && typeof mod.bootstrapDestinyFlower === 'function') {
          mod.bootstrapDestinyFlower(window);
        }
        return true;
      }).catch(function(err) {
        console.warn('[DestinyFlower] 꽃 아트 부트스트랩 실패:', err);
        return false;
      });
    });
  }
...
function __cdEnsureSukuyoZiweiCoreLoaded() {
   ...
   * 모달이 통째로 안 열렸다(호출부 openSukuyoModal 의 catch 가 로그만 남기고 끝난다).
   * 숙요·자미의 달력은 체인 1번(한국 음양력 코어)에서 나오므로 그 대기는 아무것도 안 지켰다. */
...
function __cdEnsureBirthModalDepsLoaded() {
  ...
  var presentationReady = window.BasicFortunePresentation
    ? Promise.resolve()
    : __cdLoadScriptOnce('/js/core/saju/basicFortunePresentation.js?v=build-785f4cf16ecf');
  ...
  if (
    ...
    tasks.push(presentationReady.then(function () {
      return __cdLoadScriptOnce('/js/core/saju/modalProfileState.js?v=build-785f4cf16ecf');
    }));
  }
...
function openAnimalTotemModal() {
  ...
  if (!hasFullTotemFlow && typeof __cdLoadScriptOnce === 'function') {
    __cdLoadScriptOnce('/js/services/animal-totem-content-engine.js?v=build-785f4cf16ecf')
      .then(function() { return __cdLoadScriptOnce('/js/animal-totem-experience.js?v=build-785f4cf16ecf'); })
      .then(function() {
