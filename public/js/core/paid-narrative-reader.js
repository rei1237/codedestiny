(function(root){
  ...
  async function run(initial, options){
    ...
    for(let wave=0;wave<40&&options.active();wave++){
      ...
      if(reply.status===202&&data.resumeBody){body=data.resumeBody;options.persist(body,data.resultId);options.show(data);if(data.retryable===false)throw new Error('생성 한도에 도달했어요. 저장된 내용을 보존했으며 추가 확인이 필요합니다.');failures=0;await options.wait(Math.min(5000,Math.max(1000,Number(data.retryAfterMs)||1000)));continue;}
