(function(global) {
  ...
  function composeConsultation(spread, userContext) {
    ...
    var headAnimal = spread.cards[0] && spread.cards[0].card;
    var opening = isOne
      ? "고요한 숲 깊은 곳에서 " + (headAnimal ? headAnimal.name_ko : "수호자") + "이(가) 당신의 이름을 부르며 걸어왔습니다. " +
        "이 한 장의 카드에 담긴 메시지는 우연이 아닌 당신의 에너지가 지금 가장 필요로 하는 진실입니다. " +
        "천천히 숨을 고르고, 수호 동물의 속삭임에 귀를 기울여보세요."
      : "고대 샤머닉 전통에서 수호 동물은 우주가 당신에게 보내는 전령입니다. " +
        "오늘 " + lines.join(", ") + "이(가) 한자리에 모인 것은 당신의 내면이 지금 이 순간 가장 필요한 진실을 향해 문을 열었기 때문입니다. " +
        "각 동물이 전하는 지혜의 층위를 따라가며 당신만의 완전한 메시지를 완성해 보세요.";
    var focus =
    ...
    return {
      ...
      closing_guidance: isOne
        ? "수호 동물의 메시지는 단 한 번의 리딩으로 끝나지 않습니다. " +
          "오늘 받은 이 한 마디를 마음의 나침반으로 삼아, 결정의 순간마다 조용히 꺼내 보세요. " +
          "당신의 수호자는 언제나 당신 안에 살아 있습니다."
        : "세 마리(또는 다섯 마리)의 수호 동물이 전한 지혜는 서로 다른 층위에서 하나의 완전한 메시지를 구성합니다. " +
          "가장 깊게 공명하는 동물의 확언을 하루의 태도로 삼고, 성찰 저널 질문 하나를 오늘 밤 잠들기 전에 기록해 보세요. " +
          "당신이 이 리딩으로 향한 에너지는 이미 변화를 향한 첫 걸음입니다."
    };
  }
