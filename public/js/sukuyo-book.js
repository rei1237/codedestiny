...
(function () {
  ...
  var CHAPTER_TITLES = [
    ...
    '⚗️ 관계를 정화하는 연금술 — 인연의 독소를 황금으로 바꾸는 법',
    '🗺️ 영혼의 마스터플랜 — 달빛 전략가의 10년 로드맵',
  ];

  var CHAPTER_SUBTITLES = [
    '27수 탄생 숙요의 본질·숨겨진 이면·카르마 패턴·재능 지수 완전 해독',
  ...
  var MYSTIC_QUOTES = [
    ...
    '카르마 패턴을 아는 자만이 반복되는 시련을 끊을 수 있습니다.',
  ...
  function _showScreen(id){
    var screens=['skStartScreen','skLoadingScreen','skResultScreen','skErrorScreen'];
  ...
  window.openSukuyoBookModal = function(){
    ...
    if(!profile){
      if(!window.__cdAdminBypass){
        ...
        alert('💫 숙요점 프리미엄을 생성하려면 생년월일·출생 시간을 입력하고 "사주 분석 시작"을 눌러주세요.');
        return;
      }
    }
  ...
  window.generateSukuyoBook = function(){
    ...
    (function generateNext(idx){
      ...
      _fetchChapter(idx).then(function(data){
        ...
        else{_failCount++;var msg=(data&&data.message)?data.message:'알 수 없는 오류';console.warn('[숙요] Chapter '+(idx+1)+' 실패:',msg);_chapters[idx]='⚠️ **이 챕터의 분析을 불러오는 데 실패했습니다.**\n\n오류: '+msg+'\n\n잠시 후 다시 시도해 주세요.';}
        _setProgress(idx+1);
        generateNext(idx+1);
      });
    })(0);
  ...
  window.downloadSukuyoBookPdf = function(){
    ...
    var fname=(profile.name||'사용자')+'_숙요점_인생_총람_'+new Date().toLocaleDateString('ko-KR').replace(/[\.\/]/g,'').trim()+'.html';
