...
export default function AppHomeClient() {
  ...
  const spotlightFeatures: Array<FeatureMarketingTarget & { eyebrow: string; image: string }> = [
    {
      title: copy.spotlight["fortune-tea-house"].title,
      subtitle: copy.spotlight["fortune-tea-house"].subtitle,
      description: copy.spotlight["fortune-tea-house"].description,
      href: "/fortune-tea-house",
      slug: "fortune-tea-house",
      featureKey: "fortune-tea-house",
      category: "tarot",
      accessType: "free",
      eyebrow: copy.spotlightHeading,
      image: "https://assets.code-destiny.com/DestinyCafe/%EC%9A%B4%EB%AA%85%EC%9D%98%20%EC%B0%BB%EC%A7%91.webp",
    },
    {
      title: copy.spotlight["neo-operation-room"].title,
      subtitle: copy.spotlight["neo-operation-room"].subtitle,
      description: copy.spotlight["neo-operation-room"].description,
      href: "/neo-operation-room",
      slug: "neo-operation-room",
      featureKey: "neo-operation-room-consultation",
      category: "saju",
      accessType: "paid",
      badges: [{ text: copy.spotlight["neo-operation-room"].badge }],
      eyebrow: copy.spotlightHeading,
  ...
  const featureBySlug = new Map(listServiceFeatures(getCurrentLoadingLocale()).map((feature) => [feature.slug, feature]));
  ...
  return (
    <>
      <header className="cd-app-bar flex min-h-16 items-center justify-between gap-3 px-4 pb-3 pt-3">
        <Link href="/app" className="flex min-w-0 items-center gap-3 no-underline" aria-label={copy.homeAria}>
          <img
          ...
          <span className="grid min-w-0 gap-0.5">
            ...
            <span className="truncate text-xs font-semibold leading-tight" style={{ color: "var(--cd-app-ink-subtle)" }}>{copy.brandTagline}</span>
          </span>
        </Link>
      ...
      <section className="cd-app-surface cd-app-enter mx-4 mt-3 p-4" aria-label={copy.heroKicker}>
        <p className="m-0 text-xs font-black" style={{ color: "var(--cd-app-accent)" }}>{copy.heroKicker}</p>
        <h1 className="cd-app-title mt-2" style={{ color: "var(--cd-app-gold)" }}>{copy.heroTitle}</h1>
      ...
      <section className="mt-4 grid gap-3 px-4" aria-label={copy.spotlightSectionAria}>
        <div className="flex items-center justify-between gap-3">
          <h2 className="cd-app-heading">{copy.spotlightHeading}</h2>
      ...
      <section className="mt-4 grid gap-3 px-4" aria-label={copy.categorySectionAria}>
        <h2 className="cd-app-heading">{copy.categoryHeading}</h2>
          ...
          {features.map((feature) => (
            ...
            >
              ...
              <span className="text-[11px] font-black" style={{ color: "var(--cd-app-accent)" }}>
                {feature.accessType === "free" ? copy.freeNow : copy.passOrPayment}
              </span>
            </FeatureMarketingLink>
          ))}
        </div>
