    ...
    try {
      ...
      const paymentResult = await ensurePaidAccess({
        ...
        onPaid: async ({ chargedCoins, requiredCoins, balanceAfter }) => {
          ...
          if (chargedCoins > 0) {
            showToast(`우리는 무슨 사이? 타로 ${Math.max(0, chargedCoins * 100).toLocaleString("ko-KR")}원 결제가 승인되었습니다. 잔여 원화 가치: ${Math.max(0, balanceAfter * 100).toLocaleString("ko-KR")}원`, "info");
          }
        },
      });
      ...
      if (!paymentResult.ok) {
        ...
        if (paymentResult.code === "INSUFFICIENT_COINS") {
          setError(`결제 가능 금액이 부족합니다. ${paymentResult.requiredCoins}결제가 필요합니다.`);
          return;
        }
