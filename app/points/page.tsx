...
const SUBSCRIPTION_BASE_PLANS = [
  {
    ...
    features:     [
      ...
      "PDF 생성 시 3,000원 자동 할인",
      "3,000원 초과 기능은 단건 결제 또는 보너스 가치/원화 결제",
      "결제 즉시 30일 이용권 활성화",
      "자동결제 상품 아님",
    ],
  },
  {
    ...
    features:     [
      ...
      "5,000원 초과 기능은 단건 결제 또는 보너스 가치/원화 결제",
      "결제 즉시 30일 이용권 활성화",
      "자동결제 상품 아님",
    ],
    badge:        "추천",
  },
  {
    ...
    features:     [
      ...
      "10,000원 초과 기능은 단건 결제 또는 보너스 가치/원화 결제",
      "결제 즉시 30일 이용권 활성화",
      "자동결제 상품 아님",
    ],
    badge:        "VVIP",
...
function mapMonthlyCreditLedgerLabel(type: MonthlyCreditLedgerItem["type"]) {
  ...
  if (normalized.includes("REFUND")) {
    return { label: "보너스 가치 환불", cls: "bg-cyan-100 text-cyan-800 border-cyan-300", prefix: "+" };
  }
  if (normalized === "MONTHLY_CREDIT_SPEND") {
    return { label: "보너스 가치 사용", cls: "bg-rose-100 text-rose-700 border-rose-300", prefix: "-" };
  }
  return { label: "보너스 가치 지급", cls: "bg-emerald-100 text-emerald-800 border-emerald-300", prefix: "+" };
}
...
function formatPaymentMethodLabel(payment: PaymentHistoryItem) {
  if (isMonthlyCreditPayment(payment)) return "보너스 가치";
...
function formatPaymentMonthlyCreditHint(payment: PaymentHistoryItem) {
  ...
  if (cost <= 0) return "";
  const amount = formatMonthlyCredits(cost);
  if (isMonthlyCreditPayment(payment)) return `보너스 가치 ${amount} 사용`;
  return `보너스 가치 기준 ${amount}`;
}
  ...
  return (
    ...
    >
      ...
      >
        ...
        <div className="mb-4 rounded-lg border border-[#8cb8ff]/28 bg-[#8cb8ff]/10 px-4 py-3">
          ...
          <ul className="mt-1.5 space-y-1 text-[11.5px] text-slate-200">
            ...
            <li className="flex items-start gap-1.5"><span className="mt-0.5 flex-shrink-0">·</span><span className="min-w-0">보너스 가치는 신규 가입·이벤트로만 지급되며 구매하거나 충전할 수 없습니다.</span></li>
      ...
      <div className="px-5 pb-5 space-y-1">
        ...
        <p className="text-[11px] text-[#9B7040]">보너스 가치는 이벤트 보너스이며 구매·충전할 수 없습니다.</p>
      </div>
    </section>
  );
}
        ...
        <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
          ...
          <div className="flex flex-col items-start gap-1 sm:items-end">
            ...
            <div className="flex flex-wrap items-center gap-2">
              ...
              >
                {isRefreshingMonthlyCredits ? "확인 중..." : "보너스 가치 확인"}
              </button>
            </div>
            <p className="max-w-[280px] text-[11px] text-slate-200 sm:text-right">
              신규 가입·이벤트로 지급되는 보너스 가치입니다. 기본 결제는 상품별 원화 단건 결제로 진행됩니다.
            </p>
          </div>
        </div>
      </div>
...
export default function PointsPage() {
  ...
  const handleRefreshMonthlyCredits = useCallback(async () => {
    ...
    try {
      ...
      pushToast("success", `보너스 가치를 확인했습니다. 현재 ${formatMonthlyCredits(latestMonthlyCredits)}입니다.`);
    } catch (error: unknown) {
      pushToast("error", getErrorMessage(error, "보너스 가치 확인에 실패했습니다."));
    } finally {
  ...
  const fetchMyPointState = useCallback(
    async () => {
      ...
      if (!response.ok) {
        ...
        throw new Error(payload.message || "결제 및 보너스 가치 정보를 불러오지 못했습니다.");
      }
  ...
  /* ── 부팅 후 보너스 가치/주문 정보 로드 ─────────────────────────────── */
  useEffect(() => {
    if (isBooting) return;

    fetchMyPointState().catch((error) => {
      pushToast("error", getErrorMessage(error, "결제 및 보너스 가치 정보를 불러오지 못했습니다."));
    });
  ...
  const requestCancelPayment = useCallback(
    async (payment: PaymentHistoryItem) => {
      const ok = window.confirm(
        `${formatWon(payment.paymentAmount)} 결제를 취소할까요?\n이미 사용한 보너스 가치 또는 이용 내역이 있으면 취소가 제한될 수 있습니다.`,
      );
      if (!ok) return;
  ...
  const handleSubscribeWithMonthlyCredit = async (plan: SubscriptionPlan) => {
    ...
    setIsProcessing(true);
    setProcessingStage(`${plan.title} 보너스 가치를 확인하고 있습니다.`, "monthly");
    ...
    try {
      ...
      if (latestMonthlyCredits < requiredMonthlyCredits) {
        pushToast("error", `보너스 가치가 부족합니다. 필요: ${formatMonthlyCredits(requiredMonthlyCredits)}`);
        return;
      }

      setProcessingStage(`${plan.title}을 보너스 가치로 활성화하고 있습니다.`, "monthly");
      const confirmData = await confirmSubscriptionWithServer({
      ...
      await syncSubscriptionAppliedStage();
      pushToast("success", confirmData.message || `${plan.title}이 보너스 가치로 활성화되었습니다.`);
      setShowStarBurst(true);
      setTimeout(() => setShowStarBurst(false), 1200);
    } catch (error: unknown) {
      pushToast("error", getErrorMessage(error, "보너스 가치 사용에 실패했습니다."));
    } finally {
      releasePaymentActionLock(actionLockKey);
      setIsProcessing(false);
    }
        ...
        >
          <div className="w-full max-w-md rounded-[20px] border border-amber-200/35 bg-[#111832] p-5 text-slate-100 shadow-[0_24px_70px_rgba(0,0,0,0.45)]">
            ...
            <div className="mt-4 grid gap-2">
              ...
              >
                <span className="block text-sm font-black">보너스 가치 사용</span>
        ...
        >
          <div className="w-full max-w-sm rounded-[20px] border border-amber-200/35 bg-[#111832] p-5 text-slate-100 shadow-[0_24px_70px_rgba(0,0,0,0.45)]">
            ...
            <p className="mt-3 text-sm leading-relaxed text-slate-200">
              {pendingMonthlyCreditPlan.title} 활성화에 보너스 가치 {formatMonthlyCredits(pendingMonthlyCreditCost)}를 사용합니다.
            </p>
            <div className="mt-4 rounded-[14px] border border-white/10 bg-white/[0.07] px-3 py-2 text-[12px] text-slate-200">
              <p>현재 보너스 가치 {formatMonthlyCredits(currentMonthlyCredits)}</p>
              <p>사용 후 예상 가치 {formatMonthlyCredits(Math.max(0, currentMonthlyCredits - pendingMonthlyCreditCost))}</p>
      ...
      <div className="relative mx-auto w-full max-w-6xl space-y-5">
            ...
            <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
              <div className="flex items-center gap-4">
                ...
                <div>
                  ...
                  <p className="mt-1 text-sm text-slate-200">
            달빛 이용권 상품과 보너스 가치를 한 화면에서 확인하세요.
                  </p>
                  <p className="mt-1 text-[12px] text-[#f3dd9a]">
                    기본 결제 단위는 원화이며, 보너스 가치는 신규 가입·이벤트 보너스로만 지급됩니다.
                  </p>
                </div>
              </div>
        ...
        <section className="rounded-[20px] border border-white/12 bg-white/[0.08] p-5">
          <div className="mb-3 flex items-center justify-between gap-3">
            <h3 className="font-bold text-white">보너스 가치 사용 내역</h3>
            <span className="text-[11px] font-semibold text-slate-300">지급 / 사용 / 잔액</span>
          </div>

          {monthlyCreditLedgers.length === 0 ? (
            <p className="text-sm text-slate-300">아직 보너스 가치 사용 내역이 없습니다.</p>
          ) : (
                ...
                return (
                  ...
                  >
                    ...
                    <div className="mt-2 grid gap-1 text-[11.5px] text-[#7A5230] sm:grid-cols-2">
                      ...
                      {paymentMethodHint ? <p>보너스 가치 표시: {paymentMethodHint}</p> : null}
