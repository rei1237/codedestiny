...
export async function generateMetadata({ params }) {
  ...
  const content = I18N_SEO_PAGES.sukuyo[locale];
...
export default async function LocaleSukuyoPage({ params }) {
  const locale = resolveLocale((await params).locale);
  const content = I18N_SEO_PAGES.sukuyo[locale];
  ...
  return (
    <I18nSeoPageTemplate
      ...
      languageLinks={getLocaleLinksForRoute("sukuyo")}
      content={content}
      currentPath={getAlternatesByRouteKey("sukuyo")[locale]}
