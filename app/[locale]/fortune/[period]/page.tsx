...
export function generateMetadata({ params }: { params: { locale: string; period: string } }) {
  const locale = resolveLocale(params.locale);
  if (!locale || !isFortunePeriodId(params.period)) return {};
  const label = periodLabel(params.period, locale);
  const base = `/fortune/${params.period}`;
...
export default function LocalizedFortunePeriodPage({ params }: { params: { locale: string; period: string } }) {
  const locale = resolveLocale(params.locale);
  if (!locale || !isFortunePeriodId(params.period)) notFound();
  const period = params.period as FortunePeriodId;
  ...
  return <>
    <main className="cd-yeoni-surface min-h-screen bg-[linear-gradient(180deg,#fffaf7_0%,#fff3f8_44%,#fffaf7_100%)] pb-24 text-[#3c1830] dark:bg-[linear-gradient(165deg,#3a0e28_0%,#2e0a20_55%,#24081a_100%)] dark:text-[#fff1f7]">
      <div className="mx-auto max-w-5xl px-4 pt-8 sm:px-6 sm:pt-12">
        ...
        <p className="mt-3 max-w-2xl text-sm leading-7 text-[#70445c]">{description} Read the score as a reflection prompt and choose the pace that fits your real schedule.</p>
