...
[작성 지시]
거시 경제 애널리스트이자 인생 전략가로서 현재의 '대한(10년 운)'을 분석해 5,000자 이상 작성하세요.
...
한국어로 작성하세요.`;
    ...
    const full16     = rawText16 || "분석 중 오류가 발생했습니다.";
  ...
  if (chapter === 15) {
    ...
    const prompt15 = `리더십 코치이자 비즈니스 컨설턴트로서 부모궁과 자녀궁 데이터를 분석하여 5,000자 이상 작성하세요.
...
## 1. 윗사람 대하는 처세술
[1,500자] 부모궁의 에너지를 분석하여, 직장 상사·멘토·투자자 등 권위 있는 사람들의 지원을 끌어내는 맞춤형 소통 방식과 윗사람 복(인덕)을 설명하세요.
[QUOTE]: 이 섹션의 핵심 인사이트를 한 문장으로.

## 2. 부하직원 다루는 처세술
[1,500자] 자녀궁을 현대적으로 해석하여, 부하직원·후배를 육성하는 리더십 스타일과 나의 창작물(지식재산, 프로젝트, 자본)이 확장되는 패턴을 분석하세요.
[QUOTE]: 이 섹션의 핵심 인사이트를 한 문장으로.
...
한국어로 작성하세요.`;
    ...
    const full15    = rawText15 || "분석 중 오류가 발생했습니다.";
...
[작성 지시]
마이크로 타임 매니저로서 올해(유년)와 매월(유월)의 변화를 분석해 5,000자 이상 작성하세요.
각 섹션에 [QUOTE]: 로 시작하는 핵심 인사이트를 반드시 포함하세요. 미신적 표현 없이 작성하세요.

## 1. 연간 기조
[1,500자] 올해 ${yearGanZhi}년 유년 궁의 별자리 이동이 가져오는 전체적인 테마와 핵심 미션을 설명하세요. 유년 주성이 어떤 에너지를 가져오는지, 선천 명반과 어떻게 공명하는지 분석하세요.
[QUOTE]: 이 섹션의 핵심 인사이트를 한 문장으로 담아주세요.

## 2. 월별 모멘텀
1월부터 12월까지 각 월의 운세를 2-3문장씩 분석하세요. 이사, 계약, 이직, 투자, 만남 등 중요한 결정을 내리기 좋은 최적의 타이밍을 짚어주세요.
형식: "X월: [분석 내용 2-3문장]" (각 월별로 줄바꿈하여 작성, 반드시 "1월:", "2월:" ... "12월:" 순서로)
[QUOTE]: 이 섹션의 핵심 인사이트를 한 문장으로 담아주세요.
...
한국어로 작성하세요.`;
  ...
  if (chapter === 13) {
    ...
    const fullText13    = rawText13 || "분석 중 오류가 발생했습니다. 잠시 후 다시 시도해 주세요.";
    return NextResponse.json({
      daehanList,
      currentPeriod,
      chapter13: {
