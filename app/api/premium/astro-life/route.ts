...
const CHAPTER_META = [
  ...
  { num: 10, title: "시냅스트리 — 관계의 심리적 투사", subtitle: "궁합 1: 두 차트 행성 각도 분석",   icon: "🔮" },
  { num: 11, title: "컴포지트 — 우리라는 독립적 운명", subtitle: "궁합 2: 합산 차트 분석",           icon: "⭕" },
...
의식적 목적(태양)이 어느 무대(하우스)에서 빛을 발하는지 분석하고 실현 방법을 제시하라.

## 3. 달의 정서적 본능 — ${p.Moon.signKo} ${p.Moon.house}하우스의 안전가옥
무의식적 감정 반응 패턴과 정서적 안정감을 얻는 방식을 분석하라.

## 4. Triad 역학 — ASC·Sun·Moon의 충돌과 조화
세 요소가 서로 충돌하거나 보완하는 지점을 에스펙트와 원소·모달리티를 중심으로 입체적으로 분석하라.
...
4하우스 행성(${getPlanetsInHouse(chart, 4)})이 암시하는 가정 환경과 성인이 된 지금도 반복되는 정서 패턴을 분석하라.
...
수성의 별자리와 하우스가 학습 방식·소통 스타일·정보 처리 패턴에 미치는 영향을 분석하라.

## 2. 사고의 편향성 진단
수성이 맺고 있는 주요 에스펙트(${fmtAspects(chart.aspects, ["Mercury"])})를 통해 인지적 강점과 맹점을 분석하라.
...
아름다움·풍요·관계에서 이 금성 배치가 만들어내는 취향과 가치관을 분석하라.

## 2. 재물과의 무의식적 관계
2하우스(${getPlanetsInHouse(chart, 2)})가 암시하는 돈에 대한 무의식적 공포와 풍요 블록을 진단하고 치유 전략을 제시하라.

## 3. 관계의 끌림 패턴
7하우스(${getPlanetsInHouse(chart, 7)})의 에너지를 통해 연인·파트너십에서 반복되는 패턴을 분석하라.
...
이 화성 배치가 만들어내는 추진력의 특성, 행동 방식, 목표 추구 패턴을 분석하라.

## 2. 에너지 고갈 시 나타나는 그림자
에너지가 소진될 때 나타나는 공격성·무력감·번아웃의 패턴과 조절법을 심리학적으로 분석하라.
...
이 목성 배치가 인생에서 가장 저항 없이 행운이 들어오는 영역과 방식을 분석하라.
...
이 세대의 천왕성 에너지가 개인의 삶에서 어떻게 독창적인 혁신으로 발현되는지 분석하라.

## 2. 해왕성의 영감과 환상 — ${p.Neptune.signKo} ${p.Neptune.house}하우스
직관·예술·영성이 이 배치에서 어떻게 나타나며, 환상과 현실의 경계를 어떻게 다룰지 분석하라.

## 3. 명왕성의 변용 에너지 — ${p.Pluto.signKo} ${p.Pluto.house}하우스
생사(生死)와 변혁의 에너지가 개인을 어떻게 해체하고 재탄생시키는지 분석하라.
...
전생(前生)의 익숙한 패턴과 자꾸만 안주하려는 습관적 행동 양식을 날카롭게 분석하라.

## 2. 노스 노드의 부름 — ${nn.signKo} ${nn.house}하우스의 영혼 목적지
두렵지만 반드시 나아가야 할 이번 생의 진화 방향을 구체적으로 제시하라.

## 3. 노드 축의 균형 맞추기
사우스와 노스 사이에서 극단에 치우치지 않고 균형을 맞추며 진화하는 방법을 분석하라.
...
※ [챕터 10: 시냅스트리]는 두 사람의 차트 비교 분석이지만, 단일 차트 데이터로 분서 시에는 이 사람이 상대방에게 투사하는 심리적 패턴을 분석한다.

[챕터 10 작성 지시]
## 1. 관계에서의 심리적 투사 패턴
태양·달·금성·화성의 별자리와 하우스를 기반으로, 이 사람이 연인·파트너에게 투사하는 심리적 패턴을 분석하라.

## 2. 상대를 자극하는 트리거 포인트
토성·명왕성의 하드 에스펙트(${fmtAspects(chart.aspects, ["Saturn","Pluto"])})로 인해 관계에서 반복되는 갈등 패턴을 분석하라.
...
[챕터 11 작성 지시]
※ 컴포지트 차트는 두 차트의 중간점이 필요하나, 단일 차트 분석 시에는 '이 관계(차트 주인의 입장)가 세상에 만들어낼 수 있는 공적 에너지'를 분석한다.

## 1. 제3의 에너지 — 이 사람이 관계에서 창조하는 힘
ASC와 7하우스(${getPlanetsInHouse(chart, 7)})를 중심으로 이 사람이 관계에서 창조하는 독자적 에너지를 분석하라.
...
차트의 전반적인 패턴(원소 균형·모달리티·반구 강세)을 분석하고 인생의 큰 맥락을 서술하라.
...
${style}`;
    ...
    default:
      return `${baseData}\n\n챕터 ${ch}에 대한 점성술 심층 분석을 5,000자 이상 작성하라.\n${style}`;
  }
}
