...
궁합 해설: 본인(${mansionFull})과 상대(${pMansionFull})의 에너지 역학을 중심으로 궁합을 심층 분석할 것`;
  })() : '';
...
반드시 ${mansionFull}의 고유 언어와 비유로 다른 숙요와 차별화`;
  ...
  switch (chapterNum) {
    case 1:
      return `당신은 동양 최고의 숙요점(宿曜點) 심층 분석 전문가입니다.
...
## 3. 빛과 그림자
"${t.hidden}" — 이 내면의 그림자를 심리학적으로 완전 해부하고, 강점이 동시에 맹점이 되는 이유를 분석하세요.
...
## 2. 에너지 상승기 vs 하강기
${mansionFull} 태생의 에너지가 최고조최저점에 달하는 달의 시기와 각 시기별 최적 행동 전략을 분석하세요.
...
"${t.wealth}" — 이 재물 코드를 완전 해부하여 이 숙요 태생이 돈을 끌어당기는 고유 패턴을 심층 분석하세요.

## 2. 달의 상승기에 집중할 투자 원칙
음력 사이클 중 이 숙요 태생에게 최적의 재정 결정 타이밍과 각 시기별 투자저축지출 전략을 제시하세요.

## 3. 재물 파괴 패턴 경고
"${t.hidden}" — 무의식적으로 저지르는 재물 소멸 패턴을 분석하고 구체적인 차단 전략을 제시하세요.
...
[작성 목차]
## 1. 당신이 조직에서 맡는 역할
"${t.karma}" — 이 카르마 패턴이 팀과 조직에서 어떤 역할로 발현되는지를 분석하세요.
...
[작성 목차]
## 1. 6대 관계 역학 완전 해부
안(安)괴(壞)성(成)쇠(衰)우(友)친(親) 각 관계 유형의 심리학적 의미와 "${t.karma}"의 카르마 패턴과 연계해 상세히 분석하세요.

## 2. 최고의 파트너와 최악의 파트너
성(成) 관계를 극대화하고 괴(壞)쇠(衰) 관계의 충격을 최소화하는 전략을 제시하세요.

## 3. 사랑과 우정의 달빛 지도
"${t.love}"의 사랑 코드를 기반으로 연애결혼우정에서 가장 잘 맞는 상대와 조심해야 할 상대를 분석하세요.
...
[작성 목차]
## 1. 나를 성장시키는 공간의 법칙
${mansionFull}의 에너지(${sukuyo.element}, ${sukuyo.direction}방, 상징: ${sukuyo.animal})와 조화로운 공간 환경의 특성을 분석하세요.
...
## 2. 깊은 연결을 만드는 달빛 기술
"${t.karma}"의 카르마 패턴에서 드러나는 감정 교류 패턴을 분석하고, 상대방과 진정한 유대를 형성하는 기술을 제시하세요.
...
[작성 목차]
## 1. 가까이해야 할 에너지
"${t.karma}"의 카르마 패턴과 성(成)친(親)우(友) 관계 유형을 통해 삶을 풍요롭게 만드는 사람장소습관의 공통 특성을 분석하세요.

## 2. 멀리해야 할 에너지
괴(壞)쇠(衰) 에너지가 ${mansionFull} 태생에게 미치는 영향을 구체적으로 분석하고 차단 전략을 제시하세요.
...
"${t.core}"의 본질 에너지가 달의 8단계 각각에서 어떻게 증폭약화되는지 상세히 분석하세요.
...
[작성 목차]
## 1. 毒이 되는 관계 패턴 해독
"${t.hidden}" — 이 내면의 그림자가 독성 관계를 끌어당기는 메커니즘을 심리학적으로 분석하세요.
...
"${t.core}" + "${t.karma}" — 이 두 에너지 축을 기반으로 향후 10년의 주요 에너지 흐름(상승기전환기정화기)을 달의 대주기와 연계해 분석하세요.
...
${styleGuide}`;
    ...
    default:
      return `${mansionFull}의 달빛 전략 챕터 ${chapterNum}을 심층 분석하는 3,500자 이상의 리포트를 작성하세요.\n\n${baseCtx}\n\n${styleGuide}`;
  }
}
...
function buildFallbackChapterText(sukuyo: SukuyoCalcResult, chapterNum: number): string {
  ...
  return `## ${meta.icon} ${meta.title}\n### ${meta.subtitle}\n\n` +
    ...
    `**사랑 코드**\n\n${t.love}\n\n` +
    `**재물 코드**\n\n${t.wealth}\n\n` +
    `---\n*💡 AI 분석 서비스가 일시적으로 응답하지 않아 기본 데이터를 표시합니다. 챕터를 다시 불러오려면 재시도 버튼을 사용하세요.*`;
}
