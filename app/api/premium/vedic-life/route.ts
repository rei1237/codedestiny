...
라그나 지배성인 ${llPos?.nameKo??lagnaLord}이 ${llPos?.signSanskrit??""} ${llPos?.house??0}하우스에 위치한 의미를 분析하라. 이 배치가 인생 여정과 주요 관심사를 어떻게 형성하는가?

## 3. 아트마카라카(Atmakaraka) — 영혼의 지표성
아트마카라카인 ${c.atmakaraka.nameKo}이 이번 생의 영혼 숙제를 어떻게 암시하는지 분析하라. 왜 이런 배치로 태어났는지 카르마(Karma) 관점에서 설명하고, 영혼의 최종 목적(Moksha)에 다가가는 방법을 제시하라.

## 4. 카르마 원인과 현생 환경의 연결
이번 생에 이런 라그나와 아트마카라카를 받은 영혼적 이유를 전생의 카르마 관점에서 분析하라.
...
[챕터 2 — 나크샤트라(Nakshatra) 심층 분析 작성 지시]
27나크샤트라 중 ${nak.ko}(${nak.name})에서 태어난 사용자의 무의식적 본능을 분析하라.

## 1. ${nak.ko} 나크샤트라의 신화적 세계
수호신(${nak.deity})과 상징(${nak.symbol})의 신화적 배경을 깊이 설명하라. 이 나크샤트라가 인류 집단 무의식에서 어떤 원형(Archetype)을 담당하는가?

## 2. 무의식적 반응 패턴 진단
${nak.ko} pada ${nak.pada}에서 태어난 사람의 심리적 특성, 감정적 반응 패턴, MBTI를 초월하는 정밀한 성격 지층을 분析하라.

## 3. 감정적 결핍의 뿌리
이 나크샤트라가 만들어내는 무의식적 결핍(Shadow)과 그것이 대인관계·커리어·건강에 미치는 영향을 분析하라.
...
현재 ${md.current?.planet??"-"} 대운 / ${md.antar?.planet??"-"} 세운을 분析하라.

## 1. 현재 다샤의 계절 — ${md.current?.planet??"-"} 대운의 본질
${md.current?.planet??"-"} 행성이 지배하는 이 시기의 핵심 테마를 분析하라. 씨를 뿌릴 때인가, 수확할 때인가, 아니면 수성(守成)의 겨울인가?

## 2. ${md.antar?.planet??"-"} 세운의 미시 전략 (종료: ${md.antar?.endDate??"-"})
현재 세운 행성의 특성에 따른 1년 단위 구체적 전략을 제시하라.

## 3. 다샤 전환점 — 다음 대운 준비
다음 대운(${md.upcoming?.planet??"-"})이 시작되는 ${md.upcoming?.startDate??"-"}을 앞두고, 지금 준비해야 할 핵심 과제를 상세히 분析하라.
...
2하우스(자산), 11하우스(이익), 목성·금성을 분析하라.

## 1. 다나 요가(Dhana Yoga) 진단
차트에 형성된 재물 요가(${c.yogas.filter(y=>y.nameKo.includes("다나")).map(y=>y.nameKo).join(",")||"잠재적 요가"})를 분析하라. 2하우스 지배성(${h2lord})과 11하우스 지배성(${h11lord})의 배치가 재물 창출에 미치는 영향을 진단하라.

## 2. 목성(${jup?.signSanskrit??""})과 금성(${ven?.signSanskrit??""})의 재물 신호
베다 점성술에서 재물의 핵심 지표성인 목성(Guru/Brihaspati)과 금성(Shukra)의 현재 상태를 분析하라.
...
10하우스, 나밤샤(D9), 다샴샤(D10)를 교차 분析하라.

## 1. 10하우스(카르마 스타나)의 비밀
10하우스 지배성(${h10lord})의 배치를 분析하라. 이 사람이 사회에서 어떤 역할을 통해 최고의 잠재력을 발휘하는가?

## 2. D10(다샴샤) 차트 — 직업 운명의 정밀 진단
다샴샤 차트에서 태양 위치(${d10sun?.signKo??""})를 중심으로 직업적 성취의 신호를 분析하라.

## 3. 천직(Dharma) 발견 공식
단순한 직업 추천을 넘어, 이 차트 주인이 사회적 권위와 명예를 얻는 메커니즘을 분析하라. 어떤 분야에서 이 사람이 자연스럽게 권위자로 대우받는가?

## 4. 조직 내 생존 전략과 리더십 스타일
이 행성 배치에서 나타나는 직장 생존법, 리더십 스타일, 상하관계 전략을 행동과학 관점에서 분析하라.
...
[챕터 6 — 나밤샤(Navamsa, D9) 심층 분析 작성 지시]

## 1. 나밤샤 차트의 비밀 — 영혼의 진짜 힘
D1(기본 차트)과 D9(나밤샤)의 차이를 설명하고, D9가 보여주는 영혼의 진짜 저력을 분析하라. 달의 D9 위치(${d9moon?.signKo??""})가 의미하는 내면의 감정적 성숙도를 분析하라.

## 2. 중년 이후의 运命的 전환
나밤샤 차트는 인생 전반기보다 후반기에 더 강하게 발현됨을 설명하고, 이 배치에서 나이 들수록 진정한 재능이 어떻게 꽃필지 분析하라.

## 3. 현재의 시련이 영혼을 단련하는 방식
D9가 보여주는 영혼의 최종 형태에 도달하기 위해 지금 통과하는 시련의 의미를 분析하라.
...
[챕터 7 — 아슈타 쿠타(Ashta Koota) 궁합 분析 작성 지시]
달 나크샤트라를 기반으로 한 8가지 베다 궁합 지표를 분析하라.

## 1. 아슈타 쿠타 8항목 해설
베다 궁합의 8가지 척도(Varna·Vashya·Tara·Yoni·Graha Maitri·Gana·Rashi·Nadi)를 ${moonNak.ko}(${moonNak.name}) 나크샤트라 기준으로 분析하라. 36점 만점 체계에서 각 항목의 의미와 가장 조화로운 나크샤트라를 설명하라.

## 2. ${moonNak.ko}의 심리적 주파수
이 나크샤트라가 방출하는 심리적 주파수가 다른 나크샤트라와 공명하거나 충돌하는 원리를 심리학적으로 분析하라.

## 3. 관계 갈등의 패턴 진단
${moonNak.ko} 의 Gana(가나: 신性/인간性/악마性)와 Nadi(나디: 체질 궁합) 특성이 관계에서 유발하는 반복적 갈등 패턴을 분析하라.
...
7하우스, 금성(${venus?.signSanskrit??""} ${venus?.house??0}하우스), 화성(${mars?.signSanskrit??""} ${mars?.house??0}하우스)을 분析하라.

## 1. 인연의 색깔 — 7하우스가 그리는 파트너의 초상
7하우스 지배성(${h7lord})의 배치와 7하우스의 행성(${c.houseTable[6]})이 암시하는 인연의 형태를 분析하라.

## 2. 연애 패턴의 카르믹 뿌리
금성(Shukra)과 화성(Mangala)의 현재 배치에서 반복되는 연애 실패의 카르마적 원인을 찾아라.

## 3. 나를 끌어당기는 무의식의 필터
어떤 유형의 사람에게 자꾸 끌리는지, 그 무의식적 매력 필터의 심리학적 구조를 분析하라.
...
[챕터 9 — 생명력과 정화 작성 지시]
6하우스(질병), 8하우스(사고·수술), 12하우스(입원·손실)와 아유르베다를 연계 분析하라.
...
8하우스(${h8lord})와 12하우스(${h12lord})의 흐름을 분析하고, 독소 축적과 에너지 고갈이 발생하는 시기와 원인을 설명하라.
...
## 3. 요가 활성화 조건과 최적의 무대
이 요가들이 가장 강하게 발현되는 다샤 시기, 환경, 직업적 조건을 분析하라.
...
## 1. 흉성 에너지 진단
현재 차트에서 가장 강한 흉성의 영향(${maleficPlanets})을 분析하고, 이것이 일상에서 어떤 방식으로 발현되는지 구체적으로 진단하라.
...
1~11챕터의 모든 분析을 종합하여 최종 미션을 작성하라.

## 1. 차트 전체 요약 — 별들이 그린 운명의 지도
라그나·아트마카라카·달의 낙샤트라·현재 다샤·검출된 요가를 종합하여 이 영혼의 전체 여정을 웅장하게 서술하라.

## 2. 단 하나의 북극성 (North Star)
이 모든 분析을 통합했을 때, 이 사람이 나아가야 할 단 하나의 방향성을 제시하라. 별들이 이 영혼에게 가장 강하게 요청하는 것은 무엇인가?
...
${STYLE_GUIDE}`;
    ...
    default:
      return `${bd}\n\n챕터 ${ch}에 대한 베다 점성술 심층 분析을 5,000자 이상 작성하라.\n${STYLE_GUIDE}`;
  }
}
