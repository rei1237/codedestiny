...
function lyricsFromAudioFileName(artistKey: ArtistKey, audioFileName: string) {
  if (artistKey !== "yeoni") return undefined;
  ...
  if (normalized.includes("운명에게 지지 않아") || (normalized.includes("destiny") && normalized.includes("not lose")) || normalized.includes("운명")) {
    const lyric = [
      "[Intro]",
      "시간은 참 빠르게 흘러",
      "내가 걸어온 길이",
...
function buildTrack(artistKey: ArtistKey, audioFileName: string, index: number): Track {
  ...
  return {
    ...
    lyrics: lyricsFromAudioFileName(artistKey, audioFileName),
    order: index + 1,
  };
}
