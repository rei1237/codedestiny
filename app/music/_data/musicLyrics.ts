...
export function lyricsFromAudioFileName(audioFileName: string) {
  ...
  if (normalizedSongKey === "moonlight tea") {
    return `[Intro]
딸랑, 문이 열리면
달빛 향이 번져와
오늘의 운세 한 잔
...
[Post-Chorus]
La-la-la, moonlight tea
La-la-la, lucky dream
La-la-la, 나의 별을 켜
운명의 찻집에서 내가 정해
...
별이 되어서라도…`;
  ...
  if (normalizedSongKey === "milky way angle") {
    return `[Intro — whispered, synth pulse building]
천문도 위에 새겨진 우리의 각도
너의 금성이 내 화성에 닿는 순간
우주가 멈췄어

[Verse 1 — main vocal, smooth and low]
처음 봤을 때 알았어
네 눈빛 안에 시리우스가 살고 있다는 걸
내 출생 차트 7번 하우스
거기 네가 딱 서 있었어
...
[Verse 2 — rapper, sharp and confident]
시나스트리 차트 펼쳐봐
두 개의 원이 겹치는 그 지점
트라인 각도 120도
우리 사이 마찰이 없어 완벽해
...
[Pre-Chorus — 3명 하모니, tension 빌드업]
은하수 건너편에 네가 서 있고
나는 오리온 허리띠를 따라가
별이 정렬되는 건 한순간이야
이 각도가 사라지기 전에

[Chorus — 전원 유니즌, 최대 에너지 드롭]
은하수를 가로질러 너에게 닿아
시나스트리가 그린 우리의 궤도
페가수스 등을 타고 달려가
별자리가 말해 — 넌 내 거야
...
[Rap Break — aggressive flow, full fire]
합반 차트 뒤집어봐
어센던트가 딱 맞아 떨어져
소울메이트 각도 컨정션
두 행성이 겹치는 건 기적이야
...
[Bridge — stripped, 2성부 감성]
별이 된다는 건
빛나기 위해 타오른다는 것
너라는 은하수 속에서
나는 기꺼이 별이 될게

[Pre-Final Chorus — 키 반음 올려, 오케스트라 스웰]
은하수 건너편에 네가 서 있고
나는 오리온 허리띠를 따라가
별이 정렬되는 건 한순간이야
이 각도가 사라지기 전에 —

[Final Chorus — 풀 폭발, 코러스 레이어드]
은하수를 가로질러 너에게 닿아
시나스트리가 그린 우리의 궤도
페가수스 등을 타고 달려가
별자리가 말해 — 넌 내 거야
