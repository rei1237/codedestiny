...
const comparisonRows = [
  ...
  {
    axis: "출생시간의 무게",
    sukuyo: "현재 서비스에서는 출생시각과 시간대를 계산에 사용합니다. 시간 미상일 때는 기본 시각의 참고 결과이며, 구간 경계에서는 숙이 달라질 수 있습니다.",
...
const faqItems = [
  {
    ...
    answer:
      "이름의 역사적 대응과 개인의 계산 결과는 별개입니다. 현재 Code Destiny의 숙요점도 라히리 기준 항성 달 황경을 사용합니다. 두 화면의 날짜뿐 아니라 출생시각·시간대·아야남샤·이름 배열을 대조해야 합니다. 음력 월일표를 쓰는 다른 숙요 서비스와는 계산법이 다릅니다. 계산값의 일치 여부와 서로 다른 체계의 해석은 나누어 읽습니다.",
  },
  {
    question: "출생시간을 모르면 어느 쪽을 봐야 하나요?",
    answer:
      "숙요점에서도 사용한 기본 시각을 확인하고 결과를 잠정적으로 읽어야 합니다. 베다에서는 출생시각을 모른다는 사실을 남기고 라그나·하우스처럼 시간에 민감한 항목을 확정하지 않는 편이 좋습니다. 임시 시각을 넣은 결과는 실제 출생시각을 확인한 명반과 구분해야 합니다.",
  },
  {
  ...
  return (
    <main className="cd-main-shell cd-guide">
      ...
      <section className="cd-card">
        <h2>숙요점은 거리를 잽니다</h2>
        <p>
          숙요점에서 나를 가리키는 기준을 본명숙이라고 합니다. 현재 Code Destiny는 출생 순간의 달 항성황경을 계산해 27구간에 배정합니다. 과거 음력 월일표 설명과 구분하며, 천문 계산값이 성격이나 관계를 과학적으로 입증한다는 뜻은 아닙니다. 궁합에서는 내 본명숙과 상대의 본명숙 사이의 관계와 거리를 읽습니다.
        </p>
        <p>
      ...
      <section className="cd-card">
        ...
        <ol>
          ...
          <li>현재 도구가 음력 날짜표를 쓰는지 달 황경을 쓰는지 확인합니다. Code Destiny의 현행 숙요는 후자입니다. 이름 대응표만으로 결과를 같게 고치지 않습니다.</li>
