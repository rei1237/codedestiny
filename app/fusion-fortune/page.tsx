  ...
  return (
    <>
      ...
      <section aria-label="초융합 심층 리딩 이용 안내" style={{ maxWidth: 960, margin: "0 auto", padding: "32px 20px 56px", lineHeight: 1.7, wordBreak: "keep-all" }}>
        <h2>이용 안내</h2>
        <p>초융합 심층 리딩은 1회 30,000원의 회당 결제입니다. 완성된 결과는 계정에 저장되어 초융합 페이지에서 언제든 다시 열람할 수 있고, 그 화면에서 PDF로 내려받을 수 있습니다. 다시 여는 데에는 추가 결제가 없습니다. 결제창에서는 이용권·단건 결제·월정석을 함께 고를 수 있으며, 금액 상한 때문에 이용권으로는 family 등급이 커버됩니다.</p>
        <p>생성이 중간에 실패하면 같은 요청을 다시 보낼 때 추가 결제가 없습니다. 결제 증빙이 요청 번호에 묶여 있어, 재시도해도 두 번 결제되지 않습니다.</p>
        <p>연이와 나누는 가벼운 운명상담은 별도 상품입니다. 로그인하면 1회 무료이고(계정당 총량이며 매일 리셋되지 않습니다) 그 뒤로는 1회 5,000원입니다. 초융합과 서로 대체되지 않습니다. 두 상담 모두 미래를 단정하거나 보장하지 않고, 지금 확인할 점과 현실적인 다음 행동을 함께 제안합니다.</p>
      </section>
