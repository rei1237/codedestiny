  ...
  return (
    ...
    >
      <div className="flex flex-wrap items-baseline justify-between gap-x-4 gap-y-1.5 border-b border-white/[0.07] px-5 py-5 sm:px-8">
        <h2 className={`m-0 ${styles.readingTitle} text-[1.16rem] text-[var(--fx-ink-1)]`}>지난 초융합 다시 보기</h2>
        <p className="m-0 text-[0.85rem] text-[var(--fx-ink-4)]">이미 결제한 결과예요. 다시 여는 데 추가 결제는 없습니다.</p>
      </div>
            ...
            <li key={item.id} className="bg-[var(--fx-deep)]">
              ...
              >
                <span className="min-w-0 flex-1 font-display text-[0.98rem] leading-snug text-[var(--fx-ink-1)]">
                  {item.title || "초융합 운세"}
                  {current && <em className="ml-2.5 rounded-full bg-[rgba(232,213,163,0.16)] px-2.5 py-0.5 align-middle text-[0.7rem] not-italic text-[var(--fx-gold)]">지금 보는 중</em>}
                </span>
                ...
                <span className="text-[0.84rem] font-bold text-[var(--fx-gold-2)]">{busyId === item.id ? "여는 중…" : "열기"}</span>
              </button>
            </li>
          );
        })}
