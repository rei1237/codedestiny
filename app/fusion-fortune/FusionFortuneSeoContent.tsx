...
export const FUSION_FORTUNE_FAQS = [
  ...
  },
  {
    ...
    answer:
      "핵심 요약, 여섯 체계의 개별 해석, 여섯을 하나로 엮은 통합 리딩, 시기와 행동, 그리고 마지막 교차 판정까지로 구성됩니다. 여기에 체계별 신호 강도 레이더, 앞으로 12개월의 시기 라인, 겹치는 신호와 엇갈리는 신호를 정리한 교차 검증 지표가 함께 제공됩니다.",
  },
  {
    question: "가격과 이용권 적용은 어떻게 되나요?",
    answer:
      "1회 30,000원이며 매번 새로 계산하고 새로 쓰는 개인화 리딩이라 회당 결제입니다. 결제창에서는 이용권·단건 결제·월정석을 함께 고를 수 있고, 금액 상한 때문에 이용권으로는 family 등급이 커버됩니다. 생성이 중간에 실패하면 같은 요청을 다시 보낼 때 추가 결제가 없습니다.",
  },
  {
...
const DELIVERABLES = [
  ...
  { title: "여섯을 하나로 좁힌 최종 판정", body: "마지막에 각 체계가 결론에 동의하는지, 조건부인지, 속도를 늦추라는지 판정하고 그 분포에서 나온 합의 강도와 함께 지금 할 일 셋을 남깁니다." },
];
    ...
    <section className={styles.seoGuide} aria-labelledby="fusion-fortune-guide-heading">
      <header className={styles.seoGuideHeader}>
        ...
        <p>
          ...
          마지막에는 여섯 체계를 각각 판정한 뒤 하나로 수렴시킨 결론이 남습니다. 1회 30,000원입니다.
        </p>
      </header>
