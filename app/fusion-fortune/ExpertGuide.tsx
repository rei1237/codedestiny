...
export function ExpertGuide({ mode = "guide", systems }: { mode?: "guide" | "value" | "usage"; systems?: readonly FusionSystemKeyLabel[] }) {
  ...
  if (mode === "usage") return <section className={styles.seoGuide} aria-label={copy.input}><h2>{copy.input}</h2><p>{copy.required}</p><p>{copy.price} <PriceBadge featureKey="fusion-fortune-consultation" /></p><p>{copy.retry}</p></section>;
