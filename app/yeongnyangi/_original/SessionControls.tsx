...
export default function SessionControls({compact=false,onLogin}:{compact?:boolean;onLogin?:()=>void}){
 ...
 return <div className={'session-controls '+(compact?'session-compact':'')} data-session-status={status}>
  {status===0?<span role="status">로그인 확인 중</span>:status===200?<><button className="session-account" type="button" aria-expanded={open} onClick={()=>setOpen(!open)}><img src="/assets/yeongnyangi/original/hero-480.webp" alt="" width="32" height="32"/>{name?name+'님 · ':''}로그인됨</button>{open&&<nav className="session-menu" aria-label="내 계정"><a href="/yeongnyangi/library/">나의 보관함</a><a href="/">CODE DESTINY</a><button type="button" onClick={logout}>로그아웃</button></nav>}</>:status===401?<button type="button" onClick={()=>onLogin?onLogin():window.location.assign(loginHref(window.location.pathname+window.location.search))}>로그인</button>:<><span role="status">확인하지 못했어요</span><button type="button" onClick={()=>void refresh()}>다시 확인</button></>}
