...
export default function QuestionSkyConsultation({mode}:{mode:SkyMode}){
  ...
  async function prepare(event:React.FormEvent){
    ...
    if(question.trim().length<5||!cityId||!localTime){setError('질문과 질문이 떠오른 도시·시각을 확인해 주세요.');return;}
    ...
    try{
      const {fortune}=await fortuneApi<{fortune:FortuneRecord}>('requests',{mode,productId:product.id,question,questionSky:{relationship,situation,topic,boundary,cityId,localTime}});
  ...
  return <section className={`${styles.consultation} ${styles.spirit}`}>
    ...
    <form className={styles.form} onSubmit={prepare}>
      ...
      <label htmlFor="sky-city">질문이 떠올랐을 때 내가 있던 도시</label><select id="sky-city" required value={cityId} onChange={e=>setCityId(e.target.value)}><option value="">질문자 도시 선택</option>{questionCities.map(c=><option value={c.id} key={c.id}>{c.name}</option>)}</select>
      <p>질문 당시 하늘의 기준을 정하기 위해 도시 중심을 사용해. 상대방의 위치가 아니야. 목록에 없다면 다른 도시를 대신 고르지 말고, 지원 지역이 추가될 때 이용해 줘.</p>
      ...
      <div className={styles.checkoutSection}><h2>상담 내용 확인</h2><p>기존 고등어 상담의 질문 순간 풀이 · {product.chapterCount}개 이야기와 영냥이의 마무리</p><strong>{product.priceKRW.toLocaleString('ko-KR')}원 · 단건 결제</strong><p>{SPIRIT_NOTICE}</p>
        <button type="submit" disabled={busy||!ready||!available}>{busy?'질문 순간의 결을 살피는 중':'결제 내용 확인하기'}</button>
