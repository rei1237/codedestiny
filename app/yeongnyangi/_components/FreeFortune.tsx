...
export default function FreeFortune(){
 ...
 useEffect(()=>{let active=true;refresh().catch(reason=>{if(active)fail(reason);});
  const auth=()=>{version.current++;timers.current.forEach(clearTimeout);timers.current=[];setState(null);setReading(null);setNotice('');setGuest(false);setReaction('idle');refresh().catch(fail);};
  ...
  const tick=setInterval(()=>{const day=new Date(Date.now()+9*3600000).toISOString().slice(0,10);if(currentDay.current&&currentDay.current!==day){version.current++;setReading(null);setReaction('idle');refresh().catch(fail);}},15000);
  return()=>{active=false;version.current++;attendanceVersion.current++;clearInterval(tick);timers.current.forEach(clearTimeout);window.removeEventListener('cd:auth-changed',auth);window.removeEventListener('storage',storage);};},[]);
 ...
 async function action(kind:'attendance'|'free/unlock'){
  ...
  const v=attendanceVersion.current,account=readDestinyProfileAccountId();
  try{const next=await fortuneApi<AttendanceState>(kind,{});if(v!==attendanceVersion.current||account!==readDestinyProfileAccountId())return;setState(next);setNotice(kind==='attendance'?(next.awarded?'출석했어. 멸치 한 마리가 쌓였어.':'오늘 출석은 이미 했어.'):'오늘의 16종을 모두 열었어.');if(next.newlyUnlocked){const reduced=matchMedia('(prefers-reduced-motion: reduce)').matches;setReaction(reduced?'done':'receive');if(!reduced)timers.current.push(setTimeout(()=>setReaction('unimpressed'),400),setTimeout(()=>setReaction('done'),1300));}}
