...
const MINGRI_TAROT_PAGE_TEXT_TRANSLATIONS = {
  ko: {
    h1: "🔮 명리학 타로",
    description:
      "연애, 재회, 사업, 건강 등 고민의 문 위에 한 장 또는 세 장의 카드를 펼치고, 십성의 결로 오늘의 선택을 차분히 읽습니다.",
    landingPoints: ["질문별 오라클 리딩", "한 장 리딩과 세 장 흐름", "카드와 십성의 조율 해석"],
    seoText:
      "명리학 타로는 질문의 문맥 위에 카드를 놓고, 십성의 상징과 현실 조건을 함께 읽어 지금 필요한 선택의 방향을 부드럽게 비춥니다.",
    ...
    valueSections: [
      ...
      },
      {
        ...
        body:
          "타워나 검 계열 카드가 나오면 결과를 비관적으로 단정하기 쉽습니다. 하지만 카드의 목적은 공포 예고가 아니라 흐름의 그림자를 알려 주는 데 가깝습니다. 일정 재배치, 대화 방식 수정, 지출 상한 설정처럼 오늘 붙잡을 작은 선택을 한 줄로 정리하면 불안은 조금씩 다룰 수 있는 감각으로 바뀝니다.",
      },
      {
        title: "5. 연애·재회 리딩은 상대 규정보다 내 패턴 점검이 우선입니다",
        body:
    ...
    metadataTitle: "🔮 명리학 타로 - 카드와 십성이 비추는 선택 리딩",
    metadataDescription:
      "연애, 재회, 사업, 건강 등 고민의 문에 맞춰 한 장 리딩 또는 세 장 흐름으로 카드와 십성의 결을 읽습니다.",
  },
};

const mingriTarotKo = MINGRI_TAROT_PAGE_TEXT_TRANSLATIONS.ko;

const SERVICE = {
  h1: mingriTarotKo.h1,
  description: mingriTarotKo.description,
  ogImage: "https://code-destiny.com/fuctionassets/ai%20tarrot.webp",
  landingPoints: mingriTarotKo.landingPoints,
  seoText: mingriTarotKo.seoText,
  localized: {
    en: {
  ...
  valueGuideTitle: mingriTarotKo.valueGuideTitle,
  valueSections: mingriTarotKo.valueSections,
};

export const metadata = withUniqueRouteMetadata("/tarot/mingri", {
  title: mingriTarotKo.metadataTitle,
  description: mingriTarotKo.metadataDescription,
});
