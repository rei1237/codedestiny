...
export const PRIVACY_POLICY_SECTIONS = [
  ...
  },
  {
    ...
    body: (
      <>
        <p>
          회원가입 시 이름, 이메일, 휴대폰 번호, 필수 동의 기록을 계정 관리, 결제 고객정보 제공, 고객 응대, 알림 발송과 부정 이용 방지를 위해 처리합니다. 휴대폰 번호는 결제 대행사에 구매자 정보로 전달해야 하므로 삭제할 수 없는 대신, 서버에 저장할 때 AES-256 방식으로 암호화해 보관합니다. 이메일 가입 시에는 비밀번호 원문이 아닌 안전하게 해시된 값이 저장되며, 소셜 가입 시에는 선택한 인증 제공자의 계정 식별자가 연결됩니다. 생년월일, 성별, 출생시간, 출생지 등 운세 계산에 필요한 입력값은 가입 후 해당 운세 기능이나 프로필을 사용할 때 별도로 입력받아 결과 생성에 사용합니다.
        </p>
        <p>
