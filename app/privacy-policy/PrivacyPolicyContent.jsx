...
export const PRIVACY_POLICY_SECTIONS = [
  ...
  },
  {
    ...
    body: (
      <>
        <p>
          회원가입 시 이름, 이메일, 휴대폰 번호, 필수 동의 기록을 필수 항목으로 수집합니다. 이름과 이메일은 계정 관리, 고객 응대, 알림 발송과 부정 이용 방지를 위해 처리합니다. 휴대폰 번호는 이메일로 가입하시는 경우 회원가입 시 필수로 수집하며, 수집 방법은 세 가지입니다 — 이메일 회원가입 화면에서 회원님이 직접 입력하시거나, 소셜 로그인 제공자(네이버·카카오)가 회원님의 동의를 받아 전달하거나, 번호가 없는 회원(구글 로그인으로 가입하신 경우와 번호가 없는 기존 회원)께서 첫 카드 결제 화면에서 별도 동의와 함께 직접 입력하시는 경우입니다. 이용 목적은 회원 식별 및 계정 관리와 결제 진행 두 가지이며, 회사는 이 번호로 광고성 문자나 전화를 보내지 않습니다. 이 번호는 결제 대행사에 구매자 정보로 전달해야 하므로 서비스 이용 중에는 삭제할 수 없는 대신, 서버에 저장할 때 AES-256 방식으로 암호화해 보관합니다. 이메일 가입 시에는 비밀번호 원문이 아닌 안전하게 해시된 값이 저장되며, 소셜 가입 시에는 선택한 인증 제공자의 계정 식별자가 연결됩니다. 생년월일, 성별, 출생시간, 출생지 등 운세 계산에 필요한 입력값은 가입 후 해당 운세 기능이나 프로필을 사용할 때 별도로 입력받아 결과 생성에 사용합니다.
        </p>
        <p>
  ...
  },
  {
    ...
    body: (
      <>
        ...
        <p>
          이용자는 수집·이용 동의를 거부할 권리가 있습니다. 다만 필수 항목(이름, 이메일, 휴대폰 번호) 동의를 거부하시는 경우에는 이메일 회원가입을 통한 계정 생성이 불가능합니다. 휴대폰 번호가 없는 회원께서 첫 카드 결제 화면에서 동의를 거부하시면 단건 결제와 이용권 구매를 포함한 카드 결제를 진행하실 수 없고 보유하신 월정석으로만 이용하실 수 있습니다.
        </p>
        <p>
