...
export const PRIVACY_POLICY_SECTIONS = [
  ...
  },
  {
    ...
    body: (
      <>
        <p>
          회원가입 시 이름, 이메일, 필수 동의 기록을 계정 관리, 고객 응대, 알림 발송과 부정 이용 방지를 위해 처리합니다. 휴대폰 번호는 가입 시 받지 않으며, 소셜 로그인 제공자(네이버·카카오)가 회원님의 동의를 받아 전달한 경우 또는 첫 카드 단건결제 시 회원님이 별도 동의와 함께 직접 입력한 경우에만 수집하고 목적은 결제 진행 하나입니다. 이 번호는 결제 대행사에 구매자 정보로 전달해야 하므로 삭제할 수 없는 대신, 서버에 저장할 때 AES-256 방식으로 암호화해 보관합니다. 이메일 가입 시에는 비밀번호 원문이 아닌 안전하게 해시된 값이 저장되며, 소셜 가입 시에는 선택한 인증 제공자의 계정 식별자가 연결됩니다. 생년월일, 성별, 출생시간, 출생지 등 운세 계산에 필요한 입력값은 가입 후 해당 운세 기능이나 프로필을 사용할 때 별도로 입력받아 결과 생성에 사용합니다.
        </p>
        <p>
  ...
  },
  {
    ...
    body: (
      <>
        ...
        <p>
          이용자는 선택 항목의 수집·이용 동의를 거부할 권리가 있습니다. 휴대폰 번호는 가입 시 받지 않는 선택 항목이며, 첫 카드 단건결제 화면에서 동의를 거부하시면 카드 단건결제만 진행할 수 없고 이용권·월정석으로는 그대로 이용하실 수 있습니다. 필수 항목(이름, 이메일) 동의를 거부하는 경우에는 계정 생성이 불가능합니다.
        </p>
        <p>
          계정과 개인정보 삭제는{" "}
