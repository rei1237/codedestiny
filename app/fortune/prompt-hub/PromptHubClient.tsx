...
import type { MeihuaCalcResult, MeihuaMode } from "./meihua-calc";
import type { DangsajuCalendarType, DangsajuMode, DangsajuResult } from "./dangsaju-calc";
import type {
  LiteCalendarType,
  LitePromptMode,
  LitePromptResult,
...
const MEIHUA_MODES: Array<{ id: MeihuaMode; label: string; description: string }> = [
...
const TARGET_PURPOSES = ["계약", "이사", "창업", "고백", "시험", "발표", "여행", "병원 방문", "중요한 만남", "기타"];
const RELATIONSHIP_TYPES = ["연애", "결혼", "썸", "재회", "친구", "가족", "사업 파트너", "직장 관계", "기타"];

const DANGSAJU_MODES: Array<{ id: DangsajuMode; label: string; description: string }> = [
...
const DANGSAJU_RELATIONSHIP_TYPES = RELATIONSHIP_TYPES;

const LITE_PROMPT_MODES: Array<{ id: LitePromptMode; label: string; description: string }> = [
...
function formatMeihuaDateTime(date: Date) {
  const pad = (value: number) => String(value).padStart(2, "0");
  return `${date.getFullYear()}-${pad(date.getMonth() + 1)}-${pad(date.getDate())} ${pad(date.getHours())}:${pad(date.getMinutes())}`;
}

function getMeihuaQuestionNotice(question: string) {
  const trimmed = question.trim();
  if (!trimmed) return "";
  const questionMarks = (trimmed.match(/[?？]/g) || []).length;
  if (questionMarks > 1 || /\b(그리고|또|동시에|둘 다|여러 가지|각각)\b/u.test(trimmed)) {
...
function getDangsajuQuestionNotice(question: string) {
  const trimmed = question.trim();
  if (!trimmed) return "";
  const questionMarks = (trimmed.match(/[?？]/g) || []).length;
  if (questionMarks > 1 || /\b(그리고|또|동시에|둘 다|여러 가지|각각)\b/u.test(trimmed)) {
...
function getLiteQuestionNotice(question: string) {
  const trimmed = question.trim();
  if (!trimmed) return "";
  const questionMarks = (trimmed.match(/[?？]/g) || []).length;
  if (questionMarks > 1 || /\b(그리고|또|동시에|둘 다|여러 가지|각각)\b/u.test(trimmed)) {
...
export default function ComprehensivePromptHubPage() {
  ...
  async function generateMeihuaPrompt() {
    ...
    let meihuaTools: typeof import("./meihua-calc");
    try {
      meihuaTools = await import("./meihua-calc");
    } catch {
      setMeihuaError(tx("입력값을 다시 확인해주세요."));
      return;
    }
    ...
    if (meihuaMode === "basic") {
      ...
      const result = meihuaTools.calculateBasicMeihua({
      ...
      setMeihuaPrompt(meihuaTools.buildMeihuaPrompt(result));
      setMeihuaError("");
    ...
    if (meihuaMode === "target") {
      ...
      const result = meihuaTools.calculateTargetDateMeihua({
        modeLabel: selectedMeihuaMode.label,
        name: meihuaName.trim(),
        gender: meihuaGender,
        birthDate: birth.value || meihuaBirthDate,
    ...
    const result = meihuaTools.calculateCompatibilityMeihua({
      modeLabel: selectedMeihuaMode.label,
      question: questionText,
  ...
  async function generateDangsajuPrompt() {
    ...
    try {
      const dangsajuTools = await import("./dangsaju-calc");
      const result =
        dangsajuMode === "basic"
          ? dangsajuTools.calculateDangsajuChart({
          ...
          : dangsajuTools.calculateDangsajuCompatibility({
              modeLabel: selectedDangsajuMode.label,
              personA: {
      ...
      setDangsajuPrompt(dangsajuTools.buildDangsajuPrompt(result));
  ...
  async function generateLitePrompt() {
    ...
    try {
      const { buildLiteFortunePrompt } = await import("./lite-prompt-tools");
  ...
  async function generateKuseiPrompt() {
    try {
      const { buildKuseiPromptPayload } = await import("./kusei-calc");
