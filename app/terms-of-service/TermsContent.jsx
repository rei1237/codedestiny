...
export const TERMS_SECTIONS = [
  ...
  },
  {
    ...
    body: (
      <>
        <p>
          이용자는 유효한 이용권 계약기간 동안 아래에서 정하는 비정상 이용에 해당하지 않는 한 추가 요금 없이 <strong>해당 등급의 적용 가격 범위(1회 정상 판매가 기준) 이하의 기능</strong>을 <strong>등급별 월 이용 한도 이내에서</strong> 이용할 수 있습니다(이하 &quot;정액 이용&quot;). 등급별 적용 가격 범위(스탠다드 5,000원 · 프리미엄 10,000원 · VVIP 꿀단지 20,000원 이하, Code Destiny Family 는 금액 상한 없음)와 월 이용 한도(스탠다드 30,000원 · 프리미엄 100,000원 · VVIP 꿀단지 200,000원 · Code Destiny Family 500,000원)는 결제 화면에 표시됩니다. 월 이용 한도에서 차감되는 금액은 <strong>해당 서비스의 정상 판매가</strong>이며, 할인·프로모션가가 적용된 경우에도 정상 판매가를 기준으로 합니다. 월 이용 한도가 0이 되어도 이용권의 만료일·등급·프로필 상한은 원래 만료일까지 유지되며, 추가 유료 콘텐츠는 단건 결제 또는 월정석으로 이용할 수 있습니다. Pass holders may use features whose standard list price is at or below their tier&apos;s coverage band (Standard KRW 5,000 · Premium KRW 10,000 · VVIP KRW 20,000; Code Destiny Family has no per-item cap) without additional charge, subject to each tier&apos;s monthly usage limit (Standard KRW 30,000 · Premium KRW 100,000 · VVIP KRW 200,000 · Code Destiny Family KRW 500,000), shown on the payment screen. The amount deducted from the monthly limit is the standard list price, even where a discounted or promotional price applies. At zero monthly allowance, the pass expiration, tier, and profile limit remain in effect until the original expiration date; additional paid content is available by single payment or Moonlight Stones.
        </p>
        <p>
          <strong>Code Destiny Family 이용권은 1회 정상 판매가에 상한이 없어</strong>, 전문가 상담과 같은 고가 상품도 위 월 이용 한도(500,000원) 안에서 이용할 수 있습니다. 그 외 등급은 정상 판매가가 해당 등급의 적용 가격 범위를 넘는 상품을 단건 결제 또는 월정석으로 이용합니다. <strong>이용 횟수 자체를 세는 별도의 포함 횟수 제도는 두지 않으며, 판정 기준은 적용 가격 범위와 월 이용 한도 둘뿐입니다.</strong> 월 이용 한도가 0이 되어도 이용권의 만료일·등급·프로필 상한은 원래 만료일까지 유지됩니다. 남은 한도는 결제 화면에서 확인할 수 있고, 추가 유료 콘텐츠에는 단건 결제 또는 월정석이 안내됩니다. 이용권 재구매는 결제 화면에 표시되는 구매 규칙을 따릅니다. <strong>월 이용 한도는 이 조에서 정하는 &quot;비정상 이용&quot;과 무관한 상품 구성이며, 모두 사용한 사실만으로는 어떠한 제재도 이루어지지 않습니다.</strong> The Code Destiny Family pass has no per-item price cap, so high-priced products such as expert consultations are included within its monthly usage limit (KRW 500,000). For other tiers, products priced above the tier&apos;s coverage band are purchased via single payment or Moonlight Stones. There is no separate included-use count: eligibility is decided by the coverage band and the monthly usage limit alone. At zero monthly allowance, the pass expiration, tier, and profile limit remain in effect until the original expiration date. The remaining allowance is shown on the payment screen, and additional paid content is offered by single payment or Moonlight Stones. Repurchasing a pass follows the purchase rules shown on the payment screen. This allowance is a product term, not a sanction, and exhausting it never blocks access.
        </p>
        <p>
