...
// 인증 상태를 읽는 화면이라 정적 export 프리렌더에 올리지 않는다(`/account/password` 와 같은 구성).
const PhoneChangeClient = dynamic(() => import("./PhoneChangeClient"), {
  ssr: false,
  loading: () => <PhoneChangeSkeleton />,
});

export default function PhoneChangeRouteClient() {
  return <PhoneChangeClient />;
}

function PhoneChangeSkeleton() {
  return (
