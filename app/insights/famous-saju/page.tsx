    ...
    <main className="min-h-screen bg-[#090b18] text-slate-100" data-famous-saju-list>
      <section className="mx-auto max-w-6xl px-5 py-12 sm:py-16">
        ...
        <section className="mt-10 max-w-3xl space-y-5 text-base leading-8 text-slate-300">
          ...
          <h2 className="text-xl font-semibold text-white">상세 페이지는 어떤 순서로 읽는가</h2>
          <p>
            각 인물의 상세 페이지는 같은 뼈대로 짜여 있습니다. 첫머리에 생년월일과 시주 포함 여부가 오고,
            그 아래 명식표에서 연주·월주·일주(시주가 있으면 시주까지)의 천간과 지지를 보여 줍니다. 처음
            읽는다면 표 전체를 해석하려 하지 말고 일주 위 글자, 곧 일간 하나만 먼저 짚어 두세요. 일간은
            명식에서 '나'에 해당하는 자리라, 뒤에 나오는 오행 균형과 십성 분석이 전부 이 글자를 기준으로
            계산됩니다. 일간을 놓치면 나머지 절이 왜 그렇게 쓰였는지 연결이 되지 않습니다.
          </p>
          <p>
            다음 절인 오행 균형은 목·화·토·금·수 다섯 기운 가운데 어느 것이 많고 어느 것이 비었는지를
            셉니다. 여기서 볼 것은 정확한 개수가 아니라 치우침의 방향입니다. 그 뒤 십성 분석은 일간과 다른
            글자들의 관계를 비견·식신·정재·정관 같은 이름으로 옮긴 것인데, 인물의 공개된 이력과 가장 자주
            맞물리는 자리가 이 절입니다. 신살 절은 원국에 더해지는 보조 표식이라 귀인·중성·주의 신호로만
            나누어 적고, 그것만으로 어떤 결론도 내리지 않습니다. 이어지는 행적 절은 명식 서술과 공개
            활동을 나란히 놓은 부분이고, 마지막의 문답과 연결 명식은 같은 궁금증을 가진 독자가 다음에 볼
            인물을 고르도록 붙여 둔 안내입니다.
          </p>
          <h2 className="text-xl font-semibold text-white">분야별로 묶어 둔 이유와 고르는 요령</h2>
          <p>
            인물은 역사 위인, 왕족·정치인, K-스타, 배우, 가수, 스포츠, 기업인, 감독·작가, 사상가·예술가,
            그리고 일본·중국·미국 세 나라의 인물로 나누어 두었습니다. 분야를 나눈 이유는 검색을 편하게
            하려는 것만이 아닙니다. 같은 일을 하는 사람들의 명식을 나란히 두면 그 분야에 유독 자주 보이는
            구성이 있고, 반대로 그 구성 없이도 같은 자리에 오른 사람이 있다는 사실이 함께 드러납니다.
            명식이 직업을 정한다는 말이 왜 성립하지 않는지는 이 대조에서 가장 분명하게 보입니다.
          </p>
          <p>
            어디서 시작할지 모르겠다면 시주가 확인된 인물부터 여는 편이 낫습니다. 네 기둥이 다 있어
            서술의 확신도가 높고, 삼주만 있는 글과 견주면 시주 한 자리가 얼마나 큰 차이를 만드는지 체감할
            수 있습니다. 해외 인물은 현지에서 공개된 생년월일을 그대로 쓰며, 날짜 경계에 걸리는 출생
            시각이 확인되지 않은 이상 시간대 차이로 일주가 바뀌는 경우는 없다고 보고 읽습니다. 역사
            인물은 음력 기록과 당시의 시간 기준 때문에 오차 범위가 더 넓다는 점을 염두에 두고 읽어 주세요.
          </p>
