...
export const metadata = createI18nMetadata({
  locale: "ko",
  routeByLocale: getAlternatesByRouteKey("insightZiweiBasics"),
  title: article.titleByLocale.ko,
  description: article.descriptionByLocale.ko,
  keywords: [article.titleByLocale.ko, "자미두수"],
  type: "article",
});

export default function ZiweiBasicsInsightPage() {
    ...
    <main className="mx-auto w-full max-w-4xl px-4 py-8 text-slate-100 md:px-6 md:py-10">
      <div className="rounded-3xl border border-white/10 bg-[#10182c] px-5 py-6 md:px-8 md:py-8">
        <div className="mb-4 flex flex-wrap items-center gap-2 text-xs text-slate-300">
          {getLocaleLinksForRoute("insightZiweiBasics").map((item) => (
            <Link
              key={item.hrefLang}
              href={item.href}
              hrefLang={item.hrefLang}
              lang={item.hrefLang}
              className={`rounded-full border px-3 py-1 ${item.href === I18N_ROUTE_MAP.insightZiweiBasics.ko ? "border-amber-300/60 bg-amber-100/10 text-amber-100" : "border-white/20 bg-white/5 text-slate-200 hover:bg-white/10"}`}
            >
              {item.label}
            </Link>
          ))}
      ...
      <article className="mt-6 rounded-3xl border border-white/10 bg-[#0f172a] px-5 py-6 md:px-8 md:py-8">
        {article.bodyByLocale.ko.map((paragraph, index) => (
          <p key={`ko-ziwei-${index}`} className="mb-4 text-sm leading-8 text-slate-200 md:text-base">
            {paragraph}
          </p>
        ))}
        <Link href="/ziwei" className="mt-4 inline-flex rounded-xl border border-cyan-300/35 bg-cyan-700/25 px-4 py-2 text-sm font-semibold text-cyan-100 hover:bg-cyan-600/35">
          자미두수 서비스로 이동
        </Link>
      </article>
