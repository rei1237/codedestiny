  ...
  return (
    <main className={`policy-doc ${policyPageClass}`}>
      <PolicyGuide
        kind="refund"
        title="환불·취소 안내"
        description="단건 결제와 30일 이용권의 환불·청약철회 기준입니다. 아래 전문은 이용약관 제12조와 동일합니다."
        meta={`시행일 ${TERMS_EFFECTIVE_DATE}`}
      />

      <PolicyAccordion id="payment-help" title="결제 반영이 늦거나, 결제 후 원래 화면으로 돌아오지 못했어요">
        <p>결제 완료 내역이 있는데 결과나 이용권이 보이지 않으면, 추가 결제 전에 결제 내역과 현재 이용 상태를 확인해 주세요.</p>
        <p>결제 시각·상품명·결제 내역과 문제가 발생한 화면을 고객센터에 알려주세요. 카드번호 전체나 비밀번호는 보내지 마세요.</p>
