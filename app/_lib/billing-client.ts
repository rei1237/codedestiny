...
export async function runBillingCoinGate(input: {
  ...
  if (typeof window !== "undefined" && typeof (window as any)._cdCoinGatePerUse === "function") {
    let cost = 50;
    try {
      ...
      if (eligibilityResult?.ok && eligibilityResult.data) {
        cost = eligibilityResult.data.coinCost;
      }
    } catch (e) {
      console.warn("[billing-client] failed to fetch eligibility for cost, using fallback 50", e);
    }
    ...
    return new Promise((resolve) => {
      ...
      const reason = input.reason || "유료 콘텐츠 분석";
      
      (window as any)._cdCoinGatePerUse(
        cost,
        reason,
        (transactionId: string, payload: any) => {
          resolve({
            ok: true,
            status: 200,
            data: {
              ...
              balance: payload?.balance ?? payload?.remainingPoints ?? payload?.user?.points ?? null,
              user: payload?.user ?? null,
            },
            message: payload?.message || "결제가 완료되었습니다.",
            error: null,
            raw: payload || {},
          });
        },
        (error: any) => {
          resolve({
            ...
            status: 402,
            data: null,
            message: error?.message || "결제가 취소되었거나 실패했습니다.",
            error: {
              code: error?.code || "PAYMENT_CANCELLED",
              message: error?.message || "결제가 취소되었거나 실패했습니다.",
            },
            raw: error || {},
          });
