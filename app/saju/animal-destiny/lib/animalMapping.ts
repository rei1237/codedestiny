...
function buildDefaultBreakdown(myAnimal: string, partnerAnimal: string, relationType: "환상" | "좋음" | "긴장" | "주의") {
  ...
  return {
    overall: {
      title: "종합 궁합",
      body: `${myAnimal}와 ${partnerAnimal}는 ${relationLine}입니다. 관계의 방향을 빠르게 정하려 하기보다, 서로의 기준과 기대치를 먼저 언어화할수록 장기 안정성이 높아집니다.`,
    },
    emotionCommunication: {
      title: "감정 및 소통 궁합",
      body: "감정의 온도와 표현 속도가 다를 수 있어, 오해는 대개 의도보다 전달 방식에서 발생합니다. 감정 해석보다 사실 확인을 먼저 하고, 감정은 짧고 명료한 문장으로 확인하는 습관이 관계 피로를 줄입니다.",
    },
    valueLifestyle: {
      title: "가치관 및 생활 패턴",
      body: "생활 리듬, 휴식 방식, 돈과 시간의 우선순위에서 차이가 드러날 가능성이 큽니다. 루틴을 강요하기보다 각자의 회복 방식과 일상 템포를 존중하면, 다른 성향이 갈등이 아니라 시너지로 바뀝니다.",
    },
    practicalAdvice: {
      title: "사주 고수의 실전 조언",
      body: "관계의 핵심은 감정 토론의 길이가 아니라 점검의 주기입니다. 월 1회 고정된 타이밍에 감정 온도, 생활 합의, 다음 한 달의 역할 분담을 함께 점검하면 궁합의 안정도가 눈에 띄게 올라갑니다.",
    },
  };
}
  ...
  if (isDogButterflyPair) {
    return {
      ...
      breakdown: {
        overall: {
          title: "종합 궁합",
          body: "명리학적으로 건록(建祿)의 현실적인 에너지와 사(死)의 정신적이고 직관적인 에너지가 만난 구조입니다. 별빛 강아지(건록)가 관계의 뼈대를 세우고 현실적인 안정을 책임진다면, 신비 나비(사)는 메마르기 쉬운 강아지의 내면에 정신적인 영감과 쉼터를 제공합니다. 생활의 궤적은 다르지만, 다름이 서로에게 독이 아닌 약이 되는 상호 보완의 인연입니다.",
        },
        emotionCommunication: {
          title: "감정 및 소통 궁합",
          body: "별빛 강아지는 행동과 결과로 애정을 증명하려 하고, 신비 나비는 언어 이면의 감정과 분위기로 소통하려 합니다. 강아지 입장에서는 나비의 생각이 뜬구름 잡는 것처럼 보일 수 있고, 나비는 강아지가 너무 현실적이라고 느낄 수 있습니다. 소통의 주파수를 맞추기 위해서는 강아지는 나비의 감정에 이유를 묻지 말고 공감해 주어야 하며, 나비는 강아지에게 모호한 표현 대신 명확한 언어로 마음을 전달해야 합니다.",
        },
        valueLifestyle: {
          title: "가치관 및 생활 패턴",
          body: "루틴과 규칙을 중시하는 별빛 강아지의 곁에서 신비 나비는 안정감을 느낍니다. 다만 생활 리듬에 있어 강아지가 자신의 템포를 나비에게 강요하기 시작하면 관계에 균열이 생깁니다. 나비에게는 혼자만의 정신적 동굴에서 에너지를 충전할 고요한 시간이 반드시 필요함을 인정해야 합니다.",
        },
        practicalAdvice: {
          title: "사주 고수의 실전 조언",
          body: "이 궁합의 핵심은 '적당한 거리감'과 '역할의 분리'입니다. 모든 것을 함께 하려 하기보다, 현실적인 결정(재무, 계획)은 건록인 별빛 강아지가 주도하고, 관계의 분위기와 정서적인 방향성은 사(死)인 신비 나비가 이끌도록 하세요. 한 달에 한 번, 서로의 감정 온도를 점검하는 티타임을 가지는 것만으로도 이 인연은 오래도록 단단하게 이어질 것입니다.",
        },
      },
    };
  }
