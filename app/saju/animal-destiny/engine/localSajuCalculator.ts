...
function buildFinalAdvancedReport(args: {
  ...
  const title = "QUANTUM MYEONGRI Engine v.2 고급 분석 리포트";

  const sections = [
    reportSection(1, "계산 기준 요약", [
      { label: "양력/음력 변환 여부", value: input.calendarType === "lunar" ? `음력 입력을 양력 기준으로 변환${input.lunarLeap ? "했고 윤달 정보를 반영" : ""}` : "양력 입력 기준", why: "원국은 실제 태양력 날짜와 절기 경계를 기준으로 재산출한다." },
      { label: "절기 기준 사용 여부", value: "사용", why: `년주는 입춘, 월주는 절입 기준이며 현재 절기 근거는 ${activeSource}이다.` },
      { label: "출생지 시간대", value: timezone, why: "일주 경계와 시주는 출생지 시간대를 기준으로 판단한다." },
      { label: "진태양시 사용 여부", value: trueSolarTimeUsed ? "사용" : "미사용", why: trueSolarTimeUsed ? "출생지 경도와 균시차를 반영했다." : "좌표 또는 옵션 조건이 충족되지 않아 표준시 기준으로 산출했다." },
      { label: "대운 시작 나이", value: daewoonStart.age ?? "미산출", why: "절입 시각까지의 시간 차이를 전통 환산법으로 나누었다." },
      { label: "대운 순행/역행", value: daewoonDirection, why: "양남음녀 순행, 음남양녀 역행 원칙을 기본값으로 적용했다." },
      { label: "계산 신뢰도", value: calculationConfidence, why: "절기 소스, 출생 시각, 진태양시 보정 여부를 함께 본다." },
    ]),
    reportSection(2, "사주 원국", [
      ...
      { label: "일간", value: `${displayValue(dayMaster.stem)} ${displayValue(dayMaster.elementKo)}`, why: `현재 강약은 ${displayValue(dayMaster.strength)}로 판정된다.` },
      { label: "월령", value: `${displayValue(monthCommand.branch)} ${displayValue(monthCommand.season)}`, why: displayValue(monthCommand.priority) },
      { label: "지장간", value: rowSummary(asReportRows(monthCommand.hiddenStems), ["stem", "tenGod"], "월지 지장간 없음"), why: "월지 지장간은 격국과 현실 작용력의 핵심 재료다." },
      { label: "십성 배치", value: `천간 ${displayValue(tenGods.visible)} / 지장간 ${displayValue(tenGods.hidden)}`, why: "드러난 십성과 숨은 십성을 분리해 본다." },
      { label: "오행 분포", value: rowSummary(asReportRows(fiveElements.ranking), ["elementKo", "power"], "오행 분포 없음", 5), why: "단순 개수보다 계절 가중치, 투간, 합충 변화를 반영한 실제 세력이다." },
      { label: "조후 상태", value: `${displayValue(johu.urgentElementKo)} 필요`, why: `기후값 ${displayValue(johu.climate)}를 기준으로 가장 급한 조후를 잡았다.` },
    ]),
    reportSection(3, "원국 핵심 진단", [
      { label: "신강/신약", value: displayValue(dayMaster.strength), why: displayValue(scoreOf("dayMasterStrength")?.naturalReason) },
      { label: "조후", value: displayValue(johu.urgentElementKo), why: displayValue(scoreOf("johuBalance")?.naturalReason) },
      { label: "격국", value: displayValue(gyeokguk.finalGyeokguk), why: displayValue(asReportRecord(gyeokguk.requiredOutput).reason) },
      { label: "용신", value: displayValue(yongshin.coreYongshinKo), why: displayValue(asReportRecord(yongshin.requiredExplanation).whyThisElement) },
      { label: "희신", value: displayValue(yongshin.heesinKo), why: "핵심 용신을 돕거나 흐름을 부드럽게 이어 주는 보조 기운이다." },
      { label: "기신", value: displayValue(yongshin.gisinKo), why: displayValue(scoreOf("gisinRisk")?.naturalReason) },
      { label: "구신", value: displayValue(yongshin.gusinKo), why: "기신을 돕는 배후 기운으로 과해지면 구조적 부담을 키운다." },
      { label: "사주의 병", value: displayValue(asReportRecord(yongshin.disease).name), why: displayValue(asReportRecord(yongshin.disease).reason) },
      { label: "사주의 약", value: displayValue((asReportRecord(yongshin.disease).medicineElements as unknown[] | undefined)?.map((element) => ELEMENT_KO[element as ElementKey])), why: "병을 치료하는 오행이 실제 처방의 핵심이다." },
      { label: "가장 중요한 구조", value: displayValue(scoring.naturalSummary), why: "18개 판별 점수를 숫자보다 자연어 근거 중심으로 요약했다." },
    ]),
    reportSection(4, "지장간·투간 분석", [
      { label: "월지 지장간", value: rowSummary(asReportRows(asReportRecord(hiddenStemActivation.monthHiddenStemProtrusion).hiddenStems), ["stem", "tenGod", "strength"], "월지 투간 없음"), why: "월령에서 천간으로 드러난 글자는 현실 작용력이 강하다." },
      { label: "천간 투출 여부", value: rowSummary(asReportRows(hiddenStemActivation.protruded), ["pillar", "branch", "stem", "tenGod"], "투출 없음"), why: "투출된 지장간은 성격·직업·재물·연애의 실제 사건으로 올라온다." },
      { label: "숨은 재능", value: displayValue(asReportRecord(hiddenStemActivation.monthHiddenStemProtrusion).summary), why: "월지 내용물이 천간으로 드러나는지 보아 재능의 표면화를 본다." },
      { label: "억눌린 욕구", value: rowSummary(asReportRows(hiddenStemActivation.hidden), ["branch", "stem", "tenGod"], "강하게 억눌린 지장간 표시 없음"), why: "숨은 십성은 운에서 같은 글자가 오거나 충합으로 열릴 때 사건화된다." },
      { label: "대운·세운에서 열리는 시기", value: rowSummary(asReportRows(asReportRecord(hiddenStemActivation.luckAmplification).rows), ["scope", "label", "effect"], "입력 운에서 직접 확인되지 않음"), why: "같은 글자, 합, 충이 들어올 때 지장간이 열린다." },
    ]),
    reportSection(5, "진술축미·창고 분석", [
      { label: "토오행 과다 여부", value: earthStorage.overburdened ? "과중" : "과중 아님", why: displayValue(earthStorage.earthDisposition) },
      { label: "정신적 압박", value: displayValue(earthStorage.pressureReading), why: "辰戌丑未는 단순 토가 아니라 책임과 감정의 저장고로 본다." },
      { label: "숨은 잠재력", value: displayValue(earthStorage.usefulReading), why: "창고 속 지장간이 용신이면 충이 오히려 자원 개방이 된다." },
      { label: "창고가 열리는 시기", value: rowSummary(asReportRows(earthStorage.openingTiming), ["scope", "label", "relation"], "직접 매칭 없음"), why: "진술충·축미충과 운의 합충을 창고 개방 신호로 본다." },
      { label: "불리하게 터지는 시기", value: rowSummary(asReportRows(earthStorage.riskTiming), ["scope", "label", "risk"], "직접 위험 매칭 없음"), why: "창고 속 기신이 열리면 압박과 문제 노출로 나타난다." },
    ]),
    reportSection(6, "도충 분석", [
      { label: "중첩 지지", value: rowSummary(asReportRows(doChung.candidates), ["repeatedBranch", "totalCount"], "도충 후보 없음"), why: "같은 지지가 3개 수준으로 중첩될 때 과포화가 발생한다." },
      { label: "불러오는 반대 지지", value: rowSummary(asReportRows(doChung.candidates), ["inducedOppositeBranch", "inducedTenGod"], "반대 지지 유도 없음"), why: "도충은 단순 강화가 아니라 반대편 충 글자를 불러오는 구조다." },
      { label: "용신/기신 변화", value: rowSummary(asReportRows(doChung.candidates), ["classification"], "변화 없음"), why: displayValue(scoreOf("doChungActivation")?.naturalReason) },
      { label: "흉에서 길 전환", value: rowSummary(asReportRows(transformation.gisinToYongshin), ["requiredPhrase", "activationTiming"], "확인된 전환 없음"), why: "기신 과포화가 용신을 유도하면 판이 바뀐다." },
      { label: "길에서 흉 전환", value: rowSummary(asReportRows(transformation.yongshinToGisin), ["requiredPhrase", "activationTiming"], "확인된 전환 없음"), why: "용신 과포화가 반대 기신을 불러오면 기반이 흔들린다." },
      { label: "실제 사건 가능성", value: rowSummary(asReportRows(doChung.candidates), ["lifeEvent"], "직접 사건 후보 없음"), why: "직업·재물·연애·이동 사건으로 현실화될 수 있다." },
    ]),
    reportSection(7, "합화·충 변이 분석", [
      { label: "천간합", value: rowSummary(asReportRows(combinationClash.stemCombinations), ["type", "leftLabel", "rightLabel", "resultType"], "천간합 없음"), why: "계절 지원과 뿌리가 있어야 합화로 본다." },
      { label: "지지합", value: rowSummary(asReportRows(combinationClash.branchCombinations), ["type", "leftLabel", "rightLabel", "resultType"], "지지합 없음"), why: "합은 길흉이 아니라 무엇을 묶고 무엇으로 변하는지가 핵심이다." },
      { label: "삼합", value: rowSummary(asReportRows(combinationClash.groupCombinations), ["type", "branches", "changedState"], "삼합 없음"), why: "흩어진 지지가 한 방향으로 모이면 운의 성격이 바뀐다." },
      { label: "방합", value: rowSummary(asReportRows(combinationClash.directionalCombinations), ["type", "branches", "changedState"], "방합 없음"), why: "계절 방향성이 모이면 생활권의 기운이 강하게 기울어진다." },
      { label: "충·형·파·해", value: rowSummary(asReportRows(combinationClash.requiredOutputRows), ["relationType", "resultType", "usefulUnfavorableShift"], "직접 변이 없음"), why: displayValue(scoreOf("clashTriggerIntensity")?.naturalReason) },
      { label: "합화/합거/용신파손/기신용신화", value: rowSummary(asReportRows(combinationClash.requiredOutputRows), ["transformationSuccess", "resultType", "usefulUnfavorableShift"], "직접 전환 없음"), why: "합과 충은 용신·기신 변화까지 재계산해야 한다." },
    ]),
    reportSection(8, "현재 대운 분석", [
      { label: "대운 간지", value: displayValue(asReportRecord(daewoon.currentDaewoon).label || asReportRecord(daewoon.currentDaewoon).ganji || rowSummary(daewoonRows, ["label", "ganji"], "대운 입력 없음")), why: "대운은 10년짜리 배경 운이다." },
      { label: "대운의 기반", value: displayValue(luckIntegratedReading.daewoonBase), why: "대운은 단순 길흉이 아니라 천간의 외부 사건성과 지지의 생활권 변화를 분리해 본다." },
      { label: "대운 천간 합화·합거", value: rowSummary(asReportRows(luckDaewoonDetail.heavenlyStemCombinationChecks), ["combinationRule", "resultType", "transformedUseState"], "대운 천간의 직접 합화·합거 없음"), why: "원국에 합하는 천간이 실제로 있는지, 합화 조건이 되는지, 안 되면 합거인지까지 본다." },
      { label: "대운 지지 지장간", value: rowSummary(asReportRows(luckDaewoonDetail.branchHiddenStems), ["stem", "tenGod", "elementKo"], "대운 지장간 없음"), why: "대운 지지의 본기와 지장간이 어떤 십성으로 들어오는지 확인한다." },
      { label: "대운 지지 합충형파해", value: rowSummary(asReportRows(luckDaewoonDetail.earthlyBranchRelations), ["luckBranch", "relation", "targetScope", "targetLifeArea"], "대운 지지의 직접 합충형파해 없음"), why: "대운 지지가 원국 어느 궁을 건드리는지에 따라 사건 영역이 달라진다." },
      { label: "대운 삼합·도충 발동", value: `${rowSummary(asReportRows(luckDaewoonDetail.groupCompletions), ["type", "branches", "targetElement"], "삼합/방합 직접 완성 없음")} / ${rowSummary(asReportRows(luckDaewoonDetail.doChungTriggered), ["repeatedBranch", "classification"], "대운 도충 직접 발동 없음")}`, why: "대운 지지는 10년 환경에서 삼합·도충 조건을 길게 깔아 둔다." },
      { label: "대운의 오행", value: displayValue(asReportRecord(daewoon.yongshinGisinChange).supportsYongshin), why: "용신을 돕는지 기신을 강화하는지부터 본다." },
      { label: "대운의 십성", value: `${displayValue(asReportRecord(daewoon.heavenlyStemAnalysis).tenGod)} / ${displayValue(asReportRecord(daewoon.earthlyBranchAnalysis).tenGod)}`, why: "천간은 드러난 사건, 지지는 생활 환경으로 본다." },
      { label: "원국과의 작용", value: displayValue(daewoon.relationSummary), why: "원국 천간·지지와 합충형파해를 비교했다." },
      { label: "격국 영향", value: displayValue(daewoon.gyeokgukChange), why: "대운이 격을 살리는지 깨는지 본다." },
      { label: "용신/기신 변화", value: displayValue(asReportRecord(daewoon.yongshinGisinChange).result), why: displayValue(scoreOf("daewoonFortune")?.naturalReason) },
      { label: "전반 5년", value: displayValue(asReportRecord(daewoon.firstSecondHalf).firstHalf), why: "천간의 사회적 사건성이 먼저 드러난다." },
      { label: "후반 5년", value: displayValue(asReportRecord(daewoon.firstSecondHalf).secondHalf), why: "지지의 생활권 변화가 깊게 체감된다." },
      { label: "대운 총평", value: displayValue(daewoon.summary), why: "간지와 원국 상호작용을 통합한 판정이다." },
      { label: "대운 활용법", value: displayValue(daewoon.howToUse), action: "좋은 운은 구조화하고, 불리한 운은 합충이 여는 정리 타이밍으로 쓴다." },
    ]),
    reportSection(9, "현재 세운 분석", [
      { label: "세운 간지", value: displayValue(currentAnnual?.label || currentAnnual?.ganji, "세운 입력 없음"), why: "세운은 대운 위에 얹히는 해당 연도의 사건 트리거다." },
      { label: "세운 사건성", value: displayValue(luckIntegratedReading.annualEvent), why: "세운은 대운의 배경 위에서 실제 사건을 촉발한다." },
      { label: "세운 천간합", value: rowSummary(asReportRows(luckAnnualDetail.heavenlyStemCombinationChecks), ["combinationRule", "resultType", "transformedUseState"], "세운 천간의 직접 합화·합거 없음"), why: "세운 천간이 원국 천간과 합하는지, 합화가 되는지, 합거로 묶이는지 확인한다." },
      { label: "세운 천간충·조후 손상", value: rowSummary(asReportRows(luckAnnualDetail.heavenlyStemClashChecks).filter((row) => row.isClash), ["clashRule", "targetScope", "damagesJohuYongshin", "meaning"], "세운 천간충 없음"), why: "천간충이 조후 용신을 건드리면 표면 길운도 몸과 판단에서 흔들릴 수 있다." },
      { label: "세운 지지 중첩·도충", value: displayValue(luckAnnualDetail.branchSaturation), why: "같은 지지가 원국·대운·세운에서 3개 수준으로 중첩되는지 확인한다." },
      { label: "세운 최종 분류", value: displayValue(luckAnnualDetail.finalClassification), why: "용신/기신, 천간충, 도충, 합화 결과를 종합한 사건성 판정이다." },
      { label: "대운과의 관계", value: displayValue(asReportRecord(daewoon.requiredOutput).bestYears || asReportRecord(daewoon.requiredOutput).cautionYears), why: "세운이 대운의 좋은 구조를 완성하는지 깨는지 본다." },
      { label: "원국과의 관계", value: rowSummary(asReportRows(combinationClash.requiredOutputRows).filter((row) => JSON.stringify(row).includes("annual")), ["relationType", "resultType"], "직접 세운 합충 입력 없음"), why: "세운 지지가 원국 궁을 건드리는지를 본다." },
      { label: "올해 핵심 사건성", value: rowSummary(asReportRows(transformation.strongestTransformations), ["category", "activationTiming"], "강한 전환 이벤트 없음"), why: "합충·도충·지장간 개방 중 올해와 겹치는 항목을 우선한다." },
      { label: "직업운", value: rowSummary(asReportRows(career.annualJobChangePromotionExpansionTiming), ["label", "timingType"], "세운 직업 타이밍 입력 없음"), why: displayValue(scoreOf("careerFortune")?.naturalReason) },
      { label: "재물운", value: displayValue(scoreOf("wealthFortune")?.naturalReason), why: "재성 자체보다 감당력과 식상생재 흐름을 본다." },
      { label: "연애운", value: rowSummary(asReportRows(loveMarriage.likelyLoveEventAnnual), ["label", "reason"], "세운 연애 사건 입력 없음"), why: displayValue(scoreOf("loveFortune")?.naturalReason) },
      { label: "건강·심리", value: displayValue(scoreOf("healthPsychPressure")?.naturalReason), why: "조후·충·도충·토창고 압박을 함께 본다." },
      { label: "조심할 달", value: rowSummary(monthlyRows.filter((row) => displayValue(row.label).includes("주의") || displayValue(row.ganji).includes("충")), ["label", "ganji"], "월운 입력 없음: 원국 용신을 충하는 달을 조심"), why: "월운은 세운의 사건을 단기화한다." },
      { label: "기회가 오는 달", value: rowSummary(monthlyRows.filter((row) => displayValue(row.label).includes("기회") || displayValue(row.ganji).includes("합")), ["label", "ganji"], "월운 입력 없음: 용신·희신이 들어오는 달을 기회로 사용"), why: "월운에서 용신이 투출하거나 합으로 살아나면 실행 창이 열린다." },
    ]),
    reportSection(10, "성격 분석", [
      { label: "겉모습", value: displayValue(personality.outwardDisposition), why: "천간에 드러난 십성과 월령의 사회성이 먼저 보인다." },
      { label: "내면", value: displayValue(personality.innerWorld), why: "일지와 지장간은 실제 내면과 반복 감정을 보여 준다." },
      { label: "강점", value: displayValue(personality.workStrength), why: "가장 강한 십성이 일할 때의 장점으로 나타난다." },
      { label: "약점", value: displayValue(personality.relationshipWeakness), why: "강한 십성이 과해질 때 인간관계의 그림자가 된다." },
      { label: "스트레스 반응", value: displayValue(personality.stressReaction), why: "조후 압박, 토다 구조, 합충이 심리 반응을 바꾼다." },
      { label: "인간관계 패턴", value: displayValue(personality.repeatingLifePattern), why: "도충·합충·강한 십성의 반복을 함께 본다." },
      { label: "성장 처방", value: displayValue(personality.improvementPoint), action: "강한 십성을 억누르지 말고 건강한 출구로 반복 사용한다." },
    ]),
    reportSection(11, "진로·재물 분석", [
      { label: "적합한 직업군", value: rowSummary(asReportRows(career.bestFitJobGroups), ["elementKo", "jobGroups"], "직업군 미산출"), why: "격국·용신·강한 십성·월지/시지 직업성을 종합했다." },
      { label: "돈 버는 방식", value: displayValue(career.monetizableAbility), why: displayValue(scoreOf("wealthFortune")?.naturalReason) },
      { label: "조직/사업/프리랜서 적성", value: displayValue(asReportRecord(career.workModeType).primary), why: displayValue(asReportRecord(career.workModeType).reading) },
      { label: "커리어 상승 시기", value: rowSummary(asReportRows(career.daewoonCareerRiseTiming), ["label", "strength", "reason"], "대운 상승기 입력 없음"), why: "대운이 직업 용신을 건드리는 시기를 본다." },
      { label: "피해야 할 선택", value: rowSummary(asReportRows(career.avoidedWorkEnvironment), ["elementKo", "warning"], "명확한 회피 환경 없음"), why: "기신 환경은 재능을 소모시키거나 결과를 흐린다." },
      { label: "지금 해야 할 행동", value: displayValue(career.immediateCareerStrategy), action: "직업 용신이 살아나는 업무·시장·역할로 현재 선택을 재배치한다." },
    ]),
    reportSection(12, "연애·결혼 분석", [
      { label: "연애 스타일", value: displayValue(loveMarriage.loveStyle), why: "배우자궁과 배우자성, 도화·홍염·합충을 함께 본다." },
      { label: "끌리는 인연", value: displayValue(loveMarriage.attractedPersonType), why: "성별 입력이 있으면 전통 배우자성을, 미입력은 재성·관성을 모두 관계 지표로 본다." },
      { label: "반복되는 문제", value: displayValue(loveMarriage.recurringRelationshipProblem), why: "관살혼잡, 재성혼잡, 식상관성 충돌, 비겁쟁재를 함께 본다." },
      { label: "결혼운", value: displayValue(loveMarriage.marriageFavorability), why: displayValue(loveMarriage.lateOrEarlyMarriage) },
      { label: "인연이 강한 시기", value: rowSummary(asReportRows(loveMarriage.strongRelationshipDaewoon), ["label", "strength"], "강한 인연 대운 입력 없음"), why: "대운에서 배우자성 또는 배우자궁 합이 살아나는지 본다." },
      { label: "이별·갈등 주의 시기", value: rowSummary(asReportRows(loveMarriage.breakupConflictAnnual), ["label", "reason"], "직접 주의 세운 없음"), why: "배우자궁 충·해·파와 기신성 관계 자극을 본다." },
      { label: "관계 처방", value: displayValue(loveMarriage.relationshipPrescription), action: "합에는 속도 조절, 충에는 거리·돈·일정 분리를 먼저 적용한다." },
    ]),
    reportSection(13, "신살 분석", [
      { label: "주요 신살", value: rowSummary(asReportRows(shinsal.activeRows), ["shinsalName", "position"], "강한 활성 신살 없음"), why: displayValue(shinsal.principle) },
      { label: "위치", value: rowSummary(asReportRows(shinsal.rows), ["shinsalName", "position"], "위치 없음"), why: "원국과 입력 운에서 어느 글자에 걸렸는지 표시한다." },
      { label: "작용 방식", value: rowSummary(asReportRows(shinsal.rows), ["shinsalName", "actualLifeManifestation"], "작용 없음"), why: "신살은 단독 길흉이 아니라 성격·연애·직업·이동·귀인운 보조 지표다." },
      { label: "대운·세운 발동", value: rowSummary(asReportRows(shinsal.luckTriggeredRows), ["shinsalName", "position"], "운 발동 없음"), why: "대운·세운에 같은 글자나 쌍지지 조건이 들어오면 발동한다." },
    ]),
    reportSection(14, "운의 환골탈태 요약", [
      { label: "흉에서 길로 바뀌는 요소", value: rowSummary(helpfulTransforms, ["requiredPhrase", "activationTiming"], "뚜렷한 기신 용신화 없음"), why: "기신이 합화·충거·도충·창고 개방으로 용신화되는지 본다." },
      { label: "길에서 흉으로 바뀌는 요소", value: rowSummary(riskyTransforms, ["requiredPhrase", "activationTiming"], "뚜렷한 용신 파손 없음"), why: "용신이 합거·충·도충·불리한 창고 개방으로 손상되는지 본다." },
      { label: "합화로 바뀌는 지점", value: rowSummary(asReportRows(luckIntegratedReading.combinationTurningPoint), ["rule", "resultType", "transformedUseState"], "현재 대운·세운에서 직접 합화 전환점 없음"), why: "합화가 성립하면 오행과 십성이 바뀌고, 성립하지 않으면 합거·합반으로 작용한다." },
      { label: "도충 가능성", value: displayValue(luckIntegratedReading.doChungPossibility), why: "같은 지지 과포화가 반대 글자를 유도하는지 확인한다." },
      { label: "지금 가장 중요한 선택", value: displayValue(scoring.strongestRisk), action: "가장 낮은 점수 항목을 먼저 정비하고, 가장 높은 점수 항목에 자원을 집중한다." },
      { label: "놓치면 안 되는 기회", value: rowSummary(opportunityTransforms, ["requiredPhrase", "activationTiming"], "잠재력 개방 이벤트 없음"), why: "오래 숨어 있던 지장간·창고·삼합 구조가 현실화되는 시점이다." },
      { label: "피해야 할 위험", value: rowSummary(collapseTransforms, ["requiredPhrase", "activationTiming"], "구조 붕괴 이벤트 없음"), why: "격국 상신, 조후 용신, 원국 균형을 잡던 글자가 손상되는지 본다." },
      { label: "실제 행동 처방", value: displayValue(luckIntegratedReading.practicalPrescription), why: "대운 기반과 세운 사건성을 분리해 실제 선택 순서를 정한다." },
    ]),
    reportSection(15, "천기적 액션 처방", [
      { label: "직업", value: displayValue(career.immediateCareerStrategy), action: "직업 용신 분야를 주력 업무로 올리고 기신 환경은 계약·역할·시간으로 제한한다." },
      { label: "돈", value: displayValue(career.investmentWealthManagementStyle), action: "재성이 과하면 보전과 현금흐름, 식상이 강하면 산출물 현금화를 우선한다." },
      { label: "연애", value: displayValue(loveMarriage.relationshipPrescription), action: "관계의 속도, 약속 범위, 돈과 시간의 경계를 먼저 정한다." },
      { label: "건강", value: displayValue(scoreOf("healthPsychPressure")?.naturalReason), action: `조후상 ${displayValue(johu.urgentElementKo)} 기운을 생활 리듬으로 보완한다.` },
      { label: "인간관계", value: displayValue(personality.relationshipWeakness), action: "반복되는 약점은 설명보다 구조 조정으로 줄인다." },
      { label: "공간·방향·색상·습관", value: `${displayValue(yongshin.coreYongshinKo)} 용신을 상징하는 색상·동선·습관을 작게 반복`, action: "부족한 오행을 장식보다 수면, 식사, 업무 리듬, 공간 정리로 먼저 채운다." },
      { label: "이번 대운에서 반드시 해야 할 것", value: displayValue(daewoon.howToUse), action: "대운 전반은 드러난 사건을 정리하고 후반은 생활권에 정착시킨다." },
      { label: "올해 반드시 피해야 할 것", value: rowSummary(asReportRows(loveMarriage.breakupConflictAnnual), ["label", "reason"], displayValue(scoreOf("gisinRisk")?.naturalReason)), action: "기신이 발동하는 선택은 확장보다 정비·분리·보류로 대응한다." },
    ]),
  ];
