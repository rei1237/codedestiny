...
const ELEMENTS: Array<{ key: ElKey; ko: string; han: string; emoji: string; color: string }> = [
  { key: "wood", ko: "목", han: "木", emoji: "🌿", color: "#4ADE80" },
  { key: "fire", ko: "화", han: "火", emoji: "🔥", color: "#FB7185" },
  { key: "earth", ko: "토", han: "土", emoji: "🪨", color: "#FBBF24" },
  { key: "metal", ko: "금", han: "金", emoji: "✨", color: "#E5E7EB" },
  { key: "water", ko: "수", han: "水", emoji: "🌊", color: "#60A5FA" },
];
...
export default function BiasDestinyElementChart({ vm }: Props) {
  ...
  return (
    <section className="space-y-3">
      <p className="text-[11px] font-semibold tracking-[0.16em] text-cyan-100/85">오행 분포 FIVE ELEMENTS</p>
      <article className="relative overflow-hidden rounded-[28px] border border-white/15 bg-[linear-gradient(140deg,rgba(6,14,38,0.8),rgba(14,10,48,0.6))] p-4 shadow-[0_0_50px_rgba(124,58,237,0.16),inset_0_1px_2px_rgba(255,255,255,0.06)] backdrop-blur-xl md:p-5">
        ...
        <div className="relative z-10 mb-3 flex items-center gap-3 text-[11px] text-white/70">
          <span className="inline-flex items-center gap-1"><span className="inline-block h-2 w-2 rounded-full bg-white/45" />나</span>
          <span className="inline-flex items-center gap-1"><span className="inline-block h-2 w-2 rounded-full bg-white/90" />{String(vm.biasName || "최애").trim() || "최애"}</span>
        </div>
          ...
          {ELEMENTS.map((e) => (
            <div key={e.key} className="flex items-center gap-3">
              <div className="flex w-12 shrink-0 items-center gap-1">
                ...
                <span className="text-xs font-bold text-white/85">{e.ko}<span className="ml-0.5 text-[10px] text-white/45">{e.han}</span></span>
              </div>
