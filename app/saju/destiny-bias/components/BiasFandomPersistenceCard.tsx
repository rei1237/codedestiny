...
export default function BiasFandomPersistenceCard({ vm }: Props) {
  ...
  return (
    <section className="space-y-3">
      <p className="text-[11px] font-semibold tracking-[0.16em] text-pink-100/85">덕질 지속력 HOW LONG IT LASTS</p>
      <article className="relative overflow-hidden rounded-[24px] border border-white/14 bg-[linear-gradient(140deg,rgba(12,10,50,0.78),rgba(24,10,58,0.6))] p-4 shadow-[0_12px_36px_rgba(2,6,28,0.4),inset_0_1px_2px_rgba(255,255,255,0.06)] backdrop-blur-xl md:p-5">
        ...
        <div className="relative z-10 flex flex-wrap gap-2">
          <span className="rounded-full border border-cyan-200/35 bg-cyan-300/10 px-3 py-1.5 text-xs font-bold text-cyan-100/90">강도 · {profile.persistenceIntensity}</span>
          <span className="rounded-full border border-[var(--bias-gold)]/45 bg-[var(--bias-gold)]/12 px-3 py-1.5 text-xs font-bold text-[var(--bias-gold)]">기간 · {profile.persistenceDuration}</span>
        </div>
