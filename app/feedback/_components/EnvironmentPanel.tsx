...
import { notCollectedList, type EnvEntry } from "../_lib/environment";
  ...
  return (
    <div className="rounded-2xl border border-[rgba(216,63,120,0.14)] bg-white/50 p-4 dark:border-white/10 dark:bg-white/[0.03]">
      <p className={`text-[13px] leading-relaxed ${INK_MUTED}`}>
        {copy.envIntroPrefix}
        <strong className={`font-bold ${INK}`}>{copy.envIntroStrong}</strong>
        {copy.envIntroSuffix}
      </p>
      ...
      <details className="group mt-3">
        ...
        <div className="mt-3 space-y-4">
          <div>
            <p className={`mb-2 text-[12px] font-bold ${INK}`}>{copy.envSentTitle}</p>
          ...
          <div>
            <p className={`mb-2 text-[12px] font-bold ${INK}`}>{copy.envNotCollectedTitle}</p>
            <ul className={`list-inside list-disc space-y-1 text-[12px] ${INK_MUTED}`}>
              {notCollectedList(copy).map((item) => (
                <li key={item}>{item}</li>
              ))}
            </ul>
          </div>
