    ...
    <div>
      <div className="flex flex-wrap items-baseline justify-between gap-2">
        <span className={`text-sm font-bold ${INK}`}>
          {copy.attachmentSectionLabel}
          {emphasize && <span className={`ml-1.5 text-[11px] font-bold ${ACCENT}`}>{copy.attachmentEmphasizeBadge}</span>}
        </span>
        <span className={`text-[12px] ${INK_MUTED}`}>
          {attachments.length}{copy.attachmentCountSuffix(MAX_ATTACHMENTS, formatBytes(MAX_UPLOAD_SIZE))}
        </span>
      </div>
      ...
      {uploadingCount > 0 && (
        <p className={`mt-2 flex items-center gap-1.5 text-[12px] ${INK_MUTED}`} role="status">
          ...
          {copy.attachmentUploadingText(uploadingCount)}
        </p>
      )}
          ...
          {attachments.map((file) => (
            <li key={file.key} className="relative overflow-hidden rounded-xl border border-[rgba(216,63,120,0.16)] bg-white/60 dark:border-white/10 dark:bg-white/[0.04]">
              ...
              <button
                ...
                aria-label={copy.attachmentRemoveAriaLabel}
