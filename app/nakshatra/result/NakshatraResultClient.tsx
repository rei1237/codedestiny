...
function viewTabs(copy: NakshatraCopy): { key: ViewMode; label: string }[] {
  return [
    { key: "both", label: copy.resultViewTabBoth },
    { key: "east", label: copy.resultViewTabEast },
    { key: "india", label: copy.resultViewTabIndia },
    { key: "unified", label: copy.resultViewTabUnified },
  ];
}
    ...
    return (
      <main className="grid min-h-[100dvh] place-items-center bg-[#070812] px-5 text-center text-slate-100">
        <div className="max-w-sm">
          <p className="text-lg font-bold text-slate-50">{copy.resultNotFoundTitle}</p>
          <p className="mt-3 text-sm leading-7 text-slate-300">
            {copy.resultNotFoundText}
          </p>
          <Link
            href="/nakshatra/calc"
      ...
      <div className="mx-auto w-full max-w-4xl">
        ...
        <header className="rounded-2xl border border-amber-200/20 bg-white/[0.03] p-6 text-center shadow-[0_20px_60px_rgba(0,0,0,0.4)] md:p-8">
          ...
          <div className="mt-5 flex flex-wrap items-center justify-center gap-2">
            <Chip>{copy.resultLordChipPrefix}{india.lordKo}</Chip>
            <Chip>{copy.resultPadaChip(summary.pada)}</Chip>
            <Chip>{copy.resultGanaChipPrefix}{india.ganaKo}</Chip>
          </div>
        </header>
          ...
          {showEast && (
            <section className="rounded-2xl border border-blue-400/20 bg-blue-500/[0.06] p-5 md:p-6" aria-labelledby="east-h">
              <h2 id="east-h" className="flex items-center gap-2 text-lg font-bold text-blue-100">
                <span aria-hidden="true">☯</span> {copy.resultEastSectionTitle} <span className="text-sm font-normal text-blue-200/80">{copy.resultEastSuffix}</span>
              </h2>
              <p className="mt-1 text-sm text-slate-200">{dongyang.archetypeTitle}</p>
              <dl className="mt-4">
                <Row label={copy.resultRowMansionLabel} value={`${dongyang.nameKo}(${dongyang.nameHan})`} />
                <Row label={copy.resultRowDirectionLabel} value={dongyang.direction} />
                <Row label={copy.resultRowSevenLuminaryLabel} value={dongyang.sevenLuminary} hint={copy.resultRowSevenLuminaryHint} />
                <Row label={copy.resultRowFourSymbolLabel} value={dongyang.fourSymbol} />
              </dl>
          ...
          {showIndia && (
            <section className="rounded-2xl border border-amber-300/20 bg-amber-500/[0.05] p-5 md:p-6" aria-labelledby="india-h">
              <h2 id="india-h" className="flex items-center gap-2 text-lg font-bold text-amber-100">
                <span aria-hidden="true">🕉</span> {copy.resultIndiaSectionTitle} <span className="text-sm font-normal text-amber-200/80">{copy.resultIndiaSuffix}</span>
              </h2>
              <p className="mt-1 break-keep text-sm text-slate-200">{india.deity} — {india.deityRole}</p>
              <dl className="mt-4">
                <Row label={copy.resultRowNakshatraLabel} value={`${india.nameKo} (${india.nameEn})`} />
                <Row label={copy.resultRowLordLabel} value={india.lordKo} hint={copy.resultRowLordHint} />
                <Row label={copy.resultRowGanaLabel} value={india.ganaKo} hint={copy.resultRowGanaHint} />
                <Row label={copy.resultRowYoniLabel} value={india.yoni} hint={copy.resultRowYoniHint} />
                <Row label={copy.resultRowNadiLabel} value={india.nadiKo} hint={copy.resultRowNadiHint} />
                <Row label={copy.resultRowMotiveLabel} value={india.motiveKo} hint={copy.resultRowMotiveHint} />
                {india.pada && india.padaDetail ? (
                  <Row label={copy.resultRowPadaLabel} value={`${india.pada} · ${india.padaDetail.navamsaSignKo}`} hint={copy.resultRowPadaHint} />
                ) : (
                  <Row label={copy.resultRowPadaLabel} value={copy.resultRowPadaOmittedText} />
                )}
                {india.dasha && (
                  <Row label={copy.resultRowCurrentDashaLabel} value={`${india.dasha.currentMahadashaKo} / ${india.dasha.currentAntardashaKo}`} hint={copy.resultRowCurrentDashaHint} />
                )}
              </dl>
              ...
              {india.indianExpert && (
                <div className="mt-4 rounded-xl border border-amber-300/20 bg-amber-500/[0.05] p-4">
                  <p className="text-xs font-semibold text-amber-200/90">{copy.resultIndianExpertLabel}</p>
        ...
        {showUnified && (
          <section className="mt-6 rounded-2xl border border-rose-300/25 bg-rose-500/[0.06] p-6 md:p-8" aria-labelledby="unified-h">
            <h2 id="unified-h" className="text-lg font-bold text-rose-100">{copy.resultUnifiedTitle}</h2>
            {unified.boundaryNote && (
              <p className="mt-3 rounded-xl border border-amber-300/30 bg-amber-400/10 px-4 py-3 text-sm leading-7 text-amber-50">
                <span className="font-semibold">{copy.resultBoundaryLabel}</span>
                {unified.boundaryNote}
              </p>
            )}
            {unified.convergence && (
              <div className="mt-4">
                <p className="text-sm font-semibold text-rose-200">{copy.resultConvergenceLabel}</p>
                <p className="mt-1.5 break-keep text-sm leading-7 text-slate-100">{unified.convergence}</p>
              </div>
            )}
            {unified.divergence && (
              <div className="mt-4">
                <p className="text-sm font-semibold text-rose-200">{copy.resultDivergenceLabel}</p>
        ...
        <footer className="mt-6 rounded-2xl border border-white/10 bg-white/[0.02] p-5 text-xs leading-6 text-slate-300">
          ...
          <p className="mt-1">
            ...
            {data.input && `${copy.resultBirthLabel}${data.input.year}-${data.input.month}-${data.input.day}${data.input.timeUnknown ? copy.resultTimeUnknownSuffix : ` ${String(data.input.hour).padStart(2, "0")}:${String(data.input.minute).padStart(2, "0")}`}`}
            {transparency.pada != null && `${copy.resultPadaSuffix}${transparency.pada}`}
          </p>
...
function TodayMoonCard({ sukuyoIndex }: { sukuyoIndex: number }) {
  ...
  const sukuyoLine = today.todaySukuyo ? ` · ${today.todaySukuyo.nameKo}(${today.todaySukuyo.nameHan})` : "";

  return (
    <section className="mt-6 rounded-2xl border border-white/10 bg-white/[0.03] p-5 md:p-6" aria-labelledby="today-h">
      <h2 id="today-h" className="text-base font-bold text-slate-100">{copy.resultTodayMoonTitle}</h2>
      <p className="mt-2 break-keep text-sm leading-7 text-slate-200">
        {copy.resultTodayMoonText(today.todayNakshatra?.nameKo || "", sukuyoLine)}
      </p>
      {today.personal?.dayFortune && (
        <p className="mt-2 break-keep text-sm leading-7 text-slate-200">
          <span className="font-semibold text-blue-100">{copy.resultEasternTodayLabel}</span>
          {today.personal.dayFortune.headline} — {today.personal.dayFortune.advice}
        </p>
      )}
      {today.personal?.taraBala && (
        <p className="mt-1 break-keep text-sm leading-7 text-slate-200">
          <span className="font-semibold text-amber-100">{copy.resultIndianTodayLabel}</span>
