...
| Code Destiny Family 30일 | Moonlight Pass 카드 | `/points?plan=family`, 인페이지 CTA | 30일 300,000원, 모든 유료 기능 | 설명이 길고 고가 플랜 정보가 큼 | 결제 상태/월정석 잔량 조회 | 상세는 `/points`로 분리 |
...
| MBTI 동물 궁합 | 관상 & 심볼 컬렉션 | `openMbtiModal` | 1회 3,000원 | 모달이 홈 안에 존재 | hidden modal | 필요 시 모달 열기 |
...
| 운명의 알 / 다마고치 | 관상 & 심볼 컬렉션 | `/tadagochi`, `openDestinyEggPage` | 1회 3,000원 | 작은 게임성 기능이 묻힘 | 별도 route/asset | 심볼 컬렉션 내부 유지 |
| 포춘텔러 물고기 | 관상 & 심볼 컬렉션 | `/fortune-teller-fish.html` | 1회 3,000원 | 정적 HTML 진입이 혼재 | static HTML asset | 카드 preview 후 이동 |
...
| 네빌 명상 실습 | 명상 컬렉션 | `/neville-meditation.html` | 1회 3,000원 | 명상 기능이 타로 사이에 묻힘 | static HTML | 명상 chip으로 별도 묶음 |
...
| 십이지신 천운 타로 | 타로 컬렉션 | `openTarotYearFortuneModal`, `/tarot/year` | 1회 3,000원 | 카드 탐색 부담 | lazy script | 타로 chip 후 노출 |
...
| 화투점 | 신탁 & 점술 컬렉션 | `openHwatuModal`, `/oracle/hwatu` | 1회 3,000원 | 신탁 카드가 많음 | modal/script | 신탁 chip 후 노출 |
...
| 스톤헨지 룬 | 신탁 & 점술 컬렉션 | `/oracle/rune/` | 1회 5,000원 | 유료 신탁 탐색 부담 | route bundle | 신탁 컬렉션 내부 |
| 지오맨시 흙점 | 신탁 & 점술 컬렉션 | `/geomancy-oracle-v4.html` | 1회 5,000원 | 정적 HTML 혼재 | static HTML | preview 후 이동 |
| 핀란드 돼지 주석점 | 신탁 & 점술 컬렉션 | `/fortune/sikojen-povailu/`, `/oracle/sikojen-povailu` | 1회 3,000원 | 무료/유료 신탁 혼재 | route/static assets | 신탁 컬렉션 내부 |
...
| 자미두수 심화 | 코즈믹 & 별자리 컬렉션 | `navigateToZiweiChart`, `/ziwei/chart` | 해금 50,000원 | 고가 잠금 UI가 큼 | 권한 확인 | preview에서 안내 |
...
| 운명의 꽃 사주 | 운명의 꽃 & 해몽 컬렉션 | `openDestinyFlowerStudio`, `/flower/destiny` | 해금 20,000원 | 꽃 4종이 비슷해 혼동 | studio modal/engine | 꽃·해몽 chip 하위 |
| 점성술 꽃 | 운명의 꽃 & 해몽 컬렉션 | `openAstrologyFlowerStudio`, `/flower/astrology` | 해금 20,000원 | 이름 차이가 작음 | studio modal/engine | source badge 강화 |
| 자미두수 꽃 | 운명의 꽃 & 해몽 컬렉션 | `openJamidusuFlowerStudio`, `/flower/jamidusu` | 해금 20,000원 | 이름 차이가 작음 | studio modal/engine | source badge 강화 |
| 숙요 꽃 | 운명의 꽃 & 해몽 컬렉션 | `openSukuyoFlowerStudio`, `/flower/sukuyo` | 해금 20,000원 | 이름 차이가 작음 | studio modal/engine | source badge 강화 |
...
| 인생의 책 AI 상담 | VVIP 운명 서고 | `/life-book-ai`, `/saju/lifebook` | AI/PDF 상담 50,000원 | PDF 흐름이 길고 민감 | AI + PDF render | 별도 route에서 단계형 |
...
| 사주 RPG | 사주 결과 특수 리포트 | 인페이지 `skillTreeCard` | 유료 해금 30코인 | 홈 첫 화면에는 불필요 | 결과 계산 후 렌더 | 결과 후 노출 |
...
| 에너지 원정 리포트 | 사주 결과 특수 리포트 | 인페이지 `energyCoordCard` | 유료 해금 50코인 | 결과 화면 길이 증가 | 결과 DOM | 결과 dashboard 탭화 |
| 명리 헬스 리포트 | 사주 결과 특수 리포트 | 인페이지 `healthReport`, `/health-report/guide` | 유료 해금 50코인 | 민감 정보라 홈 노출 부적합 | 결과 DOM | 결과 후 고지와 함께 분리 |
