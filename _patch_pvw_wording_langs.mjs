...
const LIFEBOOK_END_MARKER = `cost:'🪙 490코인',ct:'paid',img:'/fuctionassets/lifebook.webp'},`;

for (const lang of LANGS) {
  const filePath = `public/${lang}/index.html`;
  try {
    let src = readFileSync(filePath, 'utf8');
    
    const startIdx = src.indexOf(OLD_D_START);
    if (startIdx === -1) {
      console.log(lang + ': START NOT FOUND, skipping');
      continue;
    }
    
    const lifebookEndIdx = src.indexOf(LIFEBOOK_END_MARKER);
    if (lifebookEndIdx === -1) {
      console.log(lang + ': LIFEBOOK END NOT FOUND, skipping');
      continue;
    }
    const lifebookEndFull = lifebookEndIdx + LIFEBOOK_END_MARKER.length;
    
    let result = src.slice(0, startIdx) + NEW_D_BEFORE_LOVESECRET + src.slice(lifebookEndFull);
    ...
    console.log(lang + ': OK (len=' + result.length + ')');
  } catch (e) {
