...
const OLD_D_END = `    openOlympusOracleModal:{cat:'올림푸스 · 별자리 신탁',title:'⚡ 올림푸스 신탁',tagline:'12 올림푸스 신이 당신의 별자리에 맞는 신탁을 내립니다',feats:['12 올림푸스 신 신탁 오라클 리딩','출생 별자리 & 재전생 수호신 해석','신탁 메시지 & 행운의 기운 분석','한 번 해금으로 평생 이용'],cost:'🔒 영구 해금 300코인',ct:'paid',img:'/fuctionassets/olympus.webp'}
  };`;
...
// openLoveSimulation은 유지할 수도 있으므로 찾아서 업데이트
const LOVE_SIM_OLD = `openLoveSimulation:{cat:'사주 · 연애 시뮬레이션',title:'💕 LOVE CODE',tagline:'사주 오행으로 찾는 운명의 소울메이트와 가상 데이트 코스 시뮬레이션',feats:['사주 8글자 기반 연애 페르소나 자동 생성','오행 궁합으로 찾는 최고 상성 상대','일간별 실시간 채팅 시뮬레이션','8가지 데이트 코스 & 크리티컬 히트 이벤트'],cost:'🪙 100코인',ct:'paid',img:'/fuctionassets/lovesimulation.webp'}`;
const LOVE_SIM_NEW = `openLoveSimulation:{cat:'사주 · 연애 시뮬레이션',title:'💕 LOVE CODE',tagline:'사주 오행으로 이 사람이 내 소울메이트인지 즉시 알 수 있습니다 — 가상 데이트로 찰떡 궁합 직접 확인',feats:['사주 8글자 기반 연애 페르소나 자동 생성','오행 궁합으로 찾는 최고 상성 & 최악 상대 비교','일간별 실시간 채팅으로 연애 케미 직접 체험','8가지 데이트 코스 & 크리티컬 히트 이벤트'],cost:'🪙 100코인',ct:'paid',img:'/fuctionassets/lovesimulation.webp'}`;

const ASTRO_OLD = `openAstroModal:{cat:'점성술 · 코즈믹 차트',title:'✨ 점성술 코즈믹 차트',tagline:'태양·달·상승궁 3각 에너지로 우주적 자아를 분석합니다',feats:['서양 점성술 전체 행성 배치 분석','태양궁·달궁·상승궁 정밀 해석','12하우스 영역별 운세 적용','트랜지트 & 프로그레션 해석 포함'],cost:'🔒 영구 해금 400코인',ct:'paid',img:'/fuctionassets/jumsung.webp'},`;
const ASTRO_NEW = `openAstroModal:{cat:'점성술 · 코즈믹 차트',title:'✨ 점성술 코즈믹 차트',tagline:'실제 행성 좌표 기반으로 계산한 정밀 차트 — 별자리 앱에서 절대 못 본 내 진짜 운명의 패턴',feats:['태양·달·상승궁 3각 에너지 완전 분석','12하우스 영역별 재물·사랑·직업 운세 파악','지금 행성 트랜지트가 내 인생에 미치는 영향 계산','400코인 단 한 번 결제로 평생 무제한 이용'],cost:'🔒 영구 해금 400코인',ct:'paid',img:'/fuctionassets/jumsung.webp'},`;

const ZIWEI_OLD = `openZiweiModal:{cat:'자미두수 · 12궁 명반',title:'🌌 자미두수(紫微)',tagline:'동양 최고 명리학 자미두수로 12궁 명반을 펼쳐드립니다',feats:['자미두수 12궁 명반 완전 분석','주성·보성·화기성 정밀 해석','명궁·재백궁·관록궁 운세 적용','한 번 해금으로 평생 이용'],cost:'🔒 영구 해금 400코인',ct:'paid',img:'/fuctionassets/jami.webp'},`;
const ZIWEI_NEW = `openZiweiModal:{cat:'자미두수 · 진로 적성 특화',title:'🌌 자미두수(紫微)',tagline:'중국 황제들이 세자 교육에 쓴 명리학 — 진로·직업 적성에 특화되어 있어 왜 이 일이 맞는지 즉시 납득됩니다',feats:['자미두수 12궁으로 내 진짜 직업 적성 완전 파악','관록궁·재백궁으로 어떤 분야에서 돈을 버는지 분석','숨겨진 재능과 성공 패턴 완전 해독','400코인 단 한 번 결제로 평생 무제한 이용'],cost:'🔒 영구 해금 400코인',ct:'paid',img:'/fuctionassets/jami.webp'},`;

const VEDIC_OLD = `navigateToVedic:{cat:'베다 점성술 · Jyotish',title:'🪐 베다 점성술',tagline:'5000년 인도 Jyotish 별자리로 업장과 운명의 패턴을 읽습니다',feats:['Jyotish 라시·낙샤트라 정밀 분석','다샤 기간별 운세 흐름 해석','카르마·업장 운명 패턴 해독','도시·나라 기반 로컬 차트 적용'],cost:'🔒 영구 해금 300코인',ct:'paid',img:'/fuctionassets/veda.webp'},`;
const VEDIC_NEW = `navigateToVedic:{cat:'베다 점성술 · Jyotish',title:'🪐 베다 점성술',tagline:'전생 업장이 지금 내 인생에 어떻게 작용하는지 — 5000년 인도 Jyotish 점성술이 카르마를 해독합니다',feats:['Jyotish 라시·낙샤트라로 내 업장 패턴 완전 해독','다샤 기간별로 기회의 시기 & 위기의 시기 미리 파악','왜 계속 같은 실수를 반복하는지 카르마 분석','300코인 단 한 번 결제로 평생 무제한 이용'],cost:'🔒 영구 해금 300코인',ct:'paid',img:'/fuctionassets/veda.webp'},`;

const OLYMPUS_OLD = `openOlympusOracleModal:{cat:'올림푸스 · 별자리 신탁',title:'⚡ 올림푸스 신탁',tagline:'12 올림푸스 신이 당신의 별자리에 맞는 신탁을 내립니다',feats:['12 올림푸스 신 신탁 오라클 리딩','출생 별자리 & 재전생 수호신 해석','신탁 메시지 & 행운의 기운 분석','한 번 해금으로 평생 이용'],cost:'🔒 영구 해금 300코인',ct:'paid',img:'/fuctionassets/olympus.webp'}`;
const OLYMPUS_NEW = `openOlympusOracleModal:{cat:'올림푸스 · 별자리 신탁',title:'⚡ 올림푸스 신탁',tagline:'12 올림푸스 신 중 누가 지금 내 편인지 — 내 수호신이 보내는 신탁 메시지를 직접 받으세요',feats:['12 올림푸스 신 중 내 수호신이 누구인지 즉시 파악','수호신이 지금 내 상황에 보내는 신탁 메시지 리딩','별자리 기반 행운·주의 에너지 완전 분석','300코인 단 한 번 결제로 평생 무제한 이용'],cost:'🔒 영구 해금 300코인',ct:'paid',img:'/fuctionassets/olympus.webp'}`;

// 먼저 D 객체 시작부터 openLoveSecretModal 끝까지 교체
const startMarker = `  var D={\n    openDestinyFlowerStudio:`;
const endMarker = `openLoveSecretModal:{cat:'프리미엄 · 연애 전략서',title:'💕 연애 비책',tagline:'운명이 설계한 사랑의 지도 — 사주 명리학자가 쓴 10가지 연애 전략',feats:['일간·십성·신살·12운성 종합 분석','도화살·홍염살 매력 해독','배우자궁(일지) 이상형 프로파일링','PDF 저장 가능한 프리미엄 리포트'],cost:'🪙 290코인',ct:'paid',img:'/fuctionassets/lovebible.webp'},`;
...
let result = src.slice(0, startIdx) + NEW_D + src.slice(endOfEndMarker);

// 나머지 항목들 교체
result = result.replace(LOVE_SIM_OLD, LOVE_SIM_NEW);
result = result.replace(ASTRO_OLD, ASTRO_NEW);
result = result.replace(ZIWEI_OLD, ZIWEI_NEW);
result = result.replace(VEDIC_OLD, VEDIC_NEW);
result = result.replace(OLYMPUS_OLD, OLYMPUS_NEW);

writeFileSync(filePath, result, 'utf8');
console.log('Done! Lines replaced successfully.');
