...
function pickGeminiModels() {
  const models = [
    process.env.GEOMANCY_GEMINI_MODEL,
...
async function callGeminiOracle(prompt) {
  ...
  const timeoutMs = Math.max(15000, Number(process.env.GEOMANCY_PROVIDER_TIMEOUT_MS || 45000));
  let lastError = "";
...
router.post("/geomancy", async (req, res) => {
  try {
    const body = req.body || {};
    const question = limitText(body?.question, 500);
    if (question.length < 2) {
      return res.status(400).json({ ok: false, message: "질문을 2자 이상 입력해 주세요." });
    }
    ...
    const ai = await callGeminiOracle(prompt);
    ...
    console.error("[oracle/geomancy]", error);
    return res.status(500).json({ ok: false, message: "지오맨시 신탁 생성 중 오류가 발생했습니다." });
  }
});
