...
function pickGeminiModels() {
  const models = [
    process.env.YOGA_GURU_GEMINI_MODEL,
...
async function callGemini(prompt) {
  ...
  if (!keys.length || !models.length) {
    return { ok: false, message: "Gemini key/model is not configured." };
  }

  const timeoutMs = Math.max(10000, Number(process.env.YOGA_GURU_PROVIDER_TIMEOUT_MS || 55000));
  ...
  return { ok: false, message: "Gemini request failed." };
}

router.post("/", async (req, res) => {
  const systemPrompt = clean(req.body?.systemPrompt);
  const userPrompt = clean(req.body?.userPrompt);
  if (!systemPrompt || !userPrompt) {
    return res.status(400).json({ ok: false, message: "systemPrompt와 userPrompt가 필요합니다." });
  }
