...
function createHealingRisingReading({ drawnCards }) {
  ...
  function coachLabel(card) {
    ...
    const engName = card.name || "";
    return `${card.nameKr}(${engName}) - ${direction}`;
  }

  function getHealingContext(card) {
    const healingData = {
      M01: {
        core: "무한한 가능성, 내면의 의지, 변화를 위한 자원",
        psychology: "당신은 이미 회복에 필요한 모든 자원과 능력을 내면에 갖추고 있습니다. 지금 필요한 것은 새로운 무언가를 찾는 것이 아니라, 이미 당신 안에 있는 그 힘을 다시 신뢰하고 깨우는 것입니다.",
        practice: "내가 지금 바로 사용할 수 있는 나의 장점이나 자원 3가지 적어보기"
      },
      M03: {
        core: "풍요로움, 무조건적인 수용, 스스로를 돌보는 모성적 에너지",
        psychology: "회복은 자신을 채찍질하는 것이 아니라, 있는 그대로의 감정을 따뜻하게 안아주는 것에서 시작됩니다. 당신의 몸과 마음이 보내는 신호에 귀를 기울이고 자신에게 가장 너그러운 사람이 되어주세요.",
        practice: "나를 위해 정성스럽게 준비한 따뜻한 차 한 잔 마시기"
      },
      W12: {
        core: "강렬한 열정과 추진력, 하지만 조절되지 않은 불의 에너지",
        psychology: card.orientation === "reversed" 
          ? "회복을 방해하는 가장 큰 장애물은 '빨리 나아져야 한다'는 조급함일 수 있습니다. 지금은 무리하게 속도를 내기보다 잠시 멈춰 서서 에너지가 밖으로 새나가지 않도록 갈무리해야 할 때입니다."
          : "당신의 가슴 속에 다시 열정이 피어오르고 있습니다. 이 에너지를 한꺼번에 쏟아붓기보다, 지속 가능한 방향으로 조금씩 나누어 사용하는 지혜가 필요합니다.",
        practice: card.orientation === "reversed"
          ? "오늘 하기로 했던 일 중 가장 급하지 않은 것 하나를 내일로 미루기"
          : "오늘 가장 설레는 일 하나를 정해서 15분만 몰입해보기"
      },
      W11: {
        core: "순수한 호기심, 새로운 소식, 작고 가벼운 시작",
        psychology: "회복의 다음 단계는 거창한 성공이 아니라, 아주 작고 신선한 시도에서 시작됩니다. 마치 어린아이가 세상을 탐색하듯, 가벼운 마음으로 새로운 배움이나 작은 행동을 시작해보세요.",
        practice: "평소 가보고 싶었던 장소에 가거나, 새로운 짧은 글 하나 읽어보기"
      },
      // Fallback for other cards
    };

    const data = healingData[card.cardId];
    if (data) return data;

    // Generic fallback based on keywords/interpretation
    const kw = Array.isArray(card.keywords) ? card.keywords.join(", ") : "";
    return {
      core: kw || "내면의 균형과 성찰",
      psychology: "이 에너지는 당신의 회복 과정에서 중요한 이정표가 됩니다. 현재의 감정을 부정하지 말고, 이 상징이 당신의 무의식에 던지는 질문에 가만히 머물러 보세요.",
      practice: "오늘의 감정을 한 문장으로 일기에 기록해보기"
    };
  }

  function formatCardSection(card) {
    if (!card) return "";
    const ctx = getHealingContext(card);
    return `
- 뽑힌 카드:
  ${coachLabel(card)}
...
- 이 위치에서의 해석:
  ${card.interpretation || "이 포지션은 현재 당신의 상태를 비추는 중요한 거울입니다."}
...
- 오늘의 작은 실천:
  ...
  const opening = `안녕하세요. 오늘 우리는 당신의 지친 마음이 어디에서 기원했는지 살피고, 다시 태양처럼 밝은 에너지를 되찾기 위한 여정을 함께 떠나보려 합니다. 이 리딩은 단순히 앞날을 예언하는 것이 아니라, 당신의 내면을 비추는 거울입니다. 카드가 전하는 상징들을 통해 당신의 마음을 있는 그대로 수용하고, 회복을 위한 구체적인 지도를 그려보는 시간이 되길 바랍니다. 당신은 이미 회복할 힘을 가지고 있습니다.`;

  const hiddenTruth = formatCardSection(c1);
  const embracePain = formatCardSection(c2);
  const silverLining = formatCardSection(c3);
  const stepForward = formatCardSection(c4);

  const actionPlan = [
    "오늘 해야 할 일 중 가장 비중이 크거나 부담스러운 것을 하나만 골라 아주 작게 쪼개어 정리해보세요. 막연한 압박감을 구체적인 행동 계획으로 바꾸는 것만으로도 불안이 크게 줄어듭니다.",
    "몸의 감각을 깨우는 작은 루틴을 실천하세요. 따뜻한 물로 샤워를 하거나, 10분간 가벼운 스트레칭을 하며 내 몸의 신호에 집중해보세요. 몸이 이완되어야 마음의 회복도 속도를 낼 수 있습니다.",
    "중요하거나 충동적인 결정이 있다면 오늘 하루만 유예해보세요. '내일 다시 생각하자'라고 스스로에게 허용하는 것만으로도 조급함에서 벗어나 더 현명한 방향을 찾을 수 있습니다.",
    "새롭게 해보고 싶은 작은 시도나 배움이 있다면 딱 10분만 시간을 내어 찾아보세요. 거창한 목표가 아니어도 좋습니다. 당신의 호기심을 자극하는 아주 작은 불씨를 발견하는 것이 중요합니다."
  ];
...
오늘 네 장의 카드는 당신을 위한 하나의 회복 서사를 들려주고 있습니다.
${c1.nameKr}의 에너지를 통해 내 안의 잠재된 자원을 발견하는 것으로 시작하여, ${c2.nameKr}처럼 자신을 부드럽게 돌보는 시간을 가져보세요. ${c3.nameKr}가 제안하는 조급함을 내려놓는 '방향 조정'의 시간을 거쳐, 마지막으로 ${c4.nameKr}와 함께 작고 신선한 시도로 다시 삶의 생동감을 되찾는 과정입니다.

당신은 이미 회복할 힘을 충분히 가지고 있습니다. 다만 지금 필요한 것은 더 세게 밀어붙이는 것이 아니라, 자신을 아끼며 천천히, 하지만 확실하게 작은 한 걸음을 내딛는 것입니다.
  `.trim();
